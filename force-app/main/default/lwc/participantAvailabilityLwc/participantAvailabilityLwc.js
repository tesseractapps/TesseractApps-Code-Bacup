...
@track selectedRole;            // Stores selected role from combobox
@track roleValue;              // Optional: stores selected role for another purpose
@track roletable;              // Stores fetched staff records
@track showRolesTable = false; // Controls table visibility
@track roleOptions = []; 
@track staffList;     
...
handlesavebutton(event) {
    ...
    if(!this.street || this.street == '' || this.street == null || this.street == undefined || this.street == '--None--'){
        this.showToast('Error', 'Please provide Address', 'error');
        return;
    }

    if(!this.city || this.city == '' || this.city == null || this.city == undefined || this.city == '--None--'){
        this.showToast('Error', 'Please provide Address', 'error');
        return;
    }

    if(!this.country || this.country == '' || this.country == null || this.country == undefined || this.country == '--None--'){
        this.showToast('Error', 'Please provide Address', 'error');
        return;
    }

    if(!this.province || this.province == '' || this.province == null || this.province == undefined || this.province == '--None--'){
        this.showToast('Error', 'Please provide Address', 'error');
        return;
    }

    if(!this.postalCode || this.postalCode == '' || this.postalCode == null || this.postalCode == undefined || this.postalCode == '--None--'){
        this.showToast('Error', 'Please provide Address', 'error');
        return;
    }

    if(!this.serviceTypevalue || this.serviceTypevalue == '' || this.serviceTypevalue == null || this.serviceTypevalue == undefined || this.serviceTypevalue == '--None--'){
        this.showToast('Error', 'Please Select Service Type', 'error');
        return;
    }

    if(!this.selectedcatlogId || this.selectedcatlogId == '' || this.selectedcatlogId == null || this.selectedcatlogId == undefined || this.selectedcatlogId == '--None--'){
...
resetFields() {
    ...
    handleEdit(event) {
        ...
        if (this.selectedavailability.length > 0) {
            ...
            if(this.typeofAddress == 'Facility'){
                this.facilityAddressCheckbox = true;
            } else if(this.typeofAddress == 'Participant'){
                this.participantAddressCheckBox = true;
            } else if(this.typeofAddress == 'Add New'){
            ...
            if(this.roleValue != null ){
                ...
                this.staffList = this.roleRows.length > 0;
