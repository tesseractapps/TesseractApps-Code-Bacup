...
export default class ParticipantAvailabilityLwc extends LightningElement {
  ...
  /* connectedCallback() {
        ...
        getFacilityData()
            .then(response => {
                console.log("✅ Facility Data fetched:", JSON.stringify(response));
                ...
                console.log('🏢 Organisation Shift Times:', JSON.stringify(this.organisationShiftTimes));
                ...
                if (this.facilityOptions.length > 0) {
                    ...
                    console.log("📌 Default Facility Selected:", this.SelectedComboBoxFacility);
                }

                // Fetch shifts for facility
                console.log("📌 Fetching Shift Data for facilityId:", this.facilityValue);
                this.fetchShiftData(this.facilityValue)
                    .then(shifts => {
                        console.log("✅ Shift Data fetched:", JSON.stringify(shifts));
                    })
                    .catch(err => {
                        console.error("❌ Error fetching shifts in connectedCallback:", err);
                    });
    ...
    fetchShiftData(facilityId) {
        console.log("📌 fetchShiftData called with facilityId:", facilityId);

        try {
            const result = getShiftsTypeByFacility({ facilityId });

            console.log("✅ Apex call executed, raw result (Promise):", result);

            // If you're returning this directly, the caller must use `.then` or `await`
            return result
            .then((data) => {
            ...
            .catch((error) => {
                console.error("❌ Error inside promise chain:", error);
                this.shiftNameOptions = [];
                throw error;
            });

        } catch (error) {
            console.error("❌ Synchronous error fetching shifts:", error);
            this.shiftNameOptions = [];
 ...
 async handleShiftNamechange(event) {
  ...
  dispalyAmPMFormat() {
    ...
    if (this.AddShiftStartTimeAMPM) {
      ...
      this.startTimeAMPM = period == "AM" ? true : false;
    ...
    if (this.AddShiftEndTimeAMPM) {
      ...
      this.endTimeAMPM = period == "AM" ? true : false;
 ...
 */
  ...
  async getserviceTypes() {
    ...
    getServiceType({ clientId: this.clientId ,shiftName:this.shiftNameValue})
      .then((result) => {
        ...
        if (Array.isArray(result.catalogueData)) {
          this.serviceEditGroupName = result.catalogueData.map((item) => {
            return {
              ...
              //  (this.otherThanNdis == true || (this.otherThanNdis == false &&  result.catalogueData[0].Name=='Miscellaneous' ))? result.amount ||0 : rec[stateField] || 0,
          ...
          this.CatlogTabel = true;
           if( this.isUpdateMode == false){
                  this.selectedcatlogId = (isNdisService == false || (isNdisService == true &&   result.catalogueData[0].Name.includes("Miscellaneous") ))?this.serviceEditGroupName[0].Id:'';
           }
  ...
  async handleEdit(event) {
    ...
    console.log(
      "📌 Using First Availability Record:",
      JSON.stringify(firstAvailability)
    );
    ...
    if (this.roleValue != null) {
      ...
      getStaffOptions({ role: this.roleValue, participantId: this.clientId })
        .then((result) => {
          ...
          console.log(
            `🧑‍🤝‍🧑 Found ${JSON.stringify(priorityStaffs)} priority staff records`
          );
