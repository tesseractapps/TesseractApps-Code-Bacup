...
this.updateStepVisibleRows();
 // Render only first step's rows
  ...
  grantAccess() {
    ...
    const selectedParticipants = this.participants.filter(p => p.isSelected);
    if (selectedParticipants.length === 0) {
        this.showToast('Error', 'Please select at least one participant.', 'error');
        return;
    }
    ...
    console.log('📌 Selected Participants:', participantIds);

    // ✅ Call the new bulk Apex method
    grantAccessToParticipants({ participantIds, formId })
        .then(() => {
            this.showToast('Success', 'Access granted to selected participants.', 'success');
            this.showAssignModal = false;
            this.selectedFormId = null;

            // Reset selections
            this.participants = this.participants.map(p => ({ ...p, isSelected: false }));
            this.filteredParticipants = [...this.participants];
            this.paginationHelper();
    ...
    computeReadonlyValue(cell, isViewMode) {
        ...
        if (cell.isTextField || cell.isTextArea || cell.isAlphaNumeric || cell.isOnlyAlphabets || cell.isNumberField || cell.isTimeField || cell.isDateField) {
...
handleAssignClick(event) {
    ...
    this.selectedFormTitle = selectedForm ? selectedForm.Name__c : 'Form';

    // ✅ Mark participants who already have access to this form
    this.participants = this.participants.map(participant => {
        const isSelected = (participant.grantedForms || []).includes(formId);
        return {
            ...participant,
            isSelected
        };
...
handleParticipantCheckboxToggle(event) {
    ...
    const allSelected = this.participants.length > 0 && this.participants.every(p => p.isSelected);
    this.isAllParticipantsSelected = allSelected;
...
handleSelectAllParticipants(event) {
    ...
    // Also update the filtered view (pagination)
    this.filteredParticipants = [...this.participants];
    this.paginationHelper();
}
...
paginateCreatedForms() {
    ...
    this.totalCreatedFormPages = Math.ceil(this.totalCreatedFormRecords / this.createdFormPageSize);
    ...
    this.disableCreatedLastPage = this.createdFormPageNumber === this.totalCreatedFormPages;
}
            ...
            cells: row.cells.map(cell => {
                if (cell.id === cellId) {
                    ...
                    cell.radioOptionsProcessed = (cell.radioOptionsProcessed || []).map(option => {
                        const isSelected = option.optionLabel === cell.selectedRadioOption;
