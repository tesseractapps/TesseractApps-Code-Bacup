...
export default class FormRender extends LightningElement {
  ...
  paginationHelper() {
    ...
    // Calculate total pages
    this.totalPagestotalParticipants = Math.ceil(
      this.participantData.length / this.pageSize
    );
    this.totalParticipants = this.participantData.length;

    // Adjust page number
    if (this.pageNumber < 1) {
      this.pageNumber = 1;
    } else if (this.pageNumber > this.totalPagestotalParticipants) {
    ...
    let startIndex = (this.pageNumber - 1) * this.pageSize;
    let endIndex = this.pageNumber * this.pageSize;
    this.recordsToDisplay = this.participantData.slice(startIndex, endIndex);

    // Update button states
    this.bDisableFirst = this.pageNumber === 1;
    this.bDisableLast = this.pageNumber === this.totalPagestotalParticipants;
  }
  ...
  grantAccess() {
    ...
    if (!this.selectedFormId) {
      this.showToast(
        "Error",
        "No form selected. Please choose a form to assign.",
        "error"
      );
      return;
    }
    ...
    grantAccessToParticipants({ participantIds, formId })
      .then(() => {
        this.showToast(
          "Success",
          "Access granted to selected participants.",
          "success"
        );
  ...
  prepareViewTable(formJson) {
    ...
    formJson.forEach((row) => {
      ...
      row.cells.forEach((cell) => {
        // ❌ Skip empty cells
        if (cell?.field && cell.field.label && cell.field.label.trim() !== "") {
          ...
          tableData.push({
            ...
            isImageFile: isImage,
            isRegularField:
              !isUploadFile && !cell.isCheckboxField && !isRichText,
            downloadUrl,
  ...
  handleParticipantCheckboxToggle(event) {
    ...
    // 🔁 Update BOTH participantData and participants
    this.participantData = this.participantData.map((p) => ({
      ...p,
      isSelected: p.Id === participantId ? isChecked : p.isSelected
    }));

    this.participants = [...this.participantData];

    // 🔁 Recalculate "Select All" state
    this.isAllParticipantsSelected =
      this.participantData.length > 0 &&
      this.participantData.every((p) => p.isSelected);
