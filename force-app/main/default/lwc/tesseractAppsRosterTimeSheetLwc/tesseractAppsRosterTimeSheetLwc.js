...
@track  apiFieldMap = {
    ...
    renderedCallback() {
        console.log('📍 Entered renderedCallback()');
        console.log('⬇️ Starting to load PDF libraries...');

        Promise.all([
            loadScript(this, jsPDFLib).then(() => console.log('✅ jsPDF library loaded.')),
            loadScript(this, autoTableLib).then(() => console.log('✅ autoTable library loaded.'))
        ])
        .then(() => {
            console.log('🧠 Processing loaded libraries...');

            const jsPDF = window.jspdf?.jsPDF;
            const autoTable = window.jspdf?.autoTable || window.autoTable;

            if (jsPDF) {
                console.log('✅ jsPDF instance is available.');
            } else {
                console.warn('⚠️ jsPDF instance not found.');
            }

            if (autoTable) {
                console.log('✅ autoTable instance is available.');
            } else {
                console.warn('⚠️ autoTable instance not found.');
            }

            if (jsPDF && autoTable) {
                jsPDF.API.autoTable = autoTable;
                console.log('🔌 PDF libraries are successfully initialized and ready to use.');
            } else {
                console.warn('⚠️ Libraries loaded but missing components. PDF generation may fail.');
            }
        })
        .catch(error => {
    ...
    handleDownloadPDF() {
        try {
            ...
            const data = this.selectedStaffShifts.map(shift => [
                shift.shiftDate,
                shift.shiftType,
  ...
  convertTo24HourFormat(timeStr) {
    ...
    handleBulksubmit(event){
        ...
        .then(response => {
            const toastEvent = new ShowToastEvent({
                title: "Success",
                message: "Timesheets have been approved successfully.",
                variant: "success"
            });
            this.dispatchEvent(toastEvent);
            this.loadShiftData();
    ...
    handleDownload() {
        const header = [
            'Staff',
            ...this.weekDays.map(day => day.dayName),
            'Total (hr)', 
            'Mileage (km)', 
            'Expense', 
            'Sleepover (hr)', 
        ...
        // 👉 Calculate centered position for facility label
        ...
        this.processedStaff.forEach(staff => {
            ...
            row.push(
                staff.totalDuration || '0',
                staff.totalMileageOthers || '0',
                `"${staff.totalMileageAmount ? `$${staff.totalMileageAmount}` : '$0.00'}"`,
                staff.totalSleepoverHours || '0',
                staff.totalvariance || '0'
            );
