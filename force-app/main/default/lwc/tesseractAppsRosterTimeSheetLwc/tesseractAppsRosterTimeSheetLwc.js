...
//import getTotalBreakHours from "@salesforce/apex/RosterTimeSheetController.getTotalBreakHours";
let jsPDF;
...
export default class Timesheet extends LightningElement {
    ...
    .then((shifts) => {
      const filteredShifts = (shifts || [])
        ...
        .map((shift) => {
          ...
          //   shift.Break_Timings__r.forEach((breakRec) => {
          ...
          //   0,
          //   parseFloat(durationCalculated) - parseFloat(totalBreakHours)
          // ).toFixed(2);
          if (shift.Break_Timings__r && shift.Break_Timings__r.length > 0) {
            console.log(`🔎 Found ${shift.Break_Timings__r.length} break records for shift: ${shift.Id}`);
            shift.Break_Timings__r.forEach((breakRec, index) => {
              const breakTime = breakRec.Break_Time__c;
              console.log(`⏱ Break #${index + 1} - Raw Break Time: ${breakTime}`);
              ...
              if (breakTime && breakTime.includes(":")) {
                ...
                if (!isNaN(hours) && !isNaN(minutes)) {
                  ...
                  console.log(`✅ Parsed Break #${index + 1}: ${hours} hours, ${minutes} minutes (${breakMinutes} mins)`);
                  console.log(`➕ Running Total Break Minutes in handleCheckClick : ${totalBreakMinutes}`);
                } else {
                  console.warn(`⚠️ Failed to parse hours/minutes for Break #${index + 1}:`, breakTime);
                }
              } else {
                console.warn(`⚠️ Invalid break time format for Break #${index + 1}:`, breakTime);
              }
            });
          } else {
          ...
          const totalBreakHours = (totalBreakMinutes / 60).toFixed(2);
          console.log(`⏳ Total Break Minutes: ${totalBreakMinutes}, Converted to Hours: ${totalBreakHours}`);

          const durationRaw = shift.Actual_Duration__c;
          const durationCalculated = isNaN(durationRaw) ? 0 : parseFloat(durationRaw);
          console.log(`📘 Raw Actual Duration: ${durationRaw}, Parsed: ${durationCalculated}`);

          const effectiveDuration = Math.max(
            0,
            durationCalculated - parseFloat(totalBreakHours)
          ).toFixed(2);
          console.log(`🧮 Effective Duration (after break deduction): ${effectiveDuration}`);
          ...
          return {
            ...
            totalDuration:
              isNaN(parseFloat(effectiveDuration)) || parseFloat(effectiveDuration) === 0
              ...
              shift.Break_Timings__r.forEach((breakRec, index) => {
                ...
                if (breakTime && breakTime.includes(":")) {
                  ...
                  if (!isNaN(hours) && !isNaN(minutes)) {
                    ...
                    console.log(`✅ Parsed Break #${index + 1}: ${hours} hours, ${minutes} minutes (${breakMinutes} mins)`);
                    console.log(`➕ Running Total Break Minutes in handleMismatchClick : ${totalBreakMinutes}`);
                  } else {
                    console.warn(`⚠️ Failed to parse hours/minutes for Break #${index + 1}:`, breakTime);
                  }
                } else {
              ...
              shift.Break_Timings__r.forEach((breakRec, index) => {
                ...
                if (breakTime && breakTime.includes(":")) {
                  ...
                  if (!isNaN(hours) && !isNaN(minutes)) {
                    ...
                    console.log(`✅ Parsed Break #${index + 1}: ${hours} hours, ${minutes} minutes (${breakMinutes} mins)`);
                    console.log(`➕ Running Total Break Minutes in handleCheckboxChange: ${totalBreakMinutes}`);
                  } else {
                    console.warn(`⚠️ Failed to parse hours/minutes for Break #${index + 1}:`, breakTime);
                  }
                } else {
  ...
  handleEditClick(event) {
    ...
    getAddShiftDataById({ shiftId: this.shiftID })
      ...
      console.log('🕵️ breakTimingList BEFORE check:', result?.breakTimingList);
       if (result.breakTimingList) {
            console.log("✅ Received breakTimingList from Apex:", result.breakTimingList);
  ...
  async handlesubmit(event) {
    ...
    console.log("➡️ Shift ID List in handlesubmit :", JSON.stringify(Array.from(totalShiftIdlist)));
    try {
  ...
  handleShiftError(event) {
    ...
    console.error("⚠️ Shift Edit Error:", JSON.stringify(event.detail, null, 2));

    // Show a user-friendly error message (optional)
    const toastEvent = new ShowToastEvent({
        title: "Error",
        message: event.detail.message || "An error occurred while saving the shift.",
        variant: "error",
    });
    this.dispatchEvent(toastEvent);
}
                ...
                shift.Break_Timings__r.forEach((breakRec, index) => {
                  ...
                  if (breakTime && breakTime.includes(":")) {
                    ...
                    if (!isNaN(hours) && !isNaN(minutes)) {
                      ...
                      console.log(`✅ Parsed Break #${index + 1}: ${hours} hours, ${minutes} minutes (${breakMinutes} mins)`);
                      console.log(`➕ Running Total Break Minutes in handleExport: ${totalBreakMinutes}`);
                    } else {
                      console.warn(`⚠️ Failed to parse hours/minutes for Break #${index + 1}:`, breakTime);
                    }
                  } else {
  ...
  handleBreakCheckboxChange(event) {
        ...
        this.selectAllChecked = this.breakTimeRecords.every(record => record.selected);
    }
