...
export default class TesseractAppsAccountingModuleLWC extends LightningElement {
                    ...
                    subCategories: category.subCategories.map(subCategory => ({
                        ...
                        ledgers: subCategory.ledgers.map(ledger => ({
                            ...
                            ? ledger.ledgerItems.map(item => ({
                                ...item,
                                id: item.Id,
                                amount:  this.formatAmount(item.amount || 0) // Set to 0 if null or undefined
                            }))
                            : []
    ...
    // @wire(getRelatedRecords, { itemId: '$itemId' })
    // wiredEntries(result) {
    ...
    //         this.entryData = data; // Store data in entryData
    ...
    //                 if (entry.Entity_Profile__r.Card_Type__c === 'Customer') {
    ...
    //                 } else if (entry.Entity_Profile__r.Card_Type__c === 'Supplier') {
    //                     // Show Name for Supplier
    //                     entityName = entry.Entity_Profile__r.Name ?? '';
    //                 } else {
    ...
    //                 // entityName: entry.Entity_Profile__r ?.Card_Type__c ?? '',
    //                 // entityName: entry.Entity_Profile__r?.Card_Type__c === 'Customer' ? `${entry.Entity_Profile__r?.First_Name__c ?? ''} ${entry.Entity_Profile__r?.Last_Name__c ?? ''}`
    //                 //    : entry.Entity_Profile__r?.Card_Type__c === 'Supplier'? entry.Entity_Profile__r?.Name ?? '': '',
    //                 entityName: entityName, 
    //                 companyName: entry.Company__r ?.Company_Name__c ?? '', 
    //                 orgName: entry.Company__r?.Organisation__r?.Name?? '',
    //                 createdDate: entry.CreatedDate ? new Date(entry.CreatedDate).toLocaleDateString() : '', 
    //                 // tax: entry.Tax__c ?? '', 
...
wiredEntries(result) {
    ...
    // handleClickCreateCompany(){
    //     console.log('Create Compamy');
