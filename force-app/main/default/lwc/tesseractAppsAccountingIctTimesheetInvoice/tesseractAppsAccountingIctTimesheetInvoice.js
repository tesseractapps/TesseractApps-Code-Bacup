...
export default class TesseractAppsAccountingIctTimesheetInvoice extends LightningElement {
   ...
   handleInvoiceData() {
    ...
    checkForOverlap() {
        if (this.invoiceStartDate && this.invoiceEndDate && this.companyId) {
            ...
            .catch(error => {
                ...
                this.showToast('Error', 'Could not validate date range.', 'error');
            });
        }
    }
    ...
    //         this.showToast('Error', 'Invoice Already created.', 'error');
    //         return;
    //     }
    //     this.invoiceRowList = this.invoiceRowList.map(sales => {
    ...
    async  saveMultipleAccounts(){
        ...
        try {
            if (!this.companyId ) {
                this.showToast('Error', 'Company is not created for this facility .', 'error');
    ...
    generatePDF() {
     ...
     console.log(' this.ictInvoiceList records list'+JSON.stringify( this.ictInvoiceList));
         ...
         if (availableSpace > tableHeight + 40) { // Check if there's enough space to fit both the table and additional text
             ...
             margin: { left: 10, right: 10 },
              ...
              // const approvedByText = "Approved By: " + rec.Approver_Name__c;
               let name = '';
                if (rec.Approver_Name__c !== undefined && rec.Approver_Name__c !== null) {
                    name = rec.Approver_Name__c;
                }
     ...
     handleRowAction(event) {
      ...
      console.log('this.invoiceIdforEmail '+this.invoiceIdforEmail);
