...
export default class TesseractAppsRosterInvoices extends LightningElement {
   ...
   @track isInvoiceflag=true;
    ...
    @wire(serviceSupportList, { clientIds: '$partcipantIdList', startDateStr: '$startDate', endDateStr: '$endDate', orgid: '$orgid',name:'$searchKey' })
    wiredServiceList(result) {
        this.servicesfromparent=[];
        this.isShowSpinner=true;
        this.wiredResult = result;
        if (result.data) {
            this.servicesfromparent = result.data;
    ...
    wiredBulkServices(result) {
        ...
        if (result.data) {
          //  this.bulkServices = result.data;
            this.recordsBulk = result.data.map(invoice => {
                let fileReference = '';
   ...
   handleSearchInput(event) {
    ...
    servicePAginationHelper(serviceList){
        ...
        this.records = serviceList.map((item,index) => ({
            ...item,
            slno:index +1,
            participantFullName: item.Client__r.First_Name__c+' '+ item.Client__r.Last_Name__c,
