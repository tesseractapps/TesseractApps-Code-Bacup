   ...
   handleSearchInput(event) {
    ...
    servicePAginationHelper(serviceList) {
        ...
        this.records = serviceList.map((item, index) => ({
            ...
            unitprice: item.Edited_Unit_Price__c != null && item.Edited_Unit_Price__c !== undefined
                    ? parseFloat(item.Edited_Unit_Price__c).toFixed(2)
                    : (item.Unit_Price__c != null && item.Unit_Price__c !== undefined
                        ? parseFloat(item.Unit_Price__c).toFixed(2)
                        : '0.00'),
