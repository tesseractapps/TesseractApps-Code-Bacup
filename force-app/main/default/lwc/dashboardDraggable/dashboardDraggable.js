...
export default class DashboardDraggable extends LightningElement {
    ...
    'Roster Manager': ['shift', 'signin', 'task', 'leave', 'participant', 'incident', 'performance', 'hr', 'fund', 'staff-status'],
    ...
    'ICT Admin': ['staff-status', 'task', 'hr', 'leave','payroll', 'icttimesheet'],
...
fetchUserAccessDetails() {
    getstaffId({ userId: this.userId })
        .then(data => {
            ...
            const userType = data.User_Type__c;
            const allowedWidgets = this.widgetAccessByRole[userType] || [];

            // Get widget types already in view
            const existingTypes = new Set(this.widgets.map(w => w.type));

            // Filter only widget types NOT already added
            ...
            console.log('Available widget types (excluding duplicates):', this.availableWidgetTypes);
        })
        .catch(error => {
    ...
    // async loadWidgets() {
    //     try {
    //         const layoutJson = await getLayout({ roleName: this.roleKey });
    //         this.widgets = layoutJson ? JSON.parse(layoutJson) : this.getDefaultWidgets();
...
findAvailablePosition(width, height) {
    ...
    handleGrandChildEvent(event) {
        console.log('Grand child event received:', event.detail);
    }
