...
export default class DashboardDraggable extends LightningElement {
    ...
    widgetAccessByRole = {
    'NDIS Org Admin': ['shift', 'signin', 'task', 'leave', 'participant', 'incident', 'performance', 'hr', 'fund', 'staff-status', 'rejectedShifts'],
    'Facility Admin': ['shift', 'signin', 'task', 'leave', 'participant', 'incident', 'performance', 'hr', 'fund', 'staff-status'],
    'Roster Manager': ['shift', 'signin', 'task', 'leave', 'participant', 'incident', 'performance', 'hr', 'fund'],
    'NDIS Staff': ['signin', 'task', 'leave','staffDashboard', 'participantSchedule', 'incident', 'performance'],
    'Payroll Admin': ['payroll'],
    'HR Admin': ['leave', 'incident', 'hr', 'performance','staffDashboard'],
        ...
        try {
            ...
            if (userType === 'NDIS Org Admin' || userType === 'ICT Admin') {
                ...
                try {
                    ...
                    console.log('getFacilityCurrentUser facility:', JSON.stringify(currentFacilities));
...
validateCache(){
    if (this.finalListFacilities.length === 0) {
        ...
        console.log('No facilities found. Cleared facilityValue and facilityLabel.');

        localStorage.removeItem('defaultFacilityId');
        localStorage.removeItem('defaultFacilityLabel');

    } else if (this.finalListFacilities.length > 0 && this.facilityValue === null) {
        ...
        localStorage.setItem('defaultFacilityId', this.facilityValue);
        localStorage.setItem('defaultFacilityLabel', this.facilityLabel);
    } else if (!this.finalListFacilities.find(fac => fac.value === this.facilityValue)) {
        console.log('ELSE IF 2 CONDITIONS');
        // facilityValue is not in the available list – fallback to first one
        this.facilityValue = this.finalListFacilities[0].value;
        this.facilityLabel = this.finalListFacilities[0].label;

        console.log('facilityValue not in finalListFacilities. Set to first item:', this.facilityValue);
...
refreshAllWidgets() {
    console.log('Selected Facility ID refreshAllWidgets1>>', this.facilityValue);
    const widgets = this.template.querySelectorAll('c-dynamic-widget');
    console.log('Selected Facility ID refreshAllWidgets2>>', this.facilityValue);
    widgets.forEach(widget => {
        if (typeof widget.refresh === 'function') {
            console.log('Selected Facility ID refreshAllWidgets3>>', this.facilityValue);
...
fetchUserAccessDetails() {
    ...
    .then(data => {
        console.log('Raw data from Apex:', JSON.stringify(data)); // 🔍 add this line
        ...
        if (userType === 'Facility Admin' && typeofUser === 'ICT') {
            allowedWidgets = ['staff-status', 'task', 'hr', 'leave', 'icttimesheet'];
        } else {
        ...
        this.availableWidgetTypes = allowedWidgets
            .filter(type => !existingTypes.has(type) && this.allWidgetTypes[type]) // <- fixes the issue
            .map(key => ({
                label: this.allWidgetTypes[key],
                value: key
            }))
    ...
    async initDashboard() {
        try {
            ...
            this.showToast('Error', 'Failed to initialize dashboard: ' + error.message, 'error');
        } finally {
...
@track noLayoutAvailable = false;
    ...
    getDefaultWidgets() {
        return [
            ...
            { 
                ...
                style: 'position:absolute;top:20px;left:440px;width:400px;height:320px;',
                visible: true,
                zIndex: 2
            },
            { 
                ...
                style: 'position:absolute;top:360px;left:20px;width:400px;height:320px;',
                visible: true,
                zIndex: 3
            },
            { 
                ...
                style: 'position:absolute;top:360px;left:440px;width:400px;height:320px;',
                visible: true,
                zIndex: 4
            }
        ];
    ...
    if (!this.isLoading && this.isEditMode && this.interactLoaded && !this.interactionInitialized) {
        setTimeout(() => {
    ...
    initializeWidgetInteractions() {
        ...
        console.log('Found widget elements for interaction:', widgetElements.length);
                ...
                .draggable({
                    ...
                    modifiers: [
                        window.interact.modifiers.snap({
                            targets: [
                                window.interact.snappers.grid({ x: this.gridSize, y: this.gridSize })
                            ],
                            range: Infinity,
    ...
    getDragListeners() {
        return {
            ...
            move: (event) => {
                ...
                const hasCollision = this.checkFinalPositionCollision(target.getAttribute('data-id'), finalX, finalY, target.offsetWidth, target.offsetHeight);
                
                if (hasCollision) {
                    // Revert to original position if collision detected
                    const originalStyle = this.parseStyle(target.getAttribute('data-original-style') || target.style.cssText);
                    finalX = parseFloat(originalStyle.left) || 20;
                    finalY = parseFloat(originalStyle.top) || 20;
                    this.showToast('Warning', 'Cannot place widget here - overlaps with another widget', 'warning');
                } else {
                ...
                this.updateWidgetPosition(target.getAttribute('data-id'), finalX, finalY);
    ...
    getResizeListeners() {
        return {
            start: (event) => {
                console.log('Resize start for:', event.target.dataset.id, 'from edges:', event.edges);
            ...
            move: (event) => {
                ...
                const hasCollision = this.checkCollisionDuringResize(target, event.rect);
    ...
    startAutoScroll(event) {
        ...
        this.autoScrollInterval = setInterval(() => {
            ...
            if (scrollX !== 0 || scrollY !== 0) {
                grid.scrollLeft = Math.max(0, Math.min(grid.scrollLeft + scrollX, grid.scrollWidth - grid.clientWidth));
                grid.scrollTop = Math.max(0, Math.min(grid.scrollTop + scrollY, grid.scrollHeight - grid.clientHeight));
            }
        }, 16); // ~60fps
    }
    ...
    checkCollisionDuringMove(movingElement, x, y) {
        ...
        const otherWidgets = this.template.querySelectorAll('.widget:not([data-id="' + movingId + '"])');
    ...
    checkFinalPositionCollision(widgetId, x, y, width, height) {
        ...
        const otherWidgets = this.template.querySelectorAll('.widget:not([data-id="' + widgetId + '"])');
    ...
    checkCollisionDuringResize(resizingElement, newRect) {
        ...
        const otherWidgets = this.template.querySelectorAll('.widget:not([data-id="' + resizingId + '"])');
    ...
    getElementBounds(element, translateX = 0, translateY = 0) {
        ...
        return {
            ...
            right: (rect.left - gridRect.left) + grid.scrollLeft + translateX + element.offsetWidth,
            bottom: (rect.top - gridRect.top) + grid.scrollTop + translateY + element.offsetHeight,
    ...
    refreshAllCharts() {
        ...
        this.chartRefreshTimeout = setTimeout(() => {
            try {
                const dynamicWidgets = this.template.querySelectorAll('c-dynamic-widget');
                ...
                this.dispatchEvent(new CustomEvent('chartrefresh', {
...
toggleEditMode() {
   ...
   async handleSaveAndExit() {
    try {
        ...
        this.showToast('Success', 'Dashboard layout saved successfully!', 'success');
    } catch (error) {
        console.error('Save error:', error);
        this.showToast('Error', 'Failed to save dashboard layout: ' + error.message, 'error');
    } finally {
        this.isLoading = false;
    }
}
    ...
    handleCancel() {
        if (confirm('Are you sure you want to cancel? All unsaved changes will be lost.')) {
            ...
            this.showToast('Info', 'Changes cancelled. Dashboard restored to last saved state.', 'info');
        }
    }
...
async confirmReset() {
    try {
        ...
        this.showToast('Success', 'All widgets removed and layout reset successfully.', 'success');
    } catch (error) {
        console.error('Reset error:', error);
        this.showToast('Error', 'Failed to reset dashboard: ' + error.message, 'error');
    } finally {
        this.showResetModal = false;
        this.isLoading = false;
    }
...
confirmDeleteWidget() {
    try {
        // Remove the widget
        this.widgets = this.widgets.filter(widget => widget.id !== this.widgetToDeleteId);
        this.widgetToDeleteId = null;
        this.showDeleteConfirmModal = false;
   ...
   async addWidget() {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
        const newEl = this.template.querySelector(`[data-id="${newWidgetId}"]`);
        if (newEl) {
        ...
        this.showToast('Error', 'Failed to add widget: ' + error.message, 'error');
    }
}
...
get fabActionsStyle() {
    return this.isEditMode ? 'opacity: 1; transform: translateY(0);' : 'opacity: 0; transform: translateY(30px); pointer-events: none;';
}
get fabMainClass() {
