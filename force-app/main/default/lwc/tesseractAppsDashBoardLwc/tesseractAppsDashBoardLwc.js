...
`;
  ...
  handleFacilitySelect(event) {
    ...
    localStorage.setItem("defaultFacilityId", this.facilityValue);
    localStorage.setItem("defaultFacilityLabel", this.facilityLabel);
    localStorage.setItem("defaultParticipantPreferredName", this.participantPrefreedName || "");
    localStorage.setItem("defaultFacilityPreferredName", this.facilityPrefreedName || "");
  ...
  handleOutsideClick(event) {
    ...
    console.log("🟥 Outside click:  BEFORE closing dropdownContainer");
    // Check if the click target is outside the dropdown container
    if (dropdownContainer && !dropdownContainer.contains(event.target)) {
      console.log("🟥 Outside click: closing dropdownContainer");
      this.isOpen = false;
 ...
 /* validateCache() {
  ...
  validateCache() {
      ...
      if (this.finalListFacilities.length > 0 && this.facilityValue) {
          ...
          if (selectedFacility) {
              ...
              console.log("Updated Preferred Names from existing facilityValue:", this.facilityPrefreedName, this.participantPrefreedName);
          }
      }
  ...
  fetchNotifications(event) {
    ...
    this.showNotifications = true;
    this.getUserNotifications(); // 🔸 Separate method called here
  }
  ...
  handleNavigation(event) {
    ...
    switch (notification) {
      ...
      case "Document Expiry in 1 Day":
        ...
        this.simulateClick('.menu-link[data-field="My Profile"]');
        // No action for expiry notifications yet
        break;
    ...
    if (!isRead) { 
        markNotificationAsRead({ notificationIds: [notificationId] }) // 🔹 send as List
        .then(() => {
