          ...
          this.finalListFacilities = [
            {
              ...
              participantPreferredName: result.Facility__r.Participant_Preferred_Name_Formla__c,
              facilityPreferredName: result.Facility__r.Facility_Preferred_Name_Formula__c,
              staffPreferredName: result.Staff_Preferred_Name_Formula__c
            }
          ];
...
`;
  ...
  handleFacilitySelect(event) {
    ...
    this.facilityPrefreedName = event.currentTarget.dataset.facilityPreferredName ;
    this.participantPrefreedName = event.currentTarget.dataset.participantPreferredName;
    ...
    console.log('this.participantPrefreedName >>', this.participantPrefreedName);
 ...
 /* validateCache() {
  ...
  validateCache() {
      ...
      console.log("Current finalListFacilities (JSON):", JSON.stringify(this.finalListFacilities, null, 2));
      ...
      if (this.finalListFacilities.length > 0 && this.facilityValue) {
          const selectedFacility = this.finalListFacilities.find(fac => fac.value === this.facilityValue);
          if (selectedFacility) {
              this.facilityPrefreedName = selectedFacility.facilityPreferredName;
              this.participantPrefreedName = selectedFacility.participantPreferredName;
              this.staffPrefreedName = selectedFacility.staffPreferredName;
              console.log("Updated Preferred Names from existing facilityValue:", this.facilityPrefreedName, this.participantPrefreedName, this.staffPrefreedName);
          }
      }
      ...
      if (this.finalListFacilities.length === 0) {
          ...
          console.log("participantPrefreedName set to:", this.participantPrefreedName);

          localStorage.setItem("defaultFacilityId", this.facilityValue);
          localStorage.setItem("defaultFacilityLabel", this.facilityLabel);
          localStorage.setItem("defaultParticipantPreferredName", firstFacility.participantPreferredName || "");
          localStorage.setItem("defaultFacilityPreferredName", firstFacility.facilityPreferredName || "");
          localStorage.setItem("defaultStaffPreferredName", firstFacility.staffPreferredName || "");
          ...
          if (selectedFacility) {
              ...
              this.participantPrefreedName = selectedFacility.participantPreferredName;
              this.staffPrefreedName = selectedFacility.staffPreferredName;
              console.log("Updated Preferred Names from existing facilityValue:", this.facilityPrefreedName, this.participantPrefreedName, this.staffPreferredName);

              // 🔹 Save into localStorage as well
              localStorage.setItem("defaultFacilityId", this.facilityValue);
              localStorage.setItem("defaultFacilityLabel", this.facilityLabel);
              localStorage.setItem("defaultParticipantPreferredName", selectedFacility.participantPreferredName || "");
              localStorage.setItem("defaultFacilityPreferredName", selectedFacility.facilityPreferredName || "");
              localStorage.setItem("defaultStaffPreferredName", selectedFacility.staffPreferredName || "");
          }
      }
        ...
        case "Enable Sign out for Shift":
         ...
         getAddShiftDataById({shiftId: whatId}).then(result=>{
               ...
               // console.log('result',JSON.stringify(result));
                ...
                this.signOutComments = result.shiftwithstaffdata.Request_sign_out_comments__c;
                this.StaffRole=result.shiftwithstaffdata.Add_Shift__r.Role__c;
                this.shifttimmings=result.shiftwithstaffdata.Add_Shift__r.Shift_Start_End_Time__c;
                this.shiftDate = new Date(result.shiftwithstaffdata.Add_Shift__r.Start_Date__c).toLocaleDateString('en-GB');
  ...
  @wire(getstaffId)
    wiredStaffId({ error, data }) {
        ...
        if (data) {
            ...
            console.log('✅ Staff ID received from Apex:', JSON.stringify(this.staffId));
        } else if (error) {
            this.error = error;
...
handleMarkAllAsRead(event) {
    ...
    console.log("📌 Unread notifications:", JSON.stringify(unreadNotifications));
  ...
  stopPropagation(event) {
   ...
   handleCloseActionTooltip(event){
      ...
      get unreadCount() {
        ...
        return this.notificationCount || (this.items || []).filter(i => !i.read).length;
    }
