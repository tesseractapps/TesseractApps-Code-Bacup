...
export default class PerformanceManagementLwc extends LightningElement {
    ...
    //@track refreshTable = [];
     ...
     @track userRole;
     @track userType;
    @track selectedYear = '2024-2025';              //'2024-2025';
    ...
    checkPerformanceForStaff(staffId) {
        ...
        checkPerformanceRecordExist({ staffId })
            .then(result => {
    ...
    wiredReviewers({ data, error }) {
        if (data) {
            ...
            this.reviewerOptions = data.map(staff => ({
                label: staff.Name,  // Display name in the combobox
        ...
        return data.map((performance) => {
            ...
            return {
                ...
                formattedReviewTo: performance.Review_Period_To__c
                    ...
                    : '',
                reviewerName: performance.Reviewer__r?.Name ?? 'N/A',
    ...
    updateLibraryGoalOptions() {
        ...
        getLibraryGoals({ levelId: this.levelId, orgid: this.orgid})
            .then((result) => {
    ...
    SubmitAll(event) {
        ...
        if (this.isStaffDisabled) {
            this.showErrorToast('A performance record already exists for the selected staff.');
            return;
        }
