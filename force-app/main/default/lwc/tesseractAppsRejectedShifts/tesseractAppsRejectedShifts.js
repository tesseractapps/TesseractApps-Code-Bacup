...
export default class TesseractAppsRejectedShifts extends LightningElement {
  ...
  initializeHeaders() {
    ...
    handlePreviousWeek() {
        this.currentStartDate = new Date(this.currentStartDate.getTime() - 7 * 24 * 60 * 60 * 1000);
 ...
 @wire(getRejectedShifts, { datePicker: '$wiredDateParams.datePicker', orgId: '$wiredDateParams.orgId' })
        ...
        holidayList({ datePicker: formattedDate, state: this.state })
            .then(response => {
                ...
                this.headers.forEach(header => {
                    ...
                    const holiday = this.holidayList.find(h => h.Date__c === headerDate);
...
AllocateShift(event){
    ...
    const selectedRecord = this.rejectedShifts.find(item => item.Id === this.shiftId);
    ...
    .then(result => {
       ...
       this.staffHorlyRateList = parsedResult
        ...
        console.log('Filtered Hourly Rates:', JSON.stringify(this.staffHorlyRateList));
...
async handleAllocateToggle(event) {
        ...
        this.staffHorlyRateList = this.staffHorlyRateList.map(staff =>
            staff.Id === selectedId ? { ...staff, isAllocate: event.target.checked } : staff
        );
        ...
        if (checked) {
            ...
            if (this.selectedStaffId && this.selectedStaffId !== selectedId) {
                ...
                this.confirMationMessage('Error', 'Only one staff can be allocated at a time.', 'error');
                return;
            }
    ...
    if (!this.selectedStaffId) {
        this.confirMationMessage('Error', 'Please select a staff to allocate.', 'error');
        return;
    }
    ...
    .then(() => {
        ...
        this.confirMationMessage('Success', 'Shift allocated successfully.', 'success');
...
handleRun() {
    ...
    assignAutoShifts({startDate: this.startDate, endDate: this.endDate,orgId:this.orgid ,AutoScheduleParameters: JSON.stringify(this.rows)}).then(result=>{
         this.confirMationMessage('Success', 'Auto-scheduling completed successfully.', 'Success');
          this.autoschdulePopup=false;
           //  this.loadShifts();
            refreshApex(this.wiredRejectedShiftsResult);
    }).catch(error=>{
