...
export default class TesseractAppsStaffManagementLwc extends LightningElement {
    ...
    wiredStaffAccessDetails({ error, data }) {
        if (data) {
            ...
            this.filteredStaffData = [...this.userData]; // Initially show all data
    ...
    filterStaffData() {
        this.filteredStaffData = this.userData.filter((user) => {
            const matchesSearchStaff = this.searchStaff
                ? user.staffName.includes(this.searchStaff)
                : true;
    ...
    paginateData() {
        let filteredStaffData1 = this.userData.filter(user => user.staffName.includes(this.searchStaff));
        const startIndex = (this.pageNumber - 1) * this.recordsPerPage;
        const endIndex = startIndex + this.recordsPerPage;
        this.paginatedData = filteredStaffData1.slice(startIndex, endIndex);
    ...
    saveStaffDetails({ staffDetails: staffDetails })
        .then(result => {
            ...
            this.showToast('Success', 'Staff details saved successfully!', 'success');
