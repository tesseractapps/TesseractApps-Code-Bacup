...
export default class AccountExpense extends LightningElement {
   ...
   // supplierSearchTerm = '';
    ...
    wiredTax(result) {
        ...
        if (data && this.createEditExpense == 'Create Purchases') {
            ...
            console.log(' this.selectedItemId IN getEntityProfileTax:',  this.selectedItemId);
            ...
            // this.selectedOptionAL = this.selectedOptionAL ? this.selectedOptionAL : (entityAccountList || '');
            ...
            case 'taxInclusive':
               ...
               if (!this.taxInclusive) {
                   ...
                   if (this.selectedEntityName) {
                      if (this.wiredEntityProfilesTax?.data.tax) {
                            // ✅ Use the already fetched tax data again
                            this.selectedOptionTax = this.appendPercentage(this.wiredEntityProfilesTax.data.tax);
                            console.log('Reusing cached tax data:', this.selectedOptionTax);
                            ...
                            console.warn('No cached tax data to reapply');
                        }
                    }
                }
                break;
    ...
    toggleDropdownTax(event) {
        ...
        const left = rect.left + scrollX - 260;  
    ...
    handleSelectionTax(event) {
        ...
        if (this.entryType === 'Sales') {
            ...
            const subTotal = parseFloat(this.subTotal.toFixed(2));
   ...
   handleSave(){
    ...
    handleSaveExpense() {
        ...
        if (this.taxInclusive === true) {
            if (!this.selectedOptionTax || this.selectedOptionTax.trim() === '') {
                throw new Error('Please select a tax option when Tax Inclusive is enabled.');
            }
        }
    ...
    handleUpdate() {
        console.log('selectedItemId handleUpdate: ' + this.selectedItemId);
        console.log('selectedOptionAL in handleUpdate: ' + this.selectedOptionAL);
        ...
        const updatedExpense = {
           ...
           // Accounting_Ledger_Items__Name__c: this.selectedOptionAL,  // Use selected ledger item
            //selectedOptionAL: this.selectedOptionAL,
    ...
    //     this.selectedOptionAL = `${accNo} - ${itemName}`;
    //     this.supplierSelectedAccountId = selectedId;
    //     this.showtable = false;

    //     console.log('Supplier selected:', this.selectedOptionAL, selectedId);
    // }
