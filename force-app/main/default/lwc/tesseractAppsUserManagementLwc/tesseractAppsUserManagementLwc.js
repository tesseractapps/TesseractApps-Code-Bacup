...
export default class TesseractAppsUserManagement extends LightningElement {
    ...
    /* loadPaginatedData() {
        ...
        /* handleFinalSubmit() {
            ...
            console.log('validRecords data before processing:', JSON.stringify(this.validRecords));
   ...
   /* handleFinalSubmit() {
        ...
        // Update records with error if required fields are missing
        this.tableData = this.tableData.map(user => {
            if (user.isSelected && isRecordInvalid(user)) {
                hasInvalidRecords = true;
                return { ...user, ProcessedStatus: 'Error ❌: Missing Required Fields' };
        ...
        try {
            validRecords = selectedRecords
                ...
                .map(user => {
                    ...
                    // ❗ Throw error if DateofBirth is invalid
                    if (!isValidDateFormat(formattedDOB)) {
                        throw new Error(`Invalid date format in field "DateofBirth" for user ${name}. Expected format is dd/MM/yyyy.`);
                    }

                    // ❗ Throw error if StartDate is invalid
                    if (!isValidDateFormat(formattedStart)) {
                        throw new Error(`Invalid date format in field "StartDate" for user ${name}. Expected format is dd/MM/yyyy.`);
                    }
            ...
            // Show error in console or UI
            console.error('Submission failed:', error.message);
            this.showToast('Error', error.message, 'error'); // Optional: if you have toast mechanism
        }
