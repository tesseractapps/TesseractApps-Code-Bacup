        ...
        switch (actionName) {
            case 'edit':
               ...
               // this.isEdit = true;
                ...
                // Step 1: Get NDIS Service Line Items
                getNDISServiceLineItem({ ServiceItemNames: this.serviceTypeName, ServiceDate: this.approvedDate })
                    .then(ndisResponse => {
                        ...
                        // Step 2: Get selected support items
                        return getSelectedSupportItems({ fundTrackerId: rowId, clientId: this.clientId })
                            .then(selectedItems => {
                               console.log('Selected Items from EDIT: ' + JSON.stringify(selectedItems));
                                ...
                                fetchedItems = fetchedItems.map(item => {
                                    ...
                                    if (match) {
                                        return {
                                            ...item,
                                            junctionId: match.Id,        // update with selected item's Id
        ...
        if (this.stateValue) {
            ...
            getNDISServiceLineItem({ ServiceItemNames: this.serviceTypeName, ServiceDate: this.approvedDate })
                .then(ndisResponse => {
                    ...
                    if (this.isEdit) {
                        return getSelectedSupportItems({ fundTrackerId: this.recordId, clientId: this.clientId })
                            .then(selectedItems => {
