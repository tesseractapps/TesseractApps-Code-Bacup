...
export default class ClientFundTranfer extends LightningElement {
  ...
  connectedCallback() {
    ...
    getClientById({ recordId: this.clientId })
      .then((result) => {
        console.log(" cleint result " + JSON.stringify(result));
        this.otherThanNdis = result[0].Facility__r.Type_of_Service__c != "NDIS";
        console.log("188 line ths.otherThanNdis :" + this.otherThanNdis);
        if (this.otherThanNdis) {
          this.filteredServiceTypeOptions = this.serviceTypeOption.filter(
            (option) => option.value.trim() === "Other"
          );
        } else {
          this.filteredServiceTypeOptions = this.serviceTypeOption.filter(
            (option) => option.value.trim() !== "Other"
        ...
        console.log("cleint result otherthanNDIS :" + this.otherThanNdis);
  ...
  // ✅ Computed getter: decide if Amount should be editable
  get isAmountEditable() {
    return (
      this.otherThanNdis ||
      this.serviceGroupName === "Miscellaneous" ||
      this.serviceGroupName === "Other"
    );
  }
  ...
  fetchInitialData() {
    organizationDetails().then((response) => {
      ...
      /*   if (this.otherThanNdis) {
            ...
            console.log('filteredServiceTypeOptions: ' + JSON.stringify(this.filteredServiceTypeOptions)); */
    });
  }
  ...
  @track serviceTypeOption = [
    ...
    {
      label: "Assistance to Access and Maintain Employment or higher education",
      value: "Assistance to Access and Maintain Employment or higher education"
    },
    {
      label:
    ...
    {
      label: "Assistive Products for Personal Care and Safety",
      value: "Assistive Products for Personal Care and Safety"
    },
    {
      label: "Communication and Information Equipment",
      value: "Communication and Information Equipment"
    },
    { label: "Community Nursing Care", value: "Community Nursing Care" },
    {
      label: "Customised Prosthetics (includes Orthotics)",
      value: "Customised Prosthetics (includes Orthotics)"
    },
    {
      label: "Sleepover ShiftDaily Personal Activities",
      value: "Daily Personal Activities"
    },
    {
      label: "Development of Daily Living and Life Skills",
      value: "Development of Daily Living and Life Skills"
    },
    {
      label: "Early Intervention Supports for Early Childhood",
      value: "Early Intervention Supports for Early Childhood"
    },
    {
      label: "Exercise Physiology & Personal Well-being Activities",
      value: "Exercise Physiology & Personal Well-being Activities"
    },
    {
      label: "Group and Centre Based Activities",
      value: "Group and Centre Based Activities"
    },
    ...
    { label: "Hearing Services", value: "Hearing Services" },
    {
      ...
      value: "High Intensity Daily Personal Activities"
    },
    {
      label: "Home Modification Design and Construction",
      value: "Home Modification Design and Construction"
    },
    ...
    {
      label: "Interpreting and Translation",
      value: "Interpreting and Translation"
    },
    {
      label: "Management of Funding for Supports",
      value: "Management of Funding for Supports"
    },
    {
      label: "Participation in Community, Social and Civic Activities",
      value: "Participation in Community, Social and Civic Activities"
    },
    {
    ...
    {
      label: "Specialised Disability Accommodation",
      value: "Specialised Disability Accommodation"
    },
    {
      label: "Specialised Driver Training",
      value: "Specialised Driver Training"
    },
    {
      label: "Specialised Hearing Services ",
      value: "Specialised Hearing Services "
    },
    {
      label: "Specialised Supported Employment",
      value: "SSpecialised Supported Employment"
    },
    {
      label: "Specialist Positive Behaviour Support",
      value: "Specialist Positive Behaviour Support"
    },
              ...
              fetchedItems = fetchedItems.map((item) => {
                ...
                if (match) {
                  ...
                  if (
                    ...
                    (this.serviceTypeName === "Miscellaneous" &&
  ...
  handleServiceStateChange(event) {
    ...
    if (this.stateValue) {
      ...
      console.log("538 values " + this.serviceTypeName);
      getNDISServiceLineItem({
        ServiceItemNames: this.serviceTypeName,
        ServiceDate: this.approvedDate
      })
        .then((ndisResponse) => {
          let newStateItems = ndisResponse.map((item) => {
            ...
            return {
              ...item,
              amount: amount !== undefined ? amount : 0.0,
              isSelected: this.otherThanNdis ? true : false //isSelected: false
            };
          });
          if (this.otherThanNdis === true) {
            this.saveDisabled = false; // Enable the save button
            this.Miscellaneous = false;
            this.selectedServiceRows = []; // Clear any previous selections
          ...
          if (this.isEdit) {
            ...
            return getSelectedSupportItems({
              ...
              const previouslySelectedIds = this.selectedSupportItemsGlobal.map(
                (i) => i.Id
              );
              ...
              [...this.selectedSupportItemsGlobal, ...newStateItems].forEach(
                (item) => {
  ...
  /*  handleSuccess(event) {
        ...
        console.log('ServiceGropData : '+  JSON.stringify(this.serviceGroupName));
        if (selectedCatalogIds.length > 0) {
            insertPromise = insertClientFundTrackerLinks({
                fundTrackerId: fundTrackerId,
                participantId: this.clientId,
  ...
  handleSaveAmount(event) {
    ...
    // optional: call apex to persist update
    console.log(
      "Updated serviceGroupName12:",
      JSON.stringify(this.serviceGroupName)
    );
  }
}