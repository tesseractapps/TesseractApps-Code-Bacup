...
export default class ClientFundTranfer extends LightningElement {
    ...
    connectedCallback() {
        ...
        this.fetchInitialData();
           getClientById({recordId:this.clientId}).then(result=>{
   ...
   /*  fetchInitialData() {
        organizationDetails().then((response) => {
            console.log('response of fetchinitialidata :'+JSON.stringify(response));
            ...
            if(this.otherThanNdis) {
            this.filteredServiceTypeOptions=this.serviceTypeOption.filter(option => option.value.trim() === "Other");
            }else{
            this.filteredServiceTypeOptions=this.serviceTypeOption
            }
            this.serviceTypeName = this.serviceTypeOption[0].value;
            console.log('filteredServiceTypeOptions '+JSON.stringify(this.filteredServiceTypeOptions));            
        });
    } */
   fetchInitialData() {
    ...
    @track serviceTypeOption = [
        ...
        { label: "Sleepover ShiftDaily Personal Activities", value: "Daily Personal Activities" },
                ...
                .then(ndisResponse => {
                    ...
                    if (this.otherThanNdis === true) {
                        ...
                        newStateItems.forEach(item => {
                            ...
                            const alreadyAdded = this.selectedServiceRows.some(row => row.Id === item.Id);
                            if (!alreadyAdded) {
