...
export default class ClientFundTranfer extends LightningElement {
  ...
  handleEntityNameChange(event) {
      ...
      this.EntityNameValue = event.detail.value;

      // Optional: Get the selected option’s label if needed
      const selectedOption = this.EntityNameOptions.find(
          option => option.value === this.EntityNameValue
      );
      const selectedLabel = selectedOption ? selectedOption.label : null;

      console.log("🔹 Selected Entity Value:", this.EntityNameValue);
      console.log("🔹 Selected Entity Label:", selectedLabel);

      // 👉 If you want to persist in localStorage (optional)
      localStorage.setItem("defaultEntityNameValue", this.EntityNameValue);
      if (selectedLabel) {
          localStorage.setItem("defaultEntityNameLabel", selectedLabel);
      }

      // 👉 Do any additional logic here (call Apex, filter data, etc.)
  }
