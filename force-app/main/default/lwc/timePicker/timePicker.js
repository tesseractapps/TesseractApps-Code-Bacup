...
export default class TimePicker extends LightningElement {
    ...
    updateFormattedTime() {
        ...
        const displayHour = this.selectedHour.toString();
        const period = this.isAm ? 'AM' : 'PM';

        this.formattedDisplayTime = `${displayHour}:${this.selectedMinute
            .toString()
            .padStart(2, '0')} ${period}`;

        this.formattedSaveTime = `${hour.toString().padStart(2, '0')}:${this.selectedMinute
            .toString()
            .padStart(2, '0')}:00Z`;
    }
