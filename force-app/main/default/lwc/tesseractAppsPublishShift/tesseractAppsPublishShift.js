...
;              if (this.tablelagfromparent == true) {
             ...
             //   console.log('Received data in role:', this.addShiftData.AddShiftRole);
                ...
                .then(response => {
                    ...
                    this.unAllocatedTableList = response.map((rec, index) => {
                      ...
                      let EOIvalue = rec.Is_EOI__c ? 'True' : 'False';
                        return {
                            ...rec,  // Correct usage of spread operator
                            serialNumber: index + 1,
                            date: formattedDate,
                            EOIvalue: EOIvalue
                        };
                    });
                     console.log('shiftlist123:', JSON.stringify(this.unAllocatedTableList));
                })
                .catch(err => {
  ...
  validateAddress() {
      ...
      handleAddShiftSave(){
       ...
       // console.log('validate '+this.validateAddress());
        ...
        if(this.isEditShitScreen==true &&  this.addShiftData.AddShiftQuantity < this.AddShiftOriginalQuantity ){
          ...
          rosterPublish({addshiftData:JSON.stringify(this.finalAddShiftData),startdate:this.startdate,enddate:this.enddate})
          .then((result) => {
                ...
                if(this.addShiftData.AddShiftEOI==false){
                 ...
                 console.log('Parsed Staff Hourly Rates:', JSON.stringify(this.staffHorlyRateList));
                      if(this.staffHorlyRateList.length>0){
                        this.selectedShiftId=this.staffHorlyRateList[0].shiftId;
    ...
    handleUnAllocated(event) {
      ...
      getStaffHourlyRates({ addShiftList: JSON.stringify(filteredShiftList),startdate:this.startdate,enddate:this.enddate })
          .then((result) => {
