           ...
           console.log('facility val from parent :', JSON.stringify(this.facilityarray));
             ...
             //   console.log('Received data in role:', this.addShiftData.AddShiftRole);
                ...
                getFacilityData().then(response => {
                      this.facilityOptions = response.map(record => ({ value: record.Id, label: record.Name,preferredName: record.Facility_Preferred_Name_Formula__c,participantPreferredName: record.Participant_Preferred_Name_Formla__c }));
            ...
            getClientFunds({ clientId: this.addShiftData.AddShiftParticipantValue })
                .then((response) => {
                    ...
                    if (!response || response.length === 0) {
                        this.serviceType = false;
                        this.savButtonDisable = true;
                        this.confirMationMessage('Error', 'No service funds available for the selected participant.', 'error');
                        return;
                    }
                if (response) {
