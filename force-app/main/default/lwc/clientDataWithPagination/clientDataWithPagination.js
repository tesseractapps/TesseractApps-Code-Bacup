...
export default class ClientDataWithPagination extends NavigationMixin(LightningElement) {
    ...
    fetchStaffData() {
        getStaffData({ clientId: this.clientid, facilityId: this.facilityId, role: this.role })
            .then(data => {
           ...
           // console.log('response>>>',response);
            ...
            if( userTpe =='NDIS Org Admin' || userTpe == 'ICT Admin'){
                        ...
                        this.finalListFacilities= this.facilityOptions ;
                             finalData =response
                              let facilityIds = [];
                            console.log('Fetch Participant filteredData>>>'+ JSON.stringify(response));
...
applyFilters() {
    ...
    if (this.fullName && this.fullName.trim() !== '') {
        ...
        result = result.filter(p =>
            (p.First_Name__c && p.First_Name__c.toLowerCase().includes(searchLower)) ||
            (p.Last_Name__c && p.Last_Name__c.toLowerCase().includes(searchLower))
        );
    }
