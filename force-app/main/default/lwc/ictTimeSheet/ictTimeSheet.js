        ...
        getCurrentLoggedUserInfo().then(userData=>{
                              const storedFacilityId = localStorage.getItem('defaultFacilityId');
                               ...
                               console.log('userType >>', userType);
                                  if(userType =='ICT Admin' || userType == 'NDIS Org Admin' || userType =='NDIS Staff' || userType =='ICT Staff'){ 
                                       
                                          this.facilityOptions = response.map(record => ({ value: record.Id, label: record.Name }));
                                           console.log(' this.facilityOptions in ICT Admin IN IF ' + this.facilityOptions) 
                                         ...
                                         getFacilityCurrentUser().then(result => {
                                                  this.facilityOptions=result.map(record => ({
                                                                  ...
                                                                  console.log(' this.facilityOptions in facility idmin' + this.facilityOptions) 
                                                    let facilityLength=Object.keys(this.facilityOptions).length;
                                                    // console.log('facility length'+facilityLength);
                                                    // console.log('facility first value'+this.facilityOptions[0].value);
                                                      this.facilityVal=storedFacilityId;
...
@track todayCssVariable;
  navigateToToday() {
    ...
    this.todayCssVariable =(presentday)+"/"+(presentMonth)+"/"+toDayNewDate.getFullYear() ; 
  ...
  handleSave(event) {
   ...
   // console.log("this.sDate final", this.sDate);
    if(statusOfIct==''|| statusOfIct=='Rejected' || statusOfIct === "undefined" || statusOfIct === "Draft" ){
     // this.isShowModal = true;
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          title: 'Error',
          message: 'You are not allowed to edit the Shift when Status: "Submitted/Approved"',
          variant: 'Error'
        })
      );
    }
...
clonePreviousDayData() {
   ...
   /*  let previousDayData = this.finalStaffList.find(staff => {
    ...
    if(previousDateStr){
    clonePreviousDayAllocation({ selectedDate: previousDateStr, staffId: this.staffData })
            .then(result => {
                if (result) {
                  this.starttime = this.convertMillisecondsToTime(result.Start_Date_Time__c);
                  console.log('START TIME--->'+this.starttime);
                  this.endTime = this.convertMillisecondsToTime(result.End_Date_Time__c);
                  this.breakStartTime = this.convertMillisecondsToTime(result.Break_Start_Time__c);
                  this.breakEndTime = this.convertMillisecondsToTime(result.Break_End_Time__c);
                  this.breakStartTime2 = this.convertMillisecondsToTime(result.Break_Start_Time2__c);
                  this.breakEndTime2 = this.convertMillisecondsToTime(result.Break_End_Time2__c);
                  this.breakStartTime3 = this.convertMillisecondsToTime(result.Break_Start_Time3__c);
                  this.breakEndTime3 = this.convertMillisecondsToTime(result.Break_End_Time3__c);
                  
                  this.uistarttime = this.formatTime(result.Start_Date_Time__c);
                  console.log('uiSTART TIME--->'+this.uistarttime);
                  this.uiendTime = this.formatTime(result.End_Date_Time__c);
                  if(this.breakStartTime && this.breakEndTime){
                    
                    this.uibreakStartTime = this.formatTime(result.Break_Start_Time__c);
                  ...
                  if(this.breakStartTime2 && this.breakEndTime2){
                    ...
                    this.uibreakStartTime2 = this.formatTime(result.Break_Start_Time2__c);
                  ...
                  if(this.breakStartTime3 && this.breakEndTime3){
                    ...
                    this.uibreakStartTime3 = this.formatTime(result.Break_Start_Time3__c);
                    ...
                    this.dispatchEvent(
                      new ShowToastEvent({
                        title: 'Error',
                        message: 'No previous day timesheet found to clone.',
                        variant: 'Error'
                      })
                    );
                    this.breaksdisable = false;
...
getPreviousWorkday(dateObj) {
    ...
    if (day === 0 || day === 6) { // If Sunday or saturday → return null
      this.breaksdisable = true;
      this.breakReadonly = true;
      this.dispatchEvent(new ShowToastEvent({
        title: 'Error',
        message: 'You cannot clone on Saturday and Sunday',
        variant: 'error'
    }));
    return null;
    
    } else if (day === 6) { // If Saturday → Move to Friday
        dateObj.setDate(dateObj.getDate() - 1);
    }else if (day === 1) { 
      dateObj.setDate(dateObj.getDate() - 3);
  } else { // Normal case → Move to the previous day
        dateObj.setDate(dateObj.getDate() - 1);
    }
...
@track endDateTimeForValidation;
  ...
  breakValueCalculation(){
    if(!this.breakStartTime && !this.breakEndTime &&
      !this.breakStartTime2 && !this.breakEndTime2 &&
      !this.breakStartTime3 && !this.breakEndTime3){
  ...
  handlecBreakhange(event) {
    if (event.target.name === "breaktime" || event.target.name === "breakTimeEdit") {
        ...
        if (regex.test(inputVal) || inputVal === "") {
            ...
            this.errorMessage = "Invalid Break! Enter a number up to 3 digits with max 2 decimals.";
  ...
  handleBreakStartEndechange(event) {
    ...
    if (this.breakStartTime && !this.breakEndTime) {
      this.errorMessage = 'Break End Time is required when Break Start Time is entered';
      this.errorMessageFlag = true;
      this.disableSaveButton = true;
  } else if (!this.breakStartTime && this.breakEndTime) {
    this.errorMessage = 'Break Start Time is required when Break End Time is entered';
    this.errorMessageFlag = true;
    this.disableSaveButton = true;
  } else if (!this.breakStartTime2 && this.breakEndTime2) {
    this.errorMessage = 'Break Start Time 2 is required when Break Start Time 2 is entered.';
    this.errorMessageFlag = true;
    this.disableSaveButton = true;
} else if (!this.breakEndTime2 && this.breakStartTime2) {
    this.errorMessage = 'Break End Time 2 is required when Break End Time 2 is entered.';
    this.errorMessageFlag = true;
    this.disableSaveButton = true;
} else if (!this.breakEndTime3 && this.breakStartTime3) {
    this.errorMessage = 'Break Start Time 3 is required when Break Start Time 3 is entered.';
    this.errorMessageFlag = true;
    this.disableSaveButton = true;
} else if (!this.breakStartTime3 && this.breakEndTime3) {
    this.errorMessage = 'Break Start Time 3 is required when Break End Time 3 is entered.';
    this.errorMessageFlag = true;
    this.disableSaveButton = true;
} else {

    // Function to parse time as Date object
    const parseTime = (time) => (time ? new Date(`${this.sDate}T${time}`) : null);
    ...
    for (let i = 0; i < breaks.length; i++) {
        ...
        if (breakStart && breakEnd) {
            ...
            if (breakEnd <= breakStart) {
                this.errorMessageFlag = true;
                this.errorMessage = 'Break End Time must be greater than Break Start Time.';
                this.disableSaveButton=true;
                console.error('Error: Break End Time must be greater than Break Start Time.');
                return;
            }
            ...
            if (breakStart < shiftStart || breakEnd > shiftEnd) {
                this.errorMessageFlag = true;
                this.errorMessage = 'Break time must be within shift start and end time.';
                this.disableSaveButton=true;
                console.error('Error: Break time must be within shift start and end time.');
                return;
            }
            ...
            for (let j = 0; j < breakTimes.length; j++) {
                ...
                if (
                    ...
                    this.errorMessageFlag = true;
                    this.errorMessage = 'Break times should not overlap.';
                    this.disableSaveButton=true;
  ...
  this.handleBreakStartEndechange({ target: { fieldName: "", value: "" } });
    ...
    breakStartEndDurationCal(){
      ...
      let endDateChange = new Date();
        ...
        console.log(' break satrt time value in 24 hours format ==>'+ this.breakStartTimePickval);
        if(this.sDate==undefined){
          this.sDate=this.dateJson;
        }
        let Dateparts=this.sDate.split("-");
        if(this.breakStartTimePickval != null && this.breakStartTimePickval != 'undefined'){
        ...
        if(this.breakEndTimePickval != null && this.breakEndTimePickval != 'undefined'){
        ...
        //console.log('end date change '+endDateChange);
        //this.calculateDuration();

       //console.log('start time validation  '+this.startDateTimeForValidation);
       // console.log('end time validation  change '+this.endDateTimeForValidation);
          if(startDateChange <= this.startDateTimeForValidation){
            this.errorMessage='The break start time must be greater than the start time';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          } else if(endDateChange >= this.endDateTimeForValidation){
            this.errorMessage='The break end time must be less than the end time';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          }else if(startDateChange > endDateChange){
            console.log('startDateChange===>'+startDateChange);
            console.log('endDateChange===>'+endDateChange);
            this.errorMessage='The Break end time must be greater than the break start time';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          }else if(startDateChange >= this.endDateTimeForValidation){
            this.errorMessage='The break start time must be less than the end time';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          }else if(endDateChange <= this.startDateTimeForValidation){
            this.errorMessage='The break end time must be greater than the start time';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          }else{
            this.errorMessage='';
      ...
      /*   if(((startDateChange < this.startDateTimeForValidation )|| (endDateChange > this.endDateTimeForValidation ))
...
timeErrorValidation(){
   if(this.whours <= 0){
    this.errorMessage='The end time must be greater than the start time';
    this.errorMessageFlag=true;
    this.disableSaveButton=true
   }else{
    this.errorMessage='';
 ...
 /*  handleSubmit(event) {
    ...
    if(this.starttime && this.endTime){
     // console.log(this.whours);
      if (this.whours === undefined) {
        this.whours = '8';
      }
      const fields = event.detail.fields;
     // fields.Start_Date_Time__c=this.spkValue;
      //fields.End_Date_Time__c=this.epkValue;
 ...
 */
   ...
   this.showSpinner=true;
    ...
    getStaffAllocation({ facId: this.facilityVal }).then(response => {
      ...
      this.records.forEach((record) => {
        ...
        finalStaffDate = '"Id"' + ':' + '"' + record.Id + '"' + ',' + '"Name"' + ':' + '"' + record.Name + '"' + ',' + '"Last_Name__c"' + ':' + '"' + record.Last_Name__c + '"' +','+'"email"'+':'+'"'+record.Email_Address__c+'"'+','+'"hoursRate"'+':'+'"'+record.Invoice_Rate__c+'"'+','+'"role"'+':'+'"'+record.Role__c+'"'+','+'"invoiceTo"'+':'+'"'+record.Invoice_To__c+'"'+','+'"description"'+':'+'"'+description +'"'+
    ...
    (this.holidayList || []).forEach((record) => {
      datesList.push({ "label": record.Date__c, "value": record.Holiday_Name__c });
  });
  
    this.holidayDateList = datesList;
    console.log('this.holidayDateList===>'+JSON.stringify(this.holidayDateList));
    ...
    for (var i = 0; i < this.finalStaffData.length; i++) {
     ...
     getAlocationData({datePicker :formattedDate,facName : this.faclitylabel}).then(records => { 
      
        this.allocationList = records;
       console.log('Allocation before loop==>'+JSON.stringify( this.allocationList));
      ...
      for (var i = 0; i < this.finalStaffData.length; i++) { 
        ...
        for (var j = 0; j < this.allocationList.length; j++) {
          ...
          if (this.finalStaffData[i].Id == this.allocationList[j].Staff__c && (this.finalStaffData[i].Day0 == this.allocationList[j].Start_Date__c)) {
            this.finalStaffData[i]['bgcolour1'] = 'slds-theme_warning';
            this.finalStaffData[i]['startTime1'] =this.fixTimeRangeFormat(this.allocationList[j].Shift_time_AMPM__c);
            this.finalStaffData[i]['hours1'] =this.allocationList[j].Working_Hours__c;
            hours=hours+this.allocationList[j].Working_Hours__c;
            staffAllocaJson.push(this.allocationList[j].Id);
            if(this.allocationList[j].Status_Formula__c && this.allocationList[j].Status_Formula__c != null ){
              ictStatus=this.allocationList[j].Status_Formula__c;
            }
            if(this.allocationList[j].Comments__c && this.allocationList[j].Comments__c != null){
              comments=this.allocationList[j].Comments__c;
            }
            this.finalStaffData[i]['alocId1'] = this.allocationList[j].Id;
            this.satffDataJasonformat[this.allocationList[j].Id]={"stringStarttime":this.allocationList[j].Start_Time_Formula__c,"StringEndTime":this.allocationList[j].End_Time_Formula__c,"breakTime":this.allocationList[j].Break__c,"startDate":this.allocationList[j].Start_Date__c,"status":this.allocationList[j].Status__c,"workingHours":this.allocationList[j].Working_Hours__c,"breakStartTime":this.allocationList[j].Break_Start_Time_Formula__c,"breakEndTime":this.allocationList[j].Break_End_Time_Formula__c,"breakStartTime2":this.allocationList[j].Break_Start_Time2_Formula__c,"breakEndTime2":this.allocationList[j].Break_End_Time2_Formula__c,"breakStartTime3":this.allocationList[j].Break_Start_Time3_Formula__c,"breakEndTime3":this.allocationList[j].Break_End_Time3_Formula__c}
          }
          if (this.finalStaffData[i].Id == this.allocationList[j].Staff__c && (this.finalStaffData[i].Day1 == this.allocationList[j].Start_Date__c)) {
            this.finalStaffData[i]['bgcolour2'] = 'slds-theme_warning';
            this.finalStaffData[i]['startTime2'] =this.fixTimeRangeFormat(this.allocationList[j].Shift_time_AMPM__c);
            this.finalStaffData[i]['hours2'] =this.allocationList[j].Working_Hours__c;
            staffAllocaJson.push(this.allocationList[j].Id);
            if(this.allocationList[j].Status_Formula__c && this.allocationList[j].Status_Formula__c != null ){
              ictStatus=this.allocationList[j].Status_Formula__c;
            }
          ...
          if (this.finalStaffData[i].Id == this.allocationList[j].Staff__c && (this.finalStaffData[i].Day2 == this.allocationList[j].Start_Date__c)) {
            this.finalStaffData[i]['bgcolour3'] = 'slds-theme_warning';
            this.finalStaffData[i]['startTime3'] =this.fixTimeRangeFormat(this.allocationList[j].Shift_time_AMPM__c);
            this.finalStaffData[i]['hours3'] =this.allocationList[j].Working_Hours__c;
            if(this.allocationList[j].Status_Formula__c && this.allocationList[j].Status_Formula__c != null){
              ictStatus=this.allocationList[j].Status_Formula__c;
            }
            if(this.allocationList[j].Comments__c && this.allocationList[j].Comments__c != null){
          ...
          if (this.finalStaffData[i].Id == this.allocationList[j].Staff__c && (this.finalStaffData[i].Day3 == this.allocationList[j].Start_Date__c)) {
            this.finalStaffData[i]['bgcolour4'] = 'slds-theme_warning';
            this.finalStaffData[i]['startTime4'] =this.fixTimeRangeFormat(this.allocationList[j].Shift_time_AMPM__c);
            this.finalStaffData[i]['hours4'] =this.allocationList[j].Working_Hours__c;
            if(this.allocationList[j].Status_Formula__c && this.allocationList[j].Status_Formula__c != null ){
              ictStatus=this.allocationList[j].Status_Formula__c;
            }
            if(this.allocationList[j].Comments__c && this.allocationList[j].Comments__c != null){
          ...
          if (this.finalStaffData[i].Id == this.allocationList[j].Staff__c && (this.finalStaffData[i].Day4 == this.allocationList[j].Start_Date__c)) {
            this.finalStaffData[i]['bgcolour5'] = 'slds-theme_warning';
            this.finalStaffData[i]['startTime5'] =this.fixTimeRangeFormat(this.allocationList[j].Shift_time_AMPM__c);
            this.finalStaffData[i]['hours5'] =this.allocationList[j].Working_Hours__c;
            if(this.allocationList[j].Status_Formula__c && this.allocationList[j].Status_Formula__c != null ){
              ictStatus=this.allocationList[j].Status_Formula__c;
            }
            if(this.allocationList[j].Comments__c && this.allocationList[j].Comments__c != null){
          ...
          if (this.finalStaffData[i].Id == this.allocationList[j].Staff__c && (this.finalStaffData[i].Day5 == this.allocationList[j].Start_Date__c)) {
            this.finalStaffData[i]['bgcolour6'] = 'slds-theme_warning';
            this.finalStaffData[i]['startTime6'] =this.fixTimeRangeFormat(this.allocationList[j].Shift_time_AMPM__c);
            this.finalStaffData[i]['hours6'] =this.allocationList[j].Working_Hours__c;
            if(this.allocationList[j].Status_Formula__c && this.allocationList[j].Status_Formula__c != null ){
              ictStatus=this.allocationList[j].Status_Formula__c;
            }
            if(this.allocationList[j].Comments__c && this.allocationList[j].Comments__c != null){
          ...
          if (this.finalStaffData[i].Id == this.allocationList[j].Staff__c && (this.finalStaffData[i].Day6 == this.allocationList[j].Start_Date__c)) {
            this.finalStaffData[i]['bgcolour7'] = 'slds-theme_warning';
            this.finalStaffData[i]['startTime7'] =this.fixTimeRangeFormat(this.allocationList[j].Shift_time_AMPM__c);
            this.finalStaffData[i]['hours7'] =this.allocationList[j].Working_Hours__c;
            if(this.allocationList[j].Status_Formula__c && this.allocationList[j].Status_Formula__c != null ){
              ictStatus=this.allocationList[j].Status_Formula__c;
            }
            if(this.allocationList[j].Comments__c && this.allocationList[j].Comments__c != null){
            ...
            this.finalStaffData[i]['totalHOurs'] = parseFloat(hours.toFixed(2));
            ...
            if((this.finalStaffData[i]['ictStatus'] =='Rejected' || this.finalStaffData[i]['ictStatus'] =='' || this.finalStaffData[i]['ictStatus'] =='Draft') ){
      ...
      console.log('staff AFTER loop==>'+JSON.stringify(this.finalStaffData));
      console.log('staffdata jason format'+JSON.stringify(this.satffDataJasonformat));
      }).catch(error=>{
  ...
  fixTimeRangeFormat(timeRangeStr) {
    ...
    const fixTime = (timeStr) => {
        ...
        if (!timePart || !ampmRaw) return timeStr;
...
@track totalHoursJson;
  handleEdit(event) {
   ...
   console.log('alocId===>'+alocId);
    console.log('StaffJsonForamt in edit  '+JSON.stringify(this.satffDataJasonformat[alocId]) )
    ...
    if(this.satffDataJasonformat[alocId]["breakStartTime"]){
      this.breakStartTimePickval =this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakStartTime"])+'.000Z';
    }
    if(this.satffDataJasonformat[alocId]["breakEndTime"]){
      this.breakEndTimePickval =this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakEndTime"])+'.000Z';
    }
    if(this.satffDataJasonformat[alocId]["breakStartTime2"] && this.satffDataJasonformat[alocId]["breakEndTime2"]){
      this.showbreak2=true;
    }
    if(this.satffDataJasonformat[alocId]["breakStartTime3"] && this.satffDataJasonformat[alocId]["breakEndTime3"]){
      this.showbreak3=true;
    }
  ...
  handleClone(event) {
    ...
    if (this.facilityVal === undefined) {
      ...
      if (this.facilityVal != undefined) {
        updateAllocationRecoreds({ facilityId: this.facilityVal, startDate: this.startDate }).then(response => {

          const event = new ShowToastEvent({
            title: '',
            message: 'Allocations cloned for this Facility',
            variant: 'success',
            mode: 'dismissable'
          });
          this.dispatchEvent(event);
    ...
    if(totalhours ==0){
      this.dispatchEvent(
        new ShowToastEvent({
            ...
            message: 'Please Create Shifts',
            variant: 'Error'
        })
      );
    }else{
      this.submissionFlag=true;
      this.confirmationData = { totalhours, dateStarts, dateEnds, allocId, stfid };
    }

  }
  
  handletimesheetsubmit(event){
    const { dateStarts, dateEnds, allocId, totalhours, stfid } = this.confirmationData;

    createTimeSheet({ WeekStartDate:dateStarts, WeekendDate:dateEnds,allocationIds:allocId,duration:totalhours,staffId: stfid}).then(response => {
...
.then(() => {
    this.showToast('Success', 'Timesheets updated successfully!', 'success');
})
.catch(error => {
...
this.handleVisbility();
  ...
  @track ledgerItems = []; 
   ...
   generateICTInvoice(event){
    ...
    this.invoiceTable=[];
    this.dateIssued='';
    if ((parseFloat(this.invoiceRate ) >0 && this.invoiceTo != "undefined" && this.descrValue != "undefined" ) ){ 
       ...
       this.ictInvoiceFlag=true;
        ...
        if(this.unitPrice == undefined || this.unitPrice == null || this.unitPrice ==''){
          this.unitPrice=0.0;
        }
        this.isHome=false;
    }else{
      this.dispatchEvent(
        new ShowToastEvent({
          title: 'Error',
          message: 'Please Provide Staff, Invoice Details and add Description',
          variant: 'Error'
        })
      );
...
handleInputChange(event) {
 ...
 handleRowAction(event) {
  ...
  handledeleteclose(event){
    ...
    generatePDF() {
     ...
     this.ictInvoiceList.forEach(rec=>{ 
       ...
       if (availableSpace > tableHeight + 30) { // Check if there's enough space to fit both the table and additional text
             doc.setFont("Roboto-Bold", "bold");
             doc.setFontSize(10);
           ...
           margin: { left: 10, right: 10 },
             theme:'grid',
             headStyles: { fillColor: [169, 169, 169],textColor: [255, 255, 255], font: "Roboto-Bold", fontStyle: "bold", halign: 'center' },
             bodyStyles: { font: "Roboto-VariableFont_wdth,wght",  fontStyle: "normal", },
    ...
    handleEdit1(event){
      this.UibreakstartTimevalue = null;
      this.UibreakendTimevalue = null;
      ...
      this.startTimeJSOn=this.satffDataJasonformat[alocId]["stringStarttime"];
      this.EndTimeJSOn=this.satffDataJasonformat[alocId]["StringEndTime"];
      this.starttime = this.convertTo24Hour(this.satffDataJasonformat[alocId]["stringStarttime"])+':00.000Z';
      console.log('this.starttime'+ this.starttime);
      this.Uistarttime = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["stringStarttime"]));
      ...
      this.EndTime = this.convertTo24Hour(this.satffDataJasonformat[alocId]["StringEndTime"])+':00.000Z';
      this.UiEndTime = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["StringEndTime"]));
      console.log('this.Endtime -------'+ this.satffDataJasonformat[alocId]["StringEndTime"]);
      ...
      console.log('this.starttime -------'+ this.satffDataJasonformat[alocId]["stringStarttime"]);
      if(this.satffDataJasonformat[alocId]["breakStartTime"]){
        console.log('breakStartTime:', this.satffDataJasonformat[alocId]?.breakStartTime);
        this.breakStartTimePickval =this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakStartTime"])+'.000Z';
        this.breakstartTimevalue = this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakStartTime"])+':00.000Z';
        this.UibreakstartTimevalue = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["breakStartTime"]));
        const breakstartTimeData = this.handleTimeSplitForField(this.UibreakstartTimevalue);
      ...
      if(this.satffDataJasonformat[alocId]["breakEndTime"]){
        this.breakEndTimePickval =this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakEndTime"])+'.000Z';
        this.breakendTimevalue = this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakEndTime"])+':00.000Z';
        this.UibreakendTimevalue = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["breakEndTime"]));
        const breakendTimeData = this.handleTimeSplitForField(this.UibreakendTimevalue);
      ...
      if(this.satffDataJasonformat[alocId]["breakStartTime2"] && this.satffDataJasonformat[alocId]["breakEndTime2"]){
        ...
        this.breakstartTime2value = this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakStartTime2"])+':00.000Z';
        this.UibreakstartTime2value = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["breakStartTime2"]));
        const breakstartTime2Data = this.handleTimeSplitForField(this.UibreakstartTime2value);
        ...
        this.breakendTime2value = this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakEndTime2"])+':00.000Z';
        this.UibreakendTime2value = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["breakEndTime2"]));
        const breakendTime2Data = this.handleTimeSplitForField(this.UibreakendTime2value);
      ...
      if(this.satffDataJasonformat[alocId]["breakStartTime3"] && this.satffDataJasonformat[alocId]["breakEndTime3"]){
       ...
       this.breakstartTime3value = this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakStartTime3"])+':00.000Z';
       this.UibreakstartTime3value = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["breakStartTime3"]));
       const breakstartTime3Data = this.handleTimeSplitForField(this.UibreakstartTime3value);
       ...
       this.breakendTime3value = this.convertTo24Hour(this.satffDataJasonformat[alocId]["breakEndTime3"])+':00.000Z';
       this.UibreakendTime3value = this.fixMidnightFormat((this.satffDataJasonformat[alocId]["breakEndTime3"]));
       const breakendTime3Data = this.handleTimeSplitForField(this.UibreakendTime3value);
      ...
      if(this.allocationStatus=='Draft' || this.allocationStatus=='Rejected' ){
        console.log('allocationStatus===>'+this.allocationStatus);
        let buttonName = event.target.name;
          if (buttonName == 'delete') {
            deleteRecord(alocId).then(() => {
    ...
    handleBreakStartEndechange1(event) {
      ...
      this.errorMessage = 'Break End Time 3 is required when Break Start Time 3 is entered.';
      this.errorMessageFlag = true;
      this.disableSaveButton = true;
      ...
      this.errorMessage = 'Break End Time 2 is required when Break Start Time 2 is entered.';
      this.errorMessageFlag = true;
      this.disableSaveButton = true;
  } else if (!this.breakstartTime2value && this.breakendTime2value) {
      this.errorMessage = 'Break Start Time 2 is required when Break End Time 2 is entered.';
      this.errorMessageFlag = true;
      this.disableSaveButton = true;
  } else {
      ...
      if(this.deleterow == 'delete'){
        ...
        this.whours = (this.totalWorkingHours - (parseFloat(this.breakTime) / 60)).toFixed(2);
    ...
    if(this.whours <= 0){
      ...
      this.disableSaveButton = false;
      if(!this.breakstartTimevalue && !this.breakendTimevalue &&
        !this.breakendTime3value && !this.breakstartTime3value &&
        !this.breakendTime2value && !this.breakstartTime2value){
    ...
    for (let i = 0; i < breakTimes.length; i++) {
        ...
        for (let j = 0; j < i; j++) { // Compare with previous breaks only
            ...
            if (
                ...
                this.errorMessage = 'Break times should not overlap.';
                console.log('Error: Break times should not overlap.'+this.errorMessage);
                this.errorMessageFlag = true;
                console.log('Error: Break times should not overlap.'+this.errorMessageFlag);
                this.disableSaveButton = true;
                return;
   ...
   try {
    this.template.querySelector('lightning-record-edit-form[data-recid="ICTEditForm"]').submit(fields);
    this.editflag1=false;
    this.handleeditClose1();
    } catch (error) {
  ...
  handleAddShiftTimeData(event) {
    ...
    console.log('data type ' + timeType);
