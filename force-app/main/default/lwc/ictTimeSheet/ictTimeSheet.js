    ...
    if(statusOfIct==''|| statusOfIct=='Rejected' || statusOfIct === "undefined" || statusOfIct === "Draft" ){
     // this.isShowModal = true;
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          title: 'Error',
          message: 'Timesheets with the status "Submitted" or "Approved" cannot be edited.',
          variant: 'Error'
        })
      );
    }
                ...
                if (result) {
                  ...
                  if(this.breakStartTime3 && this.breakEndTime3){
                    ...
                    this.dispatchEvent(
                      new ShowToastEvent({
                        title: 'Error',
                        message: 'No timesheet from the previous day is available to clone.',
                        variant: 'Error'
                      })
                    );
                    this.breaksdisable = false;
  ...
  handlecBreakhange(event) {
    if (event.target.name === "breaktime" || event.target.name === "breakTimeEdit") {
        ...
        if (regex.test(inputVal) || inputVal === "") {
            ...
            this.errorMessage = "Invalid break. Please enter a number up to 3 digits with a maximum of 2 decimal places.";
    ...
    for (let i = 0; i < breaks.length; i++) {
        ...
        if (breakStart && breakEnd) {
            ...
            if (breakStart < shiftStart || breakEnd > shiftEnd) {
                this.errorMessageFlag = true;
                this.errorMessage = 'Break time must fall within the start and end times.';
                this.disableSaveButton=true;
                console.error('Error: Break time must be within start and end time.');
                return;
            }
      ...
      let endDateChange = new Date();
       ...
       // console.log('end time validation  change '+this.endDateTimeForValidation);
          if(startDateChange <= this.startDateTimeForValidation){
            this.errorMessage='Break start time must be later than the shift start time.';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          } else if(endDateChange >= this.endDateTimeForValidation){
            this.errorMessage='Break end time must be earlier than the shift end time.';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
            ...
            this.errorMessage='Break start time must be earlier than the shift end time.';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          }else if(endDateChange <= this.startDateTimeForValidation){
            this.errorMessage='Break end time must be later than the shift start time.';
            this.errorMessageFlag=true;
            this.disableSaveButton=true;
          }else{
            this.errorMessage='';
  ...
  handledeleteclose(event){
    ...
    for (let i = 0; i < breakTimes.length; i++) {
        ...
        if (breakStart < shiftStart || breakEnd > shiftEnd) {
            ...
            this.errorMessageFlag = true;
            this.errorMessage = 'Break time must fall within the shift start and end times.';
            this.disableSaveButton = true;
            return;
        }
  ...
  triggerBreakChange1('Break_End_Time3__c', this.breakendTime3value);
    ...
    paginationHelper() {
      ...
      let sourceRecords = this.filteredRecords || this.paginationRecords;
        ...
        console.log('this.finalStaffData in paginationHelper:', JSON.stringify(this.finalStaffData)); 
    ...
    handleSearchName(event) {
      ...
      if (!this.searchName) {
          ...
          this.filteredRecords = this.paginationRecords.filter(staff =>
              (staff.Name && staff.Name.toLowerCase().includes(this.searchName)) ||
              (staff.Last_Name__c && staff.Last_Name__c.toLowerCase().includes(this.searchName))
          );
      }
