...
export default class ServiceSupportPlan extends NavigationMixin(LightningElement) {
    ...
    @wire(getBulkServicesHandler, { PartcipantIdList: '$partcipantIdList', sDate: '$startDate', eDate: '$endDate',  orgid: '$orgIdValue'})
    ...
    generateBase64Data() {
        ...
        generateOldPdf(){
            try {
                ...
                console.log('this.invRecords[0] >>'+JSON.stringify(this.invRecords[0]));
                const { jsPDF } = window.jspdf;
                var doc = new jsPDF();
                const invoice = this.invRecords[0];
