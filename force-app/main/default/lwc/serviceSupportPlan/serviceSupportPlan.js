...
export default class ServiceSupportPlan extends NavigationMixin(LightningElement) {
  ...
  this.sectionIcons[sectionId] = sectionElement.classList.contains('hidden-section') ? '\u2B9C' : '\u2B9F';
    ...
    @wire(serviceSupportList, { clientId: '$clientId' , sDate: '$startDate', eDate: '$endDate'})
    ...
    @wire(getBulkServicesHandler, { PartcipantIdList: '$partcipantIdList', sDate: '$startDate', eDate: '$endDate' ,orgid:'$orgId'})
          ...
          this.showLoadingSpinner = true; 
            this.bulkServices = result.data.map(invoice => {
                ...
                if (invoice.Participant__r) {
                  //  console.log('invoice.Participant__r ==> '+JSON.stringify(invoice.Participant__r ))
                   fileReference = invoice.Participant__r.First_Name__c ? invoice.Participant__r.First_Name__c :'' +' '+ invoice.Participant__r.First_Name__c ? invoice.Participant__r.First_Name__c:'' ;
   ...
   /*  handleError(event){    
    ...
    async getSelectedRec() {   
        ...
        if (selectedRecords.length > 0) {
            ...
            try {
                const participantServices = await getSupportData({ SPIdList: selectedRecords });
                console.log('result in serviceSupportList:', JSON.stringify(participantServices)); 
                const companyDetails = await getCompanyAndAccountData({ participantIdList: JSON.stringify([this.clientId]) });
                console.log('✅ Result for participantId', JSON.stringify(companyDetails));
            ...
            if (noDataNames.length > 0) {
              ...
              //  console.warn("🚫 Participants with no data:", noDataNames);
               ...
               // ✅ Now after building sales entry, call update
                ...
                console.log('✅ Updated for participantId', JSON.stringify(updateResult));
       ...
       /*  getNDISServiceLineItem({ ServiceItemNames: this.ndisName,ServiceDate: this.selectedShiftDate}).then(result => {
         ...
         this.serviceGroupName=[];
           ...
           //  this.NdisServiceGroupNameinEdit=true;
            ...
            console.log('this.serviceGroupName >>'+ JSON.stringify(this.serviceGroupName));
            ...
            if (Array.isArray(result.catalogueData)) {
                ...
                this.showfunds=false;
                console.log('✅ Mapped serviceEditGroupName:', JSON.stringify(this.serviceGroupName));
            } 
        })    
            .catch(error => {
                console.error('Error:', error);
        ...
        switch (actionName) {
            case 'edit':
                ...
                if (this.stateValue) {
                    ...
                    .then((selectedItems) => {
                        console.log('Selected Support Items from Apex:', JSON.stringify(selectedItems));
                        const selectedIds = selectedItems.map(item => item.NDIS_Support_Catalogue__c);
                    ...
                    .then((catalogData) => {
                        ...
                        this.NdisServiceGroupNameinEdit = true;
                        console.log('Updated Service Group Name List:', JSON.stringify(this.serviceGroupName));
                    })
                    .catch(error => {
                        console.error('Error fetching selected support items or catalog items:', error);
                        this.serviceGroupName = [];
                    });
                }
    ...
    handleSelectAll(event) {
        ...
        console.log('All selectedRecordMap --> ', Array.from(this.selectedRecordMap.entries()));
    }
    handleCheckboxChange(event) {
      ...
      //  this.selectedRecordMap = new Map();
        if(event.currentTarget.checked){
            console.log(' in if ');
            this.selectedRecordMap.set(recordId, status);
             console.log('this.selectedRecordMap'+JSON.stringify(this.selectedRecordMap));
        } else {
               console.log(' in else ');
            this.selectedRecordMap.delete(recordId);
        }
     console.log('this.selectedRecordMap ==> 1'+JSON.stringify(this.selectedRecordMap));
        this.checkStatus = Array.from(this.selectedRecordMap.values()).join(',');
  ...
  *  serviceChange(event){
    ...
    get isServiceGroupVisible() {
        return this.stateValue && this.serviceGroupName && this.serviceGroupName.length > 0;
    }
        ...
        if (uistatus === 'Invoice Generated') {
            this.dispatchEvent(
                new ShowToastEvent({
                    ...
                    message: 'You cannot delete this service because an invoice has already been generated.',
                    variant: 'error'
                })
            );
            return;
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Action Not Allowed',
                    message: 'You cannot delete this service as it has already been completed.',
                    variant: 'error'
                })
            );
            return;
