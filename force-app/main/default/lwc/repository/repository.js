...
export default class Repository extends NavigationMixin(LightningElement) {
   ...
   /*  userTypeOption = [
    ...
    handleUserSelection(event) {
        ...
        if (isChecked) {
            // Add user ID to the selected list
            if (!this.selectedUserIds.includes(userId)) {
                this.selectedUserIds.push(userId);
            }
        } else {
            // Remove user ID from the selected list
            this.selectedUserIds = this.selectedUserIds.filter(id => id !== userId);
        }

        console.log('Selected User Ids:', JSON.stringify(this.selectedUserIds));
        this.checkSubmitButtonState(); // Check button state after user selection
    }
    ...
    wiredUsers(result) {
        console.log('wiredUsers is triggered');
    ...
    checkSubmitButtonState() {
        // Enable Submit button only if at least one user is selected and "View" is checked
        this.saveDisabled = !(this.selectedUserIds.length > 0 && this.isViewFlag);
    }
    ...
    handleSubmitPermissions(event){
        ...
        insertRepositorySecurity({ repositoryId: this.selectedRowId,selectedUsers: this.selectedUserIds, isView: this.isViewFlag}).then(() => {
