                ...
                if (result) {
                    result.forEach((record) => {
                        let tempConRec = {
                            ...
                            fileSize: this.formatFileSize(parseInt(record.Size__c))
                        };
...
//ENABLE THIS AFTER IMPLEMENTING WORD IN TSIGN
      ...
      //  console.log('base64>> ',this.base64FileData);
        ...
        if(this.UpdateDetails == false){
          ...
          //  console.log('in is edit false'); 
            ...
            uploadFile({base64: JSON.stringify(this.base64FileData), filename:this.fileName, recordId:this.recordId,obj:'agree'})
            .then(result => {
    ...
    fetchSignatureStatus() {
        ...
        let promises = this.data.map(attachment => {
            console.log(`📄 Checking signature status for Record ID: ${attachment.Id}`);
    
            return checkSignatureStatus({ recordId: attachment.Id })
                .then(status => {
                    console.log(`✅ Signature status fetched for Record ID: ${attachment.Id}`, status);
                ...
                .catch(error => {
                    console.error(`❌ Error fetching signature status for Record ID: ${attachment.Id}`, error);
