...
export default class TesseractAppsRosterCreation extends LightningElement {
  ...
  fetchInitialData() {
    organizationDetails().then((response) => {
      ...
      console.log(' type of org '+response.listofPriceBook.Other_than_NDIS_User__c);
      ...
      .then((response) => {
        if (response) {
          ...
          this.fundOption = response.map((rec) => {
            return { label: rec.Registration_Group__c, value: rec.Id,Plan_Type__c:rec.Plan_Type__c };
          });
    ...
    if (event.target.name == "serviceStaff") {
      ...
      getOverlappingShiftsData({
        ...
        if (result == true) {
          this.confirMationMessage(
            "Error",
            "The selected staff already has a shift within the start and end times.",
            "Error"
          );
          this.isDisableSaveButton = true;
        } else {
  ...
  handleUnAllocated(event) {
    ...
    if (parseInt(event.currentTarget.dataset.unallocated) > 0) {
      ...
      this.confirMationMessage(
        "Error",
        "The quantity must be greater than zero before assignment.",
        "Error"
      );
    }
  }
  ...
  getServiceStaffHourlyRate(isHoliday, dayName, staffId, shiftType) {
    ...
    console.log("staffRate " + JSON.stringify(staffRate));
  ...
  handleDeleteShift(event) {
    ...
    if (
      ...
      this.confirMationMessage(
        "Error",
        "Cannot delete shift while it is In Progress or Completed.",
        "error"
      );
  ...
  handleCreateShift() {
    ...
    console.log(
      "isSingleClassForServiceCreation =====> " + this.isSingleClassForServiceCreation
    );
    ...
    getFundsData({ fundsId: serviceTypeIdList })
      .then((result) => {
        ...
        this.SplitShiftRows.forEach((row) => {
          ...
          console.log(
            `➕ Row: serviceTypeId=${row.serviceTypeId}, duration=${row.duration}, amount=${row.serviceAmount}, total=${totalAmount}`
          );
        ...
        usageMap.forEach((usedAmount, serviceTypeId) => {
          ...
          if (fund) {
            ...
            console.log(
              `🔍 Checking serviceTypeId=${serviceTypeId}: used=${usedAmount}, spent=${fund.spent}, total=${totalSpent}, available=${fund.available}`
            );
