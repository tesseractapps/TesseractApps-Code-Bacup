...
async  validateRecurringOptions() {
  ...
  handleParticipantChange(event) {
    ...
    getClientFunds({ clientId: newParticipant,shiftName:this.addShiftData.AddShiftTypeName })
      .then((response) => {
      ...
      getCatalogueData({
        ...
        .then((result) => {
          ...
          this.serviceGroupName = catalogueData.map((rec) => {
            return {
              ...rec,
              amount: (this.otherThanNdis == true || (this.otherThanNdis == false &&   result.catalogueData[0].Name.includes("Miscellaneous") ))? result.amount ||0 : rec[stateField] || 0 ,
