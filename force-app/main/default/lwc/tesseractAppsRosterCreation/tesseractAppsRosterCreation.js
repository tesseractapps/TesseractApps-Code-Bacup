      ...
      getAddShiftDataById({shiftId:this.shiftStaffId}).then(result=>{
       ...
       console.log('shift in edit '+JSON.stringify(shift));
        const buildRow = (label, startMs, endMs, rate, duration, index,sourceLabel) => {
          /*   console.log('📦 buildRow called with:', {
                label,
                startMs,
                endMs,
                rate,
...
deleteRateRow(event) {
  ...
  this.isDisableSaveButton = false;
  console.log('this.rateRows before remove => '+JSON.stringify(this.rateRows));
  console.log('this.LongShiftTimeSlots before remove==> '+JSON.stringify(this.LongShiftTimeSlots));
   ...
   console.log('this.LongShiftTimeSlots after nullfy remove==> '+JSON.stringify(this.LongShiftTimeSlots));
   ...
   // this.LongShiftTimeSlots={};
      ...
      // Sort rateRows by index to maintain correct order
      this.rateRows.sort((a, b) => a.index - b.index);

      this.rateRows.forEach(row => {
      const baseType = row.rowShiftType;

      // Track how many times this type appears
      ...
      const uniqueKey = typeCount[baseType] === 1 ? baseType : `${baseType} ${typeCount[baseType]}`;

      shiftTypeMap[uniqueKey] = { ...row };
      });

      this.LongShiftTimeSlots = shiftTypeMap;

      console.log('rateRows after delete:', JSON.stringify(this.rateRows));
      console.log('LongShiftTimeSlots after delete:', JSON.stringify(this.LongShiftTimeSlots));
...
handleRateRowTimeChange(event) {
  ...
  console.log('updated rate rows '+JSON.stringify(this.rateRows));
    ...
    if (!startDateStr || !endDateStr || !shiftStartTimeStr || !shiftEndTimeStr) {
     console.log('❌ Missing shift time data. Skipping processing.');
    return;
    }
    ...
    console.log('🟢 Parsed Shift Start Time:', shiftStartHour, shiftStartMin);
    console.log('🟢 Parsed Shift End Time:', shiftEndHour, shiftEndMin);
    ...
    console.log('📆 Constructed shiftStart:', shiftStart.toISOString());
    console.log('📆 Constructed shiftEnd:', shiftEnd.toISOString());
 ...
 for (let i = 0; i < this.rateRows.length; i++) {
  ...
  console.log(`🌀 Loop Row ${i + 1} -- Type: ${rowShiftType}, Start: ${startTime} (${startAmPm}), End: ${endTime} (${endAmPm})`);

  if (!startTime || !endTime || !rowShiftType) {
    console.warn(`⚠️ Row ${i + 1} skipped due to missing values`);
    continue;
  }
  ...
  if (i === 0) {
    ...
    console.log(`🕒 Row ${i + 1} Initial Start: ${rowStart.toISOString()}`);
    ...
    console.log(`🔄 Row ${i + 1} Derived Start (prev end): ${rowStart.toISOString()}`);
  ...
  if (rowEnd <= rowStart) {
    console.warn(`⚠️ Row ${i + 1} End before Start — adjusting`);
    rowEnd.setDate(rowEnd.getDate() + 1);
  }
  ...
  console.log(`✅ Row ${i + 1} Start: ${rowStart.toISOString()}, End: ${rowEnd.toISOString()}, Duration: ${this.rateRows[i].duration}`);

  // VALIDATIONS
  if (rowEnd <= rowStart) {
    this.confirMationMessage('Invalid Time', `Row ${i + 1}: End time must be after start time`, 'error');
    console.error(`❌ Row ${i + 1} Validation Error: End before Start`);
    hasError = true;
  }
  ...
  if (i > 0) {
    if (lastRowEnd && rowStart < lastRowEnd) {
      ...
      console.error(`❌ Row ${i + 1} Validation Error: Overlap with previous row`);
      hasError = true;
    }
    ...
    if (lastRowEnd && rowStart.getTime() !== lastRowEnd.getTime()) {
      ...
      console.error(`❌ Row ${i + 1} Validation Error: Gap from previous row`);
      hasError = true;
    }
    ...
    if (rowStart < shiftStart || rowEnd > shiftEnd) {
      ...
      console.error(`❌ Row ${i + 1} Validation Error: Outside shift time`);
      hasError = true;
    }
  }
...
console.log('updated rate rows  11'+JSON.stringify(this.rateRows));
  ...
  this.LongShiftTimeSlots = shiftTypeMap; */
    ...
    this.LongShiftTimeSlots = shiftTypeMap;
   console.log('rateRows IN TIME change: :', JSON.stringify(this.rateRows));
      console.log('LongShiftTimeSlots  IN TIME change:', JSON.stringify(this.LongShiftTimeSlots));
    ...
    .then(result => {
     ...
     if(this.validateInputs() && this.validateAddress() && this.addShiftData.AddShiftDuration >0){
        createAddShift({addshiftData:JSON.stringify(this.finalAddShiftData),isRecurring:this.AddShiftRecurringCheckboxValue,typeOfRecur:this.RecurValue,
          ...
          const shift=result.shiftwithstaffdata;
          console.log('shift in edit '+JSON.stringify(shift));
         const buildRow = (label, startMs, endMs, rate, duration,index) => {
            const startTime24 = this.formatMillisecondsToTime(startMs);
            const endTime24 = this.formatMillisecondsToTime(endMs);
        ...
        const tryBuildRow = (label, isActive, startTime, endTime, rate, duration, index) => {
              if (isActive && startTime && endTime) {
          rows.push(buildRow(label, startTime, endTime, rate, duration, index));
          }
        };

        tryBuildRow('Morning', shift.Is_long_Morning__c, shift.Long_Morning_Start_Time__c, shift.Long_Morning_End_Time__c, shift.Long_Morning_Hourly_Rate__c, shift.Long_Morning_Shift_Duartion__c, shift.Long_Morning_Index__c);
        tryBuildRow('Afternoon', shift.Is_Long_Afternoon__c, shift.Long_Afternoon_Start_Time__c, shift.Long_Afternoon_End_Time__c, shift.Long_Afternoon_Hourly_Rate__c, shift.Long_Afternoon_Shift_Duartion__c, shift.Long_Afternoon_Index__c);
        tryBuildRow('Night', shift.Is_Long_Night_Shift__c, shift.Long_Night_Start_Time__c, shift.Long_Night_End_Time__c, shift.Long_Night_Hourly_Rate__c, shift.Long_Night_Shift_Duartion__c, shift.Long_Night_Index__c);
        tryBuildRow('Sleepover Shift', shift.Is_Long_SleepOver__c, shift.Long_Sleepover_Start_Time__c, shift.Long_Sleepover_End_Time__c, shift.Long_Sleepover_Allowance__c, shift.Long_Sleepover_Duartion__c, shift.Long_Sleepover_index__c);
        tryBuildRow('Morning', shift.Is_long_Morning_Shift_two__c, shift.Long_Morning_Start_Time_Two__c, shift.Long_Morning_End_Time_Two__c, shift.Long_Morning_Rate_Two__c, shift.Long_Morning_Duartion_Two__c, shift.Long_Morning_Index_Two__c);
        tryBuildRow('Afternoon', shift.Is_Long_afternoon_shift_two__c, shift.Long_Afternoon_Start_Time_two__c, shift.Long_afternoon_End_Time_Two__c, shift.Long_Afternoon_Rate_Two__c, shift.Long_Afternoon_Duration_Two__c, shift.Long_Afternoon_Index_Two__c);
        console.log('rows :  1112 ', JSON.stringify(rows)); 
        this.rateRows = rows;
      this.IsLongShift = this.rateRows.length >= 1;
