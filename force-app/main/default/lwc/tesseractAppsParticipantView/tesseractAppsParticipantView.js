...
import updateUnAssignedShifts from '@salesforce/apex/RosterAutoScheduleTriggerHandler.updateUnAssignedShifts';
...
export default class TesseractAppsParticipantView extends LightningElement {
    ...
    @track rows = [
      ...
      { id: '4', label: 'Nominated Staff', isCheckbox: true, value: false },
  ...
  handleholidayPopup() {
    console.log('current recurrenceDatesList before removing holidaysList dates:', JSON.stringify(this.recurrenceDatesList));
    console.log('holidaysList:', JSON.stringify(this.holidaysList));

    // Extract all holiday dates from holidaysList
    ...
    console.log('this.holidayPopup >>', this.holidayPopup);
  }
  ...
  handleholidayChange(event) {
    ...
    if (isChecked) {
        // ✅ Add date to selectedHolidayDates if it's not already in the list
        if (!this.selectedHolidayDates.includes(date)) {
            this.selectedHolidayDates.push(date);
            console.log('Date added to selectedHolidayDates:', date);
        } else {
            console.log('Date already exists in selectedHolidayDates:', date);
        }

        // ✅ Remove date from unselectedHolidayDates
        this.unselectedHolidayDates = this.unselectedHolidayDates.filter(d => d !== date);
    } else {
        // ✅ Remove date from selectedHolidayDates
        this.selectedHolidayDates = this.selectedHolidayDates.filter(d => d !== date);

        // ✅ Add date to unselectedHolidayDates
    ...
    // Set removeHolidayDate to the current holidaysList
    this.removeHolidayDate = this.holidaysList;

    console.log('current removeHolidayDate after removal:', JSON.stringify(this.removeHolidayDate));
    console.log('current recurrenceDatesList before filtering:', JSON.stringify(this.recurrenceDatesList));
    console.log('unselectedHolidayDates:', JSON.stringify(this.unselectedHolidayDates));

    // ✅ Remove holiday dates from recurrenceDatesList, except the selected ones
    if (this.removeHolidayDate && this.selectedHolidayDates.length > 0) {
        const selectedSet = new Set(this.selectedHolidayDates);
        const holidaysToRemove = this.removeHolidayDate
            .map(h => h.Date__c)
            .filter(holidayDate => !selectedSet.has(holidayDate)); // keep selected, remove others

        this.recurrenceDatesList = this.recurrenceDatesList.filter(
            date => !holidaysToRemove.includes(date)
        );
    }
  ...
  handleSearchName(event) {
   ...
   // this.isShowSpinner = true;
    ...
    handleRateRowTimeChange(event) {
     ...
     // console.log('this.rateRows '+ JSON.stringify(this.rateRows));
      ...
      if (!hasError && this.rateRows.length > 0) {
        ...
        if (this.rateRows.length > 1) {
          const lastRow = this.rateRows[this.rateRows.length - 1];
          const [lH, lM] = lastRow.endTime?.split(':');
          let lastEnd = new Date(`${endDateStr}T${lH}:${lM}:00`);
    ...
    handleRun() {
      ...
      this.isShowSpinner=true;
      updateUnAssignedShifts({ startDate: this.startDate, endDate: this.endDate })
      .then(() => {
