  ...
  getOrganisationTimings() {
    ...
    switch (this.addShiftData.AddShiftType) {
      case "Morning":
        ...
        this.addShiftData.AddShiftStartTime = this.convertTo24HourFormat(
          this.organisationShiftTimes.Morning_Shift_Start_Time__c.toLowerCase()
        );
        this.addShiftData.AddShiftEndTime = this.convertTo24HourFormat(
      ...
      .then((result) => {
        switch (this.addShiftData.AddShiftType) {
          case "Morning":
            if (
              !this.validateShiftFields(
                result.Morning_Shift_Start_Time__c,
                result.Morning_Shift_End_Time__c
              )
            )
              return;
            this.addShiftData.AddShiftStartTimeAMPM =
              result.Morning_Shift_Start_Time__c?.toUpperCase();
            this.addShiftData.AddShiftEndTimeAMPM =
              result.Morning_Shift_End_Time__c?.toUpperCase();
            this.addShiftData.AddShiftStartTime = this.convertTo24HourFormat(
              result.Morning_Shift_Start_Time__c?.toLowerCase()
            );
            this.addShiftData.AddShiftEndTime = this.convertTo24HourFormat(
              result.Morning_Shift_End_Time__c?.toLowerCase()
            );
            this.checkFatigue();
            break;
          ...
          case "Afternoon":
            if (
              !this.validateShiftFields(
                result.Afternoon_Shift_Start_Time__c,
                result.Afternoon_Shift_End_Time__c
              )
            )
              return;
            this.addShiftData.AddShiftStartTimeAMPM =
              result.Afternoon_Shift_Start_Time__c?.toUpperCase();
            this.addShiftData.AddShiftEndTimeAMPM =
              result.Afternoon_Shift_End_Time__c?.toUpperCase();
            this.addShiftData.AddShiftStartTime = this.convertTo24HourFormat(
              result.Afternoon_Shift_Start_Time__c?.toLowerCase()
            );
            this.addShiftData.AddShiftEndTime = this.convertTo24HourFormat(
              result.Afternoon_Shift_End_Time__c?.toLowerCase()
            );
            this.checkFatigue();
            break;
          ...
          case "Night":
            if (
              !this.validateShiftFields(
                result.Night_Shift_Start_Time__c,
                result.Night_Shift_End_Time__c
              )
            )
              return;
            this.addShiftData.AddShiftStartTimeAMPM =
              result.Night_Shift_Start_Time__c?.toUpperCase();
            this.addShiftData.AddShiftEndTimeAMPM =
              result.Night_Shift_End_Time__c?.toUpperCase();
            this.addShiftData.AddShiftStartTime = this.convertTo24HourFormat(
              result.Night_Shift_Start_Time__c?.toLowerCase()
            );
            this.addShiftData.AddShiftEndTime = this.convertTo24HourFormat(
              result.Night_Shift_End_Time__c?.toLowerCase()
            );
            //  this.disableTimeButton=true;
            this.checkFatigue();
            break;
          ...
          case "General":
            if (
              !this.validateShiftFields(
                result.General_Shift_Start_Time__c,
                result.General_Shift_End_Time__c
              )
            )
              return;
            this.addShiftData.AddShiftStartTimeAMPM =
              result.General_Shift_Start_Time__c?.toUpperCase();
            this.addShiftData.AddShiftEndTimeAMPM =
              result.General_Shift_End_Time__c?.toUpperCase();
            this.addShiftData.AddShiftStartTime = this.convertTo24HourFormat(
              result.General_Shift_Start_Time__c?.toLowerCase()
            );
            this.addShiftData.AddShiftEndTime = this.convertTo24HourFormat(
              result.General_Shift_End_Time__c?.toLowerCase()
            );
            //   this.disableTimeButton=true;
            this.checkFatigue();
            break;
          ...
          case "Custom":
            if (
              !this.validateShiftFields(
                result.Custom_Shift_Start_Time__c,
                result.Custom_Shift_End_Time__c
              )
            )
              return;
            this.IsLongShift = true;
            this.addShiftData.AddShiftStartTimeAMPM =
              result.Custom_Shift_Start_Time__c?.toUpperCase();
            this.addShiftData.AddShiftEndTimeAMPM =
              result.Custom_Shift_End_Time__c?.toUpperCase();
            this.addShiftData.AddShiftStartTime = this.convertTo24HourFormat(
              result.Custom_Shift_Start_Time__c?.toLowerCase()
            );
            this.addShiftData.AddShiftEndTime = this.convertTo24HourFormat(
              result.Custom_Shift_End_Time__c?.toLowerCase()
            );
            //   this.disableTimeButton=false;
          ...
          case "Sleepover Shift":
            if (
              !this.validateShiftFields(
                result.Sleepover_Shift_Strat_Time__c,
                result.Sleepover_Shift_End_Time__c
              )
            )
              return;
            this.addShiftData.AddShiftStartTimeAMPM =
              result.Sleepover_Shift_Strat_Time__c?.toUpperCase();
            this.addShiftData.AddShiftEndTimeAMPM =
              result.Sleepover_Shift_End_Time__c?.toUpperCase();
            this.addShiftData.AddShiftStartTime = this.convertTo24HourFormat(
              result.Sleepover_Shift_Strat_Time__c?.toLowerCase()
            );
            this.addShiftData.AddShiftEndTime = this.convertTo24HourFormat(
              result.Sleepover_Shift_End_Time__c?.toLowerCase()
            );
            // this.disableTimeButton=false;
            this.checkFatigue();
            break;
          ...
          default:
            console.warn(
              `Unknown shift type: ${this.addShiftData.AddShiftType}`
            );
            this.disableTimeButton = false;
            break;
        }
  ...
  handleServiceStaffchange(event) {
    ...
    getOverlappingShiftsData({
      ...
      if (result == true) {
        this.confirMationMessage(
          "Error",
          "The selected staff already has a shift within the start and end times. Please switch to the staff view and assign the services to the particular participant.",
          "Error"
        );
        this.isDisableSaveButton = true;
      } else {
 ...
 */
  ...
  validateCustomShifts(cleanedSegments) {
    ...
    const payload = {
      shiftStartDate: this.addShiftData.AddShiftStartDate, // format: "YYYY-MM-DD"
      shiftStartTime: this.addShiftData.AddShiftStartTime, // format: "HH:mm:ssZ"
    ...
    validateSegments(payload)
      .then((result) => {
        ...
        // Handle other cases where result.isValid might be false or there might be error messages
        if (!result.isValid) {
          this.isDisableSaveButton = !result.isValid;
          this.confirMationMessage(
            "Invalid Time",
            result.errorMessage,
            "error"
        ...
        getPublishData({
          ...
          .then((result) => {
            ...
            if (!result || result.length === 0) {
              ...
              const hasPublished = result.some(
                (shift) =>
                  shift.Is_Auto_Schedule_Shift__c === true &&
                  shift.Is_Published_Shifts__c === true
              );
              const hasUnpublished = result.some(
                (shift) =>
                  shift.Is_Auto_Schedule_Shift__c === true &&
                  shift.Is_Published_Shifts__c === false
              );

              if (hasPublished) {
                this.shiftPublishStatus = "Published";
              } else if (hasUnpublished) {
                this.shiftPublishStatus = "Unpublished changes";
              } else {
                this.shiftPublishStatus = "";
              }
            }
  ...
  handleCreateShift() {
    ...
    if (this.isEditShiftScreenFlag == false) {
      successMessage =
        "Your shift has been successfully created. Please proceed with adding staff/participants to this shift.";
    } else {
      successMessage =
        "Your shift has been successfully updated. Please proceed with adding staff/participants to this shift.";
    }
    if (
      ...
      .then((result) => {
        ...
        this.SplitShiftRows.forEach((row, index) => {
          if (
            ...
            console.warn(
              `⚠️ Skipping row [${index}] due to missing data:`,
              row
            );
            return;
          }
          ...
          if (this.AddShiftRecurringCheckboxValue && this.numberOfOccurrences) {
            totalAmount *= this.numberOfOccurrences;
            console.log(
              `🔁 Recurrence applied: ${this.numberOfOccurrences} times → totalAmount = ${totalAmount}`
            );
          }

          console.log(
            `➕ Row [${index}] - serviceTypeId=${row.serviceTypeId}, duration=${row.duration}, rate=${row.serviceAmount}, total=${totalAmount}`
          );
          ...
          if (usageMap.has(row.serviceTypeId)) {
            ...
            console.log(
              `🆕 Initialized usageMap[${row.serviceTypeId}] = ${totalAmount}`
            );
          }
        });
        ...
        usageMap.forEach((usedAmount, serviceTypeId) => {
          ...
          if (fund) {
            ...
            // ✅ Only check if the *current usage* exceeds available
        ...
        if (exceededServices.length > 0) {
          ...
          const message = exceededServices
            .map((s) => {
              ...
              if (this.AddShiftRecurringCheckboxValue) {
                const totalRecurring = s.usedAmount * this.numberOfOccurrences;
                console.log("totalRecurring >>", totalRecurring);
