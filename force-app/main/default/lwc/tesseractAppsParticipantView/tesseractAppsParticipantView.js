...
async  handleAddShiftTimeData(event) {
  ...
  getFundOptions() {
    if (this.serviceParticipant) {
      getClientFunds({ clientId: this.serviceParticipant,shiftName:this.addShiftData.AddShiftTypeName })
        .then((response) => {
      ...
      getCatalogueData({
        ...
        .then((result) => {
          ...
          this.serviceGroupName = catalogueData.map((rec) => {
            return {
              ...rec,
              amount: (this.otherThanNdis == true || (this.otherThanNdis == false &&  result.catalogueData[0].Name=='Miscellaneous' ))? result.amount ||0 : rec[stateField] || 0 ,
              Support_Item_Name__c: (this.otherThanNdis == true || (this.otherThanNdis == false &&   result.catalogueData[0].Name.includes("Miscellaneous") ))? result.editedCatalogName ?  result.editedCatalogName:result.catalogueData[0].Support_Item_Name__c : result.catalogueData[0].Support_Item_Name__c 
            };
          });
...
async  handleEditShiftScreen(event) {
    ...
    getAddShiftDataById({ shiftId: this.shiftStaffId }).then((result) => {
      ...
      this.shiftStatusUnassigned = result.shiftwithstaffdata.Status__c;
      this.shiftwithstaffIdUnassigned = result.shiftwithstaffdata.Id;
      console.log('this.shiftStatusUnassigned >>>', this.shiftStatusUnassigned);
      console.log('this.shiftwithstaffIdUnassigned >>>', this.shiftwithstaffIdUnassigned);
      this.addShiftData = {
      ...
      console.log('this.addShiftData.AddShiftStatus==>  '+this.addShiftData.AddShiftStatus ) ; 
      ...
      setTimeout(() => {
        ...
        this.fetchStaffRoles();
            ...
            if (this.addShiftData.AddShiftStatus === "Unassigned") {
                            ...
                            unAssignedList({ shiftWithStaffId: this.ShiftwithStafftoApexId })
                              .then((result) => {
                                console.log("👉 First .then() result:", JSON.stringify(result));

                                if (result && result.length > 0) {
                                  this.ServiceTypeIdInParticipant = result[0].Funds_Tracker__c;
                                  console.log("✅ Assigned ServiceTypeIdInParticipant:", this.ServiceTypeIdInParticipant);

                                  this.serviceParticipant = result[0].Client__c;
                                  console.log("✅ Assigned serviceParticipant:", this.serviceParticipant);

                                  this.serviceTypeIdFromShift = result[0].Service_Type__c; // ✅ reuse existing property if you already have one
                                  console.log("✅ Assigned serviceTypeIdFromShift:", this.serviceTypeIdFromShift);

                                  this.NdisServiceGroupName = true;
                                  console.log("✅ NdisServiceGroupName set:", this.NdisServiceGroupName);

                                  return getClientFunds({ clientId: this.serviceParticipant,shiftName:this.addShiftData.AddShiftTypeName });
                                }
                              })
                              .then((response) => {
                                console.log("👉 Second .then() response:", JSON.stringify(response));

                                if (response) {
                                  this.TotalFunds = response;
                                  console.log("✅ Assigned TotalFunds:", JSON.stringify(this.TotalFunds));
                                  ...
                                  console.log("✅ Assigned fundOption:", JSON.stringify(this.fundOption));

                                  this.servicePlan =
                                    this.TotalFunds.find(
                                      (opt) => opt.Id === this.ServiceTypeIdInParticipant
                                    )?.Plan_Type__c || "";
                                  console.log("✅ Assigned servicePlan:", this.servicePlan);
 
                                  this.SplitShiftRows = this.SplitShiftRows.map((row) => ({
                                    ...row,
                                    serviceTypeId: this.ServiceTypeIdInParticipant,
                                  }));
                                  console.log("✅ Updated SplitShiftRows:", JSON.stringify(this.SplitShiftRows));
                
                                     console.log("✅ Assigned ServiceTypeIdInParticipant ==>:", this.ServiceTypeIdInParticipant);

                                  return getCatalogueData({
                                    serviceType: String(this.ServiceTypeIdInParticipant),
                                    clientId: this.serviceParticipant,
                                  });
                                }
                              })
                              .then((catLogResult) => {
                                console.log("👉 Third .then() catLogResult:", JSON.stringify(catLogResult));

                                if (catLogResult) {
                                  const catalogueData = catLogResult.catalogueData;
                                  const stateField = catLogResult.statesCombined;

                                  this.stateValue = stateField;
                                  console.log("✅ Assigned stateValue:", this.stateValue);

                                  this.serviceGroupName = catalogueData.map((rec) => ({
                                    ...rec,
                                    amount:
                                      (this.otherThanNdis == true || (this.otherThanNdis == false &&   result.catalogueData[0].Name.includes("Miscellaneous") ))? result.amount ||0 : rec[stateField] || 0,
                                    isSelected: this.serviceTypeIdFromShift === rec.Id,
                                     Support_Item_Name__c: (this.otherThanNdis == true || (this.otherThanNdis == false &&   result.catalogueData[0].Name.includes("Miscellaneous") ))? result.editedCatalogName ?  result.editedCatalogName:result.catalogueData[0].Support_Item_Name__c : result.catalogueData[0].Support_Item_Name__c 
                                  }));
                                  console.log("✅ Assigned serviceGroupName:", JSON.stringify(this.serviceGroupName));
                                    if (this.addShiftData.AddShiftType === "Custom") {
                                      let shifNames = this.shiftNameOptions.filter(
                                            (rec) => rec.value === this.addShiftData.AddShiftTypeName
                                            );
                                            console.log("shifNameOptions in unassigned  ==>" + JSON.stringify(shifNames));
                                            if(shifNames !=null && shifNames!= undefined ){
                                                 this.getShiftTimingsByFacility(shifNames);
                                        }
                                  ...
                                  this.serviceNdisSelection(this.serviceTypeIdFromShift);
                                  console.log("✅ serviceNdisSelection called with:", this.serviceTypeIdFromShift);
 ...
 */
  ...
  async handleShowMoreModal(event) {
    ...
    getServicesByDate({
      ...
      this.moreShiftlist = result.map((rec, idx) => {
        ...
        console.log(`   ▶ newInlineStyle for record [${idx}]:`, newInlineStyle);
        ...
        console.log(`✅ Final transformed record [${idx}]:`, JSON.stringify(transformed));
        return transformed;
      });

      // 🔹 Log the final shift list ready to display in modal
      console.log("📋 Final More shift list:", JSON.stringify(this.moreShiftlist));
    }).catch((error) => {
  ...
  HandleServiceEdit(event) {
    ...
    getClientFunds({ clientId: event.currentTarget.dataset.participantid ,shiftName:this.addShiftData.AddShiftTypeName})
      .then((response) => {
        console.log("funds " + JSON.stringify(response));
        if (response.length>0) {
      ...
      .then((result) => {
        ...
        if (result.enrichedSegmentsJson) {
          try {
            ...
            console.log("enriched result ", JSON.stringify( this.rateRow));
          } catch (parseError) {
                    ...
                    if (
                      ...
                      if( this.addShiftData.AddShiftStatus== 'Unassigned'){
                            ...
                            if (this.rateRows.length > 0) {
                              ...
                              console.log('this.shiftStatusUnassigned in Create if >>>', this.shiftStatusUnassigned);
                              console.log('this.shiftwithstaffIdUnassigned in Create if >>>', this.shiftwithstaffIdUnassigned);
                            //  console.log("Selected split shift Id " +JSON.stringify(this.SplitShiftRows));
                            console.log("shiftTypeMap " +JSON.stringify(shiftTypeMap))

                                  createUnassignedShift({
                                      shiftwithstaffIdUnassigned: this.ShiftwithStafftoApexId,
                                      staffId:this.addShiftData.AddShiftStaffValue,
                                      staffHourlyrate:this.addShiftData.AddShiftStaffHourlyRate,
                                      longShiftTimeSlots:JSON.stringify(shiftTypeMap),
                                      ndsisValue:this.SplitShiftRows[0].selectedNdisIdValue
                                  })
                                  .then(() => {
                                       this.isShowSpinner = false;
                                         this.handleRefresh();
                        ...
                        createAddShift({
                          ...
                          if (result.isSuccess) {
                            ...
                            console.log('this.shiftStatusUnassigned in Create >>>', this.shiftStatusUnassigned);
                            console.log('this.shiftwithstaffIdUnassigned in Create >>>', this.shiftwithstaffIdUnassigned);
