...
export default class OfferTemplateList extends LightningElement {
    ...
    fetchRecordData() {
        ...
        getOfferTemplateDetails({ recordId: this.recordId })
            .then((data) => {
                if (data) {
                    const expirationTime = new Date(data.Expire__c);
                    const now = new Date();
    
                    this.isLinkExpired = now > expirationTime;
    ...
    fetchRecipientSpecificData() {
        ...
        getChildSignatureDetails({ recordId: this.recordId, currentUserEmail: this.currentUserEmail })
            .then((childData) => {
                ...
                const expirationTime = new Date(childData.Expire__c);
                const now = new Date();
...
createCanvasImage(text, fontSize = 15, fontFamily = 'Arial') {
    const canvas = document.createElement('canvas');
    const context = canvas.getContext('2d');

    if (!isNaN(Date.parse(text))) { // Check if it's a valid date
        const dateObj = new Date(text);
        const month = (dateObj.getMonth() + 1).toString().padStart(2, '0'); // Get Month (0-based, so +1)
        const day = dateObj.getDate().toString().padStart(2, '0'); // Get Day
        const year = dateObj.getFullYear(); // Get Year
        text = `${month}-${day}-${year}`; // Convert to MM-DD-YYYY format
    }
    ...
    const textHeight = fontSize * 1.5; // Approximate height including line spacing
    const padding = 20; // Add some padding for the canvas
    ...
    context.fillText(
        text,
        canvas.width / 2, // Center x-coordinate
        canvas.height / 2 // Center y-coordinate
    );
...
handleEditPlaceholder(event) {
    ...
    // Determine the appropriate image source field dynamically
    let srcField = '';
    if (placeholder.isName) srcField = 'FNimageSrc';
    else if (placeholder.isLastName) srcField = 'LNimageSrc';
    else if (placeholder.isInitials) srcField = 'INimageSrc';
    else if (placeholder.isFullName) srcField = 'FullNameSrc';
    else if (placeholder.isEmail) srcField = 'EmailSrc';
    else if (placeholder.isContactNumber) srcField = 'PhoneSrc';
    else if (placeholder.isAddress) srcField = 'AddressSrc';
    else if (placeholder.isDate) srcField = 'DateSrc';
    else if (placeholder.isDateOfSigning) srcField = 'DateOfSigningSrc';
    ...
    // Store current placeholder details
    this.currentPlaceholderId = placeholder.id;
    this.currentSrcField = srcField;
    this.editableText = placeholder.value; // Set initial value in the input
...
handleModalInputChange(event) {
    this.editableText = event.target.value; // Update the editable text
}
...
saveEditedText() {
    ...
    placeholder.value = this.editableText.trim();
