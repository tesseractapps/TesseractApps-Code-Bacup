    ...
    /* loadServiceCatalogues() {
        getServiceCataloguesByClient({ clientId: this.clientId })
            .then(result => {
                ...
                // --------- Build base options ----------
                let options = Array.from(supportNameSet)
                ...
                if (this.otherThanNdis === false) {
                    ...
                    // NDIS → group Miscellaneous

                    const hasMiscChildren = options.some(opt =>
                        opt.label.startsWith('Miscellaneous -')
                    );

                    // remove:
                    //  - Miscellaneous - *
                    //  - plain Miscellaneous (we will add it once at the end)
                    options = options.filter(opt =>
                        !opt.label.startsWith('Miscellaneous -') &&
                        opt.label !== 'Miscellaneous'
                    );
                    ...
                    // Non-NDIS → group Others

                    const hasOtherChildren = options.some(opt =>
                        opt.label.startsWith('Others -')
                    );

                    // remove:
                    //  - Others - *
                    //  - plain Others (we will add it once at the end)
                    options = options.filter(opt =>
                        !opt.label.startsWith('Others -') &&
                        opt.label !== 'Others'
                    );
 ...
 previewSpentForPeriods({ clientId: this.clientId,fundTrackerId: this.recordId, periods: previewPayload })
  ...
  /*this.generatedPeriods = this.generatedPeriods.map(p => {
    ...
    return {
      ...
      // ✅ Carry forward ONLY when period is completed
      carryForward:
        p.status === 'Completed'
          ? Math.max(0, allocated - spent)
          : 0
    };
  }); */
  this.generatedPeriods = this.generatedPeriods.map(p => {
  ...
  evaluateOverspendAndToggleSave() {
      ...
      // if (!this.generatedPeriods || this.generatedPeriods.length === 0) {
      //   this.saveDisabled = false;
      //   return;
      // }

      // // Total allocated after Apex / preview cascade
      // const totalAllocated = this.generatedPeriods.reduce(
      //   (sum, p) => sum + (parseFloat(p.amount) || 0),
      //   0
      // );

      // // Total spent (Completed + Active)
      // const totalSpent = this.generatedPeriods.reduce(
      //   (sum, p) => sum + (parseFloat(p.spentAmount) || 0),
      //   0
      // );

      // // 🔴 HARD BLOCK: spent > allocated
      // if (totalSpent > totalAllocated) {
      //   this.saveDisabled = true;

      //   this.showToast(
      //     'Error',
      //     'Total spent exceeds total allocated funds. Please adjust periods or allocation.',
      //     'error'
      //   );
      // } else {
      //   // ✅ Valid cases (including exact zero balance)
      //   this.saveDisabled = false;
      // }
