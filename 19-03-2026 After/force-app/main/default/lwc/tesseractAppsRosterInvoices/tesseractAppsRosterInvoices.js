...
/* wiredServiceList(result) {
    ...
    if (result.data) {
        ...
        // ✅ FIX: Clear old data when apex returns empty
        if (participantIdsValues.length === 0) {
            console.log('⚠️ No participants found – clearing old table data');
            this.records = [];
            this.accList = [];
            this.totalRecords = 0;
        ...
        getParticipantFacilityMap({ participantIds: participantIdsValues })
            .then(participantFacilityMap => {
                ...
                // ✅ FIX: Clear old data when facility filter returns empty
                if (filterData.length === 0) {
                    console.log('⚠️ No records after facility filter – clearing old table data');
                    this.records = [];
                    this.accList = [];
                    this.totalRecords = 0;
                    this.pendingInvoices = 0;
            ...
            .catch(error => {
                // ✅ FIX: Handle getParticipantFacilityMap failure gracefully
                console.error('❌ Error in getParticipantFacilityMap:', error);
                this.records = [];
                this.accList = [];
                this.totalRecords = 0;
                this.pendingInvoices = 0;
...
/* wiredServiceList(result) {
    ...
    if (result.data) {
        ...
        // ✅ No records at all → clear immediately, don't wait for facility map
        ...
        const participantIdsValues = [...new Set(serviceData.map(r => r.Client__c))];
        ...
        getParticipantFacilityMap({ participantIds: participantIdsValues })
            .then(participantFacilityMap => {
                ...
                let filterData = serviceData.filter(rec => {
                    ...
                    // ✅ faclist can be string OR array — handle both
                ...
                // ✅ Facility filter returned nothing → show empty state cleanly
                if (filterData.length === 0) {
                    this.records         = [];
                    this.accList         = [];
                    this.totalRecords    = 0;
                ...
                // ... rest of accountingService mapping unchanged ...
