...
/* calculateWeekDaysWithDates(startOfWeek, numberOfDays = 7) {
    const days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]; // keep if used elsewhere
    const weekDaysWithDates = [];
    ...
    for (let i = 0; i < numberOfDays; i++) {
        ...
        const dayName = currentDate.toLocaleDateString("en-US", { weekday: "short" }); // ✅ FIX
...
navigateToToday() {
  ...
  async handleConfirmSetHours(event) {
    ...
    console.log('slected combo box facility =>'+this.SelectedComboBoxFacility)
    ...
    const setHours = parseFloat(event.currentTarget.dataset.sethours);
      ...
      let rolesArray=[];
        ...
        //======================================================
        // Validate Selected Facility + Role
        //======================================================

        const selectedFacilityId = this.SelectedComboBoxFacility;
        const selectedRole = role;

        let isExactMatch = false;

        // Build Facility -> Roles JSON
        let facilityRoleMap = {};

        //------------------------------------------------------
        // Step 1: Add ALL staff facilities
        //------------------------------------------------------
        if (facilityVal) { 

            facilityVal.split(',').forEach(facilityId => {

                facilityId = facilityId.trim();

                const facilityObj = this.facilityOptions.find(
                    f => f.value === facilityId
                );

                facilityRoleMap[facilityId] = {
                    facilityId,
                    facilityName: facilityObj ? facilityObj.label : facilityId,
                    staffRoles: [],
                    hasRoles: false
                };
            });
        ...
        //------------------------------------------------------
        // Step 2: Populate roles for each facility
        //------------------------------------------------------
        if (roleFacilityVal) {

            roleFacilityVal.split(',').forEach(item => {

                let [roleName, facilityId] = item.split('-');

                roleName = roleName?.trim();
                facilityId = facilityId?.trim();

                // Safety (in case a role exists for a facility not present in facilityVal)
                if (!facilityRoleMap[facilityId]) {
                ...
                facilityRoleMap[facilityId].staffRoles.push(roleName);
                facilityRoleMap[facilityId].hasRoles = true;
        ...
        console.log('Facility Role Map', JSON.stringify(facilityRoleMap));

        //------------------------------------------------------
        // Show popup if selected Facility + Role doesn't exist
        //------------------------------------------------------

        if (!isExactMatch) {

            const selectedFacility = this.facilityOptions.find(
                f => f.value === selectedFacilityId
            );
            ...
            this.invalidFacilityRoleData = {
                ...
                selectedFacilityName: selectedFacility
                    ...
                    : '',
                selectedRole,
                staffFacilities: Object.values(facilityRoleMap)
            };

            console.log(
                'invalidFacilityRoleData',
                JSON.stringify(this.invalidFacilityRoleData)
            );

            this.showInvalidFacilityRolePopup = true;
            return;
        }
        console.log('Facility Role Map', JSON.stringify(facilityRoleMap));
        ...
        const selectedFacility =
            this.facilityOptions.find(f => f.value === selectedFacilityId);
 ...
 /*  if (facilityVal) {
  ...
  async createCalenderShift() {
   ...
   this.originalServiceAmounts={};
    ...
    // If selected staff doesn't exist, default to the first staff
    if (!selectedStaff) {
        this.addShiftData.AddShiftStaffValue =
   ...
   /*  event.stopPropagation();
    ...
    getAddShiftDataById({ shiftId: this.shiftStaffId }).then(async (result) => {
     ...
     // this.splitShift = result.shiftwithstaffdata.Split_Shifts__c;
      ...
      if(result.shiftwithstaffdata.New_Split_Shift__c || result.shiftwithstaffdata.Split_Shifts__c || result.shiftwithstaffdata.Status__c == "Completed" || result.shiftwithstaffdata.Status__c == "InProgress" || this.addShiftData.sil){
          this.disableBrokenShift=true;
         this.disableSplitShift=true;
       ...
       if(result.shiftwithstaffdata.Broken_Shift__c || result.shiftwithstaffdata.Status__c == "Completed" || result.shiftwithstaffdata.Status__c == "InProgress" || this.addShiftData.sil){
       ...
       if(result.shiftwithstaffdata.Broken_Extended_Duration__c >0){
        ...
        if (shiftType == "Custom") {
         ...
         if( result.shiftwithstaffdata.Status__c == "Completed" || result.shiftwithstaffdata.Status__c == "InProgress"){
          this.disableGroupShift=true;
         } 
      const rows = [];
    ...
    this.AddShiftAndServices = await Promise.all(
      this.AddShiftAndServices.map(async (row, i) => {
        if (i === index) {
          ...
          //  this.selectedStaffLabel =  this.groupShift !== true ?updatedRow.stafflabel : "";
