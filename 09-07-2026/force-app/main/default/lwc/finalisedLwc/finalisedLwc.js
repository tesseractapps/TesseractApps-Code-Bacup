...
export default class FinalisedLwc extends NavigationMixin(LightningElement) {
    ...
    loadStaffData(){
        ...
        accpetedStaffPayroll({orgName: this.userOrgName,staffId: null,startDate: this.financialStartDate,enddate: this.financialEndDate,isFinalised:true}).then(result=>{ 
            console.log(' result of pay load  => '+JSON.stringify(result));
             
            if (result != null) {
            // Step 1: Filter only "Submitted" or "STP Filed" records
            const validRecords = result.filter(r =>
                         r.Payroll_Setting__r.Status__c === "Submitted" || r.Payroll_Setting__r.Status__c === "STP Filed"
            );

            // Step 2: Sort by End_Date__c (ascending) to get earliest first (i.e., the last applicable pay cycle)
            validRecords.sort((a, b) => new Date(b.End_Date__c) - new Date(a.End_Date__c));

            console.log('🧠 Sorted by End_Date__c (desc):');
            validRecords.forEach(r => {
                     console.log(`${r.Resource_Name__c} - ${r.End_Date__c}`);
            });

            // Step 3: Pick earliest End_Date__c record per Staff__c
            const mapStaffPayList = new Map();
            validRecords.forEach(ss => {
                if (!mapStaffPayList.has(ss.Staff__c)) {
            ...
            const latestEntriesArray = Array.from(mapStaffPayList.values());
                ...
                this.records = latestEntriesArray.map(value=>{
                        return {
                                    ...
                                    End_Date__c:value.End_Date__c,
                                    Start_Date__c:value.Start_Date__c
                                }
                }); 
                 console.log('FINALISED  records'+JSON.stringify( this.records));  
    ...
    handleRowSelection(event) {
            ...
            for (let rec of  event.detail.selectedRows) {
                if (rec.Pay_Event_Indicator__c == true ) {
                    ...
                    const event = new ShowToastEvent({
                        title: 'Warning',
                        message: 'STP filing is required for the selected staff.',
                        variant: 'Info',
                        mode: 'dismissable'
                    });
                    this.dispatchEvent(event);
