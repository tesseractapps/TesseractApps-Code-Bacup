...
import getRemainingFacilitySlots from '@salesforce/apex/LimitCheckService.getRemainingFacilitySlots';
...
export default class TesseractAppsFacilityBulkUploadLwc extends LightningElement {
  ...
  @track selectedRecordIds = new Set();
   ...
   async handleFinalSubmit() {
    ...
    try {
        const remainingSlots = await getRemainingFacilitySlots();
        console.log('Remaining facility slots:', remainingSlots);
        ...
        if (remainingSlots === 0) {
            this.showToast(
                'Plan limit exceeded',
                'Facility limit already reached. Please upgrade your plan.',
                'error'
            );
            return;
        }
        ...
        if (selectedCount > remainingSlots) {
            this.showToast(
                'Limit exceeded',
                `You can create only ${remainingSlots} facilities.`,
                'error'
            );
            return;
        }
        ...
        console.error('Facility limit check error:', error);
        this.showToast('Error', 'Error checking facility limit', 'error');
        return;
    }
