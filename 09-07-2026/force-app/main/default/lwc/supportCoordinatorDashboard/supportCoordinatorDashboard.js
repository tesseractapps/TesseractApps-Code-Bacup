...
export default class DashboardUi extends LightningElement {
    ...
    async loadPinnedParticipants() {
        try {
            ...
            this.participants = data.map(p => {
                // console.log('Budget raw value:',  p.Budget_Spent_to_Date__c );
                ...
                const budgetSpentToDate = Number(p.Budget_Spent_to_Date__c) || 0;
                const spentAmount = Number(p.Total_Budget_Spent__c) || 0;
                const totalBudget = Number(p.Total_Plan_Budget__c) || 0;
                const totalSpent = budgetSpentToDate + spentAmount;

                const spentPercent = totalBudget > 0
                    ? Math.min((totalSpent  / totalBudget) * 100, 100)
                    : 0;
                console.log('Budget_Spent_to_Date__c:', budgetSpentToDate);
                console.log('Spent Amount:', spentAmount);
                console.log('totalSpent:', totalSpent);
                console.log('Total Budget:', totalBudget);
                console.log('Spent Percent:', spentPercent);
               // const percent = Math.min(spentPercent, 100).toFixed(2);
                const percent = spentPercent % 1 !== 0
                    ? Number(spentPercent.toFixed(2))
                    : spentPercent;
                ...
                return {
                    ...
                    percent:percent,
                    //Math.min(Math.round(spentPercent), 100),
   ...
   handleParticipantRecordsPerPageChange(event) {
    ...
    handleParticipantCreated(event) {
        console.log(
            'Participant created:'
        );
