...
ensureEntityFieldsOnRows() {
    this.serviceGroupName = JSON.parse(
        JSON.stringify(
            this.serviceGroupName.map(item => ({
                ...item,
                EntityNameValue: item.EntityNameValue ?? null,
                EntityNameLabel: item.EntityNameLabel ?? null
            }))
        )
    );
}
            ...
            .then((selectedItems) => {
              ...
              fetchedItems = fetchedItems.map((item) => {
                ...
                if (match) {
                  ...
                  if (this.accountingService === 'Tesseract System') {
                      updatedItem.EntityNameValue = match.Entity_Profile__c || null;
                  } else if ( this.accountingService === 'Xero' || this.accountingService === 'MYOB' ) {
                      updatedItem.EntityNameValue = match.Xero_Entity__c || null;
                  }
      ...
      if (newValue === 'Add New Entity') {
         this.serviceGroupName = this.serviceGroupName.map(row => {
              if (row.Id === rowId) {
                  return {
                      ...row,
                      EntityNameValue: null,
                      EntityNameLabel: null
                  };
              }
              return row;
          });
              ...
              return getSelectedSupportItems({
                ...
                newStateItems = newStateItems.map((item) => {
                  ...
                  if (match) {
                    ...
                    if (this.accountingService === 'Tesseract System') {
                        item.EntityNameValue = match.Entity_Profile__c || null;
                    } else if ( this.accountingService === 'Xero' || this.accountingService === 'MYOB' ) {
                        item.EntityNameValue = match.Xero_Entity__c || null;
                    }
...
openFundForManageInvoice(selectedFund) {
      ...
      const approvedAmount =
          parseFloat(selectedFund.amountApproved || 0);

      this.approvedAmount =
          this.formatCurrency(approvedAmount);

      const availablefunds =
          parseFloat(selectedFund.availableFunds || 0);

      this.availablefunds =
          this.formatCurrency(availablefunds);

      const spentAmount =
          parseFloat(selectedFund.spentAmount || 0);

      this.spentAmount =
          this.formatCurrency(spentAmount);

      this.originalParentSpent =
          spentAmount;

      this.originalAvailableFunds =
          availablefunds;
...
.then((ndisResponse) => {
    ...
    console.log(
        "✅ NDIS Service Line Items Response:",
        JSON.stringify(ndisResponse)
    );

    if (ndisResponse && ndisResponse.length > 0) {

        this.shiftNameValue =
            ndisResponse[0].Shift_Name__c;

        console.log(
            "🟩 Stored shiftNameValue:",
            this.shiftNameValue
        );
        ...
        console.warn(
            "⚠️ No NDIS Service Line Items returned."
        );
...
.catch((error) => {
    ...
    console.error(
        "❌ Error fetching Entity from Fundtracker:",
        error
    );
});
...
.then((ndisResponse) => {
    ...
    .then((selectedItems) => {
        ...
        fetchedItems =
            fetchedItems.map((item) => {
                ...
                if (
                    ...
                    updatedItem.EntityNameValue =
                        match.Entity_Profile__c || null;
                    ...
                    updatedItem.EntityNameValue =
                        match.Xero_Entity__c || null;
                }

                if (
                    this.otherThanNdis === true ||
                    (
                        this.serviceTypeName.includes(
                            "Miscellaneous"
                        ) &&
                    ...
                    updatedItem.serviceSupportItem =
        ...
        this.records1 = fetchedItems;
        this.totalRecords1 =
            this.records1.length;

        this.pageSize1 =
            this.pageSizeOptions1[0];
...
.catch((error) => {
    ...
    console.error(
        "Error fetching NDIS Catalog data",
        error
    );
