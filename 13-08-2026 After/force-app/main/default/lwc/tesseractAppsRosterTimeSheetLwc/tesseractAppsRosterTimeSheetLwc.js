...
import sendSelectedPayPeriodTimesheets from '@salesforce/apex/XeroIntegrationTimeSheet.sendSelectedPayPeriodTimesheets';
...
import shouldRemoveMinimumEngagement from "@salesforce/apex/BrokenShiftValidations.shouldRemoveMinimumEngagement";
import updateActualStartTime from '@salesforce/apex/RosterTimeSheetController.updateActualStartTime';
import updateActualEndTime from '@salesforce/apex/RosterTimeSheetController.updateActualEndTime';
import getStaffEarningRatesForUI from '@salesforce/apex/XeroIntegrationTimeSheet.getStaffEarningRatesForUI';
import insertBulkTimeSheetWithSelectedRates from '@salesforce/apex/XeroIntegrationTimeSheet.insertBulkTimeSheetWithSelectedRates';
...
export default class tesseractAppsRosterTimeSheetLwc extends LightningElement {
  ...
  resizeObserver;
  _observedContainer;
  @track pageSizeBulk = 5;
  @track isPageSizeBulkManuallySet = true;
  @track state;
  @track weekStartDate;
  @track weekEndDate;
 ...
 // @track isExtendedShift = false;
  ...
  @track grandTotal = 0;
  @track isExtendedShift = false;
  @track isExtendedShiftSectionVisible = true;
  @track isSleepOver = false;
  @track StaffName1;
  ...
  @track showEarningRateSelector = false;
  @track categoryRateOptions = [];
  @track selectedRateMappings = {};
  @track earningRateModalTitle = 'Select Earning Rates from Xero';
  @track selectedStaffForRates = null;
  @track isLoadingRates = false;
  @track earningRates = [];
  @track ordinaryEarningsRateId = null;
...
@track isBrokenShift=false;
  ...
  @track shiftEndDate;
   @track shiftStartDate;
  @track brokenAllowance;
  @track schadsMinimumEngageApplied=false;
  @track showOverTimeRate;
  @track totalOverTimeWages=0; 
  @track hasMinimumEngagement=false;
  @track shiftDayName='';
  @track isHoliday=false;
  filterOption = [
      {label: 'All', value: 'All'},
      {label: 'Extended/Overtime', value: 'Extended'},
      { label: 'Sleepover', value: 'Sleepover' },
      { label: 'SIL', value: 'SIL' }
  ];
  @track selectedFilterValue='All';
  @track weeklyViewFilterValue='All';
  @track selectedStaffShifts=[];
  @track originalSelectedStaffShifts=[];
  @track orgname;
  @track statePostal;
  @track orgLogo;
  @track address;
  @track shiftOriginalDuartion=0;
  @track  StaffPaidBreak=false;
...
handleSectionToggle(event) {
  ...
  fetchOrganizationDetails() {
    organizationDetails()
      .then((response) => {
        ...
        this.WeekStartDay = org.Week_Start_Day__c;
        this.abn = org.ABN__c;
        ...
        if (org.Accounting_Services__c === 'Xero') {
            ...
            this.Accountingtitle1 = "Send Bulk Timesheet to  Xero";
        ...
        if (org.Accounting_Services__c === 'MYOB') {
            ...
            this.Accountingtitle1 = "Send Bulk Timesheet to  MYOB";
        }
  ...
  loadShiftData() {
    ...
    console.log("========== loadShiftData ==========");
    console.log("staffMembers:", this.staffMembers.length);
 ...
 /*  getStartOfWeek(date) {
  ...
  generateMockData() {
    ...
    this.allStaff.forEach((staffRecord, staffIndex) => {
        ...
        console.log('all shifts '+JSON.stringify(baseStaff.allShifts));
      const filteredShifts = baseStaff.allShifts.filter(shift => {
          ...
          if (this.selectedView === 'autoApproved') {
              matchesViewFilter = approvalStatus === 'Approved';
          }
          ...
          // If view filter fails -> stop here
          if (!matchesViewFilter) {
              return false;
          }
          ...
          console.log('Selected Shift Filter:', this.selectedFilterValue);
          if (this.selectedFilterValue === 'All') {
                return true;
            }

          if (this.selectedFilterValue === 'Extended') {

              console.log('Extended_Duration__c:', shift.Extended_Duration__c);

              return (shift.Extended_Duration__c > 0 || shift.Over_Time_Tier_One__c >0 || shift.Over_Time_Tier_Two__c >0 );
          }

          if (this.selectedFilterValue === 'Sleepover') {

              console.log('Shift_Category__c:', shift.Shift_Category__c);

              return shift.Shift_Category__c === 'Sleepover Shift';
          }

          if (this.selectedFilterValue === 'SIL') {

              console.log('SIL__c:', shift.SIL__c);

              return shift.SIL__c === true;
          }
       ...
       const shiftsForTotals = filteredShifts;
        ...
        filteredShifts.forEach((shift, shiftIndex) => {
            ...
            const shiftObj = {
                id: `${staffId}-${shift.Id}`,
                shiftCategory: shift.Shift_Category__c,
                date: shift.Date__c,
                isForDay: true,
                ...
                logInTimeFormatted: shift.Login_Time_Formula__c,
                logOutTimeFormatted: shift.Logout_Time_Formula__c,
                totalLoggedHours,
        ...
        staff.shiftRows = this.weekDays.map((day, dayIndex) => {
            ...
            return {
                ...
                shifts: shiftsForDay.map(s => ({
                    id: s.Id,
                    shiftCategory: s.Shift_Category__c,
                    scheduledStart: s.Start_time_Formula__c,
                    scheduledEnd: s.End_time_formula__c,
                    login: s.Login_Time_Formula__c,
                    logout: s.Logout_Time_Formula__c,
                    scheduledDuration: s.Shift_Calculated_Duration_In_Hours_and_Mins__c,
                    actualDuration: s.Actual_Duration_In_Hours_and_Mins__c
                }))
            };
        });
        ...
        // ======================================
        staff.totalOvertimeTierOne = parseFloat(
            shiftsForTotals
                .reduce(
                    (sum, s) => sum + (parseFloat(s.Over_Time_Tier_One__c) || 0),
                    0
                )
                .toFixed(2)
        );

        staff.totalOvertimeTierTwo = parseFloat(
            shiftsForTotals
                .reduce(
                    (sum, s) => sum + (parseFloat(s.Over_Time_Tier_Two__c) || 0),
                    0
                )
                .toFixed(2)
        );
        ...
        if (staff.totalOvertimeTierOne > 0) {
            overtimeLines.push(
                `${staff.totalOvertimeTierOne.toFixed(2)} hr (1.5x)`
            );
        }

        if (staff.totalOvertimeTierTwo > 0) {
            overtimeLines.push(
                `${staff.totalOvertimeTierTwo.toFixed(2)} hr (2x)`
            );
        }

        // staff.weeklyOvertimeDisplay =
        //     overtimeLines.length > 0
        //         ? overtimeLines.join('\n')
        //         : '0.00';

        staff.weeklyOvertimeTierOneDisplay =
            staff.totalOvertimeTierOne > 0
                ? `${staff.totalOvertimeTierOne.toFixed(2)} hr (1.5x)`
                : '';

        staff.weeklyOvertimeTierTwoDisplay =
            staff.totalOvertimeTierTwo > 0
                ? `${staff.totalOvertimeTierTwo.toFixed(2)} hr (2x)`
                : '';

        console.log("Weekly Tier 1:", staff.totalOvertimeTierOne);
        console.log("Weekly Tier 2:", staff.totalOvertimeTierTwo);
       ...
       // =========================================
        // STAFF FILTER FOR selectedFilterValue ONLY
        // =========================================

        const shouldFilterStaff =
            this.selectedFilterValue &&
            this.selectedFilterValue !== 'All';

        if (shouldFilterStaff && filteredShifts.length === 0) {

            console.log(
                `Skipping staff ${staffName} - no matching shifts for filter ${this.selectedFilterValue}`
            );

            return; // Skip this staff completely
        }
...
navigateToDay(event) {
    ...
    const startDay = this.weekDayMap[weekStartDay];
    ...
    this.weekStartDate =this.formatDate(startOfWeek);
    this.weekEndDate =this.formatDate(endOfWeek);
    this.datePickerString = this.formatDateToString(selectedDate);
    localStorage.setItem( "timesheetDatePicker",this.datePickerString);
    console.log("Week Start Date:",this.weekStartDate);
    console.log("Week End Date:", this.weekEndDate);
    this.loadShiftData();
  ...
  get processedStaff() {
      ...
      console.log("========== processedStaff ==========");
      console.log("staffMembers:", this.staffMembers?.length);
      console.log("paginatedStaff:", this.paginatedStaff?.length);
      ...
      return this.paginatedStaff.map((staff) => {
        //  console.log("👤 Staff Record:", JSON.stringify(staff, null, 2));
          ...
          const shiftRows = this.weekDays.map((day, index) => {
              ...
              console.log("🔍 Shifts For Day:", JSON.stringify(shiftsForDay, null, 2));
              ...
              console.log("Shifts For Day Count:", shiftsForDay.length);

              const shiftDetails = shiftsForDay.map(s => ({
                  id: s.id,
                  shiftCategory: s.shiftCategory,
                  formattedStart: s.startTime,
                  formattedEnd: s.endTime,
                  formattedLogin: s.logInTimeFormatted,
                  formattedLogout: s.logOutTimeFormatted,
                  scheduledDuration: s.actualDuration,
                  actualDuration: s.formattedLoggedHours
              }));
          ...
          const processedStaff = {
              ...
              isSelected: this.selectedStaffIds?.includes(staff.staffId),

              shiftRows
          };

         /*  console.log("✅ Processed Staff:", JSON.stringify(processedStaff, null, 2)); */
          return processedStaff;
      });
  }

  get hasProcessedStaff() {
    return this.processedStaff && this.processedStaff.length > 0;
}
  ...
  setPageSizeByZoomAndScreen() {
    this.handleRefresh();
    if (this.isPageSizeManuallySet) return;
    const container = this.template.querySelector('.table-container');
    if (!container) return;

    const containerHeight = container.getBoundingClientRect().height;

    const headerEl = this.template.querySelector('.custom-header-row');
    const headerHeight = headerEl ? headerEl.getBoundingClientRect().height : 50;

    const rowEl = this.template.querySelector('.custom-data-row');
    const rowHeight = rowEl ? rowEl.getBoundingClientRect().height : 55;

    const availableHeight = containerHeight - headerHeight - 20;
    let calculatedPageSize = Math.floor(availableHeight / rowHeight);

    if (calculatedPageSize < 1) {
  ...
  handleCheckClick(event) {
    ...
    if (
      ...
      this.refresh === false &&
      this.actualStartFlag === false &&
      this.actualEndFlag === false
    ) {
    ...
    getMismatchedShifts({ staffId: staffId, weekStart: start, weekEnd: end })
      .then((shifts) => {
        ...
        const filteredShifts = (shifts || [])
          .filter((shift, index) => {
            ...
            // =========================================
              ...
              console.log('Selected Filter:', this.selectedFilterValue);
              ...
              if (this.selectedFilterValue === 'Extended') {
                  ...
                  return isValid && (shift.Extended_Duration__c > 0 || shift.Over_Time_Tier_One__c >0 || shift.Over_Time_Tier_Two__c >0 );
              }
              ...
              if (this.selectedFilterValue === 'Sleepover') {
                  ...
                  return isValid &&
                        shift.Shift_Category__c === 'Sleepover Shift';
              }
          ...
          .map((shift, index) => {
            ...
            console.log("Raw Over_Time_Tier_One__c:", shift.Over_Time_Tier_One__c);
            console.log("Raw Over_Time_Tier_One_Wage__c:", shift.Over_Time_Tier_One_Wage__c);
            console.log("Raw Over_Time_Tier_Two__c:", shift.Over_Time_Tier_Two__c);
            console.log("Raw Over_Time_Tier_Two_Wage__c:", shift.Over_Time_Tier_Two_Wage__c);
            ...
            return {
              ...
              //Sai Eswar
              overtimeTierOneDisplay:
                  shift.Over_Time_Tier_One__c > 0
                      ? `${shift.Over_Time_Tier_One__c} Hrs (1.5x)`
                      : "0.00 (1.5x)",

              overtimeTierTwoDisplay:
                  shift.Over_Time_Tier_Two__c > 0
                      ? `${shift.Over_Time_Tier_Two__c} Hrs (2x)`
                      : "0.00 (2x)",
        ...
        this.ismissmatchedshifts =
            filteredCount === 0 || totalShifts !== filteredCount;

        console.log("Total Shifts:", totalShifts);
        console.log("Filtered Shifts:", filteredCount);
        console.log("ismissmatchedshifts:", this.ismissmatchedshifts);
        ...
        this.originalSelectedStaffShifts = filteredShifts;
  ...
  handleMismatchClick(event) {
    ...
    getMismatchedShifts({ staffId: staffId, weekStart: start, weekEnd: end })
      .then((shifts) => {
            ...
            console.log(
                ...
                | `
                  ...
                  if (this.selectedFilterValue === 'Extended') {
                      ...
                      return isMismatch && (shift.Extended_Duration__c > 0 || shift.Over_Time_Tier_One__c >0 || shift.Over_Time_Tier_Two__c >0 ) ;
                  }
                  // Sleepover
                  if (this.selectedFilterValue === 'Sleepover') {
                      ...
                      return isMismatch && shift.Shift_Category__c === 'Sleepover Shift';
                  }
                  // SIL
                  if (this.selectedFilterValue === 'SIL') {
                      ...
                      return isMismatch && shift.SIL__c === true;
                 }

                 // =========================================
                  // WEEKLY VIEW FILTER (ADDITIONAL FILTER)
                  // =========================================

                  console.log('Weekly Filter:', this.weeklyViewFilterValue);

                  // ALL
                  if (this.weeklyViewFilterValue === 'All') {
                      return isMismatch;
                  }

                  // EXTENDED
                  if (this.weeklyViewFilterValue === 'Extended') {

                      console.log(
                          'Weekly Extended_Duration__c:',
                  ...
                  // SLEEPOVER
                  if (this.weeklyViewFilterVaue === 'Sleepover') {

                      console.log(
                          'Weekly Shift_Category__c:',
                          shift.Shift_Category__c
                      );
                  ...
                  // SIL
                  if (this.weeklyViewFilterValue === 'SIL') {
        ...
        console.log("========== FINAL OVERTIME DATA ==========");

        this.selectedStaffShifts.forEach((shift) => {
            console.log(
                `Shift ${shift.Id}`,
                {
                    overtime: shift.overtime,
                    overtimeTierOneWage: shift.overtimeTierOneWage,
                    overtimeTierTwo: shift.overtimeTierTwo,
                    overtimeTierTwoWage: shift.overtimeTierTwoWage
                }
            );
        });
        ...
        this.originalSelectedStaffShifts = mismatchedShifts;
        ...
        if (!mismatchedShifts || mismatchedShifts.length === 0) {
          ...
          this.handleCheckClick({
            currentTarget: { dataset: { staffId: this.StaffId, staffName: this.StaffName1 } }
          });
    ...
    getMismatchedShifts({ staffId: staffId, weekStart: start, weekEnd: end })
      .then((shifts) => {
        ...
        const filteredShifts = (shifts || []).filter((shift) => {
              ...
              const isMismatch =
                  Math.abs(totalWorked - duration) > bufferHours;
              ...
              // checked = show all shifts
              // unchecked = only mismatches

              const matchesCheckbox =this.isChecked ? true : isMismatch;
              ...
              let matchesSelectedFilter = true;
                ...
                if (this.selectedFilterValue === 'Extended') {
                    ...
                    matchesSelectedFilter =
                        shift.Extended_Duration__c > 0 ||
                        shift.Over_Time_Tier_One__c > 0 ||
                        shift.Over_Time_Tier_Two__c > 0;

                } else if (
                    this.selectedFilterValue === 'Sleepover'
                ) {
             ...
             // =========================================
              ...
              console.log(
                  'weeklyViewFilterValue =>',
                  this.weeklyViewFilterValue
              );

              // ALL
              if (
                  !this.weeklyViewFilterValue ||
                  this.weeklyViewFilterValue === 'All'
              ) {
                  return true;
              }
              ...
              // SLEEPOVER
              if (this.weeklyViewFilterValue === 'Sleepover') {
  ...
  handleBackClick(event) {
    ...
    /* this.shiftInformation = false;
        ...
        console.log('this.facilityIdList >>>>>', this.facilityIdList);
  ...
  handleStartTimeChange(event) {
      ...
      console.log("✅ Start Time Updated →", {
       ...
       this.recalculateExtendedDuration();
  ...
  handleEditClick(event) {
    ...
    this.shiftOriginalDuartion=0;
    this.StaffPaidBreak=false;
    this.isExtendedShiftSectionVisible = true;
    ...
    getAddShiftDataById({ shiftId: this.shiftID })
      .then((result) => {
        ...
        const penaltyMode = data.Shift_Facility__c
            ? data.Shift_Facility__r?.Shift_Penalty_Mode__c
            : data.Add_Shift__r?.Facility__r?.Shift_Penalty_Mode__c;

        console.log('penaltyMode =>', penaltyMode);

        this.isExtendedShiftSectionVisible = penaltyMode !== 'Schads';

         this.shiftEndDate=data.Shift_End_Date__c;
          this.shiftOriginalDuartion= data.Duration__c
           this.StaffPaidBreak= data.Staff_Paid_Break__c
         this.shiftStartDate=data.Date__c;
       ...
       this.orignalShiftTime = parsedTimes.start; 
        ...
        this.brokenExtendedDuration=parseFloat(data.Broken_Extended_Duration__c || 0).toFixed(2);
         ...
         this.isHoliday=data.Public_holiday__c;
         this.shiftDayName=data.Shift_Day_Name__c
      ...
      //  console.log('etimeParts ==>'+etimeParts);
        ...
        this.extendedDuartion = parseFloat(data.Extended_Duration__c || 0);
         this.hasMinimumEngagement=data.Minimum_Engagement_Applied__c;
  ...
  async handlesubmit(event) {
   ...
   /*  if (totalHours == 0 && status !='Cancelled') {
    ...
    try {
      ...
      this.originalSelectedStaffShifts = [];
  ...
  handleUpdate(event) {
    ...
    // 🔧 Force-close any open time-picker dropdowns before submit
    this.template
        .querySelectorAll('c-tesseract-apps-time-picker')
        .forEach((picker) => {
            if (typeof picker.closeDropdown === 'function') {
    ...
    fields.Minimum_Engagement_Applied__c = this.hasMinimumEngagement;
  ...
  handleLogOutChnage(event) {
    ...
    this.recalculateBrokenExcessHours(event.target.value,durationHours.toFixed(2));
   
  }

  recalculateExtendedDuration() {
    console.log("========== recalculateExtendedDuration ==========");

    console.log("Start Time (12h):", this.AddShiftStartTimeAMPM1);
    console.log("End Time (12h):", this.AddShiftEndTimeAMPM1);
    console.log("End Date:", this.enddate);
    ...
    if (
        ...
        console.warn("Missing required values. Resetting duration.");

        this.isExtendedShift = false;
        this.extendedDuartion = "0.00";
        this.extendedHoursandmins = "0:00";
        return;
    }

    // Convert 12-hour format to 24-hour format
    const startTime24 = this.convertTo24HourFormat(
        this.AddShiftStartTimeAMPM1.toLowerCase()
    );

    const endTime24 = this.convertTo24HourFormat(
        this.AddShiftEndTimeAMPM1.toLowerCase()
    );
    ...
    let startDateTime = new Date(
        `${this.enddate}T${startTime24.replace("Z", "")}`
    );

    let endDateTime = new Date(
        `${this.enddate}T${endTime24.replace("Z", "")}`
    );

    console.log("Start DateTime:", startDateTime);
    console.log("End DateTime (Before):", endDateTime);

    // Overnight shift
    if (endDateTime < startDateTime) {
        console.log("Overnight shift detected. Adding one day to end time.");
        endDateTime.setDate(endDateTime.getDate() + 1);
    }

  console.log("End DateTime (After):", endDateTime);

      // Calculate worked duration
    const durationMs = endDateTime.getTime() - startDateTime.getTime();
    let completedDuration = durationMs / (1000 * 60 * 60);

    console.log("Worked Duration:", completedDuration);

    // ==========================================
    // Apply unpaid break logic (same as Apex)
    // ==========================================
    const scheduledDuration = parseFloat(this.shiftOriginalDuartion || 0);

    if (scheduledDuration >= 5 && !this.StaffPaidBreak) {
        console.log("Applying unpaid 30-minute break deduction.");
        completedDuration -= 0.5;
    }

    console.log("Completed Duration (after break):", completedDuration);

    // ==========================================
    // Extended Duration (same as Apex)
    // ==========================================
    let extendedDuration = completedDuration - scheduledDuration;

    // Prevent negative values
    extendedDuration = Math.max(0, extendedDuration);

    console.log("Scheduled Duration:", scheduledDuration);
    console.log("Extended Duration:", extendedDuration);

    // Update UI
    this.isExtendedShift = extendedDuration > 0;
    this.extendedDuartion = extendedDuration.toFixed(2);

    const totalMinutes = Math.round(extendedDuration * 60);
    ...
    console.log("extendedHoursandmins:", this.extendedHoursandmins);
} 

  recalculateBrokenExcessHours(logoutTime24,durationHours) {
    ...
    const isSunday = this.shiftDayName === 'Sunday';

    const isSaturday = this.shiftDayName === 'Saturday';

    const isHoliday = this.isHoliday === true;
  ...
  console.log('isHoliday =>' + isHoliday);
    ...
    if (isSunday || isHoliday) {
        ...
        this.brokenExtendedDuration = 0;
        this.brokenExtendedRate = 0;
        this.brokenExtendedWage = 0;
    }
    // =====================================
    // SATURDAY
    // =====================================
        ...
        this.brokenExtendedDuration =
            excess.toFixed(2);

        this.brokenExtendedRate =
            excess <= 0
            ? 0
            : (this.currentShiftrates / 1.5) * 2;

        this.brokenExtendedWage =
            (
                this.brokenExtendedRate * excess
            ).toFixed(2);
    }
    // =====================================
    // WEEKDAYS
    ...
    this.calculateExtendedWage();
     this.checkMinimumEngagementRemoval(durationHours)
/* 
...
async checkMinimumEngagementRemoval(durationHours) {

    try {

        const result =
            await shouldRemoveMinimumEngagement({

                addShiftId:this.addShiftId,
              

                currentShiftId:this.shiftID,

                currentEditedDuration:
                    durationHours
            });

        console.log(
            'shouldRemoveMinimumEngagement => ',
            result
        );

        this.hasMinimumEngagement = !result;
        ...
        console.error(
            'Error in checkMinimumEngagementRemoval',
            error
        );
  ...
  showXeroResultModal = false;
  xeroModalTitle = '';
  xeroModalItems = [];
  showSendAnywayButton = false;

  formatDateforXero(dateValue) {
      if (!dateValue) {
          return '';
      }
  ...
  /* async handleAddToXero1(event) {
      ...
      if (this.AccountingServices === 'Xero') {
          ...
          .then((result) => {
              ...
              const popupErrors = result.filter(
                  item => item.showInPopup === true
              );

              if (popupErrors.length > 0) {

                  const hasPayPeriod = popupErrors.some(
                      item => item.payPeriodMismatch === true
                  );

                  const hasOtherErrors = popupErrors.some(
                      item => item.payPeriodMismatch !== true
                  );

                  // Modal Title
                  if (hasPayPeriod && hasOtherErrors) {
                      this.xeroModalTitle = 'Xero Validation Results';

                  } else if (hasPayPeriod) {
                      this.xeroModalTitle = 'Pay Period Mismatch';

                  } else {
                      this.xeroModalTitle = 'Xero Validation Error';
                  }

                  // Show Send Anyway only if pay period exists
                  this.showSendAnywayButton = hasPayPeriod;

                  // Build modal items
                  this.xeroModalItems = popupErrors.map(item => ({
                      staffId: item.staffId,
                      staffName: item.staffName || 'Unknown',

                      payPeriodMismatch:
                          item.payPeriodMismatch === true,

                      selected: false,

                      // IMPORTANT
                      suggestedStartDate:
                          item.suggestedStartDate || null,

                      suggestedEndDate:
                          item.suggestedEndDate || null,

                      suggestedStartDay:
                          item.suggestedStartDay || null,

                      suggestedEndDay:
                          item.suggestedEndDay || null,

                      message:
                          item.payPeriodMismatch &&
                          item.suggestedStartDay &&
                          item.suggestedEndDay &&
                          item.suggestedStartDate &&
                          item.suggestedEndDate
                              ? `The selected timesheet period does not match this staff member's Xero pay cycle (${item.suggestedStartDay} to ${item.suggestedEndDay}).
                              I can resend the timesheet for the correct period: ${this.formatDateforXero(item.suggestedStartDate)} to ${this.formatDateforXero(item.suggestedEndDate)}.`
                                      : item.message || 'Unknown error'
                  }));

                  console.log(
                      'Initial Modal Items:',
                      JSON.stringify(this.xeroModalItems)
                  );

                  this.selectedStaffIds = [];
                  this.showXeroResultModal = true;
                  return;
              }
              ...
              if (successItems.length > 0) {
                  this.dispatchEvent(
                      new ShowToastEvent({
                          title: 'Success',
                          message:
                              'Timesheet sent to Xero successfully!',
                          variant: 'success'
                      })
                  );
              } else {
                  this.dispatchEvent(
                      new ShowToastEvent({
                          title: 'Info',
                          message:
                              'No successful timesheets processed.',
                          variant: 'info'
                      })
                  );
              }
          ...
          .catch((error) => {
              console.error(
                  '💥 Error sending timesheet:',
                  error
              );

              this.xeroModalTitle = 'Unexpected Error';
              this.showSendAnywayButton = false;

              this.xeroModalItems = [
                  {
                      staffId: 'system',
                      staffName: 'System',
                      payPeriodMismatch: false,
                      selected: false,
                      suggestedStartDate: null,
                      suggestedEndDate: null,
                      suggestedStartDay: null,
                      suggestedEndDay: null,
                      message:
                          error?.body?.message ||
                          error?.message ||
                          'Unexpected error'
      ...
      if (this.AccountingServices === 'MYOB') {
          ...
          try {
              ...
              for (const staffId of this.selectedStaffIds) {
                  ...
                  const result = await createMyobTimesheet({
                      staffId: staffId,
                      startDateStr: startDateStr,
                      endDateStr: endDateStr,
                  ...
                  const staffRecord =
                      this.staffMembers?.find(staff => staff.Id === staffId) ||
                      this.allStaff?.find(staff => staff.Id === staffId);
              ...
              this.xeroModalTitle = hasErrors
                  ? 'MYOB Timesheet Results'
                  : 'Success';

              this.showSendAnywayButton = false;

              // Build modal items
              this.xeroModalItems = results.map(item => ({
                  staffId: item.staffId,
  ...
  async handleAddToXero1(event) {
      ...
      console.log('📋 handleAddToXero1 called');
      console.log('this.weekStartDate', this.weekStartDate);
      console.log('this.weekEndDate', this.weekEndDate);
      console.log('Selected Staff IDs:', this.selectedStaffIds);

      // Check if we have selected staff
      ...
      // For Xero, open the rate selector modal
      if (this.AccountingServices === 'Xero') {
          // ✅ Store the selected staff IDs in bulkStaffIds to preserve them
          this.bulkStaffIds = [...this.selectedStaffIds];
          console.log('📋 Bulk Staff IDs stored:', this.bulkStaffIds);
          console.log('📋 Number of staff in bulk:', this.bulkStaffIds.length);
          
          // Check if we have more than one staff selected for bulk
          if (this.selectedStaffIds.length > 1) {
              // Bulk mode - use the first staff to fetch rates
              this.isBulkMode = true;
              console.log('🔄 Bulk mode enabled for ' + this.selectedStaffIds.length + ' staff members');
              this.fetchStaffEarningRates(this.selectedStaffIds[0], true);
          } else {
              // Single staff mode
              this.isBulkMode = false;
              console.log('🔄 Single staff mode');
              this.fetchStaffEarningRates(this.selectedStaffIds[0], false);
          }
          return;
      }

      // For MYOB, use the existing logic
      if (this.AccountingServices === 'MYOB') {
          const savedToken = sessionStorage.getItem('myobAccessToken');
          const refreshToken = sessionStorage.getItem('myobRefreshToken');
          const savedCompanyFileUri = sessionStorage.getItem('companyFileUri');
          ...
          try {
              ...
              for (const staffId of this.selectedStaffIds) {
                  ...
                  if (result?.accessToken) {
                      currentAccessToken = result.accessToken;
                      sessionStorage.setItem('myobAccessToken', currentAccessToken);
                  }

                  if (result?.refreshToken) {
                      currentRefreshToken = result.refreshToken;
                      sessionStorage.setItem('myobRefreshToken', currentRefreshToken);
                  }

                  const staffRecord = this.staffMembers?.find(staff => staff.Id === staffId) ||
                                    this.allStaff?.find(staff => staff.Id === staffId);

                  results.push({
                      staffId: staffId,
                      staffName: staffRecord ? (staffRecord.Display_Nickname__c || staffRecord.Name) : 'Unknown Staff',
                      message: result?.message || 'Unknown response',
                      success: result?.success === true
                  });
              }
              ...
              // Show results
              this.handleXeroResult(results);
              
              // Show modal if there were errors
              const hasErrors = results.some(item => item.success !== true);
              if (hasErrors) {
                  this.xeroModalTitle = 'MYOB Timesheet Results';
                  this.showSendAnywayButton = false;
                  this.xeroModalItems = results.map(item => ({
                      staffId: item.staffId,
                      staffName: item.staffName,
              ...
              // Clear selected staff after sending
              this.selectedStaffIds = [];
              this.bulkStaffIds = [];
  ...
  // In the fetchStaffEarningRates method, transform data for radio group
  async fetchStaffEarningRates(staffId, isBulk = false) {
      this.isLoadingRates = true;
      this.selectedStaffForRates = staffId;
      this.isBulkMode = isBulk;
      
      // If in bulk mode, use the stored bulkStaffIds
      if (isBulk && this.bulkStaffIds && this.bulkStaffIds.length > 0) {
          this.selectedStaffIds = [...this.bulkStaffIds];
          console.log('📋 Bulk mode with ' + this.selectedStaffIds.length + ' staff members');
      }
      
      try {
          if (isBulk && this.selectedStaffIds && this.selectedStaffIds.length > 1) {
              // =====================================================
              // BULK MODE: Fetch categories for ALL staff members
              // =====================================================
              console.log('📋 Bulk mode: Fetching categories for ALL ' + this.selectedStaffIds.length + ' staff members');
              
              // ✅ FETCH CATEGORIES FOR ALL STAFF MEMBERS
              const allPromises = this.selectedStaffIds.map(id => {
                  return getStaffEarningRatesForUI({
                      staffId: id,
                      startDateStr: this.weekStartDate,
                      endDateStr: this.weekEndDate
                  });
              });
              
              // Wait for all results
              const allResults = await Promise.all(allPromises);
              console.log('📋 All staff results:', allResults);
              
              // Collect all unique categories from ALL staff
              const uniqueCategories = new Set();
              let allEarningRates = [];
              let ordinaryRateId = null;
              ...
              allResults.forEach((result, index) => {
                  if (result.success) {
                      if (!firstValidResult) {
                          ...
                          allEarningRates = result.earningRates || [];
                          ordinaryRateId = result.ordinaryEarningsRateId;
                          staffName = result.staffName || 'Staff';
                      }
                      
                      // ✅ Add categories from THIS staff member
              ...
              // If no categories from any staff, add default
              if (uniqueCategories.size === 0) {
                  console.log('⚠️ No categories found for any staff, adding default "General"');
                  uniqueCategories.add('General');
              }
              
              const categoriesList = Array.from(uniqueCategories);
              console.log('📋 UNIQUE categories from ALL staff members:', categoriesList);
              console.log('📋 Total earning rates:', allEarningRates.length);
              
              // Build category options for ALL unique categories
              this.categoryRateOptions = categoriesList.map(category => {
                  // Filter earning rates based on category
                  const filteredRates = this.filterRatesByCategory(category, allEarningRates);
                  
                  // Build radio options
                  const radioOptions = filteredRates.map(rate => {
                      let label = rate.name || 'Unknown Rate';
                      
                      if (rate.rateType) {
                          label = label + ' (' + rate.rateType + ')';
                      }
                      
                      if (rate.ratePerUnit !== undefined && rate.ratePerUnit !== null) {
                          label = label + ' - $' + rate.ratePerUnit;
                      }
                  ...
                  let hasRates = filteredRates && filteredRates.length > 0;
                  
                  if (hasRates) {
                      // Try to use ordinary rate ID as default
                      const matchedRate = filteredRates.find(r => r.id === ordinaryRateId);
                      if (matchedRate) {
                          selectedValue = matchedRate.id;
                      } else {
                          selectedValue = filteredRates[0].id;
                      }
                  }
                  
                  // ✅ Apply selection to ALL staff members (bulk mode = same rate for all)
                  if (selectedValue && hasRates) {
                      this.selectedStaffIds.forEach(id => {
                          if (!this.selectedRateMappings[id]) {
                              this.selectedRateMappings[id] = {};
                          }
                          this.selectedRateMappings[id][category] = selectedValue;
                      });
                  }
                  
                  return {
                      category: category,
                      matchedRateId: ordinaryRateId,
                      availableRates: filteredRates,
                      radioOptions: radioOptions,
                      selectedValue: selectedValue,
                      displayName: category.charAt(0).toUpperCase() + category.slice(1),
                      hasRates: hasRates,
                      message: hasRates ? null : 'No earning rates found for this category.'
                  };
              });
              
              // Set modal title
              let staffCount = this.selectedStaffIds ? this.selectedStaffIds.length : 0;
              this.earningRateModalTitle = 'Select Earning Rates for ' + staffCount + ' Staff Members';
              
              this.showEarningRateSelector = true;
              console.log('✅ Bulk modal opened with ' + this.categoryRateOptions.length + ' categories from ALL staff');
              
          } else {
              // =====================================================
              // SINGLE MODE: Original implementation
              // =====================================================
              const result = await getStaffEarningRatesForUI({
                  staffId: staffId,
                  startDateStr: this.weekStartDate,
                  endDateStr: this.weekEndDate
              });
              
              console.log('Fetched earning rates from Xero:', result);
              console.log('Categories from result:', result.categories);
              
              if (result.success) {
                  // Process categories with display data already formatted
                  this.categoryRateOptions = (result.categories || []).map(cat => {
                      // Build radio options with formatted labels
                      const radioOptions = (cat.availableRates || []).map(rate => {
                          let label = rate.name || 'Unknown Rate';
                      ...
                      let selectedValue = null;
                      let hasRates = cat.availableRates && cat.availableRates.length > 0;
                      
                      if (hasRates) {
                          selectedValue = cat.matchedRateId || cat.availableRates[0].id;
                      }
                      
                      // ✅ Store the selected rate for this staff
                      if (selectedValue && hasRates) {
                          if (!this.selectedRateMappings[staffId]) {
                              this.selectedRateMappings[staffId] = {};
                          }
                          this.selectedRateMappings[staffId][cat.category] = selectedValue;
                      }
                      
                      return {
                          category: cat.category,
                          matchedRateId: cat.matchedRateId,
                          availableRates: cat.availableRates || [],
                          radioOptions: radioOptions,
                          selectedValue: selectedValue,
                  ...
                  this.earningRates = result.earningRates || [];
                  this.ordinaryEarningsRateId = result.ordinaryEarningsRateId;
                  
                  let staffName = result.staffName || 'Staff';
                  this.earningRateModalTitle = 'Select Earning Rates for ' + staffName;
                  
                  this.showEarningRateSelector = true;
                  console.log('✅ Single staff modal opened');
                  
              } else {
                  this.dispatchEvent(
                      new ShowToastEvent({
                          title: 'Error',
                          message: result.message || 'Failed to fetch earning rates from Xero',
                          variant: 'error'
                      })
                  );
              }
          ...
          this.dispatchEvent(
              new ShowToastEvent({
                  title: 'Error',
                  message: 'Failed to fetch earning rates from Xero',
                  variant: 'error'
              })
          );
      } finally {
  ...
  // Helper method to filter rates by category
  filterRatesByCategory(category, earningRates) {
      if (!category || !earningRates || earningRates.length === 0) {
          return [];
      }
      ...
      if (categoryLower.includes('sleepover')) {
          keywords.push('sleepover');
      }
      if (categoryLower.includes('morning')) {
          keywords.push('morning');
      }
      if (categoryLower.includes('afternoon')) {
          keywords.push('afternoon');
      }
      if (categoryLower.includes('night')) {
          keywords.push('night');
      }
      if (categoryLower.includes('broken')) {
          keywords.push('broken');
      }
      if (categoryLower.includes('custom')) {
      ...
      if (keywords.length === 0) {
          const words = categoryLower.split(' ');
      ...
      const filtered = earningRates.filter(rate => {
          const rateName = (rate.name || '').toLowerCase().trim();
          if (!rateName) return false;
          
          // Check if rate is current
          if (rate.isCurrent === false) return false;
          
          // Check if rate name contains any keyword
          for (const keyword of keywords) {
              if (rateName.includes(keyword)) {
                  return true;
              }
          }
          return false;
      ...
      // If no matches found, return all rates as fallback
      if (filtered.length === 0) {
          console.log('⚠️ No matching rates for category: ' + category + ', returning all rates as fallback');
          return earningRates.filter(rate => rate.isCurrent !== false);
      }
      
      return filtered;
  }

  // Updated handleRateSelectionChange for radio group
  handleRateSelectionChange(event) {
      const staffId = event.target.dataset.staffId;
      const category = event.target.dataset.category;
      const earningRateId = event.detail.value;
      
      console.log('Rate selection changed:', { staffId, category, earningRateId });
      
      if (earningRateId) {
          // ✅ Update for the specific staff
          if (!this.selectedRateMappings[staffId]) {
              this.selectedRateMappings[staffId] = {};
          }
          this.selectedRateMappings[staffId][category] = earningRateId;
          
          console.log(`✅ Updated ${category} for ${staffId} to:`, earningRateId);
          
          // Update the selected value in category options for UI
          this.categoryRateOptions = this.categoryRateOptions.map(cat => {
              if (cat.category === category) {
                  return Object.assign({}, cat, { selectedValue: earningRateId });
              }
              return cat;
          });
          
          // ✅ If in bulk mode, apply the SAME selection to ALL staff
          // BUT only if they don't already have a selection for this category
          if (this.isBulkMode && this.selectedStaffIds && this.selectedStaffIds.length > 1) {
              console.log('🔄 Bulk mode: Applying selection to ALL staff members');
              this.selectedStaffIds.forEach(id => {
                  if (id !== staffId) {
                      if (!this.selectedRateMappings[id]) {
                          this.selectedRateMappings[id] = {};
                      ...
                      if (!this.selectedRateMappings[id][category]) {
                          this.selectedRateMappings[id][category] = earningRateId;
                      }
                  }
              });
          }
          
          // Clear any error highlighting for this category
          this.clearCategoryHighlight(category);
          
          console.log('Selected rate for', category, ':', earningRateId);
          console.log('Current selections:', JSON.stringify(this.selectedRateMappings, null, 2));
      }
  }

  clearCategoryHighlight(category) {
      const categoryElements = this.template.querySelectorAll('[data-category-container]');
      categoryElements.forEach(el => {
          if (el.dataset.categoryName === category) {
              el.style.border = '1px solid #e5e7eb';
              el.style.backgroundColor = 'white';
              el.style.boxShadow = '0 2px 8px rgba(0,0,0,0.06)';
              
              const errorMsg = el.querySelector('.category-error-message');
              if (errorMsg) {
                  errorMsg.remove();
              }
          }
  ...
  handleCloseRateSelector() {
      ...
      this.showEarningRateSelector = false;
      this.categoryRateOptions = [];
      this.selectedRateMappings = {};
      this.isBulkMode = false;
      this.selectedStaffForRates = null;
      this.bulkStaffIds = [];
      
      // ✅ Clear saved selections when user explicitly closes the modal
      this._savedRateMappings = {};
      this._savedCategoryRateOptions = [];
      this._savedOrdinaryEarningsRateId = null;
      this._savedStaffForRates = null;
      this._savedBulkStaffIds = [];
      this._savedSelectedStaffIds = [];
  }

  sendAnywayOptions = [
      {
          label: 'Yes, send using the suggested pay period',
          value: 'YES'
      }
  ];

  get isSendAnywayDisabled() {
      return this.selectedSendAnywayOption !== 'YES';
  }

  handleSendAnywayOptionChange(event) {
      this.selectedSendAnywayOption = event.detail.value;
  }
  ...
  /* handleAddToXero(event) {
              ...
              if (popupErrors.length > 0) {
                  ...
                  this.xeroModalItems = popupErrors.map(item => ({
                      ...
                      message:
                          ...
                          item.suggestedStartDate &&
                          item.suggestedEndDate
                              ? `Please select a ${item.suggestedStartDay} to ${item.suggestedEndDay} pay period.
      I will send the Timesheet from ${this.formatDateforXero(item.suggestedStartDate)} to ${this.formatDateforXero(item.suggestedEndDate)}.`
                              : item.message || 'Unknown error'
                  }));
      ...
      if (this.AccountingServices === 'MYOB') {
          ...
          .then((result) => {
              ...
              this.xeroModalTitle = 'MYOB Timesheet Result';
              this.showSendAnywayButton = false;
          ...
          .catch((error) => {
              ...
              this.xeroModalTitle = 'MYOB Error';
              this.showSendAnywayButton = false;
  ...
  handleAddToXero(event) {
      ...
      // Close rate selector modal if open
      this.showEarningRateSelector = false;
      this.categoryRateOptions = [];
      this.selectedRateMappings = {};
      this.isBulkMode = false;
      ...
      // Set single staff mode
      this.selectedStaffIds = [staffId];
      this.isBulkMode = false;

      // ✅ Store single staff in bulkStaffIds for consistency
      this.bulkStaffIds = [staffId];

      // Open rate selector modal
      this.fetchStaffEarningRates(staffId, false);
  }

  handlePayPeriodSelection(event) {
      const staffId = event.target.dataset.id;
      const checked = event.target.checked;

      console.log('Checkbox Changed:', staffId, checked);

      this.xeroModalItems = this.xeroModalItems.map(item => {
  ...
  get isSendAnywayDisabled() {
      return !this.xeroModalItems.some(
          item =>
              item.payPeriodMismatch &&
              item.selected
      );
  ...
  handleSendAnyway() {
      console.log('🚀 handleSendAnyway called');
      console.log('📋 xeroModalItems before send:', JSON.stringify(this.xeroModalItems, null, 2));

      // Get selected staff with pay period mismatch
      const selectedStaff = this.xeroModalItems.filter(
          item =>
              item.payPeriodMismatch === true &&
              item.selected === true
      );
      ...
      if (selectedStaff.length === 0) {
          console.log('⚠️ No staff selected for resend');
          this.dispatchEvent(
              new ShowToastEvent({
                  title: 'Info',
                  message: 'Please select at least one staff member to send.',
                  variant: 'info'
              })
          );
          return;
      ...
      console.log('📋 Selected staff for resend:', selectedStaff.length);

      // Build arrays for the Apex call
      const staffIds = selectedStaff.map(item => item.staffId);
      const startDates = selectedStaff.map(item => item.suggestedStartDate);
      const endDates = selectedStaff.map(item => item.suggestedEndDate);
      
      // ✅ IMPORTANT: Use the saved rate selections
      let categoryRateSelections = {};
      
      // ✅ FIX: Use savedRateMappings if available, otherwise use current
      const rateMappings = this._savedRateMappings || this.selectedRateMappings || {};
      const categoryOptions = this._savedCategoryRateOptions || this.categoryRateOptions || [];
      const ordinaryRateId = this._savedOrdinaryEarningsRateId || this.ordinaryEarningsRateId;
      const staffForRates = this._savedStaffForRates || this.selectedStaffForRates;
      const bulkStaffIds = this._savedBulkStaffIds || this.bulkStaffIds || [];
      const selectedStaffIds = this._savedSelectedStaffIds || this.selectedStaffIds || [];
      
      console.log('📋 Using saved rateMappings:', JSON.stringify(rateMappings, null, 2));
      console.log('📋 Using saved categoryOptions:', JSON.stringify(categoryOptions, null, 2));
      console.log('📋 Using saved ordinaryRateId:', ordinaryRateId);
      
      // Get category names from saved options
      const categoryNames = categoryOptions.map(cat => cat.category);
      console.log('📋 Category names from saved options:', categoryNames);
      
      // For each selected staff, build their rate selections
      staffIds.forEach(id => {
          categoryRateSelections[id] = {};
          
          // 1st priority: Use saved rateMappings for this specific staff
          if (rateMappings && rateMappings[id]) {
              categoryRateSelections[id] = JSON.parse(JSON.stringify(rateMappings[id]));
              console.log('📋 Using saved rateMappings for staff:', id, categoryRateSelections[id]);
              return;
          }
          
          // 2nd priority: Use saved rateMappings from the first staff
          const firstStaffId = staffIds[0] || staffForRates || (bulkStaffIds.length > 0 ? bulkStaffIds[0] : null);
          if (firstStaffId && rateMappings && rateMappings[firstStaffId]) {
              categoryRateSelections[id] = JSON.parse(JSON.stringify(rateMappings[firstStaffId]));
              console.log('📋 Using first staff\'s saved selections for staff:', id, categoryRateSelections[id]);
              return;
          }
          
          // 3rd priority: Build from saved categoryOptions
          if (categoryOptions && categoryOptions.length > 0) {
              categoryOptions.forEach(cat => {
                  let rateId = cat.selectedValue;
                  if (!rateId) {
                      rateId = cat.matchedRateId;
                  }
                  if (!rateId && cat.availableRates && cat.availableRates.length > 0) {
                      rateId = cat.availableRates[0].id;
                  }
                  if (rateId) {
                      categoryRateSelections[id][cat.category] = rateId;
                      console.log('📋 Added from saved categoryOptions:', cat.category, '->', rateId);
                  }
              });
              console.log('📋 Built from saved categoryOptions for staff:', id, categoryRateSelections[id]);
              return;
          }
          
          // 4th priority: Last resort - use ordinary rate
          if (ordinaryRateId) {
              // Try to get categories from the original modal
              const originalCategories = categoryOptions.map(cat => cat.category);
              if (originalCategories.length > 0) {
                  originalCategories.forEach(catName => {
                      categoryRateSelections[id][catName] = ordinaryRateId;
                  });
                  console.log('⚠️ Last resort - using ordinary rate for all categories for staff:', id);
              }
          }
      });
      
      // ✅ If still empty, try to use the selections from the first staff in the modal
      if (Object.values(categoryRateSelections).every(sel => Object.keys(sel).length === 0)) {
          console.log('⚠️ All selections are still empty, checking if we have any staff with selections');
          
          // Check if any staff in the original selection has mappings
          const allStaffIds = [...staffIds, ...(bulkStaffIds || []), ...(selectedStaffIds || [])];
          for (const staffId of allStaffIds) {
              if (rateMappings && rateMappings[staffId] && Object.keys(rateMappings[staffId]).length > 0) {
                  // Found a staff with selections - use them
                  staffIds.forEach(id => {
                      categoryRateSelections[id] = JSON.parse(JSON.stringify(rateMappings[staffId]));
                  });
                  console.log('📋 Found selections for staff', staffId, 'and applied to all');
                  break;
              }
          }
      }
      ...
      console.log('📋 Start Dates:', JSON.stringify(startDates));
      console.log('📋 End Dates:', JSON.stringify(endDates));
      console.log('📋 Final Category Rate Selections:', JSON.stringify(categoryRateSelections, null, 2));

      // Validate we have selections
      const hasEmptySelections = Object.values(categoryRateSelections).some(
          sel => Object.keys(sel).length === 0
      );
      
      if (hasEmptySelections) {
          console.error('❌ Some staff have no rate selections!');
          // Try to get from the first non-empty staff
          const staffWithSelections = Object.keys(rateMappings || {}).find(
              id => rateMappings[id] && Object.keys(rateMappings[id]).length > 0
          );
          
          if (staffWithSelections) {
              staffIds.forEach(id => {
                  if (Object.keys(categoryRateSelections[id]).length === 0) {
                      categoryRateSelections[id] = JSON.parse(JSON.stringify(rateMappings[staffWithSelections]));
                      console.log('📋 Copied selections from staff', staffWithSelections, 'to:', id);
                  }
              });
          }
      }
      ...
      // ✅ Call Apex with 4 parameters
      sendSelectedPayPeriodTimesheets({
          staffIds: staffIds,
          startDates: startDates,
          endDates: endDates,
          categoryRateSelections: categoryRateSelections
      })
      .then((result) => {
          ...
          const failed = result.filter(item => item.success !== true);

          if (failed.length > 0) {
              console.log('⚠️ Some staff failed:', failed.length);
              this.xeroModalTitle = 'Retry Results';
              this.showSendAnywayButton = false;

              this.xeroModalItems = failed.map(item => ({
                  staffId: item.staffId,
                  staffName: item.staffName || 'Unknown',
                  payPeriodMismatch: false,
                  selected: false,
          ...
          console.log('✅ All staff sent successfully');
          this.showXeroResultModal = false;
          this.xeroModalItems = [];
          this.showSendAnywayButton = false;
          this.selectedSendAnywayOption = '';
          
          // ✅ Clear saved selections after successful send
          this._savedRateMappings = {};
          this._savedCategoryRateOptions = [];
          this._savedOrdinaryEarningsRateId = null;
          this._savedStaffForRates = null;
          ...
          this.dispatchEvent(
              new ShowToastEvent({
                  title: 'Success',
                  message: 'Selected staff timesheets sent successfully!',
                  variant: 'success'
              })
          );
      })
      .catch((error) => {
          ...
          this.xeroModalTitle = 'Error';
          this.xeroModalItems = [{
              staffId: 'system',
              staffName: 'System',
              payPeriodMismatch: false,
              selected: false,
              message: error?.body?.message || error?.message || 'Failed to send timesheets.',
              isSuccess: false
          }];
          this.showXeroResultModal = true;
      })
  ...
  handleCloseXeroModal() {
      this.showXeroResultModal = false;
  }

  async handleConfirmAndSend(event) {
      console.log('🚀 handleConfirmAndSend called');
      console.log('📋 selectedRateMappings:', JSON.stringify(this.selectedRateMappings, null, 2));
      console.log('📋 categoryRateOptions:', JSON.stringify(this.categoryRateOptions, null, 2));
      console.log('📋 isBulkMode:', this.isBulkMode);
      console.log('📋 selectedStaffForRates:', this.selectedStaffForRates);
      console.log('📋 bulkStaffIds:', this.bulkStaffIds);
      console.log('📋 selectedStaffIds:', this.selectedStaffIds);

      // Validate that all categories have a selected rate
      const unselectedCategories = this.categoryRateOptions.filter(
          category => category.hasRates && !category.selectedValue
      );

      if (unselectedCategories.length > 0) {
          const categoryNames = unselectedCategories.map(cat => cat.displayName).join(', ');
          
          this.dispatchEvent(
              new ShowToastEvent({
                  title: 'Selection Required',
                  message: `Please select an earning rate for: ${categoryNames}`,
                  variant: 'error',
                  mode: 'dismissable'
              })
          );
          
          // Highlight the unselected categories
          this.highlightUnselectedCategories(unselectedCategories);
          return;
      }

      // Check if any categories with rates exist and none are selected
      const hasRatesCategories = this.categoryRateOptions.filter(cat => cat.hasRates);
      if (hasRatesCategories.length > 0) {
          const allSelected = hasRatesCategories.every(cat => cat.selectedValue);
          if (!allSelected) {
              this.dispatchEvent(
                  new ShowToastEvent({
                      title: 'Selection Required',
                      message: 'Please select an earning rate for all shift categories before sending.',
                      variant: 'error',
                      mode: 'dismissable'
                  })
              );
      ...
      try {
          ...
          // ✅ DETERMINE STAFF IDs
          if (this.isBulkMode) {
              // Use bulkStaffIds for bulk mode
              if (this.bulkStaffIds && this.bulkStaffIds.length > 0) {
                  staffIds = [...this.bulkStaffIds];
                  console.log('📋 Bulk mode: using bulkStaffIds with ' + staffIds.length + ' staff members');
              } else if (this.selectedStaffIds && this.selectedStaffIds.length > 0) {
                  staffIds = [...this.selectedStaffIds];
                  console.log('📋 Bulk mode: using selectedStaffIds with ' + staffIds.length + ' staff members');
              } else {
                  // Fallback
                  staffIds = [this.selectedStaffForRates];
                  console.log('⚠️ No bulk staff IDs found, using single staff');
              }
          } else {
              // Single staff mode
              staffIds = [this.selectedStaffForRates];
              console.log('📋 Single mode: staff ID:', this.selectedStaffForRates);
          }
          
          console.log('📋 Staff IDs to process:', staffIds);

          // ✅ BUILD SELECTIONS FOR EACH STAFF
          // The key insight: In bulk mode, ALL staff get the SAME rate selections
          // because the UI shows combined categories and user picks ONE rate per category
          
          // Get the source selections (from the first staff or from categoryRateOptions)
          let sourceSelections = null;
          
          // First try: Get from selectedRateMappings for the first staff
          const firstStaffId = staffIds[0];
          if (this.selectedRateMappings && this.selectedRateMappings[firstStaffId]) {
              sourceSelections = this.selectedRateMappings[firstStaffId];
              console.log('✅ Using selectedRateMappings for staff', firstStaffId, 'as source:', sourceSelections);
          }
          
          // Second try: Build from categoryRateOptions
          if (!sourceSelections || Object.keys(sourceSelections).length === 0) {
              sourceSelections = {};
              this.categoryRateOptions.forEach(cat => {
                  if (cat.selectedValue) {
                      sourceSelections[cat.category] = cat.selectedValue;
                      console.log('✅ Added from categoryRateOptions:', cat.category, '->', cat.selectedValue);
                  }
              });
              console.log('✅ Built sourceSelections from categoryRateOptions:', sourceSelections);
          }
          
          // Third try: Look for any staff with selections in selectedRateMappings
          if (!sourceSelections || Object.keys(sourceSelections).length === 0) {
              for (const [staffId, staffSelections] of Object.entries(this.selectedRateMappings || {})) {
                  if (staffSelections && Object.keys(staffSelections).length > 0) {
                      sourceSelections = staffSelections;
                      console.log('✅ Found selections for staff', staffId, 'as source:', sourceSelections);
                      break;
                  }
              }
          }
          
          // If still no source selections, use categoryRateOptions directly
          if (!sourceSelections || Object.keys(sourceSelections).length === 0) {
              sourceSelections = {};
              this.categoryRateOptions.forEach(cat => {
                  if (cat.selectedValue) {
              ...
              console.log('⚠️ Using categoryRateOptions as fallback source:', sourceSelections);
          }
          
          // ✅ APPLY SOURCE SELECTIONS TO ALL STAFF
          staffIds.forEach(id => {
              if (!selections[id]) {
                  selections[id] = {};
              }
              // Copy ALL selections from source to this staff
              for (const [category, rateId] of Object.entries(sourceSelections)) {
                  selections[id][category] = rateId;
              }
              console.log('📋 Applied selections to staff:', id, selections[id]);
          });
          
          // ✅ VERIFY: Log what we're sending
          console.log('📤 FINAL Selections being sent to Apex:', JSON.stringify(selections, null, 2));
          console.log('📤 Staff IDs being sent:', staffIds);
          
          if (Object.keys(selections).length === 0) {
              this.dispatchEvent(
                  new ShowToastEvent({
                      title: 'Error',
                      message: 'No rate selections found. Please try again.',
                      variant: 'error'
                  })
              );
              this.isLoading = false;
          ...
          // ✅ CALL APEX WITH SELECTIONS
          const result = await insertBulkTimeSheetWithSelectedRates({
              staffIds: staffIds,
              startDateStr: this.weekStartDate,
              endDateStr: this.weekEndDate,
              categoryRateSelections: selections
          });
          
          console.log('📥 Result from Xero:', JSON.stringify(result, null, 2));
          
          if (!result || result.length === 0) {
              this.dispatchEvent(
                  new ShowToastEvent({
                      title: 'Info',
                      message: 'No response received from Xero.',
                      variant: 'info'
                  })
              );
              this.isLoading = false;
          ...
          // Use handleXeroResult to show ALL results
          this.handleXeroResult(result);
          
          // Close the rate selector modal
          this.showEarningRateSelector = false;
          this.categoryRateOptions = [];
          this.selectedRateMappings = {};
          this.isBulkMode = false;
          ...
          this.xeroModalItems = [
              {
                  ...
                  suggestedStartDay: null,
                  suggestedEndDay: null,
                  message: error?.body?.message || error?.message || 'Failed to send timesheet to Xero.',
                  isSuccess: false
              }
          ];
          this.showXeroResultModal = true;
      ...
      if (
        ...
        getMismatchedShifts({
          ...
          .then((shifts) => {
            ...
            const processedShifts = (shifts || []).filter((shift) => {

             
                console.log('weeklyViewFilterValue Filter:', this.weeklyViewFilterValue);
               // ALL
                if (this.weeklyViewFilterValue === 'All') {
  ...
  handleDownloadPDF() {
    try {
      ...
      let weeklyFilterLabel = 'All';

      if (this.weeklyViewFilterValue === 'Sleepover') {
          weeklyFilterLabel = 'Sleepover Shift';
      }
      else if (this.weeklyViewFilterValue === 'Extended') {
          weeklyFilterLabel = 'Extended/Overtime';
      }
      else if (this.weeklyViewFilterValue === 'SIL') {
          weeklyFilterLabel = 'SIL';
      }
      ...
      doc.text(
          `Selected Filter: ${weeklyFilterLabel}`,
          10,
          53
      );
    ...
    try {
     ...
     // =====================================
      ...
      let csvContent =
        `"${this.orgname || ''}"\n` +
        `"${this.statePostal || ''}"\n\n` +
        `"Staff","${this.StaffName1}"\n` +
        `"Applied Filter","${weeklyFilterLabel}"\n\n`;

      csvContent += [headers, ...dataRows]
  ...
  createMockShifts() {
    ...
    this.staffDirectory.forEach((staffRecord) => {
      ...
      const filteredShifts = staffSummary.allShifts.filter((shift) => {
      ...
      if (shouldFilterStaff && filteredShifts.length === 0) {
          ...
          console.log(
              `Skipping ${staffSummary.staffName} - no matching shifts`
          );
    ...
    const totalLoggedHours =
        logIn && logOut
            ? this.calculateHoursDifference(logIn, logOut)
            : 0;
    ...
    staffSummary.shifts.push({
        ...
        formattedLoggedHours:
            this.formatHoursToHrMin(totalLoggedHours),
        clientNames: Array.from(clientNames),
...
// =========================================
// USE FILTERED SHIFTS FOR ALL TOTALS
// =========================================
    ...
    staffSummary.hasShiftForAnyDay =
        staffSummary.shifts.length > 0;

    staffSummary.totalDuration = shiftsForTotals.reduce(
        (sum, s) => sum + (s.Duration__c || 0),
        0
    );

    staffSummary.totalMileage = shiftsForTotals.reduce(
        (sum, s) => sum + (s.Mileage__c || 0),
        0
    );

    staffSummary.totalExpense = shiftsForTotals.reduce(
        (sum, s) => sum + (s.Expense__c || 0),
        0
    );

    staffSummary.totalSleepoverHours = shiftsForTotals.reduce(
        (sum, s) => sum + (s.Sleepover_Shift_Hours__c || 0),
        0
    );

    staffSummary.varianceCount = shiftsForTotals.reduce(
        (sum, s) => sum + (s.Variance_Wage__c || 0),
        0
    );
    ...
    staffSummary.totalvariance = parseFloat(
        shiftsForTotals
            .reduce(
                (sum, s) =>
                    sum + (parseFloat(s.Extended_Duration__c) || 0),
                0
            )
            .toFixed(2)
    );
    ...
    // 🧾 Reimbursements
    shiftsForTotals.forEach((s) => {

        if (Array.isArray(s.Reimbursements__r)) {

            s.Reimbursements__r.forEach((r) => {

                staffSummary.totalMileageAmount +=
                    parseFloat(r.Mileage_Amount__c || 0);

                staffSummary.totalMileageOthers +=
                    parseFloat(r.Mileage_Others__c || 0);
            });
        }
    });
  ...
  get processedSummaries() {
    ...
    return this.paginatedSummaries.map((staff) => {
      ...
      const processedStaff = {
        ...staff,
        isSelected: staff.isSelected || false,
        shiftRows
      };
     /*  console.log(
        "✅ Processed Staff:",
  ...
  handleDownloadClickfortimesheet() {
    ...
    for (let i = 0; i < totalCols; i++) {
        ...
        if (i === 0) {
            orgNameRow.push(`"${this.orgname || ''}"`);
            orgAddressRow.push(`"${this.statePostal || ''}"`);
        } else {
            orgNameRow.push("");
            orgAddressRow.push("");
        }
    ...
    if (this.selectedFilterValue === 'Sleepover') {
        filterLabel = 'Sleepover Shift';
    }
    else if (this.selectedFilterValue === 'Extended') {
        filterLabel = 'Extended/Overtime';
    }
    else if (this.selectedFilterValue === 'SIL') {
        filterLabel = 'SIL';
    }
    ...
    for (let i = 0; i < totalCols; i++) {
        ...
        if (i === 0) {
            filterRow.push(`"Applied Filter: ${filterLabel}"`);
        }
        else {
            filterRow.push("");
        }
  ...
  handleDownloadPDFFortimesheet() {
    ...
    dateChunks.forEach((chunk, chunkIndex) => {
      ...
      // Facility title moved below header
      doc.setFontSize(12);
  ...
  addPdfHeader(doc) {
    ...
    const pageWidth = doc.internal.pageSize.getWidth();
      ...
      // ==========================
    // LOGO LEFT SIDE (FIT 40x20 NO STRETCH)
    // ==========================
    if (this.orgLogo) {
        try {
            ...
            // keep aspect ratio and fit inside box
            ...
            // optional: center inside 40x20 area
            const x = 10 + (maxWidth - imgWidth) / 2;
            const y = 8 + (maxHeight - imgHeight) / 2;
...
convertTableBlockToCsv(cell) {
  ...
  handleCheckboxStaffChange(event) {
      const staffId = event.target.dataset.id;
      const isChecked = event.target.checked;
      ...
      // Limit MYOB selection to 20
      if (
          this.AccountingServices === 'MYOB' &&
          isChecked &&
          this.selectedStaffIds.length >= 20
      ) {
          ...
          /* this.showToast(
              'Selection Limit',
              'For MYOB, you can select a maximum of 20 staff members at a time.',
              'warning'
          ); */
      ...
      this.staffMembers = this.staffMembers.map(staff => {
          if (staff.staffId === staffId) {
              return { ...staff, isSelected: isChecked };
          }
          return staff;
      });
      ...
      if (isChecked) {
          if (!this.selectedStaffIds.includes(staffId)) {
              this.selectedStaffIds = [...this.selectedStaffIds, staffId];
          }
      } else {
          this.selectedStaffIds = this.selectedStaffIds.filter(id => id !== staffId);
      }

      console.log('Selected IDs:', this.selectedStaffIds);
  }
   handleFilterChange(event) {
        this.selectedFilterValue = event.target.value;

        console.log('Selected Value:', this.selectedFilterValue);
        this.generateMockData();
    }

    handleWeeklyFilterChange(event){
      this.weeklyViewFilterValue = event.target.value;

        console.log('Selected Value:', this.weeklyViewFilterValue);
        console.log('this.StaffName1:', this.StaffName1);
        console.log('this.StaffId:',this.StaffId);
        this.applyWeeklyFilter();
    }
  ...
  applyWeeklyFilter() {
      ...
      // EXTENDED
          ...
          filteredShifts =
              this.originalSelectedStaffShifts.filter(
                  shift =>
                      shift.Extended_Duration__c > 0 ||
                      shift.Over_Time_Tier_One__c > 0 ||
                      shift.Over_Time_Tier_Two__c > 0
      ...
      // SLEEPOVER
      else if (this.weeklyViewFilterValue === 'Sleepover') {
      ...
      // SIL
          ...
          filteredShifts =
              this.originalSelectedStaffShifts.filter(
                  shift => shift.SIL__c === true
              );
      }

      this.updateWeeklyTotals(filteredShifts);
  }

  updateWeeklyTotals(shifts) {

      this.selectedStaffShifts = shifts;

      this.selectedShiftIds =
          shifts.map(shift => shift.Id);

      this.totalShiftDuration = shifts.reduce(
          (sum, shift) =>
              sum + (parseFloat(shift.totalDuration) || 0),
          0
      );
      this.totalShiftDuration =
          parseFloat(this.totalShiftDuration.toFixed(2));

      this.totalKms = shifts.reduce(
          (sum, shift) =>
              sum + (parseFloat(shift.mileageOthersTotal) || 0),
          0
      );
      this.totalKms =
          parseFloat(this.totalKms.toFixed(2));

      this.totalexpenses = shifts.reduce(
          (sum, shift) =>
              sum + (parseFloat(shift.mileageAmountTotal) || 0),
          0
      );
      this.totalexpenses =
          parseFloat(this.totalexpenses.toFixed(2));

      this.sleepoverDuration = shifts.reduce(
          (sum, shift) =>
              sum + (parseFloat(shift.Long_Sleepover_Duartion__c) || 0),
          0
      );
      this.sleepoverDuration =
          parseFloat(this.sleepoverDuration.toFixed(2));

      this.totalApprovedLoggedDuration = shifts.reduce(
          (sum, shift) => {
              return shift.Approval_Status__c === 'Approved'
                  ? sum + (parseFloat(shift.totalDuration) || 0)
                  : sum;
      ...
      this.totalApprovedLoggedDuration =
          parseFloat(this.totalApprovedLoggedDuration.toFixed(2));
  }

  /* handleupdateActualTimeClick(event) {
      const shiftId = event.currentTarget.dataset.id;

      updateActualStartTime({ shiftId })
          .then(() => {
              if (this.currentViewFlag === "check") {
                this.handleCheckClick({
                  currentTarget: { dataset: { staffId: this.StaffId } }
              ...
              this.showToast(
                  'Success',
                  'Actual Start Time updated successfully.',
                  'success'
              );
          })
          .catch(error => {
              this.showToast(
                  'Error',
                  error?.body?.message || 'Failed to update Actual Start Time.',
                  'error'
              );
              console.error(error);
          });
  ...
  actualStartShiftId;
  actualStartFlag = false;

  handleupdateActualTimeClick(event) {
      this.actualStartShiftId  = event.currentTarget.dataset.id;
      this.actualStartFlag = true;
  }

  handleActualStartCancel() {
      this.actualStartFlag = false;
      this.actualStartShiftId = null;
  }

  handleActualStartConfirm() {

      updateActualStartTime({ shiftId: this.actualStartShiftId })
          .then(() => {
            if (this.currentViewFlag === "check") {
                this.handleCheckClick({
                  currentTarget: { dataset: { staffId: this.StaffId } }
  ...
  actualEndFlag = false;
  pendingEndTimeShiftId;
  
  handleUpdateActualEndTimeClick(event) {
      this.pendingEndTimeShiftId = event.currentTarget.dataset.id;
      this.actualEndFlag = true;
  }

  handleActualEndTimeCancel() {
      this.actualEndFlag = false;
      this.pendingEndTimeShiftId = null;
  }

  handleActualEndTimeConfirm() {

      console.log(
          'pendingEndTimeShiftId =',
          this.pendingEndTimeShiftId
      );

      updateActualEndTime({ shiftId: this.pendingEndTimeShiftId })
          .then(() => {
            if (this.currentViewFlag === "check") {
                this.handleCheckClick({
                  currentTarget: { dataset: { staffId: this.StaffId } }
            ...
            this.showToast(
                'Success',
                'Actual End Time updated successfully.',
                'success'
            );
            this.actualEndFlag = false;
            this.pendingEndTimeShiftId = null;
          ...
          .catch(error => {
              console.error(
                  'updateActualEndTime Error:',
                  JSON.stringify(error)
              );

              this.showToast(
                  'Error',
                  error?.body?.message || 'Failed to update Actual End Time.',
                  'error'
              );
          });
  }

  isStartTimeCopied = false;
  isEndTimeCopied = false;

  handleupdateEditActualTimeClick() {
      console.log('===== CLONE START TIME CLICKED =====');
      this.isStartTimeCopied = true;
  }

  handleupdateEditActualEndTimeClick() {
      console.log('===== CLONE END TIME CLICKED =====');
      this.isEndTimeCopied = true;
  }

  // START TIME POPUP
  handleStartTimeCancel() {
      console.log('===== START TIME COPY CANCELLED =====');
      this.isStartTimeCopied = false;
  }

  handleStartTimeConfirm() {
      console.log('===== START TIME COPY CONFIRMED =====');

      this.isStartTimeCopied = false;

      const startTimeField = this.template.querySelector(
          '[data-id="actualLoginTime"]'
      );

      if (startTimeField && startTimeField.value) {

          const display = startTimeField.value; // e.g. "9:00 AM"
          ...
          const twentyFourHourFormat =
              `${String(hour24).padStart(2, '0')}:${String(minute).padStart(2, '0')}:00`;

          // Reset first to force child component refresh
          this.startTimeSelectedHour1 = null;
          this.startTimeSelectedMinute1 = null;
          this.startTimeAMPM1 = null;
          this.AddShiftStartTimeAMPM1 = null;

          // Small delay so LWC rerenders
          setTimeout(() => {
              this.startTimeSelectedHour1 = String(hour);
              this.startTimeSelectedMinute1 = String(minute).padStart(2, '0');
              this.startTimeAMPM1 = ampm;
              this.AddShiftStartTimeAMPM1 = display;
              this.startTimeInMs = twentyFourHourFormat;

              console.log('✅ Start Time Copied →', {
                  display: this.AddShiftStartTimeAMPM1,
                  hour: this.startTimeSelectedHour1,
                  minute: this.startTimeSelectedMinute1,
                  ampm: this.startTimeAMPM1,
  ...
  // END TIME POPUP
  handleEndTimeCancel() {
      console.log('===== END TIME COPY CANCELLED =====');
      this.isEndTimeCopied = false;
  }

  handleEndTimeConfirm() {
      console.log('===== END TIME COPY CONFIRMED =====');

      this.isEndTimeCopied = false;

      const endTimeField = this.template.querySelector(
          '[data-id="actualLogoutTime"]'
      );

      if (endTimeField && endTimeField.value) {

          const display = endTimeField.value; // e.g. "5:00 PM"
          ...
          setTimeout(() => {
              this.endTimeSelectedHour1 = String(hour);
              this.endTimeSelectedMinute1 = String(minute).padStart(2, '0');
              this.endTimeAMPM1 = ampm;
              this.AddShiftEndTimeAMPM1 = display;
              this.endTimeInMs = twentyFourHourFormat;

              console.log('✅ End Time Copied →', {
                  display: this.AddShiftEndTimeAMPM1,
                  hour: this.endTimeSelectedHour1,
                  minute: this.endTimeSelectedMinute1,
                  ampm: this.endTimeAMPM1,
          ...
          console.error('End Time field not found');
      }
  }
...
handleShiftDetails(event) {
    ...
    const staffId = event.currentTarget.dataset.staff;
    const date = event.currentTarget.dataset.date;

    console.log("========== HANDLE SHIFT DETAILS ==========");
    console.log("Staff Id:", staffId);
    ...
    this.isShiftClosing = false;
    this.isShiftDetailsOpen = true;

    requestAnimationFrame(() => {

        const panel = this.template.querySelector("section.shiftPanel");

        if (!panel) {
            console.log("Shift panel not found");
            return;
        }
        ...
        const ctxRect = ctxEl
            ? ctxEl.getBoundingClientRect()
            : { top: 0, left: 0 };

        let left = (clickedRect.right - ctxRect.left) + gap;

        left = Math.min(
            left,
            (window.innerWidth - ctxRect.left) - panelWidth - padding
        );

        left = Math.max(padding, left);

        const iconCenterY =
            clickedRect.top + clickedRect.height / 2;

        let top =
            (iconCenterY - ctxRect.top) - pointerAnchor;

        const maxTop =
            (window.innerHeight - ctxRect.top)
            - panelHeight
            - padding;
        ...
        const pointerTop =
            (iconCenterY - ctxRect.top)
            - top;

        this.shiftPanelStyle =
            `top:${top}px;
            left:${left}px;
            --availPointerTop:${pointerTop}px;`;

        console.log("Shift Panel Style:", this.shiftPanelStyle);
    ...
    const staff = this.processedStaff.find(
        s => s.staffId === staffId
    );
    ...
    const shiftRow = staff.shiftRows.find(
        r => r.date === date
    );
    ...
    if (!shiftRow) {
        console.log("Shift row not found");
        this.selectedShiftList = [];
        return;
    }

    console.log("shiftRow:", JSON.stringify(shiftRow, null, 2));
    console.log("shiftRow.shifts:", JSON.stringify(shiftRow.shifts, null, 2));

    // ==========================================================
    // Assign shifts to popup
    // ==========================================================

    this.selectedShiftList = shiftRow.shifts;

    console.log("isShiftDetailsOpen:", this.isShiftDetailsOpen);
    console.log("selectedShiftList length:", this.selectedShiftList.length);

    requestAnimationFrame(() => {
        console.log(
            "Popup exists:",
            this.template.querySelector("section.shiftPanel")
        );
    });

    console.log("Selected Shift List:");
    console.log(JSON.stringify(this.selectedShiftList, null, 2));
...
shiftStopInsideClick(event) {
    event.stopPropagation();
}

closeShiftDetails = (event) => {
  if (event) event.stopPropagation();
  if (!this.isShiftDetailsOpen) return;

  this.isShiftClosing = true;
  ...
  window.setTimeout(() => {
    this.isShiftDetailsOpen = false;
    this.isShiftClosing = false;
  }, 180);
};

// get shiftPanelClass() {
//   return `availPanel ${this.isShiftClosing ? 'availClosing' : 'availOpen'}`;
// }

get shiftPanelClass() {
    return this.isShiftClosing
        ? "availPanel availClosing shiftPanel"
        : "availPanel availOpen shiftPanel";
}


@track isSelectAllChecked = false;

handleCheckboxAllStaffChange(event) {
    const isChecked = event.target.checked;

    console.log('--- Select All Checkbox Click ---');
    console.log('Checked:', isChecked);

    // Get current page staff IDs
    const currentPageStaffIds = this.paginatedStaff.map(staff => staff.staffId || staff.Id);

    // Update staffMembers (source of truth)
    this.staffMembers = this.staffMembers.map(staff => {
        if (currentPageStaffIds.includes(staff.staffId || staff.Id)) {
            return {
                ...staff,
    ...
    // Update allStaff for consistency
    if (this.allStaff) {
    ...
    // Update selectedStaffIds list
    if (!this.selectedStaffIds) {
        this.selectedStaffIds = [];
    }

    if (isChecked) {
        // Add all current page staff IDs that aren't already selected
        const newIds = currentPageStaffIds.filter(id => !this.selectedStaffIds.includes(id));
        this.selectedStaffIds = [...this.selectedStaffIds, ...newIds];
    } else {
        // Remove current page staff IDs from selection
        this.selectedStaffIds = this.selectedStaffIds.filter(id => !currentPageStaffIds.includes(id));
    }

    // Update Select All checkbox state
    this.isSelectAllChecked = isChecked;

    console.log('Selected IDs after Select All:', this.selectedStaffIds);
}
...
updateSelectAllState() {
    ...
    if (currentPageStaffIds.length === 0) {
        this.isSelectAllChecked = false;
        return;
    }
    
    // Check if all staff on current page are selected
    const allSelected = currentPageStaffIds.every(id => 
...
async handleConfirmAndSend(event) {
    ...
    try {
        ...
        if (this.isBulkMode) {
            // Use bulkStaffIds for bulk mode (preserved from handleAddToXero1)
            if (this.bulkStaffIds && this.bulkStaffIds.length > 0) {
                staffIds = [...this.bulkStaffIds];
                console.log('📋 Bulk mode sending for ' + staffIds.length + ' staff members from bulkStaffIds');
            } else if (this.selectedStaffIds && this.selectedStaffIds.length > 0) {
                staffIds = [...this.selectedStaffIds];
                console.log('📋 Bulk mode sending for ' + staffIds.length + ' staff members from selectedStaffIds');
            } else {
                // Fallback - use the first staff
                staffIds = [this.selectedStaffForRates];
                console.log('⚠️ No bulk staff IDs found, using single staff');
            }
            
            console.log('📋 Staff IDs to send:', staffIds);
            
            // Build selections for all staff
            // ✅ FIX: In bulk mode, apply the SAME selections to ALL staff
            // because the UI shows combined categories and user picks ONE rate per category
            const firstStaffId = staffIds[0];
            if (this.selectedRateMappings[firstStaffId]) {
                const firstStaffSelections = this.selectedRateMappings[firstStaffId];
                staffIds.forEach(id => {
                    if (!selections[id]) {
                        selections[id] = {};
                    }
                    // Copy ALL selections from first staff to all others
                    for (const [category, rateId] of Object.entries(firstStaffSelections)) {
                        selections[id][category] = rateId;
                    }
                    console.log('📋 Applied selections to staff:', id, selections[id]);
                });
            } else {
                // Fallback - use the categoryRateOptions directly
                staffIds.forEach(id => {
                    if (!selections[id]) {
                        selections[id] = {};
                    }
                    this.categoryRateOptions.forEach(cat => {
                        if (cat.selectedValue) {
                            selections[id][cat.category] = cat.selectedValue;
                        }
                    });
                });
            }
            ...
            // Single staff mode
            staffIds = [this.selectedStaffForRates];
            if (this.selectedStaffForRates && this.selectedRateMappings[this.selectedStaffForRates]) {
                selections[this.selectedStaffForRates] = this.selectedRateMappings[this.selectedStaffForRates];
            } else {
                // Fallback - use categoryRateOptions
                const staffId = this.selectedStaffForRates;
                if (!selections[staffId]) {
                    selections[staffId] = {};
                }
                this.categoryRateOptions.forEach(cat => {
                    if (cat.selectedValue) {
                        selections[staffId][cat.category] = cat.selectedValue;
                    }
                });
            }
        }
...
highlightUnselectedCategories(unselectedCategories) {
    // Get all category container elements
    const categoryElements = this.template.querySelectorAll('[data-category-container]');
    
    categoryElements.forEach(el => {
        const categoryName = el.dataset.categoryName;
        const isUnselected = unselectedCategories.some(cat => cat.category === categoryName);
        
        if (isUnselected) {
            el.style.border = '2px solid #c23934';
            el.style.backgroundColor = '#fdf8f8';
            el.style.boxShadow = '0 0 0 3px rgba(194, 57, 52, 0.2)';
            
            // Add error message below the category
            const existingError = el.querySelector('.category-error-message');
            if (!existingError) {
                const errorDiv = document.createElement('div');
                errorDiv.className = 'category-error-message';
                errorDiv.style.cssText = 'color: #c23934; font-size: 12px; margin-top: 8px; display: flex; align-items: center; gap: 4px;';
                errorDiv.innerHTML = `
                    <lightning-icon icon-name="utility:error" size="xx-small" style="--slds-c-icon-color-foreground: #c23934;"></lightning-icon>
                    Please select an earning rate
                `;
...
handleXeroResult(result) {
    console.log('🔍 handleXeroResult called with:', JSON.stringify(result, null, 2));

    // ✅ IMPORTANT: Save the current rate selections before showing the modal
    // This preserves them for the "Send Anyway" flow
    this._savedRateMappings = JSON.parse(JSON.stringify(this.selectedRateMappings || {}));
    this._savedCategoryRateOptions = JSON.parse(JSON.stringify(this.categoryRateOptions || []));
    this._savedOrdinaryEarningsRateId = this.ordinaryEarningsRateId;
    this._savedStaffForRates = this.selectedStaffForRates;
    this._savedBulkStaffIds = this.bulkStaffIds ? [...this.bulkStaffIds] : [];
    this._savedSelectedStaffIds = this.selectedStaffIds ? [...this.selectedStaffIds] : [];
    
    console.log('💾 Saved rate selections for Send Anyway:', JSON.stringify(this._savedRateMappings, null, 2));
    ...
    if (result.length === 0) {
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Info',
                message: 'No staff were processed.',
                variant: 'info'
            })
        );
        return;
    ...
    // SEPARATE RESULTS
    // ============================================================
    const popupErrors = result.filter(item => item.showInPopup === true);
    const successItems = result.filter(item => item.success === true);
    const regularErrors = result.filter(item => item.success !== true && item.showInPopup !== true);

    console.log('📊 Success items:', successItems.length);
    console.log('📊 Popup errors:', popupErrors.length);
    console.log('📊 Regular errors:', regularErrors.length);

    // ============================================================
    // BUILD MODAL ITEMS - Include ALL items
    // ============================================================
    let allModalItems = [];

    // 1. Add popup errors (with pay period handling)
    popupErrors.forEach(item => {
        let message = item.message || 'Unknown error';
        
        // Check if this is a pay period mismatch
        if (item.payPeriodMismatch === true) {
            if (
                item.suggestedStartDate &&
                item.suggestedEndDate
            ) {
                message = `The selected timesheet period does not match this staff member's Xero pay cycle.\n                Suggested pay period: ${this.formatDateforXero(item.suggestedStartDate)} to ${this.formatDateforXero(item.suggestedEndDate)}.`;
            } else {
                message = `The selected timesheet period does not match this staff member's Xero pay cycle.`;
            }
        }
        ...
        allModalItems.push({
            staffId: item.staffId,
            staffName: item.staffName || 'Unknown',
            payPeriodMismatch: item.payPeriodMismatch === true,
            selected: false,
            suggestedStartDate: item.suggestedStartDate || null,
            suggestedEndDate: item.suggestedEndDate || null,
            suggestedStartDay: item.suggestedStartDay || null,
            suggestedEndDay: item.suggestedEndDay || null,
            message: message,
            isSuccess: false
        });
    });

    // 2. Add regular errors (showInPopup: false) - KEEP ORIGINAL MESSAGE
    regularErrors.forEach(item => {
        let errorMsg = item.message || 'Unknown error';
        ...
        } else if (errorMsg && errorMsg.toLowerCase().includes('not found')) {
            errorMsg = 'Staff member not found in Xero. Please check the Xero Employee ID.';
        } else if (errorMsg && errorMsg.toLowerCase().includes('unauthorized')) {
            errorMsg = 'Xero authentication failed. Please re-authorize the connection.';
        }
    ...
    successItems.forEach(item => {
        allModalItems.push({
            ...
            suggestedStartDay: null,
            suggestedEndDay: null,
            message: '✅ ' + (item.message || 'Timesheet sent to Xero successfully!'),
            isSuccess: true
        });
    });
    ...
    // ============================================================
    const hasPayPeriod = popupErrors.some(item => item.payPeriodMismatch === true);
    const hasRegularErrors = regularErrors.length > 0;
    const hasPopupErrors = popupErrors.length > 0;
    const hasSuccess = successItems.length > 0;

    if (hasPopupErrors && !hasRegularErrors && !hasSuccess) {
        if (hasPayPeriod && popupErrors.some(item => item.payPeriodMismatch !== true)) {
            this.xeroModalTitle = 'Xero Validation Results';
        } else if (hasPayPeriod) {
            this.xeroModalTitle = 'Pay Period Mismatch';
        } else {
    ...
    } else if (hasPopupErrors || hasRegularErrors) {
        if (hasSuccess) {
            this.xeroModalTitle = 'Xero Timesheet Results';
        } else {
            this.xeroModalTitle = 'Xero Error';
        }
    } else if (hasSuccess && !hasPopupErrors && !hasRegularErrors) {
        this.xeroModalTitle = 'Xero Success';
    } else {
        this.xeroModalTitle = 'Xero Results';
    }
    ...
    // Set modal items
    this.xeroModalItems = allModalItems;

    console.log('Final Modal Items:', JSON.stringify(this.xeroModalItems));
    
    // Show modal if there are any items
    if (this.xeroModalItems.length > 0) {
        this.showXeroResultModal = true;
    }
}

handleCloseXeroModal() {
    console.log('🔒 Closing Xero result modal');
    this.showXeroResultModal = false;
    this.xeroModalItems = [];
    this.xeroModalTitle = '';
    this.showSendAnywayButton = false;
    ...
    this.isLoading = false;
    // ✅ DO NOT clear selectedRateMappings or categoryRateOptions
    // They are needed for "Send Anyway"
}

async handleMyobBulkSend() {
    // Existing MYOB bulk send logic
    const savedToken = sessionStorage.getItem('myobAccessToken');
    const refreshToken = sessionStorage.getItem('myobRefreshToken');
    const savedCompanyFileUri = sessionStorage.getItem('companyFileUri');
    ...
    try {
        ...
        if (errorCount === 0) {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Success',
                    message: `${successCount} timesheets sent to MYOB successfully!`,
                    variant: 'success'
                })
            );
        } else {
            const errors = results.filter(r => !r.success);
            const errorMessages = errors.map(r => `${r.staffName}: ${r.message}`).join('\n');
        ...
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Error',
                message: error?.body?.message || error?.message || 'Failed to send to MYOB',
                variant: 'error'
            })
        );
    } finally {
...
// Computed getters - add these to your component
get hasCategories() {
    console.log('hasCategories called:', this.categoryRateOptions);
    console.log('hasCategories result:', this.categoryRateOptions && this.categoryRateOptions.length > 0);
    return this.categoryRateOptions && this.categoryRateOptions.length > 0;
}

get isSendButtonDisabled() {
    if (this.isLoadingRates) return true;
    
    const hasRatesCategories = this.categoryRateOptions.filter(cat => cat.hasRates);
    if (hasRatesCategories.length === 0) return true;
    
    // Check if all categories with rates have a selected value
    const allSelected = hasRatesCategories.every(cat => cat.selectedValue);
    return !allSelected;
}

get noCategoriesMessage() {
    return 'No shift categories found for this staff in the selected period.';
}

get loadingMessage() {
    return 'Fetching earning rates from Xero...';
}
