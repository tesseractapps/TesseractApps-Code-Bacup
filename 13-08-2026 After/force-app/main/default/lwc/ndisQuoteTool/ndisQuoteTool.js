...
export default class NdisQuoteTool extends LightningElement {
   ...
   @track contactsId=null;
    ...
    facilityLabel = '';
    logo;
    @track noContactRecordsFlag = false;
    ...
    wiredData({ data, error }) {
        if (data) {
            ...
            const industry = this.industryType; // or this.selectedIndustry
    ...
    loadNDISData() {
        ...
        let options = Object.keys(this.catalogue).map(type => ({
            label: type,
            value: type
        }));
         console.log('options :', JSON.stringify(options));
        if (industry === 'NDIS') {
            // Hide Others & Miscellaneous
            options = options.filter(({ value }) =>
                    !value.startsWith('Others') &&
                    !value.startsWith('Miscellaneous -')
             );
            console.log('options in NDIS:', JSON.stringify(options));
        } else {
            // Move Others & Miscellaneous to the end
           options = options.filter(({ value }) =>
                !value.startsWith('Miscellaneous') &&
                !value.startsWith('Others -')
            );

            console.log('options in Non- NDIS:', JSON.stringify(options));
        }
         options.sort((a, b) => {
...
/* =========================
    ...
    resizeObserver = null;
    pageSizeBulk = 'Auto';
    isPageSizeBulkManuallySet = false;

    get pageSizeOptions() {
        return ['Auto', 5, 10, 20, 50, 100];
    }
    ...
    async  connectedCallback() {
        try {
            ...
            // Load all facilities
            const facresponse = await getFacilityData();
            ...
            // Facility Admin / HR Admin / Roster Manager
            else if (
                ...
                const result = await getFacilityCurrentUser();
            ...
            this.multiFacilityDroDownList = [...this.finalListFacilities];
            this.orginalSelectedFacilities = [...this.finalListFacilities];
            ...
            const selectedFacility = this.multiFacilityDroDownList.find(
                item => item.value === this.selectedFacilityId
            );
            ...
            console.log('Initial Selected Facility:', this.selectedFacilityId);

            // Refresh wired methods
            //this.refreshKey++;
            ...
            // console.error('❌ PDF load failed', e);
             console.error('❌ connectedCallback initialization failed', e);
            this.pdfReady = false;
        }
    }
    ...
    loadOrganizationDetails() {
        organizationDetails()
            .then(response => {
                console.log('Organization Details:', JSON.stringify(response));
                this.orgLogo= response.bolbdata

                console.log('Organization Id:', this.orgLogo);

                const orgTier = response.listofPriceBook.Organization_Tier__c;
                console.log('Organization Tier:', orgTier);

                this.isStart = (orgTier === 'Start');
    ...
    initResizeObserver() {
        const activeBodyScroll = Array.from(this.template.querySelectorAll('.body-scroll'))
            .find(el => el.offsetParent !== null);
            
        if (activeBodyScroll) {
            if (this.resizeObserver) {
                if (this._observedElement !== activeBodyScroll) {
                    this.resizeObserver.disconnect();
                    this.resizeObserver.observe(activeBodyScroll);
                    this._observedElement = activeBodyScroll;
                }
            } else {
    ...
    setPageSizeByZoomAndScreen() {
        const bodyScrolls = this.template.querySelectorAll('.body-scroll');
        bodyScrolls.forEach(bodyScroll => {
            if (bodyScroll.offsetParent !== null) {
                const availableHeight = bodyScroll.clientHeight;
                const row = bodyScroll.querySelector('tr');
                const rowHeight = row ? row.offsetHeight : 45;
                
                if (availableHeight > 0) {
                    const calculatedSize = Math.max(1, Math.floor(availableHeight / rowHeight));
                    
                    if (this.isQuoteDraftScreen) {
                        if (!this.isPageSizeBulkManuallySet) {
                            this.draftPageSize = calculatedSize;
                        ...
                        if (!this.isPageSizeManuallySet) {
                            ...
                            } else if (this.isTSignHistoryScreen) {
...
@track industryType='';
    ...
    this.industryType = event.target.dataset.industry;
        this.builderMode = 'NEW'; 
        this.lastScreen = 'NEW_QUOTE';  //Static value
    console.log('🆕 New Quote → Participant selected:', clientId);
    console.log('🆕this.industryType  → Participant selected:',this.industryType );
    this.resetflag=true;
    ...
    this.resetBuilderState?.();
        // Manendra added for Pre-populate Quote State from Participant State
    if (this.selectedClient?.State__c) {
        this.state = this.stateLabelToValueMap[this.selectedClient.State__c] || '';
    }
    console.log('Participant State:', this.selectedClient?.State__c);
        ...
        .then(result => {
           ...
           const filteredByFacility = (result || []).filter(r =>
            ...
            this.clientList = filteredByFacility.map(r => {
               ...
               // const industry= r?.Facility__r?.Type_of_Service__c || '';
                const participantFacility = r.Participant_Facilities__r?.[0];
                const industry =participantFacility?.Facility__r?.Type_of_Service__c || '';
...
paginateParticipants() {
  ...
  console.log('records IN PAGNATION ' +JSON.stringify(records));
    ...
    this.noContactRecordsFlag = this.totalNewContacts === 0;
...
// }
    ...
    openModalForEdit(event) {
        ...
        getClients({ clientId: this.clientId })
            .then(result => {
                ...
                this.selectedFacilityId = result.Facility__c;
            })
   ...
   handleStateChange(event) { this.state = event.detail.value; }
    ...
    handleRoleChange(event) { this.role = event.detail.value; }
     handleNdisId(event){ this.NdisId = event.detail.value;}
...
resetNewContactForm() {
    ...
    handleSave() {
        ...
        const newContact = {
            ...
            Ndis_Id__c: this.NdisId,
            Facility__c: this.selectedFacilityId 
        };
...
handleNewContactSelect(event) {
    ...
    console.log('🆕this.industryType  → New Contact selected:',this.industryType );
    ...
    getQuoteByIdWithLines({ quoteId })
        .then(result => {
           ...
           this.quoteState = this.state;
            ...
            this.serviceEnd = quote.Service_Period_End__c || '';
            this.preparedBy = quote.Prepared_By__c || '';
            this.additionalNotes = quote.Additional_Notes2__c || '';
            this.quotePdfUrl = quote.QuotePdf_Url__c || null;
           ...
           // this.quotePdfUrl = quote.QuotePdf_Url__c || null;
            ...
            this.quoteSupportItems = lineItems.map(li => {
                ...
                const supportItemOptions = serviceType? Object.keys(this.catalogue[serviceType]).map(k => ({
                    label: `${this.catalogue[serviceType][k].name} - ${this.catalogue[serviceType][k].code}`,
                    value: k
                }))
                : [];

                const selectedSupportItem = supportItemOptions.find(
                        opt => opt.value === li.Support_Item__c
                    );
                ...
                return {
                    ...
                    supportItemLabel:selectedSupportItem?.label || li.Support_Item__c,
                    ...
                    supportItemOptions: serviceType
                        ? Object.keys(this.catalogue[serviceType]).map(k => ({
                            label: `${this.catalogue[serviceType][k].name} - ${this.catalogue[serviceType][k].code}`,
                            value: k,
                            icon: this.catalogue[serviceType][k].isOrgRecord
                                ? 'assured_workload'
                                : 'business'
                        }))
                        : [],
...
@track lastScreen = null;
    ...
    this.quoteSupportItems = this.quoteSupportItems.map(row => {
        if (row.id === rowId) {
            ...
            const selectedOption = this.serviceTypeOptions?.find(
                opt => opt.value === serviceType
            );
            row.serviceTypeLabel = selectedOption?.label || serviceType;
           ...
           /*  row.supportItemOptions = Object.keys(itemsMap).map(key => {
            ...
            // row.supportItemOptions = Object.keys(itemsMap).map(key => {
            //             row.catalogueMap[key] = itemsMap[key].id;

            //             return {
            //                 label: `${itemsMap[key].name} - ${itemsMap[key].code}`,
            //                 value: key,
            //                 isOrgRecord: itemsMap[key].isOrgRecord,
            //             };
            //         });
    ...
    this.quoteSupportItems = this.quoteSupportItems.map(row => {
        if (row.id === rowId) {
            ...
            console.log('✅ catalogueId resolved:', row.catalogueId);
             ...
             const selectedOption = row.supportItemOptions?.find(
                    opt => opt.value === value
                );
             row.supportItemLabel = selectedOption?.label || value;

            const itemData =
                this.catalogue[row.serviceType]?.[value];
            row.catalogueId = itemData?.id || null;
            row.code = itemData?.code || '';
 ...
 await this.waitForPdfReady();
    ...
    // });
        ...
        loadDraftQuotes() {
            ...
            getDraftQuotes({facilityId: this.facilityId})
...
resetPagination() {
    ...
    loadNewContacts(search = '') {
        ...
        getNewContacts({ searchTerm: search, facilityId: this.facilityId })
            .then(result => {
                ...
                this.newContactList = result.map(c => {
                    ...
                    const initials = name
                        ...
                        .substring(0, 2)
                        .toUpperCase();
                    const industryType = c.Facility__r
                                        ? c.Facility__r.Type_of_Service__c
                                        : '';
 ...
 getPdfClient() {
    if (this.selectedEntityType === 'PARTICIPANT') {
        ...
        if (this.selectedEntityType === 'NEW_CONTACT' && this.newContact) {
            console.log('ndis id included',JSON.stringify(this.newContact));
        return {
            name: this.newContact?.Name || '-',
            ndis: this.newContact?.Ndis_Id__c || '-', // no NDIS for new contact
...
async loadNewContactForPdf() {
 ...
 async  generateQuotePdfBase64() {
    ...
    if (!this.pdfReady) {
      ...
      const getIntervalLabel = (freq, interval) => {
        if (!interval) return '';
    ...
    if (this.orgLogo) {
                try {
                   ...
                   /*  const logoX = 10 + (maxWidth - imgWidth) / 2;
                    ...
                    const logoX = MARGIN;   // Same starting X as organization name
                    const logoY = 5;
...
y += CARD_HEIGHT + 8;
    ...
    body: lines.map(li => ([
        `${li.supportItem} - ${li.code || ''} \n ${li.freq || 1} x ${li.interval?.toLowerCase()}, ${li.dur || 0} ${getIntervalLabel(li.dur, li.durationPeriod) || ''}`,
        li.serviceType || '',
        `${li.qty} Hour`,
        money(li.price),
        money(li.cost)
...
//     get bDisableLast() { return this.pageNumber === this.totalPages || this.totalPages === 0; }
    ...
    get selectedTableTitle() {
        switch (this.selectedStatus) {
            ...
            case 'Pending': return 'In Progress Documents';
            case 'Completed': return 'Processed Documents';
...
updateDisplayedRecords() {
    console.log(`🔄 Updating records for status: ${this.selectedStatus}`);
     console.log('this.allDocuments',JSON.stringify(this.allDocuments));
    let filteredData = [];
...
getCurrentDataArray() {
    switch (this.selectedStatus) {
        ...
        case 'Pending': return this.inProgressDocuments;
        case 'Completed': return this.processedDocuments;
...
closeQuote() {
    ...
    getFrequencyLabel(freq, interval) {
        ...
        console.log('Service Type:', contact.industryType);
        ...
        this.email = contact.Email__c;
        this.phone = contact.Phone__c;
        this.role = contact.Role__c;
        this.state = this.stateLabelToValueMap[contact.State__c];
        this.NdisId = contact.Ndis_Id__c;
        this.selectedFacilityId = contact.Facility__c;
        this.industryType = contact.industryType;
    }
