...
handleAbnSelection(event) {
    ...
    connectedCallback(){
       ...
       this.hideParentHandler();
        ...
        if(this.isCustomer==='Customer'){
            console.log('isCustomer in  connectedCallback : '+ this.isCustomer);
    ...
    handleSubmitProfile(event) {
        ...
        const requiredFields = ['Name__c', 'First_Name__c', 'Last_Name__c'];
        const missingField = requiredFields.find(fieldName => !fields[fieldName] || !this.template.querySelector(`lightning-input-field[field-name="${fieldName}"]`));
        // ✅ STANDARD FIELD VALIDATION (optional but recommended)
        // ✅ Validate lightning-input only
      ...
      // this.template.querySelector('lightning-record-edit-form[data-recordform="profileForm"]').submit(fields);
        ...
        if(this.isCustomer === 'Supplier'){
            if (!this.selectedOption || !this.supplierSearchTerm || !this.street || !this.city || !this.country || !this.postalcode || !this.province) {                                                              
        ...
        if(this.isCustomer === 'Customer'){
            if(!this.selectedOption1 || !this.customerSearchTerm || !this.street || !this.city || !this.country || !this.postalcode || !this.province) {
                this.showToast('Error', 'Please select the required fields. ', 'error');
                return; 
            }
        }
        ...
        */
        ...
        if (this.isCustomer === 'Supplier') {
            ...
            if (!this.selectedOption) {
                this.showToast('Error', 'Please select a Tax Code.', 'error');
                return;
            }

            if (!this.supplierSearchTerm || !this.supplierSelectedAccountId) {
                // this.showToast('Error', 'Please select an Account List from the dropdown.', 'error');
                this.showToast('Error', 'Please verify the account list selection (it may be auto-filled from cached data) and complete the entity configuration before proceeding.', 'error');
                return;
            }

            // if (!this.street || !this.city || !this.country || !this.postalcode || !this.province) {
            //     this.showToast('Error', 'Please complete the address fields.', 'error');
            //     return;
            // }
        }
        ...
        if (this.isCustomer === 'Customer') {
            ...
            if (!this.customerSearchTerm || !this.customerSelectedAccountId) {
                // this.showToast('Error', 'Please select an Account List from the dropdown.', 'error');
                this.showToast('Error', 'Please verify the account list selection (it may be auto-filled from cached data) and complete the entity configuration before proceeding.', 'error');
                return;
            }
