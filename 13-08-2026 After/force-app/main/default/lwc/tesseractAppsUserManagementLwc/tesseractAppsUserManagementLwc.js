...
export default class TesseractAppsUserManagement extends LightningElement {
    ...
    set contactData(value) {
        ...
        if (value && Object.keys(value).length > 0) {
            ...
            this.selectedUserType = 'NDIS Participants';
    ...
    taxFreeOptions = [
        ...
        { label: 'No Tax-Free Threshold + FS', value: 'No Tax-Free Threshold + FS' },
        { label: 'Tax Free Threshold + FS', value: 'Tax Free Threshold + FS' },
        { label: 'Tax Free Threshold + HELP', value: 'Tax Free Threshold + HELP' },
        { label: 'Tax Free Threshold HELP + FS', value: 'Tax Free Threshold HELP + FS' },
        { label: 'Tax Free Threshold + STSL', value: 'Tax Free Threshold + STSL' },
        { label: 'No Tax-Free Threshold +STSL', value: 'No Tax-Free Threshold +STSL' }
    ];
    ...
    moduleMap = {
        ...
        'SC with NDIS Org Admin': ['Admin', 'Human Resources', 'Incident Register', 'Participants', 'Repository', 'Roster Manager', 'Sign In','My Profile','Accounting', 'Access Manager', 'T sign', 'Forms', 'Reports', 'Support Coordinator'],
        ...
        'SC with Roster Manager' : ['Roster Manager', 'Human Resources', 'Sign In', 'Participants','Accounting', 'T sign', 'Access Manager', 'Incident Register', 'Repository', 'My Profile', 'Forms', 'Reports', 'Support Coordinator'],
    ...
    // Modified loadUserTypeValues method
    loadUserTypeValues() {
        ...
        .then(result => {
            ...
            // ✅ Store in new variable for filtered values
            ...
            // Log each value
            this.filteredUserTypeValues.forEach((item, index) => {
        ...
        .catch(error => {
            ...
            this.filteredUserTypeValues = []; // Set to empty array on error
            this.isLoading = false;
    ...
    handleDownloadTemplate(event){
        ...
        const fileUrl = '/resource/Bulk_UploadUser';
    ...
    handleInstructions(event){
        ...
        if(this.typeOfUser=='NDIS'){
            const fileUrl = '/resource/Bulk_Upload_Instructions_For_NDISUser';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
            
        } else if(this.typeOfUser=='ICT'){
            const fileUrl = '/resource/Bulk_Upload_Instructions_For_ICTUser';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        }   
    }
    ...
    handleSwitchToSingleUserForm() {
        console.log('🔄 Switching to Single User Form');
        
        // Reset bulk upload flags
        this.isMultiUserUpload = false;
        this.isAccessManager = true;
        this.isProcessed = false;
        this.issingleUserUpload = false; // Make sure single user form is also reset
        ...
        const fileInput = this.template.querySelector('#uploadPdf');
        if (fileInput) {
        ...
        console.log('✅ Switched to single user form mode');
        console.log('isMultiUserUpload:', this.isMultiUserUpload);
        console.log('isAccessManager:', this.isAccessManager);
    }
    ...
    parseCSV(data) {
        ...
        /*Required headers - ❌ REMOVED Role column */
        let requiredHeaders = [
            'First Name',
            'Last Name',
            'Gender',
            ...
            'Email Address',
            'Date of Birth',
            'User Type',  // ✅ Role will be auto-mapped from User Type
            'Facility',
            'Australian Business Number',
            'Tax File Number',
            'Tax Free Threshold',
        ...
        /* ❌ REMOVED Role validations - Role will be auto-mapped in handleFinalSubmit */
        // Only keep User Type specific validations if needed
    ...
    async handleFinalSubmit() {
        ...
        const mapUserTypeToRole = (userType) => {
            const roleMap = {
                ...
                'NDIS Participant': 'Roster Admin',
                'NDIS Participants': 'Roster Admin',
        ...
        if (unprocessedRecords.length === 0) {
            this.showToast('Warning', 'No unprocessed records selected.', 'warning');
            return;
        }

        // ⭐ STEP 0 — BASE REQUIRED FIELDS (ROLE REMOVED)
        let requiredFields = [
        ...
        // ⭐ STEP 2 — VALIDATE EACH SELECTED ROW & AUTO-MAP ROLE
        // ✅ Track all validation errors for each row
        const errorsByRow = {};
            ...
            if (user.isSelected && user.ProcessedStatus !== 'Success ✅') {
                ...
                const missingFields = requiredFields
                    .filter(field => {
                        ...
                        return !user[normalizedKey] || user[normalizedKey].trim() === '';
                    })
                    .map(field => field.label);

                if (missingFields.length > 0) {
                    errorMessages.push(`Missing → ${missingFields.join(', ')}`);
                }

                // ✅ AUTO-MAP ROLE FROM USER TYPE
                const mappedRole = mapUserTypeToRole(user.usertype);
                
                // ✅ Validate that the user type maps to a valid role
                if (!mappedRole) {
                    errorMessages.push(`Invalid User Type: ${user.usertype}`);
                } else {
                    // ✅ Update the role with mapped value
                ...
                if (abn === "" && tfn === "") {
                    errorMessages.push(`Enter Australian Business Number OR Tax File Number for ${name}`);
                }

                if (abn !== "" && tfn !== "") {
                    errorMessages.push(`Only one allowed → Australian Business Number OR Tax File Number for ${name}`);
                }

                if (abn !== "" && abn.length !== 11) {
                    errorMessages.push(`Australian Business Number must be exactly 11 digits for ${name} (got ${abn.length})`);
                }

                if (tfn !== "" && tfn.length !== 9) {
                    errorMessages.push(`Tax File Number must be exactly 9 digits for ${name} (got ${tfn.length})`);
                }

                // ✅ Phone Number Validation
                const phone = (user.phonenumber || '').trim();
                if (phone && !/^\d+$/.test(phone)) {
                    errorMessages.push(`Contact Number must contain only digits (got: ${phone})`);
                }
                if (phone && phone.length !== 10) {
                    errorMessages.push(`Contact Number must be exactly 10 digits (got ${phone.length})`);
                }
                ...
                if (this.typeOfUser === 'NDIS') {
                    if (!['NDIS Org Admin', 'ICT Admin', 'Facility Admin', 'SC with NDIS Org Admin', 
                        'Roster Manager', 'HR Admin', 'Payroll Admin', 'NDIS Participant', 
                        'NDIS Participants', 'Payroll Accountant for Multiple', 
                        'Accountant for Organisation', 'SC with Roster Manager', 
                        'NDIS Staff'].includes(user.usertype)) {
                        errorMessages.push(`Invalid User Type for NDIS: "${user.usertype}"`);
                    }
                }

                // ✅ ICT Specific Validations
                if (this.typeOfUser === 'ICT') {
                    if (!['ICT Admin', 'Facility Admin', 'ICT Staff'].includes(user.usertype)) {
                        errorMessages.push(`Invalid User Type for ICT: "${user.usertype}"`);
                    }
                }

                // ✅ If there are errors, mark the row
                if (errorMessages.length > 0) {
                    hasInvalidRecords = true;
                    return {
                        ...user,
                        ProcessedStatus: `Error ❌: ${errorMessages.join(' | ')}`
                    };
                }
            }
            return user;
        ...
        if (hasInvalidRecords) {
            ...
            // ✅ Show a single toast with summary
            const errorCount = this.tableData.filter(r => r.ProcessedStatus && r.ProcessedStatus.includes('Error')).length;
            this.showToast(
                'Validation Errors',
                `${errorCount} row(s) have validation errors. Please check the Processed Status column for details.`,
                'error'
            );
        ...
        // ⭐ STEP 3 — CHECK FACILITY ACTIVE STATUS & VALIDITY
        try {
            const facilityNames = [...new Set(unprocessedRecords.map(r => r.facility))];
            console.log('🏢 Facilities to validate:', facilityNames);
            
            for (const facilityName of facilityNames) {
                if (!facilityName || facilityName.trim() === '') {
                    this.tableData = this.tableData.map(user => {
                        if (user.isSelected && (!user.facility || user.facility.trim() === '')) {
                            return {
                                ...user,
                                ProcessedStatus: `Error ❌: Facility name is empty or invalid`
                            };
                        }
                        return user;
                    });
                    ...
                    this.showToast(
                        'Error',
                        'One or more records have an empty or invalid facility name.',
                        'error'
                    );
                    return;
                }
                ...
                try {
                    ...
                    if (isActive === null || isActive === undefined) {
                        this.tableData = this.tableData.map(user => {
                            if (user.isSelected && user.facility === facilityName) {
                                return {
                                    ...user,
                                    ProcessedStatus: `Error ❌: Facility "${facilityName}" does not exist`
                                };
                            }
                            return user;
                        });
                        ...
                        this.showToast(
                            'Error',
                            `The facility "${facilityName}" does not exist. Please check the facility name.`,
                            'error'
                        );
                        return;
                    }
                    ...
                    if (!isActive) {
                        this.tableData = this.tableData.map(user => {
                            if (user.isSelected && user.facility === facilityName) {
                                return {
                                    ...user,
                                    ProcessedStatus: `Error ❌: Facility "${facilityName}" is inactive`
                                };
                            }
                            return user;
                        });
                    ...
                    console.log(`✅ Facility "${facilityName}" is valid and active`);

                } catch (error) {
                    console.error(`Error checking facility "${facilityName}":`, error);
                    
                    this.tableData = this.tableData.map(user => {
                        if (user.isSelected && user.facility === facilityName) {
                            return {
                                ...user,
                                ProcessedStatus: `Error ❌: Unable to validate facility "${facilityName}"`
                            };
                        }
                        return user;
                    });
                    ...
                    this.showToast(
                        'Error',
                        `Unable to validate facility "${facilityName}". Please try again.`,
                        'error'
                    );
                    return;
                }
            ...
            console.error('Error in facility validation:', error);
            this.showToast('Error', 'Unable to verify facility status.', 'error');
            return;
        }

        // ⭐ STEP 4 — PREP CLEAN Records & SUBMIT
        try {
    ...
    @track _isUserManagement = true;
    @track _isstaffManagement = false;
    @track _isuserReport = false;
    @track _issingleUserUpload = false;
    @track _isMultiUserUpload = false;

    get isUserManagement() {
        return this._isUserManagement;
    }
    set isUserManagement(value) {
        this._isUserManagement = value;
        this._syncRecordRoute();
    }

    get isstaffManagement() {
        return this._isstaffManagement;
    }
    set isstaffManagement(value) {
        this._isstaffManagement = value;
        this._syncRecordRoute();
    }

    get isuserReport() {
        return this._isuserReport;
    }
    set isuserReport(value) {
        this._isuserReport = value;
        this._syncRecordRoute();
    }

    get issingleUserUpload() {
        return this._issingleUserUpload;
    }
    set issingleUserUpload(value) {
        this._issingleUserUpload = value;
        this._syncRecordRoute();
    }

    get isMultiUserUpload() {
        return this._isMultiUserUpload;
    }
    set isMultiUserUpload(value) {
        this._isMultiUserUpload = value;
        this._syncRecordRoute();
    }
    ...
    handleCancel() {
        console.log('🔄 Cancel clicked in bulk upload');
        ...
        // Switch back to single user form
        this.isMultiUserUpload = false;
        this.isAccessManager = true;
        this.issingleUserUpload = false;
        ...
        console.log('✅ Bulk upload cancelled, returned to main view');
    }
    ...
    // --- Hash routing bridge implementation ---
    _hasRestored = false;
    _suppressSync = false;
    _syncTimeout;
    ...
    handleSubPopup(event) {
        this._syncRecordRoute();
    }

    _getActiveSelector() {
        if (this._isUserManagement) return 'c-tesseract-apps-user-access-lwc';
        if (this._isstaffManagement) return 'c-tesseract-apps-staff-management-lwc';
        if (this._isuserReport) return 'c-tesseract-apps-staff-reports-lwc';
        return null;
    }
    ...
    _syncRecordRouteActual() {
        ...
        // if (this._isUserManagement) {
        if (this._issingleUserUpload) {
            slug = 'user-management/single-user-upload';
        } else if (this._isMultiUserUpload) {
            slug = 'user-management/multi-user-upload';
        } else if (this._isUserManagement) {        
            slug = 'user-management';
            selector = 'c-tesseract-apps-user-access-lwc';
        } else if (this._isstaffManagement) {
            slug = 'staff-management';
            selector = 'c-tesseract-apps-staff-management-lwc';
        } else if (this._isuserReport) {
            slug = 'user-report';
            selector = 'c-tesseract-apps-staff-reports-lwc';
        }
        ...
        if (selector) {
            ...
            if (child) {
                uid = child.recordUid || '';
                isEdit = child.isEdit || false;
                step = child.currentStep || '';
                popup = child.activePopup || '';
            }
        }
    ...
    _restoreFromUrlHash() {
        ...
        const modIdx = parts.findIndex(p => p === 'access-manager');
        if (modIdx < 0) return false;

        const subParts = parts.slice(modIdx + 1);
        ...
        if (subParts.includes('single-user-upload')) {
            isSingleUpload = true;
            tabSlug = 'user-management';
        } else if (subParts.includes('multi-user-upload')) {
            isMultiUpload = true;
            tabSlug = 'user-management';
        }

        if (!isSingleUpload && !isMultiUpload) {
        const first = subParts[0];
        const isUid = /^(fac|x|org|stf|par)-/i.test(first) || (first.length === 18 && !first.includes('-'));
        ...
        if (isUid) {
            ...
            const modes = subParts.slice(2);
            isEdit = modes.includes('edit');
            step = modes.find(m => /^step[1-5]$/.test(m)) || '';
            popup = modes.find(m => m !== 'edit' && !/^step[1-5]$/.test(m)) || '';
        } else {
            tabSlug = first;
            const modes = subParts.slice(1);
            isEdit = modes.includes('edit');
            step = modes.find(m => /^step[1-5]$/.test(m)) || '';
            popup = modes.find(m => m !== 'edit' && !/^step[1-5]$/.test(m)) || '';
        }
        ...
        if (isSingleUpload) {
            this._issingleUserUpload = true;
            this.isAccessManager = false;
        } else if (isMultiUpload) {
            this._isMultiUserUpload = true;
            this.isAccessManager = false;
        } else {
        if (tabSlug === 'user-management') {
            this._isUserManagement = true;
            this.moduleName = 'isUserManagement';
        } else if (tabSlug === 'staff-management') {
            this._isstaffManagement = true;
            this.moduleName = 'isstaffManagement';
        } else if (tabSlug === 'user-report') {
            this._isuserReport = true;
            this.moduleName = 'isuserReport';
        }
        }
        ...
        if (uid) {
            setTimeout(() => {
                const child = this.template.querySelector(this._getActiveSelector());
                if (child) {
                    child.openByUID(uid, isEdit);
                }
            }, 100);
        }
        return true;
    ...
    @api async openByUID(uid, tab, isEdit) {
        ...
        const tabSlug = tab || 'user-management';
        // if (tabSlug === 'user-management') {
        if (tabSlug === 'single-user-upload') {
            this._issingleUserUpload = true;
            this.isAccessManager = false;
        } else if (tabSlug === 'multi-user-upload') {
            this._isMultiUserUpload = true;
            this.isAccessManager = false;
        } else if (tabSlug === 'user-management') {        
            this._isUserManagement = true;
            this.moduleName = 'isUserManagement';
        } else if (tabSlug === 'staff-management') {
            this._isstaffManagement = true;
    ...
    handleBulkCreateUser(event) {
        ...
        isStaffLimitReached()
            .then(result => {
                ...
                console.log('✅ Bulk upload mode activated');
                console.log('isMultiUserUpload:', this.isMultiUserUpload);
                console.log('isAccessManager:', this.isAccessManager);
            })
            .catch(error => {
                console.error('Error checking staff limit:', error);
                this.showToast('Error', 'Unable to check staff limit.', 'error');
            });
    }
