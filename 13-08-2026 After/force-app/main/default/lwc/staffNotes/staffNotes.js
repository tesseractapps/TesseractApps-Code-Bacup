...
export default class StaffNotes extends LightningElement {
   ...
   handleViewAttachment(event) {
    ...
    // =====================================================================
    // HASH ROUTER bridge (additive) — APIs & reactive modal getter/setter.
    // =====================================================================
    get showNotesModal() {
        return this._showNotesModal;
    }
    set showNotesModal(val) {
        this._showNotesModal = val;
        this.dispatchEvent(new CustomEvent('subpopup', {
            detail: { slug: val ? 'notes-modal' : '' },
            bubbles: true,
            composed: true
        }));
    }
    ...
    @api openPopup(slug) {
        if (slug === 'notes-modal' || slug === 'create-note') {
            this.showNotesModal = true;
        }
    }
