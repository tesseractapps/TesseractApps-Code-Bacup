...
export default class FormRender extends LightningElement {
  ...
  @track orgLogoUrl = "";
  //manendra added for sortin table 
  @track sortField = '';
...
console.log('FIRST RECORD:', source?.[0]);
console.log('KEYS:', Object.keys(source?.[0] || {}));
const sortedData = this.sortData(source);
console.log('First before sort:', source.map(x => x[this.sortField]));
console.log('First after sort:', sortedData.map(x => x[this.sortField]));
// totals
...
const start = (this.pageNumber - 1) * this.pageSize;
const end = start + this.pageSize;
this.recordsToDisplay = sortedData.slice(start, end);
...
handleSort(event) {
    ...
    // Sort the array that drives this table
    this.filteredFormResponses = this.sortData(this.filteredFormResponses);

    // Reset this table's pagination
    this.formPageNumber = 1;

    // Refresh displayed rows
    this.updatePaginatedFormResponses();
}
    sortData(data) {
    console.log('sortField:', this.sortField);
    console.log('First Record:', data?.[0]);
    console.log('Value being sorted:', data?.[0]?.[this.sortField]);
        if (!this.sortField) {
            return [...data];
        }
