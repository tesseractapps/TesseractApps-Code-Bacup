...
import listOfClientJournalByParticipantsAndShift from "@salesforce/apex/ParticipantDetailsHandler.listOfClientJournalByParticipantsAndShift";
    ...
    get generateMinutesOptions() {
      ...
      emptyFields(){
        ...
        this.selectedShift='';
         ...
         this.isBreakinVisible = false;
         this.isBreakoutVisible = false;
          this.disableChecklistButtons = true;
      }
...
@track participantId;
@track disableChecklistButtons=true;
    ...
    handleSelected(event) {  
      ...
      this.signinList = this.signinList.map(item => ({
          ...item,
            ...
            if (selectedRow.Services_and_Support_Plans__r && selectedRow.Services_and_Support_Plans__r.length > 0) {
                  //  const firstPlan = selectedRow.Services_and_Support_Plans__r[0];
                   // this.participant = firstPlan.Client__r.Display_Nickname__c;  // Access Name__c inside Client__r
                    // this.participantId = firstPlan.Client__r.Id;
                    this.participantOptions =
                      selectedRow.Services_and_Support_Plans__r.map(plan => ({
                          label: plan.Client__r.Display_Nickname__c,
                          value: plan.Client__r.Id
                      }));
                      this.participant = this.participantOptions.map(item => item.label).join(', ');
                      this.participantId = this.participantOptions[0].value;
                       this.selectedParticipantIds = this.participantOptions.map(item => item.value);
                } else {
                    this.participant = '';
                    this.participantId = '';
                    this.participantOptions = [];
              ...
              const starttimeArry = selectedRow.Start_Time_24_format__c ?selectedRow.Start_Time_24_format__c.split(":") : selectedRow.Add_Shift__r.Start_time_Formula__c.split(":");
              const endTimeArry =selectedRow.End_Time_24_format__c  ?selectedRow.End_Time_24_format__c.split(":")  :  selectedRow.Add_Shift__r.End_time_formula__c.split(":");
              ...
              // Enable Checklist/Activity Log/Journal for 14 days from shift date
              const expiryDate = new Date(shiftDate);
              ...
              this.disableChecklistButtons = !(todayDate >= shiftDate && todayDate <= expiryDate);
 ...
 // Toggle between mute/unmute
  ...
  handleSleepoverLogHours(){
    ...
    @track clientjournal=false;
    @track jNoteType;
    @track description;
    @track journalUploadedFiles = []; 
    @track journalUploadResponse;
    @track showViewModal = false;
    @track journalheading;
    selectedAttachments = [];

    handleJournal(event){
     this.selectedAttachments = [];
     this.buttonlabel = "Save";
     this.journalheading = "Create Journal";
     this.reason=null;
     this.jounelId=null;
     this.correctionflag=false;
     this.jNoteType='';
     this.description='';
     this.clientjournal=true;
     console.log('this.clientjournal',this.clientjournal);
     this.isShowActivity=false;
     this.isShowChecklsit=false;
     this.participantdisable=false;
 ...
 formatOptions = [
  ...
  handleSave1(event) {
    if (!this.jNoteType) {
      ...
      */
       const event = new ShowToastEvent({
                title: 'Error',
                message: 'Please Select Notes Type',
                variant: 'error'
            });
    ...
    if (!this.participantId) {
     /*  const participant = this.template.querySelector('[data-id="Participant"]');
      participant.reportValidity();
      */
      const event = new ShowToastEvent({
                title: 'Error',
...
this.journalUploadedFiles = [];
  ...
  handleparticipant(event){
    this.participantId=event.detail.value;
  ...
  handleclientjournal(){
    listOfClientJournalByParticipantsAndShift({  participantIds: this.selectedParticipantIds, shiftId: this.selectedShift }).then((response) => {
      ...
      response.forEach((record) => {
        console.log('Journal Record',JSON.stringify(record));
        let tempConRec = Object.assign({}, record);
        
        if (tempConRec.ShiftwithStaff__r) {
          const shiftDate = new Date(tempConRec.ShiftwithStaff__r.Shift_Start_DateTime__c);
          const date = shiftDate.toLocaleDateString('en-AU', { day: '2-digit', month: '2-digit', year: 'numeric'});
          tempConRec.shiftDetails = `${date} (${tempConRec.ShiftwithStaff__r.Shift_Start_End_Time__c})`;
        } else {
      ...
      tempConRec.descriptionPreview =
        ...
        console.log('AttachmentsCHECK', JSON.stringify(tempConRec.attachments) );
      ...
      getCorrectionAttachments({ correctionIds: correctionIds})
      .then(result => {

    correctionAttachmentMap = result || {};
    this.clientJournelList.forEach(parentJournal => {

    if ( parentJournal.Client_Journels__r && parentJournal.Client_Journels__r.length > 0) {

        parentJournal.corrections = parentJournal.Client_Journels__r.map((corr, index) => {
            ...
            console.log('Correction UI Attachments',corr.Id, JSON.stringify(correctionAttachmentMap[corr.Id]));
                return {
    ...
    this.displayedClientJournels = [...this.clientJournelList];
    this.noRecordsFlag1 = this.displayedClientJournels.length === 0;
  ...
  @track journalModalType = 'view';
  @track correctionflag=false;
  @track jounelId;
  @track buttonlabel = "Save";
  @track participantdisable = false;
  currentUrl = '';
 ...
 handleEdit(event) {
    event.stopPropagation();
    console.log( 'journalUploadedFiles On Edit',JSON.stringify(this.journalUploadedFiles));
    this.jounelId = event.currentTarget.dataset.id;
    this.participantId = event.currentTarget.dataset.participantid;
    this.Journal='Journal Correction';
    this.journalheading = "Journal Correction";
    this.participantdisable=true;
    ...
    this.editAttachments = selectedJournal?.attachments? JSON.parse(JSON.stringify(selectedJournal.attachments)): [];
  
    console.log('Edit Attachments',JSON.stringify(this.editAttachments));
    this.selectedAttachments = JSON.parse(JSON.stringify(selectedJournal.attachments || []) );

    console.log('Selected Attachments', JSON.stringify(this.selectedAttachments));

    console.log('existingFilesJson',JSON.stringify(this.selectedAttachments));
    //  END
