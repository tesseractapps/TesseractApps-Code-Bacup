...
export default class RosterSettings extends LightningElement {
  ...
  shiftTypeOptionsforCustom = [
   ...
   /* colorOptions = [
     { label: "Deep Purple", value: "#4B2882" },
    { label: "Royal Blue", value: "#2F5794" },
    { label: "Dark Green", value: "#177246" },
    { label: "Orange", value: "#C86112" },
    { label: "Red", value: "#C0301D" },
 ...
 */
  ...
  connectedCallback() {
    ...
    */
    ...
    if (this.facilityoptions && this.facilityoptions.length > 0) {
      this.facilityValue = localStorage.getItem("defaultFacilityId");
      console.log("this.facilityValue >>", this.facilityValue);
      if (this.facilityValue) {
        console.log("this.facilityValue >>", this.facilityValue);
        this.loadFacilityData();
  ...
  //       this.WeekStartDay = result.Week_Start_Day__c;
  ...
  updateShiftsWithSalesforceData(salesforceShifts) {
    ...
    salesforceShifts.forEach((sfShift, index) => {
      ...
      let localShift = {
        ...
        gridStyle: sfShift.Id ? 'gap: 10px;' : 'gap: 0px;',
        name: sfShift.Name || "",
      ...
      localShift.address = sfShift.Type_of_Address__c;
        ...
        const mappedShifts = (sfShift.Custom_Shift_Timings__r || []).map(
          (t, tIndex) => {
            ...
            const mappedShift = {
              ...
              actualDuration: t.Actual_Duration__c
            };
...
handleMaxDurationInputForCustom(event) {
  ...
  console.log("🎯 [START] handleMaxDurationInputForCustom triggered");
  ...
  if (rawValue && rawValue.includes(".")) {
    const parts = rawValue.split(".");
    console.log("🔍 Decimal parts:", parts);

    if (parts[1].length > 2) {
      console.log(`⚙️ Trimming decimals → ${parts[1]} → ${parts[1].substring(0, 2)}`);
      rawValue = `${parts[0]}.${parts[1].substring(0, 2)}`;
      event.target.value = rawValue;
      console.log("🔧 Updated Input After Trim:", rawValue);
    }
  }
  ...
  if (rawValue !== "" && isNaN(numericValue)) {
    console.warn("❌ Not a valid number → abort update");
    return;
  }

  // ---------------------------------------------------------
  // 🔹 Step 3: Blank input → clear fields
  // ---------------------------------------------------------
  if (rawValue === "") {
    console.log("🧹 Blank input detected → clearing value");
      ...
      if (String(template.id) === String(templateId)) {
        const updatedShifts = template.shifts.map((shift) => {
          if (String(shift.id) === String(shiftId)) {
            console.log(`🧽 Clearing field "${field}" for shift ${shiftId}`);
  ...
  // ---------------------------------------------------------
  // 🔹 Step 4: Update shift value
  // ---------------------------------------------------------
  console.log("✍️ Updating template → shift → field...");
    ...
    if (String(template.id) === String(templateId)) {
      const updatedShifts = template.shifts.map((shift) => {
        if (String(shift.id) === String(shiftId)) {
          ...
          console.log("🎯 TARGET SHIFT FOUND:", JSON.parse(JSON.stringify(shift)));
          ...
          console.log("🟦 Updated Shift BEFORE validation:", JSON.parse(JSON.stringify(updatedShift)));

          // ---------------------------------------------------------
          // ⏳ Step 5: Skip validation while user is typing
          // ---------------------------------------------------------
          if (
            rawValue.endsWith('.') ||
            rawValue.endsWith('.0') ||
            rawValue.endsWith('.00') ||
            rawValue === '.'
          ) {
            console.log("⏳ User typing… skipping validation");
            return updatedShift;
          }

          // ---------------------------------------------------------
          // 🔹 Step 6: Calculate actual duration
          // ---------------------------------------------------------
          console.log("🧮 Calculating actual duration…");

          const actual = this.calculateDurationFromStartEnd(updatedShift);
          const entered = Number(updatedShift.duration || 0);

          console.log(`🔎 ENTERED duration = ${entered}`);
          ...
          // ---------------------------------------------------------
          if (entered > 0 && actual > 0 && entered < actual) {
            console.warn("❌ Duration mismatch: Entered < Actual");
            ...
            Promise.resolve().then(() => {
              this.showToast(
                "Duration Mismatch",
                `Entered Duration (${entered} hrs) is LESS than Actual Shift Duration (${actual.toFixed(2)} hrs).`,
                "error"
              );
            });
          } else {
            console.log("✅ Duration valid — no mismatch");
          }

          console.log("🟩 Final Shift After Update:", JSON.stringify(updatedShift));
          return updatedShift;
        }
  ...
  console.log("📦 FINAL Updated Templates:", JSON.parse(JSON.stringify(this.templates)));
}



calculateDurationFromStartEnd(shift) {
    console.log("🟦 [DURATION] START calculating actual duration...");

    const toMinutes = (hour, minute, ampm) => {
        console.log(`   🔹 Converting to minutes → hour=${hour}, minute=${minute}, ampm=${ampm}`);

        if (!hour || !minute || !ampm) {
            console.warn("   ⚠ Missing one or more time fields → returning 0");
            return 0;
        }
        ...
        console.log(`   ✅ Converted to totalMinutes = ${total}`);
        return total;
    };

    const startMin = toMinutes(shift.startHour, shift.startMinute, shift.startAMPM);
    const endMin   = toMinutes(shift.endHour, shift.endMinute, shift.endAMPM);

    console.log(`🟦 startMin=${startMin}, endMin=${endMin}`);

    if (startMin === 0 && endMin === 0) {
        console.warn("⚠ Both start and end = 0 → returning duration=0");
        return 0;
    }
    ...
    const hours = diff / 60;
    console.log(`🟩 FINAL actual duration (hrs) = ${hours}`);
  ...
  handleShiftTypeChange(event) {
    ...
    console.log('🏠 handleAdressChange called');
    ...
    let shift;
    if (salesforceId) {
      console.log('Searching for shift with salesforceId:', salesforceId);
      shift = this.shifts.find((s) => s.salesforceId === salesforceId);
    } else if (localId) {
      console.log('Searching for shift with localId:', localId);
      shift = this.shifts.find((s) => s.id === localId);
    } else {
      console.warn('⚠️ No valid ID found in dataset (neither salesforceId nor localId).');
    }

    if (shift) {
      console.log('Before update:', { ...shift });
      shift.address = value;
      console.log('✅ Updated shift:', { ...shift });
    } else {
      console.warn('⚠️ Shift not found for given IDs:', { localId, salesforceId });
    }
  }
  ...
  handleAdressChangeforDesign(event) {
    const localId = event.target.dataset.id; // For create
    const salesforceId = event.target.dataset.salesforceid; // For update
    const value = event.detail.value; // Combobox uses event.detail.value instead of event.target.value
    ...
    if (shift) {
      shift.address = value;
      this.clearShiftMessage(shift); // Assuming you want to clear errors or messages like before
    }
  }
...
handleStartTimeChangeforDesign(event) {
 ...
 updateShiftField(id, field, value) {
  console.log('🔧 updateShiftField called with:', { id, field, value });
  console.log('Before update:', JSON.stringify(this.shiftsforDesign));

  this.shiftsforDesign = this.shiftsforDesign.map((shift) => {
    if (String(shift.id) === id) {
      console.log(`Updating shift with id=${id}: setting ${field} to`, value);
      return { ...shift, [field]: value };
    }
    return shift;
  });

  console.log('After update:', JSON.stringify(this.shiftsforDesign));
}
...
@track WeekStartDay = 'Sunday';

@track fortnightlyWeekStart = 'Monday';
...
handleFortnightlyWeekStartChange(event) {
    this.fortnightlyWeekStart = event.detail.value;
    console.log('Fortnightly Week Start:', this.fortnightlyWeekStart);
}
