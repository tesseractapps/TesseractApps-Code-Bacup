...
//import getServiceFundTrackerEntities from '@salesforce/apex/RosterInvoicesHandler.getServiceFundTrackerEntities';
import updateServiceEntity from '@salesforce/apex/RosterInvoicesHandler.updateServiceEntity';
import updateInvoiceStatus from '@salesforce/apex/RosterInvoicesHandler.updateInvoiceStatus';
...
export default class TesseractAppsRosterInvoices extends LightningElement {
   ...
   @track isInvoiceflag=true;
    ...
    @track isPageSizeIssuedManuallySet = false;
    ...
    @track isPageSizeCancelledManuallySet = false;
    ...
    @track createServiceEntityFlag=false;
    ...
    @track isPageSize3ManuallySet = true;
    resizeObserver;
    @track records3 = [];  
    ...
    @track entityServicesMap= new Map();
    @track groupedEntities= [];
    @track refreshTimestamp;
     @track refreshTimestamp1;
    ...
    connectedCallback() {
        ...
        console.log('Invoice connectedCallback called');
         this.refreshTimestamp = Date.now();
          this.refreshTimestamp1 = Date.now();
  ...
  //manendra end
    ...
    wiredServiceList(result) {
        ...
        if (result.data) {
            ...
            if (this.accountingService === 'Tesseract System') {
                ...
                entityPromise = this.loadEntityProfiles();
                ...
                entityPromise = this.loadXeroEntities();
            ...
            .then(() => {
                ...
                console.log(
                    '✅ ENTITY OPTIONS READY:',
                    JSON.stringify(this.entityOptions)
                );

                console.log(
                    '✅ ENTITY OPTIONS COUNT:',
                    this.entityOptions?.length
                );
            ...
            getParticipantFacilityMap({ participantIds: participantIdsValues })
                .then(participantFacilityMap => {
                    let participantFilteredData = serviceData.filter(rec => {
                    ...
                    let filterData = participantFilteredData.filter(rec => {
                        // const shiftFacility =
                        //     rec.ShiftwithStaff__r?.Shift_Facility__c ||
                        //     rec.ShiftwithStaff__r?.Add_Shift__r?.Facility__c;

                        // return Array.isArray(this.faclist)
                        //     ? this.faclist.includes(shiftFacility)
                        //     : this.faclist === shiftFacility;
                        if (rec.ShiftwithStaff__r) {
                    ...
                    // const fundTrackerIds = [
                    //     ...new Set(
                    //         filterData
                    //             .map(rec => rec.Funds_Tracker__c)
                    //             .filter(id => id)
                    //     )
                    // ];
                    // const supportCatalogueIds = [
                    //     ...new Set(
                    //         filterData
                    //             .map(rec => rec.Service_Type__c)
                    //             .filter(id => id)
                    //     )
                    // ];
                    // console.log('fundTrackerIds => ', JSON.stringify(fundTrackerIds));
                    // console.log('supportCatalogueIds => ', JSON.stringify(supportCatalogueIds));

                    // getServiceFundTrackerEntities({
                    //     fundTrackerIds: fundTrackerIds,
                    //     supportCatalogueIds: supportCatalogueIds,
                    //     facilityId: this.faclist
                    // })
                    // .then(junctionRecords => {
                    //     console.log('junctionRecords => ', JSON.stringify(junctionRecords));
                    //     const junctionMap = {};

                    //     junctionRecords.forEach(rec => {
                    //         const key = rec.Funds_Tracker__c + '_' + rec.NDIS_Support_Catalogue__c;
                    //         junctionMap[key] = rec;
                    //         console.log('Junction Key => ', key, rec.Entity_Profile__c, rec.Xero_Entity__c);
                    //     });

                    //     this.servicesfromparent = filterData.map(service => {
                    //         const key = service.Funds_Tracker__c + '_' + service.Service_Type__c;
                    //         const junctionRec = junctionMap[key];
                    //         console.log('Service Key => ', key, junctionRec);

                    //         let entityId = '';
                    //         let serviceEntityName  = 'N/A';
                    //         if (this.accountingService === 'Tesseract System') {
                    //             entityId = junctionRec?.Entity_Profile__c || '';
                    //             serviceEntityName = junctionRec?.Entity_Profile__r?.Name__c
                    //                 ? junctionRec.Entity_Profile__r.Name__c
                    //                 : `${junctionRec?.Entity_Profile__r?.First_Name__c || ''} ${junctionRec?.Entity_Profile__r?.Last_Name__c || ''}`.trim() ||
                    //                    'N/A';
                    //         } else if ( this.accountingService === 'Xero' || this.accountingService === 'MYOB' )  {
                    //             this.accountingServiceflag = true;
                    //             entityId = junctionRec?.Xero_Entity__c ||'';
                    //             //serviceEntityName  = junctionRec?.Xero_Entity__r?.Name || '';
                    //             serviceEntityName =  `${junctionRec?.Xero_Entity__r?.First_Name__c || ''} ${junctionRec?.Xero_Entity__r?.Last_Name__c || ''}`.trim() || 'N/A';
                    //         }
                    //         console.log(
                    //             'Mapped Entity => ',
                    //             {
                    //                 serviceId: service.Id,
                    //                 fundTracker: service.Funds_Tracker__c,
                    //                 serviceType: service.Service_Type__c,
                    //                 junctionId: junctionRec?.Id,
                    //                 entityId: entityId,
                    ...
                    //             serviceEntityName: serviceEntityName,
                    //             xeroContactId: junctionRec?.Xero_Entity__r?.Xero_Contact_Id__c || null
                    //         };
                    //     });
                    //     console.log('Final servicesfromparent => ', JSON.stringify(this.servicesfromparent));
                    //     this.shiftWithStaffIds = this.servicesfromparent
                    //         .filter(rec => rec.ShiftwithStaff__c)
                    //         .map(rec => rec.ShiftwithStaff__c);
                    //     this.fetchShiftStaffDetails();
                    // })
                    // .catch(error => {
                    //     console.error( 'getServiceFundTrackerEntities error => ', error );
                    ...
                    this.servicesfromparent = filterData.map(service => {
                        ...
                        if (this.accountingService === 'Tesseract System') {
                            ...
                            entityId =service.Entity_Profile__c || '';
                            serviceEntityName =
                                service.Entity_Profile__r?.Name__c
                                ? service.Entity_Profile__r.Name__c
                                : `${service.Entity_Profile__r?.First_Name__c || ''} ${service.Entity_Profile__r?.Last_Name__c || ''}`.trim() || 'N/A';
                            ...
                            this.accountingServiceflag = true;
                            entityId =service.Xero_Entity__c || '';
                            serviceEntityName = `${service.Xero_Entity__r?.First_Name__c || ''} ${service.Xero_Entity__r?.Last_Name__c || ''}`.trim() || 'N/A';
                            xeroContactId = service.Xero_Entity__r?.Xero_Contact_Id__c || null;
                        }
                        let entityExists = true;

                        if (this.accountingService === 'Tesseract System') {
                            entityExists = this.entityOptions?.some(
                                option => String(option.value) === String(entityId)
                            ) || false;
                        }
                        ...
                        return {
                            ...
                            entityProfileId: entityExists ? entityId : '',
                            xeroContactId: entityExists ? xeroContactId : null,
                            serviceEntityName: entityExists ? serviceEntityName : '',
            ...
            .catch(error => {
                ...
                console.error(
                    '❌ Error loading entity options:',
                    error
                );

                this.entityOptions = [];

                // If entity loading fails, you can still continue
                // with your existing service processing.
            });
        }
    }
    ...
    @wire(getBulkServicesHandler, { PartcipantIdList: '$invoicePartcipantIdList', sDate: '$startDate', eDate: '$endDate', orgid: '$orgid', name:'$searchKey', planType:'$invoicePlanType',  refreshKey: "$refreshTimestamp1"  })
    async  wiredBulkServices(result) {
        ...
        console.log('this.myobResponseforInvoiceStatus >>>>> iN wIRE',JSON.stringify(this.myobResponseforInvoiceStatus, null, 2));
        if (this.xeroResponseforInvoiceStatus) {
            ...
            try {
                ...
                console.log( '✅ Xero Apex Response in getBulkServicesHandler:',  JSON.stringify(data, null, 2));
            } catch (e) {
        ...
        if (this.myobResponseforInvoiceStatus) {
            try {
                ...
                console.log( '✅ MYOB Apex Response:',JSON.stringify(data, null, 2));
            } catch (e) {
                ...
                getParticipantFacilityMap({participantIds: participantIdsValues}).then(participantFacilityMap=>{
                    ...
                    let mappedRecords = filterData.map(invoice => {
                        ...
                        console.log(
                            ...
                            'Is_Service_Invoice__c=', invoice.Is_Service_Invoice__c,
                            'Is_Sill_Invoice__c=', invoice.Is_Sill_Invoice__c
                        );
                        let fileReference = '';
                        let invoiceNo = '';
                        let invoiceType = '';
                        ...
                        else if (invoice.Invoice_No__c) {
                            // Xero invoices — neither flag set
                           // invoiceNo = invoice.Name;
                            if(this.accountingService==='Xero'){
                                invoiceNo = invoice.Invoice_No__c; 
                            } else{
                        ...
                        if (invoice.Participant__r) {
                            ...
                            fileReference = invoice.Participant__r && invoice.Participant__r.Name__c
                        ...
                        if (this.accountingService === 'Tesseract System') {
                            if (invoice.Accounting_Journal_Entry__r && invoice.Accounting_Journal_Entry__r.length > 0) {
                                ...
                                // entityName = journalEntry.Entity_Profile_Name__c ? journalEntry.Entity_Profile_Name__c : 'N/A';
                                    entityName =journalEntry && journalEntry.Entity_Profile_Name__c
                                        ? journalEntry.Entity_Profile_Name__c
                                        : 'N/A';
                                // entityLedgerItem=journalEntry.Accounting_Ledger_Items__r.Name ? journalEntry.Accounting_Ledger_Items__r.Name : 'N/A';
                                entityLedgerItem = journalEntry.Accounting_Ledger_Items__r
                                        ? journalEntry.Accounting_Ledger_Items__r.Name
                                        : 'N/A';
                            ...
                            entityName = entity
                                ...
                                : 'N/A';
                            xeroContactId = entity?.Xero_Contact_Id__c || '';
                            ...
                            if (xeroInvoiceId) {
                                ...
                                if (this.accountingService === 'MYOB') {
                                    ...
                                    console.log('addToXeroTitle :', addToXeroTitle);
                                }
                                ...
                                if (this.accountingService === 'MYOB') {
                                    ...
                                    console.log('addToXeroTitle 11 :', addToXeroTitle);
                                }
                            }
                        ...
                        console.log('🏷️ Entity Name :', entityName);
                       // const existing = this.recordsBulk.find(r => r.Id === invoice.Id);
                        let mapped = {
                            Id: invoice.Id,
                            Name: invoice.Name,
                    ...
                    this.issuedRecords = mappedRecords.filter(
                        rec =>
                            rec.Status__c === 'Issued' ||
                            rec.Status__c === 'Paid' ||
                            rec.Status__c === 'Unpaid'
                    );
                    ...
                    this.pageSizeBulk = this.isPageSizeBulkManuallySet ? this.pageSizeBulk : (this.totalvalueflag ? 5 : 10);
    ...
    //         console.log('checkJSPDFIntialized in renderedCallback:', this.jsPDFInitialized  );
    ...
    //              }  
    //              //manendra added for issue invoices pdf
    //     if (window.jspdf?.jsPDF && window.autoTable) {
    //         if (!window.jspdf.jsPDF.API.autoTable) {
    //             window.jspdf.jsPDF.API.autoTable = function(options) {
    //                 return window.autoTable(this, options);
    //             };

    //         }
    //         console.log('AUTO TABLE REGISTERED');
    //     }
    // //end
    //                  else {
    ...
    renderedCallback() {
        if (!this.resizeObserver) {
            ...
            const containers = this.template.querySelectorAll('.table-container, .ndis-table-container');
        ...
        loadScript(this, jsPDF)
            .then(() => loadScript(this, autoTable))
            .then(() => loadScript(this, robotoFont))
            .then(() => {
                ...
                // Register autoTable (jsPDF guaranteed loaded first now)
                if (window.jspdf?.jsPDF && window.autoTable) {
                    if (!window.jspdf.jsPDF.API.autoTable) {
                        window.jspdf.jsPDF.API.autoTable = function(options) {
                            return window.autoTable(this, options);
                    ...
                    console.error('❌ window.autoTable not found after load');
                }

                this.jsPDFInitialized = true; // ✅ set ONCE, LAST
    ...
    setPageSizeByZoomAndScreen() {
        setTimeout(() => {
            // 1. Pending / Total Value Services Table
            if (!this.isPageSizeManuallySet && !this.totalvalueflag) {
                this.calculateAndSetPageSize('.yourInvoiceTable', '.staff-table', (rows) => {
            ...
            // 2. Bulk Invoices Table (under totalvalueflag) - Sizing is fixed (no auto)

            // 3. Issued Invoices Table
            if (!this.isPageSizeIssuedManuallySet && this.generatedflag) {
                this.calculateAndSetPageSize('.issuedInvoiceTable', '.staff-table', (rows) => {
                    if (this.issuedPageSize !== rows) {
                        this.issuedPageSize = rows;
                        this.issuedPageNumber = 1;
                        this.updateIssuedPagination();
                    }
                });
            ...
            // 4. Cancelled Invoices Table
            if (!this.isPageSizeCancelledManuallySet && this.cancelledflag) {
                this.calculateAndSizeTable('.cancelledInvoiceTable', '.staff-table', (rows) => {
                    if (this.cancelledPageSize !== rows) {
                        this.cancelledPageSize = rows;
                        this.cancelledPageNumber = 1;
                        this.updateCancelledPagination();
                    }
                });
            ...
            // 5. NDIS Services Table (Add Service)
            if (!this.isPageSize3ManuallySet && this.NdisServiceGroupName) {
                this.calculateAndSetPageSize('.ndis-table-container', '.ndis-table', (rows) => {
                    if (this.pageSize3 !== rows) {
                        this.pageSize3 = rows;
                        this.pageNumber3 = 1;
    ...
    calculateAndSetPageSize(containerSelector, tableSelector, setSizeCallback) {
        ...
        if (container) {
            ...
            const header = container.querySelector(tableSelector + ' thead');
            const headerHeight = header ? (header.getBoundingClientRect().height || header.offsetHeight) : 40;
            const availableHeight = containerHeight - headerHeight;
            
            let rowHeight = 40;
            const firstRow = container.querySelector(tableSelector + ' tbody tr');
            if (firstRow) {
    ...
    servicePAginationHelper(serviceList) {
        ...
        try {
            this.records = serviceList.map((item, index) => {
               // console.log(`\n🔹 Processing record ${index + 1} with Id=${item.Id}`);
                //console.log(`\n🔹 Processing record ${index + 1} with Status=${item.ShiftwithStaff__r.Status__c || ''}`);
                console.log(
                    `\n🔹 Processing record ${index + 1} with Status=${item.ShiftwithStaff__r?.Status__c || ''}`
                );
                // --- base fields ---
                ...
                //let shiftStatus=item.ShiftwithStaff__r.Status__c =='InProgress' ? 'In Progress' : item.ShiftwithStaff__r.Status__c || '';
                let shiftStatus = item.ShiftwithStaff__r?.Status__c == 'InProgress' ? 'In Progress' : item.ShiftwithStaff__r?.Status__c || 'N/A';


                    let shiftStatusClass = 'shift-status-badge';

                    switch (shiftStatus) {
                        case 'Draft':
                            shiftStatusClass += ' status-draft';
                            break;

                        case 'Completed':
                            shiftStatusClass += ' status-completed';
                            break;

                        case 'In Progress':
                            shiftStatusClass += ' status-in-progress';
                            break;

                        case 'Accepted':
                            shiftStatusClass += ' status-accepted';
                            break;

                        case 'Cancelled':
                            shiftStatusClass += ' status-cancelled';
                            break;

                        default:
                            shiftStatusClass += ' status-default';
                            break;
                    }
               ...
               const participantInvoiceAmount = serviceAmount + reimbursementAmount; */
                ...
                // const serviceEntityName = item.serviceEntityName || '';
                // const entityProfileId = item.entityProfileId || null;
                 console.log('entityOptions IN servicePAginationHelper : ', JSON.stringify(this.entityOptions));
                // const entityExists = this.entityOptions?.some(
                //     option => option.value === item.entityProfileId
                // ) || false;
               let entityExists = true;

                if (this.accountingService === 'Tesseract System') {
                    entityExists = this.entityOptions?.some(
                        option => String(option.value) === String(item.entityProfileId)
                    ) || false;
                }

                const serviceEntityName = entityExists
                    ? (item.serviceEntityName || 'N/A')
                    : 'N/A';

                const entityProfileId = entityExists
                    ? (item.entityProfileId || null)
                    : null;
        ...
        this.pageSize = this.isPageSizeManuallySet ? this.pageSize : (this.totalvalueflag ? 5 : 10);
    ...
    paginationHelper() {
        //manendra added for sorting the data based on the field and direction
        const state = this.tableSortState.pendingServices;
        if (state.field) {
       ...
       // this.isAllSelected = false;
        this.isAllSelected =
            ...
            this.accList.every(acc => acc.isChecked);
        console.log(
            this.accList.map(r => ({
                Id: r.Id,
                isChecked: r.isChecked,
                showCreateCompanyIcon: r.showCreateCompanyIcon,
                isCompanyEntityMissing: r.isCompanyEntityMissing
            }))
        );
        //this.disableBool = true;
        const selectedRows = this.accList.filter(acc => acc.isChecked);

        if (selectedRows.length === 0) {
            this.disableBool = true;
        } else if (this.accountingService === 'Tesseract System') {
            this.disableBool = selectedRows.some(acc => !acc.isGenerateAllowed);
        } else {
    ...
    async  handleCheckboxChange(event) {
        ...
        this.records = this.records.map(rec => rec.Id === recordId ? { ...rec, isChecked } : rec);
        ...
        // }));
        const currentPageIds = new Set(this.accList.map(r => r.Id));

        this.accList = this.accList.map(acc => ({
            ...acc,
            showCreateCompanyIcon: false,
            isCompanyEntityMissing: false,
            isGenerateAllowed: true
        }));

        this.records = this.records.map(rec =>
            currentPageIds.has(rec.Id)
                ? {
                    ...rec,
                    showCreateCompanyIcon: false,
                    isCompanyEntityMissing: false,
    ...
    async handleSelectAll(event) {
        ...
        const currentPageIds = new Set(this.accList.map(acc => acc.Id));
        ...
        console.log('Selected Records Count:',this.accList.filter(acc => acc.isChecked).length);
    ...
    @track showGenerateInvoicePopup= false;
    ...
    getGroupedEntities() {
        this.entityServicesMap = new Map();

        this.accList.filter(rec => rec.isChecked)
            .forEach(rec => {
                const entityId =  rec.entityProfileId || '';
                if (!this.entityServicesMap.has(entityId)) {
                    this.entityServicesMap.set(entityId, {
                        entityId,
                        entityName: rec.serviceEntityName,
                        //entityLabel: `${rec.serviceEntityName} (Entity Name)`,
                        checked: true,
                        services: []
                    });
                }
                this.entityServicesMap
                    .get(entityId)
                    .services
                    .push({
                        ...rec,
                ...
                console.log(
                    'Service accountItemId',
                    rec.Id,
                    rec.accountItemId
                );

            });
        console.log( 'Entity Map', this.entityServicesMap );

        this.groupedEntities = [];
        this.entityServicesMap.forEach(group => {
            let total = 0;
            group.services.forEach(service => {
                total += Number(service.participantInvoiceAmount || 0);
            });

            this.groupedEntities.push({
                entityId: group.entityId,
                entityName: group.entityName,
                //entityLabel: `${group.entityName} (Entity Name)`,
                checked: true,
                allSelected: true, 
                total,
        ...
        console.log( 'groupedEntities',JSON.stringify(this.groupedEntities) );
    }
    
    async handleGenerateSelectedInvoices() {
        const selectedEntities =
            this.groupedEntities .filter(
                    entity => entity.checked
                )
                .map(entity => ({
                    ...entity,
                    services:
                        entity.services.filter(service =>
                                service.checked
                        )
                }))
                .filter(
        ...
        console.log( 'Selected Entities:', JSON.stringify(selectedEntities));
        ...
        try {
            ...
            // Assuming `this.selectedInvoiceType` holds value: 'Xero' or 'Accounting'
            if (this.accountingService === 'Tesseract System') {
                await this.handleAccountingInvoiceGeneration( selectedEntities );
            } else if (this.accountingService === 'Xero' || this.accountingService === 'MYOB') {
               // await this.handleXeroInvoiceGeneration();
               await this.handleXeroInvoiceGeneration(selectedEntities);
            }
        } catch (error) {
            console.error('❌ Error in handleGenerateInvoice:', error);
              this.showProgressBar = false;
    ...
    async handleAccountingInvoiceGeneration(selectedEntities) {
        console.log( 'Selected Entities:', JSON.stringify(selectedEntities) );
        let totalInvoices = 0;
        // Count total participant invoices
        selectedEntities.forEach(entity => {
            const participantMap = new Map();
            entity.services.forEach(service => {
                const participantId = service.Client__r.Id;
                if (!participantMap.has(participantId)) {
                    participantMap.set(participantId,[]);
                }
                participantMap.get(participantId).push(service);
            });
            totalInvoices +=participantMap.size;
        });
        ...
        for (const entity of selectedEntities) {
            console.log( 'Processing Entity:', entity.entityName );
            ...
            console.log('Participants:', participantMap.size );
            const allParticipantKeys = Array.from(participantMap.keys());
            console.log( ' All Participant IDs for Entity:', entity.entityName,allParticipantKeys );
            // LOOP PARTICIPANTS
            
            for (const key of allParticipantKeys) {
                ...
                const participantServices = participantMap.get(key) || [];
                console.log( ' participantServices:', JSON.stringify(participantServices));

                const missingAccountItem = participantServices.some(
                    service =>
                        service.accountItemId === null ||
                        service.accountItemId === undefined ||
                        service.accountItemId === ''
                );

                if (missingAccountItem) {

                    console.error(
                        'Account ItemId is missing for participant:',
                        key
                    );
                    ...
                    this.showToast(
                        'Error',
                        //'Please select the Account List from the dropdown in the Entity before generating the invoice.',
                        'Invoice could not be generated because the account configuration is incomplete. Please update the Account List in the entity from Accounting Settings and try again.',
                        'error'
                    );
                ...
                this.salesEntry = {
                    ...
                    entityProfileId: entity.entityId,
                ...
                console.log('Amount Array:', JSON.stringify(this.amountarrey));
                ...
                try {
                    // STEP 1: Fetch invoice from Apex
                    const response = await getAccountingInvoiceById({
                    ...
                    console.error( '❌ Error while fetching invoice or generating PDF:', error );
                    this.showProgressBar = false;
                }
                invoiceIndex++;
                this.progressValue =
    ...
    async handleXeroInvoiceGeneration(selectedEntities) {

        console.log('🚀 ===== handleXeroInvoiceGeneration STARTED =====');
        console.log('Selected Entities:', JSON.stringify(selectedEntities));

        if (!selectedEntities || selectedEntities.length === 0) {
            console.warn('⚠️ selectedEntities is empty or undefined. Exiting...');
            return;
        }
        ...
        for (const entity of selectedEntities) {
            ...
            const participantMap = new Map();
            // if (!entity.entityId) {
            //     console.warn('⚠️ Entity Id missing:', entity.entityName);
            //     continue;
            // }
            if (!entity.services || entity.services.length === 0) {
                console.warn('⚠️ No services found for Entity:', entity.entityName);
                continue;
            }
            entity.services.forEach(service => {
                const participantId = service.Client__r.Id;
                if (!participantId) {
                    console.warn('Participant Id missing for service:', service.Id);
                    return;
                }
                if (!participantMap.has(participantId)) {
                    participantMap.set(participantId, []);
            ...
            console.log( 'Participants inside entity:',  participantMap.size );
            const allParticipantKeys = Array.from(participantMap.keys());
            console.log( 'Participant Ids:',  allParticipantKeys);

             // LOOP PARTICIPANTS
            for (const key of allParticipantKeys) {
                console.log('Processing Participant:', key);
                ...
                try {
                    ...
                    /* participantServices.forEach(service => {
                        if (!service.Id) {
                            console.warn('⚠️ Service Id missing');
                            return;
                        }
    ...
    if (
        service.ParticipantReimbrusements__r &&
        Array.isArray(service.ParticipantReimbrusements__r)
    ) {

        service.ParticipantReimbrusements__r.forEach(rec => {

            if ((rec.Reimbrusement_Amount__c || 0) <= 0) {
                return;
            }

            this.xeroSalesEntryList.push({
                ...
                Description__c:
                    `Reimbursement - ${rec.Reimbrusment_Support_item__c}`,

                Quantity__c: rec.Reimbrusment_Quantity__c, 

                UnitPrice__c: rec.Reimbrusment_Rate__c,

                Amount__c: rec.Reimbrusment_Rate__c,
                ...
                lineItems: rec.Reimbrusment_line_item__c,
                    ...
                    // PROGRESS BAR
                    invoiceIndex++;
                    this.progressValue = Math.round( (invoiceIndex / totalInvoices) * 100 );
    ...
    async generateBase64Data() {
        ...
        // doc.addImage(this.orgLogo, 'PNG', 20, 5, 10, 10, );
            ...
            if (this.orgLogo) {
                try {
                    ...
                    // Calculate scale without stretching
                    ...
                    // Center logo inside 40x20 area
                    const logoX = 10 + (maxWidth - imgWidth) / 2;
                    const logoY = 5 + (maxHeight - imgHeight) / 2;
                    ...
                    doc.addImage(
                        ...
                        logoX,       // X
                        logoY,       // Y
                        imgWidth,    // proportional width
                        imgHeight    // proportional height
                    );


                    headerY = 30;   // text starts below logo
            ...
            doc.text(this.orgName.toUpperCase(), 10, headerY); 
           ...
           console.log('616');
            if (this.accountingService === 'Tesseract System') {
                ...
                doc.setFont("Roboto-Bold", "bold");
                doc.setFontSize(12);
                doc.text("ABN: "+ invoice.Company__r.ABN__c, 10, headerY + 5);
                doc.setFont("Roboto-VariableFont_wdth,wght", "normal");
        
            
                doc.setFontSize(10);
                doc.text(invoice.Company__r.Address_Latest__Street__s+",", 10,headerY + 10 );
                doc.text(`${invoice.Company__r.Address_Latest__City__s} ${invoice.Company__r.Address_Latest__StateCode__s} ${invoice.Company__r.Address_Latest__PostalCode__s},`, 10, headerY + 15);
                // doc.text("Contact: "+(invoice.Company__r.Phone_Number__c || ''), 10, headerY + 20);
                doc.text("Contact: "+invoice.Company__r.Phone_Number__c, 10, headerY + 20);
                ...
                doc.setFont("Roboto-Bold", "bold");
                doc.setFontSize(12);
                doc.text("ABN: "+ invoice.Participant__r.Facility__r.ABN__c, 10, headerY + 5);
                doc.setFont("Roboto-VariableFont_wdth,wght", "normal");
        
            
                doc.setFontSize(10);
                doc.text(invoice.Participant__r.Facility__r.Address__Street__s+",", 10,headerY + 10 );
                doc.text(`${invoice.Participant__r.Facility__r.Address__City__s} ${invoice.Participant__r.Facility__r.Address__StateCode__s} ${invoice.Participant__r.Facility__r.Address__PostalCode__s},`, 10, headerY + 15);
                //doc.text("Contact: "+(invoice.Participant__r.Facility__r.Phone__c || ''), 10, headerY + 20);
                doc.text("Contact: "+invoice.Participant__r.Facility__r.Phone__c, 10, headerY + 20);
        ...
        // doc.text("Invoice Number", 150, 24);
            ...
            doc.setFont("Roboto-Bold", "bold");
            doc.setFontSize(12);
            doc.text(invoice.Name, 158, invoiceHeaderY + 5);
            ...
            doc.setFont("Roboto-VariableFont_wdth,wght", "normal");
            doc.setFontSize(10);
            doc.text("Date Issued: "+newDate, 158, invoiceHeaderY + 10);
            doc.setFont("Roboto-VariableFont_wdth,wght", "normal");
            doc.setFontSize(10);
            ...
            let participantStartY = taxInvoiceY + 6;

            const participant = invoice.Participant__r || {};

            // ✅ Correct participant name
            const participantName = participant.Name__c || '';
            const participantDob = participant.Date_Of_Birth__c || '';
            const formattedParticipantDob = new Date(participantDob)
                .toLocaleDateString('en-GB');
            console.log('participantDob : ',formattedParticipantDob);
        ...
        //    const participantAddress = [
        //     participant?.Facility__r?.Address__Street__s,
        //     participant?.Facility__r?.Address__City__s,
        //     participant?.Facility__r?.Address__StateCode__s,
        //     participant?.Facility__r?.Address__PostalCode__s
        // ]
            ...
            doc.setFont("Roboto-Bold", "bold");
            doc.text("DOB", 10, participantStartY + 5);
            doc.text(":", 55, participantStartY + 5);
            doc.setFont("Roboto-VariableFont_wdth,wght", "normal");
            doc.text(formattedParticipantDob, 58, participantStartY + 5, {
                maxWidth: 130   // keeps it clean on page
            });

            doc.setFont("Roboto-Bold", "bold");
            doc.text("Address", 10, participantStartY + 10);
            doc.text(":", 55, participantStartY + 10);
            doc.setFont("Roboto-VariableFont_wdth,wght", "normal");
            doc.text(participantAddress, 58, participantStartY + 10, {
                maxWidth: 130   // keeps it clean on page
            });
            ...
            if (participantNDIS) {
                doc.setFont("Roboto-Bold", "bold");
                doc.text("NDIS Number", 10, participantStartY + 15);
                doc.text(":", 55, participantStartY + 15);
                doc.setFont("Roboto-VariableFont_wdth,wght", "normal");
                doc.text(participantNDIS, 58, participantStartY + 15);
            }

            // ✅ IMPORTANT: push table BELOW participant section
            let participantEndY = participantStartY + 21;
        ...
        console.log('RESULT'+JSON.stringify(result));
            ...
            console.log('typeof doc.autoTable = ', typeof doc.autoTable);
            console.log('typeof window.autoTable = ', typeof window.autoTable);
            console.log('typeof window.jspdf.autoTable = ', typeof window.jspdf?.autoTable);
       ...
       const result1 = await uploadFile({
        ...
        console.log('data in uploadFile', result1);
   ...
   createRow(service, accountValue) {
    ...
    handleRecordsPerPageBulk(event) {        
        const selectedValue = event.target.value;
        if (selectedValue === 'Auto') {
            this.isPageSizeBulkManuallySet = false;
            this.setPageSizeByZoomAndScreen();
        } else {
            this.isPageSizeBulkManuallySet = true;
            this.pageSizeBulk = parseInt(selectedValue, 10);
            this.pageNumberBulk = 1;
            this.paginationHelperBulk();
        }
    }
    ...
    const state = this.tableSortState.totalInvoices;
    let displayRecords = [...this.recordsBulk];
    if (state.field) {
   ...
   // this.bulkSelectAllChecked = Boolean(false);
       ...
       // console.log('this.records in paginationHelper:', JSON.stringify(this.records));
        ...
        this.bulkServices.push(displayRecords[i]);    // maendra added for table sorting

        } 
        console.log("🔹 bulkServices in pagination :", JSON.stringify(this.bulkServices));
        this.bulkSelectAllChecked =
                this.bulkServices.length > 0 &&
                this.bulkServices.every(inv => inv.isChecked);

        // Restore Bulk Invoice button
        this.disableBulkInvoice =
            !this.bulkServices.some(inv => inv.isChecked);

        // Restore selected ids
        this.selectedInvoiceIds = this.bulkServices
            .filter(inv => inv.isChecked)
            .map(inv => inv.Id);

        // Restore Export button
        this.showExportCSV =
            this.accountingService === 'Tesseract System' &&
 ...
 saveRow(event) {
    ...
    this.searchKey               = '';
        ...
        case "Pending":
            ...
            this.totalvalueflag = false;
            this.cancelledflag = false;
            this.isPageSizeManuallySet = false; // allow auto sizing on Pending tab
            this.pageSize = 10; // Default pending page size
            this.paginationHelper();
        ...
        case "Total":
            ...
            this.totalvalueflag = true;
            this.cancelledflag = false;
            this.pageSize = 5; // Default Total Values Services Table size (no Auto)
            this.pageSizeBulk = 5; // Default Total Values Invoices Table size (no Auto)
            this.isPageSizeManuallySet = true;
            this.isPageSizeBulkManuallySet = true;
            this.paginationHelper();
            this.paginationHelperBulk();
    ...
    updateIssuedPagination() {
    //manendra added for sorting table
    const state = this.tableSortState.issuedInvoices;
    let displayRecords = [...this.issuedRecords];
    if (state.field) {
        displayRecords = this.sortData(
            displayRecords,
            state.field,
       ...
       // this.issuedPageList = this.issuedRecords.slice(start, end).map(r => ({
        this.issuedPageList = displayRecords.slice(start, end).map(r => ({ // manendra added for table sorting
            ...r,
            isChecked: false
        }));
    }
    ...
    handleRecordsPerPageIssued(event) {
        const selectedValue = event.target.value;
        if (selectedValue === 'Auto') {
            this.isPageSizeIssuedManuallySet = false;
            this.setPageSizeByZoomAndScreen();
        } else {
            this.isPageSizeIssuedManuallySet = true;
            this.issuedPageSize = parseInt(selectedValue, 10);
            this.issuedPageNumber = 1;
            this.issuedTotalPages = Math.ceil(this.issuedTotal / this.issuedPageSize) || 1;
            this.updateIssuedPagination();
        }
    ...
    updateCancelledPagination() {
        //manendra added for sorting table
        const state = this.tableSortState.cancelledInvoices;
        let displayRecords = [...this.cancelledRecords];
        if (state.field) {
            displayRecords = this.sortData(
                displayRecords,
                state.field,
       ...
       // this.cancelledPageList = this.cancelledRecords.slice(start, end);
         this.cancelledPageList = displayRecords.slice(start, end);// manendra added for sorting table
    }

    handleRecordsPerPageCancelled(event) {
        const selectedValue = event.target.value;
        if (selectedValue === 'Auto') {
            this.isPageSizeCancelledManuallySet = false;
            this.setPageSizeByZoomAndScreen();
        } else {
            this.isPageSizeCancelledManuallySet = true;
            this.cancelledPageSize = parseInt(selectedValue, 10);
            this.cancelledPageNumber = 1;
            this.cancelledTotalPages = Math.ceil(this.cancelledTotal / this.cancelledPageSize) || 1;
            this.updateCancelledPagination();
        }
    ...
    handleGetEmailBody() {
     ...
     handleSendEmail(event){
        ...
        async handleAddToXero(event) {
            ...
            // =====================================
            // XERO CONTACT VALIDATION
            // =====================================
            if (this.accountingService === 'Xero' && !invoice.xeroContactId) {
                this.showToast(
                    'Validation Error',
                    'Entity is not synced. Please sync it in the Xero module before proceeding.',
                    'error'
                );
                return;
            }
            ...
            // =====================================
            // MYOB CONTACT VALIDATION
            // =====================================
            if (this.accountingService === 'MYOB' && !invoice.xeroContactId) {
                this.showToast(
                    'Validation Error',
    ...
    handleOpenCreateCompany(event) {
        ...
        try{
            ...
            const companyEntityId = event.currentTarget.dataset.companyEntityId;
            const entityProfileId = event.currentTarget.dataset.entityProfileId;
            console.log(' Facility Id:', facilityId);
            console.log('Fund Tracker Id:', fundTrackerId);
            console.log('participantId :', participantId);
            console.log('Company Entity Id:', companyEntityId);
            console.log('Service Entity Id:', entityProfileId);
            ...
            this.createServiceEntityFlag = false;
            // setTimeout(() => {
            //     this.showProgressBar = false;
            // }, 3000);
            // Case 1:
            ...
            // Case 2:
            // Company Entity exists but Service Entity missing
            else if (!entityProfileId) {
               
                setTimeout(() => {
                    this.createServiceEntityFlag = true;
                    this.createCompanyFlag = false;
                    this.showProgressBar = false;
            ...
            console.error('handleOpenCreateCompany Error:', error);
            this.showProgressBar = false;
        } 
    ...
    async  handleCreateCompanyBack() {
        ...
        // if(!this.accountingServiceflag){
        //     await this.checkCompanyExist();
        // }
        await this.checkCompanyExist();
        this.accList = this.accList.map(acc => {
            if (acc.Id === this.currentServiceId) {
                return {
                    ...acc,
                    showCreateCompanyIcon: false,
                   // entityProfileId: entityId,
                    //serviceEntityName: entityName
                    //isEditingServiceEntity: false
                };
            }
            return acc;
        });
        ...
        if (this.isEntityFromAddService) {
             if (this.accountingService === 'Tesseract System') {
                ...
                await this.loadXeroEntities();
            }
                    
             await this.fetchNdisServiceLineItems();
    ...
    async handleCreateEntityBack(event) {
        ...
        if(this.isEntityFromAddService){
           ...
           this.entityOptions = [
                ...this.entityOptions.filter(o => o.value !== entityId),
                newOption
            ];
    ...
    async checkCompanyExist() {
        if(this.accountingService === 'Xero' || this.accountingService === 'MYOB'){
            return;
        }
        const allKeys = [
            ...new Set(
       ...
       // const pendingFundUpdates = [];
        ...
        let hasServiceEntityMissing = false;
        /* --------------------------------------------------
        ...
        * -------------------------------------------------- */
        this.accList = this.accList.map(acc => {
           ...
           // const fundEntityId = fundEntityMap[fundTrackerId] || null;
            const companyEntityId = companyId ? companyEntityMap[companyId] || null : null;
            console.log( 'SERVICE ENTITY CHECK => ', acc.Id,  'entityProfileId => ', acc.entityProfileId, 'serviceEntityName => ',  acc.serviceEntityName);
            const serviceEntityId = acc.entityProfileId || null;
            // const isCompanyMissing       = !companyId;
            // const isCompanyEntityMissing = companyId && !companyEntityId;
            // const isAutoUpdatingEntity   = companyEntityId && !serviceEntityId;
            const isCompanyMissing = !companyId;
            const isCompanyEntityMissing = !companyEntityId;
            console.log('companyId :', companyId);
            console.log('companyEntityId :', companyEntityId);
            // console.log('fundEntityId  :', fundEntityId);
            console.log('serviceEntityId  :', serviceEntityId);
            ...
            if (!companyId || missingClientIds.includes(clientId)) {
                ...
                showCreateCompanyIcon = acc.isChecked === true;
            } else if (companyId && companyEntityId && !serviceEntityId) {
                showCreateCompanyIcon = acc.isChecked === true;
                 if (acc.isChecked) {
                    hasServiceEntityMissing = true;
                }
            }
            ...
            //const finalEntityId = serviceEntityId;
            console.log(
    'Ledger Mapping',
    acc.entityProfileId,
    entityLedgerMap[acc.entityProfileId]
);
            ...
            return {
                ...
                entityProfileId: acc.entityProfileId,
                serviceEntityName: acc.serviceEntityName,
                // accountItemId: finalEntityId
                //     ? entityLedgerMap[finalEntityId]
                //     : null,
                 accountItemId: acc.entityProfileId && entityLedgerMap[acc.entityProfileId]
                        ? entityLedgerMap[acc.entityProfileId]
                        : null,
        ...
        console.log(
            'accList after checkCompanyExist',
            JSON.stringify(this.accList)
        );
        const pageMap = new Map(this.accList.map(r => [r.Id, r]));

        this.records = this.records.map(rec => {
            const updated = pageMap.get(rec.Id);
        ...
        * -------------------------------------------------- */
        ...
        } else if (hasServiceEntityMissing ) {

            this.disableBool = true;

             if (!this.hideCompanyToast) {
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Entity details are missing.',
                        message: 'Please use the option shown in the Status column to create the entity.',
...
get isIssuedHeaderChecked() {
    ...
    // }
    get isBulkHeaderChecked() {
        if (!this.bulkServices || this.bulkServices.length === 0) {
            return false;
        }

        const validRows = this.bulkServices.filter(r =>
            r.invoiceEntityName &&
            r.invoiceEntityName.trim() !== '' &&
            r.invoiceEntityName !== 'N/A'
        );

        return (
            validRows.length > 0 &&
            validRows.every(r => r.isChecked)
        );
    }
    ...
    checkBulkEntityExist() {
        ...
        const listName = this.generatedflag
            ? 'issuedPageList'
            : 'bulkServices';
        const currentPageIds = new Set(this[listName].map(r => r.Id));
        ...
        this.recordsBulk = this.recordsBulk.map(inv => {
            const pageRecord = this[listName].find(r => r.Id === inv.Id);

            if (pageRecord && currentPageIds.has(inv.Id)) {
                return {
                    ...inv,
                    isChecked: pageRecord.isChecked
                };
            }
        ...
        if (hasMissingEntity) {
            if (this.accountingService === 'Tesseract System') {
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error',
                        message: 'Entity details are missing for selected records.',
                        variant: 'error'
                    })
                );
            }
...
handleBulkInvoiceCheckboxChange(event) {
    ...
    this.showExportCSV =  this.accountingService === 'Tesseract System' && this.selectedInvoiceIds.length > 0;
...
handleSelectAllInvoices(event) {
    ...
    const currentPageIds = new Set(
        this[listName].map(inv => inv.Id)
    );

    this.recordsBulk = this.recordsBulk.map(inv =>
        currentPageIds.has(inv.Id)
            ? { ...inv, isChecked }
            : inv
    );
...
handleBulkInvoiceEmail() {
    ...
    async handleAddServiceChange(event) {
        const fieldName = event.target.name;
        const fieldValue = event.target.value;
        ...
        console.log(' Start Time in handleAddServiceChange:', this.startTime);
        console.log(' End Time in handleAddServiceChange:', this.endTime);
    }
    // handleAddServiceEntityChange(event) {
    //     const rowId = event.target.dataset.id;
    //     const value = event.detail.value;

    //     if (value === 'Add New Entity') {

    //         this.selectedServiceRowId = rowId;
    //         console.log('selectedServiceRowId >>>', this.selectedServiceRowId);
    //         console.log('serviceTypeId >>>', this.serviceTypeId);
    ...
    //         this.participantId = this.serviceParticipant;
    //         console.log('createCompanyFundTrackerId >>>', this.createCompanyFundTrackerId);
    //         console.log('createCompanyFacilityId >>>', this.createCompanyFacilityId);
    ...
    //     });
    // }
    async handleAddServiceEntityChange(event) {
        const rowId = event.target.dataset.id;
        const value = event.detail.value;
       ...
       if (value === 'Add New Entity') {
            ...
            try {
                ...
                const result = await getCompanyAndAccountData1({
                    participantIdList: [this.serviceParticipant],
                    dashboardFacilityId: this.faclist,
                    entityIds: []
                });
                ...
                const companyId = companyResponse[this.serviceParticipant];
                const facilityId = participantFacilityMap[this.serviceParticipant];

                const companyEntityId =
                    companyId && !companyId.startsWith('NoData ')
                        ? companyEntityMap[companyId]
                        : null;
                ...
                // Company missing
                if (!companyId || companyId.startsWith('NoData ')) {

                    this.createCompanyFlag = true;

                }
                // Company exists but Company Entity missing
                else if (!companyEntityId) {

                    this.createCompanyFlag = true;

                }
                // Company Entity exists -> Create Service Entity
                else {
    ...
    async fetchNdisServiceLineItems() {
        try {
            ...
            const result = await getCatalogueData({
                ...
                dateOfService:this.AddServiceStartDate ||''
            });
            ...
            this.EntityNameOptions = [...(this.entityOptions || [])];

            // Always add Add New Entity option

           // if (!this.EntityNameOptions.some(opt => opt.value === 'Add New Entity')) {
               if (this.accountingService === 'Tesseract System') {
                    this.EntityNameOptions.push({
           ...
           // }
            ...
            console.log(
                'EntityNameOptions ==> ',
                JSON.stringify(this.EntityNameOptions)
            );
            ...
            this.records3 = catalogueData.map((rec) => {
                ...
                const entityId = result.entityIdMap?.[rec.Id] || '';
                const entityName = result.entityNameMap?.[rec.Id] || '';
                const entityExists = this.EntityNameOptions?.some(
                ...
                return {
                    ...
                    //entityId: result.entityIdMap?.[rec.Id] || '',
                    //entityName: result.entityNameMap?.[rec.Id] || '',
                    // entityId,
                    // entityName,
                    entityId: entityExists ? entityId : '',
                    entityName: entityExists ? entityName : '',
                    showEntityDropdown: !entityExists,
                    label: displayName,
                    value: rec.Id
                };
            });
            ...
            // this.records3.forEach(row => {
            //     if (row.entityId && !entityMap.has(row.entityId)) {
            ...
            // this.EntityNameOptions = [...entityMap.values()];
    ...
    saveAddService() {
       ...
       const selectedRows = this.records3.filter(r => r.isSelected);
        ...
        console.log('📦 Final save payload:', JSON.stringify(payload));
         ...
         this.showProgressBar = true;
         saveServiceSupportPlans({ services: payload ,accountingService: this.accountingService})
        .then(savedServices => {
            ...
            const manualRows = savedServices.map((svc, index) => ({
                ...
                Funds_Tracker__r: {
                    ...
                    //Entity_Profile__c:  svc.Funds_Tracker__r.Entity_Profile__c || ''
                },
    ...
    handleRecordsPerPage3(event) {
        const selectedValue = event.target.value;
        this.isPageSize3ManuallySet = true;
        this.pageSize3 = parseInt(selectedValue, 10);
        this.pageNumber3 = 1;
        this.paginationHelper3();
        console.log('pageSize3 in handleRecordsPerPage3:', this.pageSize3);
    }
    ...
    get pageSizeOptions() {
        return [
            ...
            { label: '5', value: 5, selected: this.isPageSizeManuallySet && this.pageSize === 5 },
            { label: '10', value: 10, selected: this.isPageSizeManuallySet && this.pageSize === 10 },
            { label: '25', value: 25, selected: this.isPageSizeManuallySet && this.pageSize === 25 },
            { label: '50', value: 50, selected: this.isPageSizeManuallySet && this.pageSize === 50 },
            { label: '75', value: 75, selected: this.isPageSizeManuallySet && this.pageSize === 75 },
            { label: '100', value: 100, selected: this.isPageSizeManuallySet && this.pageSize === 100 }
        ];
    }

    get pageSizeOptionsTotalServices() {
        return [
            { label: '5', value: 5, selected: this.pageSize === 5 },
            { label: '10', value: 10, selected: this.pageSize === 10 },
            { label: '25', value: 25, selected: this.pageSize === 25 },
            { label: '50', value: 50, selected: this.pageSize === 50 },
            { label: '75', value: 75, selected: this.pageSize === 75 },
            { label: '100', value: 100, selected: this.pageSize === 100 }
        ];
    }

    get pageSizeOptionsBulk() {
        return [
            { label: '5', value: 5, selected: this.pageSizeBulk === 5 },
            { label: '10', value: 10, selected: this.pageSizeBulk === 10 },
            { label: '25', value: 25, selected: this.pageSizeBulk === 25 },
            { label: '50', value: 50, selected: this.pageSizeBulk === 50 },
            { label: '75', value: 75, selected: this.pageSizeBulk === 75 },
            { label: '100', value: 100, selected: this.pageSizeBulk === 100 }
        ];
    }

    get issuedPageSizeOptions() {
        return [
            { label: 'Auto', value: 'Auto', selected: !this.isPageSizeIssuedManuallySet },
            { label: '5', value: 5, selected: this.isPageSizeIssuedManuallySet && this.issuedPageSize === 5 },
            { label: '10', value: 10, selected: this.isPageSizeIssuedManuallySet && this.issuedPageSize === 10 },
            { label: '20', value: 20, selected: this.isPageSizeIssuedManuallySet && this.issuedPageSize === 20 },
            { label: '50', value: 50, selected: this.isPageSizeIssuedManuallySet && this.issuedPageSize === 50 }
        ];
    }

    get cancelledPageSizeOptions() {
        return [
            { label: 'Auto', value: 'Auto', selected: !this.isPageSizeCancelledManuallySet },
            { label: '5', value: 5, selected: this.isPageSizeCancelledManuallySet && this.cancelledPageSize === 5 },
            { label: '10', value: 10, selected: this.isPageSizeCancelledManuallySet && this.cancelledPageSize === 10 },
            { label: '20', value: 20, selected: this.isPageSizeCancelledManuallySet && this.cancelledPageSize === 20 },
            { label: '50', value: 50, selected: this.isPageSizeCancelledManuallySet && this.cancelledPageSize === 50 }
        ];
    }

    get pageSizeOptions3() {
        return [
            { label: '5', value: 5, selected: this.pageSize3 === 5 },
            { label: '10', value: 10, selected: this.pageSize3 === 10 },
            { label: '20', value: 20, selected: this.pageSize3 === 20 },
            { label: '50', value: 50, selected: this.pageSize3 === 50 }
        ];
    }
    paginationHelper3() {
        ...
        console.log('pageSize3:', this.pageSize3);
        console.log('totalRecords3:', this.totalRecords3);
        console.log(' this.totalPages3:', this.totalPages3);
        ...
        console.log('pageNumber3:', this.pageNumber3);
        console.log('pageSize3:', this.pageSize3);
        console.log('totalRecords3:', this.totalRecords3);
    
     
        for (let i = (this.pageNumber3 - 1) * this.pageSize3; i < this.pageNumber3 * this.pageSize3; i++) {
            console.log('i =', i);
            if (i === this.totalRecords3) {
                break;
            }
            console.log('Pushing:', JSON.stringify(this.records3[i]));
            this.serviceGroupName.push(this.records3[i]);            
        } 
         
        // this.addServicePaginationVisible = this.totalPages3 > 0;
        this.addServicePaginationVisible = this.totalPages3 > 0;
        console.log('this.serviceGroupName length :', this.serviceGroupName.length);

        console.log('serviceGroupName (paginated) >>>',  JSON.stringify(this.serviceGroupName));
    ...
    handleBackToManageInvoice(event) {
        ...
        console.log( '⬅️ Returning to Manage Invoice' );
       // this.isAddNewService = true;
        this.isFromManageInvoice = false;
        this.tilesflag = false;
        this.isModalRelate = true;
        ...
        if (cancelled) {
            ...
            console.log(' this.isAddNewService IN CANCEL ', this.isAddNewService );
            if (this.isEntityFromAddService) {
                this.isAddNewService = true;
                this.isEntityFromAddService = false;
        ...
        if (this.isEntityFromAddService) {
             ...
             console.log(' this.isAddNewService  IN isEntityFromAddService', this.isAddNewService );
        ...
        else if (isEditFromManageInvoice) {
            console.log(' this.isAddNewService  IN isEditFromManageInvoice', this.isAddNewService );
            console.log( 'calling refreshApex for this.wiredResult 111 ');
            this.refreshTimestamp = Date.now();
            setTimeout(() => {
                refreshApex(  this.wiredResult );
            }, 1000);
             ...
             console.log( 'else block  ');
              console.log(' this.isAddNewService  IN else block  ', this.isAddNewService );
            //this.showCreateFundsFlag = false;
            this.isAddNewService = true;
            this.showNoFundsModal = false;
            if (this.isEntityFromAddService) {
        ...
        this.cancelledflag = false;
          console.log(' this.isAddNewService  AT the end ', this.isAddNewService );
            //this.showCreateFundsFlag = false;
    ...
    get ndisContainerClass() {
        return this.ndisflag
            ? 'table-container ndis-table-container with-ndis'
            : 'table-container ndis-table-container no-ndis';
    }
       ...
       return fetchEntityList()
        .then((result) => {
           ...
           // }
            ...
            console.log("🔹 entityOptions (with Add option):", JSON.stringify(this.entityOptions));
        })
        .catch((error) => {
    ...
    loadEntityProfiles() {
        ...
        return fetchEntity({ facilityId: this.faclist })
            .then((result) => {
                ...
                // 2️⃣ CASE: No entities → fetch company directly
                    ...
                    return fetchCompanyByFacility({ facilityId: this.faclist })
                        .then((company) => {
                            if (company) {
                            ...
                            console.log('entityOptions length : ', this.entityOptions.length);
                        })
                        .catch((error) => {
     ...
     handleEntityChange(event) {
        ...
        console.log("🔹 issuedPageList:", JSON.stringify(this.issuedPageList));

        
        if (this.generatedflag === true) {
            // ✅ store as draft (DO NOT update actual field)
            this.issuedPageList = this.issuedPageList.map(inv => {
                console.log('🔹 Current inv.Id =>', inv.Id);
                console.log('🔹 Current invoiceEntityName =>', inv.invoiceEntityName);
                console.log('🔹 RecordId to Match =>', recordId);

                if (inv.Id === recordId) {
                    console.log('✅ MATCH FOUND');
                    console.log('✅ Returning Updated Object =>', JSON.parse(JSON.stringify({
                        ...inv,
                        draftEntityName: value
                        //invoiceEntityName: value
                    })));
                    return {
                        ...inv,
    ...
    saveentityRow(event) {
        ...
        if (this.generatedflag === true) {
         ...
         console.log("🔹 issuedPageList 11:", JSON.stringify(this.issuedPageList));
         ...
         console.log("🔹 bulkServices:", JSON.stringify(this.bulkServices));
        ...
        console.log('✅ Selected Row:',   JSON.parse(JSON.stringify(row)));
        ...
        const finalValue = row.draftEntityName 
         ...
         console.log('📌 draftEntityName =>', row.draftEntityName);
        console.log('📌 invoiceEntityName =>', row.invoiceEntityName);
    ...
    handleCloseGenerateInvoicePopup() {
        this.showGenerateInvoicePopup = false;
        this.isInvoiceflag = true;
        this.showProgressBar = false;
        this.progressValue = 0;
    ...
    handleEntitySelection(event) {
        ...
        const checked = event.target.checked;
        this.groupedEntities = this.groupedEntities.map(entity => {
            if (entity.entityId === entityId) {
                entity.checked = checked;
                entity.allSelected = checked;
        ...
        console.log('Entity Selection:', JSON.stringify(this.groupedEntities) );
    }

    handleEntityServiceSelectAll(event) {
        const entityId = event.target.dataset.entityid;
        const checked = event.target.checked;
        this.groupedEntities = this.groupedEntities.map(entity => {
            if (entity.entityId === entityId) {
        ...
        console.log('Entity Service Select All:', JSON.stringify(this.groupedEntities) );
    }
    ...
    handleServiceSelection(event) {
        ...
        this.groupedEntities = this.groupedEntities.map(entity => {
            if (entity.entityId === entityId) {
                ...
                // Header/Entity checkbox becomes checked
                //  if any service is checked
                entity.checked = entity.services.some(
                    service => service.checked
                );
                entity.allSelected = entity.services.every(
                    service => service.checked
                );
            }
            return {
        ...
        console.log('Service Selection:',JSON.stringify(this.groupedEntities));
    }
    @track currentServiceId;
    @track hideCompanyToast=false;
    async enableServiceEntityEdit(event) {
        try {
            event.preventDefault();
            const recordId = event.currentTarget.dataset.id;
            this.currentServiceId = recordId;
            if (!recordId) return;

            let row = this.accList.find(acc => acc.Id === recordId);

            console.log('Before checkCompanyExist', JSON.parse(JSON.stringify(row)));

            // ---------------------------------------------------
            // If details are not available, populate them first
            // ---------------------------------------------------

            if (
                !row.matchedFacilityId ||
                !row.participantId ||
                !row.companyEntityId
            ) {

                // Temporarily mark row as selected
                this.accList = this.accList.map(acc => ({
                    ...acc,
                    isChecked: acc.Id === recordId
                }));

                this.selectedRecordMap.clear();

                this.selectedRecordMap.set(recordId, {
                    clientId: row.Client__r.Id,
                    status: row.Status__c,
                    bill: row.Bill_Participant__c,
                    fundsTrackerId: row.Funds_Tracker__c
                });
                this.hideCompanyToast = true;
                await this.checkCompanyExist();
                this.hideCompanyToast = false;
                // fetch updated row
                row = this.accList.find(acc => acc.Id === recordId);

                console.log('After checkCompanyExist', JSON.parse(JSON.stringify(row)));
            }

            console.log('Facility Id:', row.matchedFacilityId);
            console.log('Fund Tracker Id:', row.fundsTrackerId);
            console.log('participantId:', row.participantId);
            console.log('Company Entity Id:', row.companyEntityId);
            console.log('Service Entity Id:', row.entityProfileId);
            
            const storedFacilityId = localStorage.getItem("defaultFacilityId");
            console.log('Clicked row in enableServiceEntityEdit :', recordId);
            console.log('FacilityId in enableServiceEntityEdit:', storedFacilityId);
            //if (!recordId) return;

            // if (this.pendingflag || this.totalvalueflag) {
            //     this.accList  = this.accList.map(acc => {
            //         return {
            //             ...acc,
            //             isEditingServiceEntity: acc.Id === recordId
            //         };
            //     });
            // } 
            // ✅ ONLY ADD THIS (no other changes)
            ...
            //setTimeout(() => {
                ...
                if (this.pendingflag || this.totalvalueflag) {
                    this.accList = this.accList.map(acc => {
                        ...
                        return {
                            ...acc,
                            isEditingServiceEntity: hasEntityOptions,
                            showCreateCompanyIcon: !hasEntityOptions
                        };
                    });
                }
                if (!hasEntityOptions) {
                    ...
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Entity details are missing.',
                            //message: 'No Entity is available. Please click the Create Company/Entity icon in the Status column.',
                            message: 'Please use the option shown in the Status column to create the entity.',
                            variant: 'warning'
                        })
                    );
                ...
                if(hasEntityOptions &&  !row.entityProfileId) {
                     this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Warning',
                            //message: 'No Entity is available. Please click the Create Company/Entity icon in the Status column.',
                            message: 'Entity details are missing.',
                            variant: 'warning'
                        })
                    );
                }
           ...
           // }, 250);
            ...
            console.error('enableServiceEntityEdit  Error:', error);
            this.resetSelectionState();
        }
    }
    resetSelectionState() {
    ...
    handleServiceEntityChange(event) {
        ...
        if (this.pendingflag || this.totalvalueflag) {
            // ✅ store as draft (DO NOT update actual field)
            this.accList = this.accList.map(acc => {
                console.log('🔹 Current acc.Id =>', acc.Id);
                console.log('🔹 Current serviceEntityId =>', acc.serviceEntityId);
                console.log('🔹 RecordId to Match =>', recordId);
                ...
                if (acc.Id === recordId) {
                    ...
                    console.log('✅ Returning Updated Object =>', JSON.parse(JSON.stringify({
                        ...acc,
                        draftServiceEntity: value
                        //invoiceEntityName: value
                    })));
                    return {
                        ...acc,
                        draftServiceEntity: value   // 🔥 TEMP VALUE
                        //invoiceEntityName:value
                    };
                }
                console.log(
                    '➡️ Returning Original acc =>',
    ...
    saveServiceEntityRow(event) {
        console.log('========== saveServiceEntityRow START ==========');
        const recordId = event.currentTarget.dataset.id;
        console.log('Clicked Record Id:', recordId);

        let currentList = [];
        if (this.pendingflag || this.totalvalueflag) {
            currentList = this.accList;
        }
        console.log('Current List Size:', currentList.length);
        const row = currentList.find(acc => acc.Id === recordId);
        if (!row) {
            console.warn('Row not found:', recordId);
            return;
        }
        const finalValue = row.draftServiceEntity
            ? row.draftServiceEntity
            : row.serviceEntityId;
        console.log('Final Entity Id:', finalValue);
        ...
        console.log('Saving Service Entity');
        console.log('Fund Tracker:', row.Funds_Tracker__c);
        console.log('Service Type:', row.Service_Type__c);
        console.log('Accounting Service:', this.accountingService);

        // updateServiceEntity({
        //     fundTrackerId: row.Funds_Tracker__c,
        //     supportCatalogueId: row.Service_Type__c,
        //     entityId: finalValue,
        //     accountingService: this.accountingService
        // })
        updateServiceEntity({
        ...
        .then(()  => {
            ...
            // const junctionId = result;
            // console.log('Returned Junction Id:', junctionId);
            // if (!junctionId) {
            ...
            //                 serviceEntityName: 'N/A',
            //                 draftServiceEntity: null,
            //                 isEditingServiceEntity: false
            //             };
            //         }
            //         return acc;
            ...
            //     // this.showToast(
            //     //     'Error',
            //     //     'No matching Service Entity mapping found.',
            //     //     'error'
            //     // );
            ...
            this.accList = this.accList.map(acc => {
                if (acc.Id === recordId) {
                    return {
                        ...acc,
                        serviceEntityId: finalValue,
                        serviceEntityName: selectedLabel,
                        draftServiceEntity: null,
                        isEditingServiceEntity: false
                    };
                }
            ...
            console.log('Service Entity Updated');
            this.refreshTimestamp = Date.now();
            refreshApex(this.wiredResult);
            console.log('========== saveServiceEntityRow END  ==========');

        })
        .catch(error => {
            console.error('updateServiceEntity Error', error);
             this.accList = this.accList.map(acc => {
                if (acc.Id === recordId) {
                    return {
                        ...acc,
            ...
            console.log('========== saveServiceEntityRow FAILED ==========');
        });
    }
    enableStatusEdit(event) {

        const rowId = event.currentTarget.dataset.id;
        console.log('statusOptions11', JSON.stringify(this.statusOptions));
        console.log('length11', this.statusOptions.length);
        if(this.generatedflag === true){
            this.issuedPageList = this.issuedPageList.map(inv => {
                return {
                    ...inv,
                    isEditingStatus: inv.Id === rowId
                };
            });
        } else if(this.totalvalueflag === true){
            this.bulkServices = this.bulkServices.map(inv => {
        ...
        console.log('length', this.statusOptions.length);
    }
    ...
    async saveStatusRow(event) {
        ...
        const row = currentList.find(inv => inv.Id === rowId);
    ...
    //manendra added for sorting
    handleSort(event) { 
    console.log('Table:', event.currentTarget.dataset.table);
    console.log('Field:', event.currentTarget.dataset.field);
        const table = event.currentTarget.dataset.table;
        const field = event.currentTarget.dataset.field;

        if (!table || !field) return;

        let state = this.tableSortState[table];
        ...
        Object.keys(state.icons).forEach(key => {
            state.icons[key] = '';
        });
        ...
        // Refresh tracked object
        this.tableSortState = {
            ...this.tableSortState
        };
        // Reset page based on table
        switch (table) {

            case 'pendingServices':
                console.log('Inside services case');
                this.pageNumber = 1;
                this.paginationHelper();
                break;
    ...
    sortData(data, sortField, sortDirection) {
        ...
        return [...data].sort((a, b) => {
            ...
            if (sortField === 'ServiceDate') {
                const [d1, m1, y1] = String(valueA).split('/');
                const [d2, m2, y2] = String(valueB).split('/');
                valueA = new Date(y1, m1 - 1, d1);
                valueB = new Date(y2, m2 - 1, d2);
