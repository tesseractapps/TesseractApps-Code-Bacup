...
import fetchFacilitiessForDashboard from '@salesforce/apex/HrHomeHandler.fetchFacilitiessForDashboard';
...
export default class TesseractAppsUserAccessLwc extends LightningElement {
    ...
    @track _editButtonModule = false;
    @track _resetPasswordModal = false;
    @track _openFacilityAccess = false;
    ...
    set showModal(value) {
        ...
        if (!this._suppressEmit) {
            this.dispatchEvent(new CustomEvent('subpopup', {
                detail: { slug: value ? 'grant-access' : '' },
                bubbles: true,
                composed: true
            }));
        }
    ...
    get editButtonModule() {
        return this._editButtonModule;
    }
    set editButtonModule(value) {
        this._editButtonModule = value;
        this.notifyStateChange();
    }

    get resetPasswordModal() {
        return this._resetPasswordModal;
    }
    set resetPasswordModal(value) {
        this._resetPasswordModal = value;
        if (!this._suppressEmit) {
            this.dispatchEvent(new CustomEvent('subpopup', {
    ...
    get openFacilityAccess() {
        return this._openFacilityAccess;
    }
    set openFacilityAccess(value) {
        this._openFacilityAccess = value;
        if (!this._suppressEmit) {
            this.dispatchEvent(new CustomEvent('subpopup', {
                detail: { slug: value ? 'facility-access' : '' },
                bubbles: true,
                composed: true
            }));
        }
    ...
    moduleMap = {
        ...
        'ICT Admin' : ['Admin', 'Human Resources',  'ICT Timesheets', 'T sign', 'My Profile','Accounting', 'Repository', 'Access Manager'],
        ...
        'NDIS Participants' :['Participants'],
   ...
   @wire(getUserTypeValuesfromOrg, { selectedroleValue: '$selectedRole', userId: '$userId' })
    ...
    showGrantAccess(event) {
        const userId = event.target?.dataset?.id;
        if (userId) {
            this.loadGrantAccessPopup(userId);
        }
    }    
...
//end
    ...
    filteredData = this.sortData(filteredData);// manendra added for sorting the data in table
    ...
    //                 this.editusertype = this.formData.user.User_Type__c === 'NDIS Participants' 
    //                     ? 'NDIS Participant' 
    //                     : this.formData.user.User_Type__c;
    ...
    handleActionClick(event) {
        ...
        this.loadRecordData(staffId).catch(error => {
            this.showToast('Error', 'Failed to load staff details', 'error');
        });
    }
    ...
    handlesavebutton(event) {
        ...
        // ✅ Map User Type to Role if role is not provided or empty
        let mappedRole = this.editrole;
        if (!mappedRole || mappedRole === '') {
            mappedRole = mapUserTypeToRole(this.editusertype);
            this.editrole = mappedRole;
            console.log('✅ Role mapped from User Type:', mappedRole);
        }

        // ✅ Validate that we have a role
        if (!mappedRole || mappedRole === '') {
            console.error('❌ No role found for User Type:', this.editusertype);
            const evt = new ShowToastEvent({
                title: 'Error',
                message: `No valid role found for User Type: ${this.editusertype}`,
                variant: 'error',
                mode: 'dismissable'
            });
            this.dispatchEvent(evt);
        ...
        .then(result => {
            ...
            if (response.status === 'Success') {
                ...
                this.paginatedData = this.paginatedData.map(user =>
                    user.staffId === updatedId || user.userId === updatedId
                        ? { 
                            ...
                            userRole: userData.role, // Update role
                            userType: userData.userType // Update user type
                        }
                        : user
                );
                ...
                // ⏳ Delay refresh and toast by 2 seconds
                setTimeout(() => {
                    console.log('🔁 Refreshing data after 2 seconds...');
   ...
   async facilityGrantAccess(event) {
        ...
        if (userId) {
         ...
         // this is the code for refreshing the facilities
          try {
            console.log('🔄 Refreshing facility options...');

            const response = await fetchFacilitiessForDashboard();

            console.log('Fresh facility data:', JSON.stringify(response) );
            ...
            if (this.userType === 'NDIS Org Admin' || this.userType === 'ICT Admin' ) {
                ...
                console.log('Fresh Org/ICT facilities:',JSON.stringify(this.finalListFacilities));
                ...
                console.log('Fresh user facilities:',JSON.stringify(this.finalListFacilities));
            }

        } catch (error) {
            console.error(
                'Error refreshing facility options:',
                error
            );
        }

            this.loadFacilityAccessPopup(userId, userRole, userType).catch(error => {
                console.error(error);
            });
        }
    }
   ...
   get placeholderText() {
     ...
     get pageSizeOptions() {
      ...
      @api startEdit() {
          // If no staffId is set, we can't edit, but if it is, set editButtonModule
          if (this.staffId) {
              this.editButtonModule = true;
          }
      }
      ...
      @api get recordUid() {
          ...
          // Find in list
          const rec = this.userData.find(u => u.staffId === this.staffId);
          if (rec && rec.staffUid) return rec.staffUid;
          return this.staffId; // Fallback to 18-char Id
      }

      @api get activePopup() {
          if (this.showModal) return 'grant-access';
          if (this.openFacilityAccess) return 'facility-access';
          if (this.resetPasswordModal) return 'reset-password';
          return '';
      }
      ...
      @api async openByUID(uid, isEdit) {
          ...
          try {
              let matchedUser = this.userData.find(u => u.staffUid === uid || u.staffId === uid);
              let resolvedStaffId = matchedUser ? matchedUser.staffId : uid;
              ...
              if (popup) {
                  await this._openPopupBySlug(popup, resolvedStaffId);
              }
          } catch (e) {
      ...
      async loadRecordData(staffId) {
          ...
          const result = await getStaffDetails({ staffId: this.staffId });
          ...
          const userTypeValues = await getUserTypeValuesfromOrg({
              selectedroleValue: this.formattedUserRole1,
              userId: this.staffId
          });
          this.UserTypeValues = userTypeValues.map(item => ({ label: item, value: item }));
        //   this.usertypename = this.formData.user.User_Type__c === 'NDIS Participants' 
        //       ? 'NDIS Participant' 
        //       : this.formData.user.User_Type__c;
        //   this.editusertype = this.formData.user.User_Type__c === 'NDIS Participants' 
        //       ? 'NDIS Participant' 
        //       : this.formData.user.User_Type__c;
          this.usertypename = this.formData.user.User_Type__c;
          this.editusertype = this.formData.user.User_Type__c;
          
          this.editButtonModule = true;
      }

      loadGrantAccessPopup(userId) {
          this.showModal = false;
          this.modalData = {};
          this.selectedModules = [];
          this.availableModules = [];
      ...
      async loadFacilityAccessPopup(userId, userRole, userType) {
          this.accessUserId = userId;
          this.facilityAccesUserRole = userRole;
          this.facilityAccessUserType = userType;
          this.userFacilities = [];
          this.updatedFacilityValues = [];
          this.selectedFacilities = [];
          this.openFacilityAccess = true;
      ...
      async _openPopupBySlug(slug, staffId) {
          const user = this.userData.find(u => u.staffId === staffId);
          if (!user) return;
          if (slug === 'grant-access') {
              this.loadGrantAccessPopup(user.userId);
          } else if (slug === 'facility-access') {
              await this.loadFacilityAccessPopup(user.userId, user.userRole, user.userType);
