...
import getStaffDetailsByStatus from '@salesforce/apex/StaffStatusController.getStaffDetailsByStatus';
import getSignInStaffDetails from '@salesforce/apex/SignInStatusController.getSignInStaffDetails';
import getLeaveStaffDetails from '@salesforce/apex/PerformanceManagmentChartController.getLeaveStaffDetails';
...
import ENABLE_TESSA from '@salesforce/schema/User.Enable_Tessa__c';
...
import TESSA_LOGO_BETA from '@salesforce/resourceUrl/TessaLogoBeta';

export default class TesseractAppsDashBoardLwc extends LightningElement {
  context = createMessageContext();

  // ===== HASH ROUTER: state (slug form) =====
  // URL model: #/<parent-slug>/<module-slug>/<record-uid>/<tab-slug>/<mode>
  _routeModule = "";        // e.g. "admin/facility"
  _routeSubView = "";       // e.g. "x-Bo00-000D-2DHp/details/edit"
  _routeRestored = false;   // restoreFromUrl() runs once after the menu renders
  _internalHashWrite = false; // suppress our own hashchange echo
  _isRestoring = false;
  // deep-link snapshot captured on the FIRST line of connectedCallback (refresh gotcha)
  _dlModule = "";
  _dlSub = "";
  _dlConsumed = false;
  // module name overrides (name -> slug) used at the hash boundary only
  // _SLUG_OVERRIDES = { Repository: "documents", "Clock In & Clock Out": "clock-in-and-out" };
  _SLUG_OVERRIDES = {
    Repository: "documents",
    "Clock In & Clock Out": "clock-in-and-out",
    "Awards & Recognition": "awards",
    "MP Awards & Recognition": "myawards",
    "My Profile": "myprofile",
    "Training & Evaluation": "training",
    "MP Training & Evaluation": "mytraining",
    "Leave Management": "leave-management",
    "MP Leave Management": "myleave-management",
    "Incident Register": "incident-register",
    "MP Reports": "myreports",
    "ManageForm": "myforms",
    "MP Performance Management": "myperformance-management",
    "Staff Availability": "myavailability"
  };  
  // Option A: shell adapts to each record child's EXISTING @api method (no child edits)
  // _RECORD_ADAPTERS = {
  //   Staff: { selector: "c-staff-data-community", open: (c, uid) => c.openCreateEditStaff(uid) },
  //   "Facility Staff": { selector: "c-staff-data-community", open: (c, uid) => c.openCreateEditStaff(uid) },
  //   Participant: { selector: "c-participant-module-lwc", open: (c, uid) => c.opendetailslwc(uid) },
  //   Participants: { selector: "c-participant-module-lwc", open: (c, uid) => c.opendetailslwc(uid) },
  //   // Admin → Organisation record view (tabs: organisation/settings/onboarding/...)
  //   Admin: {
  //     selector: "c-org-lwc-community",
  //     open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
  //     create: (c) => c.openCreate()
  //   }
  // };

  _RECORD_ADAPTERS = {
    Staff: {
      selector: "c-staff-data-community",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    "Facility Staff": {
      selector: "c-staff-data-community",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
    ...
    "Human Resources Staff": {
      selector: "c-tesseract-apps-create-employee",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit), 
      create: (c, step) => c.openCreate(step)
    },  
    "Human Resources": {
    ...
    Participant: {
      selector: "c-client-data-with-pagination",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    Participants: {
      selector: "c-participant-module-lwc",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    // Admin → Organisation record view (tabs: organisation/settings/onboarding/...)
    ...
    Facility: {
      selector: "c-facility-data-community",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },    
    "Access Manager": {
      selector: "c-tesseract-apps-user-management-lwc",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    "Roster Manager": {
      selector: "c-tesseract-apps-roster-creation",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)  
    },
    Reimbursement: {
      selector: "c-staff-submissions-lwc",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit)
    },    
    "Awards & Recognition": {
      selector: "c-awards-and-recognization-lwc",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    ...
    "Training & Evaluation": {
      selector: "c-tesseract-apps-wyzed-training",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    "MP Training & Evaluation": {
      selector: "c-hr-training",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)  
    },
    "Leave Management": {
      selector: "c-leave-management-lwc",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    "MP Leave Management": {
      selector: "c-staff-leave-management-lwc",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)   
    },
    "Incident Register": {
      selector: "c-form-incident-v2",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit),
      create: (c, step) => c.openCreate(step)
    },
    Repository: {
      selector: "c-repository",
      open: (c, uid, tab, isEdit) => c.openByUID(uid, tab, isEdit)      
    }
  };
  ...
  @track bellNotification =false;
  @track facilityOnchangeTriggered=false;

  //Sai Eswar
  @track showStaffStatusModal = false;
  @track staffStatusLabel = '';
  @track staffStatusList = [];
  @track staffStatusLoading = false;
  ...
  get showNotificationBadges() {
    return this.userType !== 'NDIS Staff' && this.userType !== 'ICT Staff';
  }
  ...
  tessalogobeta = TESSA_LOGO_BETA;


  async connectedCallback() {
     this.facilityOnchangeTriggered=false;

    // ⛳️ Refresh gotcha: snapshot the deep link BEFORE any module auto-opens.
    this._captureInitialDeepLink();
    // Back/Forward + external hash edits → re-restore. (Our own writes are suppressed.)
    this._boundHashChange = this._onHashChange.bind(this);
    window.addEventListener("hashchange", this._boundHashChange);
    // Bubbling record/tab/mode changes from children (fired as `subrouteupdate`).
    this._boundSubroute = this.handleSubrouteUpdate.bind(this);
    this.template.addEventListener("subrouteupdate", this._boundSubroute);

    // Facility auto-switching from child record loads
    this._boundRecordFacilityResolved = this.handleRecordFacilityResolved.bind(this);
    this.template.addEventListener("recordfacilityresolved", this._boundRecordFacilityResolved);    
    ...
    console.log('Is Support supportcoordinatorNDisflag:', this.supportcoordinatorNDisflag);
    console.log('Is Support supportCoordUserModule:', this.supportCoordUserModule);
    console.log('Is Support supportCoordinator:', this.supportCoordinator);
   ...
   // this.subscribeToCreateComoanyMessage();
    ...
    orgDetails().then((response) => {
      ...
      this.Orgabn = response.ABN__c;
       this.Otherleavetype = response.Other_Leave_Type__c;
...
console.log('[Support Coordinator] userData?.User_Type__c =', userData?.User_Type__c);
console.log('[Support Coordinator] userData?.SC__c =', userData?.SC__c);
if (
    userData?.User_Type__c === 'Support Coordinator' ||
    userData?.SC__c === true
) {
    console.log('[Support Coordinator] Auto opening');
    this.supportCoordinatorflag = true;
    // Let handleModuleClick take the normal open path and clear dashboardflag.
...
// 🪄 Trigger both menu and module logic
  ...
  disconnectedCallback() {
    ...
    if (this._boundSubroute) {
      this.template.removeEventListener("subrouteupdate", this._boundSubroute);
    }
    if (this._boundRecordFacilityResolved) {
      this.template.removeEventListener("recordfacilityresolved", this._boundRecordFacilityResolved);
    }    
  }
  // handleOutsideClick = () => {
  ...
  // handleFacilitySelect(event) {facilityLabel
  ...
  handleFacilitySelect(event) {
     this.facilityOnchangeTriggered=true;
    ...
    // Clear sub-route (record details/tabs) to navigate back to the main list/cards view
    if (this._routeSubView) {
        this._routeSubView = "";
        this._syncHash();
    }
  ...
  handleRecordFacilityResolved(event) {
    console.log('📌 [tesseractAppsDashBoardLwc] handleRecordFacilityResolved caught event! detail:', JSON.stringify(event.detail));
    const facilities = event.detail.facilities;
    if (!facilities || facilities.length === 0) return;
    
    // If the record has multiple facilities, open the 0-th facility
    // line changed by praveen along with Riyaz
    let targetFacility
    if(event.detail.isApplyFilter){
      targetFacility = facilities.find(
    ...
    // if (!targetFacility || !targetFacility.id) return;
    // console.log('📌 [tesseractAppsDashBoardLwc] targetFacility resolved to:', JSON.stringify(targetFacility));
    // this.changeFacilityProgrammatically(targetFacility.id);
    if (!targetFacility) return;
    
    let facilityId = targetFacility.id;
    const facilityName = targetFacility.name;
    
    if (!facilityId && facilityName && this.allFacilities) {
      const found = this.allFacilities.find(fac => 
        String(fac.label).trim().toLowerCase() === String(facilityName).trim().toLowerCase()
      );
      if (found) {
        facilityId = found.value;
      }
    }
    
    if (!facilityId) return;
    console.log('📌 [tesseractAppsDashBoardLwc] targetFacility resolved to ID:', facilityId);
    // this.changeFacilityProgrammatically(facilityId);    
    this.changeFacilityProgrammatically(facilityId, true);    
  }

  // changeFacilityProgrammatically(newFacilityId) {
  changeFacilityProgrammatically(newFacilityId, skipRefresh) {
    console.log('📌 [tesseractAppsDashBoardLwc] changeFacilityProgrammatically called with:', newFacilityId);
    console.log('📌 [tesseractAppsDashBoardLwc] allFacilities in shell:', JSON.stringify(this.allFacilities));    
    if (!this.allFacilities || this.allFacilities.length === 0) return;
    
    // Find the facility in allFacilities
    const foundFac = this.allFacilities.find(fac => fac.value === newFacilityId);
    console.log('📌 [tesseractAppsDashBoardLwc] foundFac:', JSON.stringify(foundFac));
    if (!foundFac) return;

    if (this.facilityValue === foundFac.value) {
      console.log('📌 [tesseractAppsDashBoardLwc] Facility already matches, skipping switch');
      return; // already selected
    }
    this.facilityValue = foundFac.value;
    this.facilityLabel = foundFac.label;
    this.facilityPrefreedName = foundFac.facilityPreferredName;
    this.participantPrefreedName = foundFac.participantPreferredName;
    this.staffPrefreedName = foundFac.staffPreferredName;

    localStorage.setItem("defaultFacilityId", this.facilityValue);
    localStorage.setItem("defaultFacilityLabel", this.facilityLabel);
    localStorage.setItem("defaultParticipantPreferredName", this.participantPrefreedName || "Participant");
    localStorage.setItem("defaultFacilityPreferredName", this.facilityPrefreedName || "Facility");
    localStorage.setItem("defaultStaffPreferredName", this.staffPrefreedName || "Staff");

    this.removeLocalNavigationItems();

    // Dispatch toast
    const toastEvent = new ShowToastEvent({
      title: "Facility Changed",
      message: `Facility has been changed to ${this.facilityLabel}`,
      variant: "warning"
    });
  ...
  refreshCurrentModule() {
    ...
    setTimeout(() => {
      const nowActiveFlag = this.getCurrentFlagName();
      if (nowActiveFlag === currentFlag) {      
      ...
      console.log(
        ...
        console.log(
          `[Facility Refresh] ⏭️ Active module changed from ${currentFlag} to ${nowActiveFlag} during refresh. Skipping re-mount.`
        );
      }      
    }, 0);
  }
...
subscribeToTsignMessage() {
  subscribe(this.context, TSIGN_MESSAGE_CHANNEL, (message) => {
    try {
      ...
      const attachmentId = message?.attachmentId; // manendra added for supportcoordinatorAttachments
      const clientEmail = message?.clientEmail;
      ...
      let resolvedServiceId = null,
          ...
          resolvedAttachmentId = null; // manendra added for supportcoordinatorAttachments
      ...
      if (source === "SERVICE") resolvedServiceId = serviceId || legacyRecordId || null;
       ...
       else if (source === "ATTACHMENT") resolvedAttachmentId = attachmentId || null;// manendra added for supportcoordinatorAttachments
      ...
      const hasServiceFlow = awsUrl && resolvedServiceId,
            ...
            hasAttachmentFlow = awsUrl && resolvedAttachmentId; // manendra added for supportcoordinatorAttachments

      if (!hasServiceFlow && !hasQuoteFlow && !hasFormFlow && !hasHrFlow && !hasAttachmentFlow) {
        console.warn("⚠️ Missing/invalid data in received TSign message:", message);
        return;
      }
      ...
      this.attachmentId = hasAttachmentFlow ? resolvedAttachmentId : null; // manendra added for supportcoordinatorAttachments
      ...
      if (this.attachmentId) console.log("📌 Attachment ID received in Dashboard:", this.attachmentId); // manendra added for supportcoordinatorAttachments
      // --- 7) Redirect ---
      this.handleTsignRedirect();
...
@track formId = null;
@track hrId = null;
@track attachmentId = null; // manendra added for supportcoordinatorAttachments
handleTsignRedirect() {
  ...
  console.log( "📌 attachment id stored in Dashboard:", this.attachmentId); // manendra added for supportcoordinatorAttachments
...
resetTsignContext() {
  ...
  this.formId = null;
  this.hrId = null;
this.attachmentId = null; // manendra added for supportcoordinatorAttachments
  ...
  @wire(getRecord, {
    recordId: Id,
    fields: [UserNameFld, UserEmail, UsrRoleName, UserTypeName, ITSupport, UserTypeOfUser, ENABLE_TESSA]
  })
  userDetails({ error, data }) {
    if (data) {
      ...
      this.enableTessa = data.fields.Enable_Tessa__c.value;
  ...
  // =====================================================================
  // HASH ROUTER  —  SEO-friendly, refresh-safe hash routing
  // URL model: #/<parent-slug>/<module-slug>/<record-uid>/<tab-slug>/<mode>
  // Name<->slug translation happens ONLY at the hash boundary.
  // No data-field / data-content / event / prop / Apex is renamed.
  // =====================================================================
  ...
  _nameToSlug(n) {
    return !n ? "" : this._SLUG_OVERRIDES[n] || this.toSlug(n);
  }

  // Record UID prefixes (formula fields). A segment with one of these prefixes is
  // a record id, kept verbatim so it still matches its stored value.
  // _isUidSegment(s) {
  //   return /^(fac|x|org)-/i.test(String(s || ""));
  // }

  //   _isUidSegment(s) {
  //   return /^(fac|x|org|stf|par)-/i.test(String(s || ""));
  // }  
    _isUidSegment(s) {
    // return /^(fac|x|org|stf|par|sft|awd|trn|lev|inc|doc|stfdoc|partdoc|jrn|att|fdb|sch|rsk)-/i.test(String(s || "")) || /^[a-z0-9]{15}$|^[a-z0-9]{18}$/i.test(String(s || ""));
    return /^(fac|x|org|stf|par|sft|awd|trn|lev|inc|doc|stfdoc|partdoc|jrn|att|fdb|sch|rsk|rem)-/i.test(String(s || "")) || /^[a-z0-9]{15}$|^[a-z0-9]{18}$/i.test(String(s || ""));
  
  }    

  _isBoundarySegment(s) {
    return this._isUidSegment(s) || 
           s === "create" || 
           s === "issue-award" ||
           s === "learning" ||
          //  /^(rejected-shifts|invoices|efficiency-report|roster-settings|shift-reports|monthly-calendar|staff-view|participant-view|cashbout|termination-payout|leave-calendar|all-staff|settings|organisation|staff|participant)$/i.test(String(s || ""));
          //  /^(rejected-shifts|invoices|efficiency-report|roster-settings|shift-reports|monthly-calendar|staff-view|participant-view|cashbout|termination-payout|leave-calendar|all-staff|settings|organisation|staff|participant|participants)$/i.test(String(s || ""));
          //  /^(rejected-shifts|invoices|efficiency-report|roster-settings|shift-reports|monthly-calendar|staff-view|participant-view|cashbout|termination-payout|leave-calendar|all-staff|settings|organisation|staff|participant)$/i.test(String(s || ""));
          //  /^(rejected-shifts|invoices|efficiency-report|roster-settings|shift-reports|monthly-calendar|staff-view|participant-view|cashbout|termination-payout|leave-calendar|all-staff|settings|organisation)$/i.test(String(s || ""));
           /^(rejected-shifts|invoices|efficiency-report|roster-settings|shift-reports|monthly-calendar|staff-view|participant-view|cashbout|termination-payout|leave-calendar|all-staff|settings|organisation|approvals|submissions|user-management|staff-management|reset-password|user-report)$/i.test(String(s || ""));
          }  
  // A record UID (known prefix) stays verbatim so it still matches its stored value.
  _segToSlug(s) {
    return this._isUidSegment(s) ? s : this._nameToSlug(s);
  }
  ...
  _pathToSlug(p) {
    return String(p || "")
      .split("/")
      .filter(Boolean)
      .map((s) => this._segToSlug(s))
      .join("/");
  }

  // Build slug -> real data-field map from the live sidebar (keeps names authoritative).
  _buildSlugToFieldMap() {
    const m = {};
    this.template.querySelectorAll("[data-field]").forEach((el) => {
      const d = el.dataset.field;
      if (d) m[this._nameToSlug(d)] = d;
    });
    return m;
  }

  // ---- read the current hash as a decoded slug path ----
  getRouteFromHash() {
  ...
  // ---- write path: split BOTH by "/", encode each segment, write only on change ----
  _encodePath(p) {
    return String(p || "")
      .split("/")
      .filter(Boolean)
  ...
  _composeHash() {
    const path = [this._encodePath(this._routeModule), this._encodePath(this._routeSubView)]
      .filter(Boolean)
      .join("/");
    return path ? "#/" + path : "";
  }
  ...
  _syncHash() {
    const newHash = this._composeHash();
    const current = window.location.hash || "";
    if (current === newHash) return;
    if (!newHash) {
      // clear the hash without adding a history entry / page jump
      history.replaceState(null, "", window.location.pathname + window.location.search);
      return;
    }
    this._internalHashWrite = true; // suppress our own hashchange echo
    window.location.hash = newHash; // real navigation → Back/Forward works
  }

  _replaceHashCanonical() {
    const newHash = this._composeHash();
    const current = window.location.hash || "";
    if (current === newHash) return;
    try {
      // Hash-relative form is the most compatible in Locker/LWS communities.
      ...
      // Fallback: guarantee the URL updates (adds a history entry, but stays correct).
      this._internalHashWrite = true;
      window.location.hash = newHash;
    }
    // Safety net: if replaceState silently no-op'd, force the hash to match.
  ...
  _onHashChange() {
    // ignore the echo from our own window.location.hash writes
    if (this._internalHashWrite) {
      this._internalHashWrite = false;
      return;
    }
    this.restoreFromUrl();
  }

  // ---- module click → module path in slug form; reset the sub-view ----
  updateUrlForModule(field, parent) {
    if (this._isRestoring) {
      return;
    }    
    // top-level items may carry only data-content
    if (!field && parent) {
      field = parent;
      parent = null;
    }
    const sameLevel =
      !parent || String(parent).toLowerCase() === String(field || "").toLowerCase();
    const modulePath = sameLevel
      ? this._nameToSlug(field)
      : this._nameToSlug(parent) + "/" + this._nameToSlug(field);

    // ⛳️ Refresh gotcha: the target module auto-opens BEFORE restore and would
    // wipe the record from the hash. Preserve the captured deep link until the
    // target module actually opens.
    if (!this._dlConsumed && this._dlSub) {
      const onPath =
        modulePath === this._dlModule || this._dlModule.startsWith(modulePath + "/");
      if (onPath) {
        this._routeModule = this._dlModule;
        this._routeSubView = this._dlSub;
        if (modulePath === this._dlModule) this._dlConsumed = true;
        this._syncHash();
        return;
      }
      this._dlConsumed = true;
    }

    this._routeModule = modulePath;
    this._routeSubView = ""; // reset the sub-view on a fresh module click
    this._syncHash();
  }

  // ---- record / tab / mode change (bubbling `subrouteupdate` from a child) ----
  // `replace` = update the URL in place (no new history entry) — used for the
  // default record view a module auto-opens, so Back skips straight to the module.
  updateUrlForSubView(subView, replace) {
    this._routeSubView = this._pathToSlug(subView);
    if (replace) this._replaceHashCanonical();
    else this._syncHash();
  }

  handleSubrouteUpdate(event) {
    const d = event && event.detail;
    const subView = typeof d === "string" ? d : d && (d.subView || d.path || "");
    const replace = !!(d && d.replace);
    this.updateUrlForSubView(subView || "", replace);
  }

  // ---- snapshot the deep link on the FIRST line of connectedCallback ----
  _captureInitialDeepLink() {
    const segs = this.getRouteFromHash()
      .split("/")
      .filter(Boolean)
      .map((s) => this._segToSlug(s));
    // const d = segs.findIndex((s) => this._isUidSegment(s) || s === "create");
    const d = segs.findIndex((s) => this._isBoundarySegment(s));    
    if (d >= 0) {
      this._dlModule = segs.slice(0, d).join("/");
      this._dlSub = segs.slice(d).join("/");
    } else {
      this._dlModule = segs.join("/");
      this._dlSub = "";
    }
  }

  // ---- replay a sidebar click (Option A) ----
  // parentField: real data-field of the top-level parent (null for a top-level module)
  // moduleField: real data-field of the (sub)module, if already resolvable from the DOM
  // moduleSlug : slug of the (sub)module, used to find its subcat AFTER the parent expands
  _replaySidebarClick(parentField, moduleField, moduleSlug) {
    console.log("🚦 [Routing Debug] _replaySidebarClick parentField:", parentField, "moduleField:", moduleField, "moduleSlug:", moduleSlug);
    // if (parentField === "Human Resources" && moduleSlug === "staff") {
    //   this.simulateClick(`.menu-link[data-field="Human Resources"]`);
    //   return;
    // }    
    const sameAsParent =
      parentField &&
      String(parentField).toLowerCase() === String(moduleField || moduleSlug).toLowerCase();
    console.log("🚦 [Routing Debug] _replaySidebarClick sameAsParent:", sameAsParent);
    if (parentField && !sameAsParent) {
      // submodule: expand the parent, then click the submodule (its subcat renders late)
      // this.simulateClick(`.menu-link[data-field="${parentField}"]`);

      // submodule: expand the parent directly instead of simulating click (avoids race conditions)
      const parentVisibilityFlags = {
        "Admin": "isAdminMenuVisible",
        "Human Resources": "isHrMenuVisible",
        "Roster Manager": "isRosterMenuVisible",
        "Payroll": "isPayrollMenuVisible",
        "Accounting": "isAccountingMenuVisible",
        "My Profile": "isMyProfileMenuVisible",
        "Support Coordinator": "supportCoordinatormoduleflag"
      };
      const flagName = parentVisibilityFlags[parentField];
      if (flagName) {
        this[flagName] = true;
      }
      
      // Highlight the parent menu link in the sidebar
      try {
        const parentLink = this.template.querySelector(`.menu-link[data-field="${parentField}"]`);
        if (parentLink) {
          const allMenuLinks = this.template.querySelectorAll(".menu-link");
          allMenuLinks.forEach((link) => link.classList.remove("active"));
          parentLink.classList.add("active");
          this.activeMenu = parentField;
        }
      } catch (e) {
        console.warn(e);
      }
      ...
      this._clickSubmoduleWhenReady(moduleField, moduleSlug);
      return;
    }
    // top-level module — its menu-link is already in the DOM
    if (moduleField && this.template.querySelector(`.menu-link[data-field="${moduleField}"]`)) {
      this.simulateClick(`.menu-link[data-field="${moduleField}"]`);
    } else if (moduleSlug) {
      this._clickTopLevelBySlug(moduleSlug);
    }
  }

  // A submodule's subcat link only exists after its parent menu is expanded — poll for it.
  _clickSubmoduleWhenReady(moduleField, moduleSlug, tries) {
    tries = tries || 0;
    const links = this.template.querySelectorAll(".subcat a[data-field]");
    console.log("🚦 [Routing Debug] _clickSubmoduleWhenReady - tries:", tries, "links found:", links.length, "looking for moduleField:", moduleField, "moduleSlug:", moduleSlug);
    for (let i = 0; i < links.length; i++) {
      const df = links[i].dataset.field;
      console.log(`  -> Checked link: field="${df}", slug="${this._nameToSlug(df)}"`);
      if ((moduleField && df === moduleField) || this._nameToSlug(df) === moduleSlug) {
        console.log("  ✅ Matching link found! Clicking it.");
        links[i].click(); // real click → handleModuleClick
        return;
      }
    }
    if (tries < 25) {
      setTimeout(() => this._clickSubmoduleWhenReady(moduleField, moduleSlug, tries + 1), 80);
    } else {
      console.warn("  ❌ Timeout reached: could not find submodule link in DOM.");      
    }
  }

  // Click a top-level menu-link found by slug (fallback when the real data-field is unknown).
  _clickTopLevelBySlug(moduleSlug) {
    const links = this.template.querySelectorAll(".menu-link[data-field]");
    for (let i = 0; i < links.length; i++) {
      if (this._nameToSlug(links[i].dataset.field) === moduleSlug) {
        links[i].click();
        return;
      }
    }
  ...
  // Is the sidebar link for the module named in the current hash rendered yet?
  // Menu items are gated behind per-module flags set only after Apex resolves, so
  // the initial restore must wait for the TARGET link — not just any menu-link —
  // or it replays a click on an element that does not exist yet (→ Dashboard).
  _hashTargetModuleReady() {
    const raw = this.getRouteFromHash();
    if (!raw) return true; // no route → nothing to wait for
    const segs = raw.split("/").filter(Boolean).map((s) => this._segToSlug(s));
    const boundary = segs.findIndex(
      // (s) => this._isUidSegment(s) || s === "create"
      (s) => this._isBoundarySegment(s)      
    );
    const modulePathSegs = boundary >= 0 ? segs.slice(0, boundary) : segs.slice();
    // Wait for the TOP-LEVEL (first) module link — it is a real menu-link gated only by
    // its module flag. A submodule's subcat link does NOT exist until its parent is
    // clicked, so waiting for it would deadlock (→ never restores, drops to Dashboard).
    const topSlug = modulePathSegs[0];
    if (!topSlug) return true;
    const links = this.template.querySelectorAll(".menu-link[data-field]");
    console.log("🚦 [Routing Debug] _hashTargetModuleReady - raw route:", raw);
    console.log("🚦 [Routing Debug] _hashTargetModuleReady - topSlug:", topSlug, "boundary index:", boundary);
    console.log("🚦 [Routing Debug] _hashTargetModuleReady - links count in DOM:", links.length);    
    for (let i = 0; i < links.length; i++) {
      // if (this._nameToSlug(links[i].dataset.field) === topSlug) return true;
      const df = links[i].dataset.field;
      console.log(`  -> Sidebar link: field="${df}", slug="${this._nameToSlug(df)}"`);
      if (this._nameToSlug(df) === topSlug) {
        console.log("  ✅ Top-level link is ready in DOM!");
        return true;
      }      
    }
    return false;
  ...
  // ---- read/restore: replay the hash, open records, canonicalize legacy URLs ----
  restoreFromUrl() {
    const raw = this.getRouteFromHash();
    if (!raw) return;
    ...
    try {
    // normalize each segment (also upgrades legacy %20 / mixed-case names to slugs)
    const segs = raw.split("/").filter(Boolean).map((s) => this._segToSlug(s));
    const map = this._buildSlugToFieldMap();

    console.log("🚦 [Routing Debug] restoreFromUrl starting. raw:", raw);
    console.log("🚦 [Routing Debug] restoreFromUrl normalized segs:", JSON.stringify(segs));
    console.log("🚦 [Routing Debug] restoreFromUrl buildSlugToFieldMap output:", JSON.stringify(map));
    // The record-uid / "create" segment is the module↔detail boundary. Everything
    // BEFORE it is the module PATH (parent[/submodule]); everything from it on is
    // record detail (tab/mode). Parse STRUCTURALLY — a submodule's subcat link isn't
    // in the DOM until its parent is clicked, so we can't rely on the DOM map here.
    const boundary = segs.findIndex(
      // (s) => this._isUidSegment(s) || s === "create"
      (s) => this._isBoundarySegment(s)      
    );
    console.log("🚦 [Routing Debug] restoreFromUrl boundary index resolved:", boundary);
    
    let modulePathSegs = boundary >= 0 ? segs.slice(0, boundary) : segs.slice();
    let detail = boundary >= 0 ? segs.slice(boundary) : [];
    if (segs[0] === "documents") {
      modulePathSegs = ["documents"];
      detail = segs.slice(1);
    }
    console.log("🚦 [Routing Debug] restoreFromUrl modulePathSegs:", JSON.stringify(modulePathSegs), "detail:", JSON.stringify(detail));
    if (!modulePathSegs.length) return; // nothing recognizable to restore

    const moduleSlug = modulePathSegs[modulePathSegs.length - 1]; // deepest (sub)module
    const parentSlug =
      modulePathSegs.length > 1 ? modulePathSegs[modulePathSegs.length - 2] : null;
    // Parent is a top-level menu-link (rendered now). The submodule's subcat may only
    // have its slug — its real data-field is resolved after the parent expands.
    // const parentField = parentSlug ? map[parentSlug] || null : null;
    let parentField = parentSlug ? map[parentSlug] || null : null;
    if (!parentField && (parentSlug === "myprofile" || parentSlug === "my-profile")) {
      parentField = "My Profile";
    }    
    let moduleField = map[moduleSlug] || null;
    if (parentSlug === "documents") {
      moduleField = "Repository";
    }
    if (!moduleField) {
      if (moduleSlug === "staff") {
        if (parentSlug === "human-resources") {
          moduleField = "Human Resources Staff";
        } else {

        moduleField = parentSlug === "facility-admin" ? "Facility Staff" : "Staff";
        }
        
      } else if (moduleSlug === "facility-staff") {
        moduleField = "Facility Staff";
      } else if (moduleSlug === "facility") {
        moduleField = "Facility";
      // } else if (moduleSlug === "participant" || moduleSlug === "participants") {
      } else if (moduleSlug === "participants") {
        moduleField = "Participants";
      } else if (moduleSlug === "participant") {      
        moduleField = "Participant";
      } else if (moduleSlug === "access-manager") {
        moduleField = "Access Manager";   
      } else if (moduleSlug === "roster-manager" || moduleSlug === "roster" || moduleSlug === "rosters") {
        moduleField = "Roster Manager";   
      } else if (moduleSlug === "awards" || moduleSlug === "awards-and-recognition") {
        moduleField = "Awards & Recognition";
      } else if (moduleSlug === "myawards" || moduleSlug === "mp-awards-and-recognition") {
        moduleField = "MP Awards & Recognition";  
      } else if (moduleSlug === "training" || moduleSlug === "learning") {
        moduleField = "Training & Evaluation";
      } else if (moduleSlug === "mytraining") {
        moduleField = "MP Training & Evaluation";    
      // } else if (moduleSlug === "leave-management" || moduleSlug === "cashbout" || moduleSlug === "termination-payout" || moduleSlug === "leave-calendar" || moduleSlug === "all-staff") {
       
      } else if (moduleSlug === "leave-management" || moduleSlug === "myleave-management" || moduleSlug === "cashbout" || moduleSlug === "termination-payout" || moduleSlug === "leave-calendar" || moduleSlug === "all-staff") {
      if (parentSlug === "myprofile" || parentSlug === "my-profile") {
          moduleField = "MP Leave Management";
        } else {
          moduleField = "Leave Management";
        }  
      } else if (moduleSlug === "myreports" || moduleSlug === "mpreports") {
        moduleField = "MP Reports";
      } else if (moduleSlug === "myforms" || moduleSlug === "manageform" || moduleSlug === "manage-form") {
        moduleField = "ManageForm";
      } else if (moduleSlug === "myperformance-management" || moduleSlug === "mpperformance-management") {
        moduleField = "MP Performance Management";
      } else if (moduleSlug === "myavailability" || moduleSlug === "staffavailability") {
        moduleField = "Staff Availability";        
      } else if (moduleSlug === "incident-register") {
        moduleField = "Incident Register";
      } else if (moduleSlug === "reimbursement" || moduleSlug === "reimbursements") {
        moduleField = "Reimbursement";        
      }
    }
    // Re-arm the deep-link guard so the replayed module click keeps the record segment
    // (needed on Back/Forward after the initial snapshot is consumed).
    if (detail.length) {
      this._dlModule = modulePathSegs.join("/");
      this._dlSub = detail.join("/");
      this._dlConsumed = false;
    }

    // replay: click the top-level parent, then the submodule (or the module directly)
    this._replaySidebarClick(parentField, moduleField, moduleSlug);

    // record detail? drive the child's existing @api method (Option A)
    const uid = detail[0] || "";
    // const isCreate = uid === "create";
    const isCreate = uid === "create" || uid === "issue-award" || uid === "learning" || uid === "cashbout" || uid === "termination-payout" || uid === "leave-calendar" || uid === "all-staff";
    const adapter = moduleField ? this._RECORD_ADAPTERS[moduleField] : null;
    let restoreTimeout = 500;
    if (adapter && (uid || isCreate)) {
      const tab = detail.slice(1).join('/');
      const isEdit = (detail[2] || "") === "edit";
      // wait for the module's child to render after the replayed click
      setTimeout(() => {
        try {
        const cmp = this.template.querySelector(adapter.selector);
        if (!cmp) return;
        if (isCreate && typeof adapter.create === "function") {
          const step = detail[1] || "";
          adapter.create(cmp, step);
        } else if (!isCreate && typeof adapter.open === "function") {
          adapter.open(cmp, uid, tab, isEdit);
          }
        } finally {
    ...
    // adopt canonical slug route state, then upgrade the URL in place (no history entry)
    this._routeModule = modulePathSegs.join("/");
    this._routeSubView = detail.join("/");
    this._replaceHashCanonical();
    // } finally {
  ...
  setModuleFlags() {
    ...
    this.smartReportsFlag =
      this.modules.includes("Reports") &&
      !this.isSupportCoordinator &&
      !this.supportcoordinatorNDisflag;
  ...
  restoreActiveModule() {
    // 🔗 URL is the source of truth for navigation. When the hash already carries a
    // route, restoreFromUrl() drives it — SKIP the sessionStorage auto-open below so
    // the cache and the router don't fight on load. (Plain "/s/" with no hash still
    // falls through to the cached last-module restore.)
    if (this.getRouteFromHash && this.getRouteFromHash()) {
      console.log("[restoreActiveModule] URL route present → skipping sessionStorage restore");
      return;
    }
  ...
  // Remove the "active" highlight from any submenu item. Called when a parent
  // module is clicked so a previously-selected sub-tab (e.g. Facility/Staff under
  // Admin) does not stay highlighted after returning to the parent module.
  clearActiveSubmenus() {
    const activeSubmenuLinks = this.template.querySelectorAll(".subcat a.active");
    activeSubmenuLinks.forEach((link) => link.classList.remove("active"));
    this.activeSubmenu = "";
  }
  ...
  handleMenuClick(event) {
    ...
    // A parent module was clicked → drop any active sub-tab highlight.
    this.clearActiveSubmenus();
  ...
  if (!this.openFromTask) {
    ...
    if (subModule !== "") {
      ...
      if (lastClickedModule === mainModule) {
        ...
        if (savedSub && savedSub !== mainModule && !this._isRestoring) {
          console.log(
    ...
    // ===== HASH ROUTER: reflect this module in the URL (slug boundary) =====
    // field = data-field (module), parent = data-content (parent module)
    this.updateUrlForModule(selectedField, selectedContent);
    ...
    if (moduleName === "Admin" && this.isAdminMenuVisible) {
        ...
        // Returning to the Admin parent → clear the active sub-tab highlight.
        this.clearActiveSubmenus();
...
handleRedirectMessage(message) {
    ...
    // 🔹 Normalize casing
    //Sai Eswar
    if (message?.target === 'StaffStatusDetail') {
        this.openStaffStatusModal(message.data);
        return; // ← STOP generic flow, same as Funds Tracker
    }
    target = target
        .toLowerCase()
        .split(" ")
  ...
  get sidebarClass() {
    let cls = this.isSidebarExpanded
...
renderedCallback() {
  ...
  // ===== HASH ROUTER: replay the URL once, after the TARGET module link renders =====
  // (module links are gated behind flags set only after Apex resolves — waiting for
  // just any .menu-link fires too early and the deep link drops to Dashboard.)
  if (!this._routeRestored && this._hashTargetModuleReady()) {
    this._routeRestored = true;
    this.restoreFromUrl();
  }
  ...
  // Set tooltips for sidebar links dynamically based on their text content
  try {
    const links = this.template.querySelectorAll('.sidebar a, .sidebar .menu-link');
    ...
    console.error('Error setting sidebar tooltips:', err);
  }
...
handleReleaseUpdateClose() {
  ...
  getUserNotifications() {
    ...
    fetchUserNotifications()
      .then((result) => {
        ...
        this.allNotifications = result.map((notif, index) => {
          const createdDate = new Date(notif.CreatedDate);
          return {
            value: notif.Id,
            label: notif.Subject__c,
  ...
  handleNavigation(event) {
   ...
   if (
    noNavigationNotifications.includes(notification) || notification.includes("Fund Usage Alert")) {
    ...
    case "Congratulations on receiving a award.":
      ...
      case "Your document has been rejected":
        ...
        break;
      case "New Document is uploaded.":
      case "Document is updated":
        this.isTaskNavigation = true;
        this.staffidflag = "Staff Document Expiry";
        this.isAdminMenuVisible = true;
        console.log("whatid" + whatId);
      ...
      case "New Task Assigned":
        ...
        this.simulateClick('.menu-link[data-field="Participants"]');
        break;
      case "Participant Document Expiry in 2 Days":
        this.participantFlag = true;
        this.navigatedParticipantId = clientId || '';
        console.log("notificationId clicked for  this.navigatedParticipantId :",  this.navigatedParticipantId);
        this.simulateClick('.menu-link[data-field="Participants"]');
  ...
  handleConfirm() {
    ...
    const storedFacilityIdBeforeLogout = localStorage.getItem("defaultFacilityId");
    ...
    if (storedFacilityIdBeforeLogout) { 
        localStorage.setItem('defaultFacilityId', storedFacilityIdBeforeLogout);
    }
...
async refreshFacilityDropdown() {
    try {
        ...
        if (userType === 'NDIS Org Admin' || userType === 'ICT Admin' || userType === 'Support Coordinator' ) {
            ...
            /* Update facility label from the selected facility value */
           
            this.validateCache();
        }

         const selectedFacility = this.finalListFacilities.find(
                facility => facility.value === this.facilityValue
            );

            if (selectedFacility) {
                this.facilityLabel = selectedFacility.label;
            } 
...
openTessaAI() {
    this.showTessaAI = true;
}

handleCloseTessaAI(event) {
    console.log('=== Dashboard Parent ===');
    console.log('Received close event:', event);
    console.log('showTessaAI before:', this.showTessaAI);

    this.showTessaAI = false;

    console.log('showTessaAI after:', this.showTessaAI);
}
...
get rosterPrintClass() {
 
    return this.isPrintMode
        ? 'roster-print-wrapper print-active'
...
handleNotificationSearch(event) {
  // Keep the raw typed value (spaces intact) for the input's display binding.
  const rawValue = event.target.value || '';
  this.notificationSearchTerm = rawValue;

  // Only trim/lowercase a local copy for matching purposes.
  const term = rawValue.toLowerCase().trim();

  if (!this.allNotifications) {
    this.allNotifications = this.filterednotification || [];
  }
  ...
  this.filterednotification = this.allNotifications.filter((n) => {
...
openStaffStatusModal(data) {
    this.staffStatusLabel = data.statusLabel;
    this.staffStatusKey = data.statusKey;
    this.staffStatusLoading = true;
    this.isLoading = true;

    // Reset search
    this.staffSearchKey = '';
    this.staffStatusList = [];
    this.allStaffStatusList = [];

    const isSignInStatus = ['Completed', 'InProgress', 'Accepted'].includes(data.statusKey);
    const isLeaveStatus = ['Requested', 'Approved', 'Rejected'].includes(data.statusKey);

    let fetchPromise;
    if (isSignInStatus) {
        fetchPromise = getSignInStaffDetails({ status: data.statusKey, facilityId: data.facilityId });
    } else if (isLeaveStatus) {
        fetchPromise = getLeaveStaffDetails({ userId: Id, status: data.statusKey, facilityId: data.facilityId })
    } else {
        fetchPromise = getStaffDetailsByStatus({ status: data.statusKey, facilityId: data.facilityId });
    }

    fetchPromise
        .then((result) => {
            if (isSignInStatus || isLeaveStatus) {
                this.staffStatusList = result;
                this.allStaffStatusList = [...result];
                return;
            }
            const formattedResult = result.map(staff => ({
                ...
                documents: (staff.documents || []).map(doc => ({
                    ...
                    expiryDate: doc.Expiry_Date__c
                        ? doc.Expiry_Date__c.split('-').reverse().join('/')
                        : ''
                }))
            }));
            this.staffStatusList = formattedResult;
            this.allStaffStatusList = [...formattedResult];
        })
        .catch((error) => console.error('Error:', error))
        .finally(() => { 
                        this.showStaffStatusModal = true;
                        this.staffStatusLoading = false;
                        this.isLoading = false; 
                      });
}

get isLeaveStatus() {
    return ['Requested', 'Approved', 'Rejected'].includes(this.staffStatusKey);
}

get isDocsExpiryStatus() {
    return this.staffStatusKey === 'visaExpiry'; 
}

closeStaffStatusModal() {
    this.showStaffStatusModal = false;
    this.staffSearchKey = '';
    this.staffStatusList = [...this.allStaffStatusList];
}

// handleStaffSearch(event) {
//     this.staffSearchKey = event.target.value;

//     const searchKey = this.staffSearchKey.trim().toLowerCase();

//     if (!searchKey) {
//         // Restore original list with all documents
//         this.staffStatusList = this.allStaffStatusList.map(staff => ({
//             ...staff,
...
//             // Search by Role
//             const matchesRole = (staff.roleNames || '')
//                 .toLowerCase()
//                 .includes(searchKey);

//             // Search by Shift Type
//             const matchesShiftType = (staff.shiftType || '')
//                 .toLowerCase()
//                 .includes(searchKey);

//             // Search by Contact Number
//             const matchesContactNumber = (staff.contactNumber || '')
//                 .toLowerCase()
//                 .includes(searchKey);

//             // Filter matching documents
//             const matchingDocuments = (staff.documents || []).filter(doc =>
//                 (doc.Document_Type__c || '')
//                     .toLowerCase()
//                     .includes(searchKey)
//             );

//             // If searching by staff details, keep all documents
//             if (matchesName || matchesEmail || matchesRole || matchesShiftType || matchesContactNumber) {
//                 return {
//                     ...staff,
//                     documents: [...(staff.documents || [])]
//                 };
...
//             // If searching by document type, keep only matching documents
...
handleStaffSearch(event) {
    ...
    if (!searchKey) {
        this.staffStatusList = JSON.parse(
            JSON.stringify(this.allStaffStatusList)
        );
        return;
    }

    // ===========================
    // Documents Expiry Search
    // ===========================
    if (this.isDocsExpiryStatus) {
        ...
        this.staffStatusList = this.allStaffStatusList
            .map(staff => {
                ...
                const matchesContact = (staff.contactNumber || '')
                    .toLowerCase()
                    .includes(searchKey);
                ...
                // Staff fields matched -> keep all documents
                if (
                    matchesName ||
                    matchesEmail ||
                    matchesRole ||
    ...
    // ===========================
    // All Other Statuses
    // (In Progress, Accepted, Active, Leave, etc.)
    // ===========================
    this.staffStatusList = this.allStaffStatusList.filter(staff => {
        ...
        const matchesLeaveType = (staff.leaveType || '')
            .toLowerCase()
            .includes(searchKey);
...
get isSignInStatus() {
    return ['Completed', 'InProgress', 'Accepted'].includes(this.staffStatusKey);
}

get isAcceptedStatus() {
    return this.staffStatusKey === 'Accepted';
}

get signInTableClass() {
    return this.isSignInStatus && this.isAcceptedStatus 
        ? 'staff-table signin-3col' 
        : 'staff-table';
}
