...
import SC__c from '@salesforce/schema/User.SC__c';
...
import getStaffNicknameByEmail from '@salesforce/apex/RepositorySecurityHandler.getStaffNicknameByEmail';
import archiveRepositoryAttachment from '@salesforce/apex/RepositoryAttachmentController.archiveRepositoryAttachment';
import restoreRepositoryAttachment from '@salesforce/apex/RepositoryAttachmentController.restoreRepositoryAttachment';
import getArchivedRepositoryAttachments from '@salesforce/apex/RepositoryAttachmentController.getArchivedRepositoryAttachments';
...
export default class Repository extends NavigationMixin(LightningElement) {
    ...
    @wire(getRecord, { recordId: Id, fields: [UserNameFld, UsrRoleName,UserEmail, USER_TYPE, SC__c] })
    userDetails(result) {
  ...
  @track storedFacilityId;
    ...
    _syncRecordRoute() {
        ...
        this._syncRouteTimeout = setTimeout(() => {
            ...
            if (!route || route === this._lastSubRoute) return;
            this._lastSubRoute = route;
            this.dispatchEvent(new CustomEvent('subrouteupdate', {
                detail: { subView: route, replace: false },
                bubbles: true,
                composed: true
            }));
        }, 50);
    ...
    _composeSubRoute() {
        if (this.OrgFlag) {
            if (this.archiveModal) {
                return 'organisation/archived-document';
            }
            if (this.isOpenModal) {
                if (this.isEdit && this.recordId) {
                    let doc = this.tempConList.find(d => d.Id === this.recordId);
                    return doc && doc.Repository_UID__c ? `${doc.Repository_UID__c}/edit` : 'organisation';
                }
                return 'organisation/upload-document';
            }
            if (this.DeleteFlag && this.recordId) {
                let doc = this.tempConList.find(d => d.Id === this.recordId);
                return doc && doc.Repository_UID__c ? `${doc.Repository_UID__c}/delete-document` : 'organisation';
            }
            if (this.isOpenPermission && this.selectedRowId) {
                let doc = this.tempConList.find(d => d.Id === this.selectedRowId);
                return doc && doc.Repository_UID__c ? `${doc.Repository_UID__c}/permissions` : 'organisation';
            }
            if (this.Consentflag && this.selectedRowId) {
                let doc = this.tempConList.find(d => d.Id === this.selectedRowId);
                return doc && doc.Repository_UID__c ? `${doc.Repository_UID__c}/consent` : 'organisation';
            }
            if (this.isModalOpen && this.selectedRowId) {
                let doc = this.tempConList.find(d => d.Id === this.selectedRowId);
                return doc && doc.Repository_UID__c ? `${doc.Repository_UID__c}/preview` : 'organisation';
            }
            if (this.isRenameModalOpen) {
                return 'organisation/rename-folder';
            }
            if (this.isDeleteModalOpen) {
                return 'organisation/delete-folder';
            }
            return 'organisation';
        }
        if (this.staffFlag) {
            return this._lastStaffSubRoute || 'staff';
        }
        if (this.ParticipantFlag) {
            return this._lastParticipantSubRoute || 'participant';
        }
        return '';
    }

    @api async openByUID(uid, tab, isEdit, mode) {
        this._suppressEmit = true;
        try {
            console.log('Repository openByUID:', uid, tab, isEdit, mode);
            if (uid === 'organisation' || uid === 'org') {
                this.OrgFlag = true;
                ...
                if (tab === 'archived-document') {
                    this.archiveModal = true;
                } else if (tab === 'upload-document') {
                    this.isOpenModal = true;
                    this.isEdit = false;
                    this.recordId = '';
                } else if (tab === 'rename-folder') {
                    this.isRenameModalOpen = true;
                } else if (tab === 'delete-folder') {
                ...
                let realMode = mode || '';
                if (realTab.includes('/')) {
                    const tabParts = realTab.split('/');
                    realTab = tabParts[0];
                    realMode = tabParts[1];
                }
                
                let subPath = realTab;
                if (subPath && realMode) subPath += `/${realMode}`;
                this._lastStaffSubRoute = realTab ? `staff/${subPath}` : 'staff';                
                setTimeout(() => {
                    const cmp = this.template.querySelector('c-staff-documents-lwc');
                    if (cmp) {
                        cmp.openByUID(realTab, realMode, isEdit);
                    }
                }, 100);
            } else if (uid === 'participant') {
                this.OrgFlag = false;
                this.staffFlag = false;
                ...
                let subPath = realTab;
                if (subPath && realMode) subPath += `/${realMode}`;
                this._lastParticipantSubRoute = realTab ? `participant/${subPath}` : 'participant';                
                setTimeout(() => {
                    const cmp = this.template.querySelector('c-participant-documents-lwc');
                    if (cmp) {
                        cmp.openByUID(realTab, realMode, isEdit);
                    }
                }, 100);
            } else if (uid && (uid.startsWith('doc-') || uid.startsWith('stfdoc-') || uid.startsWith('partdoc-'))) {
                if (uid.startsWith('stfdoc-')) {
                    this.OrgFlag = false;
                    this.staffFlag = true;
                    this.ParticipantFlag = false;
                    this.isHome = true;
                    this.closeModalsAndPanels();
                    this._lastStaffSubRoute = tab ? `staff/${uid}/${tab}` : `staff/${uid}`;
                    
                    setTimeout(() => {
                        const cmp = this.template.querySelector('c-staff-documents-lwc');
                        if (cmp) {
                            cmp.openByUID(uid, tab, isEdit);
                        }
                    }, 100);
                } else if (uid.startsWith('partdoc-')) {
                    this.OrgFlag = false;
                    this.staffFlag = false;
                    this.ParticipantFlag = true;
                    this.isHome = true;
                    this.closeModalsAndPanels();
                    this._lastParticipantSubRoute = tab ? `participant/${uid}/${tab}` : `participant/${uid}`;
                ...
                } else if (uid.startsWith('doc-')) {
                    this.OrgFlag = true;
                    this.staffFlag = false;
                    this.ParticipantFlag = false;
                    
                    if (!this.tempConList || this.tempConList.length === 0) {
                        this._pendingRoute = { uid, tab, isEdit };
                        return;
                    }
                    
                    let doc = this.tempConList.find(d => d.Repository_UID__c === uid);
                    if (!doc && this.records) {
                        doc = this.records.find(d => d.Repository_UID__c === uid);
                    }
                    
                    if (doc) {
                        this.recordId = doc.Id;
                        this.selectedRowId = doc.Id;
                        // this.isHome = false;
                        ...
                        if (tab === 'edit' || isEdit) {
                            ...
                            this.isEdit = true;
                            this.isOpenModal = true;
                            this.selectedValue = doc.Document_Type__c || '';
                            if (doc.Repository_Attachments__r && doc.Repository_Attachments__r.length > 0) {
                                this.attachmentid = doc.Repository_Attachments__r[0].Id;
                                this.key = doc.Repository_Attachments__r[0].key__c;
                            }
                        } else if (tab === 'delete-document') {
                            this.isHome = true;
                            ...
                            this.loadConsent();
                        } else if (tab === 'preview' || !tab) {
                            this.isHome = false;
                            this.isModalOpen = true;
                            if (doc.Repository_Attachments__r && doc.Repository_Attachments__r.length > 0) {
                                this.attachmentid = doc.Repository_Attachments__r[0].Id;
                                this.currentUrl = doc.Repository_Attachments__r[0].Awsurl__c;
                            }
                        }
                    } else {
                        console.warn('Repository document not found for UID:', uid);
                    }
                }
            }
    ...
    _restoreFromUrlHash() {
        ...
        const uid = parts[docIdx + 1] || 'organisation';
        const tab = parts[docIdx + 2] || '';
        const modes = parts.slice(docIdx + 3);
        const mode = modes[0] || '';
        const isEdit = modes.includes('edit');
        
        this.openByUID(uid, tab, isEdit, mode);
        
        if (parts.length <= docIdx + 1) {
            setTimeout(() => {
                this.dispatchEvent(new CustomEvent('subrouteupdate', {
                    detail: { subView: uid, replace: true },
                    bubbles: true,
                    composed: true
                }));
            }, 100);
    ...
    handleSubrouteUpdate(event) {
        ...
        if (this.staffFlag) {
            this._lastStaffSubRoute = subView;
        } else if (this.ParticipantFlag) {
            this._lastParticipantSubRoute = subView;
        }
        ...
        fetchRepositoryData({ organisationId: this.orgId})  // facilityId: this.storedFacilityId
            .then((data) => {
                ...
                if (data) {
                    ...
                    this.tempConList = data.map((record) => {
                        ...
                        return {
                            ...
                            Repository_UID__c: record?.Repository_UID__c || '',
                            HasArchivedAttachments: record?.HasArchivedAttachments || false,
                ...
                if (this._pendingRoute) {
                    const { uid, tab, isEdit } = this._pendingRoute;
    ...
    @track isDocTypeView = true;
     toggleSection(event) {
        ...
        this.repoDocTypeList = this.repoDocTypeList.map(group => ({
            ...group,
            expand: group.key === key ? !group.expand : group.expand
        }));
    }
...
*/
    ...
    buildDocTypeView(records) {
        ...
        records.forEach(repo => {
            ...
            map[rawType].attachmentCount +=
                repo.Repository_Attachments__r
                    ? repo.Repository_Attachments__r.length
                    : 0;
            // Active count can hide docs still sitting in Archive - keep folder locked if so
            if (repo.HasArchivedAttachments) {
                map[rawType].hasArchivedAttachments = true;
            }
        });
        ...
        this.repoDocTypeList.forEach(group => {
            ...
            // group.showDelete = !isDefault && group.attachmentCount === 0;
            group.showDelete = !isDefault && group.attachmentCount === 0 && !group.hasArchivedAttachments;
            group.showEdit = !isDefault;
        });
  ...
  /*   get isAllConsentSelected() {
   ...
   get isAllConsentSelected() {
    const enabledUsers = this.finalUsers;/* .filter(u => !u.disableConsent); */
    console.log('enabledUsers:', JSON.stringify(enabledUsers));

    if (enabledUsers.length === 0) return false;

    const selectedCount = enabledUsers.filter(u => u.consentRequired).length;
     console.log('selectedCount:', selectedCount);
    console.log('enabledUsers.length:', enabledUsers.length);

   const result = selectedCount === enabledUsers.length;

     console.log('isAllConsentSelected:', result);
     return result;
}  
    ...
    handleCardClick(event) {
        ...
        if (this.staffFlag && !this._lastStaffSubRoute) {
            this._lastStaffSubRoute = 'staff';
        } else if (this.ParticipantFlag && !this._lastParticipantSubRoute) {
            this._lastParticipantSubRoute = 'participant';
        }        
        this._syncRecordRoute();
  }
    ...
    wiredUsers(result) {
        ...
        //         ...item.user,
        //         hasViewPermission: item.hasViewPermission || false, 
        ...
        if (data) {
            ...
            // Step 3: call staff mapping
            getStaffNicknameByEmail({ userIds })
                .then(mapResult => {

                    // Step 4: merge staff name
                    this.allUsers = users.map(u => {
                        const email = u.Email?.trim().toLowerCase();

                        return {
                            ...u,
                            staffName: (email && mapResult[email])
                                ? mapResult[email]
                ...
                .catch(err => {
                    console.error('Staff mapping error:', err);
                    this.allUsers = users;
                    this.filteredUsers = [...users];
                });
        }
...
exportCSV() {
    ...
    source.forEach(parent => {
         // ❌ Skip parent if no child records
    if (!parent.Repository_Attachments__r || parent.Repository_Attachments__r.length === 0) {
        return;
    }
        // Parent Row
    ...
    loadConsent() {
        ...
        .then(result => {
            ...
            // Step 2: call email → staff mapping
            return getStaffNicknameByEmail({ userIds })
                .then(mapResult => {

                    // Step 3: merge staff names
                    this.ConsentList = result.map(rec => {
                        const email = rec.User__r?.Email;
    ...
    loadRepositoryTypes() {
        ...
        return getRepoTypes({ organisationId: this.orgId })
            .then(result => {
                ...
                this.options = [...new Set([...this.defaultTypes, ...orgTypes, 'Others'])];
    ...
    handleAddOption() {
        ...
        console.log('🟢 Updated Dropdown Options:', this.orgId);
...
handleArchive(event) {
    ...
    .then(() => {
        ...
        console.log('✅ Archived Attachment:', this.attachmentid);

        this.fetchInitialRepositoryData();

        // Optional: if archive modal is open
        // this.loadArchivedFiles();
    })
    .catch(error => {
...
loadArchivedFiles() {
    ...
    console.log('📥 Loading Archived Repositories...');
    console.log('Organisation Id =>', this.orgId);

    getArchivedRepositoryAttachments({
        organisationId: this.orgId
    })
    .then(result => {

        console.log(
            '✅ Archived Repository Result:',
        ...
        const records = result.map(rec => {
            ...
            return {
                ...
                Name: rec.Repository__r
                    ? rec.Repository__r.Name
                    : '',

                Type: rec.Repository__r
                    ? rec.Repository__r.Document_Type__c
                    : '',
                ...
                FileUrl: rec.Awsurl__c,

                Key: rec.key__c,

                RepositoryId: rec.Repository__c
            };
        });

        console.log(
            '✅ Mapped Archived Repositories:',
    ...
    .catch(error => {
        ...
        console.error(
            '❌ Error Loading Archived Repositories:',
            JSON.stringify(error)
        );
    });
}
...
handleArchiveSearch(event) {
    ...
    this.archivedRecords = this.allArchivedRecords.filter(repo =>
        (repo.Name &&
