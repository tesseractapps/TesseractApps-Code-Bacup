...
import sendLeaveNotification from '@salesforce/apex/LeaveController.sendLeaveNotification';
...
export default class StaffLeaveManagementLwc extends LightningElement {
  ...
  handleleave(event) {
    ...
    if (!this.typeOfEmployee) {
      this.dispatchEvent(
        new ShowToastEvent({
          title: 'Employment Type and Primary Role are missing in the Staff record.',
          message: 'Please set the role as Primary and select Employment Type in the Staff record before applying leave.',
          variant: 'error',
          mode: 'dismissable',
        })
      );
  ...
  handlecancelleave(event) {
    ...
    this.cancelcomments = '';
    if (status === 'Cancelled' || status === 'Withdrawn' || status === 'Rejected') {
      this.dispatchEvent(
        new ShowToastEvent({
  ...
  wiredClient(result) {
    ...
    if (data) {
      ...
      if(this.typeOfUser === 'ICT User'){
       ...
       const roles = this.clientData[0].StaffRoles__r || [];
       const primaryRole = roles.find(r => r.Primary__c && r.Type_of_Job__c);

       this.typeOfEmployee = primaryRole?.Type_of_Job__c || '';
      }
   ...
   const fieldName = event.target.fieldName;
    ...
    getStaffById({ recordId: this.staffId })
        .then((data) => {
            ...
            if (this.typeOfUser === 'ICT User') {
                this.typeOfEmployee = this.clientData[0].Type_of_Employe__c;
                console.log('ict this.typeOfEmployee',this.typeOfEmployee);
            } else {
                const roles = this.clientData[0].StaffRoles__r || [];
                const primaryRole = roles.find(r => r.Primary__c && r.Type_of_Job__c);
                this.typeOfEmployee = primaryRole?.Type_of_Job__c || '';
                 console.log('ndis this.typeOfEmployee',this.typeOfEmployee);
            }

            this.baseRate = this.clientData[0].Working_Hours_Rate__c;
            this.leaveLoadingEnabled = this.clientData[0].Facility__r?.Organisation__r?.Leave_Loading__c;
            this.leaveLoadingRate = this.clientData[0].Facility__r?.Organisation__r?.Leave_Loading_Rate__c;

            let leaves = this.clientData[0].Leaves__r || [];

            this.annualUsed = Number(
                leaves.reduce((sum, rec) => sum + (rec.Annual_Used__c || 0), 0).toFixed(2)
            );

            this.personalUsed = Number(
                leaves.reduce((sum, rec) => sum + (rec.Personal_Used__c || 0), 0).toFixed(2)
            );
...
loadLeavesData() {
    if (!this.staffId || !this.orgId) return;

    getLeaves({ recordId: this.staffId, OrgId: this.orgId })
        .then((data) => {
            ...
            let approved = 0, rejected = 0, cancelled = 0, requested = 0;

            data.forEach((rec) => {
                switch (rec.Status__c) {
  ...
  handleInput(event) {
    ...
    if (fieldName === 'From__c') {
        ...
        if (selectedDate < today) { // Clear invalid value
            event.target.value = null;
            this.fromDate = null;
            this.saveButtonDisable = true;
            this.dispatchEvent(new ShowToastEvent({title: 'Validation Error', message: 'Past dates are not allowed.', variant: 'error', mode: 'dismissable'}));
            return;
        } else {
            this.saveButtonDisable = false;
        }
  ...
  handleSuccess(event) {
    ...
    //manendra added for sending notification to manager when leave is applied
...
//manendra updated this method  for sortin table
  updatePagedData() {
      // Sort the entire dataset before pagination
      let displayLeaves = [...this.leaves];
      if (this.sortField) {
          displayLeaves = this.sortData(displayLeaves);
      }
      // Calculate current page
      const start = (this.pageNumber - 1) * this.pageSize;
      const end = start + this.pageSize;
      // Slice sorted dataset for current page
      this.pagedLeaves = displayLeaves.slice(start, end);
  }
...
lastPage() {
  ...
  handleResize() {
    if (this.isPageSizeManuallySet || this.isPageSizeBulkManuallySet) {
      return;
    }
    this.setPageSizeByZoomAndScreen();
  }
  ...
  setPageSizeByZoomAndScreen() {
    ...
    setTimeout(() => {
      const container = this.template.querySelector('.table-container');
      const header = this.template.querySelector('.header-wrapper');
      const rows = this.template.querySelectorAll('.table-row');
      ...
      if (container) {
        ...
        if (rows && rows.length > 0) {
          rowHeight = rows[0].clientHeight || 45;
        }

        const availableHeight = containerHeight - headerHeight - 10; // buffer/padding
        if (availableHeight > 0) {
          const calculatedPageSize = Math.floor(availableHeight / rowHeight);
          const newPageSize = Math.max(3, calculatedPageSize); // Minimum of 3 rows
...
get bDisableLast() {
    ...
    @api async openByUID(uid, tab, isEdit) {
        ...
        try {
            let found = this.leaves ? this.leaves.find(r => r.Leave_UID__c === uid || r.Id === uid) : null;
            if (!found) {
    ...
    @api openCreate(step) {
        // no-op for staff leave management, request leave triggers creation flow
        this.handleleave();
    }
...
sortData(data) {
    ...
    return [...data].sort((a, b) => {
        ...
        // Empty / null / N/A always at bottom
        const emptyA =
            valueA === null ||
            valueA === undefined ||
            valueA === '' ||
        ...
        // Handle formatted leave dates: dd/MM/yyyy
        if (field === 'fromdate' || field === 'Todate') {

            const [dayA, monthA, yearA] = String(valueA)
                .split('/')
                .map(Number);

            const [dayB, monthB, yearB] = String(valueB)
                .split('/')
                .map(Number);
        ...
        // Natural / alphanumeric text sorting
