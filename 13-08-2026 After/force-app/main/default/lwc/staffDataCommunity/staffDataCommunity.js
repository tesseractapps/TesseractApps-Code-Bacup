...
export default class StaffDataCommunity extends NavigationMixin(LightningElement) {
    ...
    @track roleEmploymentData = {};// manendra added for role memory storage
...
// Previous Button
get showPrevious() {
    return !this.SupportCoordinationflag &&
...
get showNext() {
    ...
    // Support Coordinator should never see Next
    if (this.SupportCoordinationflag) {
        return false;
    }
...
get showSave() {
    ...
    // Support Coordinator should always see Save
    if (this.SupportCoordinationflag) {
        return true;
    }
...
hasSuperFund = false;
@track SupportCoordinationflag = false;
  @api
...
@track DocumentColumns = [
    ...
    connectedCallback() {  
        ...
        if(this.whatId){
            ...
            organizationDetails().then(response => {  
                ...
                console.log(
                    'Leave_Loading__c type:',
                    typeof response.listofPriceBook.Leave_Loading__c
                );

                console.log(
                    'Leave_Loading__c value:',
                    response.listofPriceBook.Leave_Loading__c
                );

                this.permanentLeaveLoadingRate = response.listofPriceBook.Leave_Loading_Rate__c;
                this.temporaryLeaveLoadingRate = response.listofPriceBook.Part_Time_Leave_Loading_Rate__c;
                ...
                // Enable/Disable Leave Loading Rate field
                this.LeaveLoadingEnable = !response.listofPriceBook.Leave_Loading__c;
                console.log(
       ...
       console.log('Languages>>', this.Languages);
            ...
            getCurrentLoggedUserInfo()
                .then(userData => {
                    ...
                    this.SupportCoordinationflag = (userType === 'Support Coordinator');
                    console.log(
                        'SupportCoordinationflag:',
                        this.SupportCoordinationflag
                    );
    ...
    avatarColor: this.getAvatarClass(rec.DisplayNicknamec || rec.Name || rec.NameToDisplayc)
            ...
            if (this._restoreFromUrlHash()) {
                console.log('Restored staff view from URL hash');
            } else {
...
console.log( 'Role Object =>', JSON.stringify(this.selectedRoleEmploymentValues[0], null, 2));
    ...
    handlePageSizeChange(event) {
        this.handleRecordsPerPage(event);
    }
    ...
    setPageSizeByZoomAndScreen() {
        ...
        const containerHeight = rect.height || container.offsetHeight || 600;
        ...
        // Available vertical height inside the component container
        const availableHeight = containerHeight - headerHeight - paginationHeight - 20;

        let rowHeight = 45; // default row height for list view
        if (this.cardFlag) {
            rowHeight = 175; // card height
        }
        ...
        this.recordsPerPage = rows * columns;
        this.pageSize = this.recordsPerPage;
    ...
    loadUserTypeValuesForSingleUser() {
        console.log('=== START: loadUserTypeValuesForSingleUser ===');
        console.log('Loading user type values for single user creation');
        ...
        .then(result => {
            ...
            // Store in filteredUserTypeValues for the combobox
            ...
            console.log('=== END: loadUserTypeValuesForSingleUser (SUCCESS) ===');
        })
        .catch(error => {
            console.error('Error loading user type values:', error);
            this.filteredUserTypeValues = [];
            this.isLoading = false;
            console.log('=== END: loadUserTypeValuesForSingleUser (FAILED) ===');
        });
    }
    ...
    get showSupportCoordinatorSteps() {

        console.log('==============================');
        console.log('supportCoordinationFlag :', this.SupportCoordinationflag);
        console.log('staffEditFlag           :', this.staffEditFlag);

        const result =
            this.SupportCoordinationflag &&
            this.staffEditFlag;

        console.log('showSupportCoordinatorSteps :', result);
        console.log('==============================');
    ...
    async handleCreateNewStaff(step) {
        // this.createCurrentStep = step || 'createstep1';
        // const stepVal = (typeof step === 'string') ? step : 'createstep1';
    ...
    try {
        ...
        // this.createCurrentStep = step || 'createstep1';
        this.createCurrentStep = stepVal;
        if(this.SupportCoordinationflag === false){
        ...
        this.loadUserTypeValuesForSingleUser();
    ...
    for (let i = 0; i < this.contactList.length; i++) {
        ...
        // If Leave Loading Rate is blank, inherit from Organisation
        if (
...
handleSubmit(event){
    ...
    saveContacts(staffRecID) {
      ...
      handleSuccess(event) { 
        ...
        // Save the currently selected employment role before sending to Apex
        this.saveCurrentRoleDraft();//manendra added for stored role employment data
        ...
        roleEmploymentData: JSON.stringify(this.roleEmploymentData),//manendra added for stored role employment data
   ...
   const payload = {
    ...
    handlecreateuser(event) {
        ...
        const mapUserTypeToRole = (userType) => {
            ...
            return roleMap[userType] || 'Portal Account Partner User';
        };
    ...
    handleeditClose(){   
        this.roleEmploymentData = {};
        this.currentEmploymentRoleKey = '';    
        this.staffEditFlag=false;
        this.supportCoordintorflag = false;
        this.cardFlag = true;
        this.adminFlag = true;
...
handleError(event) {
    ...
    async handlefacStatus(event){   
        ...
        console.log('Staff Id        :', this.facId);
        console.log('Current Status  :', this.facstatus);
        console.log('Original Status :', this.originalToggleState);
        ...
        if(this.facstatus === 'true'){
            console.log('Current Status in true block :', this.facstatus);
           
            try {
                console.log('Calling checkFutureShifts for staff:', this.facId);
                const shifts = await checkFutureShifts({ staffId: this.facId });
                console.log('Shift Count:', shifts ? shifts.length : 'NULL');
                if (shifts && shifts.length > 0) {
                    console.warn('Future shifts FOUND.');
                    ...
                    this.finalStatus='true';
                    this.message= 'Staff is Active'
                    console.log('this.finalStatus in true block :', this.finalStatus);
                } else {
                    console.log('No future shifts found.');
                    this.handleStatusFlag = true;
            ...
            console.log('Current Status in false block :', this.facstatus);
            this.finalStatus='true';
            this.message= 'Staff is Active'
            console.log('this.finalStatus in false block :', this.finalStatus);
            this.handleStatusFlag = true;
            this.hasFutureShifts = false;
        }
        console.log('Final Status:', this.finalStatus);
 ...
 handlestatuschange() {
    ...
    addingPreTax(){
        ...
        fetchStaff({recordId :this.parentStaffId }).then(response => {
            ...
            this.satffDataJasonformat[response.Id]={"uid":response.Staff_UID__c || '',"street":response.Address__Street__s,"city":response.Address__City__s,"stateCode":response.Address__StateCode__s,"countryCode":response.Address__CountryCode__s,"postalCode":response.Address__PostalCode__s,"childRecords":response.Child_Staffs__r,
...
get viewToggleTarget() {
 ...
 fetchFacility(facId) {
        ...
        console.log('typeof facId:', typeof facId); 
        // getfacilityById({ facId: facId })
           return getfacilityById({ facId: facId })//manendra added for role memory storage
            .then(result => {
                if (result) {
                    this.typeOfService = result.Type_of_Service__c;
                    console.log('Type of Service in fetchFacility :', this.typeOfService);
                        // ===============================
                        // Reset all award visibility flags
                        // ===============================
...
handleEmploymentRoleSelect(event) {
    ...
    console.log('Role Value =>', selectedOption.value);
    console.log('Role Label =>', selectedOption.displaylabel);
    if (!selectedOption) {
        console.error('❌ No option found at index:', index);
        return;
    }

    this.saveCurrentRoleDraft();// manendra added to save the current role draft before changing the role
    ...
    // manendra added to save the current role draft before changing the role
    this.currentEmploymentRoleKey =`${selectedOption.value}_${selectedFacilityValue}`;
    ...
    .then(() => {
        ...
        if (!restored) {
            ...
            console.log('[Employment Draft] No draft found. Resetting values.');

            // Reset values (your existing code)
            this.jobType = '';
            this.categoryType = '';
            this.classificationLevel = '';
            this.classificationPayType = '';
            ...
            console.log('[Employment Draft] Draft restored successfully.');

        }

    })
    .catch(error => {
        console.error('Error while loading facility:', error);
    });
//manendra added for role memory storage
    // Close dropdown
    this.roleDropDownForEmployment = false;
...
validateAndGetMissingFields() {
    ...
    // 🔹 EMPLOYMENT ROLE VALIDATION (Step 2)
    if (this.createCurrentStep === 'createstep2' && !this.ictUserType && !this.supportCoordintionflag) {
        if (!this.selectedEmploymentRole) {
    ...
    if (this.createCurrentStep === 'createstep3' && !this.supportCoordintionflag) {
        ...
        if (abnVal && tfnVal) {
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Validation Error',
                    message: 'TFN or ABN only one field is required.',
                    variant: 'error'
                })
            );
...
handleSupportCoordinatorSave() {

    const errors = this.validateSupportCoordinatorFields();
...
validateSupportCoordinatorFields() {
    let errors = [];

    const fields = this.template.querySelectorAll(
        'lightning-input-field, lightning-input, lightning-combobox'
    );
...
handleSupportCoordinatorSuccess(event){
    ...
    // =====================================================================
    // HASH ROUTER bridge (additive) — mirroring facilityDataCommunity.
    // =====================================================================
    _suppressEmit = false;
    _lastSubRoute = '';
    _childPopup = '';
    ...
    _composeSubRoute() {
        ...
        const child = this.template.querySelector('c-create-edit-staff-lwc');
        const isEditMode = child ? child.isEdit : false;
        if (isEditMode) {
    ...
    // _syncRecordRoute() {
    //     if (this._suppressEmit) return;
    //     // editstaffflag is the authoritative "a record is on screen" signal
    //     if (!this.editstaffflag) return;
    //     if (!this._recordUid) return;
    ...
    //         if (this.satffDataJasonformat[ids[i]] && this.satffDataJasonformat[ids[i]].uid === uid) {
    //             return ids[i];
    //         }
    //     }
    //     return '';
    ...
    @api async openByUID(uid, tab, isEdit) {
        ...
        try {
            // Wait for records to be loaded if the uid is unresolved
            // if (!this._staffIdForUid(uid)) {
            //     this.refreshKey = this.refreshKey + 1;
            //     await new Promise(resolve => setTimeout(resolve, 600));
            // }
            // const staffId = this._staffIdForUid(uid);
            let staffId = this._staffIdForUid(uid);
            let staffRecord = null;
            if (!staffId) {
                const response = await fetchStaff({ recordId: uid });
                if (response && response.Id) {
                    staffRecord = response;
            ...
            if (staffId) {
                // Facility Auto-Switching logic
                let rec = staffRecord;
                if (!rec && this.staffList) {
                    rec = this.staffList.find(c => c.Id === staffId);
                }
                if (rec) {
                    let facilities = [];
                    if (rec.Staff_Facilities__r) {
                        facilities = rec.Staff_Facilities__r;
                    } else if (rec.Facility__c) {
                        facilities = [{ Facility__c: rec.Facility__c, Facility__r: { Name: rec.Facility__r?.Name } }];
                    }
                    
                    if (facilities && facilities.length > 0) {
                        const activeFacs = facilities.map(f => ({
                            id: f.Facility__c,
                            name: f.Facility__r?.Name
                        })).filter(f => f.id);
    ...
    _restoreFromUrlHash() {
        const parts = (window.location.hash || '')
            ...
            .split('/')
            .filter(Boolean);
        const hasCreate = parts.includes('create');
        if (hasCreate) {
            const createIdx = parts.indexOf('create');
            const step = parts[createIdx + 1] || 'step1';
            this._suppressEmit = true;
            try {
                this.handleCreateNewStaff('create' + step);
            } finally {
                this._suppressEmit = false;
            }
        ...
        const tab = parts[uidIdx + 1] || 'employee';
        const modes = parts.slice(uidIdx + 2);
        const isEdit = modes.includes('edit');
        const step = modes.find((m) => /^step[1-5]$/.test(m));
        const popup = modes.find((m) => m !== 'edit' && !/^step[1-5]$/.test(m));
        const staffId = this._staffIdForUid(uid);
