...
//import getAllLedgerItemsForPurchases from '@salesforce/apex/AccountingChartController.getAllLedgerItemsForPurchases';
...
export default class AccountExpense extends LightningElement {
   ...
   // supplierSearchTerm = '';
    FilteredLedgerItems = [];
    @track globalSearchValue = ''; //search
   ...
   // @track invoiceTable = [];  // displayed filtered dataset
    ...
    connectedCallback(){
       ...
       /*  var today = new Date(new Date().getFullYear(), new Date().getMonth(), 2);
        ...
        this.startdateValueData = pastDate.toISOString().slice(0, 10);  
        this.enddateValueData = today.toISOString().slice(0, 10);
...
// ============================
//  Global Search Logic vamshi - handlePurchaseSearch()
// ============================
handlePurchaseSearch(event) {
    const searchValue = (event.target.value || '').trim().toLowerCase();
    this.globalSearchValue = searchValue;

     const searchKey = this.clean(searchValue);   // NEW
    // Start from the full list of records
    let result = [...this.records];

    if (searchValue) {
        result = result.filter((rec) => {
            const purchaseId = rec.Name ? rec.Name.toLowerCase() : '';
            const description = rec.description ? rec.description.toLowerCase() : '';
            const entityName = rec.entityName ? rec.entityName.toLowerCase() : '';
            const ledgerItem = rec.ledgerItem ? rec.ledgerItem.toLowerCase() : '';
            //const gst = rec.gst ? rec.gst.toString().toLowerCase() : '';
            //const amount = rec.amount ? rec.amount.toString().toLowerCase() : '';

            const gst = this.clean(rec.gst);
            const amount = this.clean(rec.amount);
            const date = rec.date ? rec.date.toLowerCase() : '';

            return (
                purchaseId.includes(searchValue) ||
    ...
    // Update visible list for first page
    const startIdx = 0;
    const endIdx = this.pageSize;
    this.Listofdata_Pagination = this.filteredRecords.slice(startIdx, endIdx);
   ...
   // Handle input changes for amount
    ...
    async handleSaveExpense() {
        ...
        try {
            ...
            //upload file logic added by vamshi 1127-
                if (this.base64FileData && this.fileName) {
            console.log("🚀 Uploading file for Accounting Invoice: 1135", this.fileName);
            ...
            await uploadFile({
                ...
                obj: "PurchaseInvoice" // 👈 This tells Apex which object
        ...
        //             console.log("✅ File ready for upload:", this.fileName);
        //         };
            ...
            if (event.target.files.length > 0) {
                ...
                const fileSize = this.file.size;
                  ...
                  if (event.target.files.length > 1) {
                    this.showToast("Error", "You can upload only one file per record.", "error");
                         return;
                        }
                // ✅ Check file size before reading
                if (fileSize > this.MAX_FILE_SIZE) {
                    this.showToast("Error", "File size cannot exceed 2 MB.", "error");
                    return;
                }
                ...
                reader.onloadend = () => {
                    let base64 = reader.result.split(",")[1];
                    this.base64FileData = base64;
                    console.log("✅ File ready for  File ready for upload:", this.fileName);
                     //console.log("🚀 Uploading file for Accounting Invoice:", this.fileName);
            console.log('upload parameters....',base64);
                };
                reader.readAsDataURL(this.file);
                console.log('file uploaded',this.file)
            }
        }
        //added by vamshi 1225 to 1232 for view function
//          handleView(event) {
...
//      }
//  openAddPayrollInvoice() {
//     this.isModalOpen = true;
//     this.isExpenses = false;   // must be false for your HTML logic
//     }
//     closeaddPayrollinvoice() {
//         this.isModalOpen = false;
//         this.isExpenses = true;    // restore main screen
//     }
get showModalView() {
   // return !this.isExpenses && this.isModalOpen;
   return this.isModalOpen && !this.isExpenses;
...
//                 message: 'You don’t have any attachments for this purchase.',
//                 variant: 'warning'
//             })
//         );
...
//     // ✅ If file is Word/Excel type → download instead of open
//     if (
//         lowerUrl.endsWith('.doc') ||
//         lowerUrl.endsWith('.docx')
       
//     ) {
//         console.log('📥 Downloading Word/Excel file:', url);
...
//         return; // ✅ stop here (don’t open iframe)
//     }
...
//     this.isModalOpen = true;
//     this.isInvoiceflag=false;
//          // 🔹 Notify parent to HIDE the Sales/Purchases banner
//     this.dispatchEvent(new CustomEvent('hidebanner', { bubbles: true, composed: true }));

//     console.log('Flags after click:', {
//         isExpenses: this.isExpenses,
...
handleView(event) {
    ...
    // ✅ If file is Word (.doc or .docx) → force download
    if (lowerUrl.endsWith('.doc') || lowerUrl.endsWith('.docx')) {
        console.log('📥 Downloading Word file:', url);

        try {
            // Create a hidden anchor element
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', ''); // triggers "Save As"
            ...
            link.click();
            document.body.removeChild(link);
            console.log('✅ Download triggered for Word file');
        } catch (error) {
            console.error('⚠️ Direct download failed, fallback to window.open:', error);
            // Fallback in case direct click fails
    ...
    // ✅ Otherwise (PDFs, images) → open in iframe
    this.currentUrl = url;
    this.isExpenses = false;
    this.isModalOpen = true;
    this.isInvoiceflag = false;

    // 🔹 Notify parent to hide the banner
    this.dispatchEvent(new CustomEvent('hidebanner', { bubbles: true, composed: true }));
...
closeaddPayrollinvoice() {
    ...
    // 🔹 Notify parent to SHOW the banner again
    this.dispatchEvent(new CustomEvent('showbanner', { bubbles: true, composed: true }));
    ...
    wiredExpenseData(result) {
            ...
            this.records = data.map(invoice => {
                ...
                return {
                   ...
                   // Status__c: invoice.Status__c,
                    ...
                    purchaseInvoiceUrl: invoice.Purchase_Invoiceurl__c
        ...
        //added by vamshi 1419-1420 to support global search 
    ...
    handleClick(event){
        ...
        //clear previous upload state before editing a new record
        this.fileName = '';
        this.base64FileData = null;
        this.file = null;
    ...
    // // If lightning-input / lightning-textarea wrapper is focused →
    // // its real input is inside shadow DOM, so dig into it.
    // if (active.shadowRoot && active.shadowRoot.activeElement) {
    //     active = active.shadowRoot.activeElement;
    // }

    // // Try reading data-id from element
    // const rowId =
    //     active?.dataset?.id ||
    ...
    handleRowFocus(event) {
        ...
        // ALT + V = focus Invoice No

        if (event.altKey && event.key.toLowerCase() === 'v') {
            event.preventDefault();

            const invoiceField = this.template.querySelector('[data-id="invoiceNoField"]');
