...
export default class TesseractAppsRosterCreation extends LightningElement {
  ...
  @track shiftDeleteOrCancelOptions=[ { label: "Cancel", value: "Cancel" },
  ...
  connectedCallback() {
    ...
    this.staffPreferredName =
      ...
      this.selectedViewType = 
         localStorage.getItem("rosterViewType") || "weekly";
      console.log('this.selectedViewType', this.selectedViewType );
      ...
      fetchBulkRoles({ facilityIDList: this.facilityValue })
        .then(facRoles => {
            ...
            if (this.OrgNisationRoles.length > 0) {
                ...
                if (!this.SelctedComboBoxRole || this.SelctedComboBoxRole === '') {
                ...
                // Apply the same logic for "All" role whether from localStorage or default
...
handleDatePickerChange(event) {
  ...
  async handleConfirmSetHours(event) {
    ...
    console.log(' facility value from staff  '+event.currentTarget.dataset.facilityval);
     this.addShiftData.AddShiftFacilityValue=this.facilityValue[0];
...
async getShiftTimingsByFacility(shifNames) {
  console.log("getShiftTimingsByFacility :called ",JSON.stringify(shifNames) );
    ...
    if (this.addShiftData.AddShiftType === "Custom") {
     ...
     console.log(' max duartion in save ');
    console.log(' max duartion in  save this.addShiftData.AddShiftDuration  '+this.addShiftData.AddShiftDuration);
    console.log(' max duartion in save this.addshiftMaxDuration '+this.addshiftMaxDuration );
    }
    if (this.addShiftData.AddShiftType === "Custom") {
      const { enrichedSegments, updatedServices } =
        await this.prepareAndValidateCustomShifts();
  ...
  const { id, role, param, facilityval, sethours, exceedhours } =
      ...
      if (isChecked) {
        ...
        if(this.shiftAddressInRosterSettings !='' && this.shiftAddressInRosterSettings != null && this.shiftAddressInRosterSettings != undefined 
            ...
            this.handleAddressSelection({
              currentTarget: {
                dataset: {
                  type: this.shiftAddressInRosterSettings == 'Facility' ? 'facility' : 'participant',  // or 'participant' or 'new'
                  index: 0           // whichever row index you want
                  }
              }
            });
        } 
 ...
 */
  ...
  handleNotificationChange(event) {
    this.addShiftData.AddShiftnotification = event.target.checked;
