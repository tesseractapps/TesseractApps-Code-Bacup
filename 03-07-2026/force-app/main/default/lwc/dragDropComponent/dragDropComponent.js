...
get isDragEnabled() {
    return this.formTitle.trim().length > 0 && this.selectedFormType.trim().length > 0;
}
...
@wire(getCurrentUserName)
    ...
    connectedCallback() {
        ...
        // Subscribe to message channel for inter-component communication
        this.subscribeToMessageChannel();
        
        if (this.layoutId) {
            // Load layout by ID if available (e.g., for edit mode)
            this.loadLayoutById(this.layoutId);
        } else {
            // Initialize with a default table structure if no layout ID is provided
        ...
        // Delay the dropdown value setting until after the component's DOM has fully rendered
        requestAnimationFrame(() => {
            const dropdown = this.template.querySelector('.form-dropdown');
            if (dropdown) {
                // Set dropdown to the current selectedFormType
                dropdown.value = this.selectedFormType;
                console.log('Dropdown initialized with selectedFormType:', this.selectedFormType);
    
                // Set to a default value if selectedFormType is empty and options are available
                if (!this.selectedFormType && this.formTypeOptions && this.formTypeOptions.length > 0) {
    ...
    loadClonedLayout(message) {
        try {
            ...
            // Use message.Name__c directly to set selectedFormType
            this.selectedFormType = message.Name__c || ''; 
    ...
    renderedCallback() {
        ...
        // Synchronize the dropdown value with selectedFormType
        requestAnimationFrame(() => {
            const dropdown = this.template.querySelector('.form-dropdown');
            if (dropdown) {
                if (!this.selectedFormType) {
                    this.selectedFormType = dropdown.value; // Sync with UI if backend is missing
                } else {
                    dropdown.value = this.selectedFormType; // Sync UI with backend
                }
                console.log('Dropdown value synchronized:', dropdown.value);
            } else {
                console.warn('Form dropdown element not found.');
            }
        });
    
        // Validate form title and form type to update send button state
        const titleValid = this.formTitle && this.formTitle.trim().length > 0;
        const formTypeValid = this.selectedFormType && this.selectedFormType.trim().length > 0;
        ...
        // Reattach cell click event listeners to ensure cell interactivity
        requestAnimationFrame(() => {
            const cells = this.template.querySelectorAll('.table-cell');
            if (cells.length > 0) {
                cells.forEach(cell => {
                    cell.removeEventListener('click', this.handleCellClick); // Prevent duplicate event listeners
                    cell.addEventListener('click', this.handleCellClick.bind(this)); // Attach event listener
                });
            } else {
        ...
        // Update button states based on current validations
        this.updateButtonStates();
    }
    ...
    handleReset() {
        ...
        this.tableRows = [];
        this.createTable(5, 2);  // Re-initialize with default 5 rows and 2 columns (or any other default setup you need)
        ...
        this.comments = '';
        this.selectedFormType = '';  // Clear the selected form type
        ...
        requestAnimationFrame(() => {
            const dropdown = this.template.querySelector('.form-dropdown');
            if (dropdown) dropdown.value = ''; // Clear the dropdown value
        });
        this.saveTableState();
    }
    ...
    handleDrop(event) {
        ...
        // Get the target cell where the field is being dropped
        const targetCellId = event.target.dataset.id;
        const targetCell = this.getCellById(targetCellId);
    
        // Check if an existing field is being dragged for rearrangement
        const draggingId = event.dataTransfer.getData('text/plain');
    
        // If rearranging an existing field
        if (this.draggingFieldId) {
            // Find the original cell containing the dragged field
            const sourceCell = this.getCellByFieldId(this.draggingFieldId);
    
            // Move the field from the source cell to the target cell
            if (sourceCell && sourceCell.field) {
                targetCell.field = sourceCell.field; // Move the field data to the target cell
                sourceCell.field = null; // Clear the original cell
                this.draggingFieldId = null; // Reset the dragged field ID
            }
        } 
        // If a new field is being added to the table
        else if (this.draggingFieldType) {
            this.resetFieldSettings(); // Reset flags for new field types
            this.currentFieldType = this.draggingFieldType;
    
            // Set flags for the new field type and show popup
            ...
            // Set the cell reference and show the popup for entering field details
        ...
        this.tableRows = [...this.tableRows];
        this.saveTableState(); // Save the state for undo functionality
    }
...
handleDateFormatChange(event) {
    ...
    handleFormTypeChange(event) {
        ...
        console.log('Form Type Selected:', this.selectedFormType); // Log to confirm
    }
    ...
    async handleSaveLayout() {
        try {
            ...
            this.showToast('Success', `Layout saved successfully with Name: ${savedLayoutName}`, 'success');
        } catch (error) {
