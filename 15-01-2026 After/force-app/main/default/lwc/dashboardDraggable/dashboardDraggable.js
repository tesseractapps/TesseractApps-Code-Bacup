...
@wire(getRecord, {
    ...
    fields: [UserNameFld, UserEmail, UsrRoleName, UserType, UserTypeOfUser]
})
...
userDetails({error, data}) {
  ...
  async initDashboard() {
    try {
      ...
      if (!this.interactLoaded) {
        console.log("📦 Loading interact.js...");
      ...
      console.log("✅ Dashboard initialized (Loading=false)");
    }
  }
  ...
  async loadWidgets() {
    try {
      console.log("📥 Fetching saved widget layout for staffId:", this.staffId);

      const layoutJson = await getLayout({ staffId: this.staffId });
      console.log("🧾 Received layout JSON:", layoutJson);

      if (!layoutJson || layoutJson.trim() === "" || layoutJson.trim() === "[]") {
        console.warn("⚠ No layout available for this user");
        this.widgets = [];
        ...
        this.widgets = JSON.parse(layoutJson).map((widget, index) => {
          ...
          console.log(`✅ Widget ${index + 1} loaded:`, parsedWidget);
          return parsedWidget;
        });

        this.noLayoutAvailable = false;
        console.log("✅ All widgets loaded:", this.widgets);
      }

      // Save original widget states for comparison/reset
      this.originalWidgetStates = JSON.parse(JSON.stringify(this.widgets));
      console.log("📌 Saved original widget states:", this.originalWidgetStates);

      // Fetch user access details
      console.log("🔐 Fetching user access details...");
      this.fetchUserAccessDetails();
    } catch (error) {
...
get toolsFabMainClass() {
    return `tools-fab-main ${this.toolsFabOpen ? 'is-open' : ''}`;
}

get toolsFabTitle() {
    return this.toolsFabOpen ? 'Close Tools' : 'Tools';
}

// Slide-left animation using inline style so you don't disturb existing FAB logic
get toolsFabActionsStyle() {
    // When closed: hidden & shifted slightly right (towards button)
    if (!this.toolsFabOpen) {
        return 'opacity:0; pointer-events:none; transform: translateX(10px) scale(0.98); transition: all 180ms ease;';
    }
    // When open: visible & slides left into place
    return 'opacity:1; pointer-events:auto; transform: translateX(0) scale(1); transition: all 180ms ease;';
}

toggleToolsFab() {
    this.toolsFabOpen = !this.toolsFabOpen;
}

handleNdisQuoteTool() {
    this.dispatchEvent(
        new CustomEvent('openmodule', {
            detail: {
                field: 'ndisquotetool',   
                content: 'ndisquotetool'  
            },
            bubbles: true,
            composed: true
        })
