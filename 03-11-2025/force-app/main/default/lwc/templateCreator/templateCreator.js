...
export default class TemplateCreator extends LightningElement {
  ...
  onFileUpload(event) {
    ...
    if (event.target.files.length > 0) {
     ...
     let MAX_PDF_SIZE = 5 * 1024 * 1024; // 5MB for PDF
     let MAX_DOC_SIZE = 3 * 1024 * 1024; // 3MB for DOC/DOCX
     
      if (this.fileType === 'application/pdf') {
          if (this.fileSize > MAX_PDF_SIZE) {
              this.showToast("Error", "PDF file size must be 5MB or smaller.", "error");
              this.showSpinner = false;
              this.isLoading = false;
               event.target.value = null;
              return;
          ...
          if (this.fileSize > MAX_DOC_SIZE) {
              this.showToast("Error", "Word document size must be 3MB or smaller.", "error");
              this.showSpinner = false;
              this.isLoading = false;
               event.target.value = null;
              return;
  ...
  handleRearrangeMove = (event) => {
    if (this.currentDraggedPlaceholder) {
      const rect = this.template
        .querySelector(".image-container")
        .getBoundingClientRect();
      const x = Math.max(
        0,
        Math.min(event.clientX - rect.left, rect.width - 50)
      ); // Stay within width
      const y = Math.max(
        0,
        Math.min(event.clientY - rect.top, rect.height - 50)
      ); // Stay within height
