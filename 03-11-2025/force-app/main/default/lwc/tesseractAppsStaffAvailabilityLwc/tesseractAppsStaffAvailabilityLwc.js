...
export default class StaffAvailabilityLwc extends LightningElement {
  ...
  async handleAddDescription(event) {
    ...
    this.todayDate = formattedDate;
    this.todayDateforheader = formattedDateforheader;
    await this.DocExpiryCheck(selectedDate);
  ...
  getShiftTimingsByFacility(shifNames) {
    try {
      ...
      console.log("✅ Display Duration:", this.shiftDuration);
  ...
  updateShiftDates() {
    ...
    preventStaffAvailabilityDuplicates({
      startDate: this.todayDate,
      endDate: this.endDate,
      startTime: this.AddShiftStartTime,
      endTime: this.AddShiftEndTime,
      staffId: this.StaffId,
      AvailId: this.AvaliableId
    }).then((result) => {
      ...
      if (this.isSaveDisabled) {
        this.dispatchEvent(
          new ShowToastEvent({
            ...
            message:
              "You already have an existing availability or shift during the selected time.",
            variant: "error"
          })
        );
      }
  ...
  handleStartTimeChange(event) {
    ...
    console.log("this.shiftDuration==>", this.shiftDuration);
    ...
    if (this.AddShiftEndTime) {
      ...
      console.log("⏱ Duration (hours):", this.shiftDurationMax);

      // 🔥 Compare and show toast
      if (this.shiftDuration < this.shiftDurationMax) {
        this.showToast(
          "Error",
  ...
  handleEndTimeChange(event) {
    ...
    if (this.AddShiftStartTime) {
      ...
      console.log("⏱ Duration (hours):", this.shiftDuration);
  ...
  calculateDuration(startTime, endTime) {
    if (!startTime || !endTime) {
      console.warn("⚠️ Missing start or end time for duration calc", {
        startTime,
        endTime
      });
      return null;
    ...
    if (durationMs < 0) {
      durationMs += 24 * 60 * 60 * 1000; // add 24 hrs
    }

    const hours = durationMs / (1000 * 60 * 60); // ms → hours
    return Number(hours.toFixed(2)); // keep 2 decimals
  }
