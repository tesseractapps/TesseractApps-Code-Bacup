...
export default class TesseractAppsMyRosterLwc extends LightningElement {
    ...
    wiredShiftColors({ error, data }) {
        if (data) {
            ...
            // Refresh shift data if already loaded to apply new colors
    ...
    refreshShiftColors() {
        this.weekDays.forEach(day => {
            ...
            day.shifts.forEach(shift => {
                ...
                shift.shiftColor = this.getShiftColor(shift.typeofshift);
                shift.shiftStyle = this.getShiftStyle(shift.typeofshift);
            });
            
            // Update available shifts
            day.availableShifts.forEach(shift => {
                shift.cssClass = this.getShiftCssClass(shift.shiftType);
                shift.shiftColor = this.getShiftColor(shift.shiftType);
                shift.shiftStyle = this.getShiftStyle(shift.shiftType);
            });
        });
    }
            ...
            response.forEach(shift => {
                ...
                if (dayIndex !== -1 && shiftDate < this.formatDate(this.sixWeeksLater)) {
                    const shiftData = {
                        ...
                        typeofshift: shift.Type_of_shift__c,
                        time: shift.Add_Shift__r.Shift_Start_End_Time__c,
                        role: shift.Add_Shift__r.Role__c,
                        shiftdate: shiftDate,
                        cssClass: this.getShiftCssClass(shift.Type_of_shift__c),
                        shiftColor: this.getShiftColor(shift.Type_of_shift__c),
                        shiftStyle: this.getShiftStyle(shift.Type_of_shift__c)
                    };
            ...
            response.forEach(shift => {
                ...
                if (dayIndex !== -1 && 
                    ...
                    const shiftData = {
                        ...
                        cssClass: this.getShiftCssClass(shift.Shift_Type__c),
                        shiftColor: this.getShiftColor(shift.Shift_Type__c),
                        shiftStyle: this.getShiftStyle(shift.Shift_Type__c)
                    };
    ...
    handleConfirm(event) {
        ...
        if (shiftDate < today) {
            this.showToast('Error', 'You cannot reject a shift that was scheduled for a previous day.', 'error');
