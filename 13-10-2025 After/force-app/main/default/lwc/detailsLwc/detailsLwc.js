...
export default class DetailsLwc extends NavigationMixin(LightningElement) {
  ...
  handleAvailableChange(event) {
    ...
    this.disableUnavailabledate = true;
    if (this.isAvailable) this.isUnavailable = false;
  ...
  handleUnavailableChange(event) {
    ...
    this.disableUnavailabledate = false;
    if (this.isUnavailable) this.isAvailable = false;
  ...
  console.log("🗓️ Selected normalized date:", selectedDate);

  this.isSaveDisabled = false;
  console.log("🔧 Initial isSaveDisabled set to:", this.isSaveDisabled);

  // Check if no date is selected
  if (!selectedDate || isNaN(selectedDate.getTime())) {
    console.log("❌ No valid date selected. Showing 'Missing Date' toast.");
    ...
    console.log("🚫 unavailableUntilDate cleared, save disabled:", this.isSaveDisabled);
    console.log("🔴 [handleUnavailableUntilChange] END (Missing Date)");
    return;
  }
  ...
  if (selectedDate < today) {
    console.log(
      "⚠️ Invalid date: Selected date is earlier than today. Showing 'Invalid Date' toast."
    );
    ...
    this.dispatchEvent(
      new ShowToastEvent({
        title: "Invalid Date",
        message: "Previous Date should not be selected.",
        variant: "error"
      })
    );

    console.log("🚫 unavailableUntilDate cleared, save disabled:", this.isSaveDisabled);
    console.log("🔴 [handleUnavailableUntilChange] END (Invalid Date)");
    return;
  }

  console.log("✅ Valid date selected. Save enabled:", !this.isSaveDisabled);
  console.log("🔵 [handleUnavailableUntilChange] END (Success)");
}
