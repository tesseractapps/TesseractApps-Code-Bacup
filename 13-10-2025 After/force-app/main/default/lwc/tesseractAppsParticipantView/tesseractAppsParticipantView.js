...
export default class TesseractAppsParticipantView extends LightningElement {
  ...
  connectedCallback() {
    ...
    organizationDetails().then((response) => {
      ...
      console.log(
        "  this.OrgNisationRoles ==>" + JSON.stringify(this.OrgNisationRoles)
      );
 ...
 */
  ...
  async getShiftTimingsByFacility(shifNames) {
    ...
    /*   if(this.addShiftData.AddShiftDuration>shifNames[0].Duration__c){
           this.confirMationMessage(
          "Error",
    ...
    this.AddShiftAndServices.forEach((service) => {
      service.billablehours =  this.getFinalDuration(this.addShiftData.AddShiftDuration, this.addShiftData.AddShiftBreak ==30?0.5:0,service.staffPaidBreak);
    ...
    console.log("Staff Options IN FACILITY :", this.staffOptions);
...
/*   handleDeleteConfirmation(event) {
  ...
  async handleSave() {
    ...
    if (
      ...
      this.confirMationMessage(
        "Error",
        "Staff selection is required in the first row service section before saving.",
        "Error"
      );
      this.isShowSpinner = false;
      return;
        ...
        .then((result) => {
          ...
          payload.staffRows.forEach((item) => {
            ...
            servicesArray.forEach((ser) => {
              ...
              // ✅ Use final amount from payload, apply recurrence if needed
              if (this.AddShiftRecurringCheckboxValue) {
                totalAmount =
                  (parseFloat(ser.amount) || 0) *
                  (this.recurOccurencesValue || 1);
              } else {
                totalAmount = parseFloat(ser.amount) || 0;
              }

              console.log(
                `➕ Service: serviceTypeId=${ser.serviceType}, amount=${ser.amount}, recurrence=${this.recurOccurencesValue}, total=${totalAmount}`
              );
              ...
              if (usageMap.has(ser.serviceType)) {
                ...
                console.log(
                  `🔁 Updated usageMap: ${ser.serviceType} → ${updated}`
                );
              } else {
                usageMap.set(ser.serviceType, totalAmount);
                console.log(
                  `🆕 Set usageMap: ${ser.serviceType} → ${totalAmount}`
                );
              }
            });
          });
 ...
 //   console.log("Start Time further split -> hour:", startHour, "minute:", startMinute);
  ...
  handleDocumentClick(event) {
    ...
    if (!clickedInside) {
      console.log(
        "🟦 Outside click: closing all table dropdowns in participant view "
      );
      this.AddShiftAndServices = this.AddShiftAndServices.map((row) => ({
        ...row,
