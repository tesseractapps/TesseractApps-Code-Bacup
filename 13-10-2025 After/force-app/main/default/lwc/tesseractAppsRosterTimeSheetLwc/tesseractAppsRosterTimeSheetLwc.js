...
export default class Timesheet extends LightningElement {
  ...
  generateMockData() {
        ...
        shifts.forEach((shift) => {
          ...
          if (!staffMap.has(staffId)) {
            staffMap.set(staffId, {
              ...
              allShifts: [],
              shifts: [],
              shiftRows: [], // 👉 new property for UI daily display
              totalDuration: 0,
              totalMileage: 0,
              totalExpense: 0,
    ...
    this.allStaff.forEach((staffRecord) => {
      ...
      staff.shiftRows = this.weekDays.map((day) => {
        const date = day.date;
        const shiftsForDay = staff.allShifts.filter((s) => s.Date__c === date);
  ...
  handleCheckClick(event) {
    ...
    console.log(
      "this.weekStartDate in handleCheckClick : ",
      this.weekStartDate
    );
    console.log("this.weekEndDate in handleCheckClick : ", this.weekEndDate);
      ...
      .then((shifts) => {
        const filteredShifts = (shifts || [])
          ...
          .map((shift) => {
              ...
              shift.Break_Timings__r.forEach((breakRec, index) => {
                ...
                if (breakTime && breakTime.includes(":")) {
                  ...
                  if (!isNaN(hours) && !isNaN(minutes)) {
                    ...
                    console.log(
                      `➕ Running Total Break Minutes in handleCheckClick : ${totalBreakMinutes}`
                    );
                  } else {
                    console.warn(
            ...
            const durationRaw = shift.Actual_Duration__c;
            const durationCalculated = isNaN(durationRaw)
              ? 0
              : parseFloat(durationRaw);
            console.log(
              `📘 Raw Actual Duration: ${durationRaw}, Parsed: ${durationCalculated}`
            );
            ...
            return {
              ...
              extendedDuration: (() => {
                const parsed = parseFloat(
                  (shift.Extended_Duration__c || 0).toFixed(2)
                );
        ...
        // );
        console.log(
          "🔍 filteredShifts before reduce:",
          JSON.stringify(filteredShifts)
        );

        this.totalShiftDuration = filteredShifts.reduce((sum, shift, index) => {
          const duration = parseFloat(shift.totalDuration);

          console.log(`➡️ Shift ${index + 1}:`, shift);
          console.log(`🔢 Raw duration value:`, shift.totalDuration);
        ...
        console.log(
          "Total Shift Duration IN handleCheckClick:",
          this.totalShiftDuration
        );
        console.log("Total KMs IN handleCheckClick:", this.totalKms);
        console.log("Total Expenses IN handleCheckClick:", this.totalexpenses);
        console.log(
          "Total Sleepover Duration IN handleCheckClick:",
          this.sleepoverDuration
        );
        console.log(
          "Approved Duration IN handleCheckClick:",
          this.totalApprovedLoggedDuration
        );

        if (!filteredShifts || filteredShifts.length === 0) {
          console.warn("⚠️ mismatched shifts found, redirecting to handleCheckClick");
          this.handleMismatchClick({
  ...
  handleEditClick(event) {
    ...
    getAddShiftDataById({ shiftId: this.shiftID })
      .then((result) => {
        ...
        console.log("data.Log_In_Date_Time__c >>>", data.Log_In_Date_Time__c);
  ...
  /* createMockShifts() {
    ...
    this.staffDirectory.forEach((staffRecord) => {
      ...
      console.log(
        `🧮 Staff: ${staffSummary.staffName} (${staffSummary.staffId}) | ` +
        `Shift count: ${staffSummary.allShifts.length} | ` +
        `Total Duration__c sum: ${staffSummary.totalHours}`
      );
