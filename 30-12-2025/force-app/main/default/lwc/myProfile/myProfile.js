...
//manendra
   ...
   handleError(event) {
    ...
    handlePreTaxRecordFormSubmit(event) {
        event.preventDefault();
        console.log('record edit form - this is an edit operation:', this.isEditOperation);
        
        const fields = event.detail.fields;
        fields.Type__c = this.typeOfDocument;
        
        // 🆕 ADD STATUS RESET LOGIC HERE - only when saving an edit
        if (this.isEditOperation) {
            fields.Status__c = 'Pending'; // 🆕 Set status to Pending in the form submission
        }
 ...
 */
   ...
   /*  renderedCallback() {
            if (this.rejectHandlerAdded) return;
            this.rejectHandlerAdded = true;

            this.template.addEventListener("click", (event) => {
                const anchor = event.target.closest(".reject-link");
                if (anchor) {
                    const docId = anchor.dataset.id;
                    this.openRejectedCommentsFromLink(docId);
        }
            });
    } */
