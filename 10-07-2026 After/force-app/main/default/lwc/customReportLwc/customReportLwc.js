...
export default class CustomReportLwc extends LightningElement {
    ...
    initializeReports() {
        ...
        console.log('Selected Facility Id:', this.selectedFacilityIdforgenerated);
    ...
    initializeGeneratedReports() {
        ...
        getReportExportLogs({ facilityId: this.selectedFacilityIdforgenerated })
            .then(result => {
                ...
                console.log('Total Count from Apex:', this.generatedCount);

                const logs = result.logs || [];

                console.log('Logs from Apex:', logs);
                ...
                this.generatedReports = logs.map(row => {
                            ...
                            if (row.Filters_JSON__c) {
                                try {
                                    const filters = JSON.parse(row.Filters_JSON__c);
                            ...
                            const reportRow = {
                                ...
                                selectedTemplateLabel: selectedTemplateLabel   // ✅ extracted value
                            };

                            console.log('Mapped Row:', reportRow);

                            return reportRow;
                        });

                console.log('===== MAPPED GENERATED REPORTS =====');
                ...
                console.log('Initial Page Number:', this.generatedPageNumber);

                this.generatedTotalRecords = this.generatedReports.length;

                console.log('Total Records After Mapping:', this.generatedTotalRecords);
                ...
                console.log('Total Pages:', this.generatedTotalPages);
            ...
            .finally(() => {
                console.log('===== initializeGeneratedReports END =====');
            });
    }
   ...
   /*  renderedCallback() {
    ...
    renderedCallback() {
        ...
        if (this.jsPDFInitialized || this.jsPDFLoading) {
            return;
        }
        ...
        .then(() => {
            ...
            if (jsPDF.API.autoTable) {
                ...
                console.error(
                    "autoTable loaded but not attached"
                );
            }
    ...
    async loadTemplates() {
        ...
        try {
            console.log('this.selectedFacilityId >>>>', this.selectedFacilityIdforgenerated);
    ...
    applyReportType(reportTypeValue) {
        ...
        if (this.reportTypeValue === 'Participant_Service_Delivery') {            
            ...
            console.log('Entered Participant_Service_Delivery block');
            console.log('reportTypeValue =>', this.reportTypeValue);
            console.log('startDate =>', this.startDate);
            console.log('endDate =>', this.endDate);
            console.log('selectedDateRange =>', this.selectedDateRange);
            console.log('selectedParticipantIds =>', this.selectedParticipantIds)
        }
    ...
    async handleGenerateReport() {
        ...
        if (this.reportTypeValue === 'SHIFT') {
            ...
            console.log('===== Participant Service Delivery Report =====');
            console.log('Facilities =>', JSON.stringify(this.selectedFacilityIds));
            console.log('Participants =>', JSON.stringify(this.selectedParticipantIds));
    ...
    async generateParticipantServiceDeliveryReport() {
        ...
        try {
            ...
            console.log('facilityIds =>', JSON.stringify(this.selectedFacilityIds));
            console.log('participantIds =>', JSON.stringify(this.selectedParticipantIds));
            console.log('startDate =>', this.startDate);
            console.log('endDate =>', this.endDate);
            ...
            console.log(
                'Participant Service Delivery Result =>',
            ...
            this.participantServiceRecords = result.map(rec => {
                return {
                    Id: rec.Id,
                    ...
                    facility:
                        rec.ShiftwithStaff__r?.Facility__c ||
                    ...
                    resourceName:
                        rec.ShiftwithStaff__r?.Status__c === 'Unassigned'
                            ? 'N/A'
                            : (rec.Resource_Name__c || '-'),

                    startTime: rec.Start_Time_Formula__c || '-',

                    endTime: rec.End_Time_Formula__c || '-',

                    qty: Number(rec.Qty__c ?? 0) === 0
                        ? '0'
                        : Number(rec.Qty__c).toFixed(2),

                    status:
                        rec.ShiftwithStaff__r?.Shift_Rejected__c
                            ? 'Rejected'
            ...
            console.log(
                'participantServiceRecords =>',
                JSON.stringify(this.participantServiceRecords)
            );
            ...
            this.reportSummary = {
                ...
                period:
                    `${this.formatDateDDMMYYYY(this.startDate)} - ${this.formatDateDDMMYYYY(this.endDate)}`
            };
            ...
            console.log(
                '===== generateParticipantServiceDeliveryReport END ====='
            );

        } catch(error) {

            console.error(
                'generateParticipantServiceDeliveryReport Error =>',
                JSON.stringify(error)
            );

            this.showToast(
                'Error',
                error?.body?.message || 'Error loading report',
                'error'
            );
        }
    }
    ...
    updateParticipantServicePagination() {
        ...
        const start =
            (this.participantServicePageNumber - 1) *
            this.participantServicePageSize;

        const end =
            start + this.participantServicePageSize;

        this.participantServiceRecordsToDisplay =
            this.participantServiceAllRecords.slice(start, end);

        this.participantServiceTotalPages =
            Math.ceil(
                this.participantServiceTotalRecords /
                this.participantServicePageSize
            ) || 1;

        this.participantServiceDisableFirst =
            this.participantServicePageNumber <= 1;

        this.participantServiceDisableLast =
    ...
    async generateRejectedShiftReport() {
        ...
        try {
            ...
            console.log('Resolved staffIds →', staffIds);

            /* ---------------------------------------
            2️⃣ Input Parameters
            --------------------------------------- */

            console.log('Apex Parameters →', {
                startDate: this.startDate,
                endDate: this.endDate,
                orgId: this.orgid,
                facilityIds: this.selectedFacilityIds,
            ...
            console.log('Apex Result Length →', result?.length);

            if (!Array.isArray(result)) {
                console.error('Invalid result received from Apex');
                this.showToast('Error', 'Invalid rejected shift data', 'error');
                return;
            }

            /* ---------------------------------------
            4️⃣ Store raw result
            --------------------------------------- */
            ...
            /* ---------------------------------------
            5️⃣ Build staff search list
            --------------------------------------- */

            if (this.isRejectedShiftReport) {

                console.log('Building rejected staff search list...');

                this.buildRejectedStaffList(this.rejectedShiftsRaw);

                console.log('Staff search list built successfully');
            }

            /* ---------------------------------------
            6️⃣ Normalize records
            --------------------------------------- */

            this.rejectedFilteredRecords = this.rejectedShiftsRaw.map(w =>
                this.normalizeRejectedShift(w)
            );

            console.log(
                'Normalized Rejected Records →',
                JSON.stringify(this.rejectedFilteredRecords, null, 2)
            );
            ...
            console.log('Rejected Metrics Calculated');

            /* ---------------------------------------
            8️⃣ Empty state check
            --------------------------------------- */

            if (!this.rejectedFilteredRecords.length) {

                console.warn('No rejected shifts found');
            ...
            console.log('Pagination Setup →', {
                pageNumber: this.rejectedPageNumber,
                pageSize: this.rejectedPageSize,
                totalRecords: this.rejectedFilteredRecords.length
            });
            ...
            console.log('Modal Visible →', this.isStaffBasedReportshowModal);
    ...
    console.log('===== PDF HEADER START =====');

    const pageWidth = doc.internal.pageSize.getWidth();
    console.log('Page Width:', pageWidth);

    console.log('Org Name:', this.orgname);
    console.log('Org Address:', this.statePostal);
    console.log('Org Logo Available:', this.orgLogo ? true : false);


    // ==========================
    // LOGO (FIT 40x20 NO STRETCH)
    // ==========================
    if (this.orgLogo) {
        try {
    ...
    console.log('Org X Position:', orgX);
...
getOrganisationCSVHeader(){
    ...
    async handleTemplateChange(event) {
        ...
        // Get selected option (value + label)
        const selectedOption = this.templatesforShiftReport.find(
            option => option.value === value
        );

        const label = selectedOption ? selectedOption.label : null;

        // Store label if needed
        this.selectedTemplateLabel = label;
    ...
    handleExportTypeChange(event) {
        ...
        if (value === 'PDF') {
            this.isPDF = true;
            console.log('PDF selected → isPDF = true');
        }
        else if (value === 'EXCEL') {
            this.isCSV = true;
            console.log('CSV selected → isCSV = true');
        }

        console.log('Updated selectedExportType:', this.selectedExportType);
        console.log('isPDF:', this.isPDF);
        console.log('isCSV:', this.isCSV);
    ...
    exportExcelFromTemplate(templateKeys, rows) {
       ...
       // ============================
        // ORG NAME + ADDRESS HEADER
        // ============================

        let csv = this.getOrganisationCSVHeader();

        console.log('CSV Org Header Added:', {
        ...
        if (this.reportTypeValue === 'REJECTED_SHIFT') {
            ...
            filePrefix = 'Participant_Service_Delivery_Report';
        }
        else if (this.reportTypeValue === 'OVER_EFF') {
    ...
    async convertPdfToBase64FromTemplate(templateKeys, rows) {
        ...
        console.log('jsPDF is initialized');
        ...
        console.log('PDF Body Rows Generated =>', body.length);
        ...
        console.log('autoTable rendered successfully');
        ...
        console.log('File Name =>', `${filePrefix}_${today}.pdf`);

        this.base64 = base64;
        this.fileName = `${filePrefix}_${today}.pdf`;

        console.log('Final Response =>', this.base64);

        const response = {
            fileName: `${filePrefix}_${today}.pdf`,
            base64Data: base64
        };
    ...
    /* exportPdfFromTemplate(templateKeys, rows) {
        ...
        if (this.reportTypeValue === 'REJECTED_SHIFT') {
            ...
            title = 'Participant_Service_Report';
        }
        else if (this.reportTypeValue === 'OVER_EFF') {
        ...
        else if (this.reportTypeValue === 'Participant_Service_Delivery') {Participant_Service_Delivery
            title = 'Participant_Service_Report';
        }
    ...
    exportPdfFromTemplate(templateKeys, rows) {
        ...
        console.log('Report Type =>', this.reportTypeValue);
        console.log('Template Keys =>', JSON.stringify(templateKeys));
        console.log('Rows Count =>', rows?.length);
        ...
        console.log('jsPDF Initialized =>', this.jsPDFInitialized);
        ...
        console.log('autoTable Available =>', !!jsPDF.API.autoTable);

        if (!jsPDF.API.autoTable) {

            console.error('autoTable not attached.');
        ...
        const headers = templateKeys.map(key => {

            const label = this.templateKeyToLabelMap?.[key] || key;
        ...
        console.log('Final PDF Headers =>', headers);

        console.log('Preparing PDF body...');
        ...
        console.log('Total Body Rows =>', body.length);

        if (body.length > 0) {
            console.log('First PDF Body Row =>', body[0]);
        }
        ...
        if (this.reportTypeValue === 'REJECTED_SHIFT') {
            ...
            title = 'Participant Service Report';
        }
        else if (this.reportTypeValue === 'OVER_EFF') {
            title = 'Under / Over Efficiency Report';
        }
        ...
        console.log('Writing title to PDF...');
        ...
        if (this.reportTypeValue === 'REJECTED_SHIFT') {
            ...
            filePrefix = 'Participant_Service_Report';
        }
        else if (this.reportTypeValue === 'OVER_EFF') {
            filePrefix = 'Under_Over_Efficiency_Report';
        }
        ...
        const fileName = `${filePrefix}_${today}.pdf`;
    ...
    handleStaffSearch(event) {
        ...
        console.log('Staff List Length:', this.staffList?.length);
        ...
        console.log('showStaffDropdown:', this.showStaffDropdown);
    }
    ...
    async handleBack() {
        ...
        /* Reset Participant Service Delivery */
        this.participantServiceAllRecords = [];
        this.participantServiceRecordsToDisplay = [];

        /* Reset Common Pagination */
        this.totalRecords = 0;
        this.totalPages = 0;
        this.pageNumber = 1;

        /* Reset Shift Pagination */
        this.bDisableFirst = true;
        this.bDisableLast = true;

        /* Reset Rejected Pagination */
        this.rejectedPageNumber = 1;
        ...
        /* Reset Reimbursement Pagination */
        this.reimbursementPageNumber = 1;
        ...
        /* Reset Participant Service Delivery Pagination */
        this.participantServicePageNumber = 1;
        ...
        console.log('All filters, records, pagination and report state cleared successfully');
    }

    async handleBack1() {
        await this.handleBackToBuilder();
    ...
    handleToggleFavorite() {
        ...
        if (this.isFavorite) {
            ...
            .then(result => {
                ...
                console.log('✅ Export Log Updated Successfully');

                console.log('Updated Record Id =>', result);
    ...
    handleFavoriteClick(event) {
        ...
        if (!reportId) {
            console.warn('⚠️ No reportId found in dataset');
            return;
        }

        /* =====================================================
        STEP 1: Find Selected Favorite
        ===================================================== */
        ...
        if (!selectedReport) {
            console.warn('⚠️ No matching favorite report found!');
            return;
        }
        ...
        console.log('Saved Filters Object:',
            JSON.parse(JSON.stringify(filters))
        );

        /* =====================================================
        STEP 2: Apply Report Type
        ===================================================== */
        ...
        console.log('Report Type Applied -> Current reportTypeValue:', this.reportTypeValue);

        /* =====================================================
        STEP 3: Restore Filters
        ===================================================== */
        ...
        console.log('Selected Roles Restored:', this.selectedRoles);
        ...
        console.log('Employment Type:', this.selectedEmploymentType);
        console.log('Location:', this.selectedLocation);
        console.log('Start Date:', this.startDate);
        console.log('End Date:', this.endDate);
        console.log( 'selectedParticipantIds ==>', JSON.stringify(this.selectedParticipantIds, null, 2));


        this.selectedStaffName = filters.staffName || null;
        this.selectedStaffId = filters.staffId || null;

        console.log('Selected Staff Name:', this.selectedStaffName);
        ...
        console.log('Staff Status:', this.selectedstaffStatus);
        console.log('Date Range:', this.selectedDateRange);
        console.log('Reimbursement Status:', this.statusValue);

        this.reportTypeValue = filters.reportType || null;

        console.log('Report Type From Filters:', this.reportTypeValue);

        /* =====================================================
        STEP 4: Restore Staff Type
        ===================================================== */

        if (filters.isAllStaff) {
            this.selectedStaffType = 'all';
            console.log('Staff Type Restored -> ALL STAFF');
        }
        else if (filters.isIndividualStaff) {
            this.selectedStaffType = 'individual';
            console.log('Staff Type Restored -> INDIVIDUAL STAFF');
        }
        else {
            this.selectedStaffType = null;
            console.log('Staff Type Restored -> NONE');
        }

        console.log('selectedStaffType:', this.selectedStaffType);

        /* =====================================================
        STEP 5: Restore Facilities
        ===================================================== */
        ...
        console.log('Facilities Restored:', this.selectedFacilityId);
        console.log('selectedFacilityIds:', this.selectedFacilityIds);
        ...
        .then(result => {
            ...
            if (!result || result.length === 0) {
                ...
                console.log('Mapped filteredStaffList →', this.filteredStaffList1);
            }
        ...
        .then(result => {
            ...
            console.log('Participant Options →', JSON.stringify(this.participantsOptions));

        })
        .catch(error => {
            console.error('Participant Apex Error →', JSON.stringify(error));
        });

        /* =====================================================
        STEP 6: Sync Role Checkboxes
        ===================================================== */
        ...
        /* =====================================================
        STEP 7: Handle Date Range
        ===================================================== */

        this.isCustomRange = this.selectedDateRange === 'CUSTOM';

        console.log('Date Range Mode:',
            this.isCustomRange ? 'CUSTOM RANGE ENABLED' : 'PRESET RANGE'
        );

        /* =====================================================
        STEP 8: Reload Staff (If Roles Exist)
        ===================================================== */
        ...
        if (this.selectedRoles.length > 0) {
            ...
            console.log('No roles selected → Staff fetch skipped');
        }

        /* =====================================================
        STEP 9: Update Favorite UI
        ===================================================== */

        if (this.selectedFacilityIds.length > 0) {
            this.processFacilities(this.selectedFacilityIds);
        }
        ...
        console.log('Favorite UI Updated:', {
            ...
            favoriteText: this.favoriteText
        });
        

        /* =====================================================
        STEP 10: Update Generate Button
        ===================================================== */
    ...
    processFacilities(selectedFacilityId) {

        console.log('===== processFacilities START =====');
        console.log('Facility IDs →', selectedFacilityId);

        if (!selectedFacilityId || selectedFacilityId.length === 0) {
            console.warn('No Facility IDs found');
            return;
        }

        this.selectedFacilityIds = [...selectedFacilityId];
        ...
        getRolesByFacilities({ facilityIds: this.selectedFacilityIds })
            .then(result => {
        ...
        console.log('===== processFacilities END =====');
    }
    ...
    createExportLog(exportType, reportType) {
        ...
        const filtersPayload = {
            ...
            status: this.selectedstaffStatus,
            facility: this.selectedFacilityIds,
            selectedParticipantId: this.selectedParticipantIds,   // <-- Add this
            reportType: this.reportTypeValue,
            reportStatus: this.selectedstaffStatus,
            reportrembursmentStatus: this.statusValue,
        ...
        const filtersJson = JSON.stringify(filtersPayload);
        console.log('filtersPayload ===> ', JSON.stringify(filtersPayload, null, 2));
        const filterKey = btoa(filtersJson);
        ...
        .then(result => {
            ...
            console.log('Returned Log Id =>', result); // 👈 ID will come here
    ...
    /* handleDownload(event) {
        ...
        if (exportType === 'EXCEL') {
            ...
            if (!this.isEmailMode) {
                ...
                this.convertCsvToBase64FromTemplate(templateKeys, rows);
            }
            ...
            if (!this.isEmailMode) {
                ...
                this.convertPdfToBase64FromTemplate(templateKeys, rows);
            }
    ...
    handleDownload(event) {
        ...
        console.log('Report Type =>', this.reportTypeValue);
        console.log('Selected Export Type =>', this.selectedExportType);
        console.log('Is Email Mode =>', this.isEmailMode);
        ...
        console.log('Template Keys BEFORE normalize =>', JSON.stringify(templateKeys));

        if (!templateKeys.length) {
            console.error('No template columns found.');
            this.showToast('Error', 'Selected template has no columns.', 'error');
            return;
        }
        ...
        console.log('Template Keys AFTER normalize =>', JSON.stringify(templateKeys));

        let rows;

        console.log('Determining report data...');

        if (this.reportTypeValue === 'REJECTED_SHIFT') {

            console.log('Loading REJECTED_SHIFT records');
            rows = this.rejectedFilteredRecords;
            console.log('Rejected Records Length =>', rows?.length);

        }
        else if (this.reportTypeValue === 'REIMBURSEMENT') {

            console.log('Loading REIMBURSEMENT records');
            rows = this.reimbursementAllRecords;
            console.log('Reimbursement Records Length =>', rows?.length);
            ...
            rows = this.staffAllRecords;
            console.log('Staff Records Length =>', rows?.length);

        }
        else if (this.reportTypeValue === 'PARTICIPANT') {

            console.log('Loading PARTICIPANT records');
            rows = this.participantAllRecords;
            console.log('Participant Records Length =>', rows?.length);

        }
        else if (this.reportTypeValue === 'Participant_Service_Delivery') {

            console.log('Loading Participant Service Delivery records');
            rows = this.participantServiceAllRecords;
            console.log('Participant Service Delivery Records Length =>', rows?.length);

        }
        else if (this.reportTypeValue === 'OVER_EFF') {

            console.log('Loading UNDER / OVER EFFICIENCY records');
            rows = this.underOverAllRecords;
            console.log('Under/Over Efficiency Records Length =>', rows?.length);

        }
        else {

            console.log('Loading DEFAULT filtered records');
            rows = this.filteredRecords;
            console.log('Filtered Records Length =>', rows?.length);
        ...
        if (rows?.length > 0) {
            console.log('First Row Preview =>', JSON.stringify(rows[0]));
        }
        ...
        console.log('Export Type Decision =>', exportType);

        if (exportType === 'EXCEL') {

            console.log('Export format => EXCEL');

            if (!this.isEmailMode) {

                console.log('Calling exportExcelFromTemplate()');
                console.log('Template Keys =>', templateKeys);
                console.log('Rows Count =>', rows.length);
                ...
                console.log('Calling convertCsvToBase64FromTemplate()');
            ...
            if (!this.isEmailMode) {
                ...
                console.log('Calling exportPdfFromTemplate()');
                console.log('Template Keys =>', templateKeys);
                console.log('Rows Count =>', rows.length);
                ...
                console.log('Email Mode Enabled');
                console.log('Calling convertPdfToBase64FromTemplate()');
    ...
    async handlePreview(event) {
        ...
        console.log('Assigned Report Metadata =>', {
            exportType: this.selectedExportType,
            reportName: this.generatedReportName,
            dateRange: this.generatedReportDateRange,
            generatedDate: this.generatedReportDate,
            generatedBy: this.generatedReportUser
        });
        ...
        /* ===============================
        APPLY REPORT TYPE UI
        =============================== */

        this.applyReportType(this.reportTypeValue);

        /* ===============================
        LOAD TEMPLATE & APPLY IT
        =============================== */
    ...
    /* applyTemplateById(templateId) {
        ...
        if (tpl && tpl.json) {
            ...
            console.warn('Template not found or no JSON');
    ...
    applyTemplateById(templateId) {
        ...
        console.log('Incoming templateId =>', templateId);
        console.log('Current reportTypeValue =>', this.reportTypeValue);
        console.log('Sections BEFORE =>', JSON.stringify(this.sections));
        console.log('Templates =>', JSON.stringify(this.templatesforShiftReport));
        ...
        console.log('Template found =>', JSON.stringify(tpl));
        ...
        if (tpl && tpl.json) {
            ...
            try {
                ...
                console.log('Parsed Template JSON =>', JSON.stringify(parsed));
                ...
                console.log('Calling applyTemplate...');

                this.applyTemplate(parsed);

                console.log('Sections AFTER applyTemplate =>',
                    JSON.stringify(this.sections)
                );
                ...
                console.log(
                    'Checked Sections =>',
                    JSON.stringify(
                        this.sections.filter(section => section.checked)
                    )
                );
    ...
    handleToggleFavoriteforGenerated(event) {
        ...
        .then(() => {
            ...
            // IMPORTANT: reassign array to trigger LWC re-render
    ...
    async handleSendEmail(event) {
        ...
        console.log('Send Email clicked for report =>', reportId);
        ...
        this.selectedReportId = reportId; // ⭐ store id

        console.log('this.reportEmailSend >>>>', this.reportEmailSend);

        this.invoiceEmailFlag = true; // open modal
    }
    ...
    async handleSendEmailtoStaff() {
        ...
        try {
            ...
            if (!this.selectedReportId) {
                console.warn('No Report Id found');
                return;
            }

            /* ---------- CALL handleDownloadReport ---------- */
            ...
            console.log('Calling handleDownloadReport with reportId =>', this.selectedReportId);
    ...
    getReportFilePrefix() {
        ...
        switch (this.reportTypeValue) {
            ...
            case 'REJECTED_SHIFT':
                return 'Rejected_Shift_Report';

            case 'REIMBURSEMENT':
                return 'Reimbursement_Report';
            ...
            case 'OVER_EFF':
                return 'Under_Over_Efficiency_Report';

            case 'Participant_Service_Delivery':
                return 'Participant_Service_Report';
    ...
    handleSelectAllRoles() {
        ...
        console.log('All roles selected →', this.selectedRoles);
    }
    ...
    handleChangeforparticipant(event) {
        ...
        console.log('Selected Participant IDs →', JSON.stringify(participantIds));

        this.selectedParticipantIds = Array.isArray(participantIds)
            ? [...participantIds]
            : [participantIds];

        console.log(
            'Stored Participant IDs →',
            JSON.stringify(this.selectedParticipantIds)
        );

        console.log('Participant Count →', this.selectedParticipantIds.length);
