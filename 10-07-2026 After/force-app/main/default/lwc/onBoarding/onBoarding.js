...
@track isUserTypeDisabled = true;
  roleOptions = [
        { label: 'Admin', value: 'Portal Account Partner Manager' },
...
async handleCreateRecords() {
  ...
  // Build a plain JSON payload (no proxies/getters) from target rows only
   ...
   const payload = targetRows.map(r => {
      ...
      // 🚫 BYPASS: Participant should NOT become Portal Account Partner Manager
...
obBuildFlatPreview(raw) {
  ...
  const labelMap = {
    ...
    postalCode: 'Postal Code', country: 'Country',  participantType: 'Who is receiving the NDIS service?'
  };
