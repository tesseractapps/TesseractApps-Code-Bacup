...
export default class VBasChildPdf extends LightningElement {
    @api childRecordId;
    ...
    connectedCallback(){
        ...
        // Check if responseDataFromParent has data before generating the PDF
    ...
    generatePayroll() {
        ...
        //  doc.text('Document ID:         '+this.responseDataFromParent[0].Document_Id__c,10, 56);
            ...
            doc.text('W1', 140, 50);
            doc.text('$'+this.responseDataFromParent[0].Total_Salary_Wages_and_Other_Payments__c+'.00', 182, 50);
            console.log('toal salary >>'+this.responseDataFromParent[0].Total_Salary_Wages_and_Other_Payments__c);
            doc.line(10, 53, 200, 53); 
            doc.text('Amount withheld from payments shown at W1', 10, 60);
            doc.text('W2', 140, 60);
            doc.text('$'+this.responseDataFromParent[0].Amount_Withheld_from_Payments_Shown__c+'.00', 182, 60);
            console.log('Amount withheld payment >>'+this.responseDataFromParent[0].Amount_Withheld_from_Payments_Shown__c);
            doc.line(10, 63, 200, 63);  
            doc.text('Amount withheld where no ABN is quoted', 10, 70);
            doc.text('W3', 140, 70);
            doc.text('$'+this.responseDataFromParent[0].Amount_Withheld_Where_no_ABN_is_Quoted__c+'.00', 182, 70);
            console.log('Amount withheld ABN >>'+this.responseDataFromParent[0].Amount_Withheld_Where_no_ABN_is_Quoted__c);
            doc.line(10, 73, 200, 73); 
            doc.text('Other amounts withheld', 10, 80);
            doc.text('W4', 140, 80);
            doc.text('$'+this.responseDataFromParent[0].Other_Amounts_Withheld__c+'.00', 182, 80);
            console.log('other amounts >>'+this.responseDataFromParent[0].Other_Amounts_Withheld__c);
            doc.line(10, 83, 200, 83);
            this.paygTotalAmountW5 = this.responseDataFromParent[0].Amount_Withheld_from_Payments_Shown__c +this.responseDataFromParent[0].Amount_Withheld_Where_no_ABN_is_Quoted__c+this.responseDataFromParent[0].Other_Amounts_Withheld__c;
