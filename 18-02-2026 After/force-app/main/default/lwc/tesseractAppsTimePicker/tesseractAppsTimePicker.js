...
export default class TesseractAppsTimePicker extends LightningElement {
  ...
  connectedCallback() {
    ...
    this.setFilteredTimes(this.times15);
  ...
  generateTimes(interval) {
    ...
    this.times15 = [...amTimes15, ...pmTimes15];
  }
...
if (
  ...
  if (!raw && (event.key === "Backspace" || event.key === "Delete")) {
    this.searchKey = "";
    this.filteredTimes = this.times15;
    this.isDropdownOpen = true;
    this.highlightIndex = -1;
...
handleOutsideClick = (event) => {
  ...
  handleClear() {
    this.searchKey = "";
    this.filteredTimes = this.times15; // Reset to 15-min intervals
    this.isDropdownOpen = true;
    this.highlightIndex = -1;
    this.setFilteredTimes(this.filteredTimes);
