    ...
    orgDetails()
      .then((result) => {
        if (result && result.Id) {
          ...
          console.log("📌 Received Org ID in Chatter:", this.orgid);
          this.fetchParticipantsByOrgId(this.orgid); // your existing method
...
processFormResponses(data) {
  ...
  this.formResponses = data.map((response, index) => {
    console.log(`🧾 [${index}] Form Response RAW:`, response);
    console.log(
      `🧾 [${index}] AWS_Url__c:`,
      response?.AWS_Url__c,
      "| AWS_Json__c:",
      response?.AWS_Json__c
    );
    ...
    const mapped = {
      ...
      completionBarStyle: `width:${Math.max(0, Math.min(100, score.pct))}%`,
      CreatedByName:
        response && response.CreatedBy && response.CreatedBy.Name
          ? response.CreatedBy.Name
          : "—"
    };
  ...
  console.log("📦 allFormResponses snapshot:", this.allFormResponses);

  // 4) Compute + store uniques and tabs
  const rawNames = this.allFormResponses.map((r) =>
    this._normalizeName(r?.Name__c)
  );
  this.uniqueNames = Array.from(new Set(rawNames));
  console.log("📑 Unique form names:", this.uniqueNames);
  ...
  console.log("📊 Filtered form responses:", this.filteredFormResponses);

  this.updatePaginatedFormResponses();
  this.fetchFormSignatureStatuses(); 


  // new: build groups in parallel (safe even if you don't use them)
...
if (cell.field?.dataType === "Dropdown Field") {
 ...
 closeView() {
  ...
  updateStepVisibleRows() {
    ...
    const page = JSON.parse(JSON.stringify(raw)); // clean refs
  ...
  handleInputChange(event) {
    ...
    // Detect LWC rich text (tag name) or your class hook
    const isRichText =
    ...
    // === NEW: derive newValue with RTE support & sanitization ===
    let newValue;
    if (inputType === "checkbox") {
      newValue = !!event.target.checked;
    } else if (isRichText) {
      const raw = event?.detail?.value ?? ""; // RTE value comes from event.detail.value
      // Strip <img ...> tags (blocks pasted/HTML images)
      let cleaned = raw.replace(/<img\b[^>]*>/gi, "");
      // Extra-strict: also remove inline CSS data-URI images if any slipped in
      cleaned = cleaned.replace(/url\(["']?data:image\/[^"')]+["']?\)/gi, "");
      newValue = cleaned;

      // If anything was removed, immediately reflect back in the editor UI
      if (cleaned !== raw) {
        // Target the exact editor instance by data-id (matches your markup)
    ...
    // Keep your date normalization (non-RTE text inputs only)
      ...
      for (let rIdx = 0; rIdx < page.length; rIdx++) {
        ...
        for (let cIdx = 0; cIdx < cells.length; cIdx++) {
          ...
          if (String(cell?.id) === cellId && cell?.field) {
            ...
            // ✅ Default: Text, date, time, number, rich text, etc.
            else {
              cell.field.value = newValue; // for RTE this is the sanitized HTML
            }

            console.log(
              `📌 stepPagedRows write @ page=${pIdx} row=${rIdx} col=${cIdx} | "${oldValue}" → "${cell.field.value}"`
            );
    ...
    if (!matched) {
      console.warn(
        `⚠️ No matching cell for id="${cellId}" found across stepPagedRows.`
      );
    }
    ...
    if (matched) {
      this.tableRows = (this.tableRows || []).map((row, rIdx) => {
        const cells = (row?.cells || []).map((c, cIdx) => {
          if (String(c?.id) !== cellId) return c;
          console.log(
            `🪟 tableRows mirror @ row=${rIdx} col=${cIdx} | "${c?.field?.value}" → "${newValue}"`
          );
          return { ...c, field: { ...(c.field || {}), value: newValue } };
        });
        return { ...row, cells };
  ...
  updateCellOnCurrentPage(cellId, updater) {
    ...
    // reassign with fresh refs so LWC definitely re-renders
    this.stepPagedRows = pages.map((p, idx) =>
      idx === i ? JSON.parse(JSON.stringify(updatedPage)) : p
    );
...
await generateUploadAndUpdateAws({
  ...
  existingAwsJson: existingAwsJson
        ...
        this.showToast("Success", "Form Updated & PDF Uploaded!", "success");
        this.isFormSelected = false;
        this.isViewModeON = true;
        this.isEditing = false;
        this.isExpandview = false;
  ...
  saveFormResponse({
    ...
    .then(async (resp) => {
      ...
      this.showToast("Success", "Form Submitted & PDF Uploaded!", "success");
      this.isFormSelected = false;
      this.isViewModeON = true;
      this.isEditing = false;
      this.isExpandview = false;
 ...
 handleAwsUploadComplete(evt) {
  ...
  // IMPORTANT: render ONLY ONCE, and DO NOT save inside this method
...
if (isHeaderLike) {
  ...
  // Try the most common fields that might contain the submitted form payload
  getResponsePayloadObject(resp) {
    const candidates = [
    ...
    // Sometimes the wire returns the object already (rare)
    if (resp && typeof resp === "object" && (resp.tableRows || resp.fields)) {
      return resp;
    }
    return null;
  }
