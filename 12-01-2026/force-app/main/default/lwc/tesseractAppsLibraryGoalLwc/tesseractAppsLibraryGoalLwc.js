...
export default class TesseractAppsLibraryGoalLwc extends LightningElement {
    ...
    handleChange(event){
        ...
        switch (fieldName) {
            case 'levelId':
             ...
             case 'facility':
                this.facilityValue =[];
                this.selectedFacility = fieldValue;
                this.facilityValue.push(this.selectedFacility);
    ...
    handleSubmit(event){
       ...
       // console.log('existingGoal  IN SUBMIT: '+JSON.stringify(existingGoal));
        ...
        fields.Facility__c= this.selectedFacility;
          ...
          if (userType === "NDIS Org Admin") {
            getFacilityData()
              .then((response) => {
                ...
                console.log("facilityOptions length in getFacilityData => " + JSON.stringify( this.facilityOptions.length)); 
                if (this.facilityOptions.length > 0) {
                    this.facilityValue =[];
                    this.facilityVal = this.facilityOptions[0].value;
                    this.selectedFacility = this.facilityVal;
            ...
            getFacilityCurrentUser().then((result) => {
              ...
              console.log("facilityOptions length in getFacilityData  getFacilityCurrentUser=> " + JSON.stringify( this.facilityOptions.length)); 
              if (this.facilityOptions.length > 0) {
                this.facilityValue =[];
                this.facilityVal = this.facilityOptions[0].value;
                this.selectedFacility = this.facilityVal;
    ...
    fetchRoles(){
        ...
        fetchBulkRoles({ facilityIDList: this.facilityValue })
            .then(facRoles => {
                ...
                this.organisationRoles = roleNames.map(role => ({
                    label: role,
                    value: role
                }));

                // Default selected value
                //this.orgRoles = roleNames.length > 0 ? roleNames[0] : null;

                
                //console.log("Selected role:", this.orgRoles);
                console.log(" this.OrgNisationRoles length   in fetchRoles " + this.organisationRoles.length);
                console.log(" this.OrgNisationRoles  in fetchRoles " + JSON.stringify(this.organisationRoles));
