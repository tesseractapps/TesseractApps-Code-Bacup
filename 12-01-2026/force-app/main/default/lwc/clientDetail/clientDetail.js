...
export default class ClientDetail extends NavigationMixin(LightningElement) {
  ...
  async fetchRolesForStaff() {
    try {
      ...
      console.log(
        "ðŸ”Ž paginatedData after mapping:",
  ...
  get groupedStaffRoleData() {
    ...
    //  console.log('ðŸŸ¢ Role State Summary:', JSON.stringify(debugLogs, null, 2));
  ...
  handleRoleToggle(event) {
    ...
    // ðŸ” Get the latest version: from modified map if exists, otherwise from original list
    const originalStaff =
    ...
    // ðŸ›  Clone the staff and their roleAssignments
    ...
    // âœ… Save back into modified map
    this.modifiedStaffMap = {
      ...this.modifiedStaffMap,
  ...
  wiredClient(result) {
      ...
      if (data) {
          ...
          // ====== ROLE HANDLING ======
          if (data[0].Participant_Staff_Associations__r &&
              data[0].Participant_Staff_Associations__r.length > 0) 
          {
              this.role = data[0].Participant_Staff_Associations__r[0].Role__c;
          ...
          if (data[0].Participant_Contacts__r) {
              this.contactList1 = data[0].Participant_Contacts__r.map((c) => ({
                  id: c.Id,  // keep actual Salesforce Id
           ...
           if (data[0].Participant_Facilities__r) {
            this.selctedMultipleFcailityValues = data[0].Participant_Facilities__r.map(f => f.Facility__c)
                        .filter(Boolean);
                this.fetchMultiFaciltyOptions();  
                
                 this.activeFaciltyDisplay= data[0].Participant_Facilities__r.map(f => f.Facility__r?.Name)
                        .filter(Boolean)
                        .join(', ')
          ...
          console.log("Mapped Contact List => ", JSON.stringify(this.contactList1));

          // ====== BUILD CONTACT TYPE OPTIONS ======
          // 1. Start with your base types EXCLUDING the special last item
          let baseOptions = [
              { label: 'Primary', value: 'Primary' },
              { label: 'Secondary', value: 'Secondary' },
              { label: 'Emergency', value: 'Emergency' }
          ...
          // Special last option (kept separate)
          const addNewOption = { label: 'Add New', value: 'Add New Contact Type' };

          // 2. Extract unique backend types
          ...
          // 3. Merge backend types into base list (avoid duplicates)
          ...
          // 4. Sort alphabetically if needed (optional)
          // baseOptions.sort((a, b) => a.label.localeCompare(b.label));

          // 5. Always append "Add New Contact Type" as the last element
      ...
      try {
          ...
          if (staffRecID && this.fileName?.length > 0) {
              try {
                  ...
                  console.error("âŒ File Upload Error:", fileError);
                  this.dispatchEvent(
                      new ShowToastEvent({
          ...
          // --------------------------------
          // ðŸ‘¥ UPDATE STAFF ASSIGNMENTS (SAFE)
          // --------------------------------
          ...
          if (safeAssignmentsJSON) {
              try {
                  ...
                  console.error("âŒ Staff assignment error:", staffError);
                  this.dispatchEvent(
                      new ShowToastEvent({
                  ...
                  return; // â— STOP further actions
              }
          } else {
              console.warn("âš ï¸ No staff assignments to update â€” skipping updateStaffAssignments()");
          }

          // --------------------------------
          // ðŸ“ž UPDATE PARTICIPANT CONTACTS (SAFE)
          // --------------------------------
          ...
          // --------------------------------
          // ðŸ“ SUBMIT PARENT lightning-record-edit-form
          // --------------------------------
          ...
          console.error("âŒ General Error in handleSuccess:", err);
          this.dispatchEvent(
              new ShowToastEvent({
  ...
  handleSubmit(event) {
    ...
    if (!this.facilityId) {
      facilityInput.reportValidity(); // shows â€œComplete this fieldâ€
      return;
    }
  ...
  get showPreferredStaffClass() {
    ...
    handleSearchChange(event) {
        ...
        // âœ… Always show dropdown when typing or clearing
    ...
    handleSelect(event) {
        ...
        console.log('ðŸ“Œ Selected Nationality via onclick:', selectedValue);
        ...
        console.log('ðŸŒ€ handleSelect complete. Nationality:', this.Nationality);
    }
