    ...
    getClientDetails({ searchTerm: search })
        .then(result => {
            ...
            this.clientList = result.map(r => {
                ...
                const quoteId = r.NDIS_Quotes__r?.[0]?.Id;
                ...
                return {
                    ...
                    displayName,
                    fullName: displayName, // backward compatibility
                    initials,

                    // 🔑 NDIS number (explicit)
...
//onst stateLabel = STATE_CODE_TO_LABEL[resolvedStateCode] || [this.state] || this.quoteStatethis.quoteState;
 ...
 await this.waitForPdfReady();
  ...
  //  this.logScreenState('New Contacts Tab');
    }
...
// }
   ...
   /*  if (!this.quotePdfUrl) {
        this.showToast(
            ...
            'Please save the quote to generate the PDF.',
            'warning'
        );
        return;
    } */

    // ✅ HTTPS URL → iframe SAFE
...
handleSendViaTsign() {
  ...
  // 2) Determine Quote Id
  const quoteId =  this.quoteHeaderId; // use whichever you have
  if (!quoteId) {
    this.showToast("Error", "Quote Id not found.", "error");
    return;
  }
