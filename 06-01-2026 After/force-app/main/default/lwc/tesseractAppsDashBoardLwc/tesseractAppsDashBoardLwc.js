...
`;
    ...
    if(this.supportCoordinator === false){
      ...
      this.setActiveMenu("supportCoordinator");
    }
    // this.isLoading = false;
    setTimeout(() => {
      this.isLoading = false;
...
// 🪄 Trigger both menu and module logic
  ...
  subscribe(this.context, TSIGN_MESSAGE_CHANNEL, (message) => {
    try {
      ...
      const source = message?.source;       // "SERVICE" | "QUOTE"
      ...
      if (!hasServiceFlow && !hasQuoteFlow) {
      ...
      // Store service/quote ids (keep your existing serviceId prop intact)
      // If you don’t yet have quoteId tracked in dashboard, you can still store it for later
      this.serviceId = hasServiceFlow ? resolvedServiceId : null;
      this.quoteId = hasQuoteFlow ? resolvedQuoteId : null; // add this property in your class

      this.source = source || (hasQuoteFlow ? "QUOTE" : "SERVICE");
  ...
  handleModuleClick(event) {
    ...
    switch (moduleName) {
      ...
      case "supportCoordinator":
        ...
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportCoordinator");
      break;
      case "supportParticipants":
        this.supportParticipantflag = true;
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportParticipants");
      break;
      case "supportTasks":
        this.supportTaskflag = true;
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportTasks");
      break;
      case "supportGoals":
        this.supportGoalsflag = true;
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportGoals");
      break;
      case "supportProviders":
        this.supportProvidersflag = true;
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportProviders");
      break;
      case "supportNotes":
        this.supportNotesflag = true;
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportNotes");
      break;
      case "supportBilling":
        this.supportBillingflag = true;
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportBilling");
      break;
      case "supportReports":
        this.supportReportsflag = true;
        this.StaffAvailabilityFlag = false;
        this.handleMenuClick(event);
        this.updateBreadcrumb("supportReports");
      break;
      case "Document":
...
handleReleaseUpdateClose() {
  ...
  handleNavigation(event) {
    ...
    if (noNavigationNotifications.includes(notification)) {
        console.log("⛔ Navigation prevented for:", notification);
