  ...
  getUploadedFileUrl({ recordId: this.recordId })
    .then((url) => {
      if (url) {
        ...
        // 🔹 return the promise so the next .then waits for loadPdf
        return this.loadPdf(url); // Load the PDF from AWS and render it
      } else {
        ...
        // nothing to render, just stop the chain
        return null;
      }
    })
    .then(() => {
      // Only gets here after loadPdf + renderPageAsImage finish
      this.isFileUploaded = true;
    })
    .catch((error) => {
