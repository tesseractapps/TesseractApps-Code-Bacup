...
export default class TesseractAppsPublishShift extends LightningElement {
 ...
 /*  @track EOIvalue; */
           ...
           console.log('facility val from parent :', JSON.stringify(this.facilityarray));
             ...
             //   console.log('Received data in role:', this.addShiftData.AddShiftRole);
                ...
                getFacilityData().then(response => {
                      this.facilityOptions = response.map(record => ({ value: record.Id, label: record.Name,preferredName: record.Facility_Preferred_Name_Formula__c,participantPreferredName: record.Participant_Preferred_Name_Formla__c, staffPreferredName: record.Staff_Preferred_Name_Formual__c }));
                   ...
                   // console.log(err);
                    ...
                    //    this.otherThanNDISUser = response.listofPriceBook.Other_than_NDIS_User__c;
                        /* this.otherThanNDISUser = false; */
   ...
   /*  const bubbleContainer = this.template.querySelector(
    ...
    handleAddShiftTimeData (event){
        if(this.addShiftData.AddShiftEOI == true){
            this.confirMationMessage('Info ','Please check the staff details before proceeding.', 'info');
    ...
    handleAddShiftChange(event) {
        ...
        console.log('this.orgId >> ', this.orgId);
        console.log('addShiftData.AddShiftFacilityValue >>', this.addShiftData.AddShiftFacilityValue);
        console.log('addShiftData.AddShiftParticipantValue >>', this.addShiftData.AddShiftParticipantValue);
        ...
        if (type === 'checkbox') {
            ...
            if (name === 'AddShiftEOI') {
                ...
                console.log('this.addShiftData.AddShiftStartTimeAMPM >>', this.addShiftData.AddShiftStartTimeAMPM);
                console.log('this.addShiftData.AddShiftEndTimeAMPM >>', this.addShiftData.AddShiftEndTimeAMPM);
                if(this.addShiftData.AddShiftType != null && this.addShiftData.AddShiftStartTimeAMPM != null && this.addShiftData.AddShiftEndTimeAMPM != null){
                    this.confirMationMessage('Info ','Please check the staff details before proceeding.', 'info');
                    this.savButtonDisable = true;
                }
            }
        ...
        if(this.addShiftData.AddShiftParticipantValue){
                ...
                .then((response) => {
                    ...
                    if (!response || response.length === 0) {
                        this.serviceType = false;
                        this.savButtonDisable = true;
                          this.confirMationMessage('Warning', 'No service funds available for the selected participant.', 'Warning');
                        return;
                    }
                if (response) {
                    this.TotalFunds = response;
                    this.ServiceTypeIdInParticipant = response[0].Id;;
                    console.log('this.ServiceTypeIdInParticipant >>', this.ServiceTypeIdInParticipant);
        ...
        .then((result) => {
          ...
          /*   this.serviceGroupName = catalogueData.map((rec) => {
             ...
             this.serviceGroupName = catalogueData.map((rec) => {
              ...
              if (this.otherThanNdis === true ||(this.otherThanNdis === false && result.catalogueData[0].Name.includes("Miscellaneous")) ) {
            ...
            console.log("📌 Selected Service Group ID:", this.selectedServiceGroupId);
            console.log('catalogueData.editedCatalogName ==> '+result.editedCatalogName);
                ...
                this.serviceNameDisplay = ( this.serviceGroupName[0].Name) + ' - ' + formattedAmount;
            ...
            this.addShiftData.selectedServiceInfoJson = JSON.stringify(this.selectedServiceInfo);
            

            console.log("📦 selectedServiceInfo:",JSON.stringify(this.selectedServiceInfo) );
            console.log("🧾 JSON format:", this.addShiftData.selectedServiceInfoJson);
            console.log("Updated addShiftData:", JSON.stringify(this.addShiftData));
        })
        .catch((error) => {
    ...
    async handleOpenEOIModal(event) {
        ...
        if (!this.addShiftData.AddShiftFacilityValue) {
            this.showToast('Error', 'Please select the Facility', 'error');
            return;
        }
        ...
        if (this.headingLabel !== 'Edit Shift(s)') {
            try {
                ...
                this.showToast('Error', 'Failed to fetch staff list', 'error');
            }
        }
    }
    ...
    fetchEOIStaff() {
        console.log('📥 Fetching EOI staff for facility:', this.addShiftData.AddShiftFacilityValue);
        ...
        .then(result => {
            ...
            if (this.addShiftData.AddShiftRole) {
                const targetRole = this.addShiftData.AddShiftRole.trim();
                console.log(`🔍 Filtering staff with role: '${targetRole}'`);

                filteredStaff = result.filter(staff => {
                    if (!staff.Role__c) {
                        console.warn(`⚠️ Skipping staff with no Role__c: ${staff.Name}`);
                        return false;
                    }

                    const roles = staff.Role__c.split(';').map(role => role.trim());
                    const hasRole = roles.includes(targetRole);

                    console.log(`👤 ${staff.Name} | Roles: [${roles.join(', ')}] | Match: ${hasRole}`);
                    return hasRole;
                });
            ...
            this.staffData = filteredStaff.map(staff => ({
                ...staff,
                isSelected: false
            }));

            console.log(`📋 Final staffData set with ${this.staffData.length} records`);
        })
        .catch(error => {
    ...
    /* fetchEOIStaff() {
        getStaffList({ orgId: this.orgId, facilityId: this.addShiftData.AddShiftFacilityValue })
            .then(result => {
                console.log(`✅ Apex returned ${result.length} staff records`);
                console.table(result);
                this.staffData = result.map(staff => ({
                    ...staff,
                    isSelected: false // For checkbox support
                }));
    ...
    handleStaffCheckboxChange(event) {
        ...
        // Update isSelected flag on the staffData array
        ...
        // Rebuild selected IDs list
        this.selectedStaffIds = this.staffData
            .filter(staff => staff.isSelected)
            .map(staff => staff.Id);

        console.log('📌 Changed Staff ID:', staffId, 'Checked:', isChecked);
        console.log('🧾 Multi-selected Staff IDs:', this.selectedStaffIds);
    }
    ...
    fetch(endpoint)
      ...
      .then((data) => {
        ...
        console.log("jsonData prepared for map:", JSON.stringify(this.jsonData2));
    ...
    handleCloseModalEOIStaff() {
        ...
        if (this.headingLabel === 'Add Shift(s)') {
            ...
            // Restore original EOI staff from addShiftData.EOI_Staff__c
            const originalSelectedIds = this.addShiftData?.EOI_Staff__c
                ? this.addShiftData.EOI_Staff__c.split(',')
                : [];

            this.selectedStaffIds = originalSelectedIds;

            this.staffData = this.staffData.map(staff => ({
                ...staff,
                isSelected: originalSelectedIds.includes(staff.Id),
                isDisabled: this.headingLabel === 'Edit Shift(s)' && originalSelectedIds.includes(staff.Id)
            }));
        }
    ...
    validateInputs() {
      ...
      let fields = [
          ...
          { name: "AddShiftDuration" , selector: ".AddShiftDuration", errorMessage: "Start time must be earlier than end time." },
  ...
  return this.addNewAddressCheckBox ? '' : 'display: none;';
    ...
    AdreesCheckboxChange(event) {
        ...
        switch (checkboxName) {
            case 'facilityAddressCheckbox':
                if (isChecked) {
                    ...
                    console.log('🧹 Facility checkbox unchecked. Clearing address fields.');
            ...
            case 'participantAddressCheckBox':
                if (isChecked) {
                    ...
                    console.log('🧹 Participant checkbox unchecked. Clearing address fields.');
                    this.EmptyAddressFields();
                }
                break;
            ...
            case 'addNewAddressCheckBox':
                if (isChecked) {
                    ...
                    console.log('🧹 Add New checkbox unchecked. Clearing address fields.');
                    this.EmptyAddressFields();
                }
                break;
        ...
        this.addShiftData.AddShiftEnterOtherLocation = this.addNewAddressCheckBox || this.participantAddressCheckBox;
        this.addShiftData.AddShiftParticipantAddressCheckbox = this.participantAddressCheckBox;
        console.log('📝 AddShiftEnterOtherLocation set to:', this.addShiftData.AddShiftEnterOtherLocation);
        console.log('📝 AddShiftParticipantAddressCheckbox set to:', this.addShiftData.AddShiftParticipantAddressCheckbox);
    ...
    getClientById({ recordId: this.addShiftData.AddShiftParticipantValue })
      ...
      handleAddShiftSave(){
       ...
       // console.log('validate '+this.validateAddress());
        ...
        console.log('Participant Value:', this.addShiftData.AddShiftParticipantValue);
        console.log('Other_than_NDIS_User__c:', this.otherThanNDISUser);
        if (this.addShiftData.AddShiftParticipantValue == null && this.otherThanNDISUser == true) {
            this.confirMationMessage("Error", "Participant is required.", "Error");
            this.isShowSpinner=false;
            return;
        }

        if(this.isEditShiftScreen==true &&  this.addShiftData.AddShiftQuantity < this.AddShiftOriginalQuantity ){
          ...
          rosterPublish({addshiftData:JSON.stringify(this.finalAddShiftData),startdate:this.startdate,enddate:this.enddate,facilityVal:facVal, selectedStaffIds: [...this.selectedStaffIds]})
          .then((result) => {
         ...
         // const Id = JSON.parse(result.shiftId);
          ...
          if(result.isSuccess){
             this.selectedShiftId=result.shiftid;
              ...
              if(this.addShiftData.AddShiftnotification ==true &&this.isEditShiftScreen==false && this.headingLabel !== 'Edit Shift(s)'){
              ...
              generateAndSendNotification({role:this.addShiftData.AddShiftRole,strdate:this.addShiftData.AddShiftStartDate,staffId:'', isRjectedAllocation:false,
                          rejectedStaffEmail:rejectedStaffEmail,facilityValue:this.addShiftData.AddShiftFacilityValue}).then(response=>{
            });
          ...
          this.confirMationMessage('Error','Start time must be earlier than end time.','Error');
        }
      }
      }
    ...
    async  handleAllocateToggle(event) {
        ...
        this.staffHorlyRateList = this.staffHorlyRateList.map(staff =>
          ...
          await this.DocExpiryCheck(staffId, this.selectedShiftDate);
        if(this.documentExpired){
             this.staffHorlyRateList = this.staffHorlyRateList.map(staff =>
            staff.Id === staffId ? { ...staff, isAllocate: false} : staff
        );
            this.dispatchEvent(
            new ShowToastEvent({
                title: 'Error',
        ...
        if(allocatedStaff.length>this.unallocatedCount){
          this.confirMationMessage('Error','Exceeded shift allocation limit. No more staff can be allocated.','Error');
 ...
 let allocateStaffJSON = this.finalAllocationList.map(rec => {
    ...
    allocateMultipleStaff({ shiftId: this.selectedShiftId, staffJson: JSON.stringify(allocateStaffJSON),shiftDate:this.selectedShiftDate,shiftwithstaff:[],isExecutedFromMobile:executedFromMobile})
    .then(result => {
      console.log('Result: '+JSON.stringify(result));
      this.confirMationMessage('Success','Shifts allocated successfully.','Success');
      this.fatigueManagementFlag=false;
        ...
        if (shiftData) {
            ...
            if (shiftData.Service_Details__c) {
                try {
                    ...
                    this.addShiftData.AddShiftParticipantValue = serviceDetails.participantId;


                    console.log('Parsed Service Type ID:', this.ServiceTypeIdInParticipant);
                    console.log('Parsed Service Group ID:', this.selectedServiceGroupId);
                    console.log('Parsed Participant ID:', this.addShiftData.AddShiftParticipantValue);
                } catch (error) {
           ...
           this.setLatitudeLongitudeMarkersOnly();
            ...
            if (this.addShiftData.AddShiftEndTimeAMPM) {
                ...
                this.endTimeAMPM = period === "AM";
            }

            if(shiftData.Participant_Address_checkbox__c  == true && shiftData.Get_Facility__c  == true) {
                this.participantAddressCheckBox = true;
                this.addNewAddressCheckBox = false;
                this.addressValueCheckbox = true;
                this.serviceType = true;
                console.log('this.participantAddressCheckBox  >>', this.participantAddressCheckBox );
                console.log('this.addNewAddressCheckBox  >>', this.addNewAddressCheckBox );
                console.log('this.addressValueCheckbox  >>', this.addressValueCheckbox );
            }

            if(shiftData.Get_Facility__c == false && shiftData.Participant_Address_checkbox__c == false)  {
                this.addressValueCheckbox = true;
            }
            ...
            console.error('Shift not found for ID:', event.currentTarget.dataset.shiftid);
        }
    }
   ...
   clearText() {
    ...
    handleRoleFilterChange(event) {
        ...
        if (this.headingLabel === 'Add Shift(s)') {
            ...
            if(role == 'all') {
                ...
                if (this.filteredRole && Array.isArray(this.filteredRole)) {
                    const filtered = this.filteredRole.filter(item => {
                        ...
                        const roles = item.role.split(';').map(r => r.trim().toLowerCase());
                    ...
                    console.log('Filtered staff hourly rate list >>', JSON.stringify(this.staffHorlyRateList));
                }
            }
        } else {
            ...
            console.log('this.unAllocatedTableList >>', JSON.stringify(this.unAllocatedTableList));
            let filteredShiftList = this.unAllocatedTableList.filter(staff => staff.Id === this.selectedShiftId);
            console.log('shift list' + JSON.stringify(filteredShiftList));
            getStaffHourlyRates({
     ...
     async processShifts(facilityId) {
        try {
         ...
         /*  console.log(
          ...
          this.otherThanNDISUser=this.shiftNameOptions[0].Facility__r.Type_of_Service__c !='NDIS';
      }
