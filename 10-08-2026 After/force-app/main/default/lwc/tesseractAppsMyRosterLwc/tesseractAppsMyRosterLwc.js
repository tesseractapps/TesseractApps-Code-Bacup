...
export default class TesseractAppsMyRosterLwc extends LightningElement {
    ...
    // Resize Observer & Page Size parameters
    recordsPerPage = 10;
    ...
    setPageSizeByZoomAndScreen() {
        ...
        const rowHeight = 60;
        const calculatedRows = Math.floor((containerHeight - headerHeight) / rowHeight);
        
        this.recordsPerPage = calculatedRows > 0 ? calculatedRows : 10;
    }
    ...
    try {
        ...
        this.orgfullname = response.Name;
        this.orgname = response.Name ? response.Name.split(' ').slice(0, 2).join(' ') : '';
        this.WeekStartDay = response.Week_Start_Day__c;
   ...
   /* setCurrentWeek() {
    ...
    setCurrentWeek() {
    const today = new Date();
    const orgWeekStart = this.WeekStartDay;

    const startDay = this.weekDayMap[orgWeekStart];
    const startOfWeek = new Date(today);
    let diff = today.getDay() - startDay;
    if (diff < 0) {
        diff += 7;
    }
    ...
    this.endDate.setDate( startOfWeek.getDate() + 6 );
   // this.datePickerString = today.toISOString().split('T')[0];
   this.datePickerString = today.toLocaleDateString('en-CA', {
   ...
   /*    navigateToDay(event) {
    ...
    const orgWeekStart = this.WeekStartDay;
    const startDay =
        this.weekDayMap[orgWeekStart];
    const startOfWeek = new Date(selectedDate);
    let diff =
        selectedDate.getDay() - startDay;
    if (diff < 0) {
        diff += 7;
    }
   ...
   // this.updateDateDisplay();
    ...
    updateDateDisplay() {
        //this.datePickerString = this.startDate.toISOString().split('T')[0];
         this.datePickerString = this.startDate.toLocaleDateString('en-CA', { timeZone: 'Australia/Sydney' });
   ...
   handleConfirm(event) {
    ...
    if (hoursDifference < this.rejectShiftHours && hoursDifference >= 0) {
        console.log('❌ Rejection blocked - shift starts within the next 24 hours.');
       this.showToast('Error', `Shifts starting within the next ${this.rejectShiftHours} hours cannot be rejected.`, 'error');
        return;
    } else {
