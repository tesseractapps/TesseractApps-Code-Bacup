...
export default class TicketManager extends LightningElement {
    ...
    moduleValues({ data }) {
        if (data) {
                ...
                switch (this.currentUserType) {
                    case 'NDIS Org Admin':
                        allowedModules = [
                            ...
                            'Accounting Software', 'ChaT', 'Reports', 'Other'
                        ];
                        break;
                    ...
                    case 'HR Admin':
                        allowedModules = [
                            ...
                            'Repository', 'Access Manager', 'My Profile', 'Reports', 'Other'
                        ];
                        break;
            ...
            .map(opt => ({
                label: opt.value === 'Repository' ? 'Documents' : opt.label,
                value: opt.value
            }));
        }
    }
...
wiredTickets(result) {
    ...
    if (data) {
        ...
        console.log('✅ Tickets fetched:', JSON.stringify(this.rawTickets));
    ...
    get paginatedTickets() {
        ...
        return this.tickets.slice(start, end).map(t => {
            ...
            return {
                ...
                priorityClass: this.getPriorityClass(t.Priority__c),
    ...
    submitTicket() {
        ...
        createTicket({ ticket: this.ticket })
            .then(result => {
                ...
                if (this.uploadedFiles1 && newTicketId) {
                    console.log('📎 File attached – uploading to AWS...');
                    return insertTicketAttachments({
    ...
    getTicketDetailsById({ ticketId })
        .then(result => {
            ...
            this.selectedTicket = {
                ...
                decodedDescription: this.decodeHtml(result.Description__c, 1000),
                moduleDisplay: result.Module__c === 'Repository' ? 'Documents' : result.Module__c
            };
            
           // this.descriptionfromTicket = this.decodeHtml(result.Description__c, 1000);
            this.descriptionfromTicket =result.Description__c;
...
formatDate(dateString) {
  ...
  //Multiple attachent upload code
   triggerFileDialog() {
...
// get ticketTableClass() {
//     let base = 'table-container ticketTable';

//     if (this.isTesseractOrg && this.canEditTickets) {
//         return `${base} full`;
//     }
//     if (!this.isTesseractOrg && this.canEditTickets) {
//         return `${base} no-org`;
...
get ticketTableClass() {
    ...
    console.log('ticketTableClass called');
    console.log('isTesseractOrg:', this.isTesseractOrg);
    console.log('canEditTickets:', this.canEditTickets);

    if (this.isTesseractOrg && this.canEditTickets) {
        console.log('Condition: isTesseractOrg && canEditTickets → full');
        return `${base} full`;
    }

    if (!this.isTesseractOrg && this.canEditTickets) {
        console.log('Condition: !isTesseractOrg && canEditTickets → no-org');
        return `${base} no-org`;
    }

    if (!this.isTesseractOrg && !this.canEditTickets) {
        console.log('Condition: isTesseractOrg && !canEditTickets → no-action');
        return `${base} no-action`;
    }

    console.log('Condition: default → base');
    return base;
}
...
getPriorityClass(priority) {
    switch (priority) {
        ...
        case 'High':
            return 'status-pill priority-high';

        case 'Medium':
            return 'status-pill priority-medium';

        case 'Low':
            return 'status-pill priority-low';
