...
export default class TesseractAppsPmsHrCoachingLwc extends LightningElement {
        ...
        getHrCoachingSessions({ facilityId: this.selectedFacilityId })
            .then(result => {
                ...
                const mappedData = sessions.map(rec => {
                    ...
                    // const coachingHrCreatedtype = rec.isCreatedByHR__c
                    //     ? 'Created by Hr'
                    //     : '';
                    const coachingCreatedtype =
                        rec.isCreatedByRc__c
                            ? 'Triggered from RC'
                            : rec.isCreatedByHR__c
                            ? 'Created by HR'
                            :rec.isCreatedByManager__c
                            ? 'Created by Manager'
                            :'';
                    ...
                    const isHrCreated = rec.isCreatedByHR__c === true;
                    ...
                    const isIndividual = (isManagerCreated || isHrCreated) && type === 'Individual Employee';
                    const isGroup = (isManagerCreated || isHrCreated) && type === 'Groups - Multiple Employees';
                    const isManagerHr = (isManagerCreated || isHrCreated) && type === 'Managers and HRs Coaching';
                    const isManagerOnly = isManagerCreated && type === 'Managers Only coaching';
                    const isHrOnly = isHrCreated && type === 'HRs Only coaching';
                    ...
                    //  Manager Label Logic
                    const hrCreatedReviewLabel = isHrCreated
                        ? (hrAckDate ? 'View' : 'Review')
                        : '';
                    const hrReviewLabel = isRc || isManagerCreated || isHrCreated
                        ? (hrAckDate ? 'View' : 'Review')
                        : '';
                    let managerCount = 0;
                    let hrCount = 0;
                    ...
                    return {
                       ...
                       // coachingHrCreatedtype:coachingHrCreatedtype,
                       ...
                       // hrContact: '', 
                        ...
                        // isHrCreated: rec.isCreatedByHR__c === true,
                        ...
                        hrCreatedReviewLabel:hrCreatedReviewLabel,
                ...
                this.pendingRecords = mappedData.filter(
                    rec => (rec.isRc || rec.isManagerCreated) && !rec.isHrCreated
                );
                // console.log('Scheduled:', JSON.stringify(this.scheduledRecords));
                console.log('Pending:', JSON.stringify(this.pendingRecords));
                this.hrCreatedRecords = mappedData.filter(rec => rec.isHrCreated);
                console.log('hrCreatedRecords  :', JSON.stringify(this.hrCreatedRecords));
    ...
    handleBack1(event) {
     ...
     handleHrCreateCoachingNote(event){

       
        console.log('isHr in handleHrCreateCoachingNote : ', this.isHr);
