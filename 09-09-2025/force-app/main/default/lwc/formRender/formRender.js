...
export default class FormRender extends LightningElement {
  ...
  handleTextAreaInput(event) {
    ...
    console.log(`🖊️ TextArea Input Triggered for Cell ID: ${cellId}`);
    console.log(`   ➤ New Value: "${textValue}"`);

    const updatedRows = this.stepPagedRows.map((row, rowIndex) =>
      row.map((cell, colIndex) => {
        if (cell.id === cellId) {
          console.log(
            `✅ Match Found at [${rowIndex}][${colIndex}] - Updating field value`
          );
          console.log(`   ➤ Old Value: "${cell.field?.value || ""}"`);
          console.log(`   ➤ New Char Count: ${textValue.length}`);
    ...
    // 🔁 Deep re-assignment to force reactive update
    this.stepPagedRows = JSON.parse(JSON.stringify(updatedRows));

    console.log("🔄 Forced deep update to stepPagedRows");
  }
