...
export default class TesseractAppsUserManagement extends LightningElement {
    ...
    handleDownloadTemplate(event){
        ...
        if(this.typeOfUser=='NDIS'){
            const fileUrl = '/resource/Staff_Details_Upload_Template';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        } else if(this.typeOfUser=='ICT'){
            const fileUrl = '/resource/Staff_Details_Upload_Template_ICT';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        }
    }
    ...
    handleInstructions(event){
        ...
        if(this.typeOfUser=='NDIS'){
            const fileUrl = '/resource/Staff_Bulk_Upload_Instructions';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
            
        } else if(this.typeOfUser=='ICT'){
            const fileUrl = '/resource/Staff_Bulk_Upload_Instructions_ICT';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        }
    ...
    parseCSV(data) {
        ...
        let requiredHeaders = [
            ...
            'DateofBirth', 'Facility', 'HourlyRate',
    ...
    handleCloseModal() {
        ...
        handleFinalSubmit() {
            console.log('📦 Table Data (Before Processing):', JSON.stringify(this.tableData, null, 2));

            // Step 1️⃣: Filter selected records
            const selectedRecords = this.tableData.filter(user => user.isSelected);
            console.log(`✅ Selected Records (${selectedRecords.length}):`, JSON.stringify(selectedRecords, null, 2));
            ...
            // Step 2️⃣: Identify unprocessed ones
            const unprocessedRecords = selectedRecords.filter(user => user.ProcessedStatus !== 'Success ✅');
            console.log(`🧾 Unprocessed Records (${unprocessedRecords.length}):`, JSON.stringify(unprocessedRecords, null, 2));
            ...
            // Step 3️⃣: Define required fields
            let requiredFields = [
            ...
            // Step 4️⃣: Remove optional fields for ICT users
            if (this.typeOfUser === 'ICT') {
                console.log('👤 ICT user detected, removing Languages and Nationality...');
            ...
            // Step 6️⃣: Validate required fields and mark errors
            ...
            if (hasInvalidRecords) {
                console.warn('🚨 Some records failed validation. Skipping those.');
            }

            // Step 7️⃣: Build valid records (from unprocessed only)
            let validRecords = [];
            try {
                validRecords = unprocessedRecords
                    ...
                    .map(user => {
                        ...
                        // Mark as processed here — avoids overlap with unprocessedRecords
                ...
                // Step 8️⃣: Detect overlap between valid + unprocessed (debug check)
                const overlap = validRecords.filter(v =>
                    unprocessedRecords.some(u => u.id === v.id && u.ProcessedStatus !== 'Success ✅')
                );
                if (overlap.length > 0) {
                    console.warn('🚨 Overlap detected! Same records exist in both valid and unprocessed lists:', JSON.stringify(overlap, null, 2));
                }

                // Step 9️⃣: Submit only valid ones
                if (validRecords.length > 0) {
                    console.log(`📤 Submitting ${validRecords.length} valid record(s):`, JSON.stringify(validRecords, null, 2));
                    this.uploadDataToApex(validRecords);
                } else {
                    console.warn('⚠️ No valid records found to submit.');
                }
