...
export default class DynamicWidget extends LightningElement {
    ...
    @api refresh(facilityIdFromParent) {
        ...
        this.loadWidgetData();  // or fetchData()
    }
    ...
    connectedCallback() {
        ...
        console.log('DynamicWidget facilityId :', this.facilityId);
    ...
    refreshAllWidgets() {
        const widgetTags = [
            'c-shiftwith-staff-status-bar-chart',
            'c-sign-in-status-chart',
            'c-invoice-status-bar-chart',
            'c-tesseract-apps-task',
            'c-tesseract-apps-leave-management-chart',
            'c-tesseract-apps-participantsgor-org-admin',
            'c-incident-registerchart',
            'c-performance-management-chart',
            'c-human-resources-menu-dashboard',
            'c-participants-funds-chart-lwc',
            'c-payroll-chart',
            'c-ict-tmesheet-chart-lwc',
            'c-tesseract-apps-staff-dashboard-lwc',
            'c-participant-schedule-chart',
            'c-staff-status-chart',
            'c-rejected-shifts-chart',
        ];
        console.log('Selected Facility ID refreshAllWidgets1>>', this.facilityId);


    widgetTags.forEach(tag => {
        const widgets = this.template.querySelectorAll(tag);
        widgets.forEach(widget => {
            if (typeof widget.refresh1 === 'function') {
                console.log('Selected Facility ID refreshAllWidgets in if loop>>', this.facilityId);
                widget.refresh1(this.facilityId);
            }
        });
    });
}
    ...
    get isRejectedShiftsWidget(){
        console.log('RejectedShiftsWidget >>',this.type === 'rejectedShifts' );
        return this.type === 'rejectedShifts';
        console.log('RejectedShiftsWidget >>',this.type === 'rejectedShifts' );
    }
    ...
    const knownTypes = [
        ...
        'staffDashboard', 'participantSchedule', 'rejectedShifts'
    ];
