...
import getUnreadLeaveRecordIds from '@salesforce/apex/NotificationServices.getUnreadLeaveRecordIds';//manendra sai added
import markNotificationRead from '@salesforce/apex/NotificationServices.markNotificationRead';
import orgDetailsCommunity from "@salesforce/apex/OrgDetails.orgDetailsRoster";
import validateLeaveThreshold from '@salesforce/apex/LeaveController.validateLeaveThreshold';
//import getStaffsByFacility from '@salesforce/apex/AdminPaginationController.getStaffsByFacility';
...
export default class LeaveManagementLwc extends LightningElement {
    ...
    @track unreadLeaveIds = [];//manendra sai added 
    @track clientData;
    ...
    @track _leaveflag = false;
    get leaveflag() { return this._leaveflag; }
    set leaveflag(val) {
        this._leaveflag = val;
    ...
    @track _cashoutFlag = false;
    get cashoutFlag() { return this._cashoutFlag; }
    set cashoutFlag(val) {
        this._cashoutFlag = val;
        this._syncRecordRoute();
    }

    @track _TerminationFlag = false;
    get TerminationFlag() { return this._TerminationFlag; }
    set TerminationFlag(val) {
        this._TerminationFlag = val;
        this._syncRecordRoute();
    }

    @track _calendarFlag = false;
    get calendarFlag() { return this._calendarFlag; }
    set calendarFlag(val) {
        this._calendarFlag = val;
        this._syncRecordRoute();
    }

    @track _staffFlag = false;
    get staffFlag() { return this._staffFlag; }
    set staffFlag(val) {
        this._staffFlag = val;
        this._syncRecordRoute();
    }    
    ...
    @track userType;
    @track storedFacilityLabel;
        //manendra added for sorting 
    @track sortField = '';
    @track showThresholdPopup=false;
    ...
    connectedCallback() {
       ...
       /*  this.currentMonth=new Date().getMonth() + 1;
        ...
        this.storedFacilityLabel = localStorage.getItem("defaultFacilityLabel");

        console.log('this.FacilityId',this.FacilityId);
         this.loadOrgData();
        this.loadStaff();
      
        this.loadStaffAndLeaves();
        // this.loadAllCashouts();
        // this.loadTerminationPayouts();
        this.loadUser();
       

        this.selectedStatus = this.statusOptions.map(x => x.value);
    ...
    orgDetailsCommunity()
      .then((result) => {
        console.log("org data1:", JSON.stringify(result));
       
        this.otherleavetype = result.Other_Leave_Type__c;
        console.log('otherleavetype',this.otherleavetype);
    ...
    loadStaffAndLeaves() {
        getStaffById({ recordId: this.StaffId })
            .then(data => {
                ...
                this.image = staff.Picture__c;
                this.StaffId = staff.Id;

                this.Annual = staff.Annual_leave_Accrued__c;
                this.Sick = staff.Personal_Leave_Accur__c;

                this.typeOfUser = staff.Type_of_User__c;
                this.staffState = staff.State__c;

                this.negativeBalance = staff.Facility__r?.Organisation__r?.Negative_Balance__c;
               // this.otherLeaveType = staff.Facility__r?.Organisation__r?.Other_Leave_Type__c;
                this.typeOfEmployee = staff.Type_of_Employe__c;

                // ✅ CRITICAL: set orgId before next call
                this.orgid = staff.Facility__r?.Organisation__c;
                let leaves = staff.Leaves__r || [];

                this.annualUsed = leaves.reduce((sum, rec) => sum + (rec.Annual_Used__c || 0), 0);
                this.personalUsed = leaves.reduce((sum, rec) => sum + (rec.Personal_Used__c || 0), 0);

                this.annualRemaining = Number(((this.Annual || 0) - this.annualUsed).toFixed(2));
                this.personalRemaining = Number(((this.Sick || 0) - this.personalUsed).toFixed(2));

                this.annualPercent = this.Annual ? Math.round((this.annualUsed / this.Annual) * 100) : 0;
                this.personalPercent = this.Sick ? Math.round((this.personalUsed / this.Sick) * 100) : 0;

                // ✅ RETURN next promise (this is the key)
                if (this.StaffId && this.orgid) {
                    return getLeaves({
            ...
            .then(leaveData => {
                ...
                this.leaves = leaveData.map(leave => ({
                    ...leave,
                ...
                this.totalRecords1 = this.leaves.length;
  ...
  /*   @wire(getLeavesByOrg, { orgId: '$orgid', startDate: '$startDate', endDate: '$endDate' })
   ...
   tLogoUrl = `${Loading_Logo}/TLogo.png`;
    ...
    async getNotifications() {
        ...
        try {
            ...
            const result =
                await getUnreadLeaveRecordIds({
                    staffId: this.currentloggedstaffid
                });

            this.unreadLeaveIds = result;
   ...
   @wire(getLeavesByUserRole, {
    ...
    async wiredLeaves(result) {
        ...
        await this.getNotifications(); // Ensure notifications are loaded before processing leaves
        try {
            /* =========================================================
            1️⃣ Preprocess leaves
            ========================================================= */
            console.log('Raw Leaves Data:', JSON.stringify(this.unreadLeaveIds));
            let allLeaves = data.map(leave => ({
                ...leave,
                 staffName: leave.Staff__r?.Display_Nickname__c || '',// manendra added fr sorting table
                //manendra added
                    hasNotification:  this.unreadLeaveIds.includes(leave.Id),
                    /* ✅ ADD THIS LINE */
    ...
    console.log('Preprocessed Leaves: ', JSON.stringify( this.orgleaves));
            /* =========================================================
            ...
            if (this.isPageSizeManuallySet) {
                // Keep the manual size
            } else {
                this.pageSize = this.recordsPerPage || 10;
            }
           /*  this.pageNumber = 1;
            this.paginationHelper(); */
            this.applyFilters();
...
loadStaff() {
        ...
        .then(result => {
            ...
            this.selectedStaff = this.staffOptions.map(x => x.value);
  ...
  if (this.userType !== 'NDIS Org Admin' && this.userType !== 'ICT Admin') {
                ...
                if (this.statusValue === 'Rejected' || this.statusValue === 'Approved' ||  this.statusValue === 'Cancelled') {
                    ...
                    this.template.querySelector('lightning-record-edit-form').submit(fields); */
                     if (this.statusValue === 'Approved') {
                            ...
                            validateLeaveThreshold({
                                fromDate: fields.From__c,
                                toDate: fields.To__c,
                            ...
                            .then(result => {
                                ...
                                if (result.length > 0) {
                                    ...
                                    this.thresholdData = result.map(item => {

                                const parts = item.leaveDate.split('-');

                                                return {
                                                    ...item,
                                                    leaveDate: `${parts[2]}-${parts[1]}-${parts[0]}`, // DD/MM/YYYY
                                                    isOpen: true,
                                                    iconName: 'keyboard_arrow_down'
                                                };
                                            });
                                    console.log('this.pendingFields',JSON.stringify(this.pendingFields));
                                    console.log('this.thresholdData',JSON.stringify(this.thresholdData));
                                    this.showThresholdPopup = true;
                                    return;

                                } else { 
                                           console.log('this.pendingFields else',JSON.stringify(this.pendingFields));
                                    // No warning
                                    this.template
                            ...
                            // Rejected & Cancelled - existing behavior
                                 this.template.querySelector('lightning-record-edit-form').submit(fields);
                        }
                            }else{ 
                                console.log('hi3');
    ...
    handleorgedit(event){
        ...
        this._selectedRecordId = event.currentTarget.dataset.id;
        
        this._selectedRecordUid = event.currentTarget.dataset.uid;        
        this.currentstaff=event.currentTarget.dataset.currentstaff;
        ...
        if(status === 'Withdrawn'|| status === 'Requested'){
         this.saveButtonDisable=false;
        }else{
            this.saveButtonDisable=true;
        }
...
.then(() => {
    ...
    this.unreadLeaveIds =
        this.unreadLeaveIds.filter(
            id => id !== this.leaverecordId
        );

    this.orgleaves =
        this.orgleaves.map(leave => {
    ...
    paginationHelper() {
        this.orgleaves = [];
        this.records = this.sortData(this.records);// manendra added fr sorting table
    ...
    */
    ...
    setPageSizeByZoomAndScreen() {
        ...
        setTimeout(() => {
            ...
            if (container) {
                ...
                const header = this.template.querySelector('.fixed-table thead') || this.template.querySelector('.staff-table thead') || this.template.querySelector('thead');
                ...
                let rowHeight = 40;
                const firstRow = this.template.querySelector('.fixed-table tbody tr') || this.template.querySelector('.staff-table tbody tr') || this.template.querySelector('tr.table-row') || this.template.querySelector('tbody tr');
                if (firstRow) {
                    rowHeight = firstRow.getBoundingClientRect().height || firstRow.offsetHeight || 40;
                }
                
                let rows = Math.floor((availableHeight + 20) / rowHeight);
                if (rows < 1) {
                    rows = 1;
                }
    ...
    if (fieldName === 'StartDateInputValue') {
        this.startDate = event.target.value;
    } else if (fieldName === 'EndDateInputValue') {
    ...
    calculateFinancialYearDates(selectedDate = new Date()) {
        ...
        console.log('FY Start:', this.startDate);
        console.log('FY End:', this.endDate);
    }
...
handlestaffChange(event) {
    ...
    console.log('Staff Event:',JSON.stringify(event.detail) );

    // Ignore search typing events
    if (!Array.isArray(event.detail.value)) {

        console.log(
            'Ignored search event:',
            event.detail.value
        );
...
handleleaveChange(event) {
    ...
    console.log('Leave Event:',JSON.stringify(event.detail) );
    ...
    this.selectedLeave = [...event.detail.value];

    console.log(
        'Selected Leave:',
        JSON.stringify(this.selectedLeave)
    );
    this.applyFilters();
}
...
applyFilters() {
    ...
    if (this.selectedStaff && this.selectedStaff.length) {
        ...
        filtered = filtered.filter(leave =>
            this.selectedStaff.includes(leave.Staff__c)
        );
    }

    // Leave Type Filter
    if (this.selectedLeave && this.selectedLeave.length) {

        filtered = filtered.filter(leave =>
            this.selectedLeave.includes(leave.Type_of_Leave__c)
        );
    }
    ...
    if (this.selectedStatus && this.selectedStatus.length) {
        ...
        filtered = filtered.filter(leave =>
            this.selectedStatus.includes(leave.Status__c)
        );
    }
    ...
    // Update cards
    this.calculateSummaryCards(filtered);
}

calculateSummaryCards(filteredLeaves) {

    // Unique Staff
    const staffIds = [...new Set(filteredLeaves.map(x => x.Staff__c))];

    const filteredStaff = this.staffList.filter(staff =>
        staffIds.includes(staff.Id)
    );

    this.totalStaffCount = filteredStaff.length;

    this.totalAnnualLeave =
        filteredStaff.reduce(
            (sum, staff) =>
                sum + Number(staff.Annual_leave_Accrued__c || 0),
            0
        );

    this.totalPersonalLeave =
        filteredStaff.reduce(
            (sum, staff) =>
                sum + Number(staff.Personal_Leave_Accur__c || 0),
            0
        );

    this.requestedLeavesCount = filteredLeaves.filter(x => x.Status__c === 'Requested').length;

    this.cancelledLeavesCount = filteredLeaves.filter(x => x.Status__c === 'Cancelled').length;
}
...
loadLeaveOptions() {
    ...
    console.log('this.OPTIONS',this.otherleavetype);
    // CONDITIONAL COMPANY LEAVES
    if (this.otherleavetype) {
      finalOptions = [...finalOptions, ...companyLeaves];
    }
    console.log('Final Options:', JSON.stringify(finalOptions));
console.log('Leave Options Count:', finalOptions.length);

    this.leaveOptions = finalOptions;
    this.selectedLeave = this.leaveOptions.map(x => x.value);
  }
    ...
    handleClear() {
        this.calculateFinancialYearDates();  
        this.selectedStaff = this.staffOptions.map(option => option.value);
        this.selectedLeave = this.leaveOptions.map(option => option.value);
        this.selectedStatus = this.statusOptions.map(option => option.value);
        this.applyFilters();
    }
   ...
   /*  exportToCSV() {
    ...
    const rows = data.map(item => [
        item.Staff__r?.Display_Nickname__c || '',
        item.Type_of_Leave__c || '',
        item.fromdate || '',
        item.Todate || '',
        item.Duration__c || '',
        item.Status__c || ''
    ...
    element.download = 'Leave_Report.csv';
...
/* exportToCSV() {
    ...
    // Report Details
    csv += `Facility Name,${this.facilityPreferredName}\n`;
    csv += `Report Duration,Financial Year\n`;
    csv += `Start Date,${this.startDate}\n`;
    csv += `End Date,${this.endDate}\n\n`;

    // Table Header
    csv += 'Staff Name,Leave Type,From Date,To Date,Duration,Status\n';

    data.forEach(row => {
        csv += [
            `"${row.Staff__r?.Display_Nickname__c || ''}"`,
            `"${row.Type_of_Leave__c || ''}"`,
            `"${row.fromdate || ''}"`,
            `"${row.Todate || ''}"`,
            `"${row.Duration__c || ''}"`,
            `"${row.Status__c || ''}"`
 ...
 */   
    ...
    exportToCSV() {
        ...
        // Header information
        const facilityName = data[0]?.Staff__r?.Facility__r?.Name || '';
        const reportDuration = `${this.startDate} To ${this.endDate}`;

        let csv = '';

        csv += `Facility Name,${this.storedFacilityLabel}\n`;
        csv += `Report Duration\n`;
        csv += `Start Date,${this.startDate}\n`;
        csv += `End Date,${this.endDate}\n`;
        csv += '\n';
        ...
        data.forEach(record => {
            ...
            const row = [
                record.Staff__r?.Display_Nickname__c || '',
                record.Type_of_Leave__c || '',
                record.fromdate || '',
                record.Todate || '',
                record.Duration_Type__c || '',
                record.Leave_Duration__c || '',
                record.Reason__c || '',
                record.Comments__c || '',
                record.Status__c || ''
            ];
...
//end  
    ...
    _composeSubRoute() {
        if (this.cashoutFlag) return 'cashbout';
        if (this.TerminationFlag) return 'termination-payout';
        if (this.calendarFlag) return 'leave-calendar';
        if (this.staffFlag) return 'all-staff';
        if (this.leaveflag && this._recordUid) return `${this._recordUid}/edit`;
        return '';
    }
    ...
    _syncRecordRouteActual() {
        if (this._suppressEmit) return;
        if (!this.cashoutFlag && !this.TerminationFlag && !this.calendarFlag && !this.staffFlag && !this.leaveflag) {
        ...
        this.notifySubRoute(route, false);
    }
    ...
    @api async openByUID(uid, tab, isEdit) {
        ...
        try {
            let found = this.records ? this.records.find(r => r.Leave_UID__c === uid || r.Id === uid) : null;
            if (!found) {
                const results = await getLeaveById({ recordId: uid });
                if (results && results.length > 0) found = results[0];
            }
            if (found) {
                this.leaverecordId = found.Id;
                this._selectedRecordId = found.Id;
                this._selectedRecordUid = found.Leave_UID__c;
                
                // Set UI flags to open detail view modal
                this.leaveflag = true;
                this.isdisablefields = true;
                this.isstatus = true;
                this.isHome2 = false;
                this.Docurl = found.Leave_Document__c;
                this.StaffId = found.Staff__c;
                this.managerId = found.Staff__r?.Manager__c;
                this.statusValue = found.Status__c;
                
                if(found.Status__c === 'Withdrawn'|| found.Status__c === 'Requested'){
                    this.saveButtonDisable = false;
                } else {
                    this.saveButtonDisable = true;
                }
                if (found.Status__c === 'Withdrawn') {
                    this.commentsFlag = true;
                } else {
                    this.commentsFlag = false;
                }
                if (found.Duration_Type__c === 'Custom Hours') {
                    this.iscustom = true;
                } else {
                    this.iscustom = false;
                }
    ...
    @api openCreate(step) {
        this._suppressEmit = true;
        try {
            this.cashoutFlag = (step === 'cashbout');
            this.TerminationFlag = (step === 'termination-payout');
            this.calendarFlag = (step === 'leave-calendar');
            this.staffFlag = (step === 'all-staff');
            this.isHome = false;
        } finally {
            this._suppressEmit = false;
        }
    ...
    _restoreFromUrlHash() {
        ...
        const hasCashout = parts.includes('cashbout');
        const hasTermination = parts.includes('termination-payout');
        const hasCalendar = parts.includes('leave-calendar');
        const hasStaff = parts.includes('all-staff');
        
        if (hasCashout || hasTermination || hasCalendar || hasStaff) {
            this._hasRestored = true;
            this._suppressEmit = true;
            try {
                this.cashoutFlag = hasCashout;
                this.TerminationFlag = hasTermination;
                this.calendarFlag = hasCalendar;
                this.staffFlag = hasStaff;
                this.isHome = false;
            } finally {
                this._suppressEmit = false;
            }
        ...
        const uidIdx = parts.findIndex((p) => /^lev-/i.test(p));
        if (uidIdx < 0) return false;
        this._hasRestored = true;
        this.openByUID(parts[uidIdx], parts[uidIdx + 1], parts.includes('edit'));
        this._lastSubRoute = this._composeSubRoute();
        return true;
    }

    // Required API state control contract properties
    @api get isEdit() { return false; }
    @api get currentStep() { return ''; }
    @api currentTabSlug() { return this.leaveflag ? 'edit' : ''; }
    @api selectTab(slug) {}
    @api startEdit() {}
    @api setStep(step) {}
    @api openPopup(slug) {}
    ...
    handleContinue() {
        ...
        this.showThresholdPopup = false;

        this.template.querySelector('lightning-record-edit-form').submit(this.pendingFields);
    }
   ...
   handleToggle(event) {
    ...
    this.thresholdData = this.thresholdData.map((item, i) => {
        if (i === index) {
