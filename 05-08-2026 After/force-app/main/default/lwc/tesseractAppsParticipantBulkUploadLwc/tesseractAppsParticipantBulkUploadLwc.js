...
import getRemainingParticipantSlots from '@salesforce/apex/LimitCheckService.getRemainingParticipantSlots';
...
export default class TesseractAppsParticipantBulkUploadLwc extends LightningElement {
   ...
   get isCompany() {
    ...
    paginateData() {
        ...
        //console.log('Paginated Data IN paginateData :', JSON.stringify(this.paginatedData));
    ...
    handleRowSelection(event) {
       ...
       // this.enableddisabled = this.selectedRecordIds.size === 0;
          const hasSuccessRecord = selectedRows.some(
             row => row.ProcessedStatus === 'Success ✅'
            );

        if (hasSuccessRecord) {
            this.showToast(
                'Warning',
                'You have selected already inserted record. Please unselect them before submitting.',
                'warning'
            );
        }

       this.enableddisabled = selectedRows.length === 0 || hasSuccessRecord; 
       this.isAllSelected = this.paginatedData.length > 0 &&  this.paginatedData.every(r => r.isSelected);
    }
    ...
    handleSelectAll(event) {
       ...
       // console.log('Paginated Data IN handleSelectAll :', JSON.stringify(this.paginatedData));
       ...
       // console.log('tableData  IN handleSelectAll :', JSON.stringify(this.tableData));
    ...
    async handleFinalSubmit() {
      ...
      // 🔥 STEP 0: PARTICIPANT LIMIT CHECK (ADDED)
    try {
        const remainingSlots = await getRemainingParticipantSlots();
        console.log('Remaining participant slots:', remainingSlots);
        ...
        if (remainingSlots === 0) {
            this.showToast(
                'Plan limit exceeded',
                'Participant limit already reached. Please upgrade your plan.',
                'error'
            );
            return;
        }
        ...
        if (selectedCount > remainingSlots) {
            this.showToast(
                'Limit exceeded',
                `You can create only ${remainingSlots} participants. Please try with ${remainingSlots} or fewer.`,
                'error'
            );
            return;
        }
        ...
        this.showToast('Error', 'Error checking participant limit', 'error');
        return;
    }
    // ================= EXISTING CODE =================
    ...
    console.log(' Full tableData:', JSON.parse(JSON.stringify(this.tableData)));
    const selectedRecords = this.tableData.filter(r => r.isSelected);
    console.log(' Full tableData after:', JSON.parse(JSON.stringify(this.tableData)));
    ...
    selectedRecords.forEach(record => {
        try {
            const missingFields = templateFields.filter(
                f => !record[f.key] || record[f.key].trim() === ''
            );

            const dob = record.DateofBirth;
            const name = `${record.FirstName || 'Unknown'} ${record.LastName || ''}`;

            const isValidDateFormat = dateStr => /^\d{2}\/\d{2}\/\d{4}$/.test(dateStr);

            // ❌ Invalid DOB
            if (dob && !isValidDateFormat(dob)) {
                dateErrors.push(`Invalid date format in "Date of Birth" for ${name}. Expected dd/MM/yyyy.`);
    ...
    // console.log(' Valid Records:', JSON.parse(JSON.stringify(validRecords)));
    // console.log('Invalid Records:', JSON.parse(JSON.stringify(invalidRecords)));
    // console.log(' Date Errors:', dateErrors);

    // 🔴 Show date errors
    if (dateErrors.length > 0) {
        dateErrors.forEach(msg => {
    ...
    //console.log('✅  this.tableData before paginated data:', JSON.stringify( this.tableData));
   ...
   this.paginatedData = [];
    ...
    // console.log('✅  this.tableData after paginated data  :', JSON.stringify( this.tableData));
}
