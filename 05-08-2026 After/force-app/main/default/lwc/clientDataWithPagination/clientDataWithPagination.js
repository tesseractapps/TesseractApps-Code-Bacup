...
export default class ClientDataWithPagination extends NavigationMixin(
  ...
  @track firstname = "";
  @track lastname = "";
  _ParticpantRecordForm = false;
  get ParticpantRecordForm() { return this._ParticpantRecordForm; }
  set ParticpantRecordForm(val) {
      this._ParticpantRecordForm = val;
  ...
  @track ndisFlag;
  _ndisCreateFlag = false;
  get ndisCreateFlag() { return this._ndisCreateFlag; }
  set ndisCreateFlag(val) {
      this._ndisCreateFlag = val;
      this._syncRecordRoute();
  }
  ...
  get showUpgradeModal() { return this._showUpgradeModal; }
  set showUpgradeModal(val) {
      this._showUpgradeModal = val;
      this._syncRecordRoute();
  }
  ...
  @track sectionFlags = {
    ...
    _showContactAttachmentModal = false;
    get showContactAttachmentModal() { return this._showContactAttachmentModal; }
    set showContactAttachmentModal(val) {
        this._showContactAttachmentModal = val;
        this._syncRecordRoute();
    }
    @track uploadedFiles = [];
    @track totalfiles = [];
    ...
    _showAttachmentListModal = false;
    get showAttachmentListModal() { return this._showAttachmentListModal; }
    set showAttachmentListModal(val) {
        this._showAttachmentListModal = val;
        this._syncRecordRoute();
    }
    ...
    if (result.data) {
       ...
       // this.visible = this.totalRecords > this.pageSize;
        if (this._restoreFromUrlHash()) {
            console.log('Restored client view from URL hash');
        } else {
...
handleeditClose() {
  ...
  _editclientflag = false;
  @api get editclientflag() { return this._editclientflag; }
  set editclientflag(val) {
      this._editclientflag = val;
      this._syncRecordRoute();
  }
  @track clientid;
  @track fName = "";
  ...
  handleSubmit(event) {
    ...
    const emailInputs = this.template.querySelectorAll('.contact-email');
     ...
     for (let i = 0; i < emailInputs.length; i++) {
      ...
      fields.Client_with_User__c=this.clientWithUser;
...
handleViewAttachment(event) {
    ...
    _clientIdForUid(uid) {
        if (!uid || !this.participantJson) return '';
        const ids = Object.keys(this.participantJson);
        const lowerUid = uid.toLowerCase();
        for (let i = 0; i < ids.length; i++) {
            if (this.participantJson[ids[i]] && 
                this.participantJson[ids[i]].uid && 
                this.participantJson[ids[i]].uid.toLowerCase() === lowerUid) {
                return ids[i];
            }
        }
        return '';
    ...
    get _recordUid() {
        return (this.participantJson && this.recordId && this.participantJson[this.recordId]
            && this.participantJson[this.recordId].uid) || '';
    }

    _currentTabSlug() {
        const child = this.template.querySelector('c-create-edit-client-lwc');
        if (child) {
    ...
    notifySubRoute(subView, replace) {
        ...
        if (!this._recordUid && !this.ParticpantRecordForm) return;
    ...
    _activePopupSlug() {
        ...
        if (this.showContactAttachmentModal) return 'contact-attachment';
        if (this.showAttachmentListModal) return 'attachment-list';
    ...
    _openPopupBySlug(slug) {
        if (slug === 'upgrade') {
            this.showUpgradeModal = true;
        } else if (slug === 'contact-attachment') {
            this.showContactAttachmentModal = true;
        } else if (slug === 'attachment-list') {
            this.showAttachmentListModal = true;
    ...
    _syncRecordRouteActual() {
        if (this._suppressEmit) {
            console.log('Routing Bridge: _syncRecordRouteActual ignored because _suppressEmit is true.');
            return;
        }
        
        console.log('Routing Bridge: _syncRecordRouteActual starting. editclientflag:', this.editclientflag, 'ParticpantRecordForm:', this.ParticpantRecordForm);
        
        if (!this.editclientflag && !this.ParticpantRecordForm) {
            console.log('Routing Bridge: No edit mode or form open, notifying module root.');
        ...
        if (this.ParticpantRecordForm) {
            ...
            console.log('Routing Bridge: Creation form route composed:', route, 'lastSubRoute:', this._lastSubRoute);
        ...
        if (!this._recordUid) {
            console.log('Routing Bridge: No record UID available, ignoring.');
            return;
        }

        const currentTab = this._currentTabSlug();
        console.log('Routing Bridge: currentTab:', currentTab, 'lastTab:', this._lastTabSlug);
        if (this._lastTabSlug && currentTab !== this._lastTabSlug) {
            this._lastTabSlug = currentTab;
            const route = this._composeSubRoute();
            console.log('Routing Bridge: Tab changed route composed:', route, 'lastSubRoute:', this._lastSubRoute);
            this._lastSubRoute = route;
        ...
        console.log('Routing Bridge: currentIsEdit:', currentIsEdit, 'lastIsEdit:', this._lastIsEdit);
        if (currentIsEdit && !this._lastIsEdit) {
            this._lastIsEdit = currentIsEdit;
            const route = this._composeSubRoute();
            console.log('Routing Bridge: Edit mode entered. Route composed:', route, 'lastSubRoute:', this._lastSubRoute);
            this._lastSubRoute = route;
            this.notifySubRoute(route, false); // push!
            return;
        }
        ...
        const route = this._composeSubRoute();
        console.log('Routing Bridge: default sync route composed:', route, 'lastSubRoute:', this._lastSubRoute);
        if (route === this._lastSubRoute) return;
        this._lastSubRoute = route;
        this.notifySubRoute(route, true); // replace!
    }
    ...
    handleChildStateChange() {
        console.log('Routing Bridge: handleChildStateChange triggered in parent.');
        this._syncRecordRoute();
    }
    ...
    @api async openByUID(uid, tab, isEdit) {
        ...
        try {
            let clientId = this._clientIdForUid(uid);
            let typeofservice = '';
            let participanttype = '';
            let clientRecord = null;

            if (!clientId) {
                // Fetch directly from Apex using the UID!
                const clientRecords = await getClientById({ recordId: uid });
                if (clientRecords && clientRecords.length > 0) {
                    // const clientRecord = clientRecords[0];
                    clientRecord = clientRecords[0];
                    clientId = clientRecord.Id;
                    typeofservice = clientRecord.Facility__r?.Type_of_Service__c || '';
                    participanttype = clientRecord.ParticipantType__c || '';

                    if (!this.participantJson) {
                        this.participantJson = {};
                    }
                    const firstName = clientRecord.First_Name__c || '';
                    const lastName = clientRecord.Last_Name__c || '';
                    this.participantJson[clientRecord.Id] = {
                        uid: clientRecord.Participant_UID__c,
                        street: clientRecord.Address__Street__s,
                        city: clientRecord.Address__City__s,
                        stateCode: clientRecord.Address__StateCode__s,
                        countryCode: clientRecord.Address__CountryCode__s,
                        postalCode: clientRecord.Address__PostalCode__s,
                        status: clientRecord.Status__c,
                        orgId: clientRecord.Facility__r?.Organisation__c,
                        firstName: firstName,
                        firstname: firstName,
                        lastName: lastName,
                        lastname: lastName,
                        name: (`${firstName} ${lastName}`.trim()) || clientRecord.Company__c || clientRecord.Name,
                        typeofservice: typeofservice,
                        participanttype: participanttype,
                        languages: clientRecord.Preferred_Languages__c || "",
                        nationality: clientRecord.Preferred_Nationality__c || ""
                    };                    
                }
            } else {
                const json = this.participantJson[clientId];
            ...
            if (clientId) {
                ...
                if (!rec && this.participantList) {
                    rec = this.participantList.find(c => c.Id === clientId);
                }
                if (rec) {
                    let facilities = [];
                    if (rec.Participant_Facilities__r) {
                        facilities = rec.Participant_Facilities__r;
                ...
                this.loadClientData(clientId, typeofservice, participanttype);
                
                // Read step and popup from hash directly
                const hash = window.location.hash || '';
                const parts = hash.replace(/^#\/?/, '').split('/').filter(Boolean);
                const uidIdx = parts.findIndex(p => /^par-/i.test(p));
                let step = '';
                let popup = '';
                if (uidIdx >= 0) {
    ...
    @api openCreate(step) {
        ...
        try {
            ...
            this.fieldErrorMap = {};
            this.ndisCreateFlag = true;
            this.createCurrentStep = step ? 'create' + step.replace('create', '') : 'createstep1';
        } finally {
            this._suppressEmit = false;
        }
    }
    ...
    _restoreFromUrlHash() {
        ...
        const tab = parts[uidIdx + 1] || 'personal';
        const modes = parts.slice(uidIdx + 2);
