...
import getStaffForCurrentUser from "@salesforce/apex/FormController.getStaffForCurrentUser";
import getAccessibleStaffForms from "@salesforce/apex/FormController.getAccessibleStaffForms";
import saveStaffFormResponse from "@salesforce/apex/FormController.saveStaffFormResponse";
import updateStaffFormResponse from "@salesforce/apex/FormController.updateStaffFormResponse";
import getFormResponsesByStaff from "@salesforce/apex/FormController.getFormResponsesByStaff";
...
export default class FormRender extends LightningElement {
  ...
  loadStaffAccessibleForms() {
    ...
    getAccessibleStaffForms({
      ...
      .catch((error) => {
        console.error("❌ Error fetching accessible forms for staff:", error);
      });
  }
  loadStaffResponses() {
    if (!this.staffId) {
    ...
    editors.forEach((container) => {
      ...
      if (toolbar) {
        const applyFormatSafely = (format, value) => {
          ...
          try {
            if (range.length > 0) {
              quill.formatText(range.index, range.length, format, value !== undefined ? value : !current);
            } else {
              quill.format(format, value !== undefined ? value : !current);
            }
          } catch (e) {
  ...
  async renderRichTextAsImage(doc, html, x, y, width, pageH) {
    console.log("---- Descriptive Text START ----");
  ...
  `;
    ...
    console.log("---- Descriptive Text END ----");
  }
  ...
  async uploadFormJsonToAws(finalRows, folderType, formTitle, existingKey = null) {
    ...
    try {
      ...
      if (existingKey) {
        ...
        key = `CustomisableForms/${this.orgid}/Staff/${dateStr}/${folderType}/${fileName}.json`;
      }
  ...
  async buildPdfDoc() {
    ...
    const tableBlocks = (this.viewTableData || []).filter((i) => i.dataType === "Table Block");
  ...
  async drawPdfContent(doc, logoData) {
    ...
    const drawHeader = () => {
      ...
      doc.text(this.selectedFormTitle || "View Ps", marginX, 22);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(10);
      ...
      if (logoData?.base64) {
        try {
          ...
          doc.addImage(logoData.base64, "PNG", pageW - logoWidth - marginX, logoY, logoWidth, logoHeight);
        } catch (e) {
    ...
    for (const row of rows) {
      ...
      if (row.cells?.[0]?.isSectionHeader) {
        ...
        doc.text(row.cells[0].title, pageW / 2, y, {
          align: "center"
        });
        y += 20;
        continue;
      ...
      row.cells.forEach((cell) => {
        ...
        /* UPLOAD IMAGE */
          ...
          const tableHeight = tableRowHeight * (rowsData.length + 1);
          rowHeight = Math.max(rowHeight, tableHeight + 10);
          return;
        } else if (cell.isRichText && value) {

        /* Descriptive Text HEIGHT (SAFE FALLBACK) */
      ...
      for (const cell of row.cells) {
        ...
        if (cell.isTableBlock) {
          ...
          doc.text((cell.label || "").toUpperCase(), marginX, tableY);
          tableY += 10;
          ...
          headers.forEach((h, ci) => {
            ...
            doc.setFontSize(9);
            doc.setTextColor(30);
            doc.text(h.header || "", hx + colW / 2, tableY + 11, {
              align: "center"
            });
          });
          tableY += rowH;
        ...
        doc.roundedRect(x, boxY, width - 4, rowHeight, 2, 2, "F");
        ...
        if (cell.isSignature && value?.startsWith("data:image")) {
          try {
            ...
            doc.addImage(value, "PNG", x + 4, imgY, imgWidth, imgHeight);
          } catch (e) {}
        } else if (cell.isUploadFile && cell.files?.length) {
        ...
        /* FILE */
          ...
          if (file.isImage) {
            try {
              ...
              const imgHeight = imgWidth * (props.height / props.width);
              const imgY = boxY + (rowHeight - imgHeight) / 2;
              doc.addImage(file.downloadUrl, "JPEG", x + 4, imgY, imgWidth, imgHeight);
            } catch (e) {}
          } else {
        ...
        /* Descriptive Text */
          try {
            await this.renderRichTextAsImage(doc, value, x + 4, boxY + 4, width - 10, pageH);
          } catch (e) {
            const lines = doc.splitTextToSize(String(value), width - 10);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Page ${i} of ${pageCount}`, pageW / 2, pageH - 10, {
        align: "center"
      });
    }
...
`;
  ...
  getBorderStyle(dataType) {
    return ["Checkbox Field", "Radio Button", "Header", "Table Block", "Descriptive Text"].includes(dataType)
  ...
  const state = this.tableSortState.formResponses;
  ...
  if (state.field) {
    ...
    this.paginatedFormResponses = displayRecords.slice(startIndex, endIndex);// manendra added for table sort
...
// Manendra added for table sorting
handleSort(event) {
    ...
    // Clear existing icons for this table only
    ...
    // Apply sorting to the correct table
    switch (table) {

        case 'publishedForms':
            this.filteredForms = this.sortData(
                [...this.filteredForms],
                state.field,
                state.direction
            );
