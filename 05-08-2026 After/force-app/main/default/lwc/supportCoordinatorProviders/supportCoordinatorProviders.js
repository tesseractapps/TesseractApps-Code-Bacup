...
export default class supportCoordinatorProviders extends LightningElement {
    ...
    // HANDLE FIELD INPUT
   /*  handleChange(event) {
        console.log('Service Type Value:', JSON.stringify(event.detail.value));
...
handleChange(event) {
    ...
    if (field === 'Service_Type__c') {
        ...
        // Ignore search box events
        if (!Array.isArray(value)) {
            return;
        }

        this.selectedServiceTypes = [...value];
        this.provider[field] = value.join(';');
        return;
    }
    ...
    openEditModal(event) {
        ...
        this.provider = {
            ...
            Industry_Type__c: p.industryType || 'NDIS'
        };
    ...
    validateProvider() {
        ...
        if (!/^[0-9]{9}$/.test(this.provider.Registration_Number__c)) {
    this.dispatchEvent(
        new ShowToastEvent({
            title: 'Invalid NDIS Provider ID',
    ...
    renderedCallback() {
        if (!this.resizeObserver) {
            const container = this.template.querySelector('.yourTableHeight');
            if (container) {
    ...
    setPageSizeByZoomAndScreen() {
        ...
        const rowHeight = 54;
        let computedPageSize = Math.floor(containerHeight / rowHeight);
        if (computedPageSize < 3) computedPageSize = 3;
