...
//    @track ParticpantRecordForm;
    @track _ParticpantRecordForm = false;
    get ParticpantRecordForm() {
        return this._ParticpantRecordForm;
    }
    set ParticpantRecordForm(value) {
        this._ParticpantRecordForm = value;
        this._syncRoute();
    }
   ...
   @track contactList = [
    ...
    @track sectionFlags = {
        ...
        @track sortIcons = {
            fullName: 'arrow_upward',
            Contact_Number__c: 'unfold_more',
            Address__Street__s: 'unfold_more',
            activeFaciltyDisplay: 'unfold_more',
            Participant_Status__c: 'unfold_more'
        };
  ...
  @track isMedicareEntered = false;
    ...
    //     console.log('[Routing] openByUID called on c-participant-module-lwc:', uid, tab, isEdit);
    @api async openByUID(uid, tabPath, isEdit) {
        console.log('[Routing] openByUID called on c-participant-module-lwc:', uid, tabPath, isEdit);    
        this.isShowSpinner = true;
        try {
            const results = await getClientById({ recordId: uid });
            if (results && results.length > 0) {
                ...
                const facId = rec.Id;
                const participantName = rec.Display_Nickname__c || `${rec.First_Name__c || ''} ${rec.Last_Name__c || ''}`.trim() || rec.Name__c;
                const typeofservice = rec.Facility__r?.Type_of_Service__c || '';
                const participanttype = rec.ParticipantType__c || '';

                if (!this.participantJson) {
                    this.participantJson = {};
                }
                this.participantJson[facId] = {
                ...
                this.isHome = false;
                this.orgId = rec.Facility__r?.Organisation__c;
                localStorage.setItem('clientRecordId', facId);

                // Wait for detailsLwc to render, then select the tab
                // setTimeout(() => {
                //     const detailsCmp = this.template.querySelector('c-details-lwc');
                //     if (detailsCmp) {

                //         const currentHashParts = (window.location.hash || '').replace(/^#\/?/, '').split('/').filter(Boolean);
                //         const currentUidIdx = currentHashParts.findIndex(p => /^par-/i.test(p));
                //         let latestTabPath = tabPath;
                //         if (currentUidIdx >= 0 && currentHashParts[currentUidIdx] === uid) {
                //             latestTabPath = currentHashParts.slice(currentUidIdx + 1).join('/') || 'details';
                //         }
                //         detailsCmp.selectTab(latestTabPath);
                //     }
                // }, 400);
setTimeout(() => {
    const detailsCmp = this.template.querySelector('c-details-lwc');
    if (detailsCmp) {
        console.log('[Routing] Selecting tab:', tabPath);
        detailsCmp.selectTab(tabPath || 'details');
    }
}, 400);                
            } else {
                console.error('[Routing] Participant not found for UID:', uid);
            }
        } catch (e) {
            console.error('[Routing] Error in openByUID:', e);
        } finally {
            this.isShowSpinner = false;
        }
    }

    @api selectTab(tabSlug) {
        console.log('[Routing] selectTab called on c-participant-module-lwc with:', tabSlug);
        const detailsCmp = this.template.querySelector('c-details-lwc');
        if (detailsCmp) {
            detailsCmp.selectTab(tabSlug);
        }
    }

    @api restoreFromUrl() {
        console.log('[Routing] restoreFromUrl called on c-participant-module-lwc');
        ...
        if (uidIdx >= 0) {
            ...
            const tabPath = parts.slice(uidIdx + 1).join('/');
            this.openByUID(uid, tabPath);
        } else if (parts.includes('create')) {
            const stepIdx = parts.indexOf('create');
            const step = parts[stepIdx + 1] || 'step1';
            ...
            this.cardFlag = true;
            this.listFlag = false;
            this._ParticpantRecordForm = false;
            localStorage.removeItem('clientRecordId');
        }
    }

    @api openParticipant(uid) {
        console.log('[Routing] openParticipant called on c-participant-module-lwc with:', uid);
        this.openByUID(uid, 'details', false);
    }    

    @api openCreate(step) {
        console.log('[Routing] openCreate called on c-participant-module-lwc with step:', step);
        this.ndisCreateFlag = false;
        ...
        this.recordId = '';
        this._ParticpantRecordForm = true;
        if (step) {
            this._createCurrentStep = step.replace('step', 'createstep');
        } else {
            this._createCurrentStep = 'createstep1';
        }
        this._syncRoute();
    }

    _syncRoute() {
        if (this._ParticpantRecordForm && !this.recordId) {
            const stepNum = this._createCurrentStep ? this._createCurrentStep.replace('createstep', '') : '1';
            const subView = `create/step${stepNum}`;
            console.log('[Routing] participantModuleLwc dispatching subrouteupdate:', subView);
        ...
        } else if (!this._ParticpantRecordForm && !this.editstaffflag) {
            console.log('[Routing] participantModuleLwc clearing subView');
            this.dispatchEvent(new CustomEvent('subrouteupdate', {
                detail: {
                    subView: '',
                    recordId: null,
...
@track _createCurrentStep = 'createstep1';
get createCurrentStep() {
    return this._createCurrentStep;
}
set createCurrentStep(value) {
    this._createCurrentStep = value;
    this._syncRoute();
}
   ...
   @wire(fetchParticipantWithPagination, {
    ...
    if (result.data) {
     ...
     //   this.visible = this.totalRecords > this.pageSize;
        ...
        const parts = (window.location.hash || '').replace(/^#\/?/, '').split('/').filter(Boolean);
        const hasUid = parts.some(p => /^par-/i.test(p));
        if (hasUid) {
            console.log('[Routing] Deep-link UID present in URL hash, prioritizing routing restore in post-wire.');
            this.restoreFromUrl();
        } else {
...
get bDisableLast() {
   ...
   childevent(event){
        ...
        // Notify dashboard shell to reset module root URL (back to list view)
        this.dispatchEvent(new CustomEvent('subrouteupdate', {
...
handleFacilitySearchChange(event) {
    this.facilitySearchTerm = event.target.value ?. trim() ?. toLowerCase();
    this.showFacilityOptions = true;
    console.log('Search:', this.facilitySearchTerm);
}
get filteredFacilityCheckboxOptions() {
    if (!this.facilitySearchTerm) {
        return this.facilityCheckboxOptions;
    }
    return this.facilityCheckboxOptions.filter(option => option.label ?. toLowerCase() ?. includes(this.facilitySearchTerm));
}
...
get tableClass() {
    return `table-container participantTable ${this.statusFlag ? 'with-status' : 'without-status'} slds-grid slds-grid_vertical slds-grow slds-scrollable_none`;
}
...
// manendra added for sorting the data in table  Server-side sorting
    handleSort(event) {
        ...
        Object.keys(this.sortIcons).forEach(key => {
            this.sortIcons[key] = 'unfold_more';
        });
