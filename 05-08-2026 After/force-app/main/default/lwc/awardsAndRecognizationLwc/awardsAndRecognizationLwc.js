...
import getAwardById from '@salesforce/apex/AwardController.getAwardById';
...
export default class AwardsAndRecognizationLwc extends LightningElement {
    ...
    // Routing state backing variables
    @track _Awardflag = false;
    @track _homeflag = false;
    @track _orgadmin = false;
    @track _staffuser = false;
    @track _isModalOpen = false;
    @track _showUpgradeModal = false;
    ...
    get Awardflag() { return this._Awardflag; }
    set Awardflag(val) {
        this._Awardflag = val;
        this._syncRecordRoute();
    }

    get homeflag() { return this._homeflag; }
    set homeflag(val) {
        this._homeflag = val;
        this._syncRecordRoute();
    }

    get orgadmin() { return this._orgadmin; }
    set orgadmin(val) {
        this._orgadmin = val;
        this._syncRecordRoute();
    }

    get staffuser() { return this._staffuser; }
    set staffuser(val) {
        this._staffuser = val;
        this._syncRecordRoute();
    }
            ...
            .then(data => {
               ...
               // this.noRecordsFlag1 = !(data && data.length > 0);
                ...
                if (data) {
                    ...
                    this.staffTotalRecords = this.staffRecords.length;
                    this.staffPageSize = this.isPageSizeManuallySet ? this.staffPageSize : 10;
                    this.staffPageNumber = 1;
    ...
    wiredAwards(result) {
       this.showSpinner=true;
        ...
        if (data) {
            ...
            const mappedAwards = data.map(Award => {
               ...
               getCurrentLoggedUserInfo().then(userData=>{
                    ...
                    this.facilityOptions = [{
                        ...
                        if( userTpe =='NDIS Org Admin' || userTpe == 'ICT Admin'){
                            ...
                            console.log('Fetch Participant>>>'+ JSON.stringify(mappedAwards));
                            finalData =mappedAwards;
                            let facilityIds = [];
                                // finalData =response;
                                    console.log('Fetch Participant filteredData>>>'+ JSON.stringify(mappedAwards));
                                facilityIds.push(storedFacilityId); 
                                console.log('facilityIds  '+JSON.stringify(facilityIds))
                                const filteredData = mappedAwards.filter(rec =>
...
boundOutsideClick
    ...
    paginationrefresh(){
        refreshApex(this.wiredorgAwards).then(() => {
            ...
            this.pageSize = this.isPageSizeManuallySet ? this.recordsPerPage : 10; // Set the default page size
    ...
    handleSubmit(event){
        ...
        fields.Archieve__c = this.Archieve;
        fields.Template_Name__c = this.awardValue;
   ...
   /*  handleStaffChange(event) {
    ...
    handleView(event) {
        ...
        this._selectedAwardId = event.currentTarget.dataset.id;
        this._selectedAwardUid = event.currentTarget.dataset.uid;        
    ...
    closepdf(event){
        this._selectedAwardId = null;
        this._selectedAwardUid = null;        
        if(this.staff){
   ...
   /*  fetchAwards(event){
    ...
    setPageSizeByZoomAndScreen() {
        ...
        setTimeout(() => {
            ...
            if (container) {
                ...
                const header = this.template.querySelector('.fixed-table thead') || this.template.querySelector('.staff-table thead');
                ...
                let rowHeight = 40;
                const firstRow = this.template.querySelector('.fixed-table tbody tr') || this.template.querySelector('.staff-table tbody tr');
                if (firstRow) {
                ...
                } else if (this.staffuser) {
                    const oldSize = this.staffPageSize;
                    this.staffPageSize = rows;
                    this.staffTotalPages = Math.ceil(this.staffTotalRecords / this.staffPageSize);
                    if (this.staffPageSize !== oldSize) {
                        this.staffPageNumber = 1;
                        this.staffPaginationHelper();
                    }
                }
    ...
    get pageSizeOptions() {
        return [
            ...
            { label: '10', value: 10, selected: this.isPageSizeManuallySet && (this.orgadmin ? this.recordsPerPage === 10 : this.staffPageSize === 10) },
            { label: '20', value: 20, selected: this.isPageSizeManuallySet && (this.orgadmin ? this.recordsPerPage === 20 : this.staffPageSize === 20) },
            { label: '50', value: 50, selected: this.isPageSizeManuallySet && (this.orgadmin ? this.recordsPerPage === 50 : this.staffPageSize === 50) }
        ];
    }
    previousPage() {
    ...
    paginationHelper() {
       ...
       // console.log("totalPages  : "+ JSON.stringify(this.totalPages));
        ...
        let tempconList=[]; 
        this.records = this.sortData(this.records);// manendra added for sorting data on pagination  
...
staffPaginationHelper() {
    ...
    let tempList = [];
this.staffRecords = this.sortData(this.staffRecords);// manendra added for sorting the data in table
    for (
...
handleStaffPageSize(event) {
    ...
    if (selectedValue === 'Auto') {
        ...
        this.isPageSizeManuallySet = true;
        this.staffPageSize = parseInt(selectedValue, 10);
        this.staffPageNumber = 1;
        this.staffPaginationHelper();
    }
}
...
//end
    ...
    handleSubPopup(event) {
        this._childPopup = event.detail.slug || '';
        this._syncRecordRoute();
    }

    get _recordUid() {
        return this._selectedAwardUid || '';
    }
    ...
    _composeSubRoute() {
        ...
        if (this.isModalOpen && this._recordUid) {
            return `${this._recordUid}/certificate`;
        }
        return '';
    }
    ...
    _syncRecordRouteActual() {
        ...
        if (!this.Awardflag && !this.isModalOpen) {
    ...
    notifySubRoute(subView, replace) {
        ...
        this.dispatchEvent(new CustomEvent('subrouteupdate', {
            detail: { subView, recordId: this._selectedAwardId || null, replace: !!replace },
            bubbles: true,
            composed: true
        }));
    }
    ...
    @api async openByUID(uid, tab, isEdit) {
        ...
        try {
            ...
            let foundAward = null;
            if (this.orgAwards && this.orgAwards.length > 0) {
                foundAward = this.orgAwards.find(a => (a.Award_UID__c && a.Award_UID__c.toLowerCase() === uid.toLowerCase()) || a.Id === uid);
            }
            if (!foundAward && this.staffRecords && this.staffRecords.length > 0) {
                foundAward = this.staffRecords.find(a => (a.Award_UID__c && a.Award_UID__c.toLowerCase() === uid.toLowerCase()) || a.Id === uid);
            }

            if (!foundAward) {
                const results = await getAwardById({ recordId: uid });
                if (results && results.length > 0) {
                    let rawAward = results[0];
                    foundAward = {
                        ...rawAward,
                        facilityName: `${rawAward.Facility__r?.Name ?? 'N/A'}`,
                        Todate: rawAward.Date__c ? new Date(rawAward.Date__c).toLocaleDateString('en-GB') : '',
                        fullName: `${rawAward.Staff__r?.Display_Nickname__c ?? 'N/A'}`
                    };
                }
            }

            if (foundAward) {
                this._selectedAwardId = foundAward.Id;
                this._selectedAwardUid = foundAward.Award_UID__c;
                this.currentUrl = foundAward.Certificate__c;
                
                if (foundAward.Award_Name__c === 'Community Champion Award') {
                    this.communityflag = true;
                } else {
                    this.communityflag = false;
                }
                ...
                this._staffuser = false;
                this._orgadmin = false;
                this._homeflag = false;
            } else {
                console.error('Routing Bridge: Award not found for UID:', uid);
            }
        } catch (e) {
    ...
    _restoreFromUrlHash() {
        ...
        const hasCreate = parts.includes('issue-award') || parts.includes('create');
        if (hasCreate) {
            this._hasRestored = true;
            const createIdx = parts.indexOf('issue-award') !== -1 ? parts.indexOf('issue-award') : parts.indexOf('create');
            const step = parts[createIdx + 1] || '';
            this.openCreate(step);
            this._lastSubRoute = this._composeSubRoute();
            return true;
        }

        const uidIdx = parts.findIndex((p) => /^awd-/i.test(p));
        if (uidIdx < 0) return false;
        this._hasRestored = true;
        const uid = parts[uidIdx];
        const tab = parts[uidIdx + 1] || 'certificate';
    ...
    @api currentTabSlug() {
        return this.isModalOpen ? 'certificate' : '';
    }
...
@track awardRecordId;
    handleEdit(event) {
    event.preventDefault();
     this.awardRecordId = event.currentTarget.dataset.id;

    const award = this.records.find(
        rec => rec.Id === this.awardRecordId
    );
    ...
    this.awardDisplayText = award.Award_Name__c;
    this.StaffFacility = award.Facility__c;
    this.selectedStaff = award.Staff__c;
    this.currentDate = award.Date__c;
    this.manager = award.Issued_by__c;
    this.Archieve = award.Archieve__c; 
    this.awardValue = award.Template_Name__c;
    this.certificateImages = this.certificateImages.map(cert => ({
        ...cert,
        isSelected: cert.label === this.awardValue
    }));

    const selected = this.certificateImages.find(
        cert => cert.label === this.awardValue
    );

    if (selected) {
        this.selectedCertificateImage = selected.url;
    }
    this.handlefacility({
        detail: {
            value: award.Facility__c
        }
    });
}

handleArchieve(event) {
    event.preventDefault();

    const awardId = event.currentTarget.dataset.id;
    const isArchived = event.currentTarget.dataset.archieve === 'true';
    console.log('archieve',isArchived);

    const fields = {
        Id: awardId,
        Archieve__c: !isArchived
    };
    ...
    updateRecord({ fields })
        .then(() => {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Success',
                    message: !isArchived? 'Award archived successfully.': 'Award unarchived successfully.',
                    variant: 'success'
                })
            );

            return refreshApex(this.wiredorgAwards);
        })
        .catch(error => {
            console.error(error);
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message: error.body?.message || 'Unable to archive award.',
                    variant: 'error'
                })
            );
        });
