...
import getTrainingById from '@salesforce/apex/HRTraining.getTrainingById';
...
//  @track activeTab='training';
    ...
    set showModal(val) {
        const oldVal = this._showModal;
        this._showModal = val;
        if (val && !oldVal) {
            this.dispatchEvent(new CustomEvent('subpopup', { detail: { slug: 'edit' }, bubbles: true, composed: true }));
        } else if (!val && oldVal) {
            this.dispatchEvent(new CustomEvent('subpopup', { detail: { slug: '' }, bubbles: true, composed: true }));
        }
        this._syncRecordRoute();
    }    
   ...
   // 🔹 Master + Segments
    ...
    @track _showAssignStaffModal = false;
    get showAssignStaffModal() { return this._showAssignStaffModal; }
    set showAssignStaffModal(val) {
        const oldVal = this._showAssignStaffModal;
        this._showAssignStaffModal = val;
        if (val && !oldVal) {
            this.dispatchEvent(new CustomEvent('subpopup', { detail: { slug: 'assign' }, bubbles: true, composed: true }));
        } else if (!val && oldVal) {
            this.dispatchEvent(new CustomEvent('subpopup', { detail: { slug: '' }, bubbles: true, composed: true }));
        }
        this._syncRecordRoute();
...
handleAssignCourse(event) {
    ...
    const trainingId = event.currentTarget.dataset.id;
        const trainingUid = event.currentTarget.dataset.uid;
        this._selectedRecordId = trainingId;
        this._selectedRecordUid = trainingUid;   
...
closeAssignModal(){
    ...
    _composeSubRoute() {
        if (this.activeTab === 'learners') {
            return 'learning';
        }
        if (this.showModal && this._recordUid) {
            return `${this._recordUid}/edit`;
        }
        if (this.showAssignStaffModal && this._recordUid) {
            return `${this._recordUid}/assign`;
        }
        return '';
    }
    ...
    _syncRecordRouteActual() {
        if (this._suppressEmit) return;
        if (this.activeTab === 'training' && !this.showModal && !this.showAssignStaffModal) {
    ...
    @api async openByUID(uid, tab, isEdit) {
        ...
        try {
            let found = this.adminTrainingList.find(r => r.uid === uid || r.id === uid);
            if (!found) {
                const results = await getTrainingById({ recordId: uid });
                if (results && results.length > 0) {
                    const rawTraining = results[0];
                    found = {
                        id: rawTraining.Id,
                        name: rawTraining.Module_Name__c,
                        category: rawTraining.Training_Category__c,
                        categoryClass: 'category-default',
                        mandatory: rawTraining.Mandatory_Document__c,
                        visibleToStaff: rawTraining.Visible_to_Staff__c,
                        renewalRequired: rawTraining.Renewal_Date_Required__c,
                        validFor: rawTraining.Valid_For__c,
                        notes: rawTraining.Notes__c,
                        courseOwnerId: rawTraining.Course_Owner_Id__c,
                        wyzedCourseId: rawTraining.Wyzed_Course_ID__c,
                        uid: rawTraining.Training_UID__c
                    };
                }
            }
            if (found) {
                this._selectedRecordId = found.id;
                this._selectedRecordUid = found.uid;
                this.activeTab = 'training';
                
                if (tab === 'assign') {
                    this.openAssignModalForRecord(found);
                } else {
                    this.modalTitle = "Edit Training";
                    this.editingDoc = { ...found };
                    this.showModal = true;
                    this.isEditMode = true;
                    
                    this.selctedMultipleFcailityValues = [];
                    this.selectedRoleValueLabels = [];
                    getFacilityComplianceByTraining({ trainingId: found.id })
                        .then(result => {
                            const facilityIds = [...new Set(result.map(r => r.Facility__c))];
                            this.existingChildData = result;
                            this.selctedMultipleFcailityValues = facilityIds;
    ...
    openAssignModalForRecord(training) {
        this.selectedCourseId = training.wyzedCourseId;
        this.selectedCourseOwnerUserId = training.courseOwnerId;
        this.editingDoc = { ...training };
        
        getFacilityComplianceByTraining({ trainingId: training.id })
        .then(result => {
            const facilityIds = [...new Set(result.map(r => r.Facility__c))];
            this.selctedMultipleFcailityValues = facilityIds;
            this.existingChildData = result;
            return getRoleOptionsByFacility({ facilityIdList: facilityIds });
        })
        .then(roleResult => {
            this.roleoptionsforFacility = roleResult.map((role, index) => {
                return {
            ...
            this.existingChildData.forEach(child => {
                ...
                roles.forEach(roleName => {
                    const match = this.roleoptionsforFacility.find(
                        r => r.label === roleName.trim() && r.facilityValue === child.Facility__c
                    );
                    if (match) {
                        selectedValues.push(match.combinedValue);
                    }
            ...
            this.selectedRoleValueLabels = selectedValues;
            return getStaffByFacilityAndRoles({ selectedRoleFacilityValues: this.selectedRoleValueLabels });
        })
        .then(result => {
            this.filteredStaffList = result.map(staff => {
                return { ...staff, isChecked: true };
            });
            this.selectedStaffIds = this.filteredStaffList.map(s => s.Id);
            this.selectedWyzedUserIds = this.filteredStaffList
                .filter(s => s.Wyzed_User_Id__c)
                .map(s => s.Wyzed_User_Id__c);
            this.selectedCourseOwnerUserId = this.editingDoc.courseOwnerId;
            this.showAssignStaffModal = true;
        })
        .catch(error => {
            console.error('Assign Course Error =>', JSON.stringify(error));
        });
    }
    ...
    _restoreFromUrlHash() {
        ...
        const hasLearning = parts.includes('learning');
        if (hasLearning) {
        ...
        const uidIdx = parts.findIndex((p) => /^trn-/i.test(p));
    ...
    @api currentTabSlug() { return this.showModal ? 'edit' : (this.showAssignStaffModal ? 'assign' : ''); }
    @api selectTab(slug) {}
    @api startEdit() {}
    @api setStep(step) {}
    @api openPopup(slug) {}
...
/* 
    ...
    if (this.activeTab === 'training') {
        ...
        this.adminTrainingList = this.sortData(this.adminTrainingList);

        this.totalPages = Math.ceil(
            (this.adminTrainingList || []).length /
            this.recordsPerPage
        );
        ...
        this.filteredList = this.sortData(this.filteredList);

        this.allTotalPages = Math.ceil(
