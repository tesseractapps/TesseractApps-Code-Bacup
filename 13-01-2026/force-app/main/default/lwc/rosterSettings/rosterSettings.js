...
handleMaxDurationInputForCustom(event) {
  ...
  if (rawValue.includes(".")) {
    ...
    if (parts[1].length > 2) {
      console.log(
        `⚙️ Truncating decimals: ${parts[1]} → ${parts[1].substring(0, 2)}`
      );
      rawValue = `${parts[0]}.${parts[1].substring(0, 2)}`;
      event.target.value = rawValue;
      console.log("🔧 Updated input after truncation:", rawValue);
  ...
  if (rawValue === "") {
    ...
    console.log(
      "✅ Updated templates after clearing blank value:",
    ...
    if (String(template.id) === String(templateId)) {
      const updatedShifts = template.shifts.map((shift) => {
        if (String(shift.id) === String(shiftId)) {
          ...
          // 🧩 FINAL LOGIC — YOUR REQUESTED CONDITION
          if (entered > 0 && actual > 0 && entered < actual) {
            console.error(
              `⛔ Duration Mismatch: entered < actual (${entered} < ${actual})`
            );
            ...
            Promise.resolve().then(() => {
              this.showToast(
                "Duration Error",
                `Actual Duration (${actual.toFixed(
                  2
                )} hrs) cannot be greater than Entered Duration (${entered.toFixed(
                  2
                )} hrs).`,
                "error"
              );
