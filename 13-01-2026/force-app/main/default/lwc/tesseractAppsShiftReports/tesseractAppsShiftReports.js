...
@track sections = [
    { key: 'shiftInformation', label: 'Shift Information', checked: false },
    { key: 'incidentRegister', label: 'Incident Register', checked: false },
    { key: 'forms', label: 'Forms', checked: false },
    { key: 'participantJournal', label: 'Participant Journal', checked: false },
    { key: 'activityLogs', label: 'Activity Logs', checked: false },
    { key: 'servicesDelivered', label: 'Services Delivered', checked: false },
    { key: 'reimbursements', label: 'Reimbursements', checked: false },
    { key: 'shiftNotes', label: 'Shift Notes', checked: false },
    ...
    async exportFilteredCSV() {
        try {
            ...
            const sectionHeaders = selectedSections.map(sec => {
                switch (sec.key) {
                    case 'shiftInformation': return 'Shift Information';
                    case 'incidentRegister': return 'Incident Register';
                    ...
                    case 'shiftNotes': return 'Shift Notes';
    ...
    console.log('Shift IDs:', this.shiftData.map(s => s.Id));
            ...
            if (selectedValue === 'exportAll') {
                console.log('⬇️ Export All selected');
                console.log('🟢 EXPORT ALL START');
                console.log('Total shifts:', this.shiftData.length);
                ...
                try {
                    ...
                    /* =========================
                    Get ALL shift IDs for bulk processing
                    ========================= */
                    const shiftIds = this.shiftData.map(shift => shift.Id);
                    console.log(`🔄 Fetching data for ${shiftIds.length} shifts in ONE call`);
                    
                    /* =========================
                    Make ONE Apex call to get ALL data
                    ========================= */
                    const allData = await getAllExportData({
                        role: this.selectedRole,
                        employmentType: this.selectedEmploymentType,
                    ...
                    console.log('🟢 RAW getAllExportData response:', JSON.stringify(allData));
                    if (allData.status === 'error') {
                        throw new Error(allData.message);
                    }
                    
                    const shiftDetails = allData.shiftDetails;
                    console.log(`✅ Received ${shiftDetails.length} shift details`);
                    shiftDetails.sort((a, b) => {
                    const dateA = a.shift.Date__c ? new Date(a.shift.Date__c) : new Date(0);
                    const dateB = b.shift.Date__c ? new Date(b.shift.Date__c) : new Date(0);
                    return dateA - dateB;
                    ...
                    // CSV Headers
                    const headers = [
                        'Date', 'Shift Type', 'Staff Name', 'Scheduled Time', 'Participants',
                        'Shift Information', 'Incident Register', 'Forms', 'Participant Journal',
                        'Activity Logs', 'Services Delivered', 'Reimbursements', 'Shift Notes',
                        'Checklist', 'Signature'
                    ];
                    ...
                    /* =========================
                    Process ALL shifts WITHOUT additional Apex calls
                    ========================= */
                    for (let i = 0; i < shiftDetails.length; i++) {
                        const shiftDetail = shiftDetails[i];
                        const shift = shiftDetail.shift;
                        ...
                        console.log('Journals count:', journals.length);
                        console.log('Forms count:', forms.length);
                        console.log(`📄 Processing shift ${i+1}/${shiftDetails.length}:`, shift.Id);
                        
                        // Convert the shift to your expected format
                        const shiftFormatted = {
                            Id: shift.Id,
                            date: shift.Date__c ? new Date(shift.Date__c).toLocaleDateString('en-GB') : 'No Date',
                            shiftType: shift.Type_of_shift__c || '',
                            name: shift.Staff__r?.Display_Nickname__c || shift.Staff_Name__c || '',
                        ...
                        checklist: checklist.map(c => ({
                            ...
                            // NEW: Add the fetched related data
                            incidents: incidents || [],
                            forms: forms || [],
                            journals: journals.map(j => ({
                                notes: j.Care_Notes__c || '',
                        ...
                        // Build each field with proper formatting
                        const rowData = [
                            this.escapeCSV(shiftFormatted.date || ''),
                            this.escapeCSV(shiftFormatted.shiftType || ''),
                            this.escapeCSV(shiftFormatted.name || ''),
                            this.escapeCSV(shiftFormatted.scheduledTime || ''),
                            this.escapeCSV(shiftFormatted.participants || ''),
                            this.escapeCSV(this.buildShiftInformation(shiftFormatted)),
                            this.escapeCSV(this.buildIncidentRegister(details.incidents)),
                            this.escapeCSV(this.buildForms(details.forms)),
                            this.escapeCSV(this.buildParticipantJournal(details.journals)),
                            this.escapeCSV(this.buildActivityLogs(details.activityLogs)),
                            this.escapeCSV(this.buildServicesDelivered(details.servicesDelivered)),
                            this.escapeCSV(this.buildReimbursements(details.reimbursements)),
                            this.escapeCSV(this.buildNotes(details.shiftNotes)),
                            this.escapeCSV(this.buildChecklist(details.checklist)),
                            this.escapeCSV(shift.Client_Signature__c ? 'Signed' : 'Not Signed')
                        ];
                        
                        csvContent += rowData.join(',') + '\n';
                    }
                    
                    console.log('✅ CSV Content built successfully');
                    ...
                    const downloadElement = document.createElement('a');
                    downloadElement.href = url;
                    downloadElement.download = `ShiftReport_${formattedSDate}_to_${formattedEDate}.csv`;
                    document.body.appendChild(downloadElement);
                    downloadElement.click();
    ...
    /* handleTemplateChange(event) {
        ...
        // When selecting existing template, load and apply it
...
handleTemplateChange(event) {
    ...
    /* applyTemplate(templateJson) {
            ...
            // Apply template without triggering unsaved changes
            this.sections = this.sections.map(sec => ({
                ...sec,
                checked: templateJson[sec.key] === true
            }));
            
            // Reset unsaved changes when applying a template
            this.hasUnsavedChanges = false;
    } */
   applyTemplate(templateJson) {
    if (!templateJson || typeof templateJson !== 'object') {
