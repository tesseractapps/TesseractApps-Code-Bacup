...
export default class FacilityDataCommunity extends NavigationMixin (LightningElement)  {
    ...
    serviceDocumentsMap = {
        'NDIS': [
            ...
            'Police Check',
            'Right to Work (Passport/Visa)',
            'WWCC (Working With Children Check)',
            'First Aid / CPR',
            'Relevant Qualification (Cert III)'
        ],
        'Nursing': [
            'AHPRA Registration',
            'Police Check',
        ...
        'Child Care': [
            ...
            'Police Check',
            'Right to Work',
            'Early Childhood Qualification (Cert III / Diploma)'
        ]
    };
...
async fetchFacilities() {
    try {
        console.log('fetchFacilities: called with firstname =', this.firstname, 'selectedName =', this.selectedName);
        ...
        console.log('fetchFacilities: raw response length =', Array.isArray(response) ? response.length : 'not array');

        let finalFacilityList = [];

        if (this.userType === 'NDIS Org Admin' || this.userType === 'ICT Admin') {
            finalFacilityList = response;
            console.log('fetchFacilities: userType =', this.userType, ', using full response list, length =', finalFacilityList.length);
        } else if (
            ...
            console.log('fetchFacilities: applying facilityIds filter, Ids length =', facilityIds.length);

            finalFacilityList = response.filter(rec => facilityIds.includes(rec.Id));
            console.log('fetchFacilities: after facilityIds filter, length =', finalFacilityList.length);
        }
        ...
        console.log(
            'fetchFacilities: refreshTable length =',
            this.refreshTable ? this.refreshTable.length : 0
        );
        ...
        console.log(
            ...
            'noRecordsFlag =', this.noRecordsFlag,
...
handleSearchKeyPress(event) {
    ...
    const value = this.firstname ? this.firstname.trim() : '';

    console.log('handleSearchKeyPress: value =', value);

    if (value) {
        this.applyFilters();
    } else {

        console.log('handleSearchKeyPress: search cleared, refetching facilities');
        this.fetchFacilities(); 
    }
}
...
handleClear() {
    console.log('handleClear: called, clearing firstname and refetching facilities');
    this.firstname = '';
    this.fetchFacilities();
}
...
applyFilters() {
    console.log(
        'applyFilters: START',
        'firstname =', this.firstname,
        'activeFilterOn =', this.activeFilterOn,
        'inactiveFilterOn =', this.inactiveFilterOn,
        'refreshTable length =', this.refreshTable ? this.refreshTable.length : 0
    );
    ...
    if (this.firstname && this.firstname.trim() !== '') {
        ...
        console.log('applyFilters: applying text search with key =', searchKey);
        ...
        console.log('applyFilters: NO text search applied (firstname empty/blank)');
    }

    const afterSearchCount = result.length;
    console.log(
        'applyFilters: after text search',
    ...
    if (this.activeFilterOn && !this.inactiveFilterOn) {
        result = result.filter(fac => fac.Status__c === true);
        console.log('applyFilters: applied ACTIVE filter only');
    } else if (this.inactiveFilterOn && !this.activeFilterOn) {
        result = result.filter(fac => fac.Status__c === false);
        console.log('applyFilters: applied INACTIVE filter only');
    } else {
        console.log('applyFilters: NO status filter or both off');
    }

    const afterStatusCount = result.length;
    console.log(
        'applyFilters: after status filter',
