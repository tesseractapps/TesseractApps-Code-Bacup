...
ob_confirmApprove = async () => { 
  ...
  try {
    ...
    // Prefer the live toggle value; if not present, fall back to existing JSON.
    let isActive;
    if (typeof this.ob_statusActive === 'boolean') {
      isActive = this.ob_statusActive;
    } else {
      const cur = data?.employment?.status;
      // Fallback keeps current behavior if a string slipped in earlier.
      isActive = (typeof cur === 'boolean') ? cur : String(cur ?? '').trim().toLowerCase() === 'active';
    }

    // Ensure employment exists and write a strict boolean
    if (!data.employment || typeof data.employment !== 'object') data.employment = {};
    data.employment.status = !!isActive;
