...
export default class TesseractAppsUserManagement extends LightningElement {
    ...
    handleDownloadTemplate(event){
        ...
        if(this.typeOfUser=='NDIS'){
            const fileUrl = '/resource/Staff_Upload_Template';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        } else if(this.typeOfUser=='ICT'){
            const fileUrl = '/resource/Staff_Upload_Template_ICT';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        }
    }
    ...
    handleInstructions(event){
        ...
        if(this.typeOfUser=='NDIS'){
            const fileUrl = '/resource/Bulk_Uploading_Instructions_For_Staff';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
            
        } else if(this.typeOfUser=='ICT'){
            const fileUrl = '/resource/Staff_Bulk_Upload_Instructions_For_ICT_Users';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        }
    ...
    handleUploadFinished(event) {
        try {
            ...
            console.log('Uploaded File:', fileInput.name);
            ...
            if (!fileInput.name.toLowerCase().endsWith('.csv')) {
                ...
                event.target.value = null;
                this.handleGlobalError(null, 'Please upload a valid CSV file.');
            ...
            this.showToast("Error", "Something went wrong while uploading file.", "error");
            this.uploadfilenameflag = false;
        } finally {
            // If needed
    ...
    handleFinalSubmit() {
        ...
        // 3️⃣ Mandatory Fields (ABN/TFN NOT included here)
        let requiredFields = [
        ...
        if (this.typeOfUser !== 'ICT') {
            requiredFields.push({ key: 'Nationality', label: 'Nationality' });
            requiredFields.push({ key: 'Languages', label: 'Languages' });
        }
        ...
        // 4️⃣ Validate each selected record
        this.tableData = this.tableData.map(user => {
            if (user.isSelected && user.ProcessedStatus !== 'Success ✅') {
                ...
                // 🔥 ABN–TFN RULE: Exactly ONE required
                ...
                if ((!abn && !tfn) || (abn && tfn)) {
                    ...
                    const message =
    `For ${name}, please provide ONLY ONE of the following:
    - Australian Business Number (ABN)
    - Tax File Number (TFN)
    You cannot leave both empty and cannot fill both.`;
        ...
        if (hasInvalidRecords) {
            console.warn("⚠ Validation errors found. Stopped processing.");
            return;
        }

        // 5️⃣ Build valid records to send to Apex
        let validRecords = [];
        ...
        try {
            validRecords = unprocessedRecords
                ...
                .map(user => {
                    ...
                    if (!isValidDateFormat(formattedDOB)) {
                        throw new Error(`Invalid date format in "Date of Birth" for ${name}. Expected dd/MM/yyyy.`);
                    }

                    if (!isValidDateFormat(formattedStart)) {
                        throw new Error(`Invalid date format in "Start Date" for ${name}. Expected dd/MM/yyyy.`);
                    }
            ...
            // 6️⃣ Send valid records to Apex
            if (validRecords.length > 0) {
                console.log(`📤 Submitting ${validRecords.length} valid records`, JSON.stringify(validRecords));
        ...
        createAccountsContactsAndStaff({ records: formattedRecords })
            .then(result => {
                ...
                this.tableData = this.tableData.map(user => {
                    ...
                    // Find this row result using EMAIL (unique key)
                    ...
                    // If Apex did not return anything for this user → treat as failed
                    return {
                        ...user,
                        ProcessedStatus: "Error ❌: Unknown Processing Error"
                    };
                });
            ...
            .catch(error => {
                ...
                // Only mark selected rows → not ALL
                this.tableData = this.tableData.map(user => {
                    if (user.isSelected) {
                        return {
                            ...user,
