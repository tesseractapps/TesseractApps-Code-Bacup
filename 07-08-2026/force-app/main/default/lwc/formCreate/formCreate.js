...
import saveFormWithVersion from "@salesforce/apex/LayoutController.saveFormWithVersion";
...
export default class DragDropComponent extends NavigationMixin(
  ...
  { id: '13', label: 'Descriptive Text', icon: 'notes' }
];
...
getTypeFlags(type) {
  return {
    ...
    isRichTextDisplay: type === 'Descriptive Text',
  ...
  get isUndoDisabled() {
    return !this.tableHistory || this.tableHistory.length <= 1;
  }

  get isRedoDisabled() {
    return !this.redoHistory || this.redoHistory.length === 0;
  }
  ...
  @track textFieldOptions = [
    ...
    { label: "Descriptive Text", value: "richText" }
  ];
...
//           dynamicFormRecordId: this.layoutData.layoutName,
//           dynamicFormGroupId: this.layoutData.formGroupId,
//           dynamicFormVersionLabel: this.layoutData.versionLabel,
...
sanitizeFieldForPersist(field) {
 ...
 if (field.isTableBlock && field.tableConfig) {
  ...
  //           /* 🔥 FORCE REACTIVITY FOR ROW LABELS */
...
// this.isVersionEditFlowActive = !!message.dynamicFormGroupId;
  ...
  renderedCallback() {
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        ...
        /* --------------------------------------
         ✅ RENDER Descriptive Text WITHOUT SANITIZING
         -------------------------------------- */
...
this.updateLiveTablePreview(cell);
        ...
        this.currentFieldType =
          "Table Block";
             ...
             else if (cell.field.dataType === "Descriptive Text") {
...
handleUndo() {
  ...
  // 🔥 STEP 5: Remove current snapshot and store it in redoHistory
  const poppedState = this.tableHistory.pop();
  if (!Array.isArray(this.redoHistory)) {
    this.redoHistory = [];
  }
  this.redoHistory.push(poppedState);
...
handleRedo() {
  console.group("🔁 Redo Triggered");

  // Clear popup / draft state if open (matching handleUndo logic)
  if (this.showPopup) {
    this.showPopup = false;
  }

  // Validate redo boundaries
  ...
  // Pop state from redo history
  const nextState = this.redoHistory.pop();

  // Push back onto tableHistory
  if (!Array.isArray(this.tableHistory)) {
    this.tableHistory = [];
  }
  this.tableHistory.push(nextState);

  // Restore state
  this.tableRows = nextState;

  // Sync column count (supports uneven rows)
  if (this.tableRows && this.tableRows.length) {
    this.currentCols = this.tableRows.reduce(
      (max, row) => Math.max(max, row.cells?.length || 0),
      0
  ...
  // FORCE FULL RE-RENDER (CRITICAL FIX)
  this.tableRows = JSON.parse(JSON.stringify(this.tableRows));

  // Delay highlight (DOM sync)
  setTimeout(() => {
    this.highlightSelectedCells();
  }, 0);
...
this.tableColsInput = this.tableColsInput || 2;
  ...
  innerCells: this.createInnerCells(
    ...
    } else if (this.draggingFieldType === "Descriptive Text") {
      this.isRichTextDisplay = true;
    ...
    if (this.draggingFieldType === "Descriptive Text") {
...
initializeQuillEditorForCell(cellId) {
    ...
    const quill = new window.Quill(container, {
        theme: "snow",
        readOnly: true,
        placeholder: "Descriptive Text Preview(Read Only)",
              ...
              : field.selectedTextFieldOption === "onlyAlphabets"
                ...
                : field.selectedTextFieldOption === "textArea"
                  ...
                  : field.selectedTextFieldOption === "richText"
                    ? "Descriptive Text Field Enabled"
                    : "-"
        };
...
isLayoutChanged() {
  ...
  handleUsePredefinedToggle(event) {
    ...
    console.log('dataset option', event.target.dataset.option);
    console.log('radioSubInputs', JSON.stringify(this.radioSubInputs));
...
if (cell.field?.dataType === "Radio Button") {
  ...
  if (
    ...
    cell.field?.dataType === "Descriptive Text"
) {
    ...
    editors.forEach(editor => {
        ...
        try {
            ...
            if (field?.value) {
                ...
                quill.root.innerHTML = `<p style="color:#999">Descriptive Text Preview</p>`;
            }
...
async handleConfirmSaveLayout() {
 ...
 @api hasUnsavedChanges() {
  ...
  console.log("🧪 UNSAVED CHECK:", {
    isFormSaved: this.isFormSaved,
    result: !this.isFormSaved
  });
...
get hideMandatory() {
    return this.currentFieldType !== 'Descriptive Text';
}
...
//   console.log("🔄 [TablePreview] updateLiveTablePreview triggered");

//   if (!this.isTableBlock || !this.currentCell) {
//     console.warn("⚠️ [TablePreview] Aborted - Not a TableBlock or currentCell missing");
//     return;
//   }
...
//   if (!cell?.field?.isTableBlock) {
//     console.warn("⚠️ [TablePreview] Target cell does not contain TableBlock");
//     return;
//   }

//   const prevCfg = cell.field.tableConfig || {};
...
//   const needRebuild =
//     !prevCfg.innerCells ||
//     prevCfg.rows !== this.tableRowsInput ||
//     prevCfg.cols !== this.tableColsInput;

//   let baseInnerCells =
//     needRebuild
//       ? this.createInnerCells(
//           Number(this.tableRowsInput),
//           Number(this.tableColsInput)
//         )
//       : prevCfg.innerCells;
...
//   /* 🔥 AUTO-FILL (IMMUTABLE) */
//  /* 🔥 AUTO-FILL (IMMUTABLE + FULL CONTROL) */
...
//       // Fill empty OR replace auto-generated
//       if (!cell.field || cell.field.isAutoGenerated) {
...
//       // Remove only auto-generated fields
//       if (cell.field?.isAutoGenerated) {
//         return {
//           ...cell,
//           field: null
//         };
...
async handlePublish() {
  ...
  try {
    ...
    // ✅ VALIDATION
    // ===============================
    if (!this.formTitle || !this.audience || !this.selectedFormType) {
