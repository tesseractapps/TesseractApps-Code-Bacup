...
import UPDATE_ONBOARDING_JSON from '@salesforce/apex/OnBoardingController.updateOnboardingJsonR';
...
const toDisplayRole = (role, userType = '') => {
    // 🔒 HARD OVERRIDE
    if (userType === 'NDIS Participant') {
        return 'Participant';
    }
...
const DATE_FIELDS = new Set(['person.dateOfBirth', 'employment.startDate']);
...
const toIsoDate = (v) => {
    ...
    // Last resort: Date parse to UTC ISO date (yyyy-mm-dd)
    const dt = new Date(v);
    if (!isNaN(dt)) {
...
export default class OnBoarding extends LightningElement {
    ...
    get filteredCount() {
        return this._filteredOrgOnboardings.length;
    }

    facilityPageSize = 10;
    facilityPageNumber = 1;

    get facilityTotalPages() {
        const total = this._allFacilityOnboardings?.length || 0;
        return Math.max(1, Math.ceil(total / this.facilityPageSize));
    }
    get facilityDisablePrev() {
        return this.facilityPageNumber <= 1;
    }
    get facilityDisableNext() {
        return this.facilityPageNumber >= this.facilityTotalPages;
    }
    get facilityCount() {
        return this._allFacilityOnboardings?.length || 0;
    }
    ...
    loadUserTypeValues() {
        ...
        // Roles to exclude - these map to Portal Account Partner Executive
        const EXECUTIVE_ROLES = new Set([
            'NDIS Org Admin',
            'ICT Admin', 
            'Facility Admin',
            'SC with NDIS Org Admin'
        ]);
        ...
        getUserTypeValuesBasedOnUser()
            .then(result => {
                ...
                // ✅ Filter out Executive roles
                const filtered = (result || []).filter(item => !EXECUTIVE_ROLES.has(item));
                
                // Store in filteredUserTypeValues
                this.filteredUserTypeValues = filtered.map(item => ({ 
                    label: item, 
                    value: item 
                }));
                ...
                console.log('Removed Executive roles:', (result || []).filter(item => EXECUTIVE_ROLES.has(item)));
            ...
            if (facilityId) {
                ...
                const mapped = (rows || []).map(r => {
                    ...
                    return {
                        ...
                        displayRole: toDisplayRole(r.role, r.userType),
                        needAccount: need,
                ...
                this._allFacilityOnboardings = mapped;
                this.facilityPageNumber = 1;
                this.applyFacilityPagination();
    ...
    async loadPillCounts() {
        ...
        try {
            ...
            const data = await getOnboardingCounts({
                facilityId,
                orgId
            });
    ...
    wiredMultiPicklistValues({
        ...
        if (data) {
            ...
            this.UserTypeValues = data.map(s => ({
                label: s,
                value: s
            }));
        } else if (error) {
    ...
    handleRoleChange = (event) => {
        ...
        console.log('🔁 handleRoleChange fired');
        console.log('🆔 Row Id:', id);
        console.log('🎭 Selected Role:', role);

        // --------------------------------------------------------
        // STEP 1: Reset row when role changes
        // --------------------------------------------------------
        this.rows = this.rows.map(r =>
            String(r.id) === id ?
            {
                ...
                role,
                roleError: !role,
                roleWrapperClass: !role ? 'SN-cell has-error' : '',
                userType: null,
                userTypeError: false,
        ...
        console.log('🧾 Rows after role change reset:', JSON.parse(JSON.stringify(this.rows)));

        // Stop if role is cleared
        if (!role) {
            console.warn('⚠️ Role cleared, skipping user type fetch');
            return;
        }

        console.log('📡 Fetching user types for role:', role);

        // --------------------------------------------------------
        // STEP 2: Fetch user types
        // --------------------------------------------------------
        this.fetchUserTypesForRole(role)
            .then((opts) => {
                console.log('✅ User types received (before filter):', opts);

                // --------------------------------------------------------
                // 🔥 SPECIAL RULE: Participant → auto-assign NDIS Participant
                // --------------------------------------------------------
                if (role === 'Participant') {
                    const participantType = opts.find(
                        o => o.value === 'NDIS Participant'
                    );
                    ...
                    this.rows = this.rows.map(r =>
                        ...
                        {
                            ...r,
                            userType: participantType?.value || 'NDIS Participant',
                            userTypeOptions: participantType ? [participantType] : [],
                            isUserTypeDisabled: true,
                            userTypeError: false,
                            userTypeWrapperClass: 'slds-is-disabled usertype_disable',
                            userTypePlaceholder: 'NDIS Participant'
                        } :
                        r
                    );

                    console.log('✅ Participant role → userType auto-set to NDIS Participant');
                    console.log('🧾 Rows after Participant auto-assign:', JSON.parse(JSON.stringify(this.rows)));
                    return; // ⛔ stop here (no further processing)
                }

                // --------------------------------------------------------
                // STEP 3: Existing JS-only rule for Partner Manager
                // --------------------------------------------------------
                let filteredOpts = opts;
                if (role === 'Portal Account Partner Manager') {
                    filteredOpts = opts.filter(
                        o => o.value !== 'NDIS Participant'
                    );
                }

                console.log('✅ User types after JS filter:', filteredOpts);

                this.rows = this.rows.map(r =>
                    String(r.id) === id ?
                    {
                        ...r,
                        userTypeOptions: filteredOpts,
                        isUserTypeDisabled: !filteredOpts.length,
                        userTypeWrapperClass: !filteredOpts.length ?
                            'slds-is-disabled usertype_disable' :
                            '',
                        userTypePlaceholder: !filteredOpts.length ?
                            'No user types for role' :
                            'Select user type'
                    } :
                    r
                );

                console.log('🧾 Rows after user type update:', JSON.parse(JSON.stringify(this.rows)));
            })
            .catch((error) => {
    ...
    // ✅ Role mapping function - Portal Roles
    ...
    getDisplayRole(userType) {
        const roleMap = {
            'NDIS Org Admin': 'Org Admin',
            'ICT Admin': 'Org Admin',
            'Facility Admin': 'Org Admin',
            'SC with NDIS Org Admin': 'Org Admin',
            'Roster Manager': 'Roster Admin',
            'HR Admin': 'Roster Admin',
            'Payroll Admin': 'Roster Admin',
            'NDIS Participants': 'NDIS Participants',
            'Payroll Accountant for Multiple': 'Roster Admin',
            'Accountant for Organisation': 'Roster Admin',
            'SC with Roster Manager': 'Roster Admin',
            'NDIS Staff': 'Staff',
            'ICT Staff': 'Staff'
        };
        return roleMap[userType] || null;
    ...
    handleUserTypeChange = (event) => {
      ...
      console.log('🔄 User Type changed for row:', id, 'Value:', userType);
      
      // Auto-assign role based on user type
      const autoAssignedRole = this.mapUserTypeToRole(userType);
      const displayRole = this.getDisplayRole(userType);
      ...
      this.rows = this.rows.map(r => 
          String(r.id) === id 
              ? { 
                  ...
                  autoAssignedRole: autoAssignedRole, // Portal role for API
                  displayRole: displayRole, // Display role for UI
                  role: autoAssignedRole // Store for API payload
              } 
              : r
      );
      
      console.log('✅ Auto-assigned Role:', autoAssignedRole, 'Display:', displayRole);
  };
    ...
    fetchUserTypesForRole(role) {
        ...
        return getUserTypeValuesfromOrg({
    ...
    async handleAddEmails() {
        ...
        let existingMap = {};
        try {
            existingMap = await getExistingEmails({
                emails: goodCandidates
            }); // Map<lowerEmail, ObjectName>
        } catch (e) {
            existingMap = {};
        }
    ...
    async handleCsvUpload(event) {
        ...
        try {
            ...
            const {
                ...
                skippedDuplicates,
                errors
            } = await this.parseCsvAndAddRows(text);
    ...
    async handleCreateRecords() {
        console.log('========== handleCreateRecords START ==========');

        if (this.isSaving) {
            console.warn('Already saving. Exiting...');
            return;
        }

        console.log('Current Rows:', JSON.parse(JSON.stringify(this.rows)));
        ...
        const selectedIdxs = this.rows
            .map((r, i) => (r.selected ? i : -1))
            .filter(i => i >= 0);

        console.log('Selected Row Indexes:', selectedIdxs);

        const targetIdxs = selectedIdxs.length
            ? selectedIdxs
            : this.rows.map((_, i) => i);

        console.log('Target Row Indexes:', targetIdxs);

        const targetRows = targetIdxs.map(i => this.rows[i]);

        console.log('Target Rows:', JSON.parse(JSON.stringify(targetRows)));

        // Validate User Type
        const missingTypeIds = targetRows
            .filter(r => !r.userType)
            .map(r => r.id);

        console.log('Missing User Type IDs:', missingTypeIds);

        if (missingTypeIds.length) {
            console.warn('Validation failed - Missing User Type');
            ...
            this.toast(
                'Missing data',
                'User Type is required for all selected rows.',
                'error'
            );
            return;
        }
        ...
        if (!facilityId) {
            console.error('Facility Id missing');
            this.toast(
                'Missing Facility',
                'Open/select a Facility before sending invites.',
                'error'
            );
            return;
        }
        ...
        const payload = targetRows.map(r => {
            ...
            let finalRole = r.autoAssignedRole || 'Portal Account Partner User';
            let finalUserType = r.userType || '';

            if (finalUserType === 'NDIS Participants') {
                finalRole = 'Portal Account Partner Manager';
                finalUserType = 'NDIS Participant';
            }
        ...
        console.log('Final Payload:', plain);
        console.log('Signer Base URL:', this.signerBaseUrl);
        console.log('Facility Id:', this.facilityId);
        console.log('Organisation Id:', this.orgId);
        ...
        try {
            ...
            if (!this.signerBaseUrl) {
                console.error('Signer Base URL is missing.');
                throw new Error('Signing page URL is missing. Set this.signerBaseUrl.');
            }

            console.log('Calling Apex createOnboardingRawAndEmail...');
            ...
            console.log('Apex Response:', JSON.parse(JSON.stringify(results)));
            ...
            console.log('lastSendResults:', this.lastSendResults);
            ...
            while (padded.length < targetRows.length) {
                padded.push(null);
            }

            console.log('Padded Results:', padded);

            const posByRowIndex = new Map();

            targetIdxs.forEach((rowIdx, pos) => {
                posByRowIndex.set(rowIdx, pos);
            });

            console.log('Position Map:', [...posByRowIndex.entries()]);
            ...
            this.rows = this.rows.map((r, i) => {
                ...
                console.log(`Processing Row ${i}`, res);
                ...
                if (!res) {
                    return {
                        ...
                        progress: 0,
                        progressStyle: 'width:0%',
                        error: r.error || 'No server response'
                    };
                }
            ...
            console.log('Rows after processing:', JSON.parse(JSON.stringify(this.rows)));

            const ok = padded.filter(x => x && x.success).length;
            const bad = padded.filter(x => !x || (x && !x.success)).length;

            console.log('Success Count:', ok);
            console.log('Failure Count:', bad);

            if (ok) {
                this.toast(
                    'Success',
                    `${ok} record(s) created & emails queued.`,
                    'success'
                );
            }
            ...
            if (bad) {
                this.toast(
                    'Some failed',
                    `${bad} row(s) failed. Check errors in the table.`,
                    'warning'
                );
            }
            ...
            if (ok > 0 && typeof this.loadOnboardings === 'function') {
                ...
                console.log('Refreshing onboarding list...');
                await this.loadOnboardings();
                console.log('Onboarding list refreshed.');
            }
            ...
            if (ok > 0 && bad === 0) {
                ...
                if (targetIdxs.length === this.rows.length) {
                    ...
                    console.log('All rows processed successfully. Clearing UI.');
                    ...
                    console.log('========== handleCreateRecords END (All Success) ==========');
                    return;
                }
            }
            ...
            if (ok > 0) {
                ...
                console.log('Success Row Indexes:', [...successSet]);

                this.rows = this.rows.filter((_, i) => !successSet.has(i));

                console.log('Remaining Rows:', JSON.parse(JSON.stringify(this.rows)));
            ...
            console.error('Error in handleCreateRecords:', e);
            console.error('Error Message:', e?.message);
            console.error('Stack:', e?.stack);

            this.rows = this.rows.map((r, i) =>
                targetIdxs.includes(i)
                    ? { ...r, progress: 0, progressStyle: 'width:0%' }
                    : r
            );
            ...
            console.log('========== handleCreateRecords END ==========');
        }
    }
    ...
    ob_handleStatusToggle = (evt) => {
        ...
        this._writeStatusIntoJsonBoolean(checked); // new strict writer
    };
    ...
    _writeStatusIntoJsonBoolean(activeBool) {
        if (!this.ob_originalData || typeof this.ob_originalData !== 'object') {
            this.ob_originalData = {};
        }
        const d = this.ob_originalData;
        d.employment = (d.employment && typeof d.employment === 'object') ? d.employment : {};
        d.employment.status = !!activeBool; // boolean, not "Active"/"Inactive"
    }
    ...
    ob_modalOpenDetails = async (event) => {
        try {
            ...
            const dto = await GET_RECEIVED_DATA({
            ...
            const draft = {};
            (this.ob_selectedRecordFields || []).forEach(f => {
                draft[f.apiName] = f.value;
            });
    ...
    ob_confirmApprove = async () => {
        ...
        try {
            ...
            // Clone to avoid mutating UI state while the call is in flight.
            const data = JSON.parse(JSON.stringify(this.ob_originalData ?? {}));

            // ✅ FORCE ACTIVE ALWAYS (toggle removed from UI)
            const isActive = true;

            // Ensure employment exists and write a strict boolean
            if (!data.employment || typeof data.employment !== 'object') data.employment = {};
            data.employment.status = true;

            // (Compatibility) also include a flat flag many Apex mappers like to read.
            data.Active = 'true';

            const updatedPayloadJson = JSON.stringify(data);
            // --------------------------------------
            ...
            if (res && res.success) {
                ...
                this.ob_showConfirmModal = false;
                this.ob_showDetailsModal = false; // optional close after approve
    ...
    obBuildFlatPreview(raw) {
        ...
        const labelMap = {
            ...
            role: 'Role',
            userType: 'User Type',
            facilityIdOrName: 'Facility',
            hourlyRate: 'Hourly Rate',
            taxFileNumber: 'Tax File Number',
            taxFreeThreshold: 'Tax Threshold',
            ...
            country: 'Country',
            participantType: 'Who is receiving the NDIS service?'
        };
    ...
    ob_toggleEdit = async () => {
        if (!this.ob_isEditing) {
            ...
            const draft = {};
            (this.ob_selectedRecordFields || []).forEach(f => {
                // For date fields, seed from original JSON (ISO) to avoid pulling any display-formatted value
                draft[f.apiName] = (DATE_FIELDS.has(f.apiName) ?
                    this.obGetByPath(this.ob_originalData, f.apiName) :
                    f.value
                );
            });
            this.ob_editDraft = draft;
        ...
        try {
            ...
            // 1) apply edits to UI list (unchanged)
            this.ob_selectedRecordFields = (this.ob_selectedRecordFields || []).map(f => {
                if (f._editable && Object.prototype.hasOwnProperty.call(this.ob_editDraft, f.apiName)) {
                    return {
                        ...f,
                        value: this.ob_editDraft[f.apiName]
                    };
                }
                return f;
            });
            ...
            for (const [path, val] of Object.entries(this.ob_editDraft || {})) {
                ...
                if (field && field._editable) {
                    // Normalize date fields to ISO before persisting
                    const out = DATE_FIELDS.has(path) ? toIsoDate(val) : val;
                    this.obSetByPath(updated, path, out);
                }
            }

            // 3) persist via NEW Apex method (JSON only, no deactivateLink)
            const payloadJson = JSON.stringify(updated);
            const saveRes = await UPDATE_ONBOARDING_JSON({
                onboardingId: this.ob_selectedRecordId,
                payloadJson
            });
    ...
    obGetByPath(root, path) {
        if (!root || !path) return undefined;
        return String(path).split('.').reduce((acc, p) => (acc && acc[p] != null ? acc[p] : undefined), root);
    }
    ...
    /* async handleFinalizeOnboarding() {
      ...
      const facilityId = this.facilityId || this.recordId;
      console.log('[OnboardingGrid] using FacilityId =', facilityId);
      console.log('[OnboardingGrid] using FacilityId =', JSON.stringify(this.onboardings));
      ...
      const payload = (this.onboardings || [])
        ...
        .map(r => {
          ...
          // Prefer boolean from parsed.employment.status; otherwise derive from any text on row/parsed
          const parsedEmp = parsed.employment || {};
          const rowText   = (r.employmentStatus != null) ? String(r.employmentStatus) : null;
          const parsedTxt = (typeof parsed.status === 'string') ? parsed.status : null;

          const isActive = (typeof parsedEmp.status === 'boolean')
            ? parsedEmp.status
            : String(rowText ?? parsedTxt ?? '').trim().toLowerCase() === 'active';
          ...
          const gender      = r.gender      || parsed.gender || ''; 
          const roleSimple = r.role || '';     // RowDTO.role (Role__c)
          const parsedParticipantType = parsed.person?.participantType || parsed.participantType || '';
          console.log('Final participantType:', parsedParticipantType);
          ...
          const abnNumber = r.abnNumber ?? parsedEmp.abnNumber ?? '';
          ...
          const needUser = !!r.userAccountNeeded; // review checkbox on the row
          ...
          return {
            ...
            EmailAddress: email,
            Gender: gender,
            FacilityId: facilityId, // if you only have Id, server side handles lookup by Id branch
            Nationality: r.nationality || parsed.nationality || '',
            Languages:   r.languages   || parsed.languages   || '',
            ParticipantType:  parsedParticipantType,


            Role: roleSimple,
            UserType: userType,
            Status: 'Reviewed', // gate for Apex
            ...
            // Send both: human-readable + strict flag
            EmploymentStatus: employmentStatus,          // 'Active' | 'Inactive'
            Active: isActive ? 'true' : 'false'          // strict for Apex
          };
        });
      ...
      try {
        //const result = await createAccountsContactsAndStaff({ records: payload }); // or createAccountsContactsAndStaff if that's your method name
        ...
        if (successes > 0 && failures === 0) {
          ...
          this.toast?.('Partial success', `Created ${successes}, failed ${failures}. See table/console for details.`, 'warning');
        } else if (successes === 0 && failures > 0) {
        ...
        if (successes > 0 && typeof this.loadOnboardings === 'function') {
          ...
          this.selectedReviewedIds = new Set();
          this.recomputeFinalizeState();
        }
    ...
    async handleFinalizeOnboarding() {
        ...
        const payload = (this.onboardings || [])
            ...
            .map(r => {
                ...
                // Debug: Check what parseOnboardingJson returns
                console.log('=== DEBUG: Row processing ===');
                console.log('Row ID:', r.id);
                console.log('Parsed object:', parsed);
                console.log('Parsed keys:', Object.keys(parsed || {}));

                // Check if participantType exists in parsed object
                console.log('Does parsed have participantType?', 'participantType' in (parsed || {}));
                console.log('parsed.participantType value:', parsed?.participantType);

                // Since parseOnboardingJson flattens the structure, all fields are at root level
                // Prefer boolean from parsed.employment.status; otherwise derive from any text on row/parsed
                const parsedEmp = parsed.employment || {};
                const rowText = (r.employmentStatus != null) ? String(r.employmentStatus) : null;
                const parsedTxt = (typeof parsed.status === 'string') ? parsed.status : null;
                ...
                // Since parseOnboardingJson flattens, all person fields are at root level
                const firstName = r.firstName || parsed.firstName || '';
                const lastName = r.lastName || parsed.lastName || '';
                const email = r.email || parsed.email || '';
                const gender = r.gender || parsed.gender || '';
                ...
                const superFundName = r.superFundName ?? parsedEmp.superFundName ?? '';
                const superMemberNumber = r.superMemberNumber ?? parsedEmp.superMemberNumber ?? '';

                // Try to get participantType from parsed object (it might be at root level)
                // Also try to extract directly from JSON string as fallback
                let participantType = '';

                // First try: from parsed object (root level)
                if (parsed && parsed.participantType) {
                    participantType = parsed.participantType;
                }
                // Second try: from row object
                else if (r.participantType) {
                    participantType = r.participantType;
                }
                // Third try: extract directly from JSON string (emergency fallback)
                else {
                    const jsonStr = r.payloadJson || r.receivedData || r.Recieved_Data__c;
                    try {
                        const directParse = JSON.parse(jsonStr);
                        if (directParse.person && directParse.person.participantType) {
                            participantType = directParse.person.participantType;
                        }
                    } catch (e) {
                        console.warn('Could not parse JSON string directly:', e);
                    }
                }

                console.log('Final participantType:', participantType);
                ...
                const addrStreet = r.addressStreet || (parsed.address && parsed.address.street) || '';
                const addrCity = r.addressCity || (parsed.address && parsed.address.city) || '';
                const addrState = r.addressState || (parsed.address && parsed.address.state) || '';
                const addrPost = r.addressPostCode || (parsed.address && parsed.address.postalCode) || '';
                const addrCountry = r.addressCountry || (parsed.address && parsed.address.country) || '';
        ...
        try {
            const result = await createAccountsContactsAndStaff({
                records: payload
            });
            console.log('[OnboardingGrid] APEX result =', result);
    ...
    get filteredRoleOptions() {
        ...
        if (this.userTypeRole === 'NDIS Org Admin' || this.userTypeRole === 'Facility Admin' || this.userTypeRole === 'ICT Admin') {
    ...
    async refreshPills() {
        if (this._countsWire) {
            try {
                await refreshApex(this._countsWire);
            } catch (e) {
                console.error(e);
            }
        }
    ...
    handlePageSizeChange = (event) => {
        this.pageSize = Number(event.target.value || event.detail?.value);
        this.pageNumber = 1;
        this.applyOrgFilterAndPaginate();
    };
    ...
    applyOrgFilterAndPaginate() {
        ...
        try {
            ...
            this._filteredOrgOnboardings = this.sortData(this._filteredOrgOnboardings); // manendra added for sorting the data in table
    ...
    // ===== FACILITY VIEW PAGINATION (onboardings table) =====
    handleFacilityPageSizeChange = (event) => {
        this.facilityPageSize = Number(event.target.value || event.detail?.value);
        this.facilityPageNumber = 1;
        this.applyFacilityPagination();
    };

    facilityGoFirst = () => {
        if (!this.facilityDisablePrev) {
            this.facilityPageNumber = 1;
            this.applyFacilityPagination();
        }
    };
    ...
    facilityGoLast = () => {
        if (!this.facilityDisableNext) {
            this.facilityPageNumber = this.facilityTotalPages;
            this.applyFacilityPagination();
        }
    };
    ...
    applyFacilityPagination() {
        //const all = this._allFacilityOnboardings || [];
        const all = this.sortData(this._allFacilityOnboardings || []); // manendra added for sorting the data in table
        // Clamp page number
        let page = this.facilityPageNumber;
        const pages = this.facilityTotalPages;
        if (page > pages) page = pages;
        if (page < 1) page = 1;
        this.facilityPageNumber = page;

        const start = (page - 1) * this.facilityPageSize;
        const end = start + this.facilityPageSize;

        this.onboardings = all.slice(start, end);

        // optional: debug
        console.log('[Facility] page', page, 'size', this.facilityPageSize,
            'slice', `[${start}, ${end})`, 'rows:', this.onboardings.length);
    }
    ...
    get tabClassSuccess() {
        return `SN-tab ${this.isTabSuccess ? 'is-active' : ''}`;
    }
    get tabClassFailed() {
        return `SN-tab ${this.isTabFailed  ? 'is-active' : ''}`;
    }
...
function parseEmails(text) {
    return splitEmails(text).map(raw => {
        ...
        return {
            email,
            error: isEmailValid(email) ? null : 'Invalid email format'
        };
    });
}
...
function parseOnboardingJson(raw) {
    try {
        ...
        const superMemberNumber =
            String(
                emp.superMemberNumber ??
                emp.SuperMemberNumber ??
                emp.Superannuation_Number ??
                emp.Superannuation_Number__c ??
                ''
            ).trim();
        ...
        const userType =
            (typeof emp.userType === 'string' && emp.userType.trim()) ?
            emp.userType :
            (meta.tokenUserType || '');

        // facility: prefer provided name; if it's just an Id, leave blank
        const facilityName =
            emp.facilityIdOrName && !/^[a-zA-Z0-9]{15,18}$/.test(emp.facilityIdOrName) ?
            emp.facilityIdOrName :
            '';

        // --- normalize status ---
        // support boolean or string; produce both a text and a boolean form
        let statusBool = false;
        if (typeof emp.status === 'boolean') {
            statusBool = emp.status;
        } else {
            const s = String(emp.status ?? '').trim().toLowerCase();
            statusBool = (s === 'active' || s === 'true'); // tolerate legacy text
        }
        const statusText = statusBool ? 'Active' : 'Inactive';

        // ================================
        // ✅ NEW: Nationality + Languages
        // ================================
        // accept possible key variants (defensive)
        const nationality =
            String(
                person.nationality ??
                person.Nationality ??
        ...
        // Normalize languages into multi-picklist style: "English;Hindi"
        // Supports: array, "English;Hindi", "English, Hindi"
        const normalizeLanguages = (v) => {
            ...
            // if already semicolon form, keep it (but clean whitespace)
            if (s.includes(';')) {
        ...
        return {
            ...
            // keep legacy flattened text for callers that use it
            status: statusText,

            // ✅ NEW fields (top-level) to pass into finalize payload/Apex
            nationality,
            languages,
            ...
            employment: {
                ...
                facilityIdOrName: emp.facilityIdOrName || '',
                superFundName: superFundName,
                superMemberNumber: superMemberNumber
            },
...
function normalizeLanguages(v) {
    ...
    // If already "English;Hindi" keep it, if "English, Hindi" convert it
