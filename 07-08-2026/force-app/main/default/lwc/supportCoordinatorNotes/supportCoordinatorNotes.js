        ...
        try {
            ...
            if (result.status === 'SUCCESS') {
               // this.participants = (result.records || []).map(item => {
                const mappedParticipants = (result.records || []).map(item => {
...
this.allMappedParticipants = [...mappedParticipants];
                console.log(
                    'Mapped Participants:',
...
handleSearchParticipant(event) {
    ...
    let filtered = [...this.allMappedParticipants];
    ...
    if (this.searchParticipant) {
        filtered = filtered.filter(item =>
            (item.name &&
                item.name.toLowerCase().includes(this.searchParticipant)) ||
            (item.email &&
                item.email.toLowerCase().includes(this.searchParticipant))
        );
    }
