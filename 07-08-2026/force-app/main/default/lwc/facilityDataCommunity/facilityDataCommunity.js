...
get showSkip() {
    if ((!this.NdisFlag || this.hideSil) && this.currentStep === 'step3') {
        return false;
    }
...
handleCreateNext() {
    ...
    // Support Coordinator → Save directly
    if (
        this.supportCoordintionflag &&
   ...
   async connectedCallback() {
        try {
            ...
            if (isStart ) {
                ...
                this.disablePayrollSettingValue = isStart;  
                this.startplan=true;
                console.log('this.disablePayrollSettingValue',this.disablePayrollSettingValue);            
            }      
       ...
       this.fetchFacilities()
        .then(() => {
            try {
                ...
                // URL wins on refresh / deep-link: if the hash names a facility, restore
                // that record+tab (and edit) and skip the localStorage fallback below.
                if (this._restoreFromUrlHash()) {
                    console.log('Restored facility view from URL hash');
                    return;
                }
    ...
    renderedCallback() {
        if (!this.hasCalculatedPageSize) {
            this.hasCalculatedPageSize = true;
            this.setPageSizeByZoomAndScreen();
        }

        // Reflect edit / wizard-step / popup state in the URL (no per-handler wiring).
        this._syncRecordRoute();
     ...
     userDetails({error, data}) {
         if (data) {
             ...
             this.typeofuser =data.fields.Type_of_User__c.value;
              ...
              this.supportCoordintionflag = this.userType === 'Support Coordinator';

              console.log('supportCoordintionflag==>' + this.supportCoordintionflag);

              this.lineItemNumberFlag = !(this.userType === 'NDIS Org Admin' || this.userType === 'Roster Manager');
...
fetchServices() {
    ...
    setPageSizeByZoomAndScreen() {
        ...
        // Available vertical height inside the viewport for the cards grid
...
async fetchFacilities() {
    ...
    try {
        ...
        if (this.userType === 'NDIS Org Admin' || this.userType === 'ICT Admin' || this.userType === 'Support Coordinator') {
    ...
    get pageSizeOptions() {
        ...
        defaultSizes.forEach(size => {
            options.push({
                label: String(size),
                value: String(size),
                selected: this.isPageSizeManuallySet && this.pageSize === size
            });
        });
...
//end
    ...
    paginationHelper() {
        ...
        this.records = this.sortData(this.records);// manendra added for sorting the data in table
    ...
    async handleCreateNewFacility() {
        ...
        console.log('Step Number', this.currentStep);
        ...
        try {
            ...
            if (limitReached) {
                ...
                //this.cardFlag = true;
                // this.listFlag = false;
                this.abnfield = false;// manendra bug fix added
    ...
    handleviewFacility(event) {
        ...
        // Put #/admin/facility/<uid>/<tab> in the URL when a record opens from the list.
        this.notifySubRoute(`${this._recordUid}/${this._currentTabSlug()}`);
    }
    ...
    // Reflect edit mode in the URL: #/admin/facility/<uid>/<tab>/edit/step1 — push a
    // history entry so browser Back closes the edit. renderedCallback keeps step/popup
    // in sync afterwards. currentStep is 'step1' at this point (set above).
    this.recordId = facId;
    const editRoute = this._composeSubRoute();
    this.notifySubRoute(editRoute);
    this._lastSubRoute = editRoute; 
    try {
        ...
        let typeOfIndustry=facility.type_of_service;
       
        if (typeOfIndustry === 'NDIS' || typeOfIndustry === 'Aged Care') {
                this.hideSil = false; // Show SIL
            } else {
                this.hideSil = true;  // Hide SIL
         }
          console.log('this.hideSil =>'+this.hideSil);
    ...
    handleClose(event){
        ...
        // Edit closed but record stays open → renderedCallback drops /edit from the URL.
    }
    ...
    @track toggleElement; 
    @track facilityname;
    @track disablePayrollSettingValue =true;
    ...
    return this.recordsToDisplay.map((fac) => {
        ...
        let silValue=fac.EnableSIL__c === true ?' - SIL enabled' :'';
        return {
            ...fac,
...
/*   handlestatuschange() {
    ...
    .then(response => {
        ...
        this.handleStatusFlag = false;
        this.fetchFacilities();
        //this event from child is for refreshing the facilities in the dashboard
        this.dispatchEvent(
            new CustomEvent('refreshfacility', {
                bubbles: true,
                composed: true
            })
        );
    ...
    handleSubmit(event){
        ...
        const stepToCheck = this.facEditFlag
            ? this.createCurrentStep   // create-mode wizard
            : this.currentStep;        // edit-mode wizard

        const missingFields = this.validateStepFields(stepToCheck);

        if (missingFields.length > 0) {
            this.triggerStepValidation(stepToCheck);
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message: 'Please fill required fields: ' + missingFields.join(', '),
            ...
            return; // stop save, do not submit form
        }
        // ⬆️ END NEW CHECK
    ...
    handleSuccess(event) {
       ...
       // this.cardFlag = true;
        ...
        // Clear local storage for active facility
        localStorage.removeItem('facilityRecordId');
        ...
        // Save closes the edit AND returns to the list → clear edit state and drop the
        // record from the URL (#/admin/facility). facilityeditflag/currentStep are reset
        // so a stale flag can't re-emit /edit on the next render.
        this.facilityeditflag = false;
        this.currentStep = 'step1';
        this.notifyModuleRoot();
        ...
        setTimeout(() => {
            this.showSpinner = false;
            this.fetchFacilities();
            publish(this.messageContext, FACILITY_DATA_REFRESH_EVENT, {facilityRefreshTrigger: 'REFRESH_FACILITY_DATA'});
            console.log('📢 Facility refresh event published AFTER SAVE');
        }, 1500);
    ...
    handleBack(event){
        ...
        // Record closed → back to the list: drop the record from the URL (#/admin/facility).
        this.notifyModuleRoot();
...
get viewToggleTarget() {
 ...
 handleOpenBMSSoftwareID(event){
  ...
  ConfirmClose (){
    ...
    // HASH ROUTER: while the shell is restoring us (openByUID), it already owns the
    // URL — suppress our own subrouteupdate emits so we don't fight it.
    _suppressEmit = false;
   ...
   handledetails(event){
        ...
        this.notifySubRoute(`${this._recordUid}/details`);
    }
    handleonboard(event){
        this.selectedServiceType = '';
        this.facilitydetails=false;
        ...
        this.notifySubRoute(`${this._recordUid}/onboarding`);
    }
    handledocument(event){
        ...
        this.notifySubRoute(`${this._recordUid}/document`);
    }
    handleidentity(event){
        ...
        this.notifySubRoute(`${this._recordUid}/identity`);
    }
    handletrainings(event){
        ...
        this.notifySubRoute(`${this._recordUid}/trainings`);
    }
    handleshiftcompliance(event){
        ...
        this.notifySubRoute(`${this._recordUid}/shiftcompliance`);
    }
    handlecompliancereport(event){
        ...
        this.notifySubRoute(`${this._recordUid}/compliancereport`);
    ...
    async handleBulkEntity(event){
        ...
        this.notifySubRoute(`${this._recordUid}/bulkentity`);
        console.log('bulkentity', this.facilityBulkEntity);
    }
    ...
    handleCatalogueClick(event){
        ...
        this.facilityBulkEntity = false;
        this.facilityCatalogue =true;
        this.notifySubRoute(`${this._recordUid}/catalogue`);
    }

    // =====================================================================
    // HASH ROUTER bridge (additive) — reuses EXISTING tab handlers, no rename.
    // URL model handled by the shell: #/admin/facility/<uid>/<tab>
    // <uid> comes from the Facility_UID__c formula field (URL boundary).
    // =====================================================================

    // Verbatim `fac-####-####-####` uid of the facility currently in view —
    // sourced from the Facility_UID__c formula field stored on the record's JSON.
    get _recordUid() {
        return (this.facilityJSONData && this.recordId && this.facilityJSONData[this.recordId]
            && this.facilityJSONData[this.recordId].uid) || '';
    }

    // tab slug -> EXISTING handler name (labels mirror the record tabs)
    get _tabHandlerBySlug() {
        return {
            'details': 'handledetails',
            'onboarding': 'handleonboard',
            'document': 'handledocument',
            'identity': 'handleidentity',
            'trainings': 'handletrainings',
            'shiftcompliance': 'handleshiftcompliance',
            'compliancereport': 'handlecompliancereport',
            'catalogue': 'handleCatalogueClick',
            'bulkentity': 'handleBulkEntity'
        };
    }

    // Which tab is currently showing → slug (used to seed the URL when a record opens).
    _currentTabSlug() {
        if (this.facilityonboard) return 'onboarding';
        if (this.facilitydocument) return 'document';
        if (this.facilityidentity) return 'identity';
        if (this.facilitytrainings) return 'trainings';
        if (this.facilityshiftcompliance) return 'shiftcompliance';
        if (this.facilitycompliancereport) return 'compliancereport';
        if (this.facilityCatalogue) return 'catalogue';
        if (this.facilityBulkEntity) return 'bulkentity';
        return 'details';
    }

    // Tell the shell about every view/tab change (bubbling + composed).
    // `replace` = update the URL in place (no new history entry).
    notifySubRoute(subView, replace) {
        console.log("?? [Facility LWC] notifySubRoute subView:", subView, "replace:", replace, "suppress:", this._suppressEmit, "uid:", this._recordUid, "facEditFlag:", this.facEditFlag);
        // Don't emit while the shell is restoring us — it already owns the URL.
        if (this._suppressEmit) return;
        // Skip until the facility uid is known, so we never emit a uid-less path.
        // if (!this._recordUid) return;
        if (!this._recordUid && !this.facEditFlag) return;        
    ...
    // Drop the record segment entirely → shell lands on the module root
    // (#/admin/facility). Used when a record view closes back to the list
    // (Back button, and after Save which also returns to the list).
    notifyModuleRoot() {
        this._lastSubRoute = '';
        // if (this._suppressEmit) return;
        // if (!this._recordUid && !this.facEditFlag && subView !== '') return;
        if (this._suppressEmit) return;
        this.dispatchEvent(new CustomEvent('subrouteupdate', {
    ...
    // Last record sub-route we pushed to the shell — lets renderedCallback skip
    // redundant emits so we don't spam the URL on every re-render.
    _lastSubRoute = '';

    // A modal/popup is open → its slug becomes a trailing URL segment. Derived from
    // the EXISTING modal flags (no per-handler wiring). Add new popups here as needed.
    _activePopupSlug() {
        if (this.isCreateCatalogueModalOpen) return 'create-catalogue';
        if (this.showUpgradeModal) return 'upgrade';
        return '';
    }

    // Compose the full record sub-route from current state:
    //   <uid>/<tab>[/edit/<step>][/<popup>]   (step is always shown, incl. step1)
    _composeSubRoute() {
        if (this.facEditFlag && !this.recordId) {
            let step = 'step1';
            if (this.createCurrentStep === 'createstep2') step = 'step2';
            else if (this.createCurrentStep === 'createstep3') step = 'step3';
            return `create/${step}`;
        }        
        let p = `${this._recordUid}/${this._currentTabSlug()}`;
        if (this.facilityeditflag || this.facEditFlag) {
            p += `/edit/${this.currentStep || 'step1'}`;
        }
    ...
    get facEditFlag() { return this._facEditFlag; }
    set facEditFlag(val) {
        this._facEditFlag = val;
        this._syncRecordRoute();
    }
    ...
    get facilityeditflag() { return this._facilityeditflag; }
    set facilityeditflag(val) {
        this._facilityeditflag = val;
        this._syncRecordRoute();
    }
    ...
    // Keep the URL's record sub-route (tab / edit / step / popup) in sync with the
    // on-screen state. Called from renderedCallback so edit-wizard steps and modals
    // are reflected without wiring every open/close handler. Only while a record is
    // on screen; uses replace so step/popup churn stays out of browser history.
    _syncRecordRoute() {
        console.log("?? [Facility LWC] _syncRecordRoute: suppress:", this._suppressEmit, "facviewFlag:", this.facviewFlag, "facEditFlag:", this.facEditFlag, "recordUid:", this._recordUid);
        if (this._suppressEmit) return;
        // facviewFlag is the authoritative "a record is on screen" signal (it stays true
        // while the edit overlay is open, and is cleared on Back/Save which return to the
        // list). Guarding on it — not the edit flags — prevents a stale facilityeditflag
        // from re-emitting /edit after Save has already reset the route to the list.
        // if (!this.facviewFlag) return;
        // if (!this._recordUid) return;
        if (!this.facviewFlag && !this.facEditFlag) return;
        if (!this._recordUid && !this.facEditFlag) return;        
        const route = this._composeSubRoute();
        console.log("?? [Facility LWC] composed route:", route, "lastSubRoute:", this._lastSubRoute);
        if (route === this._lastSubRoute) return;
        this._lastSubRoute = route;
        this.notifySubRoute(route, true);
    }

    // Best-effort re-open of a popup named in the URL on refresh / deep-link.
    _openPopupBySlug(slug) {
        if (slug === 'create-catalogue') this.isCreateCatalogueModalOpen = true;
        else if (slug === 'upgrade') this.showUpgradeModal = true;
    }
    // Resolve a Facility_UID__c value back to its record Id from the loaded list.
    _facIdForUid(uid) {
        if (!uid || !this.facilityJSONData) return '';
        const ids = Object.keys(this.facilityJSONData);
        for (let i = 0; i < ids.length; i++) {
            if (this.facilityJSONData[ids[i]] && this.facilityJSONData[ids[i]].uid === uid) {
                return ids[i];
            }
        }
        return '';
    ...
    // Select a tab by slug via its EXISTING handler (no duplicated logic).
    _selectTab(slug) {
        const fn = this._tabHandlerBySlug[slug] || 'handledetails';
        if (typeof this[fn] === 'function') this[fn]();
    }

    // Shell -> child: open the facility named by <uid> at <tab>. Emits are
    // suppressed here — the shell already owns the URL during restore.
    @api async openByUID(uid, tab, isEdit) {
        this._suppressEmit = true;
        try {
            // The list drives the uid->Id map; make sure it's loaded before resolving.
            if (!this._facIdForUid(uid)) {
                await this.fetchFacilities();
            }
            const facId = this._facIdForUid(uid);
            if (facId) {
                this.viewFacilityById(facId);
                this._selectTab(tab || 'details');
                if (isEdit) {
                    this.handleEdit({ currentTarget: { dataset: { id: facId } } });
                }
            }
        } catch (e) {
    ...
    // Facility create is driven from its own list UI — safe no-op for router symmetry.
    // @api openCreate() {}
    @api async openCreate(step) {
        this._suppressEmit = true;
        try {
            await this.handleCreateNewFacility();
            if (step) {
                if (step === 'step1') this.createCurrentStep = 'createstep1';
                else if (step === 'step2') this.createCurrentStep = 'createstep2';
                else if (step === 'step3') this.createCurrentStep = 'createstep3';
            }
        } finally {
            this._suppressEmit = false;
        }
    ...
    // URL is the source of truth on refresh / deep-link. If the hash names a facility
    // (#/admin/facility/<uid>/<tab>[/edit]), open that record+tab here in connectedCallback
    // so a refresh restores the same view instead of dropping to the list. Emits are
    // suppressed (the shell owns the URL). Returns true if it handled the hash.
    _restoreFromUrlHash() {
        ...
        if (createIdx >= 0) {
            const step = parts[createIdx + 1] || 'step1';
            this.openCreate(step);
            return true;
        }            
        const uidIdx = parts.findIndex((p) => /^fac-/i.test(p));
        if (uidIdx < 0) return false;
        const uid = parts[uidIdx];
        const tab = parts[uidIdx + 1] || 'details';
        // Everything after the tab is a mode: edit, a wizard step, and/or a popup slug.
        const modes = parts.slice(uidIdx + 2);
        const isEdit = modes.includes('edit');
        const step = modes.find((m) => /^step[1-4]$/.test(m));
        const popup = modes.find((m) => m !== 'edit' && !/^step[1-4]$/.test(m));
        const facId = this._facIdForUid(uid);
        ...
        try {
            ...
            if (popup) this._openPopupBySlug(popup);
        } finally {
            this._suppressEmit = false;
        }
        // Seed _lastSubRoute so renderedCallback doesn't immediately re-emit the same route.
        this._lastSubRoute = this._composeSubRoute();
        return true;
    }
    ...
    validateCreateCatalogueForm() {
        ...
        if (!this.formData.supportItemNumber || !this.formData.supportItemNumber.trim()) {
            ...
            this.showToast(
                'Validation Error',
                'Support Item Number is required',
                'error'
            );
            return false;
        }
        ...
        const list = Array.isArray(this.allServiceCatalogue?.catalogues)
            ...
            if (duplicateRecord) {
                this.showToast(
                    'Validation Error',
                    `Duplicate Support Item Number already exists for "${duplicateRecord.Name}"`,
                    'error'
                );
                return false;
            }
...
//     // 🔹 ADDRESS VALIDATION
// // ADDRESS VALIDATION — check by data-id OR fallback to always validate on step1
//     const addressCmp = currentStepEl.querySelector('[data-id="mandatory-address"]')
//                     || currentStepEl.querySelector('lightning-input-address');
...
//     if (!currentStepEl) return missing;

//     // Standard required fields (skip disabled + ABN)
//     currentStepEl.querySelectorAll(
//         'lightning-input, lightning-input-field, lightning-combobox, lightning-textarea'
//     ).forEach(field => {
//         if (field.required && !field.disabled && field.dataset.id !== 'abnInput') {
//             if (!field.reportValidity()) {
//                 const label = field.dataset.label || field.label || field.fieldName || 'Field';
//                 missing.push(label.replace(/c$/g, '').replace(/__/g, ' '));
//             }
//         }
//     });

// // AFTER (fixed — always validates address on step1 regardless of attribute)
// const addressCmp = currentStepEl.querySelector('[data-id="mandatory-address"]')
//                 || currentStepEl.querySelector('lightning-input-address')
//                 || currentStepEl.querySelector('[data-id*="address"]');

// // Also unconditionally validate if we're on step1 and any address field is tracked
// // ✅ AFTER — consistent use of `missing`
// if (addressCmp || stepName === 'step1' || stepName === 'createstep1') {
//     if (!this.street) missing.push('Street');  // ← correct    if (!this.street)      missingFields.push('Street');
//     if (!this.city)        missing.push('Suburb');
//     if (!this.province)    missing.push('State');
//     if (!this.postalcode)  missing.push('Post Code');
//     if (!this.country)     missing.push('Country');
// }

//     // Services
//     if (currentStepEl.querySelector('.dropdown-container1')) {
//         if (!this.allServices || !this.allServices.some(svc => svc.checked)) {
//             missing.push('Services');
//         }
//     }

//     // Roles (NDIS only)
//     if (this.NdisFlag && currentStepEl.querySelector('.dropdown-container')) {
//         if (!this.options || !this.options.some(opt => opt.checked && opt.isActive)) {
//             missing.push('Roles');
//         }
//     }
...
//     const abnInput = currentStepEl.querySelector('[data-id="abnInput"]');
//     if (abnInput) {
//         const abnValue = (this.ABN || '').replace(/\D/g, '');
//         if (!abnValue) { abnInput.reportValidity(); missing.push('ABN'); }
//         else if (abnValue.length !== 11) missing.push('ABN (must be 11 digits)');
//     }
...
validateStepFields(stepName) {
    ...
    // otherwise falls back to tracked JS properties (this.street, this.city, etc.)
    // Runs unconditionally on step1 / createstep1 so no address field is ever silently skipped.
    if (stepName === 'step1' || stepName === 'createstep1') {
...
triggerStepValidation(stepName) {
    ...
    stepContainer.querySelectorAll('lightning-input-field').forEach(field => field.reportValidity());
    stepContainer.querySelectorAll('lightning-input').forEach(input => input.reportValidity());
    stepContainer.querySelectorAll('lightning-combobox').forEach(combo => combo.reportValidity());
}
...
// Add this method to facilityDataCommunity.js
closeAllDropdowns() {
    this.isOpen = false;
    this.isServicesOpen = false;
    this.isDocumentsOpen = false;
