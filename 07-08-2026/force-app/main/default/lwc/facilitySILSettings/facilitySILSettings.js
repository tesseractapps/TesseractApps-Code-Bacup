...
import Type_of_Service__c from '@salesforce/schema/Facility__c.Type_of_Service__c';
...
@track isInvoiceEdited = false;
    ...
    @wire(getRecord, {
        recordId: '$facilityId',
        fields: [ENABLE_SIL_FIELD,Type_of_Service__c]
    })
    wiredFacility({ data, error }) {
        if (data) {
            this.enableSIL = data.fields.EnableSIL__c.value  ;
            /*  if (!this.enableSIL) {
                const typeOfService = data.fields.Type_of_Service__c.value;

                if (typeOfService === 'NDIS' || typeOfService === 'Aged Care') {
                    this.enableSIL = true;
                }
            } */
...
handleEntityChange(event) {
    ...
    loadServiceTypes() {
        ...
        //fetchServiceTypes({ facilityId: this.facilityId })
        fetchServiceTypes()
            .then(result => {
        ...
        fetchParticipantNDISCatalogue({
            ...
            .then(result => {
                this.serviceTableData = (result || []).map(item => ({
                    ...
                    isSelected:        false,
                    isOrgRecord:       item.Is_Government__c
                }));
            ...
            .then(rows => {
                ...
                this.serviceTableData = (rows || []).map(r => {
                    const mappedRow = {
                        ...
                        isOrgRecord:       r.Is_Government__c
                    };
