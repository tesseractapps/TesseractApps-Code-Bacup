...
console.log('Invoice Number:', invoice.Invoice_Number__c);
console.log('Name:', invoice.Name);
  ...
  paginationHelper() {
    ...
    this.selectAllChecked = false; 
    //manendra added fr table sort
  const state = this.tableSortState.services;
  const displayRecords = this.sortData(
      this.records,
      state.sortField,
      state.sortDirection
  );
    ...
    for (
      ...
      this.accList.push(displayRecords[i]);// manendra added for table sort
    }
  ...
  paginationHelper1() {
    this.invoiceTable = []; 
    //manendra added for table sort
    const state = this.tableSortState.invoices;
    console.log('records1:', this.records1);
    console.log('First record:', this.records1[0]);
    console.log('Invoice Numbers:', this.records1.map(r => r.invoiceNumber));
    console.table(this.records1);
    const displayRecords = this.sortData(
    this.records1,
    state.sortField,
    state.sortDirection
  );
  console.log('Sort Field:', state.sortField);
  console.log('Sort Direction:', state.sortDirection);
  console.log(
    'Sorted Values:',
    displayRecords.map(r => r[state.sortField])
  )
    if (this.totalRecords > 0) {
    ...
    for (
     ...
     this.invoiceTable.push(displayRecords[i]);// manendra added for table sort
    }
    //console.log(' this.accList in paginationHelper:', JSON.stringify( this.accList));
    //refreshApex(this.wireServiceList);
  }
...
/*           // Handle Date fields FIRST
         ...
         // Handle Date fields FIRST
          if (sortField === 'Date_of_Service__c') {
              const dateA = valueA ? new Date(valueA).getTime() : 0;
              const dateB = valueB ? new Date(valueB).getTime() : 0;
