...
const isImageUrl = (u) =>
  typeof u === "string" && (/\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i.test(u) || u.includes("rendition=ORIGINAL_JPG"));
...
export default class FormRender extends LightningElement {
  ...
  wiredParticipants;
  @track
  participants = []; // Stores fetched records
  @track
  recordsToDisplay = []; // Stores paginated records
  @track
  totalRecords = 0;
  @track
  pageNumber = 1;
  @track
  pageSize = 5; // Default number of records per page
  @track
  totalPages = 0;
  @track
  isDesktop = true; // Simulated for desktop behavior
  @track
  visible = true; // For visibility control
  @track
  noRecordsFlag = false; // Flag to handle no records message
  @track
  bDisableFirst = true; // Disable First & Previous initially
  @track
  bDisableLast = true; // Disable Next & Last initially
  @track
  ...
  @track
  grantedForms = new Set();
  @track
  showPublishedForms = false;
  @track
  paginatedFormResponses = []; // Stores paginated submitted forms
  @track
  totalFormRecords = 0;
  @track
  formPageNumber = 1;
  @track
  formPageSize = 10; // Default number of records per page
  @track
  totalFormPages = 0;
  @track
  disableFirstFormPage = true;
  @track
  disableLastFormPage = true;
  @track
  searchQuery = "";
  @track
  filteredForms = [];
  @track
  showFormsAccess = false;
  @track
  facilityId;
  ...
  pagedRows = [];
  @track
  isIncidentRegisterSelected = false;
  @track
  isCustomisableFormSelected = true;
  @track
  ...
  selectedParticipantId = "";
  @track
  staffParticipantForms = [];
  roleOptions = [
    {
      label: "Org Admin",
      value: "Portal account partner Executive"
    },
    {
      label: "Roster Admin",
      value: "Portal account partner Manager"
    },
    {
      label: "Staff",
      value: "Portal account partner User"
    }
  ];
  ...
  @track
  selectedParticipantStaffNames = "";
  @track
  grantSearchQuery = "";
  @track
  filteredAvailableForms = [];
  @track
  selectedParticipantName;
  @track
  selectedSubmissionDate;
  @track
  isExpandview = false; // you already have this
  ...
  @track
  filteredFormResponses = []; // Filtered results
  @track
  searchFormsAccessQuery = "";
  @track
  filteredParticipants = []; // Used for displaying filtered + paginated records

  awsPayloadCache = new Map();
  @track
  uniqueNames = []; // stored unique names
  @track
  nameTabs = []; // prebuilt tabs for the wrapper
  @track
  ...
  @track
  activeSignatureCellId = null;
  @track
  signatureMode = "draw"; // draw | upload | type
  @track
  signatureTypedText = "";
  @track
  signaturePad; // later
  ...
  connectedCallback() {
    ...
    orgDetails()
      .then((result) => {
        if (result && result.Id) {
          ...
          this.facilityPreferredName = result.Facility_Preferred_Name_Formula__c;
          this.participantPreferredName = result.Participant_Preferred_Name_Formula__c;
          ...
          getOrgLogo({
            ...
            .then((src) => {
              ...
              this.orgLogoUrl = decodedSrc.startsWith("/") ? window.location.origin + decodedSrc : decodedSrc;
    ...
    if (!this.quillLoaded) {
      ...
      Promise.all([loadStyle(this, QUILL + "/quill.snow.css"), loadScript(this, QUILL + "/quill.min.js")])
        .then(() => {
    ...
    getParticipants({
      ...
      .then((data) => {
        ...
        this.participants = data.map((participant, index) => {
          ...
          const grantedForms = participant.Accessible_Forms__c ? participant.Accessible_Forms__c.split(";") : [];
  ...
  filterParticipantOptions() {
    ...
    if (this.userType === "NDIS Staff") {
      ...
      this.participantOptions = this.participants
        .filter((p) => p.visibleStaffMembers && p.visibleStaffMembers.some((staff) => staff.name === fullName))
        .map((p) => ({
          label: p.Name,
          value: p.Id
        }));
  ...
  loadFormData(formJson, isViewMode) {
    ...
    processedRows.forEach((row) => {
      const isPageBreak = row.cells.some(
        (cell) => cell.field?.dataType === "Blank" && cell.field?.label?.toLowerCase().trim() === "page break"
      );
      if (isPageBreak) {
  ...
  renderedCallback() {
        ...
        .then((formRecords) => {
          if (!formRecords || formRecords.length === 0) {
            ...
            this.forms = formRecords.map((form) => {
              ...
              const formattedCreated = this.viewFormatCreatedDate(form.CreatedDate);
              return {
                ...form,
                miniRows,
    ...
    if (!this.jsPDFInitialized) {
      Promise.all([loadScript(this, jsPDF), loadScript(this, autoTable)])
        .then(() => {
          ...
          // ---- jsPDF version ----
          const jspdfVersion = jsPDFConstructor?.version || window.jspdf?.jsPDF?.version || "UNKNOWN";
          ...
          console.log("🔗 BEFORE attach → jsPDF.API.autoTable:", jsPDFConstructor?.API?.autoTable);
          ...
          console.log("🔗 AFTER attach → jsPDF.API.autoTable:", jsPDFConstructor?.API?.autoTable);
    ...
    try {
      ...
      const textareas = this.template.querySelectorAll("textarea.input-textarea");
      textareas.forEach((el) => {
        ...
        if (el.value !== modelVal) {
          ...
          console.warn(`🔧 Sync textarea id="${id}" dom="${el.value}" → model="${modelVal}"`);
  ...
  scoreCompletionFromPayload(root) {
    ...
    while (stack.length) {
      ...
      if (typeof node === "object") {
        ...
        const isHeaderish = node?.dataType === "Header" || node?.type === "header" || node?.isHeader === true;
        ...
        const looksLikeField =
          ...
          !isHeaderish &&
          !isPageBreakish &&
          !isInsideRadioInternals &&
          // ← exclude radio suboptions wherever they live
        ...
        const isRadioField =
          node?.dataType === "Radio Button" || node?.isRadio === true || node?.isRadioButton === true;
        for (const [key, v] of Object.entries(node)) {
          ...
          stack.push({
            node: v,
            inRadioInternals: nextInRadioInternals
          });
        }
      }
    }
    ...
    if (data) {
      this.participants = data.map((participant) => {
        ...
        return {
          ...participant,
          visibleStaffMembers,
          hasVisibleStaff: visibleStaffMembers.length > 0,
          // ✅ Add this
          extraStaffCount,
          extraStaffNames,
          hasExtraStaff: extraStaffCount > 0,
          grantedForms: participant.Accessible_Forms__c ? participant.Accessible_Forms__c.split(";") : []
        };
      });
    ...
    const renderRows = innerCells.map((innerRow, rIndex) => {
      return {
        ...
        renderCells: columns.map((col, cIndex) => {
          ...
          const innerCell = (innerRow.cells || []).find((c) => Number(c.col) === Number(innerIndex));
          if (!innerCell) {
          ...
          const field = {
            ...(innerCell?.field ? JSON.parse(JSON.stringify(innerCell.field)) : {}),
            value: sourceValue,
        ...
        if (cell?.isTableBlock && cell?.field?.tableConfig) {
          ...
          (config.renderRows || []).forEach((r) => {
            (r.renderCells || []).forEach((c) => {
              if (c?.field?.dataType === "Dropdown Field" && c?.field?.selectedDropdownOption === "multiSelect") {
          ...
          if (cell?.field?.dataType === "Table Block") {
            ...
            const newInnerCells = innerCells.map((r) => ({
              ...
              cells: (r.cells || []).map((c) => {
                ...
                updatedValue = updated?.field?.value ?? updated?.value ?? "";
                ...
                const isPredefinedDropdown =
                  baseField.isDropdown && baseField.selectedDropdownOption === "predefinedList";
                return {
                  ...c,
  ...
  initializeQuillEditors() {
    ...
    editors.forEach((container) => {
      ...
      if (toolbar) {
        ...
        // hijack handlers to focus first (avoids blur)
        toolbar.addHandler("bold", () => applyFormatSafely("bold", !quill.getFormat().bold));
        toolbar.addHandler("italic", () => applyFormatSafely("italic", !quill.getFormat().italic));
        toolbar.addHandler("underline", () => applyFormatSafely("underline", !quill.getFormat().underline));
        toolbar.addHandler("strike", () => applyFormatSafely("strike", !quill.getFormat().strike));

        // color background
        toolbar.addHandler("color", (value) => applyFormatSafely("color", value));
        toolbar.addHandler("background", (value) => applyFormatSafely("background", value));
      }
  ...
  renderRichText() {
    ...
    const renderCell = (cell) => {
      ...
      if (cell.isRichText && cell.value) {
        ...
        container.innerHTML = `<div class="ql-editor">${cell.value}</div>`;
  ...
  async handleDeleteUploadedFile(evt) {
    ...
    const removeFromField = (field) => {
      ...
      return {
        ...
        contentType:
          nextUploaded.length === 1 ? nextUploaded[0]?.type || null : nextUploaded.length ? "multiple" : null,
        meta: {
          ...(field.meta || {}),
    ...
    this.tableRows = (this.tableRows || []).map((row) => ({
      ...
      cells: row.cells.map((cell) => {
        ...
        if (cell.id === cellId) {
          return {
            ...cell,
            field: removeFromField(cell.field)
          };
        }
  ...
  looksLikeUpload(item) {
    ...
    if (item?.value?.isUploadFile && item?.value?.uploadUrl) {
      console.log("✅ Detected upload object with uploadUrl:", item.value.uploadUrl);
      return true;
    }
  ...
  triggerSignatureFileInput() {
    const input = this.template.querySelector('input[data-role="signature-upload"]');
    if (input) {
      input.click();
    }
  }
  ...
  async uploadFormJsonToAws(finalRows, folderType, formTitle, existingKey = null) {
    if (!folderType) {
      throw new Error("[FormJSON] folderType is required (DraftForms / PublishedForms / etc)");
    }
    try {
      ...
      if (existingKey) {
        ...
        key = `CustomisableForms/${this.orgid}/Participant/${dateStr}/${folderType}/${fileName}.json`;
      }
      ...
      console.log("📤 [FormJSON] PUT response:", putResp.status, putResp.statusText);
      if (!putResp.ok) {
  ...
  async uploadPdfFileToS3({ file, responseId, isUpdate, existingAwsJson }) {
    ...
    // ---- presign ----
    console.log(`🔐 [uploadPdfFileToS3] Requesting presign (${isUpdate ? "UPDATE" : "CREATE"})`);
    ...
    console.log("📤 [uploadPdfFileToS3] PUT response:", putResp.status, putResp.statusText);
    if (!putResp.ok) {
  ...
  async drawPdfContent(doc, logoData) {
    ...
    this.viewTableData.forEach((item) => {
      if (!item.isPageBreak && item.isVisible) {
        ...
        const isHeaderLike =
          ...
          item.type === "header" ||
          item.field?.isHeader === true ||
          item.field?.dataType === "Header" ||
          item.field?.type === "header";
        if (isHeaderLike) {
          const parseStyle = (s = "") => {
            const out = {};
            s.split(";").forEach((p) => {
          ...
          const styleSrc =
            item.headerStyle || item.inlineStyle || item.field?.headerStyle || item.field?.inlineStyle || "";
        ...
        if (item.field?.dataType === "Dropdown Field" || item.isDropdownField) {
          rawValue = item.field?.displayValue || item.field?.value || item.value || "";
        } else {
          rawValue = item.field?.value ?? item.value ?? "";
        }

        // ⭐ SIGNATURE detection (PDF)
        const isSignature =
          item.isSignature === true ||
          item.field?.dataType === "Signature" ||
          (item.value && String(item.value).startsWith("data:image"));
        console.log("🖊️ PDF signature check:", {
          label: item.label,
          isSignature,
          hasBase64: String(rawValue || "").startsWith("data:image")
        });
        let value = "";
        ...
        if (item.isCheckbox) {
          value = rawValue === true || rawValue === "true" || rawValue === "action:approval" ? "Yes" : "No";
        } else if (isRichTextItem) {
          value = stripHtml(rawValue);
        } else if (isSignature && rawValue) {
          value = " "; // keep cell height

          // store image for autoTable hook
          if (!this._pdfSignatureImages) {
            this._pdfSignatureImages = [];
          }
          this._pdfSignatureImages.push({
            rowIndex: rows.length,
            // where it will appear
            base64: rawValue
          });
          console.log("🖊️ PDF signature queued for table render");
    ...
    const logoReservedWidth = logoData?.base64 ? 65 : 0;
    const contentMaxW = headerW - headerPad * 2 - logoReservedWidth;
    ...
    const headerHeight = headerPad + nameBlockH + betweenBlocks + bodyTotalH + headerPad;
    ...
    if (logoData?.base64) {
      try {
        ...
        const logoX = headerX + headerW - logoWidth - 10;
        const logoY = headerY + (headerHeight - logoHeight) / 2;
        doc.addImage(optimizedLogo, "JPEG", logoX, logoY, logoWidth, logoHeight);
        console.log("🖼️ Optimized logo placed inside header");
      } catch (e) {
    ...
    const centerX = headerX + (headerW - logoReservedWidth) / 2;
    ...
    try {
      doc.autoTable({
        ...
        // 🔥 PRE-CALCULATE SAFE SIZE + ROW HEIGHT
        didParseCell: (data) => {
          if (!this._pdfSignatureImages?.length) return;
          if (data.section !== "body") return;
          if (data.column.index !== 1) return;
          const sig = this._pdfSignatureImages.find((s) => s.rowIndex === data.row.index);
          if (!sig) return;
          try {
            const imgProps = doc.getImageProperties(sig.base64);
            const maxWidth = 60; // max width allowed
            const maxHeight = 35; // 🔥 hard height limit

            let imgWidth = maxWidth;
            let imgHeight = imgWidth * (imgProps.height / imgProps.width);

            // 🔥 Prevent tall image overflow
            ...
            sig._scaledWidth = imgWidth;
            sig._scaledHeight = imgHeight;

            // 🔥 Dynamically adjust row height
            data.cell.styles.minCellHeight = imgHeight + 8;
          } catch (e) {
            console.error("❌ Signature sizing failed:", e);
          }
        },
        // 🔥 DRAW IMAGE SAFELY CENTERED
        didDrawCell: (data) => {
          if (!this._pdfSignatureImages?.length) return;
          if (data.section !== "body") return;
          if (data.column.index !== 1) return;
          ...
          if (!sig) return;
          try {
            const imgWidth = sig._scaledWidth;
            const imgHeight = sig._scaledHeight;
            const x = data.cell.x + (data.cell.width - imgWidth) / 2;
            const y = data.cell.y + (data.cell.height - imgHeight) / 2;
            doc.addImage(
              sig.base64,
              "JPEG",
              // 🔥 use JPEG if optimized
              x,
              y,
              imgWidth,
              imgHeight
            );
            console.log("🖼️ Signature drawn safely");
          } catch (e) {
            console.error("❌ Signature draw failed:", e);
          }
        }
      });
    } catch (e) {
    ...
    let imgY = (doc.lastAutoTable?.finalY ?? headerY + headerHeight + 6) + 10;
    for (const img of imageRows) {
      try {
        const { dataUrl, format } = await this.getDataUrlAndFormat(img.url, img.contentType);
        if (imgY + 40 > pageH) {
          doc.addPage();
          imgY = 10;
        }
  ...
  async drawPdfContentinView(doc) {
      ...
      if (!item.isPageBreak && item.isVisible) {
        ...
        if (isHeaderLike) {
          ...
          const styleSrc =
            item.headerStyle ||
            item.inlineStyle ||
            // ← section header (your current shape)
            item.field?.headerStyle ||
            item.field?.inlineStyle ||
            // ← nested field shape
            "";
          const s = parseStyle(styleSrc);
          ...
          console.log("PDF Section Header →", {
            ...
            styleSrc,
            parsedStyle: s,
            shape: item.isSectionHeader ? "sectionHeader" : item.field?.isHeader ? "field.header" : "other"
          });
    ...
    doc.autoTable({
      head: headers,
      body: rows,
      // now includes full-width section rows
      startY: headerY + headerHeight + 6,
      styles: {
        font: "Roboto",
        fontSize: 10,
...
.ql-editor li {
  ...
  fetchFormSignatureStatuses() {
    ...
    const promises = rows.map((r) => {
      ...
      return checkFormSignatureStatus({
        formResponseId: r.Id
      })
        .then((status) => {
    ...
    Promise.all(promises)
      .then((results) => {
        ...
        this.filteredFormResponses = this.filteredFormResponses.map((r) => {
          ...
          console.log("🔄 Row updated:", r.Id, "isSigned:", updated.isSigned, "signatureUrl:", updated.signatureUrl);
          return updated;
        });
  ...
  async handleTsignFromForms(event) {
    ...
    try {
      ...
      if (!awsUrl) {
        this.showToast("Missing PDF", "Please submit the form to generate the PDF before using TSign.", "warning");
        return;
      }
      const clientId = resp?.Client_Id__c;
      let clientEmail = "";
      if (clientId) {
        clientEmail = await getClientEmailByClientId({
          clientId
        });
      }
  ...
  initializeSignaturePad() {
    console.log("🖊️ [Signature] initializeSignaturePad called");
  ...
  buildViewTableDataFromSubmitRows(allRows) {
    ...
    allRows.forEach((row) => {
      row.cells.forEach((cell) => {
        ...
        const isPageBreak = dataType === "Blank" && label.toLowerCase() === "page break";
        ...
        if (dataType === "Dropdown Field") {
          value = f.displayValue || (f.options || []).find((o) => o.value === f.value)?.label || f.value || "";
        }
        ...
        out.push({
          ...
          isCheckbox: cell.isCheckboxField || dataType === "Checkbox Field" || dataType === "Checkbox",
          isRichText: dataType === "Descriptive Text"
        });
      });
    });
    return out;
  ...
  async handleInsertSubmit(responseAwsMeta) {
    ...
    this.selectedParticipantName = resp?.Participant_Name__c || this.selectedParticipantName || "";
  ...
  async handleUpdateSubmit(responseAwsMeta) {
    ...
    this.selectedParticipantName = resp?.Participant_Name__c || this.selectedParticipantName || "";
    const existing = (this.formResponses || []).find((r) => r.Id === updatedId) || {};
    const existingAwsJson = existing.AWS_Json__c || existing.Aws_Json__c || "";
  ...
  async processSubmit() {
    ...
    const allRows = (this.stepPagedRows || []).flat().map((row) => ({
      ...row,
      cells: row.cells.map((cell) => {
        const field = cell.field;
        if (field && field.dataType === "Blank" && field.label?.toLowerCase().trim() === "page break") {
          return cell;
        }
        let newField;
        ...
        if (cell.isDropdownField && !cell.isMultiSelect) {
          const selectedOption = (cell.field?.options || []).find((o) => o.isSelected);
          const selectedValue = cell.field?.value || "";
          const displayValue = cell.field?.displayValue || selectedOption?.label || selectedValue;
          newField = {
            ...cell.field,
            value: selectedValue,
            displayValue
        ...
        if (cell.isRadioButton || field?.dataType === "Radio Button") {
          const selectedRadio = cell.selectedRadioOption || field?.value?.selectedOption || field?.value || "";

          // 🔥 IMPORTANT: Prefer LABEL over ID
          const subInputLabel =
            field?.value?.subLabel ||
            // ✅ correct label
            cell.subInputValue ||
          ...
          newField = {
            ...
            value: subInputLabel ? `${selectedRadio} - ${subInputLabel}` : selectedRadio
          };
          return {
            ...cell,
    ...
    // ✅ Set header values
    this.selectedFormTitle = this.selectedForm?.Name__c || this.selectedFormTitle || "Form";
    this.selectedSubmissionDate = new Intl.DateTimeFormat("en-GB").format(new Date());
    this.selectedParticipantName = this.selectedParticipantName || this.clientName || this.selectedClientName || "";
    ...
    if (this.isEditing) {
      const existing = (this.formResponses || []).find((r) => r.Id === this.selectedResponseId) || {};
      const existingJson = existing.Response_JSON__c;
      if (existingJson && typeof existingJson === "string") {
        try {
          existingJsonKey = JSON.parse(existingJson)?.key || null;
        } catch (e) {
    ...
    (this.stepPagedRows || []).flat().forEach((row) => {
      row.cells.forEach((cell) => {
        ...
        if (field && field.isRequired === true) {
          const isEmpty = field.value === undefined || field.value === null || field.value.toString().trim() === "";
          if (isEmpty) {
  ...
  handleMultiSelectChange(event) {
    ...
    const applyChange = (cell) => {
      ...
      const updatedOptions = (cell.field?.options || []).map((opt) => {
        const match = opt.label === optionLabel || opt.value === optionLabel;
        return match
          ? {
              ...opt,
        ...
        cells: row.cells.map((cell) => {
          ...
          if (cell?.isTableBlock && cell?.field?.tableConfig) {
            ...
            (config.innerCells || []).forEach((r, rIndex) => {
              (r.cells || []).forEach((c, cIndex) => {
                const renderCell = config.renderRows?.[rIndex]?.renderCells?.find(
    ...
    this.stepPagedRows = JSON.parse(JSON.stringify(this.stepPagedRows.map((page) => page.map(updateRow))));
  }
  handleParticipantDropdownClick() {
    ...
    if (this.userType === "NDIS Staff") {
      this.participantOptions = this.participants
        .filter((p) => p.visibleStaffMembers && p.visibleStaffMembers.some((staff) => staff.email === this.userEmail))
        .map((p) => ({
          label: p.Name,
          value: p.Id
        }));
    ...
    this.groupedResponses = this.groupedResponses.map((g) =>
      g.id === key
        ? {
            ...g,
            expanded: exp,
            arrowClass: this.getArrowClass(exp)
          }
        : g
    );
  ...
  syncMultiSelectFromValue(cell) {
    if (!cell?.field || cell.field.dataType !== "Dropdown Field" || cell.field.selectedDropdownOption !== "multiSelect")
      return cell;
    ...
    const updatedOptions = (cell.field.options || []).map((opt) => ({
      ...opt,
      isSelected: selectedValues.includes(opt.value) || selectedValues.includes(opt.label)
    }));
    return {
      ...cell,
      field: {
          ...
          if (cell?.isTableBlock && cell?.field?.tableConfig) {
            ...
            (config.renderRows || []).forEach((r) => {
              (r.renderCells || []).forEach((c) => {
                ...
                c.isDropdownOpen = String(c.id) === cellId ? !c.isDropdownOpen : false;
              });
            });
  ...
  buildCellRuntimeModel(cell, rowIndex, colIndex) {
    ...
    // =====================================================
    let isSignature = cell.field?.dataType === "Signature" || cell.field?.isSignature === true;
    if (isSignature) {
    ...
    if (cell.field?.dataType === "Upload File") {
      isUploadField = true;
      const urls = Array.isArray(cell.field.value) ? cell.field.value : [];
    ...
    if (cell.field?.dataType === "Dropdown Field") {
        ...
        if (type === "staff" || type === "participant" || type === "facility") {
          ...
          if (type === "staff" && this.staffOptions?.length) {
            options = this.staffOptions.map((o) => {
              ...
              const isSelected = storedNorm === idNorm || storedNorm === labelNorm;
              if (isSelected) {
          ...
          if (type === "participant" && this.participants?.length) {
            ...
            options = this.participants.map((p) => {
              ...
              const isSelected = storedNorm === idNorm || storedNorm === nameNorm;
              if (isSelected) {
                matchedId = p.Id;
      ...
      // ---- manual multi
      if (cell.field.selectedDropdownOption === "multiSelect" && cell.field.multiSelectValues) {
    ...
    if (cell.field?.dataType === "Radio Button") {
      ...
      cell.radioOptionsProcessed = (cell.field.radioOptions || []).map((opt) => {
        const isSelected = String(selectedRadioOption) === String(opt.optionLabel);
        ...
        if (opt.usePredefinedOptions) {
          ...
          processedSubOptions = source.map((o) => ({
            ...
            isSelected: String(subInputValue) === String(o.value) || String(subInputValue) === String(o.label)
          }));
        }
        ...
        // =====================================================
          ...
          processedSubOptions = (opt.values || []).map((v) => ({
            label: v,
            value: v,
            isSelected: String(subInputValue) === String(v)
          }));
        }
        return {
          ...opt,
    ...
    let headerText = null;
    let headerStyle = "";
    if (cell.field?.dataType === "Header" || cell.field?.isHeader === true) {
      isHeader = true;
    ...
    if (isTableBlock) {
      const tableConfig = JSON.parse(JSON.stringify(cell.field?.tableConfig || {}));
      const columns = tableConfig.columns || [];
      const rowNames = tableConfig.rowNames || [];
      const innerCells = tableConfig.innerCells || [];
        ...
        return {
          ...
          renderCells: columns.map((col, cIndex) => {
            ...
            if (col?.isRowLabel) {
              ...
              // 🔥 BUILD CELL
              // ===============================
              const rebuiltCell = this.buildCellRuntimeModel(innerCell, rIndex, cIndex);
              console.groupEnd();
    ...
    // VISIBILITY
    // =====================================================
    const isPageBreak = cell.field?.dataType === "Blank" && cell.field?.label?.toLowerCase().trim() === "page break";
    ...
    return {
      ...
      isVisible,
      isTextField:
        cell.field?.dataType === "Text Field" && !isTextArea && !isAlphaNumeric && !isOnlyAlphabets && !isRichText,
      isTextArea,
      isAlphaNumeric,
      isOnlyAlphabets,
      isNumberField: cell.field?.dataType === "Number Field",
  ...
  buildGroups(rows) {
    ...
    this.totalGroupPages = Math.ceil((this.totalGroupRecords || 0) / this.formPageSize);
  }
...
`;
  ...
  buildValueMapFromRuntime() {
    ...
    (this.stepPagedRows || []).flat().forEach((row) => {
      row.cells.forEach((cell) => {
        ...
        // 🔥 Upload (FIXED)
        if (cell.isUploadField || field?.isUpload || field?.dataType === "Upload File") {
          values[field.id] = {
            value: field.value || [],
        ...
        if (cell.isDropdownField && cell.isMultiSelect) {
          const selected = (field.options || []).filter((o) => o.isSelected).map((o) => o.label);
        ...
        if (cell.isDropdownField && !cell.isMultiSelect) {
          ...
          values[field.id] = {
            value: field.value || "",
            displayValue: field.displayValue || selectedOption?.label || field.value || ""
          };
          return;
        }

        // Number / Time / Date
        if (cell.isNumberField || cell.isTimeField || cell.isDateField) {
          values[field.id] = field.value || "";
          return;
        }
        ...
        if (cell.isRadioButton) {
          const base = cell.selectedRadioOption || field?.value?.selectedOption || field.value || "";
          ...
          const subLabel =
            ...
            // fallback
            "";
          values[field.id] = subLabel ? `${base} - ${subLabel}` : base;
        }
  ...
  expandview() {
    ...
    console.log("🔀 expand toggle →", this.isExpandview ? "expanded" : "compressed");
  }
  ...
  async getDataUrlAndFormat(url) {
    ...
    const mime = typeof dataUrl === "string" ? dataUrl.slice(5, dataUrl.indexOf(";")) : "";
  ...
  getFormattedValue(cell) {
    ...
    if ((cell.isDropdownField || dataType === "Dropdown Field") && !cell.isMultiSelect) {
      const field = cell.field || {};
      const matchedOption = (field.options || []).find((opt) => opt.value === field.value);
      return (
        field.displayValue ||
        // 🔥 THIS WILL NOW WORK
        matchedOption?.label ||
    ...
    if (cell.isRadioButton || dataType === "Radio Button") {
      ...
      const selectedOption = cell.selectedRadioOption || fieldValue || "";
  ...
  getUniqueNamesFromPage() {
    const raw = (this.paginatedFormResponses || []).map((r) => r?.Name__c || "Unnamed");
  ...
  async handleEdit(event) {
    ...
    let templateRaw = response?.Form_Reference__r?.Form_JSON__c;
    if (templateRaw) {
      try {
    ...
    try {
      ...
      this.viewLogFormHeader("before-edit-load", this.selectedFormTitle, this.selectedFormType, response);
      ...
      const filteredRows = parsedResponseJson.map((row, rowIndex) => ({
        ...
        cells: row.cells.map((cell, colIndex) => {
          ...
          return this.buildCellRuntimeModel(cell, rowIndex, colIndex);
        })
      }));
      ...
      this.tableRows.forEach((row) => {
        row.cells.forEach((cell) => {
          if (cell.isDropdownField && cell.field?.selectedDropdownOption === "predefinedList") {
      ...
      this.stepCurrentPageIndex = 0;
      this.updateStepVisibleRows();
      this.viewTableData = this.buildViewTableDataFromSubmitRows(this.stepPagedRows.flat());
      ...
      const visibleLabels = this.tableRows
        .flatMap((row) => row.cells)
        .filter((cell) => cell.isVisible && cell.field?.label && cell.field?.dataType !== "Blank")
        .map((cell) => cell.field.label);
      console.log("🧾 Visible Field Labels AFTER edit build:", visibleLabels);
    } catch (err) {
      console.error("❌ Error loading response for edit:", err);
      this.showToast("Error", "Failed to load submitted form for editing.", "error");
    }
  }
    ...
    if (this.selectedForm) {
      ...
      const filteredRows = parsedJson.map((row, rowIndex) => ({
        ...
        cells: row.cells.map((cell, colIndex) => {
          ...
          if (cell.field?.isTableBlock || cell.field?.dataType === "Table Block") {
            isTableBlock = true;
            const builtConfig = this.buildTableBlockRenderModel(cell);
            console.log("🆕 TableBlock INIT → renderRows:", builtConfig.renderRows?.length);
            cell = {
              ...cell,
              field: {
                ...cell.field,
  ...
  handleGrantAccessClick(event) {
    ...
    this.selectedParticipantStaffNames =
      participant?.visibleStaffMembers?.map((staff) => staff.name).join(", ") || "No Staff Assigned";
  ...
  handleInputChange(event) {
    ...
    const isRichText =
      target?.tagName?.toUpperCase() === "LIGHTNING-INPUT-RICH-TEXT" || target?.classList?.contains("rich-text-editor");
    ...
    if (inputType === "checkbox") {
      ...
      let cleaned = raw.replace(/<img\b[^>]*>/gi, "").replace(/url\(["']?data:image\/[^"')]+["']?\)/gi, "");
      newValue = cleaned;
      if (cleaned !== raw) {
        const rte = this.template.querySelector(`.rich-text-editor[data-id="${cellId}"]`);
    ...
    this.updateCellOnCurrentPage(cellId, (cell) => {
      ...
      if (inputType === "checkbox") {
        ...
        // ✅ NORMAL DROPDOWN (NO CHANGE)
          ...
          displayValue = selectedOption?.label || selectedOption?.value || newValue;
        }
        updated.field = {
          ...safeField,
          value: newValue,
  ...
  handleRadioChange(event) {
    ...
    const buildUpdatedCell = (cell) => {
      ...
      const sourceOptions = cell.radioOptionsProcessed || field.radioOptions || [];
      const selectedOptionObj = sourceOptions.find((opt) => String(opt.optionLabel) === String(selectedValue));
      field.value = {
      ...
      const radioOptionsProcessed = sourceOptions.map((option) => {
        const isSelected = String(selectedValue) === String(option.optionLabel);
        return {
          ...option,
          isSelected,
  ...
  handleRadioSubInputChange(event) {
    ...
    const applySubChangeToCell = (cell) => {
      ...
      const selectedRadio = cell.selectedRadioOption || field.value?.selectedOption || "";
      console.log("🔹 Selected Radio Option:", selectedRadio);
      const sourceOptions = cell.radioOptionsProcessed || field.radioOptions || [];
      const selectedOption = sourceOptions.find((o) => String(o.optionLabel) === String(selectedRadio));
      ...
      if (subType === "dropdown") {
        if (selectedOption?.usePredefinedOptions) {
          ...
          const match = source.find((o) => String(o.value) === String(subValueRaw));
      ...
      const updatedProcessed = sourceOptions.map((opt) => {
        const isMainSelected = String(opt.optionLabel) === String(selectedRadio);
        if (!isMainSelected) {
          return {
            ...opt,
            isSelected: false
  ...
  handleSearchChange(event) {
    ...
    this.filteredForms = this.forms.filter((f) => (f.Name__c || "").toLowerCase().includes(this.searchQuery));
  }
  ...
  async handleView(event) {
    ...
    try {
      ...
      console.log("📅 Submission Date:", this.selectedSubmissionDate);
        console.log("📅 staffNameToDisplay:", this.staffNameToDisplay);
      ...
      this.tableRows = (formJson || []).map((row, rowIndex) => ({
        ...
        cells: row.cells.map((cell, colIndex) => {
          if (cell.isDropdownField && cell.isMultiSelect) {
            const selectedOptionsArray = cell.field.value ? cell.field.value.split(", ") : [];
      ...
      console.error("❌ Error resolving Response_JSON__c or preparing view:", e);
      this.showToast("Error", "Failed to load submitted form data.", "error");
    }
  }
  get hasResponses() {
    return Array.isArray(this.paginatedFormResponses) && this.paginatedFormResponses.length > 0;
  }
  get imageUrl() {
  ...
  injectValuesIntoTemplate(template, valueMap) {
    template.forEach((row) => {
      row.cells.forEach((cell) => {
        if (cell.field?.isRichTextDisplay) {
          return; // ❌ DO NOT INJECT
        }
        if (cell.field?.id && valueMap.hasOwnProperty(cell.field.id)) {
          const saved = valueMap[cell.field.id];

          // 🔥 Upload field (CRITICAL FIX)
          if (cell.field?.dataType === "Upload File" && saved && typeof saved === "object") {
            cell.field = {
              ...cell.field,
              ...saved,
              meta: {
          ...
          if (cell.field?.dataType === "Table Block") {
            ...
            innerRows.forEach((innerRow, r) => {
              (innerRow.cells || []).forEach((innerCell, c) => {
                ...
                if (innerField.isCheckbox) {
                ...
                } else if (innerField.isDropdown && innerField.selectedDropdownOption === "predefinedList") {
                  const selectedValue = typeof value === "object" ? value.value : value;
                  let selectedLabel = typeof value === "object" ? value.displayValue || value.label : "";
                  ...
                  innerField.displayValue = selectedLabel || innerField.displayValue || selectedValue || "";
                } else {
                  innerField.value = value ?? "";
                }
              });
  ...
  isDisplayOnly(node) {
    const t = (node?.type || node?.ctrlType || node?.fieldType || "").toString().toLowerCase();
    return ["header", "heading", "title", "section", "divider", "hr", "html", "image", "button", "spacer"].includes(t);
  }
  get isDrawMode() {
  ...
  isFilled(val) {
    ...
    if (Array.isArray(val)) return val.filter((v) => this.isFilled(v)).length > 0;
  ...
  get isTypeMode() {
    return this.signatureMode === "type";
  }
  lastFormPage() {
  ...
  persistVisiblePage() {
    ...
    this.stepPagedRows = (this.stepPagedRows || []).map((page, idx) => (idx === i ? cloned : page));
  }
  prepareViewTable(formJson) {
    ...
    formJson.forEach((row, rowIndex) => {
      ...
      row.cells.forEach((cell, colIndex) => {
        ...
        if (cell.field?.isRichTextDisplay === true && cell.field?.richTextContent) {
          normalizedValue = cell.field.richTextContent;
        }
        const isUploadFile = dataType === "upload file" || cell.field?.isUpload === true;
        const isRichText =
        ...
        const isSignature = dataType === "Signature" || cell.field?.isSignature === true;
        const isTableBlock = dataType === "Table Block" || cell.field?.isTableBlock === true;
        const toArrayFn = typeof toArray === "function" ? toArray : this.viewToArray.bind(this);
        const isImageUrlFn = typeof isImageUrl === "function" ? isImageUrl : this.viewIsImageUrl.bind(this);
        ...
        if (isTableBlock) {
          ...
          const rowNames = tableConfig.rowNames || [];
          const hasRowLabel = headers.length > 0 && headers[0]?.isRowLabel === true;
          const tableClass = hasRowLabel ? "view-table-block has-row-label" : "view-table-block";
          const rows = matrix.map((rowObj, rIndex) => {
            ...
            return {
              ...
              cells: headers.map((col, cIndex) => {
                ...
                const rawCell = actualColIndex !== null ? rowCells.find((c) => c.col === actualColIndex) : null;
                ...
                // 📅 DATE FORMAT FIX
                // =========================
                if (field.dataType === "Date Field" && rawValue) {
                  try {
                ...
                if (field.isRichTextDisplay === true && field.richTextContent) {
                ...
                // 📁 FILE HANDLING
                // =========================
                const fileUrls = field.urls || field.downloadLink || (Array.isArray(rawValue) ? rawValue : []);
                ...
                if (field.isDropdown === true && field.selectedDropdownOption === "predefinedList") {
                  displayValue = field.displayValue || rawValue || "";
                }
                ...
                // 🟣 Descriptive Text
                // =========================
                const richTextHtml = isRichText ? `<div class="ql-editor">${rawValue || ""}</div>` : null;
                return {
                  key: `${rIndex}-${cIndex}`,
                  cellClass: "",
                  isRowLabel: false,
                  value: displayValue,
                  isCheckbox,
                  checkboxIcon: isCheckbox ? (rawValue ? "✔" : "✖") : null,
                  isUploadFile,
                  files,
                  isRichText,
                  richTextHtml,
                  isSignature,
                  isRadioButton,
                  isRegularField: !isCheckbox && !isUploadFile && !isRichText && !isSignature && !isRadioButton
                };
              })
            };
          });
        ...
        processedRow.cells.push({
          ...
          richTextHtml: isRichText ? `<div class="ql-editor">${normalizedValue || ""}</div>` : null,
          colspan: cell.colspan || 1,
          rowspan: cell.rowspan || 1,
          gridStyle: `grid-column: span ${cell.colspan || 1}; grid-row: span ${cell.rowspan || 1};`,
          ...
          isRichText,
          isRadioButton,
          isRegularField: !isUploadFile && !isCheckbox && !isRichText && !isRadioButton && !isSignature
        });
      });
  ...
  processCellField(cell, rowIndex = 0, colIndex = 0) {
    ...
    if (cell.field?.dataType === "Radio Button") {
      ...
      radioOptionsProcessed = (cell.field.radioOptions || []).map((option) => {
        const isSelected = String(selectedRadioOption) === String(option.optionLabel);
        let processedSubOptions = [];
        if (option.usePredefinedOptions) {
          ...
          processedSubOptions = source.map((opt) => ({
            ...
            isSelected: String(subInputValue) === String(opt.value) || String(subInputValue) === String(opt.label)
          }));
        } else {
    ...
    if (cell.field?.isSignature || cell.field?.dataType === "Signature") {
      isSignature = true;
      signatureButtonLabel = cell.field?.value ? "View / Update Signature" : "Add Signature";
    }
  ...
  async processFormResponses(data) {
    ...
    const rawNames = this.allFormResponses.map((r) => this._normalizeName(r?.Name__c));
  ...
  recomputeFiltersAndPagination() {
    ...
    if (this.selectedNameFilter !== "ALL") {
      filtered = base.filter((r) => this._normalizeName(r?.Name__c) === this.selectedNameFilter);
    }
    this.filteredFormResponses = filtered;
    this.totalFormRecords = filtered.length;
    this.totalFormPages = Math.max(1, Math.ceil(this.totalFormRecords / this.formPageSize));
    this.updatePaginatedFormResponses();
  }
        ...
        cells: (row.cells || []).map((cell, cIdx) => {
          ...
          if (cell.isDropdownField && !cell.isMultiSelect) {
            ...
            return {
              ...
              domKey: cell.id,
              isVisible:
                !(cell.field?.dataType === "Blank" && cell.field?.label?.toLowerCase().trim() === "page break") &&
                cell.style !== "display: none;",
              hasContent: !!(cell.field?.label || cell.field?.value)
            };
          }
  ...
  viewIsHeaderCell(cell) {
    ...
    const opt = (cell?.field?.selectedTextFieldOption || "").toLowerCase().trim();
    return (
      t === "header" ||
      t === "heading" ||
      t === "section header" ||
      (t === "text field" && ["header", "heading", "title", "h1", "h2", "h3"].includes(opt)) ||
      cell?.isHeader === true ||
  ...
  viewLogFormHeader(where, title, type, resp) {
    console.log(`🧩 [${where}] Header snapshot → title=`, title, ", type=", type);
    if (!title) {
      console.warn(`⚠️ [${where}] selectedFormTitle is empty. Checked fields: Name__c, Name.`, {
        Name__c: resp?.Name__c,
        Name: resp?.Name
      });
    }
    if (!type) {
      console.warn(`⚠️ [${where}] selectedFormType is empty. Checked fields: Form_Type__c, Type__c, FormType.`, {
