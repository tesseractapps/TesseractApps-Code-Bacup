        ...
        .then(result => {
          ...
          const time = task.Task_Time__c;
           ...
           if (time !== null && time !== undefined) {
            ...
            this.disableTimeButton = this.statusValue !== 'Open';
...
fetchSelectedStaff() {
 ...
 handleFileDeleted(event) {
    console.group('handleFileDeleted');

    const { fileId, files = [] } = event.detail;

    console.log('Remaining files from child:', JSON.stringify(files));
  ...
  /*   // Clear selected files
    ...
    // If no files remain, hide the template
    ...
    console.log('Parent uploadedFiles:', JSON.stringify(this.uploadedFiles));
    console.groupEnd();
}
   ...
   handlefilecancel(event){
    ...
    console.log('Remaining files from child:', JSON.stringify(this.uploadedFiles));
}
