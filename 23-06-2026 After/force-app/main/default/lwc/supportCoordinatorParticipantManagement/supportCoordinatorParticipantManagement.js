import { LightningElement, track, api } from 'lwc';
    ...
    import deleteContacts from "@salesforce/apex/SupportParticipantController.deleteContacts";
    ...
    @api async openParticipantDetailsFromDashboard(participantId) {
        console.log('==============================');
        console.log('📥 openParticipantDetailsFromDashboard called');
        ...
        try {
            ...
            this.selectedParticipant  = {
                ...
                phone: participant.Phone__c || '',
                email: participant.Email__c || '',
                ndisNumber: participant.NDIS_Number__c || '',
                status: participant.Status__c || '',
                risklevel: participant.Risk_Level__c || '',
                industryType: participant.Type_of_Industry__c || '',
                classification: participant.Classification1__c || '',
                participantAbout: participant.About_Participant1__c || '',
            };
    ...
    //Sai Eswar
    @track supportClientAttachments = [];
    @track supportClientFolders = [];
...
@track isAllCoordinatorSelected = false;
@track isAllCatalogueSelected = false;
    ...
    get step4Class() { 
     ...
     get step5Class() { 
        return this.createCurrentStep === 'createstep5' ? '' : 'slds-hide'; 
    }
    get totalSteps() {
         return 5; 
    }
    ...
    get showSave()      { 
        return !!this.editingParticipantId || (this.currentStepNumber === this.totalSteps); 
    }
            ...
            this.allParticipants = res.records.map((p, index) => {
                ...
                return {
                   ...
                   // name: `${p.First_Name__c || ''} ${p.Last_Name__c || ''}`.trim(),
                    ...
                    riskClassFull: `${riskBase}  slds-truncate`,  //slds-text-heading_small
                   // abn:p.abn,
                    status:p.Status__c,
                    risklevel:p.Risk_Level__c,
                    industryType:p.Type_of_Industry__c,
                    classification:p.Classification1__c,
                    participantAbout:p.About_Participant1__c
                };
            });
...
//     console.log( 'Selected Classification:', JSON.stringify(this.selectedClassification));
//     }
...
handleClassificationChange(event) {
    ...
    console.log(
        ...
        JSON.stringify(this.selectedClassification)
    );
}
   ...
   createParticipantcloseModal() {
    ...
    async createParticipantsaveParticipant() {
        ...
        const participantPayload = {
           ...
           selectedClassification: Array.isArray(this.selectedClassification)
            ? this.selectedClassification.join(',')
            : '',
          //  selectedServiceTypes:this.selectedServiceTypes || '',
            ...
            supportClientAttachments: this.supportClientAttachments || [],
            supportClientFolders: this.supportClientFolders || [],
            deletedAttachmentIds: this.deletedAttachmentIds || [],
            editedAttachments: this.editedAttachments || []
        };

        console.log('participantPayload => ', JSON.stringify(participantPayload));
        console.log('supportClientFolders =>',JSON.stringify(this.supportClientFolders));
        console.log('deletedAttachmentIds =>',JSON.stringify(this.deletedAttachmentIds));
        console.log('editedAttachmentIds =>',JSON.stringify(this.deletedAttachmentIds));
        ...
        try {
            ...
            console.log('supportClientAttachments',JSON.stringify(this.supportClientAttachments));

            console.log('Participant Saved => ', participantId);
            console.log('Participant Saved => ', participantId);

            this.supportClientFolders = [];
            this.supportClientAttachments = [];
            this.deletedAttachmentIds = [];

            console.log('EDIT MODE FOLDERS =>',JSON.stringify(this.supportClientFolders));
            console.log('EDIT MODE ATTACHMENTS =>',JSON.stringify(this.supportClientAttachments));
            ...
            if (this.isOpenedFromDashboard) {
                this.dispatchEvent(
                    new CustomEvent('participantcreated')
                );
    ...
    handleparticipantCreateChange(event) {
        ...
        switch(field){
            ...
            case 'coordinatorCurrentAmountSpent':
                ...
                //                                         : Number(this.supportCoordinationBudget || 0);
                this[field] = value;
console.log('refreshPlanBudgetValidation ENTERED');
                // ✅ Remaining Budget
               ...
               this[field] = Number(value) || 0;
 console.log('Calling refresh validation');
this.refreshPlanBudgetValidation();
...
console.log('====================');
    ...
    budgetFields.forEach(fieldName => {
        ...
        /* const input =
            this.template.querySelector(
                `[name="${fieldName}"]`
            ); */ //manendra commented for dynamic input fetching
    ...
    console.log(
        ...
        if (
            budgetSpent > totalPlanBudget &&
            fieldName === 'budgetSpent'
        ) {

            input.setCustomValidity(
                'Budget Spent to Date cannot exceed Total Plan Budget'
            );
            ...
            (
                fieldName === 'budgetSpent' ||
                fieldName === 'providerBudget' ||
                fieldName === 'supportCoordinationBudget'
            )
        ) {

            input.setCustomValidity(
                'Provider Budget + Support Coordination Budget cannot exceed Remaining Budget'
            );
    ...
    handleViewDetails(event) {
        ...
        console.log('Participant found:',JSON.stringify(record, null, 2));
        ...
        this.selectedParticipant = {
            ...
            mobile: record.phone || '',
            ndis: record.ndisNumber || '',
            totalBudget: record.planBudget || 0,
            remainingBudget: (Number(record.planBudget) || 0) - (Number(record.budgetSpent) || 0),
            budgetUsedPercent:
                record.planBudget
                    ? Math.round(
            ...
            classification:record.classification || '',
            participantAbout:record.participantAbout || ''
        };
        ...
        console.log( 'selectedParticipant =>',JSON.stringify(this.selectedParticipant, null, 2));
        console.log('showDetailsPage:', this.showDetailsPage   );
    }
    ...
    get details() {
        ...
        return {
           ...
           // abn:p.abn,
            ...
            participantAbout:p.participantAbout || '',
    ...
    async handleEditPlan(event){ 
        ...
        try {
            ...
            this.selectedClassification =
                                        participantDetails.Classification1__c
                                            ? participantDetails.Classification1__c
                                                .split(',')
                                                .map(item => item.trim())
                                            : [];
            this.selectedTypeOfIndustry = participantDetails.Type_of_Industry__c || '';
            this.aboutParticipant = participantDetails.About_Participant1__c || '';
                ...
                // serviceType: group.Service_Type__c || '',
                    ...
                    supportItems:
                        (group.Support_Catalogues__r || []).map(item => ({
                            ...
                            id: item.Id,
                            //catalogueId: item.Id,
                            catalogueId:item.Support_Coordinator_NDIS_Catalogue__c || null,
                ...
                (participantDetails.Support_Client_Contacts__r || []).map(contact => ({
                    ...
                    uploadedFiles:
                        (contact.Support_Client_Contact_Attachment__r || []).map(file => ({
                            ...
                            size: file.Size__c || '',
                            type: file.Type__c || '',
                            comments: file.Comments__c || '',
                            otherComments: file.Other_Comments__c || '',
                            expiryDate: file.Expiry_Date__c || '',
                            userId: file.User__c || '',
                            addedBy: file.Added_By__c || ''
                        }))
                }));
                 await this.loadContactAttachmentCounts();
    ...
    handleRowProviderChange(event) {
        ...
        // 🔥 Ignore search event from combobox
        if (!Array.isArray(event.detail.value)) {
            console.log('Ignoring search value:', event.detail.value);
            return;
        }
    ...
    handleCoordinatorServiceTypeChange(event) {
        const value = event.detail.value;

        console.log( 'selectedCoordinatorServiceType:',  value );

        const alreadyExists = this.coordinatorPendingServiceGroups.some( group => group.serviceType === value );
    ...
    async handleEditService(event) {
      ...
      //  const group = this.displayedServiceGroups.find(
       ...
       // console.log('👉 Clicked serviceType:', serviceType);
        ...
        this.isAllCatalogueSelected =this.ndisCatalogue.length > 0 &&this.ndisCatalogue.every(row => row.selected);
    ...
    async loadExistingParticipantCatalogues() {
        ...
        existing.forEach(rec => {
            if (rec.IsActive__c) {
                const key =
                rec.Support_Coordinator_NDIS_Catalogue__c +
                '-' +
                rec.State__c ;
               // '-' +
        ...
        const match = Object.values(this.selectedCatalogueMap).find(rec =>
            rec.Support_Coordinator_NDIS_Catalogue__c === row.id &&
   ...
   handleSelectAllCatalogue(event) {

        const checked = event.target.checked;

        this.isAllCatalogueSelected = checked;

        this.ndisCatalogue = this.ndisCatalogue.map(row => {
            return {
                ...row,
                selected: checked,
                disableProvider: !checked,
                isEditingAmount: false
            };
        });
        // update select all checkbox
       this.isAllCatalogueSelected =this.ndisCatalogue.length > 0 &&this.ndisCatalogue.every(r => r.selected);


        // load providers only when selecting
        if (checked && (!this.providerOptions || this.providerOptions.length === 0)) {
            this.loadProviders();
        }

        console.log(
            'Select All ndisCatalogue',
            JSON.stringify(this.ndisCatalogue)
        );
    }
    ...
    async handleAddService() {
        ...
        const selectedItems = this.ndisCatalogue.filter(r => r.selected === true)//manendra added for unselected record update in edit mode
    ...
    async handleEntryChange(event) {
        ...
        if (field === 'providerId') {
            ...
            this.supportItemOptions = filteredItems.map(i => ({
                label: i.Support_Coordinator_NDIS_Catalogue__r
    ...
    /*    if (field === 'supportItemName') {
        ...
        if (field === 'supportItemName') {
            ...
            if (selected) {
                ...
                // 🔥 STORE BOTH
                this.selectedSupportItemLabel =
                    selected.Support_Coordinator_NDIS_Catalogue__r
                        ?.Support_Item_Name__c;
   ...
   supportItemNumber: this.selectedSupportItemNumber,
    ...
    i => i.Support_Coordinator_NDIS_Catalogue__r
...
handleSaveContactAttachment() {
    ...
    if(this.attachmentName === '' || this.attachmentName === null || this.attachmentName === undefined){
            ...
            console.error('❌ No attachment name provided');

            this.dispatchEvent(
                new ShowToastEvent({
                    title: "Error",
                    message: "Please provide attachment name",
                    variant: "error"
                })
            );
 ...
 if(!this.attachmentType){
            ...
            console.error('❌ No attachment type selected');
...
handleDeleteAttachment(event) {
    ...
    //     });
    // }
    async loadContactAttachmentCounts() {

    try {

        const contactIds = this.contactList
            .filter(con => con.id && con.id !== 1)
            .map(con => con.id);
        ...
        // ✅ WAIT FOR APEX
        const result = await getContactAttachmentCounts({
            contactIds: contactIds
        });
    ...
    //      console.log( 'handleOpenAttachmentList Clicked');
    //     const index = event.currentTarget.dataset.index;
    //     console.log( ' index inhandleOpenAttachmentList ',index);
    //     console.log(  ' Current Contact List =>',  JSON.stringify(this.contactList) );

    //     const contact = this.contactList[index];
    //     console.log( ' contact inhandleOpenAttachmentList ',contact);

    //     if (!contact || !contact.id) {
    //          console.error(' Contact or Contact Id Missing');
    //         return;
    //     }
    ...
    //      console.log( ' this.showAttachmentListModal  ',this.showAttachmentListModal);
...
handleOpenAttachmentList(event) {
    ...
    //       console.log( 'loadContactAttachments Clicked');
    //        console.log( ' this.selectedContactId  in loadContactAttachments ',this.selectedContactId);
    
    //     getContactAttachments({ contactId: this.selectedContactId })
    //     .then(result => {
    //         console.log('result in load attachments  ', JSON.stringify(result));
           
    //          this.contactAttachments =result.map((file, index) => {
    //             console.log( ` Processing Backend File ${index + 1}:`,  JSON.stringify(file) );
    ...
    //             console.log( ' Normalized contactAttachment:', JSON.stringify(normalizedFile) );
    //             return normalizedFile;
    //         });
    ...
    //                         comments: file.Comments__c || '',
    //                         otherComments: file.Other_Comments__c || '',
    //                         expiryDate: file.Expiry_Date__c || null,
    //                         userId: file.User__c || '',
    //                         addedBy: file.Added_By__c || '',
    //                         viewUrl: file.Amazon_file_URL__c || ''
    //                     };
    ...
    //         console.log(  ' Attachments synced:',  JSON.stringify(this.contactList[contactIndex])
    //         );
    //     })
    //     .catch(error => {
    ...
    loadContactAttachments() {
        ...
        .then(result => {
            ...
            console.log( ' Attachment Count:', result?.length || 0 );
            ...
            console.log(  ' Final contactAttachments:',  JSON.stringify(this.contactAttachments) );
            console.log(  ' Finding contact index...' );
            ...
            if (contactIndex !== -1) {
                console.log(' Syncing uploadedFiles into contactList'  );

                this.contactList[contactIndex].uploadedFiles =
                    result.map((file, index) => {

                        console.log(  ` Sync File ${index + 1}:`, JSON.stringify(file) );
                        ...
                        console.log( ' uploadedFile Object:', JSON.stringify(uploadedFile) );
                        return uploadedFile;
                    });
    ...
    async handleEditCoordinatorService(event) {
       ...
       // console.log('👉 Current providerOptions:', JSON.stringify(this.providerOptions))
        ...
        this.isAllCoordinatorSelected =this.coordinatorCatalogue.length > 0 && this.coordinatorCatalogue.every(row => row.selected);
                ...
                this.coordinatorCatalogue.map(row =>
                    row.id === recordId
                        ? {
                           ...
                           amount: this.oldCoordinatorAmountValue
                        }
                        : row
                );
...
// }
                ...
                this.ndisCatalogue.map(row =>
                    ...
                    row.id === recordId
                        ...
                        ? {
                            ...row,
                            isEditingAmount: false,
                           // amount: parseFloat( this.oldServiceRowAmountValue).toFixed(2)
                           amount: this.oldServiceRowAmountValue
                        }
    ...
    get isCoordinatorServiceTypeDisabled() {
        return this.isCoordinatorEditMode;
    }
   ...
   this.tryLoadSupportCoordinatorCatalogue();
    ...
    handleCoordinatorCatalogueSelect(event) {
        ...
        this.coordinatorCatalogue = this.coordinatorCatalogue.map(r =>
            ...
            this.isAllCoordinatorSelected = this.coordinatorCatalogue.length > 0 &&
           this.coordinatorCatalogue.every(r => r.selected);
    }

    handleSelectAllCoordinatorCatalogue(event) {

    const checked = event.target.checked;

    this.isAllCoordinatorSelected = checked;
    ...
    handleAddCoordinatorService() {
        ...
        const selectedItems =
            this.coordinatorCatalogue.filter(r => r.selected === true) //manendra added for unselected record update in edit mode
...
handleAttachmentSave(event) {
    ...
    console.log(
        'supportClientAttachments BEFORE =>',
        JSON.stringify(this.supportClientAttachments)
    );

    if (!Array.isArray(this.supportClientAttachments)) {

        console.log(
            'Initializing supportClientAttachments'
        );

        this.supportClientAttachments = [];
    }

    this.supportClientAttachments.push(event.detail);

    console.log(
        'supportClientAttachments AFTER =>',
        JSON.stringify(this.supportClientAttachments)
    );
...
handleFolderSave(event) {
    ...
    console.log(
        'ALL FOLDERS =>',
        JSON.stringify(this.supportClientFolders)
    );
}

handleAttachmentDelete(event) {
    console.log('DELETE IDS RECEIVED =>',JSON.stringify(event.detail));
    this.deletedAttachmentIds = event.detail || [];
}

handleTempAttachmentDelete(event) {

    const tempId = event.detail;

    console.log(
        'TEMP DELETE RECEIVED =>',
        tempId
    );
    ...
    this.supportClientAttachments =
        (this.supportClientAttachments || [])
        .filter(
            att => att.tempId !== tempId
        );

    console.log(
        'ATTACHMENTS AFTER TEMP DELETE =>',
        JSON.stringify(
            this.supportClientAttachments
        )
    );
...
handleAttachmentEdit(event){
  ...
  handleAboutParticipantChange(event) {
    try {
      this.aboutParticipant = event.target.value;
      this.showClearIcon = this.aboutParticipant.length > 0;
    } catch (error) {
...
//           this.aboutParticipant += Array.from(event.results)
...
//   }
  ...
  clearText() {
    ...
    handleFocus() {
        try {
            this.showMuteIcon = true;
            const speechComp = this.template.querySelector('c-speech-to-text');
            if (speechComp) {
    ...
    handleTranscript(event) {
        this.aboutParticipant = (this.aboutParticipant || '') + event.detail.text;
    }
    handleClear() {
        this.aboutParticipant = '';
    }  
