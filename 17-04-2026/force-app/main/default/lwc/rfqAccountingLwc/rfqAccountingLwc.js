...
export default class RfqAccountingLwc extends LightningElement {
   ...
   //@track selectedRowIdTax;
    ...
    return (value || '')
     ...
     //global search logic 
handleRfqSearch(event) {
    const searchValue = (event.target.value || '').trim().toLowerCase();
    this.globalSearchValue = searchValue;

    const searchKey= this.clean(searchValue);
    // Start from all records
    ...
    if (searchValue) {
        result = result.filter((rec) => {
            ...
            const title = rec.dynamicTitle ? rec.dynamicTitle.toLowerCase() : '';
   ...
   handleSelectionTax(event) {
    ...
    wiredInvoiceData(result) {
        ...
        if (data) {
            this.records = data.map(invoice => {
                ...
                return {
                   ...
                   // invoiceNumber: invoice.Invoice_Number__c,
                    ...
                    dynamicTitle:invoice.Invoice_No__c?.startsWith('INV-') ? 'RFQ Sales Invoice' :
            ...
            //added by vamshi to implement Search
 ...
 */
    ...
    handleCloseModal(event){
     ...
     handleGetEmailBody() {
        ...
        .then(result => {
            console.log('Result from Apex in sendEmailforInvoiceforSales :', result);
