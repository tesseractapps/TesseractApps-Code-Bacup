...
export default class PerformanceManagementLwc extends LightningElement {
   ...
   // { label: '2023-2024', value: '2023-2024' },
    ...
    connectedCallback(){
        ...
        console.log('storedFacilityId in wiredStaffList'+storedFacilityId);
        console.log('storedFacilityLabel IN wiredStaffList'+storedFacilityLabel);
        if (!this.facilityValue) {
            this.facilityValue = []; // Ensure it's an array
        }

        if (storedFacilityId && !this.facilityValue.includes(storedFacilityId)) {
            this.facilityValue.push(storedFacilityId);
            //this.SelectedComboBoxFacility = storedFacilityId;
            console.log('facilityValue after: ' + JSON.stringify(this.facilityValue));
           this.fetchRoles();
        }
   ...
   // Wire to get Staff data from Apex
       ...
       // console.log('orgid  in staff list'+this.orgid);
        if (data) {
           // console.log('data  IN  getStaffList'+  JSON.stringify(data));
            ...
            console.log('storedFacilityId in wiredStaffList'+storedFacilityId);
            console.log('storedFacilityLabel IN wiredStaffList'+storedFacilityLabel);
            console.log('data length in wiredStaffList '+ data.length);
        ...
        //     });
           ...
           // console.log('filteredStaff: ' + JSON.stringify(filteredStaff));
            ...
            //  console.log('facilityValue: ' + JSON.stringify(facilityValue));
            if (!this.facilityValue) {
              this.facilityValue = []; // Ensure it's an array
            }
    ...
    mapPerformanceData(data) {
        ...
        return data.map((performance) => {
           ...
           // const roles = performance.Staff__r?.StaffRoles__r || [];
            ...
            return {
               ...
               //fullName: performance.Staff__r?.Display_Nickname__c ?? '',
                ...
                //     : '',
                staffRole: performance.Role__c
                        ? performance.Role__c.split(';').join(', ')
                        : ''
            };
        });
    }
    ...
    wiredGoals( result) {
       console.log('performanceId in getGoalsByPerformanceId:', this.performanceId);
         console.log('result in  getGoalsByPerformanceId: ', JSON.stringify(result));
    ...
    fetchStaffDetails(staffId) {
        ...
        getStaffReviewerDetails({ staffOrReviewerId: staffId, isReviewer: false })
            .then((result) => {
                ...
                this.facilityId = result.staffDetails.Facility__c || ''; 
               ...
               const roles = result.staffRoles || [];

                if (roles.length > 0) {
                    this.staffRole = roles.map(r => r.RoleName__c).join(', ');
                } else {
                    this.staffRole = '';
                }
    ...
    updateLibraryGoalOptions() {
        ...
        console.log('facility in update library goal:' + this.facilityId);
        // Assuming you fetch the library goal list based on levelId
        getLibraryGoals({ levelId: this.levelId, staffRole: this.staffRole, facilityId: this.facilityId, orgid: this.orgid})
            .then((result) => {
    ...
    handleStaffView(event){
        ...
        this.staffDetailedView = true; 
            if (this.performanceId) {
                ...
                this.homeflag = false
                console.log('performanceId  IN handleStaffView: '+ this.performanceId);
                setTimeout(() => {
                    refreshApex(this.wiredGoalsResult);
                   }, 0);    
            } else {
    ...
    handleSaveGoal(event) {
        ...
        if (goalToSave) {
            ...
            if ((goalToSave.Reviewer_Comments__c || goalToSave.Reviewer_Assigned_Weightage__c) &&
                (!goalToSave.Staff_Comments__c || !goalToSave.Staff_Assigned_Weightage__c)) {

                this.showToast(
                    'Error',
                    'Reviewer cannot update comments or weightage until Staff completes both comments and weightage.',
                    'Error'
                );
                return;
            }
    ...
    fetchRoles(){
        console.log('facilityValue in fetchRoles: ' + JSON.stringify(this.facilityValue));
        fetchBulkRoles({ facilityIDList: this.facilityValue })
            .then(facRoles => {
                console.log("facRoles " + JSON.stringify(facRoles));
    
                const roleNames = facRoles
                    .map(r => r.Role_Name__c)      
                    .filter(r => !!r)              
                    .sort();                       

            // Clear activeSections
                this.activeSections = [];

                // Build checkbox options and fill active sections
                this.OrgNisationRoles = roleNames.map(role => {
                    this.activeSections.push(role);
                    return {
                        label: role,
                        value: role
                    };
                ...
                console.log(" this.OrgNisationRoles  in fetchRoles " + JSON.stringify(this.OrgNisationRoles));
    
                // Removed duplicate initialization - keeping only one instance
                if (!this.facilityValue) {
                    this.facilityValue = [];
                }
