...
export default class IssuesSearchComponent extends NavigationMixin(LightningElement) {
   ...
   // @track UploadFile = 'Upload CSV File';
    ...
    @track userFName;
    @track userLName;
    @track activeSections = ['Reporter Details', 'Incident Details', 'Action Taken'];
        ...
        organizationDetails().then(response => {
          ...
          //  this.orgId = response.listofPriceBook.Id;
           ...
           // console.log('org roles '+ JSON.stringify(this.OrgNisationRoles));
            ...
            if(this.orgId != null){
                fetchClient({recordId: this.orgId, firstname: null, lastname: null}).then(response=>{
        ...
        getStaffByEmail({ email: this.userEmail }).then(response => {
           // console.log('Response is >> ' + JSON.stringify(response)); 
            ...
            if (response && response.length > 0 && response[0].Role__c) {
                ...
                this.rolesArray = userData.Role__c ? userData.Role__c.split(';'): [];
               // this.role = response.Role__c ? response.Role__c.split(';') : [];
                this.role = this.rolesArray[0];  // Assuming you want to select the first role as default
        ...
        searchIssues({listOfsearchString: JSON.stringify(listOfsearchString), typeofIssue:this.typeOfIssue})
            .then(response=>{
                ...
                response.forEach(rec=>{
                    ...
                    this.JSONData[rec.Id]={"typeOfIssue":rec.Type_of_Issue__c,"firstName":rec.First_Name__c,"others":rec.Allegation_description__c,"lastName":rec.Last_Name__c,
       ...
       // console.log(event.target.label);
        ...
        inp.forEach(function(element){
            ...
            if(element.name=="searchID"){
               ...
               // console.log('listofserachstring>>>'+this.value);
            } 
            else if(this.value != null){
                listOfsearchString.push(this.value);
               // console.log('listofserachstring>>>'+this.value);
            } 
            else if(this.valuePriority != null){
                listOfsearchString.push(this.valuePriority);
            }
            /* else if(element.name=="submittedBy") status
   ...
   /*  handleSerachID(event){
        this.searchID = event.target.value;
        console.log('serach Id>>'+this.searchID);
    } */
    ...
    handleedit(event) {
        ...
        this.UIincidentDateTime=this.formatdate(this.incidentDate)+ this.formattime(this.incidentTime);   
        this.allegedDateTime=this.formatdate(this.allegedDate)+ this.formattime(this.allegedTime);     
        ...
        console.log('participant value',this.partcipantValue);
  ...
  /*   fetchIssues(){        
    ...
    handleChange(event) {
       ...
       // console.log('onchange status>>'+this.value);
    }
        ...
        */ 
        ...
        if (field === 'Date') {
           ...
           // console.log(this.incidentDate);
            ...
            this.incidentTime = event.target.value;
  ...
  /*   handleRadioChange(event) {
    ...
    handleFinalise(event) { 
        try{ 
            if(!this.recordId){     
            saveCaseRecord({ organisationName: this.orgId, firstName: this.userFName, others:this.others, lastName: this.userLName, clientId: this.client, facilityId: this.facilityName, role:this.role, email:this.email, phone: this.phone, status: this.status,
                ...
                participantValue: this.partcipantValue, subContractorValue: this.subContractor,selectedParticipant: this.client1,selectedFacility: this.facilityName1,selectedStaff: this.staffName,selectedEmployee:this.staffName1 ,mostReleventOthers:this.mostRelevantOtherValue}).then((response) => {                   
                ...
                */
                updateCaseRecord({ recordId: this.recordId,organisationName: this.orgId, firstName: this.userLName, others:this.others, lastName: this.userLName, clientId: this.client, facilityId: this.facilityName, role:this.role, email:this.email, phone: this.phone, status: this.status,
                    priorityValue: this.priorityValue, raisedByValue: this.raisedByValue, issueTypeValue: this.issueTypeValue, allegedDate: this.allegedDate, allegedTime: this.allegedTime, incidentDate: this.incidentDate, incidentTime: this.incidentTime, description: this.description,
                    reportToPolice: this.reportedToPolice, regardingtheIssue: this.clientConcerns, policeComments: this.policeComments, actionsTaken: this.actionsTaken, additionalInfo: this.additionalInfo, futureActions: this.futureActions,
                    participantValue: this.partcipantValue, subContractorValue: this.subContractor,selectedParticipant: this.client1,selectedFacility: this.facilityName1,selectedStaff: this.staffName,selectedEmployee:this.staffName1 ,mostReleventOthers:this.mostRelevantOtherValue}).then((response) => {                         
                        if(this.fileName.length > 0){
    ...
    generatePdf(caseRecord) {
       ...
       if(caseRecord.Reported_to_the_Police__c === 'Yes'){
        ...
        addLabelAndText('What specific actions have been taken to ensure the health, safety and wellbeing of the Participant’s involved?', stripHtml(caseRecord.Description1__c) || 'N/A');
