...
//                     // Fallback to existing cell.field.value (preserves signatures, uploads, rich text).
...
//             // 🔥 SIGNATURE GUARD: Signature value is base64, not a DOM input value.
//             // Never overwrite it from the DOM — no <input> exists for signatures.
...
//                         // 🔥 SIGNATURE GUARD inside table block cells
...
//                         if (!isTCellSignature) {
//                             const tInput = this.template.querySelector(
//                                 `[data-id="${tCell.id}"]`
//                             );
//                             if (tInput) {
//                                 tCell.field.value = tInput.value;
//                             }
//                         }
//                     });
//                 });
...
//                         }
//                     } else {
//                         cell.field.value = input.type === 'checkbox' ? !!input.checked : input.value;
//                     }
//                 }
//             }
...
//       // ─── Signature ───────────────────────────────────────
//       // The template cell may only have isSignature: true (no dataType).
//       // Without this, decorateRowsForUI won't flag it as a signature cell
//       // and the base64 value won't render.
//       const isSignatureCell =
...
//       if (isSignatureCell) {
//         cell.field.isSignature = true;
//         cell.field.dataType    = 'Signature';   // normalize so decorateRowsForUI always detects it
//         // value already copied above — guard against accidental null wipe
...
handleInputChange(event) {
  ...
  // Rich text is now handled ONLY by blur event in initializeQuillEditors
  if (isRichText) {
    console.log(`⏭️  [handleInputChange] Skipping Rich Text cell ${cellId} - handled by Quill blur`);
    return;
  }
...
//     // ✅ clone pristine layout — never mutates originalTemplate
...
//             // =====================================================
//             // 🔥 TABLE BLOCK — sync innerCells values from pagedRows
//             // =====================================================
//             if (
//                 cell.field.dataType === "Table Block" ||
//                 cell.field.isTableBlock
//             ) {
//                 // Source of truth: pagedRows has the live typed values
//                 let pagedCell = null;

//                 // Search ALL pages — table block could be on any page
//                 for (const page of (this.pagedRows || [])) {
//                     for (const pagedRow of page) {
//                         const found = pagedRow.cells?.find(
//                             c => String(c.id) === String(uiCell.id)
//                         );
//                         if (found) { pagedCell = found; break; }
//                     }
//                     if (pagedCell) break;
//                 }
...
//                 if (!cell.field.tableConfig) cell.field.tableConfig = {};

//                 // ── Sync innerCells values ──────────────────────────
...
//                         // ✅ FIX: dropdown label fields — preserve in payload
...
//                 // ── Also sync renderRows for display/read purposes ──
...
//                         cell.field.tableConfig.renderRows[rIdx] = { ...sRow };
//                     }
//                     (sRow.renderCells || []).forEach((sRc, cIdx) => {
//                         const tRc = cell.field.tableConfig.renderRows[rIdx]
//                             ?.renderCells?.[cIdx];
//                         if (tRc && sRc.field?.value !== undefined) {
//                             tRc.field = {
//                                 ...(tRc.field || {}),
//                                 value:         sRc.field.value,
//                                 selectedLabel: sRc.field.selectedLabel ?? tRc.field?.selectedLabel ?? "", // ✅
//                                 displayValue:  sRc.field.displayValue  ?? tRc.field?.displayValue  ?? ""  // ✅
//                             };
//                         }
//                     });
//                 });
...
//                             uiCell.field.options.filter(o => o.isSelected);
...
//         const maxWidth = 60;   // max width inside cell
//         const maxHeight = 25;  // max height allowed
...
//   // ✅ SAFE REFRESH (only if method exists)
//   if (typeof this.updateStepVisibleRows === 'function') {
...
//   if (!this.activeSignatureCellId) {
//     console.warn("⚠️ No activeSignatureCellId");
//     console.groupEnd();
//     return;
//   }

//   if (!Array.isArray(this.pagedRows) || !this.pagedRows.length) {
//     console.warn("⚠️ pagedRows not initialized");
//     console.groupEnd();
//     return;
//   }
...
//   const currentPage = this.pagedRows[this.currentPageIndex] || [];
...
//         console.log("✅ Signature applied to:", cell.id);
...
//   if (!updated) {
//     console.warn("⚠️ No matching cell found");
//   }

//   // 🔥 Force LWC reactivity safely
//   this.pagedRows = [...this.pagedRows];
...
//     const updatedPages = this.pagedRows.map((page, pIdx) => {
//         if (pIdx !== pageIndex) {
...
//     this.pagedRows = updatedPages; // 🔥 new reference triggers LWC reactivity
...
//     let tableRowsMatchType = "none"; // track which case matched
...
//                 // Step 2a: Update renderRows → safeRenderRows (template binding)
...
//                 // Step 2b: Sync innerCells (prevents rebuild from wiping the value)
...
//                 console.groupEnd(); // CASE B Table Block
...
//     // Verify pagedRows final value
//     const finalPagedCell = (this.pagedRows[pageIndex] || [])
...
//     // Verify tableRows CASE A final value
...
//     // Verify tableRows CASE B final value (inside Table Block renderRows)
...
//     // Full deep dump (structured — collapsible in DevTools)
