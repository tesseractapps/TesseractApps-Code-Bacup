  ...
  async handleEditShiftScreen(event) {
    ...
    getAddShiftDataById({ shiftId: this.shiftStaffId }).then(async (result) => {
     ...
     // this.splitShift = result.shiftwithstaffdata.Split_Shifts__c;
      ...
      if(result.shiftwithstaffdata.New_Split_Shift__c || result.shiftwithstaffdata.Split_Shifts__c || result.shiftwithstaffdata.Status__c == "Completed" || result.shiftwithstaffdata.Status__c == "InProgress"){
          this.disableSplitShift=true;
      } 
       
       if(result.shiftwithstaffdata.Broken_Shift__c || result.shiftwithstaffdata.Status__c == "Completed" || result.shiftwithstaffdata.Status__c == "InProgress"){
      ...
      if(result.shiftwithstaffdata.Broken_Extended_Duration__c >0){
        ...
        if (shiftType == "Custom") {
         ...
         if( result.shiftwithstaffdata.Group_Shifts__c || result.shiftwithstaffdata.Status__c == "Completed" || result.shiftwithstaffdata.Status__c == "InProgress"){
          this.disableGroupShift=true;
         } 
      const rows = [];
 ...
 handleShiftTemplateOptionChange(event) {
  ...
  const spanError = this.validateTemplateSpan();
    ...
    const payLoadForTempalte = {
        ...
        multiFacility: this.facilityValue ? this.facilityValue.join(',') : null
    };
...
validateTemplateSpan() {
   ...
   /*  loadTemplates() {
        getTemplates({facilityIds: this.facilityValue})
            .then(result => {
                  console.log('roster Tempaltes '+JSON.stringify(result));
