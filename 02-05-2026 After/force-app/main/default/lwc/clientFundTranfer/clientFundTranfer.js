...
async loadFundSplits(fundTrackerId, retry = true) {//manendra modifyed old method is alredy is in commented form above
  try {
    ...
    //     // baseAmount:r.Allocated__c,
    //     //  originalAmount: i === 0
    //     //     ? (r.Allocated__c || 0)
    //     //     : Math.max(0, (r.Allocated__c || 0) - (result[i - 1].Carry_Forward__c || 0)),

    //     // baseAmount: i === 0
    //     //     ? (r.Allocated__c || 0)
    //     //     : Math.max(0, (r.Allocated__c || 0) - (result[i - 1].Carry_Forward__c || 0)),
    //      allocatedAmount: r.Allocated__c,
    //      originalCarryForward: r.Carry_Forward__c || 0,

    // // 🔥 KEEP THIS (correct base logic)
    // baseAmount: i === 0
    //     ? r.Allocated__c
    //     : (r.Allocated__c - (result[i - 1].Carry_Forward__c || 0)),
    ...
    result.map((r, i) => {
        ...
        const allocated = r.Allocated__c || 0;
        const prevCarry = i > 0 ? (result[i - 1].Carry_Forward__c || 0) : 0;

        const base = i === 0 ? allocated : allocated - prevCarry;
        ...
        return {
            ...
            //amount: allocated,
            amount: this.formatNumber(allocated),
            allocatedAmount: allocated,
            originalBaseAmount: base,
            baseAmount: base,
...
autoRecalculatePeriods() {
  console.log('autoRecalculatePeriods called  ');
    ...
    for (let i = 0; i < num; i++) {
        ...
        // Preserve threshold and status from existing period at same index if available
    ...
    previewSpentForPeriods({ clientId: this.clientId, fundTrackerId: this.recordId, periods: previewPayload })
        .then(result => {
            ...
            this.generatedPeriods = periods.map((p, index) => {
                ...
                // return {
                //     ...p,
                //     amount: allocatedWithCarry.toFixed(2),
                //     spentAmount: spent || 0,
                //     carryForward
                // };
                 return {
...
//     const totalSpent     = periods.reduce((sum, p) => sum + (parseFloat(p.spentAmount) || 0), 0);
//     const availableFunds = Number((approvedTotal - totalSpent).toFixed(2));

//     // ── Guard 1: cannot go below own spent ──
//     const periodSpent = parseFloat(period.spentAmount) || 0;
//     if (newValue < periodSpent) {
//         this.dispatchEvent(new ShowToastEvent({
//             title: 'Invalid Amount',
//             message: `Cannot be less than spent ($${periodSpent.toFixed(2)}). Reverted.`,
//             variant: 'error'
//         }));
//         event.target.value = period.allocatedAmount;
//         return;
//     }

//     // ── Guard 2: max allowed = approvedTotal - spent of periods BEFORE this one ──
//     // Reasoning: periods before are already committed (completed with spend)
//     // This period + all after must fit within what remains after prior spend
//     const spentBeforeIndex = periods.reduce((sum, p, i) => {
//         if (i < index) return sum + (parseFloat(p.spentAmount) || 0);
//         return sum;
//     }, 0);
...
//             message: `Maximum allowed for this period is $${maxAllowedForPeriod.toFixed(2)} (approved $${approvedTotal.toFixed(2)} minus prior spent $${spentBeforeIndex.toFixed(2)}). Reverted.`,
//             variant: 'error'
//         }));
//         event.target.value = period.allocatedAmount;
//         return;
...
//     const oldValue = parseFloat(period.allocatedAmount) || 0;
//     if (oldValue === newValue) return;

//     // ── Run rebalance on temp copy — validate after cascade ──
//     const tempPeriods = periods.map(p => ({ ...p }));
//     tempPeriods[index] = { ...tempPeriods[index], allocatedAmount: newValue };

//     const success = this.rebalanceAllocationsFromIndex(index, tempPeriods, oldValue, newValue);
...
//     // ── Guard 3: verify totalBase = approvedTotal ──
//     // Sum of all clean bases must always equal approvedTotal
//     // This is the invariant that keeps funds balanced
//     const totalBase = tempPeriods.reduce((sum, p) =>
//         sum + (parseFloat(p.baseAmount) || 0), 0
//     );

//     console.log(`=== GUARD3 | totalBase:${totalBase} | approvedTotal:${approvedTotal}`);

//     if (Math.abs(totalBase - approvedTotal) > 0.01) {
//         this.dispatchEvent(new ShowToastEvent({
//             title: 'Fund Balance Error',
//             message: `Total base ($${totalBase.toFixed(2)}) does not match approved ($${approvedTotal.toFixed(2)}). Reverted.`,
//             variant: 'error'
//         }));
//         event.target.value = period.allocatedAmount;
//         return;
...
//     // ── Guard 4: active/future (allocated + spent) must equal approvedTotal ──
//     // Active/future periods hold all unspent funds
//     // Their (allocated + spent) = approvedTotal confirms no leakage
//     const activeFutureAllocatedPlusSpent = tempPeriods.reduce((sum, p) => {
//         if (p.status !== 'Completed') {
//             return sum + (parseFloat(p.amount) || 0) + (parseFloat(p.spentAmount) || 0);
//         }
//         return sum;
//     }, 0);

//     console.log(`=== GUARD4 | activeFuture(allocated+spent):${activeFutureAllocatedPlusSpent} | approvedTotal:${approvedTotal}`);

//     // ── All guards passed — apply ──
//     event.target.value = tempPeriods[index].amount;
//     this.generatedPeriods = [...tempPeriods];
//     this.calculateTotal();
// }
...
//     console.log(`=== REBALANCE | index:${changedIndex} | old:${oldValue} | new:${newValue} | approvedTotal:${approvedTotal} | availableFunds:${availableFunds}`);

//     // ══════════════════════════════════════════
//     // STEP 1: Strip carry from every period
//     // base = allocatedAmount - carry received from previous completed period
//     // This gives the clean user-intended base for each period
//     //
//     // Example: P1=2000 carry=2000, P2=4000
//     //   P2 base = 4000 - 2000 = 2000 (its own share, not inflated by carry)
//     // ══════════════════════════════════════════
//     let prevCarry = 0;
//     for (let i = 0; i < periods.length; i++) {
//         const p         = periods[i];
//         const allocated = i === changedIndex
//             ? newValue
//             : (parseFloat(p.allocatedAmount) || 0);

//         const base = Number((allocated - prevCarry).toFixed(2));

//         periods[i].baseAmount = base;

//         if (p.status === 'Completed') {
//             const spent = parseFloat(p.spentAmount) || 0;
//             prevCarry   = Math.max(0, allocated - spent);
//         } else {
//             prevCarry = 0;
//         }
//     }
...
//     periods.forEach((p, i) => console.log(`P${i+1} | allocated:${p.allocatedAmount} | base:${p.baseAmount}`));

//     // ══════════════════════════════════════════
//     // STEP 2: Carry cascade
//     // Every period: amount = ownBase + incomingCarry
//     // Completed: carry = amount - spent → flows to next
//     // Active/Future: carry stops here
//     // ══════════════════════════════════════════
//     let runningCarry = 0;
//     console.log('=== CASCADE ===');
...
//         const p     = periods[i];
//         const spent = parseFloat(p.spentAmount) || 0;
//         const base  = parseFloat(p.baseAmount) || 0;
//         const total = base + runningCarry;
//         let   carry = 0;

//         console.log(`P${i+1} | base:${base} | carryIn:${runningCarry} | total:${total} | spent:${spent} | status:${p.status}`);

//         if (p.status === 'Completed') {
//             carry = total - spent;
//             if (carry < 0) {
//                 this.dispatchEvent(new ShowToastEvent({
//                     title: 'Invalid — Below Spent',
//                     message: `Period ${i+1}: $${total.toFixed(2)} allocated is less than $${spent.toFixed(2)} spent. Reverted.`,
//                     variant: 'error'
//                 }));
//                 return false;
//             }
//             runningCarry = carry;
//             console.log(`P${i+1} COMPLETED → total:${total} | carry:${carry}`);
//         } else {
//             carry        = 0;
//             runningCarry = 0;
//             console.log(`P${i+1} ACTIVE/FUTURE → total:${total}`);
//         }

//         periods[i] = {
//             ...p,
//             amount:          Number(total.toFixed(2)),
//             allocatedAmount: Number(total.toFixed(2)),
//             baseAmount:      base,
//             carryForward:    Number(carry.toFixed(2))
//         };
//     }
...
//     // STEP 3: Verify
//     // ══════════════════════════════════════════
//     const totalBase = periods.reduce((sum, p) =>
//         sum + (parseFloat(p.baseAmount) || 0), 0
//     );
//     const activeFutureTotal = periods.reduce((sum, p) =>
//         p.status !== 'Completed' ? sum + (parseFloat(p.amount) || 0) : sum, 0
//     );

//     console.log(`=== VERIFY | totalBase:${totalBase} | approvedTotal:${approvedTotal} | activeFutureTotal:${activeFutureTotal} | availableFunds:${availableFunds}`);
//     console.log(`totalBase=approved: ${Math.abs(totalBase - approvedTotal) < 0.01 ? '✅' : '❌'}`);
//     console.log(`activeFuture=available: ${Math.abs(activeFutureTotal - availableFunds) < 0.01 ? '✅' : '❌'}`);
...
//     // ── Guard 2: max allowed = approvedTotal - spent of periods before this one ──
//     const spentBeforeIndex = periods.reduce((sum, p, i) =>
//         i < index ? sum + (parseFloat(p.spentAmount) || 0) : sum, 0
//     );
//     const maxAllowedForPeriod = Number((approvedTotal - spentBeforeIndex).toFixed(2));
...
//         this.dispatchEvent(new ShowToastEvent({
//             title: 'Exceeds Maximum Allowed',
//             message: `Maximum allowed is $${maxAllowedForPeriod.toFixed(2)} (approved $${approvedTotal.toFixed(2)} minus prior spent $${spentBeforeIndex.toFixed(2)}). Reverted.`,
//             variant: 'error'
//         }));
//         event.target.value = period.allocatedAmount;
//         return;
...
//     // ── Guard 3: totalBase must equal approvedTotal ──
//     const totalBase = tempPeriods.reduce((sum, p) =>
//         sum + (parseFloat(p.baseAmount) || 0), 0
//     );
//     if (Math.abs(totalBase - approvedTotal) > 0.01) {
//         this.dispatchEvent(new ShowToastEvent({
//             title: 'Fund Balance Error',
//             message: `Total ($${totalBase.toFixed(2)}) does not match approved ($${approvedTotal.toFixed(2)}). Reverted.`,
//             variant: 'error'
//         }));
//         event.target.value = period.allocatedAmount;
//         return;
...
//     // ── Guard 4: active/future total must equal availableFunds ──
//     const activeFutureTotal = tempPeriods.reduce((sum, p) =>
//         p.status !== 'Completed' ? sum + (parseFloat(p.amount) || 0) : sum, 0
//     );
//     if (Math.abs(activeFutureTotal - availableFunds) > 0.01) {
//         this.dispatchEvent(new ShowToastEvent({
//             title: 'Fund Balance Error',
//             message: `Active/future total ($${activeFutureTotal.toFixed(2)}) must equal available ($${availableFunds.toFixed(2)}). Reverted.`,
//             variant: 'error'
//         }));
//         event.target.value = period.allocatedAmount;
//         return;
...
//     console.log(`=== REBALANCE | index:${changedIndex} | old:${oldValue} | new:${newValue} | diff:${diff} | approvedTotal:${approvedTotal} | availableFunds:${availableFunds}`);

//     // ══════════════════════════════════════════
//     // STEP 1: Strip carry — get clean base for every period
//     // base = allocatedAmount - carry received from previous completed period
//     // ══════════════════════════════════════════
//     let prevCarry = 0;
//     for (let i = 0; i < periods.length; i++) {
...
//         periods[i].baseAmount = Number((allocated - prevCarry).toFixed(2));
...
//     // ══════════════════════════════════════════
//     // STEP 2: Balance the bases
//     // After stripping carry, totalBase may not equal approvedTotal
//     // because user changed one period's value.
//     // Find the imbalance and absorb it into the nearest editable period
//     // after changedIndex.
//     //
//     // Rule:
//     //   - If changedIndex period is Active/Future → absorb diff in NEXT period
//     //   - If changedIndex period is Completed → absorb diff in FIRST Active/Future period
//     //     (completed periods carry forward so their diff flows naturally)
//     // ══════════════════════════════════════════
//     const currentTotalBase = periods.reduce((sum, p) =>
//         sum + (parseFloat(p.baseAmount) || 0), 0
//     );
//     const imbalance = Number((approvedTotal - currentTotalBase).toFixed(2));

//     console.log(`=== IMBALANCE CHECK | currentTotalBase:${currentTotalBase} | approvedTotal:${approvedTotal} | imbalance:${imbalance}`);

//     if (Math.abs(imbalance) > 0.01) {
//         // Find best period to absorb imbalance
//         // Priority: next period after changedIndex that can absorb it
//         let absorbed = false;

//         // First try: next period immediately after changedIndex
//         if (changedIndex + 1 < periods.length) {
//             const nextIdx     = changedIndex + 1;
//             const nextBase    = parseFloat(periods[nextIdx].baseAmount) || 0;
//             const newNextBase = Number((nextBase + imbalance).toFixed(2));

//             if (newNextBase >= 0) {
//                 console.log(`=== ABSORB IN P${nextIdx+1} | base:${nextBase} → ${newNextBase}`);
//                 periods[nextIdx].baseAmount = newNextBase;
//                 absorbed = true;
//             }
//         }

//         // Second try: spread across ALL periods after changedIndex
//         if (!absorbed) {
//             let remaining = imbalance;
//             for (let i = changedIndex + 1; i < periods.length; i++) {
//                 const base = parseFloat(periods[i].baseAmount) || 0;
//                 if (remaining > 0) {
//                     periods[i].baseAmount = Number((base + remaining).toFixed(2));
//                     remaining = 0;
//                     absorbed  = true;
//                 } else if (remaining < 0 && base > 0) {
//                     const drain = Math.min(base, Math.abs(remaining));
//                     periods[i].baseAmount = Number((base - drain).toFixed(2));
//                     remaining  = Number((remaining + drain).toFixed(2));
//                     if (Math.abs(remaining) < 0.01) { absorbed = true; break; }
//                 }
//             }
//         }

//         // Third try: period before changedIndex (last resort)
//         if (!absorbed || Math.abs(periods.reduce((s, p) => s + (parseFloat(p.baseAmount) || 0), 0) - approvedTotal) > 0.01) {
//             let remaining = Number((approvedTotal - periods.reduce((s, p) => s + (parseFloat(p.baseAmount) || 0), 0)).toFixed(2));
...
//         // Final check — if still imbalanced, block
//         const finalBase = periods.reduce((sum, p) => sum + (parseFloat(p.baseAmount) || 0), 0);
//         if (Math.abs(finalBase - approvedTotal) > 0.01) {
//             this.dispatchEvent(new ShowToastEvent({
//                 title: 'Exceeds Available Funds',
//                 message: `Cannot absorb this change across periods. Reverted.`,
//                 variant: 'error'
//             }));
//             return false;
//         }
...
//     console.log('=== BASES AFTER BALANCE ===');
//     periods.forEach((p, i) => console.log(`P${i+1} | base:${p.baseAmount}`));

//     // ══════════════════════════════════════════
//     // STEP 3: Carry cascade
//     // ══════════════════════════════════════════
//     let runningCarry = 0;
//     console.log('=== CASCADE ===');
...
handleAmountBlur(event) {
    const index    = parseInt(event.target.dataset.index, 10);
    //const newValue = parseFloat(event.target.value) || 0;
    const newValue = this.parseNumber(event.target.value) || 0;
    let   periods  = [...this.generatedPeriods];
    const period   = periods[index];
    ...
    if (newValue > maxAllowedForPeriod) {
        this.dispatchEvent(new ShowToastEvent({
            title: 'Error',
            message: `Exceeds Maximum Allowed. Reverted.`,
            variant: 'error'
        }));
        event.target.value = period.allocatedAmount;
        return;
    ...
    if (Math.abs(totalBase - approvedTotal) > 0.01) {
        this.dispatchEvent(new ShowToastEvent({
            title: 'Fund Balance Error',
           // message: `Total base ($${totalBase.toFixed(2)}) does not match approved ($${approvedTotal.toFixed(2)}). Reverted.`,
            message: ` Allocated amount exceeds Approved  budget. Please adjust the amount `,
    ...
    // ── Guard 4: active/future (allocated - spent) must equal availableFunds ──
    // const activeFutureAllocated = tempPeriods.reduce((sum, p) =>
    //     p.status !== 'Completed' ? sum + (parseFloat(p.amount) || 0) : sum, 0
    // );
    const activeFutureAllocated = tempPeriods.reduce((sum, p) =>
      p.status !== 'Completed' ? sum + (parseFloat(p.allocatedAmount) || 0) : sum, 0
  );
    const activeFutureSpent = tempPeriods.reduce((sum, p) =>
        p.status !== 'Completed' ? sum + (parseFloat(p.spentAmount) || 0) : sum, 0
    );
    const activeFutureNet = Number((activeFutureAllocated - activeFutureSpent).toFixed(2));

    console.log(`=== GUARD4 | activeFutureAllocated:${activeFutureAllocated} | activeFutureSpent:${activeFutureSpent} | activeFutureNet:${activeFutureNet} | availableFunds:${availableFunds}`);

    if (Math.abs(activeFutureNet - availableFunds) > 0.01) {
        this.dispatchEvent(new ShowToastEvent({
            title: 'Fund Balance Error',
           // message: `Active/future net ($${activeFutureNet.toFixed(2)}) must equal available funds ($${availableFunds.toFixed(2)}). Reverted.`,
           message: `The remaining funds are not properly distributed. Please adjust your allocation. Reverted.`,
           variant: 'error'
        }));
        //event.target.value = period.allocatedAmount;
        event.target.value = this.formatNumber(period.allocatedAmount);
      ...
      // tempPeriods[index].amount = this.formatNumber(newValue);
      tempPeriods[index] = {
          ...tempPeriods[index],
...
rebalanceAllocationsFromIndex(changedIndex, periods, oldValue, newValue) {
    ...
    // STEP 1: Strip carry — get clean base for every period
    // base = allocatedAmount - carry received from previous completed period
    // If no completed periods, base = allocatedAmount (no carry to strip)
    // ══════════════════════════════════════════
    let prevCarry = 0;
    for (let i = 0; i < periods.length; i++) {
        const p         = periods[i];
    ...
    // After stripping carry, check if totalBase = approvedTotal
    // If not, absorb imbalance into nearest period after changedIndex
    // ══════════════════════════════════════════
    const currentTotalBase = periods.reduce((sum, p) =>
        sum + (parseFloat(p.baseAmount) || 0), 0
    );
    ...
    if (Math.abs(imbalance) > 0.01) {
        ...
        if (changedIndex + 1 < periods.length) {
            const nextIdx     = changedIndex + 1;
            const nextBase    = parseFloat(periods[nextIdx].baseAmount) || 0;
            const newNextBase = Number((nextBase + remaining).toFixed(2));
        ...
        // Second try: spread across all periods after changedIndex
        if (!absorbed || Math.abs(remaining) > 0.01) {
            for (let i = changedIndex + 1; i < periods.length; i++) {
                const base = parseFloat(periods[i].baseAmount) || 0;
                if (remaining > 0) {
                    periods[i].baseAmount = Number((base + remaining).toFixed(2));
        ...
        // Third try: periods before changedIndex (last resort)
        if (!absorbed || Math.abs(remaining) > 0.01) {
            for (let i = changedIndex - 1; i >= 0; i--) {
                const base = parseFloat(periods[i].baseAmount) || 0;
                if (remaining > 0) {
        ...
        // Final check — still imbalanced after all attempts
        const finalBase = periods.reduce((sum, p) =>
            sum + (parseFloat(p.baseAmount) || 0), 0
        );
        if (Math.abs(finalBase - approvedTotal) > 0.01) {
            this.dispatchEvent(new ShowToastEvent({
    ...
    // ══════════════════════════════════════════
    // STEP 3: Carry cascade
    // Every period: amount = base + incomingCarry
    // Completed: carry = amount - spent → flows to next
    // Active/Future: carry stops, runningCarry resets
    // ══════════════════════════════════════════
    let runningCarry = 0;
    console.log('=== CASCADE ===');
    ...
    for (let i = 0; i < periods.length; i++) {
        ...
        if (p.status === 'Completed') {
            ...
            if (carry < 0) {
                this.dispatchEvent(new ShowToastEvent({
                    title: 'Invalid — Below Spent',
                   // message: `Period ${i+1}: $${total.toFixed(2)} allocated is less than $${spent.toFixed(2)} spent. Reverted.`,
                    message: `Allocated amount cannot be less than the spent amount. Changes reverted.`,
                    variant: 'error'
                }));
                return false;
            }
        ...
        periods[i] = {
            ...
            amount:          this.formatNumber(total),        // ✅ formatted for UI display
            allocatedAmount: Number(total.toFixed(2)),         // ✅ raw number for calculations
            baseAmount:      base,
            carryForward:    Number(carry.toFixed(2)),         // ✅ raw number
            carryFormatted:  this.formatNumber(carry),         // ✅ formatted for UI display
            spentFormatted:  this.formatNumber(parseFloat(p.spentAmount) || 0) 
        };
    }
    ...
    // const activeFutureAllocated = periods.reduce((sum, p) =>
    //     p.status !== 'Completed' ? sum + (parseFloat(p.amount) || 0) : sum, 0
    // );
    const activeFutureAllocated = periods.reduce((sum, p) =>
        p.status !== 'Completed' ? sum + (parseFloat(p.allocatedAmount) || 0) : sum, 0
    );
    const activeFutureSpent = periods.reduce((sum, p) =>
        p.status !== 'Completed' ? sum + (parseFloat(p.spentAmount) || 0) : sum, 0
    );
    const activeFutureNet = Number((activeFutureAllocated - activeFutureSpent).toFixed(2));

    console.log(`=== VERIFY | totalBase:${totalBase} | approvedTotal:${approvedTotal} | activeFutureAllocated:${activeFutureAllocated} | activeFutureSpent:${activeFutureSpent} | activeFutureNet:${activeFutureNet} | availableFunds:${availableFunds}`);
    console.log(`totalBase=approved: ${Math.abs(totalBase - approvedTotal) < 0.01 ? '✅' : '❌'}`);
    console.log(`activeFutureNet=available: ${Math.abs(activeFutureNet - availableFunds) < 0.01 ? '✅' : '❌'}`);
    ...
    console.log('METHOD IS handleSaveFundSplits');
            ...
            try {
              ...
              const splitsToSave = this.generatedPeriods.map(p => ({
                ...
                Allocated__c: parseFloat(p.allocatedAmount) || 0,   //added
                Status__c: p.status,
                Name: p.label
              }));
