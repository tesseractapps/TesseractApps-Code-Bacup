...
export default class MyProfileStaffReportsLwc extends LightningElement {
    ...
    formatCSVDate(value) {
        ...
        // 👇 HANDLE dd/mm/yyyy FORMAT (IMPORTANT)
        if (value.includes('/')) {
    ...
    handleDetailedWagesCSV() { 
        ...
        this.excelDataWages.forEach(fieldsData => {
            csvContent += '"' + fieldsData.sName + '",' +
                        ...
                        // '"' + fieldsData.eDate + '",' +
                        // '"' + fieldsData.npdate + '",' +
                        this.formatCSVDate(fieldsData.sDate) + ',' +
                        this.formatCSVDate(fieldsData.eDate) + ',' +
                        this.formatCSVDate(fieldsData.npdate) + ',' +
    ...
    @track timeSheetHeader = ['Start Date', 'End Date', 'Approved by', 'Hours'];
    handleTimesheetReportCSV() {
        ...
        // Add header row
        csvContent += this.timeSheetHeader.join(',') + '\n';
       
        this.timesheetResult = [...this.weeklySummaries ]
        console.log('timesheet result >>'+JSON.stringify(this.timesheetResult));
        
        this.timesheetResult.forEach((fieldsData, index) => {
            console.log(`🔁 Looping row ${index + 1}`);
            console.log('📄 fieldsData:', JSON.stringify(fieldsData));
           
            const approverName =  fieldsData.approvers ;
                
            console.log('✅ Approver Name:', approverName);
            console.log('⏱️ Working_Hours__c:', fieldsData.totalHours);

            // csvContent += '"' + (fieldsData.startDate || '') + '",' +  
            //             '"' + (fieldsData.endDate || '') + '",' +    
            //             '"' + approverName + '",' +                      
            //             (fieldsData.totalHours != null ? fieldsData.totalHours : '') + '\n';

            csvContent += this.formatCSVDate(fieldsData.startDate) + ',' +
                            this.formatCSVDate(fieldsData.endDate) + ',' +
                            '"' + approverName + '",' +
                            (fieldsData.totalHours != null ? fieldsData.totalHours : '') + '\n';

            console.log(`✅ Row ${index + 1} added to CSV`);
        });
