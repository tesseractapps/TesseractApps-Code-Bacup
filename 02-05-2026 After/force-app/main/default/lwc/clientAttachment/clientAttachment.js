...
export default class ClientAttachment extends LightningElement {
    ...
    @track folderOptions = []; // optional (for combobox)
    ...
    loadStaffOptions() {
        fetchStaffOptions()
            .then(result => {
                ...
                .filter(staff => staff.Facility__c === this.storedFacilityId) // 👈 filter here
                .map(staff => ({
                    label: staff.NameToDisplay__c || staff.Name,
                    value: staff.Id
                }));
...
/*
loadFolders() {
    getFolders({ clientId: this.clientId })
        .then(result => {

            this.folders = result;

            // build tree
            this.buildFolderTree(result);

            // build dropdown (optional)
...
buildFolderTree(data) {
    ...
    data.forEach(f => {
        if (f.Parent_Folder__c) {
            if (map[f.Parent_Folder__c]) {
...
handleFolderClick(event) {
    ...
    this.folderTree = this.updateTree(this.folderTree, null, (n) => ({
        cssClass: 'folder-item' + (n.Id === id ? ' active' : '')
    }));

    const selected = this.folders.find(f => f.Id === id);
    this.currentFolderName = selected ? selected.Name : 'All Documents';
