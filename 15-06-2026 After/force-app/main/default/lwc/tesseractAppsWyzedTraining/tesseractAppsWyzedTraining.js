...
export default class TesseractAppsWyzedTraining extends NavigationMixin(LightningElement) {
 ...
 @track currentUser;
  ...
  get learnersDynamicClass() {
    return this.matchedStaff === true?'table-container learners-table-matched':'table-container learners-table';
  }
  ...
  // Pagination controls
    ...
    get totalPages() {
        if (!this.adminTrainingList || this.adminTrainingList.length === 0) {
            return 1;
        }
        return Math.ceil(this.adminTrainingList.length / this.pageSize);
    }
