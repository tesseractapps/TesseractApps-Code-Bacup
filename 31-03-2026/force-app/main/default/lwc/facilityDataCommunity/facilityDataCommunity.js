   ...
   handledetails(event){
    ...
    handleSelectAll(event) {
        ...
        if (checked) {
            ...
            console.log('Select All = FALSE → Mark visible as unselected');

            const visibleIds = new Set(
                this.filteredCatalogs
                    .filter(r => !r.isSelectedcheckBox)
                    .map(r => r.Id)
            );
    ...
    handleRowSelection(event) {
        ...
        // 🔥 Prepare minimal payload object
        const payloadRecord = {
            Id: recordId,
            isSelected: checked
        };
        ...
        const index = this.trackedRecords.findIndex(
            r => r.Id === recordId
        );
        ...
        if (index > -1) {
            ...
            this.trackedRecords = this.trackedRecords.map(r =>
                r.Id === recordId ? payloadRecord : r
            );

            console.log('Updated tracked record');
            ...
            console.log('Added new tracked record');
        }

        this.updateSelectAllCheckbox();
        this.cataloguePaginate();

        console.log('Tracked Records:', this.trackedRecords);
