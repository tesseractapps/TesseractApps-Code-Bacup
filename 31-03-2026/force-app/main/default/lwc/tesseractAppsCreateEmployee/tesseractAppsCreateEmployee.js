...
@track sectionIcons = {
  ...
  handleEditClick(event) {
   ...
   console.log('this.records',JSON.stringify(this.records));
    this.paginationHelper(); */ // refresh visible page
}
handleAssignClose(){
...
handleSave(event) {
  /*   const rowId = event.currentTarget.dataset.id;
    const row = this.records.find(r => r.Id === rowId);
 */
    ...
    .then(() => {
      ...
      /*   const approverName =
            this.records.find(r => r.Id === row.selectedStaffId)?.Display_Nickname__c || '—';

        this.records = this.records.map(r =>
            r.Id === rowId
                ? {
                    ...r,
                    reportsTo: approverName,
                    isEditing: false
                }
                : r
        );

        this.paginationHelper(); // 🔥 re-render current page  */
  ...
  handleFaciltyChange(event) {
    /*   praveen changes for nursing start*/
      ...
      /*  praveen changes for nursing awards end*/
       ...
       this.loadDocumentTypesForFacility(this.StaffFacility);
  }
...
//     getFacilityData()
//     .then(result => {
//         this.facilityOptions = result.map(f => ({
//             label: f.Name,
//             value: f.Id
//         }));
//     })
...
handleFacilityChange(event) {
    ...
    //     console.log('Roles selected from child:', JSON.stringify(event.detail));

    //     // values from child component
    //     this.selectedRoleValues = event.detail.values || [];
    //     this.selectedPrimaryRole = event.detail.primaryValue;

    //     // 🔹 build employment dropdown list
    //     this.selectedRoleEmploymentValues = this.roleoptionsforFacility
    //         .filter(role => this.selectedRoleValues.includes(role.value))
    //         .map(role => {
    //             return {
    //                 label: role.label,
    //                 value: role.value,
    ...
    //     /* roles coming from child combobox */
    //     let selectedValuesFormChild = event.detail.values || [];
    //       this.selectedPrimaryRole = event.detail.primary;


    //     console.log('selectedRoleValues:', JSON.stringify(this.selectedRoleValues));
    //   this.selectedRoleValues = this.roleoptionsforFacility
    //     .filter(role => selectedValuesFormChild.includes(role.displaylabel))
    //     .map(role => {
    //         return {
    //             label: role.label,
    //             value: role.value,
...
//     /* save UI values for child */
//     this.selectedRoleValueLabels = [...selectedValuesFromChild];
