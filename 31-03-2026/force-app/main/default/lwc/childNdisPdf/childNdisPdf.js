        ...
        this.currentRecordPdf = this.currentRecordPdf.map(record => {
            ...
            allocationData.forEach(item => {
                const shifts = [
                    ...
                    { name: 'Extended Broken Hours', rate: item.Broken_Extended_Rate__c, hours: item.Broken_Extended_Duration__c } ,
             ...
             /* 🔥 START: ADDED PAID & UNPAID LEAVE FROM staffpayrollData */
            ...
            if (payroll) {
                ...
                // Paid Leave
                if (payroll.Paid_Leave_Hours__c > 0) {
                    allShifts.push({
                        name: 'Paid Leave',
                        rate: payroll.Paid_Leave_Rate__c,
                        hours: payroll.Paid_Leave_Hours__c,
                        qty: 1
                    });
                }

                // Unpaid Leave (optional display)
                if (payroll.Unpaid_Leave_Hours__c > 0) {
                    allShifts.push({
                        name: 'Unpaid Leave',
                        rate: 0,
                        hours: payroll.Unpaid_Leave_Hours__c,
                        qty: 1
                    });
                }
            }

            /* 🔥 END: ADDED PAID & UNPAID LEAVE FROM staffpayrollData */
