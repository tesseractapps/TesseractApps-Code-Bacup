...
handlePeriodTypeChange(event) {
 ...
 // this.lastGeneratedPeriodType = this.selectedPeriodType;
    ...
    for (let i = 0; i < num; i++) {
      ...
      periods.push({
        ...
        endDate: end.toISOString().slice(0, 10),
        amount: amountPerPeriod.toFixed(2),
        originalAmount: Math.round(amountPerPeriod), 
        carryForward: 0,
        status: 'Future',
 ...
 //status: i === 0 ? 'Active' : 'Future',
   ...
   .then(result => {
  const activeIndex = this.generatedPeriods.findIndex(
    p => p.status === 'Active'
  );
  ...
  this.generatedPeriods = this.generatedPeriods.map((p, index) => {
    ...
    // 🔑 FIX: only count spend up to ACTIVE period
    ...
    if (index === activeIndex && cumulativeSpent > 0) {
      carryForward = Math.max(0, allocated - cumulativeSpent);
    }
  ...
  this.calculateTotal();
  console.log('✅ Preview spent + carry forward applied');
})
.catch(err => {
  ...
  handleInputChange(event) {
    ...
    // if (this.generatedPeriods[index]?.isLocked) {
    //   return;
    // }
    const field = event.target.dataset.field;
    let value = event.target.value;
  ...
  if ( field === 'amount' &&
   ...
   handleEndDateCascade(changedIndex, newEndDate) {
    ...
    // adjustAllocationCascade(changedIndex) {
    //   const periods = [...this.generatedPeriods];

    //   const approvedTotalAmount =
    //     parseFloat(this.approvedAmount) || 0;

    //   // 🔢 Calculate total allocated across all periods
    //   const totalAllocatedAmount = periods.reduce(
    //     (sum, period) => sum + (parseFloat(period.amount) || 0),
    //     0
    //   );

    //   // 🚨 Amount exceeding approved total
    //   let excessAllocation =
    //     totalAllocatedAmount - approvedTotalAmount;
    ...
    //   // ⚠️ Soft warning (user can proceed)
    //   this.showToast(
    //     'Warning',
    //     'You have exceeded the total approved amount. Subsequent periods have been adjusted automatically.',
    //     'warning'
    ...
    //   // 🔁 Deduct excess from subsequent periods only
    //   for (
    //     let periodIndex = changedIndex + 1;
    //     periodIndex < periods.length && excessAllocation > 0;
    //     periodIndex++
    //   ) {
    //     const currentAmount =
    //       parseFloat(periods[periodIndex].amount) || 0;

    //     if (currentAmount <= 0) continue;

    //     const deductionAmount =
    //       Math.min(currentAmount, excessAllocation);

    //     periods[periodIndex].amount =
    //       Math.round(currentAmount - deductionAmount);

    //     excessAllocation -= deductionAmount;
    //   }
  ...
  // adjustOverflowFromNextPeriods(changedIndex) {
  //   const approvedNum = parseFloat(this.approvedAmount) || 0;

  //   if (!approvedNum) return;

  //   let totalAllocated = this.generatedPeriods.reduce(
  //     (sum, p) => sum + (parseFloat(p.amount) || 0),
  //     0
  //   );

  //   if (totalAllocated <= approvedNum) return;

  //   let overflow = totalAllocated - approvedNum;

  //   // ⚠️ Warning (soft enforcement)
  //   this.showToast(
  //     'Warning',
  //     'You have exceeded the total approved amount. Excess will be adjusted from subsequent periods.',
  //     'warning'
  //   );

  //   // 🔁 Deduct overflow from subsequent periods
  //   this.generatedPeriods = this.generatedPeriods.map((p, i) => {
  //     if (i <= changedIndex || overflow <= 0) {
  //       return p;
  //     }

  //     const available = parseFloat(p.amount) || 0;
 ...
 /*  async handleSaveFundSplits() {
    ...
    console.log('splitsToSave generate..',JSON.stringify(splitsToSave));
      ...
      this.generatedPeriods = result.map(r => ({
        ...
        carryForward: r.Carry_Forward__c,
        status: r.Status__c,
        spentAmount: r.Spent_Amount__c || 0, // ✅ ADD THIS
        label: r.Name
      }));
      console.log('Fund splits saved: this.generatedPeriods', JSON.stringify(this.generatedPeriods));
...
//v2 handleSaveFundSplits
  ...
  //   // 🔑 Detect STRUCTURAL change (THIS FIXES 500 ERROR)
  ...
  //     console.log('🔁 Period type changed → replaceFundSplits');

  //     // ✅ SAFE: delete + insert only
  ...
  //   } else {
  //     console.log('✏️ Period type unchanged → updateFundSplits');

  //     // ✅ SAFE: update only
  //     result = await updateFundSplits({
  //       fundTrackerId: this.recordId,
  //       splits: splitsToSave
  //     });

  //     // 🔁 OPTIONAL BACKWARD COMPATIBILITY
  //     // If you haven’t created updateFundSplits yet, use:
    ...
    try {
      ...
      const splitsToSave = this.generatedPeriods.map(p => ({
        ...
        End_Date__c: p.endDate
          ...
          : null,
        Allocated__c: parseFloat(p.amount) || 0,
       Carry_Forward__c: parseFloat(p.carryForward),
        Status__c: p.status,
        Name: p.label
      }));
 //Carry_Forward__c: parseFloat(p.carryForward) || 0, commented to show carryforward based on apex
  ...
  //     //  NEW LOGIC (safe, isolated)
  //       await recalculateFundPeriods({
  //           fundTrackerId: this.recordId,
  //           clientId: this.clientId
  //       });
  ...
  //   }
  // }
//   recalculateCarryForwardForPeriod(index) {
//   const period = this.generatedPeriods[index];

//   const allocated = parseFloat(period.amount) || 0;
//   const spent = parseFloat(period.spentAmount) || 0;

//   const carryForward = Math.max(0, allocated - spent);

//   this.generatedPeriods[index] = {
//     ...period,
//     carryForward
//   };
// }
...
recalculateCarryForwardForPeriod(index) {
  ...
  const allocated = parseFloat(period.amount) || 0;
  const spent = parseFloat(period.spentAmount) || 0;
  const status = period.status; // 'Active' or 'Future'

  let carryForward = 0;

  // 🔑 Apply carry forward ONLY for current (Active) period
  if (status === 'Active'  && spent > 0) {
...
//     // 2️⃣ Recalculate + CAPTURE RESULT
...
//     // 3️⃣ IMPORTANT: Update UI with recalculated data
//     this.generatedPeriods = recalculatedSplits.map(r => ({
//       id: r.Id,
//       startDate: r.Start_Date__c,
//       endDate: r.End_Date__c,
//       amount: r.Allocated__c,
...
//     console.log(
//       '🧾 generatedPeriods AFTER recalculation:',
//       JSON.stringify(this.generatedPeriods)
//     );

//     this.showToast(
//       'Success',
//       'Fund Tracker and Period Splits updated successfully!',
//       'success'
//     );

//   } catch (error) {
//     console.error('❌ handleUpdateFund error:', error);
//     this.showToast(
//       'Error',
    ...
    async recalculateAfterSave() {
        try {
         ...
         await this.handleSaveFundSplits();
            ...
            // ⛔ Guard: period type changed but Generate not clicked
            // if (
            //   this.lastGeneratedPeriodType &&
            //   this.selectedPeriodType &&
            //   this.selectedPeriodType !== this.lastGeneratedPeriodType
            // ) {
            //   this.showToast(
            //     'Warning',
            //     'You changed the period type. Please click Generate before saving.',
            //     'warning'
            //   );
            //   return; // STOP SAVE
            // }
