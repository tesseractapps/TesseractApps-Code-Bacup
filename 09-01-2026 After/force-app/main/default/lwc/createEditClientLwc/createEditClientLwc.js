...
@track contactTypeOptions = [
    ...
    async fetchRolesForStaff() {
        try {
            ...
            console.log('âœ… Active roles (await):', JSON.stringify(this.activeRoles));
            ...
            console.log('ðŸ”Ž paginatedData after mapping:', JSON.stringify(this.paginatedData));
            console.log('this.participanteditflag >>', this.participanteditflag);
            if (this.participanteditflag != false) {
                await this.fetchStaffMembers(); // ðŸ” Ensure this populates paginatedData
                this.showSpinner = false;
            }

        } catch (error) {
            console.error('âŒ Error fetching staff roles:', error);
            this.showSpinner = false;
        }
    }
...
processStaffData(staffData) {
      ...
      this.filteredStaffMembers = [...this.staffMembers]; // âœ… Important
  }
      ...
      async fetchRoles() {
         ...
         if (!this.facilityId) {
             console.warn("âš ï¸ No facilityId found, cannot fetch Facility Roles.");
             return;
         }
    ...
    get staffRoleData() {
     ...
     get groupedStaffRoleData() {
        ...
        // Build map: staffId â†’ Set of active roles
        ...
        staffList.forEach((originalStaff) => {
            ...
            this.roles.forEach((roleName) => {
                ...
                // âœ… Prioritize modified value if present
                let isChecked;
        ...
        // âœ… Log full role comparison
    
       // console.log('âœ… Final Grouped Data:', JSON.stringify(grouped, null, 2));
         console.log('ðŸ“Š groupedStaffRoleData result length:', grouped.length);
    ...
    handleRoleToggle(event) {
        ...
        // âœ… Store multiple modified staff entries
        if (changedStaff) {
            this.modifiedStaffMap[staffId] = changedStaff;
        }

        console.log('ðŸ—‚ï¸ Modified Staff Map:', JSON.stringify(this.modifiedStaffMap));
    }
...
console.log('company flag',this.ndiscreateflag);
  ...
  const activeTab = localStorage.getItem('activeAdminClientTab');
   ...
   switch (activeTab) {
    ...
    this.Nationality = ''; // âœ… ensures no default value
...
wiredClient(result) {
    console.log("ðŸ”µ [wiredClient] Fired");
    this.wiredClientResult = result;

    const { data, error } = result;

    if (data) {
        console.log("ðŸŸ¢ [wiredClient] Data received:");
        ...
        // CLIENT BASIC FIELDS
        // ------------------------------------
        console.log("ðŸ“Œ Extracting client fields...");
        ...
        console.log("ðŸ· First Name:", this.firstName);
        console.log("ðŸ· Last Name:", this.lastName);
        console.log("ðŸ“ Address:", this.street1, this.city, this.province, this.postalcode);
        console.log("ðŸŒ Nationality:", this.Nationality);
        console.log("ðŸ—£ Languages:", this.selectedLangs);
        console.log("ðŸ¢ Organization:", this.orgId);
        console.log("ðŸ« Facility:", this.facilityId);
        ...
        if (client.Participant_Staff_Associations__r && client.Participant_Staff_Associations__r.length > 0) {
            ...
            console.log("ðŸ§‘â€ðŸ’¼ Role from associations:", this.role);
        } else {
            this.role = '';
            console.log("âš ï¸ No staff role assigned.");
        }

        // ------------------------------------
        // CONTACT LIST
        // ------------------------------------
        console.log("ðŸ“ž Mapping Contact List...");
        ...
        if (client.Participant_Contacts__r) {
            ...
            this.contactList1 = [];
            console.warn("âš ï¸ No Participant Contacts found.");
        }
        ...
        console.log("ðŸ“ž Final Contact List:", JSON.stringify(this.contactList1));

        // ------------------------------------
        // CONTACT TYPE OPTIONS
        // ------------------------------------
        console.log("âš™ï¸ Building Contact Type Options...");
        ...
        console.log("ðŸ“¦ Backend Types:", [...backendTypes]);
        ...
        console.log("ðŸ Final Contact Type Options:", JSON.stringify(this.contactTypeOptions));

        console.log("ðŸŸ© [wiredClient] Completed successfully");

    } else if (error) {
        console.error("âŒ [wiredClient] Error:");
...
addContactRow() {
  ...
  handleSubmit(event){
   ...
   // 2ï¸âƒ£ Find all Primary contacts
    ...
    // 3ï¸âƒ£ At least ONE Primary required
    ...
    //                 })
    //             );
    //             return; // â— Stop execution immediately
    //         }
    //     }
    ...
    const fullAddress = `${this.street}, ${this.city} ${this.postalcode}, AU`;
        ...
        fetch(endpoint)
            ...
            .then(data => {
                ...
                // ðŸš€ Submit the form AFTER geocode response
    ...
    try {
        ...
        if (this.fileName && this.fileName.length > 0) {
            try {
                ...
                console.error("âŒ File upload error:", uploadErr);
            }
        }

        // -------------------------------
        // 2ï¸âƒ£ SAFE CONTACT UPDATE
        // -------------------------------
        ...
        if (safeContactPayload) {
            try {
                ...
                console.error("âŒ Contacts update error:", contactError);
                return; // stop further processing
            }
        } else {
            console.warn("âš ï¸ No contacts to update â€” skipping updateParticipantContacts()");
        }
              ...
              console.log("âœ… participantFacilityResult updated."+JSON.stringify(participantFacilityResult));

        // -------------------------------
        // 3ï¸âƒ£ INSERT STAFF RECORDS
        // -------------------------------
        try {
            ...
            console.error("âŒ Error inserting staff record:", staffErr);
        }
        ...
        // -------------------------------
        // 4ï¸âƒ£ FINAL REFRESH AFTER DELAY
        // -------------------------------
        setTimeout(() => {
            refreshApex(this.wiredClientResult);
        }, 2000);
        ...
        console.error("âŒ Unexpected handleSuccess error:", err);
        this.dispatchEvent(
            new ShowToastEvent({
  ...
  handleeditClose(event){
    ...
    this.handleflag();
     ...
     if (this.participanteditflag) {
        ...
        this.modifiedStaffMap = {}; // âœ… Clear unsaved changes
        ...
        console.log('ðŸ”„ Participant edit cancelled â€“ data reverted.');
    }
   // this.selectedRoles = [...this.originalSelectedRoles];
    ...
    // â— Reset to original data (if needed)
    //this.filteredStaffMembers = [...this.staffMembers];
  }
    ...
    if (this.detailsflag) {
       ...
       this.originalStaffMembers = this.staffMembers.map(staff => ({
        ...
        this.originalModifiedStaffMap = Object.fromEntries(
            ...
            try {
                // ðŸ” Force data load in sequence
                await this.fetchRoles();
                await this.fetchRolesForStaff();   // will call fetchStaffMembers() internally

                // âœ… Make sure filtered list is set
                this.filteredStaffMembers = [...this.staffMembers];

                console.log('âœ… Data loading completed.');
            } catch (err) {
                console.error('âŒ Error loading roles/staff:', err);
            }
    }
 ...
 handleFormLoad(event) {
    ...
    console.log('âœ… Facility ID:', this.facilityId);
}
  ...
  handleUpdateStaff() {
        ...
        .then(() => {
            console.log('âœ… Successfully updated staff assignments.');
            ...
            this.participantflag = true;
            this.fetchRolesForStaff();
            // âœ… Clear modified staff map
            this.modifiedStaffMap = {};
            this.searchStaff = '';
        })
        .catch(error => {
            console.error('âŒ Error updating staff assignments:', error);
        });
    }
    ...
    handleSearchChange(event) {
        ...
        // ðŸ”¥ CRITICAL: Always sync the input value with Nationality
    ...
    toggleDrop(event) {
        ...
        if (this.showDropdown) {
            ...
            // âœ… Bind & attach outside click listener
            ...
            // âœ… Remove listener when closed
    ...
    handleSelect(event) {
        ...
        console.log('ðŸ“Œ Selected Nationality:', selectedValue);
        
        // ðŸ”¥ Update both the display and stored value
        this.Nationality = selectedValue || '';
        this.showDropdown = false;

        console.log('ðŸŒ€ Nationality after selection:', this.Nationality);
    }
    ...
    handleDropdownToggle(event) {
        ...
        if (this.isExpanded) {
            // âœ… Bind and attach outside click listener
            this._boundHandleClickOutside = this.handleClickOutside.bind(this);
            window.addEventListener('click', this._boundHandleClickOutside);
        } else {
            // âœ… Remove listener when closed
    ...
    handleLanguageToggle(event) {
        ...
        console.log('ðŸŒ€ [handleLanguageToggle] START');
        console.log('ðŸ‘‰ Toggled Language:', optionId);
        console.log('âœ… Checked:', isChecked);

        // ðŸ§© Ensure selectedLangs is defined as a string
        if (!this.selectedLangs) {
            this.selectedLangs = '';
        }

        // ðŸ§© Convert to array safely (handles both array and string cases)
        let langsArray = [];
        ...
        // ðŸ§© Add or remove the selected language
        if (isChecked) {
            if (!langsArray.includes(optionId)) {
                langsArray.push(optionId);
                console.log('âž• Added:', optionId);
            }
        } else {
            langsArray = langsArray.filter(v => v !== optionId);
            console.log('âž– Removed:', optionId);
        }

        // ðŸ§© Store as a semicolon-separated string
        this.selectedLangs = langsArray.join(';');

        console.log('ðŸ“‹ Updated Selected Languages (semicolon):', this.selectedLangs);

        // Refresh UI or data
        this.refreshValues();

        console.log('ðŸ” Values refreshed');
        console.log('ðŸŒ€ [handleLanguageToggle] END');
    }

    refreshValues() {
    console.log('ðŸŒ€ [refreshValues] START');
    console.log('ðŸ“‹ Current Selected Languages:', JSON.stringify(this.selectedLangs));

    // ðŸ”’ SAFETY CHECK â€” ENSURE selectedLangs is always a string
    let langStr = this.selectedLangs || "";

    // Convert if backend accidentally returns array
    if (Array.isArray(langStr)) {
        console.warn('âš ï¸ selectedLangs is array. Converting to comma string.');
    ...
    console.log("ðŸ”§ Normalized Language String:", langStr);
    ...
    this.Languages = this.Languages.map(opt => {
        ...
        console.log(`ðŸ”„ Processing: ${opt.id} | Selected: ${isSelected}`);
    ...
    console.log('âœ… Updated Languages State:', JSON.stringify(this.Languages));
    console.log('ðŸŒ€ [refreshValues] END');
}
    ...
    handleClickOutside(event) {
       ...
       let clickedInsideFacility = false; 
        ...
        // âœ… Close Nationality dropdown
        if (this.showDropdown && !clickedInsideNationality) {
            this.showDropdown = false;
            window.removeEventListener('click', this._boundHandleClickOutside);
        }

        // âœ… Close Language dropdown
        if (this.isExpanded && !clickedInsideLanguage) {
            this.isExpanded = false;
            window.removeEventListener('click', this._boundHandleClickOutside);
        }
