...
import UsrRoleName from "@salesforce/schema/User.User_Type__c";
...
export default class TesseractAppsParticipantView extends LightningElement {
  ...
  set isModalOpen(val) {
    ...
    this.dispatchEvent(new CustomEvent('subpopup', {
      detail: { slug: val ? 'more-shifts' : '' },
      bubbles: true,
      composed: true
    }));
    this.dispatchEvent(new CustomEvent('childstatechange', { bubbles: true, composed: true }));
  }
  ...
  @track rows = [
   ...
   /*  { id: "3", label: "Employment Type", isRadio: true, value: "Permanent" }, */
    { id: "3", label: "Employment Type", isCheckboxGroup: true, value: ["Permanent"]},
...
@api
set isPrintMode(value) {

    this._isPrintMode = value;

    console.log('Child received isPrintMode =', value);
...
get isPrintMode() {
  ...
  _showServiceItemModal = false;
  get showServiceItemModal() { return this._showServiceItemModal; }
  set showServiceItemModal(val) {
    this._showServiceItemModal = val;
    this.dispatchEvent(new CustomEvent('subpopup', {
      detail: { slug: val ? 'service-item' : '' },
      bubbles: true,
      composed: true
    }));
    this.dispatchEvent(new CustomEvent('childstatechange', { bubbles: true, composed: true }));
  ...
  @track selectedParticipantLabel='Select Participant';
  ...
  get showErrorModal() { return this._showErrorModal; }
  set showErrorModal(val) {
    this._showErrorModal = val;
    this.dispatchEvent(new CustomEvent('subpopup', {
      detail: { slug: val ? 'error' : '' },
      bubbles: true,
      composed: true
    }));
    this.dispatchEvent(new CustomEvent('childstatechange', { bubbles: true, composed: true }));
  ...
  @track unassignedFlagConfirmation=false;
  @track handleUnassignedConfirmationCompleted=false;
 ...
 _calendarView;
  ...
  @track showExtendedBrokenUI = false;
    ...
    // @track showFormsModal = false;
     @track showExtendedBrokenUI = false;
     _showFormsModal = false;
     get showFormsModal() { return this._showFormsModal; }
     set showFormsModal(val) {
       this._showFormsModal = val;
       this.dispatchEvent(new CustomEvent('subpopup', {
         detail: { slug: val ? 'forms' : '' },
         bubbles: true,
         composed: true
       }));
       this.dispatchEvent(new CustomEvent('childstatechange', { bubbles: true, composed: true }));
    ...
    @track fundVisibility;
    // @track showUnavailableParticipantModal=false;
    _showUnavailableParticipantModal = false;
    get showUnavailableParticipantModal() { return this._showUnavailableParticipantModal; }
    set showUnavailableParticipantModal(val) {
      this._showUnavailableParticipantModal = val;
      this.dispatchEvent(new CustomEvent('subpopup', {
        detail: { slug: val ? 'unavailable-participant' : '' },
        bubbles: true,
        composed: true
      }));
      this.dispatchEvent(new CustomEvent('childstatechange', { bubbles: true, composed: true }));
    ...
    @api setStep(step) {}
    @api openPopup(slug) {
      if (slug === 'more-shifts') this.isModalOpen = true;
      else if (slug === 'service-item') this.showServiceItemModal = true;
      else if (slug === 'error') this.showErrorModal = true;
      else if (slug === 'forms') this.showFormsModal = true;
      else if (slug === 'unavailable-participant') this.showUnavailableParticipantModal = true;
    }   
    ...
    @api openShiftCreate() {
      ...
      if (this.chosenRole && this.chosenRole.length > 0) {
          this.addShiftData.AddShiftRole = this.chosenRole[0];
      }
      const today = new Date().toISOString().split('T')[0];
      this.addShiftData.AddShiftStartDate = today;
      this.dispatchEvent(new CustomEvent("shiftcreationparticipantview", {
          detail: { isEdit: false }
      }));
    }

    @api openShiftEdit(shiftStaffId, serviceId) {
      this.openEditShiftScreen(shiftStaffId, serviceId);
    }    
    @track enableSilValue;
    _selectedServiceMap = new Map();
    ...
    @track selectedGroupShift=[];
     ...
     @track displayParticipantDropDown=false;
  ...
  set calendarview(value) {
      ...
      // 🔥 This runs whenever parent updates the value
      console.log('Received new view type:', value);
      console.log('Received new view type:', this._calendarView);
  }
  ...
  openPublishFlag() {
    ...
    this.publishEndDate =this.endDate;
    this.publishStartDate = this.startDate;
  ...
  renderedCallback() {
    ...
    console.log('Participant rendered. isPrintMode in participant view =', this.isPrintMode);
...
@api
set refreshparticipant(value) {
    this._refreshParticipant = value;
    console.log('roster template refresh  ')
...
@api
navigateToToday() {
    console.log('🚀 navigateToToday() called from Participant View');
    
    const today = new Date();
    const todayStr = today.toISOString().split('T')[0];
    
    if (this.isDateInCurrentView(todayStr)) {
        console.log('✅ Today is in current view → scrolling only');
        this.scrollToTodayColumn();
        return;
    }
    
    // Today not in current view - request parent to reload
    console.log('🔄 Today not in current view → requesting parent reload');
    this.dispatchEvent(new CustomEvent('todayrequested', {
...
get refreshparticipant() {
  ...
  @wire(getRecord, { recordId: Id, fields: [UserEmail,UsrRoleName] })
  userDetails({ error, data }) {
    if (data) {
      this.currentUserEmail = data.fields.Email.value;
       this.currentUserRole = data.fields.User_Type__c.value;
      if (
        this.currentUserRole == "NDIS Org Admin" 
      
      ) {
        this.fundVisibility = true;
      }
       console.log('currentuser role=>'+this.currentUserRole);
      if (this.currentUserEmail) {
        currnetLoggedInFcaility({ UserEmail: this.currentUserEmail }).then(
          (result) => {
            console.log("result  currnetLoggedInFcaility==>" + JSON.stringify(result));
           ...
           const patchedRows = this.rows.map((r) => {
                  ...
                  if (r.isCheckboxGroup) {
                    let valuesArray = [];

                    if (typeof incoming === "string") {
                      valuesArray = incoming.split(",").map(v => v.trim());

                      // reverse mapping you did in Apex
                      valuesArray = valuesArray.map(v =>
                        v === "Full-time and part-time" ? "Permanent" : v
                      );
                    }
            ...
            patchedRows.sort((a, b) => {
              ...
              return 0; // neither ordered
               ...
               console.log("patchedRows==>" + JSON.stringify(patchedRows));
  ...
  connectedCallback() {
    ...
    this.globalServiceKeyListener = this.onServiceKeyDown.bind(this);
    window.addEventListener('keydown', this.globalServiceKeyListener);
    this.handleParticipantPasteShortcutBound =this.handleRosterPasteShortcut.bind(this);
    ...
    window.addEventListener( 'keydown',this.handleParticipantPasteShortcutBound );
    ...
    organizationDetails().then((response) => {
      ...
      fetchBulkRoles({facilityIDList:this.facIdlist}).then(
        ...
        console.log('week data view'+ this.calendarview);
          if(this.facilityeventtriggered==true){ 

          const isOpenedCreatePage = localStorage.getItem('isOpenedCreatePageIn') === 'true';

          this.openParticipantshiftView = isOpenedCreatePage;
        //  this.isHome = !isOpenedCreatePage;

          console.log('opened create page:', isOpenedCreatePage);
          console.log('openParticipantshiftView:', this.openParticipantshiftView);
...
this._handleKeyUp = (e) => {
    if (!e.ctrlKey && !e.metaKey) this.isCtrlHeld = false;
};
...
document.removeEventListener('keyup', this._handleKeyUp);
    ...
    window.removeEventListener('keydown', this.globalServiceKeyListener);
    if (this.handleDocumentClickBound) {
      document.removeEventListener("click", this.handleDocumentClickBound);
    }
      window.removeEventListener('keydown',this.handleParticipantPasteShortcutBound );
  }
  ...
  wiredGetParticipantData(result) {
   ...
   console.log("✅ RAW Participant Data:", JSON.stringify(result));
    ...
    //  console.log("✅ Week Data:", JSON.stringify(weekData));
      ...
      const initializedParticipants = rawParticipants.map((participant) => ({
        ...
        servicesByDay: (participant.servicesByDay || []).map((day) => ({
          ...
          services: (day.services || []).map((serv) => ({
            ...
            dynamicClass: this.getBaseIconClass()
          }))
        }))
      }));
      ...
      this.refreshParticipantDataUI();

      if (this._selectedServiceMap && this._selectedServiceMap.size > 0) {

              console.log('♻ Restoring selected participant services');

              this._selectedServiceMap.forEach((value, key) => {
                  this.updateServiceSelectionUI(key, true);
              });

              // force render once
              this.participantData = JSON.parse(JSON.stringify(this.participantData));

              console.log('✅ Participant selection restore completed');
       }
  ...
  emptyFields() {
    ...
    this.selectedParticipantLabel = "Select Participant";
    ...
    this.handleUnassignedConfirmationCompleted=false;
 ...
 this.displayParticipantDropDown=false
  }
  ...
  async handleOpenParticipantShiftView(event) {
    ...
    let unavailableStart = event.currentTarget.dataset.unavaialstartdate;
    let unavailableEnd = event.currentTarget.dataset.unavaialenddate;
   ...
   const normalize = (date) => {
    ...
    let selectedDate = normalize(shiftdate);
    let startDate = normalize(unavailableStart);
    let endDate = normalize(unavailableEnd);
    console.log('Unavailable Start:', startDate);
    console.log('Unavailable End:', endDate);
    console.log('Selected Date:', selectedDate);

    // check if selected date is outside range
   if (selectedDate >= startDate && selectedDate <= endDate) {
        this.confirMationMessage(
          "Error",
          "This date is marked as unavailable. Please pick another available date.",
          "Error"
        );
        this.isShowSpinner = false;
    ...
    this.emptyFields();
    let participantMultifacility=event.currentTarget.dataset.facvalue;
    console.log('participant multi faciltiy '+JSON.stringify(participantMultifacility));
    console.log('final list faciltiyy '+JSON.stringify(this.finalSelectedFacilities));
  ...
  // this.addShiftData.AddShiftFacilityValue=this.finalSelectedFacilities[0].value;
      if (participantMultifacility) {

        let facilityArr = participantMultifacility.split(',').map(f => f.trim());
  ...
  /*   this.addShiftData.AddShiftFacilityValue =
   ...
   const participantRecord = this.participantoptions.find(
      (rec) => rec.value === participantid
    );
     await this.updateStaffConflicts();

    let participantLabel = participantRecord ? participantRecord.label : '';
 ...
 async handleParticipantChange(event){
    ...
    const selectedDate = this.addShiftData.AddShiftStartDate;

    if (selectedDate && this.participantAvailabilityMap[this.serviceParticipant]) {
      const availability = this.participantAvailabilityMap[this.serviceParticipant];
      ...
      const start = normalize(availability.start);
      const end = normalize(availability.end);
      ...
      if (sel >= start && sel <= end) {
        ...
        this.addShiftData.AddShiftStartDate = null;
      }
    }
    ...
    this.AddShiftAndServices = await Promise.all(
      this.AddShiftAndServices.map(async (row) => {
        ...
        try {
          const response = await getClientFunds({ clientId: this.serviceParticipant });
          if (response) {
            const fetchedServiceTypes = response.map((rec) => ({
              label: rec.Registration_Group__c,
              value: rec.Id,
  ...
  async handleAddShiftChange(event) {
    ...
    if(event.target.name == "AddShiftFacilityValue"){
        ...
        //  this.serviceParticipant=this.participantoptions[0].value;
         await this.updateStaffConflicts();
      ...
      /*     this.serviceParticipant='';
          ...
          // Get first participant (if available)
            const hasParticipants = this.participantoptions.length > 0;

            const selectedParticipant = hasParticipants
              ? this.participantoptions[0]
              : {
                  value: null,
                  label: "Select Participant"
                };

            this.serviceParticipant = selectedParticipant.value || "";
            this.selectedParticipantLabel = hasParticipants
              ? selectedParticipant.label
              : "";

            let fetchedServiceTypes = [];

            if (hasParticipants) {
              try {
                const response = await getClientFunds({
                  clientId: selectedParticipant.value
                });

                if (response) {
                  fetchedServiceTypes = response.map((rec) => ({
                    label: rec.Registration_Group__c,
                    value: rec.Id,
                    Plan_Type__c: rec.Plan_Type__c,
                    thresholdCrossed: rec.Threshold_Crossed__c,
            ...
            this.AddShiftAndServices = this.AddShiftAndServices.map((row) => ({
              ...
              participantlabel: hasParticipants
                ? selectedParticipant.label
                : "Select Participant",
      ...
      console.log(
        ...
        if (this.isEditShiftScreenFlag && !this.groupShift ) {
            ...
            this.AddShiftAndServices = this.AddShiftAndServices.map((row, index) => ({
                ...
                participant: this.serviceParticipant,
                shiftWithStaffId: null,
                addressSource: index === 0 ? addressType : row.addressSource,
                hourlyrate: index === 0 ? rate : row.hourlyrate
            }));
    ...
    if (event.target.name == "AddShiftRole") {
     ...
     if (event.target.name == "AddShiftStartDate"){
       this.addShiftData.AddShiftBreak = this.getDuration(
         ...
         if (this.serviceParticipant && this.participantAvailabilityMap[this.serviceParticipant]) {
            ...
            const selectedDate = normalize(this.addShiftData.AddShiftStartDate);
            const startDate = normalize(availability.start);
            const endDate = normalize(availability.end);

            console.log('Checking availability:', availability);

            // ❗ Throw error if date falls within unavailable range
            if (selectedDate >= startDate && selectedDate <= endDate) {
              this.confirMationMessage(
                "Error",
                "This date is marked as unavailable. Please pick another available date.",
  ...
  if (this.addShiftData.AddShiftStaffValue) {
    ...
    if (selectedStaff) {
      ...
      this.AddShiftAndServices = this.AddShiftAndServices.map((row) => ({
        ...
        hourlyrate:  this.getServiceStaffHourlyRate(this.addShiftData.AddShiftHoliday, this.AddShiftDayName, this.addShiftData.AddShiftStaffValue, this.addShiftData.AddShiftType )
      }));
    }
  }
 ...
 handleNotificationChange(event) {
  ...
  async getShiftTimingsByFacility(shifNames) {
    ...
    if (this.addShiftData.AddShiftStaffValue) {
        ...
        rate = this.getServiceStaffHourlyRate(
        this.addShiftData.AddShiftHoliday,
        this.AddShiftDayName,
        this.addShiftData.AddShiftStaffValue,
        this.addShiftData.AddShiftType
 ...
 // console.log("Staff Data: " + JSON.stringify(staffData));
     ...
     if(this.orignalShiftTime && timeType=='AddShiftEndTime'){
           console.log(' this.addShiftData ==>'+this.addShiftData.AddShiftEndTimeAMPM);
            ...
            const isAllowedCategory = ALLOWED_CATEGORIES.includes(typeOfJob);
             ...
             this.recalculateMinimumEngagement(this.addShiftData.AddShiftStartTimeAMPM,this.addShiftData.AddShiftEndTimeAMPM);
  ...
  validateAddress() {
    ...
    async handleEditShiftScreen(event) {
      ...
      await this.openEditShiftScreen(shiftStaffId, serviceId);
  }


 async openEditShiftScreen(shiftStaffId,serviceId ) {
    /* event.stopPropagation(); */
    try{
   ...
   // let serviceId=serviceId;
    ...
    // this.dispatchEvent(new CustomEvent("shiftcreationparticipantview"));
    // this.dispatchEvent(new CustomEvent("shiftcreationparticipantview", {
    //     detail: { isEdit: true, shiftStaffId: this.shiftStaffId }
    // }));    
    ...
    this.rowShiftTypeEnable=false;
    this.rowShiftTimingsEnable=false;
    this.unassignedFlagConfirmation = false
    this.disableNotification=false;
    this.showErrorModal=false;
    this.handleUnassignedConfirmationCompleted=false;
    this.staffDateConflicts = [];
 ...
 this.displayParticipantDropDown=false
    ...
    getAddShiftDataById({ shiftId: this.shiftStaffId }).then(async (result) => {
      ...
      this.dispatchEvent(new CustomEvent("shiftcreationparticipantview", {
          detail: { isEdit: true, shiftStaffId: this.shiftUIDFormula }
      }));      
      ...
      this.RolesStaffId = this.addShiftData.AddShiftStaffValue;
        ...
        console.log("this.addShiftData.AddShiftStaffValue in edit  ==>" + this.addShiftData.AddShiftEndDate);
      ...
      if (
        ...
        result.shiftwithstaffdata.Status__c == "Completed"  || result.shiftwithstaffdata.Status__c == "Cancelled"
      ) {
     ...
     //this.hourlyRateEditable = this.addShiftData.AddShiftType == "Sleepover Shift";
      ...
      console.log("service in edit ==> " + JSON.stringify(this.servicesList));
        console.log("👉 this.addShiftData.AddShiftStatus in edit ", this.addShiftData.AddShiftStatus);
      if (this.addShiftData.AddShiftStatus === "Unassigned") {
      ...
      // 🔹 Main method
      //  this.AddShiftAndServices = [this.initRow()];
      if (shiftType === "Custom" && this.addShiftData.AddShiftStatus != "Unassigned" || (shiftType === "Custom" && this.addShiftData.AddShiftStatus == "Unassigned" &&  this.servicesList.length > 1)  ) {
        ...
        console.log(
          " this.addShiftData.AddShiftTypeName ==> " + this.addShiftData.AddShiftTypeName
        );
        let shifNames = this.shiftNameOptions.filter(
          (rec) => rec.value === this.addShiftData.AddShiftTypeName
        );
      ...
      if(result.shiftwithstaffdata.Add_Shift__r.Group_Shifts__c){
          this.otherGroupShiftShiftIds  = [];
         this.displayParticipantDropDown=true;
    ...
    const rows = await Promise.all(
        this.servicesList.map(async (service) => {
            try {
                ...
                if (service.Id === serviceId  || this.groupShift==true ) {
 ...
 */
  ...
  get isNoParticipantDataFound() {

    return (
        this.participantData &&
        Array.isArray(this.participantData.Participnat) &&
        this.participantData.Participnat.length === 0
    );
}
 ...
 handleInputChange(event) {
  ...
  else if (event.target.value instanceof Array) {
    value = [...event.target.value];
  }
  // slider / other inputs
  else {
  ...
  async handleSave() {
    ...
    console.log(
     ...
     if ( !this.serviceParticipant ) {
      this.confirMationMessage(
        "Error",
        this.participantPreferredName +" is required before saving." ,
        "Error"
      );
       this.isShowSpinner = false;
      return;
    ...
    if (!allHaveStaff && this.handleUnassignedConfirmationCompleted==false) {
      this.unassignedFlagConfirmation =true;
      return;
    } 
    ...
    console.log("services array==> " + serviceTypeIdList);
     if (serviceTypeIdList.length > 0 && this.addShiftData.sil !=true) {
      ...
      const staffIds = staffRows
        .filter(staff => staff.staffId !== null && staff.staffId !== undefined)
   ...
   formatDisplayDate(dateString) {
    ...
    async hanleProceedWithShiftCompliance(){
            ...
            this.staffDateWarnings=[];
                ...
                const staffIds = this.finalEndStaffRows.filter(staff => staff.staffId !== null && staff.staffId !== undefined).map(staff => staff.staffId);
                const Unavailresult = await  checkShiftWithinAvailability({
                    staffIds: staffIds,
 ...
 handleShiftComplianceWarnings(validationResult) {
  ...
  postSuccessOperation(result){
    ...
    // =========================================
         ...
         if (!this.isEditShiftScreenFlag && this.addShiftData.AddShiftnotification == true && shiftsJson[0] !=undefined  && this.selectedShiftMode=='Active') {
      ...
      const reducedResult = this.AddShiftAndServices.reduce((acc, row) => {
        ...
        if (!acc[key]) {
          ...
          acc[key] = {
            ...
            unAssignedServiceCreation :row.unAssignedServiceCreation,
...
/*   get getIconItemClass() {
    ...
    if (this.selectedViewType === 'fortnightly') {
        classString += " fortnightly-font-9";
    }
 ...
 getBaseIconClass() {
    ...
    if (this.selectedViewType === 'fortnightly') {
     ...
     // Print only
    if (this.isPrintMode && this.selectedViewType === 'fortnightly') {
        classString += " participant-print-font";
    }
  ...
  async processShifts(facilityId) {
    try {
      ...
      if (this.isEditShiftScreenFlag != true) {
             this.addShiftData.AddShiftTypeName= this.shiftNameOptions[0].value;
  ...
  initRow() {
    return {
      ...
      addressSource: "",
      staffPaidBreak:false,
      unAssignedServiceCreation:false,
       extraHours:0,
      doubleRate:0,
        ...
        if (i === index) {
          ...
          try {
              ...
              console.log("🔄 Updating row with new service items...");
                ...
                console.log("ShiftTypeMap:", JSON.stringify(result.ShiftTypeMap));
                console.log("Current Shift Type:", this.addShiftData.AddShiftType);
                  ...
                  console.log("updatedRow.allservicetypes:", JSON.stringify(updatedRow.allservicetypes));
          ...
          console.log("🏁 SERVICE TYPE PROCESSING COMPLETED");
            ...
            updatedRow.unitprice = event.currentTarget.dataset.unit || 0;
             updatedRow.unit=event.currentTarget.dataset.unit || 0;
 ...
 handleClearSelection(event) {
  ...
  this.AddShiftAndServices = this.AddShiftAndServices.map((row, i) => {
    if (i === index) {
      ...
      if (field === 'staff') {
        updatedRow.staff = null;
        updatedRow.stafflabel = 'Select Staff';
        updatedRow.hourlyrate = 0;
      }
...
get getFirstCellClass() {
    let classString = "firstcellwidth";

    // Append the specific class for the fortnightly width change
    if (this.selectedViewType === 'fortnightly') {
        classString += " fortnightly-cell-width";
    }

    return classString;
}

get getStaffCellClass() {
    let classString = "staffcellwidth";
...
get getAddIconClass() {
    ...
    // Append the specific class for the fortnightly font size change
    if (this.selectedViewType === 'fortnightly') {
        classString += " add-icon-fortnightly-15px";
    }

    return classString;
}

get getOverflowTextClass() {
    let classString = "overflowtext";

    // Append the specific class for the fortnightly max-width change
    if (this.selectedViewType === 'fortnightly') {
        classString += " fortnightly-overflow-width";
    }

    return classString;
}

get getFontSizeClass() {
    let classString = "overflowtext";

    // Append the specific class for the fortnightly max-width change
    if (this.selectedViewType === 'fortnightly') {
        classString += " fortnightly-font-size";
    }
...
//     let cls = this.selectedViewType === 'fortnightly'
//         ? 'slds-table fortnightly-scroll-container'
//         : 'slds-table';

//     if (this.isPrintMode && this.selectedViewType === 'fortnightly') {
//         cls += 'slds-table participant-print-fortnight';
//     }
...
get rosterTableClass() {
    if (this.isPrintMode) {
        if (this.selectedViewType === 'fortnightly') {
            return 'slds-table participant-print-fortnight print-fortnight';
        }
        return 'slds-table participant-print-week print-week';
    }
...
handleCloseUnassigned() {
    this.unassignedFlagConfirmation = false;
    this.isShowSpinner = false;
    this.handleUnassignedConfirmationCompleted=false;
 }
  handleUnassignedFinalAllocate() {
    console.log('handleUnassignedFinalAllocate');

    this.isShowSpinner = false;
    this.unassignedFlagConfirmation = false;
    this.handleUnassignedConfirmationCompleted=true;
    this.AddShiftAndServices = this.AddShiftAndServices.map((row) => ({
        ...row,
        unAssignedServiceCreation: row.staff == null, 
    }));

    console.log(
      'addShiftService ' + JSON.stringify(this.AddShiftAndServices)
    );
  ...
  async recalculateMinimumEngagement(editStartAMPM, editedEndAMPM) {
  
      console.log("===== MIN ENG (APEX FETCH) START =====");
  
      console.log("🔎 Incoming editStartAMPM >>>", editStartAMPM);
      console.log("🔎 Incoming editedEndAMPM >>>", editedEndAMPM);
      console.log("🔎 AddShiftId >>>", this.addShiftData?.AddShiftId);
      console.log("🔎 AddShiftEndDate >>>", this.addShiftData?.AddShiftEndDate);
      ...
      try {
          ...
          console.log("🔎 brokenShiftData RAW >>>", JSON.stringify(brokenShiftData));
  
          if (!brokenShiftData || !brokenShiftData.length) {
              console.log("❌ No broken shift records returned");
              return;
          }
  
          // 🔎 Helper to convert AM/PM to Date
          const toDate = (timeAMPM) => {
  
              console.log("🕒 toDate INPUT >>>", timeAMPM);
              console.log("🕒 typeof INPUT >>>", typeof timeAMPM);
  
              const t24 = this.convertTo24HourFormat(
                  timeAMPM.toLowerCase()
              ).replace("Z","");
  
              console.log("🕒 converted 24hr >>>", t24);
  
              const dateStr = `${this.addShiftData.AddShiftEndDate}T${t24}`;
          ...
          brokenShiftData.forEach((seg, index) => {
  
              console.log("========== LOOP START ==========");
              console.log("Row Index >>>", index);
              console.log("Start_time_Formula__c >>>", seg.Start_time_Formula__c);
              console.log("End_time_formula__c >>>", seg.End_time_formula__c);
  
              let startAMPM = seg.Start_time_Formula__c;
              let endAMPM   = seg.End_time_formula__c;
  
              console.log("Comparing START >>>", startAMPM, " WITH EDIT >>>", editStartAMPM);
  
              // 🔥 Detect edited row by START TIME
              if (
                  startAMPM?.trim()?.toLowerCase() ===
                  editStartAMPM?.trim()?.toLowerCase()
              ) {
                  console.log("✅ MATCHED EDITED SEGMENT — Replacing End Time");
                  endAMPM = editedEndAMPM;
              }
  
              console.log("Final START used >>>", startAMPM);
              console.log("Final END used >>>", endAMPM);
              ...
              if (endDT <= startDT) {
                  console.log("🌙 Overnight detected — adding 1 day");
                  endDT.setDate(endDT.getDate() + 1);
              }
              ...
              if (
                  ...
                  console.log("🎯 editedBaseDuration updated >>>", editedBaseDuration);
              }
  
              console.log("========== LOOP END ==========");
          });
  
          console.log("🔢 totalWorkedHours FINAL >>>", totalWorkedHours);
  
          let uplift = totalWorkedHours < 2 ? (2 - totalWorkedHours) : 0;
  
          console.log("📈 uplift calculated >>>", uplift);
  
          // 🔹 Apply uplift ONLY to LAST service row
          const lastIndex = this.AddShiftAndServices.length - 1;
          ...
          console.log("🧩 Last Row BEFORE >>>", JSON.stringify(lastRow));
  
          lastRow.isMinimumEngagementApplied = uplift > 0;
          lastRow.billablehours = editedBaseDuration + uplift;
  
          lastRow.amount = (
              (lastRow.unitprice || 0) * lastRow.billablehours
          ).toFixed(2);
  
          console.log("🧩 Last Row AFTER >>>", JSON.stringify(lastRow));
  
          this.AddShiftAndServices = this.AddShiftAndServices.map((row, idx) =>
              idx === lastIndex ? lastRow : row
          );
  
          console.log("===== MIN ENG (APEX FETCH) END SUCCESS =====");
  
      } catch (error) {
          console.error("❌ Error fetching broken shifts:", error);
      }
  }
...
get serviceItemDropDownClass() {
 ...
 onServiceKeyDown(event) {
    if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'c') {
        console.log('📋 Selected Services:', this._selectedServiceList);
    }
}
handleServiceCopyClick(event) {

    // 🔹 Ctrl check
    if (!(event.ctrlKey || event.metaKey)) {
        console.log('⛔ Ignored click (no Ctrl)');
        return;
    }
    ...
    const shiftId=event.currentTarget.dataset.shiftwithstaffid;

    const key = `${participantId}_${day}_${serviceId}`;

   // console.log('🟡 Clicked:', { serviceId, participantId, day });

    if (!this._selectedServiceMap) {
        this._selectedServiceMap = new Map();
    ...
    if (this._selectedServiceMap.has(key)) {
        this._selectedServiceMap.delete(key);
        action = '❌ Removed';
    } else {
        this._selectedServiceMap.set(key, {
            serviceId,
            participantId,
            day,
            shiftId
    ...
    const isSelected = this._selectedServiceMap.has(key);

    console.log(`${action}:`, key, '| Selected:', isSelected);

    // 🔥 UI update
    this.updateServiceSelectionUI(key, isSelected);

    this._selectedServiceList = Array.from(this._selectedServiceMap.values());

    console.log('📦 Current Selection Count:', JSON.stringify(this._selectedServiceList));
}

updateServiceSelectionUI(key, isSelected) {

    this.participantData = {
        ...this.participantData,
        Participnat: this.participantData.Participnat.map(participant => ({
            ...
            servicesByDay: participant.servicesByDay.map(day => ({
                ...
                services: day.services.map(serv => {
                    ...
                    const serviceKey = `${participant.participantId}_${day.Shiftdate}_${serv.Id}`;

                    if (serviceKey === key) {

                        const baseClass = (serv.dynamicClass || 'icon-item')
                        ...
                        return {
                           ...
                           dynamicClass: isSelected
                              ? baseClass.includes('selected-service')
                                  ? baseClass
                                  : `${baseClass} selected-service`
                              : baseClass
                        };
                    }
...
handleDaySelection(event) {
    ...
    const clickedDate = event.currentTarget.dataset.weekdate;
    const clickedParticipantId = event.currentTarget.dataset.participantid;
    ...
    this.participantData.Participnat.forEach(participant => {
       if (participant.participantId !== clickedParticipantId) return;

        const participantId = participant.participantId;

        participant.servicesByDay.forEach(d => {
            (d.services || []).forEach(serv => {

                const key = `${participantId}_${d.Shiftdate}_${serv.Id}`;
    ...
    console.log('🔢 Services Matched:', matchCount);
    console.log('🧩 Keys to update:', keysToUpdate.length);

    // 🔥 batch UI update
    this.updateServiceSelectionBatch(keysToUpdate);

    this._selectedServiceList = Array.from(this._selectedServiceMap.values());

    console.log('📦 Final Selected Count:', this._selectedServiceList.length);
}
updateServiceSelectionBatch(keys) {
        ...
        Participnat: this.participantData.Participnat.map(participant => ({
            ...
            servicesByDay: participant.servicesByDay.map(day => ({
                ...
                services: day.services.map(serv => {
                    ...
                    const key = `${participant.participantId}_${day.Shiftdate}_${serv.Id}`;

                    if (keys.includes(key)) {

                        const isSelected = this._selectedServiceMap.has(key);
                        const baseClass = this.getBaseIconClass();
...
get showBulkActionBar() {
    // this.staffReassignLabel=this.staffPreferredName + " Reassign";
     this.staffReassignLabel= "Re-assign "+ this.staffPreferredName

    console.log('🟡 _selectedServiceList:', this._selectedServiceList);

    console.log('🟡 _selectedServiceMap:', this._selectedServiceMap);
    ...
    console.log(
        '🟡 Service Map Size:',
        this._selectedServiceMap
            ? this._selectedServiceMap.size
            : 0
    );

    const result =
        this._selectedServiceMap &&
        this._selectedServiceMap.size > 0;

    console.log('✅ showBulkActionBar:', result);

    // Notify parent
    // this.dispatchEvent(
    //     new CustomEvent('bulkactionchange', {
    //         detail: {
    //             showBulkActionBar: result
    //         },
    //         bubbles: true,
    //         composed: true
    //     })
...
get calendarPartClass() {
    ...
    let cls = this.showBulkActionBar
        ? 'calendar-part custom-scroll bulk-action-cal'
        : 'calendar-part custom-scroll';

    if (this.isPrintMode && this.selectedViewType === 'fortnightly') {
        cls += ' participant-print-calendar';
    }
   ...
   handleDeselectRosters() {
    ...
    console.log('🧹 Clearing all selected roster UI');


    // =====================================================
    // 🔹 PARTICIPANT SERVICE RESET
    // =====================================================

    if (this.participantData?.Participnat?.length) {
        ...
        this.participantData = {
            ...
            Participnat:
                this.participantData.Participnat.map(participant => ({

                    ...participant,

                    servicesByDay:
                        participant.servicesByDay.map(day => ({

                            ...day,

                            services:
                                day.services.map(serv => ({
    ...
    if (this._selectedServiceMap) {
        this._selectedServiceMap.clear();
    }
    ...
    this.participantData =
        JSON.parse(JSON.stringify(this.participantData));
    this.refreshParticipantDataUI();

    console.log('✅ All roster selections cleared');
}
...
handlePasteRoster() {
    ...
    console.log(
        'selected Shift roster ',
        JSON.stringify(this._selectedServiceList)
    );
...
handleRosterPasteShortcut(event) {
  ...
  handlePublishFromCopyPaste(){
      const evt = new CustomEvent(
        'publishfromparticipantview',
        {
            detail: {
                selectedServices:
                    this._selectedServiceList
  ...
  handleUnpublishFromCopyPaste(){
     const evt = new CustomEvent(
        'unpublishfromparticipantview',
        {
            detail: {
                selectedServices:
                    this._selectedServiceList
  ...
  handleStaffReassignFromCopyPaste(){
     const evt = new CustomEvent(
        'staffreassignmentfromparticipantview',
        {
            detail: {
                selectedServices:
                    this._selectedServiceList
...
async handleBulkEditFromCopyPaste(){
        ...
        if (!shiftIds.length) {
         ...
         getRosterCopyPreview({ shiftIds }) .then(result => {
            ...
            // 🔥 assign to UI
            this.copyRosterData = result; 
           this.bulkEditShiftOptions = result.map(shift => {
                    const matchedService =
                        (shift.services || []).find(

                            svc =>

                                this._selectedServiceList.some(
              ...
              if (this.bulkEditShiftOptions.length) {
                    this.selectedBulkEditShiftId = this.bulkEditShiftOptions[0].value;
                     const selectedOption = this.bulkEditShiftOptions.find(x => x.value ===this.selectedBulkEditShiftId );
                    console.log( 'selectedOption =>', JSON.stringify(selectedOption));
                    this.openEditShiftScreen(this.selectedBulkEditShiftId, selectedOption?.serviceId);
  ...
  async handleBulkShiftChange(event) {
    ...
    this.selectedBulkEditShiftId = event.detail.value;

    // 🔥 find selected option
    const selectedOption = this.bulkEditShiftOptions.find(x => x.value ===this.selectedBulkEditShiftId );
    console.log( 'selectedOption =>', JSON.stringify(selectedOption));
    await this.openEditShiftScreen(this.selectedBulkEditShiftId, selectedOption?.serviceId);
}
get computedDayCellClass() {
    let cls = this.selectedViewType === 'fortnightly' ? 'day-cell fortnightly' : 'day-cell';

    // Ctrl held OR bulk action bar active — suppress hover & plus icon
  ...
  get staffRowClass() {
      return this.addShiftData.sil ==true ?'staffRow-width' : 'staffRow-width'
      ...
      try{
          ...
          const conflictResponse = await checkStaffConflicts({
              ...
              currentShiftId : this.shiftStaffId ? this.shiftStaffId : null,
               otherGroupShiftShiftIds : this.otherGroupShiftShiftIds &&this.otherGroupShiftShiftIds.length > 0? this.otherGroupShiftShiftIds: null
   ...
   */
          ...
          this.AddShiftAndServices = this.AddShiftAndServices.map(row => {
              let selectedStaffConflict = conflictResponse[row.staff];
              let updatedFilteredStaff = (row.filteredstaff || []).map(opt => {
                  let optConflict = conflictResponse[opt.value];
                  return {
                      ...opt,
                      warningCount: optConflict ? optConflict.count : 0,
                      conflicts: optConflict ? optConflict.conflicts : [],
                      hasStaffConflicts: optConflict && optConflict.count > 0
                  };
              });

              return {
                  ...row,
                  filteredstaff: updatedFilteredStaff,
                  warningCount: selectedStaffConflict ? selectedStaffConflict.count : 0,
                  staffConflicts: this.decorateStaffConflicts(selectedStaffConflict ? selectedStaffConflict.conflicts : []),
                  hasStaffConflicts: selectedStaffConflict && selectedStaffConflict.count > 0
              };
          });
  ...
  handleGroupShiftClick(event) {
    ...
    this.selectedGroupShift = null;
  

    for (const participant of this.participantData.Participnat) {

        for (const day of participant.servicesByDay) {

            for (const serv of day.services) {

                if (serv.Id === serviceId) {

                    this.selectedGroupShift = serv.groupShiftData;
                    this.groupShiftRatio = serv.groupShiftRatio;

                    console.log('Group Shift Data',
                        JSON.stringify(this.selectedGroupShift));

                    console.log('Ratio',
                        this.groupShiftRatio);
...
refreshParticipantDataUI() {
    if (this.participantData && this.participantData.Participnat) {
        this.participantData.Participnat = this.participantData.Participnat.map(participant => {
            const servicesByDay = (participant.servicesByDay || []).map(day => {
                let dayHasGroupShift = false;
                const services = (day.services || []).map(serv => {
                    if (serv.showGroupShiftIcon) {
                        dayHasGroupShift = true;
                    }
                    if (!serv.originalInLineHeight) {
                        serv.originalInLineHeight = serv.inLineHeight || '';
                    }
                    const isExpanded = this.expandedCardIds.has(serv.Id);
                    const newHeight = this.adjustShiftHeight(serv.originalInLineHeight, isExpanded, serv.showGroupShiftIcon);
                    
                    let processedGroup = serv.groupShiftData;
                    if (serv.showGroupShiftIcon && serv.groupShiftData && !serv.groupShiftData.barStyle) {
                        const colorSource = serv.originalColor || serv.inLineHeight || '';
                        processedGroup = this.processGroupShiftData(serv.groupShiftData, colorSource);
                    }
                    
                    return {
                        ...serv,
                        isHovered: serv.isHovered || false,
                        hideIfNotHovered: serv.hideIfNotHovered || false,
                        dynamicClass: serv.dynamicClass || this.getBaseIconClass(),
                        inLineHeight: newHeight,
                        isGroupShiftExpanded: isExpanded,
                        groupShiftChevron: isExpanded ? 'expand_less' : 'expand_more',
                        groupShiftData: processedGroup,
                        groupShiftRatio: serv.groupShiftRatio || (processedGroup ? `Group ${processedGroup.staffList?.length || 0}:${processedGroup.participantList?.length || 0}` : '')
                    };
                });
...
handleGroupShiftBarToggle(event) {
    ...
    if (this.expandedCardIds.has(serviceId)) {
        this.expandedCardIds.delete(serviceId);
    } else {
        this.expandedCardIds.add(serviceId);
    }
  ...
  async refreshServiceItemsForRows() {
      ...
      this.AddShiftAndServices = await Promise.all(
          ...
          this.AddShiftAndServices.map(async (updatedRow) => {
              ...
              // Skip rows without Participant or Service Type
              ...
              try {
                  ...
                  console.log("🔄 REFRESHING SERVICE ITEMS");
                  console.log("Participant:", updatedRow.participant);
                  console.log("Service Type:", updatedRow.servicetype);
                  console.log("Current Service Item:", updatedRow.serviceitem);
                  console.log("Current Service Label:", updatedRow.serviceitemlabel);
                  ...
                  console.log("Refreshed Catalogue:", JSON.stringify(result));
                  ...
                  this.serviceGroupName = catalogueData.map((rec) => {
                      ...
                      if (
                          ...
                          amountVal =
                              result.clientJunctionMapAmount[rec.Id] || 0;
                  ...
                  console.log(
                      "Refreshed serviceGroupName",
                      JSON.stringify(this.serviceGroupName)
                  );
                  //=====================================================
                // Preserve existing Service Item if available
                // Otherwise use existing Shift Type auto selection
                //=====================================================
                ...
                //-----------------------------------------------------
                // First preference : Match current Service Item Label
                //-----------------------------------------------------

                const existingService = this.serviceGroupName.find(item =>
                    item.label === updatedRow.serviceitemlabel
                );

                if (existingService) {

                    console.log("Existing Service Item found in refreshed catalogue");

                    autoSelectedServiceItem = existingService.value;
                    autoSelectedUnitPrice = existingService.unit;
                    serviceItemLabel = existingService.label;

                } else {

                    console.log("Existing Service Item not found.");
                    console.log("Trying Shift Type auto selection...");

                    //-------------------------------------------------
                    // Your ORIGINAL Shift Type logic
                    //-------------------------------------------------
                    ...
                    if (result.ShiftTypeMap && this.addShiftData.AddShiftType) {
                        ...
                        const matchingService = this.serviceGroupName.find(item => {
                            ...
                            return (
                                shiftTypes &&
                                shiftTypes.split(';').includes(this.addShiftData.AddShiftType)
                            );
                        ...
                        if (matchingService) {
                            ...
                            autoSelectedServiceItem = matchingService.value;
                            autoSelectedUnitPrice = matchingService.unit;
                            serviceItemLabel = matchingService.label;
                ...
                console.log("Selected Service Item:", autoSelectedServiceItem);
                console.log("Selected Unit Price:", autoSelectedUnitPrice);
                console.log("Selected Label:", serviceItemLabel);
                //----------------------------------------------------
                ...
                return updatedRow;
                    ...
                    console.error("Error Details:", {
                        ...
                        participant: updatedRow.participant
...
prepareRosterForPrint() {
    ...
    console.log('Child calendarBody:', body);
    ...
    console.log('Child calendarHeader:', header);

    if (header) {
        header.classList.add('print-header');
    }

    const table = this.template.querySelector('table.table1');
...
scrollToTodayColumn() {
    console.log('📜 scrollToTodayColumn() called in Participant View');
    
    // Wait for DOM to render
    requestAnimationFrame(() => {
        // Find the day cells in the table header
        ...
        // Find today's column by checking the date attribute
        for (let cell of headerCells) {
            const dateSpan = cell.querySelector('.day-date');
            if (dateSpan) {
                // We need to find the actual date from the weekDataJson
                const weekData = this.participantData?.weekData || [];
                const idx = Array.from(headerCells).indexOf(cell);
                if (idx < weekData.length) {
                    const dateStr = weekData[idx]?.weekDays;
                    if (dateStr) {
                        // weekDays format: "DD-MM-YYYY"
        ...
        if (!targetElement) {
            console.log('⚠️ Today\'s column not found in participant view');
            return;
        }
        
        // Find the scrollable container
        const scrollContainer = this.template.querySelector('.calendar-part');
        if (!scrollContainer) {
            console.log('⚠️ Scroll container not found in participant view');
            return;
        }
...
isDateInCurrentView(dateString) {
    // Check if the given date (in YYYY-MM-DD format) is within the current loaded range
    const weekData = this.participantData?.weekData || [];
    if (weekData.length === 0) return false;
    
    const firstDayStr = weekData[0].weekDays;
    const firstDayParts = firstDayStr.split('-');
    const firstDayFormatted = `${firstDayParts[2]}-${firstDayParts[1]}-${firstDayParts[0]}`;
    
    const lastDayStr = weekData[weekData.length - 1].weekDays;
    const lastDayParts = lastDayStr.split('-');
    const lastDayFormatted = `${lastDayParts[2]}-${lastDayParts[1]}-${lastDayParts[0]}`;
    
    return dateString >= firstDayFormatted && dateString <= lastDayFormatted;
}
...
/**
 * Called by parent to reload participant view with a new date
 */
reloadWithDate(date) {
    console.log('📢 Child reloadWithDate() called with:', date);
    
    // Update the date range
    const startOfWeek = this.getStartOfWeek(date);
    this.currentStartOfWeek = startOfWeek;
    
    // Update week data
    this.weekDaysWithDates = this.calculateWeekDaysWithDates(startOfWeek);
...
/**
 * Get start of week (same as parent)
 */
getStartOfWeek(date) {
    const weekDayMap = {
        Sunday: 0,
    ...
    const selectedStartDay = weekDayMap[this.WeekStartDay || 'Monday'];
...
/**
 * Calculate week days with dates
 */
