                ...
                this.records = JSON.parse(JSON.stringify(
                    result.map((item) => ({
                       ...
                       // hasUnreadBadge:this.unreadFundsTrackerIds.includes(item.Id),//manendra added for badges
                        ...
                        // Only Active Fund Trackers
                        // const activeFunds = this.accList.filter(
                        //     acc => acc.status && acc.status.toLowerCase() === 'active'
                        // );
                        const activeFunds = this.accList.filter(
                            acc => (acc.status || '').trim().toLowerCase() === 'active'
                        );

                        this.totalApprovedAmount = activeFunds.reduce((sum, acc) => sum + (acc.amountApproved || 0), 0);
                        this.totalSpentFunds = activeFunds.reduce((sum, acc) => sum + (acc.spentAmount || 0), 0);
                        this.totalAvailableFunds = activeFunds.reduce((sum, acc) => sum + (acc.availableFunds || 0), 0);

                        // Total Support Items (Active only)
                        const { totalSupportItems, totalAmount } = activeFunds.reduce(
                            (acc, curr) => {
                                const childCount = curr.childItems ? curr.childItems.length : 0;

                                const sumAmount = curr.childItems
                                    ? curr.childItems.reduce(
                                        (sum, child) => sum + (child.Amount__c || 0),
                                        0
                                    )
                                    : 0;
...
handleToggleShiftName(event) {
    ...
    const updatedRow = this.serviceGroupName.find(
        r => String(r.Id) === String(rowId)
    );

    // Sync master list
    if (updatedRow) {
        this.records1 = this.records1.map(row =>
...
//period management end
  ...
  handleEntityNameChange(event) {
      ...
      if (newValue === 'Add New Entity') {
       ...
       console.log(" this.serviceGroupName in  handleEntityNameChange  :" + JSON.stringify( this.serviceGroupName ));
       console.log(" this.records1  in  handleEntityNameChange  :" + JSON.stringify(this.records1));
  }
  ...
  handleSubmit(event) {
   ...
   // fields.Entity_Profile__c = this.EntityNameValue;
    ...
    console.log('Registration_Group__c =', fields.Registration_Group__c);
