      ...
      this.rejectedShifts.forEach((shift, index) => {
         ...
         /*  let shiftColorStyle = shift.Shiftcolor__c; // fallback from backend
          ...
          /* const builtShift = {
              ...
              shiftColor: shiftColorStyle,
              isAssigned: shift.Rejected_Assigned__c || false,
              showDot: this.unreadRejectedShiftIds.includes( shift.Id ), // manendra added for notification badge count
          ...
          // Location in the shift object
          const shiftAddress = shift.Shift_Address__Street__s && shift.Shift_Address__Street__s.trim() !== '' 
              ? shift.Shift_Address__Street__s 
              : this.formatAddress(shift.Add_Shift__r?.Location__c) || '—';

          // Then in the builtShift object:
          const builtShift = {
              id: shift.Id,
              staffName: shift.Staff__r?.Name || "Unknown",
              staffName1: shift.Staff__r?.Display_Nickname__c || "Unknown",
              time: shift.Shift_Start_End_Time__c ? shift.Shift_Start_End_Time__c : shift.Add_Shift__r?.Shift_Start_End_Time__c || "",
              role: shift.Role_formula__c ? shift.Role_formula__c : shift.Add_Shift__r?.Role__c || "",
              location: shiftAddress, // Updated
              direction: shift.Add_Shift__r?.Direction__r?.Name || "",
              staffId: shift.Staff__c,
              picture: shift.Staff__r?.picture__c || "",
              shiftColor: shiftColorStyle,
              isAssigned: shift.Rejected_Assigned__c || false,
              showDot: this.unreadRejectedShiftIds.includes(shift.Id),
              rejectedComments: shift.RejectedComments__c || '',
              reassignedStaffName: shift.reassignedStaffName || null,
              reassignedDate: shift.reassignedDate || null,
              participantName: shift.Services_and_Support_Plans__r?.length 
...
.catch(error => {
    ...
    // Address
    const shiftAddress = selectedRecord.Shift_Address__Street__s && selectedRecord.Shift_Address__Street__s.trim() !== '' 
        ? selectedRecord.Shift_Address__Street__s 
        : selectedRecord.Add_Shift__r?.Location__Street__s && selectedRecord.Add_Shift__r?.Location__Street__s.trim() !== '' 
            ? selectedRecord.Add_Shift__r?.Location__Street__s 
            : '—';
    ...
    this.selectedRejectedShift = {
        ...
        // Participant
        participant: selectedRecord.Services_and_Support_Plans__r?.[0]?.Participant_Name__c || '—',

        // Rejection Reason
        rejectionReason: selectedRecord.RejectedComments__c || '—'
    };
