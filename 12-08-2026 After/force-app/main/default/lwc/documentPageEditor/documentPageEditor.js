...
wiredTemplates(result) {
  ...
  updateTemplatePagination() {
    ...
    const sorted = this.sortData(filtered, 'document');// manendra added for sorting table

    //this.templateListTotalRecords = filtered.length;
    this.templateListTotalRecords = sorted.length; // manendra added for sorting table
    ...
    this.templateListPaginated = sorted.slice(start, end); // manendra added for sorting table
...
updateCreatedTemplatePagination() {
    ...
    const sorted = this.sortData(filtered, 'created');// manendra added for sorting table
    // this.createdTemplateTotalRecords = filtered.length;
    this.createdTemplateTotalRecords = sorted.length;// manendra added for sorting table
    ...
    //this.createdTemplateListPaginated = filtered.slice(start, end).map(tpl => ({
      this.createdTemplateListPaginated = sorted.slice(start, end).map(tpl => ({ // manendra added for sorting table
        ...tpl,
        isMenuOpen: this.openMenuRowId === tpl.Id
    }));
...
/* updateArchivedPagination() {// manendra commented this function and added new one to sort data before pagination
  ...
  updateArchivedPagination() {
      ...
      const start =
          (this.archivedTemplatePageNumber - 1) *
          this.archivedTemplatePageSize;
      const end = start + this.archivedTemplatePageSize;
      this.archivedTemplateListPaginated =
          sorted.slice(start, end);
      this.disableArchivedFirst =
          this.archivedTemplatePageNumber === 1;
      this.disableArchivedLast =
          this.archivedTemplatePageNumber ===
          this.archivedTemplateTotalPages;
  }
handleArchivedNext() {
...
handleDownloadDocument(event) {
    ...
    const fileName = event.currentTarget.dataset.name || 'Document';
...
sortData(data, table) {
    ...
    return [...data].sort((a, b) => {
        ...
        const valueA = a[state.field];
        const valueB = b[state.field];
