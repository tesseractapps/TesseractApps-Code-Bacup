...
export default class SupportCoordinatorNotes extends LightningElement {
    ...
    async handleChange(event) {
        ...
        if (field === 'participant') {
            ...
            // Service Type Options from supportCatalogueGroups
            this.serviceTypeOptions =
        ...
        if (field === 'serviceType') {
            ...
            this.serviceItemOptions =
                (selectedGroup.Support_CoordinatorCatalogue__r || []).map(item => ({
                    label:
    ...
    handleDurationChange(event) {
        ...
        const hours = parseInt(updatedForm.durationHours, 10) || 0;
        const minutes = parseInt(updatedForm.durationMinutes, 10) || 0;
    ...
    handleDurationChange(event) {
        ...
        if (field === 'durationMinutes' && value > 59) {
            value = 59;
        }
        ...
        // Decimal hours for cost calculation
        const totalHours = hours + (minutes / 60);
    ...
    async loadSupportItems(participantId) {
        try {
            ...
            this.supportItemOptions = data.map(item => ({
                label:
                    item.SupportCatalogue_To_NDISSupportCatalogue__r
    ...
    async handleSave() {
        ...
        if (this.form.isBillable) {
            const totalBudget = Number(this.funding.totalBudget) || 0;
            const currentSpending = Number(this.funding.currentSpending) || 0;
            const entryCost = Number(this.entryCost) || 0;

            const remainingFunds = totalBudget - currentSpending;
            ...
            if (entryCost > remainingFunds) {
                this.showToast(
                    'Insufficient Funds',
                   // `Remaining budget is $${remainingFunds.toFixed(2)}, but this entry requires $${entryCost.toFixed(2)}`,
                    'Budget Allocation is required in Support Coordinator Services while creating a Participant.',
                    'error'
                );
                return;
            }
        ...
        try {
            ...
            console.log(
                '💰 Funding Payload:',
                JSON.stringify(this.funding, null, 2)
            );
            ...
            await saveTimeEntry({
                ...
                fundingJson: JSON.stringify(this.funding)
            });
