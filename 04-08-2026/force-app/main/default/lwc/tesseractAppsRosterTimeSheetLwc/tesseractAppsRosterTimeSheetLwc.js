...
import insertBulkTimeSheet from "@salesforce/apex/XeroIntegrationTimeSheet.insertBulkTimeSheet";
...
async checkMinimumEngagementRemoval(durationHours) {
  ...
  handleSendAnyway() {
      ...
      const staffIds = selectedStaff.map(
          item => item.staffId
      );

      const startDates = selectedStaff.map(
          item => item.suggestedStartDate
      );

      const endDates = selectedStaff.map(
          item => item.suggestedEndDate
      );
      ...
      .then((result) => {
          ...
          // IF ANY FAILURES -> SHOW MODAL AGAIN
          if (failed.length > 0) {
