...
import createSupportBudget from '@salesforce/apex/SupportBudgetController.createSupportBudget';
import getSupportBudgets from '@salesforce/apex/SupportBudgetController.getSupportBudgets';
import deleteSupportBudget from '@salesforce/apex/SupportBudgetController.deleteSupportBudget';
import updateSupportBudget from '@salesforce/apex/SupportBudgetController.updateSupportBudget';
import validateBudgetDates from '@salesforce/apex/SupportBudgetController.validateBudgetDates';
//import updateRecord from '@salesforce/apex/SupportBudgetController.updateRecord';
...
export default class SupportCoordinatorBudget  extends LightningElement {
  ...
  @track selectedServiceItem;
  @track selectedAmount;
  @track totalHours = 0;
  @track calculatedAmount = 0;
  @track budgets = [];
  @track isEditBudget = false;
  @track fileName;
  @track budgetId;
  @track actualAmount = 0;
  @track existingFilesJson = [];
  @track OrgNisationRoles = [];
  // @track existingUploadedFiles = [];
  recurrenceDates = [];
  ...
  connectedCallback() {
    ...
    console.log('Participant Id from parent:',this.participantId);
    this.fetchBudgets();
  ...
  async initializeComponent() {
    try {
      ...
      console.log('organizationDetails response:', JSON.stringify(orgResponse));

      console.log(
          'listofPriceBook:',
          JSON.stringify(orgResponse?.listofPriceBook)
      );

      // console.log(
      //     'Roles__c:',
      //     orgResponse?.listofPriceBook?.Roles__c
      // );
      this.organisationId = orgResponse?.listofPriceBook?.Id;
  ...
  handleDelete(event) {
      ...
      this.budgetId = event.currentTarget.dataset.id;
      this.DeleteFlag = true;
  }

  handleConfirmBudgetDelete(){
      deleteSupportBudget({budgetId: this.budgetId})
      .then(() => {
          this.showToast(
              'Success',
          ...
          this.deleteBudgetFlag = false;
          // REFRESH CARDS
          this.fetchBudgets();
          this.handleclose();
      })
      .catch(error => {
          console.error('Delete Budget Error:',JSON.stringify(error));
          this.showToast(
              'Error',
              error.body?.message ||
  ...
  get calendarDays() {
    ...
    this.budgets.forEach(budget => {const normalizedDate = new Date(budget.Budget_Date__c).toISOString().split('T')[0];

        if(!budgetMap.has(normalizedDate)){
    ...
    for (let i = 1; i <= totalDays; i++) {
      ...
      const budgetSlots = budgetMap.get(dateStr) || [];
      ...
      const combinedSlots = [
          ...
          ...budgetSlots.map((budget, index) => {

              console.log(`\n========== BUDGET SLOT ${index + 1} ==========`);
              console.log('🆔 Id:', budget.Id);
              console.log('📅 Budget_Date__c:', budget.Budget_Date__c);
              console.log('⏰ Start_Time__c (raw):', budget.Start_Time__c);
              console.log('⏰ Start Time (formatted):',this.convertMillisTo12Hour(budget.Start_Time__c));
              console.log('⏰ End_Time__c (raw):', budget.End_Time__c);
              console.log('⏰ End Time (formatted):',this.convertMillisTo12Hour(budget.End_Time__c));
              console.log('🛠️ Service_Type__c:', budget.Service_Type__c);
              console.log('🧾 Service_Item__r:',JSON.stringify(budget.Service_Item__r, null, 2));
              console.log('🧾 Service_Item_c:',budget.Service_Item__c);
              console.log('💰 Unit_Price__c:', budget.Unit_Price__c);
              console.log('💵 Total_Amount__c:', budget.Total_Amount__c);
              console.log('⌛ Total_Hours__c:', budget.Total_Hours__c);
              console.log('⌛ uploadedFiles:', budget.Amazon_file_URL__c);

              const mappedBudget = {

                  id: budget.Id,
                  startDate: budget.Budget_Date__c,
                  startTime: this.convertMillisTo12Hour(budget.Start_Time__c),
                  endTime: this.convertMillisTo12Hour(budget.End_Time__c),
                  // ✅ HERE SERVICE TYPE WILL SHOW
                  shiftType: budget.Service_Type__c,
                  selectedServiceType: budget.Service_Type__c,
                  selectedServiceItem: budget.Service_Item__c,
                  selectedAmount: budget.Unit_Price__c,
                  calculatedAmount: budget.Total_Amount__c,
                  actualAmount: budget.Actual_Amount__c,
                  totalHours: budget.Total_Hours__c,
                  availableClass: 'Budget',
                  uploadedFiles: budget.Amazon_file_URL__c,
                  invoiceNo: budget.Invoice_No__c,

                  timeRange:
                      `${this.convertMillisTo12Hour(budget.Start_Time__c)}
                      - ${this.convertMillisTo12Hour(budget.End_Time__c)}`,
              ...
              console.log(
                  '✅ Final Budget Object:',
                  JSON.stringify(mappedBudget, null, 2)
              );
      ...
      // console.log(
      //     '🎯 FINAL combinedSlots:',
      //     JSON.stringify(combinedSlots, null, 2)
      // );
      console.log('================================================');

      const hasMore = combinedSlots.length > displayLimit;
          ...
          combinedSlots
              ...
              .map(slot => {
                  ...
                  return {
                      ...
                      style:
                          `background-color:${colourWithAlpha};
                          color:${slot.colour};
                          border-left:4px solid ${slot.colour};
                          border-radius:6px;
                          width:100%;
                          text-align:left;
                          padding-left:2px;
      ...
      const moreCount = hasMore ? combinedSlots.length - displayLimit : 0;
...
this.availability.forEach((available) => {
    ...
    const dateKey =
        new Date(available.Start_Date__c)
            .toISOString()
            .split('T')[0];
    ...
    combinedMap.get(dateKey).push({
        ...
        shiftType:
            available.Shift_Type__c,

        shiftName:
            available.Shift_Name__c,

        startTimeFormatted:
            this.formatTime(
                available.Start_Time__c
            ),

        endTimeFormatted:
            this.formatTime(
                available.End_Time__c
            ),

        Assigned_Service_OR_Shift__c:
            available.Assigned_Service_OR_Shift__c,

        Start_Date__c:
            available.Start_Date__c
    });
});
      ...
      this.budgets.forEach((budget) => {
          ...
          const dateKey =
              new Date(budget.Budget_Date__c)
                  .toISOString()
                  .split('T')[0];
          ...
          combinedMap.get(dateKey).push({
              ...
              isBudget: true,
              shiftType: budget.Service_Type__c,
              serviceType:budget.Service_Type__c,
              serviceItemName: budget.Service_Item__c,
              selectedAmount: budget.Unit_Price__c,
              totalHours: budget.Total_Hours__c,
              calculatedAmount: budget.Total_Amount__c,
              actualAmount: budget.Actual_Amount__c,
              startTimeFormatted:this.formatTime(budget.Start_Time__c),
              endTimeFormatted:this.formatTime(budget.End_Time__c),
              Start_Date__c: budget.Budget_Date__c,
              uploadedFiles: budget.Amazon_file_URL__c,
              invoiceNo: budget.Invoice_No__c,
          });
      });

      const availableForDate = combinedMap.get(date) || [];
  ...
  async getserviceTypes() {
    try {
      const result = await getServiceType({ clientId: this.participantId });
  ...
  async handleAddDescription(event) {
    ...
    this.headingLabel = "Create Schedule";
    ...
    console.log('Groups:',JSON.stringify(participant.Support_CatalogueGroups__r));

    // SERVICE TYPES
    const groups = participant.Support_CatalogueGroups__r || [];

    this.serviceTypeOptions = groups.map(group => ({label: group.Name,value: group.Name}));

    console.log('serviceTypeOptions:',JSON.stringify(this.serviceTypeOptions));
  ...
  handleStartTimeChange(event) {
   ...
   // this.isSaveDisabled = !(this.AddShiftStartTimeAMPM && this.AddShiftEndTimeAMPM &&  !hasError);
  }

  handleEndTimeChange(event) {


      if (!event.detail || !event.detail.displaytime) {
        this.endTimeSelectedHour = null;
        this.endTimeSelectedMinute = null;
        this.endTimeAMPM = null;
        this.AddShiftEndTimeAMPM = null;
        this.AddShiftEndTime = null;
...
formatDisplayDate(dateString) {
  ...
  async handleEditClick(event) {
      try {
          console.log('================ HANDLE EDIT CLICK START ================');

          this.headingLabel = 'Edit Schedule';
          this.successmessage =
              'Availability shift updated successfully.';
          ...
          if (!event) {
              console.error('❌ Event is undefined/null');
              return;
          }
          ...
          if (!event?.currentTarget?.dataset) {
              console.error(
                  '❌ event.currentTarget.dataset is missing'
              );
              return;
          }
          ...
          const serviceItemName = dataset.serviceitemname;
          const unitPrice = dataset.unitprice;
          const totalHours = dataset.totalhours;
          const totalAmount = dataset.totalamount;
          const actualAmount = dataset.actualamount;
          const uploadedFiles = dataset.uploadedfiles;
          const invoiceNo = event.currentTarget.dataset.invoiceno;
          ...
          console.log('📌 serviceItemName:', serviceItemName);
          ...
          const groups =
              participant?.Support_CatalogueGroups__r || [];
          ...
          const starttimeDetails =
              this.splitTimeParts(startTime);

          console.log(
              '📌 starttimeDetails:',
              JSON.stringify(starttimeDetails)
          );

          this.startTimeSelectedMinute =
              starttimeDetails.minute;
          this.startTimeSelectedHour =
              starttimeDetails.hour;
          this.startTimeAMPM =
              starttimeDetails.period;

          this.AddShiftEndTimeAMPM = endTime;

          const endtimeDetails =
              this.splitTimeParts(endTime);

          console.log(
              '📌 endtimeDetails:',
              JSON.stringify(endtimeDetails)
          );

          this.endTimeSelectedMinute =
              endtimeDetails.minute;
          this.endTimeSelectedHour =
              endtimeDetails.hour;
          this.endTimeAMPM =
              endtimeDetails.period;
          ...
          this.existingFilesJson = uploadedFiles;
          ...
          /* if (uploadedFiles) {
              ...
              console.warn('⚠️ No uploaded files found');
              this.uploadedFiles = [];
          } */
          ...
          console.log('✅ Basic values assigned');

          // HANDLE ONCHANGE
          console.log('================ SERVICE TYPE CHANGE ================');
          console.log(
              '📌 Calling handleServiceTypeChange with:',
              serviceType
          );

          await this.handleServiceTypeChange({
              detail: {
                  value: serviceType
              }
          });

          console.log('✅ handleServiceTypeChange completed');

          // SERVICE ITEM
          setTimeout(() => {
              console.log(
                  '📌 Setting selected service item:',
                  serviceItemName
              );
          ...
          console.log('✅ Final UI flags updated');
          console.log('================ HANDLE EDIT CLICK END ================');

      } catch (error) {
          console.error(
              '❌ HANDLE EDIT CLICK ERROR:',
              error
          );
      }
  }
...
handleServiceTypeChange(event) {
    ...
    if(
        ...
        this.serviceItemOptions =
            selectedGroup
                .Support_Catalogues__r
                .map(item => ({

                    label: item.Support_Item_Name__c,
                    value: item.Support_Item_Name__c,
...
handleServiceItemChange(event) {
    ...
    if(
        !this.selectedGroup ||
        !this.selectedGroup.Support_Catalogues__r
    ){
        return;
    }
    ...
    // FETCH AMOUNT
    this.selectedAmount = selectedItem?.Amount__c || 0;
...
handleActualAmountChange(event) {
    console.log('================ ACTUAL AMOUNT CHANGE ================');

    console.log('📌 Raw Input Value:', event.target.value);

    this.actualAmount = parseFloat(event.target.value) || 0;

    console.log('✅ Updated actualAmount:', this.actualAmount);
}
handleInvoiceNoChange(event) {
    ...
    if (value.length > 12) {
        ...
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Warning',
                message: 'Invoice No cannot exceed 12 characters.',
                variant: 'warning'
            })
        );

        value = value.slice(0, 12); // or value.slice(0, 12)

        event.target.value = value;
    }

    this.invoiceNo = value;
    console.log( 'Updated invoiceNo:', this.invoiceNo );
}
...
calculateHoursAndAmount() {
    ...
    // START TIME
    let startHour = parseInt(this.startTimeSelectedHour, 10);
    const startMinute = parseInt(this.startTimeSelectedMinute, 10);
    // END TIME
    let endHour = parseInt(this.endTimeSelectedHour, 10);
    const endMinute = parseInt(this.endTimeSelectedMinute, 10);

    // CONVERT AM/PM
    if(this.startTimeAMPM === 'PM' &&startHour !== 12){
        startHour += 12;
      }

    if(this.startTimeAMPM === 'AM' &&startHour === 12){
        startHour = 0;
    }

    if(this.endTimeAMPM === 'PM' &&endHour !== 12){
        endHour += 12;
    }

    if(this.endTimeAMPM === 'AM' &&endHour === 12){
        endHour = 0;
    }
    ...
    // CALCULATE AMOUNT
    const unitPrice = parseFloat(this.selectedAmount || 0);
    this.calculatedAmount = (unitPrice *parseFloat(this.totalHours)).toFixed(2);
    console.log('Calculated Amount:',this.calculatedAmount);
}

async handlesavebutton() {
    console.log('================ HANDLE SAVE BUTTON START ================');

      if (!this.selectedServiceType) {
      this.showToast('Validation Error', 'Service Type is mandatory.', 'error');
      return;
  }

  if (!this.selectedServiceItem) {
      this.showToast('Validation Error', 'Service Item is mandatory.', 'error');
      return;
  }

  if (!this.AddShiftStartTimeAMPM) {
      this.showToast('Validation Error', 'Start Time is mandatory.', 'error');
      return;
  }

  if (!this.AddShiftEndTimeAMPM) {
      this.showToast('Validation Error', 'End Time is mandatory.', 'error');
      return;
  }
    ...
    try {
      if (this.AddShiftRecurringCheckboxValue) {
        ...
        const validationResult = await validateBudgetDates({
            participantId: this.participantId,
            recurrenceDates: this.recurrenceDates,
            isEditMode: this.isEditBudget,
        ...
        Object.entries(validationResult).forEach(([dateString, reason]) => {
            this.budgetDateConflicts.push({
        ...
        if (this.budgetDateConflicts.length > 0) {
      ...
      console.log('📌 isEditBudget:', this.isEditBudget);
      ...
      console.log('📌 selectedServiceItem:', this.selectedServiceItem);
      console.log('📌 selectedAmount:', this.selectedAmount);
      console.log('📌 totalHours:', this.totalHours);
      console.log('📌 calculatedAmount:', this.calculatedAmount);
      ...
      const convertedBudgetDate =
          this.convertDateToISO(this.todayDate);

      console.log('📌 Converted Budget Date:', convertedBudgetDate);
      // for (const budgetDate of this.recurrenceDates) {
      //   console.log('📌 budgetDate:', budgetDate);
      // }

      if (this.isEditBudget) {
          console.log('================ UPDATE BUDGET FLOW ================');

          const updatePayload = {
              budgetId: this.budgetId,
              serviceType: this.selectedServiceType,
              serviceItem: this.selectedServiceItem,
              unitPrice: parseFloat(this.selectedAmount),
              totalHours: parseFloat(this.totalHours),
              totalAmount: parseFloat(this.calculatedAmount),
              actualAmount: parseFloat(this.actualAmount) || 0,
              startTime: this.AddShiftStartTimeAMPM,
              endTime: this.AddShiftEndTimeAMPM,
            //  budgetDate: convertedBudgetDate,
             recurrenceDates: this.recurrenceDates,
              amazonFileUrl: JSON.stringify({
          ...
          updateSupportBudget(updatePayload)

              .then(() => {
                  console.log('✅ updateSupportBudget SUCCESS');
                  ...
                  console.log(
                      '📡 Calling getSupportBudgets for participant:',
                      this.participantId
                  );

                  return getSupportBudgets({
                      participantId: this.participantId
                  });
              })

              .then(result => {
                  console.log('✅ getSupportBudgets SUCCESS');
              ...
              .finally(() => {
                  console.log('🔄 UPDATE FLOW FINALLY BLOCK');
                  ...
                  console.log(
                      '================ HANDLE SAVE BUTTON END ================'
                  );
              });

      } else {
          console.log('================ CREATE BUDGET FLOW ================');
          ...
          createSupportBudget(createPayload)

              .then(() => {
                  console.log('✅ createSupportBudget SUCCESS');
              ...
              .finally(() => {
                  console.log('🔄 CREATE FLOW FINALLY BLOCK');
        ...
        this.showToast(
            'Error',
            error?.body?.message || 'Failed to save budget',
            'error'
        );
...
hanleRecurShiftsCreation() {
    ...
    // Dates shown in popup
    const conflictDates = this.budgetDateConflicts.map(
        item => item.dateString
    );

    console.log(
        'Conflict Dates:',
        JSON.stringify(conflictDates)
    );

    // Keep only non-conflicting dates
    this.recurrenceDates = this.recurrenceDates.filter(
        date => !conflictDates.includes(date)
    );

    console.log(
        'Valid Recurrence Dates:',
        JSON.stringify(this.recurrenceDates)
    );
    ...
    if (this.recurrenceDates.length === 0) {
        ...
        this.showToast(
            'Error',
            'All recurrence dates already have budgets.',
            'error'
        );
    ...
    // Continue save
    this.saveBudgetAfterConflictCheck();
}
async saveBudgetAfterConflictCheck() {
    ...
    if (this.isEditBudget) {
        ...
        await updateSupportBudget(updatePayload);
        ...
        await createSupportBudget(createPayload);
    }

    const result = await getSupportBudgets({
        participantId: this.participantId
    });
...
fetchBudgets(){
    getSupportBudgets({participantId: this.participantId})
    .then(result => {console.log('Fetched Budgets:',JSON.stringify(result));
        this.budgets = result;
    })

    .catch(error => {console.error('Fetch Budget Error:',JSON.stringify(error));});
}
...
onFileUpload(event) {
  ...
  handleDeleteFile(event) {
    ...
    console.log('Updated file list:', JSON.stringify(updatedFiles));
...
// deleteBudgetFile() {
//    const amazonFileUrl = JSON.stringify({
//         files: this.uploadedFiles
//     });
//     console.log('Updated file list:', JSON.stringify(amazonFileUrl));
//     updateRecord({
//         budgetId: this.budgetId,
//         amazonFileUrl: amazonFileUrl
//     })
//         .then(() => {
//             console.log('✅ Backend updated after delete');
//         })
//         .catch(error => {
