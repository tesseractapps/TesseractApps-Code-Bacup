...
export default class MyProfile extends NavigationMixin(LightningElement){
    ...
    recordsPerPage = 10;
    isPageSizeManuallySet = true;
    pageSizeBulk = 5;
    isPageSizeBulkManuallySet = true;
    resizeObserver;
...
@track LANGUAGE_OPTIONS = [
  ...
  @track selectedLangs = [];
    ...
    connectedCallback(){
        ...
        if (this.bellNotification) {
        this.sectionFlags = {
            ...this.sectionFlags,
            staffDocumentation: true,
           // staffDocumentation1: true
        };
        this.sectionIcons = {
        ...this.sectionIcons,
        staffDocumentation: { ...ICON_DOWN },
    ...
    renderedCallback() {
        if (!this.resizeObserver) {
            const container = this.template.querySelector('.docTable, .documentTable');
            if (container) {
    ...
    setPageSizeByZoomAndScreen() {
        ...
        if (availableHeight > 0 && rowHeight > 0) {
            let computedPageSize = Math.floor(availableHeight / rowHeight);
            if (computedPageSize < 1) {
                computedPageSize = 1;
            }
            
            if (this.pageSize !== computedPageSize) {
    ...
    fetchStaff({ recordId: this.staffId })
        .then(result => {
            ...
            if (data) {
                ...
                this.DateOfBirth = this.clientData[0].Date_Of_Birth__c;
                ...
                console.log('this.ictUserType',this.ictUserType)
                ...
                childRec.forEach(rec => {
                    const status = rec.Status__c || 'Pending';
                     const complianceValue = this.formatBoolean(rec.Compliance__c);
                    // const statusClass =
                    //     status === 'Approved'
                    ...
                    if (rec.Document_Type__c) {
                        this.DocumentTableData.push({
                            ...
                            //         : '',
                            // ✅ TRUE/FALSE → YES/NO
                            ComplianceFormatted: complianceValue,

                            // ✅ BADGES
                            statusClass: this.getStatusClass(rec.Status__c),
                            complianceClass: this.getComplianceClass(complianceValue)
                        });
                    }
                });

                const contacts = data.Staff_Contacts__r || [];

                if (contacts.length > 0) {
                    this.contactList = contacts.map(contact => ({
                        id: contact.Id,
                        recordId: contact.Id,
                        ...
                        contactNumber: contact.Phone__c || '',
                        email: contact.Email__c || '',
                        contactType: contact.Name || '', // "Emergency"
                        notify: false,
                    ...
                    // ✅ Add one empty row at the end (optional but recommended)
                    this.contactList.push(this.getEmptyContactRow());

                } else {
                    // fallback if no contacts exist
                    this.contactList = [this.getEmptyContactRow()];
                }
...
//manendra
    ...
    handleSubmit(event){
        ...
        const contactField = this.template.querySelector('[data-id="contact"]');
        console.log('Contact Field:', contactField);

        if (contactField) {
            console.log('Contact Value:', contactField.value);

            const contact = (contactField.value || '').replace(/\D/g, '');
   ...
   handleError(event) {
    ...
    //     const currentPageSize = this.pageSize || 10;
    ...
    return fetchStaff({ recordId: this.staffId }).then(response => {
        ...
        childRec.forEach(rec => {
            ...
            const complianceValue = this.formatBoolean(rec.Compliance__c);
            // const statusClass =
            //     status === 'Approved' ? 'status-pill status-approved' :
            //     status === 'Rejected' ? 'status-pill status-rejected' :
            //     'status-pending';
    ...
    handleRowActions(event) {
       ...
       //const withoutPoints = rawType.split(' * ')[0];

        // Step 2: take only document name before " - "
        this.typeOfDocument = rawType;
        ...
        switch (actionName) {
            ...
            */
            ...
            case 'view_details':
                ...
                console.log("=== Navigation Handler Triggered ===");
                console.log("event:", event);
                console.log("URL:", url);
                console.log("employeeeditflag:", this.employeeeditflag);
                event.preventDefault();
                this.isHome = false;
                console.log("isHome:", this.isHome);

                this.archiveModal = false;
                console.log("archiveModal:", this.archiveModal);
    ...
    handlePreTaxSuccess(event) {
        ...
        // 🔹 CALL APEX TO SET STATUS = PENDING
...
event.stopPropagation(); // Prevent triggering the outside click listener when clicking inside the grid
    ...
    //staff documentation new implementation
     ...
     ALLOWED_FILE_EXTENSIONS = ['pdf', 'png', 'jpg', 'jpeg','docx','doc','heic', 'jpe', 'odt', 'rtf', 'txt'];
 ...
 handleFileDeleted(event) {
   ...
   /*  handleDocTypeChange(event) {
    ...
    //     console.log('🔵 [handleDocTypeChange1] Triggered');
    //     console.log('➡️ Selected Value:', selectedType);
        

    //     // ✅ Assign full value
    //     this.typeOfDocument = selectedType;

    //     // 🔍 Extract points from string (since node doesn't have points)
    //     let extractedPoints = 0;
    //     const match = selectedType.match(/\((\d+)\s*points\)/i);

    //     if (match) {
    //         extractedPoints = parseInt(match[1], 10);
    //         console.log('✅ Points extracted from string:', extractedPoints);
    //     } else {
    //         console.warn('⚠️ Points not found in value string:', selectedType);
    //     }

    //     this.points = extractedPoints;

    //     // 🔍 Normalize key for meta lookup (IMPORTANT FIX)
    //     const normalizedKey = selectedType.split(' * ')[0].trim();

    //     console.log('🔑 Normalized Meta Key:', normalizedKey);
    //     console.log('🗂️ Available Meta Keys:', Object.keys(this.documentMetaMap));

    //     const meta = this.documentMetaMap[normalizedKey];
    ...
    //     } else {
    //         console.warn('⚠️ No meta found for key:', normalizedKey);
    //     }

    //     // 🔍 Final logs
    //     console.log('🎯 Assigned Type:', this.typeOfDocument);
    //     console.log('📊 Final Points:', this.points);
    //     console.log('📅 Expiry Required:', this.isExpiryRequired);

    //     const docMeta = this.documentMetaMap?.[this.typeOfDocument];

    //     console.log('➡️ Selected Value:', docMeta);

    //     const isExpiryRequired = docMeta && docMeta.expiryRequired === true;

    //     console.log('🟢 [handleDocTypeChange1] Completed');
    // }
   ...
   /*  openRejectedCommentsFromLink(docId) {
    ...
    /* buildHierarchicalFromFacility(result) {
        ...
        const getCategory = (doc) => {
            return doc.Identity_Category__c 
                || doc.Documents_Category__c 
                || doc.Training_Category__c 
                || '';
        };

        const buildLabel = (doc, displayName) => {
            const category = getCategory(doc);

            return withMandatoryMark(doc, displayName) +
        ...
        identitySections.push({
            ...
            children: groupedIdentity.Primary.length
                ? groupedIdentity.Primary.map(doc => {
                    ...
                    return {
                        id: `id-${doc.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
                        label: buildLabel(doc, displayName),
                        value: buildLabel(doc, displayName),
                        points: doc.points || 0,
                        children: []
                    };
                })
...
get docTableClass() {
    return this.ictUserType
        ? 'table-container staffTable docTable no-status'
        : 'table-container staffTable docTable';
}
    ...
    getComplianceClass(value) {
        if (value === 'Yes') return 'status-pill status-approved';
        if (value === 'No') return 'status-pill status-rejected';
        return 'status-pill status-default';
    }


    get documentTableClass() {
        return this.ictUserType
            ? 'table-container documentTable no-status'
            : 'table-container documentTable';
    }
    ...
    handleArchiveSearch(event) {
        ...
        this.archivedRecords =
            this.allArchivedRecords.filter(doc =>
                doc.Name &&
