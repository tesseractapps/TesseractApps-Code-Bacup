 ...
 */
  ...
  async getserviceTypes() {
    ...
    const financialYear = this.getFinancialYear(this.todayDate);// manendra added financial year to get service type based on financial year 2024-06-12

console.log('todayDate =>', this.todayDate);
console.log('financialYear =>', financialYear);
    getServiceType({ clientId: this.clientId, financialYear: financialYear })
      .then((result) => {
...
//manendra added for financial year 2024-06-12
...
console.log('--- Resetting Time-Related Fields (END) ---');
  ...
  _syncRoute(replace = false) {
    let subView = "";
    if (this.editButtonModule) {
      const isNew = this.buttonTitle === "Create Shift";
      const isView = this.buttonTitle === "View Shift";
      if (isNew) {
        subView = "add-shift";
      } else {
        const uid = this._getScheduleUid(this.availabilityId);
        subView = isView ? `${uid}/view` : `${uid}/edit`;
      }
    } else if (this.DeleteFlag && this.availabilityId) {
      const uid = this._getScheduleUid(this.availabilityId);
      subView = `${uid}/delete`;
    } else if (this.isPopoverVisible && this.selectedavailability && this.selectedavailability.length > 0) {
      const firstShift = this.selectedavailability[0];
      const uid = firstShift.Schedule_UID__c || firstShift.Id;
      subView = `${uid}`;
    }

    console.log("[Routing] participantAvailabilityLwc dispatching subrouteupdate:", subView);
    this.dispatchEvent(
      new CustomEvent("subrouteupdate", {
        detail: {
          subView,
          replace
        },
  ...
  _getScheduleUid(id) {
    const record = this.availability.find(r => r.Id === id);
    return record ? record.Schedule_UID__c : id;
  }
  ...
  processSubroute(subroute) {
    ...
    if (subroute === "add-shift") {
      ...
      const uid = parts[0];
      const action = parts[1] || "popup";

      if (uid.startsWith("sch-") || /^[a-z0-9]{15}$|^[a-z0-9]{18}$/i.test(uid)) {
        if (this.availability && this.availability.length > 0) {
          const record = this.availability.find(r => r.Schedule_UID__c === uid || r.Id === uid);
          if (record) {
            this.availabilityId = record.Id;
            
            if (action === "edit") {
              this.isFormDisabled = false;
              this.successmessage = "Shift updated successfully.";
              this._loadEditDetails(record.Id);
              this.buttonTitle = "Update Shift";
              this.isCreateMode = false;
              this.isUpdateMode = true;
            } else if (action === "view") {
              this.isFormDisabled = true;
              this.successmessage = "Shift details.";
              this._loadEditDetails(record.Id);
              this.buttonTitle = "View Shift";
              this.isCreateMode = false;
              this.isUpdateMode = false;
              ...
              const targetDate = record.Start_Date__c;
              const availableForDate = this.availability.filter(a => a.Start_Date__c === targetDate);
              this.selectedavailability = availableForDate.map(available => ({
                ...available,
                formattedStart: this.formatTime(available.Start_Time__c),
                formattedEnd: this.formatTime(available.End_Time__c)
              ...
              const servicesForDate = this.services.filter(s => s.Date_of_Service__c === targetDate);
              this.selectedServices = servicesForDate.map(service => ({
                ...service,
                formattedStart: this.formatTime1(service.start_datetime__c),
                formattedEnd: this.formatTime1(service.end_datetime__c)
  ...
  _loadEditDetails(recordId) {
    this.quantity = '';
    
    const record = this.availability.find(r => r.Id === recordId);
    if (!record) return;

    this.todayDate = record.Start_Date__c;
    this.shiftTypeValue = record.Shift_Type__c;
    this.quantity = record.Quantity__c;
    this.clientFacilityId = record.Facility__c ? record.Facility__c : (this.participantFaciltiyOptions && this.participantFaciltiyOptions.length > 0 ? this.participantFaciltiyOptions[0].value : '');
    this.shiftNameValue = record.Shift_Name__c;
    
    this.AddShiftStartTimeAMPM = this.formatTime(record.Start_Time__c);
    ...
    this.AddShiftEndTimeAMPM = this.formatTime(record.End_Time__c);
