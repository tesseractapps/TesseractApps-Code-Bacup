        ...
        facRoles =>{
          ...
          if (this.OrgNisationRoles.length > 0) {
              ...
              if (this.SelctedComboBoxRole === 'All') {
                  // Push all role values except "All" from OrgNisationRoles
...
/*   loadStaffData() {
 ...
 // this.staffData = [];
 // this.isShowSpinner = true;
  // REMOVE the setTimeout that hides spinner - let wired function handle it
...
/* refreshStaffData() {
  return new Promise((resolve) => {
    ...
    refreshApex(this.wiredStaffData).then(() => {
      setTimeout(() => {
        // Double check if data loaded, if not retry
        if (this.staffData.length === 0) {
...
async getShiftTimingsByFacility(shifNames) {
 ...
 async handleShiftDeleteConfirmation(event) {
    ...
    let servResult= await getSeriveList({shiftStaffId:this.shiftDeleteCOnfirmationInfo.shiftID});
  ...
  async processShifts(facilityId) {
    try {
      ...
      this.shiftNameOptions = data.map((option) => ({
       ...
       /*  typeOfAddress:option.Type_of_Address__c?option.Type_of_Address__c:'' */
      }));
      console
