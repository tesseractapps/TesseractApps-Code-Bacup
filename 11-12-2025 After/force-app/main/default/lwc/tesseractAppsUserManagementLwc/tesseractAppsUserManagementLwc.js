...
export default class TesseractAppsUserManagement extends LightningElement {
    ...
    handleDownloadTemplate(event){
        ...
        if(this.typeOfUser=='NDIS'){
            const fileUrl = '/resource/User_Upload_NDIS_Template';
            window.open(fileUrl, '_blank');
            
        } else if(this.typeOfUser=='ICT'){
            const fileUrl = '/resource/ICT_User_Template';
            window.open(fileUrl, '_blank');
        }
    }
    ...
    handleInstructions(event){
        console.log('type of user: ',this.typeOfUser);
        if(this.typeOfUser=='NDIS'){
            const fileUrl = '/resource/Bulk_Upload_Instructions_For_User';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
            
        } else if(this.typeOfUser=='ICT'){
            const fileUrl = '/resource/ICT_User_Bulk_Instructions';  // Update the file URL if necessary
            window.open(fileUrl, '_blank');
        }   
    }
    ...
    /* parseCSV(data) {
        ...
        let requiredHeaders = [
            ...
            'DateofBirth', 'Role', 'UserType', 'Facility', 'HourlyRate', 'AustralianBusinessNumber',
    ...
    parseCSV(data) {
        ...
        // 4. Default required headers
        let requiredHeaders = [
            'FirstName', 'LastName', 'Gender', 'PhoneNumber', 'EmailAddress',
            'DateofBirth', 'Role', 'UserType', 'Facility',
            'AustralianBusinessNumber', 'TaxFileNumber', 'TaxFreeThreshold',
            'StartDate', 'Status', 'AddressStreet', 'AddressCity',
            'AddressState', 'AdressPostCode', 'AddressCountry', 'ManagementFee',
            'Languages', 'Nationality', 'Frequency'
        ];

        // 4A. ICT USER: Add HourlyRate, remove Languages & Nationality
        if (this.typeOfUser === 'ICT') {
            requiredHeaders.push('HourlyRate');
            requiredHeaders = requiredHeaders.filter(h => h !== 'Languages' && h !== 'Nationality');
        }

        // ❗ NOTE:
        // NDIS DOES NOT REQUIRE HOURLYRATE
        // So NO change needed for NDIS — HourlyRate stays optional

        // 5. Missing required columns check
        for (let required of requiredHeaders) {
            const exists = headerRow.some(h => h.replace(/ /g, '') === required);
            if (!exists) {
                this.dispatchEvent(
        ...
        // 7. NDIS Role validation
        if (this.typeOfUser === 'NDIS') {
            const validRoles = ['Org Admin', 'Staff', 'Roster Admin'];
            const validRosterAdminTypes = [
                'Roster Manager', 'HR Admin', 'Payroll Admin', 'NDIS Participant',
                'Payroll Accountant for Multiple', 'Accountant for Organisation'
            ];
            ...
            for (let record of parsedData) {
                if (!validRoles.includes(record.Role)) {
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Invalid Role',
                            message: `Invalid Role for NDIS user: "${record.Role}". Allowed: ${validRoles.join(', ')}`,
                            variant: 'error'
                        })
                    );
                    return [];
                ...
                // Org Admin or Facility Admin user types
                if (record.Role === 'Org Admin' &&
                    record.UserType !== 'NDIS Org Admin' &&
                    record.UserType !== 'NDIS FacilityAdmin') {
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Invalid Role / User Type',
                            message: `For NDIS, Org Admin must be "NDIS Org Admin" or "NDIS FacilityAdmin".`,
                            variant: 'error'
                        })
                    );
                    return [];
            ...
            for (let record of parsedData) {
                if (!validRoles.includes(record.Role)) {
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Invalid Role',
                            message: `Invalid Role for ICT user: "${record.Role}". Allowed: ${validRoles.join(', ')}`,
                            variant: 'error'
                        })
                    );
                    return [];
                ...
                if (record.Role === 'Org Admin' && !validOrgAdminTypes.includes(record.UserType)) {
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Invalid Role / User Type',
                            message: `For ICT Org Admin, UserType must be one of: ${validOrgAdminTypes.join(', ')}`,
                            variant: 'error'
                        })
                    );
                    return [];
    ...
    handleFinalSubmit() {
        ...
        // -------------------------------------------------------
        if (this.typeOfUser === 'ICT') {
            // Add HourlyRate only for ICT
            requiredFields.push({ key: 'HourlyRate', label: 'Hourly Rate' });

            // ICT does NOT require Languages or Nationality
        ...
        this.tableData = this.tableData.map(user => {
            if (user.isSelected && user.ProcessedStatus !== 'Success ✅') {
                ...
                // 2️⃣ Skip ABN/TFN if missing fields already exist
                if (user.ProcessedStatus && user.ProcessedStatus.startsWith('Error ❌: Missing')) {
                    return user;
                }

                // 3️⃣ ABN / TFN validation
        ...
        try {
            validRecords = unprocessedRecords
                .filter(user => requiredFields.every(field => !!user[field.key]))
                .map(user => {
        ...
        return createAccountsContactsAndStaff({ records: formattedRecords })
            .then(result => {
                ...
                this.tableData = this.tableData.map(user => {
                    ...
                    if (successRecord) {
                        return { ...user, isSelected: false, ProcessedStatus: 'Success ✅' };
                    }
                    if (failedRecord) {
            ...
            .catch(error => {
                ...
                throw error; // REQUIRED so handleFinalSubmit() knows error happened
            });
    }
    ...
    handleOutsideClick(event) {
        ...
        // Find the dropdown container in the template
        const container = this.template.querySelector('[data-id="languageDropdown"]');

        // If container not found, don't randomly close
        if (!container) {
            return;
        }
