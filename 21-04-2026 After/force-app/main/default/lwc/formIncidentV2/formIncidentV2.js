...
buildResponsePayloadFromTemplate() {
  ...
  payload.forEach((row, rowIndex) => {
    ...
    row.cells.forEach((cell, cellIndex) => {
      ...
      if (isDropdown) {
        ...
        if (isMultiSelect) {
          ...
          if (uiCell.field.options?.length) {
            ...
            // 👇 keep SAME existing format (comma string)
            finalValue = selectedLabels.join(", ");
        ...
        // ====================================================
        // ✅ SINGLE SELECT (EXISTING LOGIC PRESERVED)
        // ====================================================
        else {
      ...
      // -----------------------------
      // ✅ Upload metadata only (UNCHANGED)
      // -----------------------------
...
//   // ⭐ UPDATE SOURCE OF TRUTH (stepPagedRows)
//   (this.stepPagedRows || []).forEach((page, pIdx) => {
//     (page || []).forEach(row => {
//       (row.cells || []).forEach(cell => {
//         if (String(cell.id) === String(this.activeSignatureCellId)) {
//           console.log('✅ Updating cell in stepPagedRows:', cell.id);
