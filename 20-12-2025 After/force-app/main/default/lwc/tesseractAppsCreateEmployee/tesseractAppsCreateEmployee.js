...
export default class TesseractAppsCreateEmployee extends NavigationMixin(
  ...
  handleNext1() {
    ...
    if (!this.ictUserType){
      const hasSelectedRole = this.roleoptionsforFacility?.some(opt => opt.checked);
      if (!hasSelectedRole) {
          isValid = false;
      ...
      /* if (!this.selectedLangs || this.selectedLangs.length === 0 || !this.selectedLangs) {
          isValid = false;
          this.showToast('Error', 'Please select at least one Language', 'error');
        }
      if (!this.Nationality || this.Nationality.length === 0 || !this.Nationality) {
          isValid = false;
          this.showToast('Error', 'Please select a Nationality', 'error');
        } */
    }
    if (isValid) {
  ...
  @wire(fetchFacilitiess, {
    recordId: "$selectedName",
    firstname: "$firstname",
    lastname: "$lastname"
  })
  recordsToDisplay(result) {
    console.log("RESULT IN   this.wiredresult --> ");
            ...
            if (userType == "NDIS Org Admin" || userType == "ICT Admin") {
              ...
              getFacilityCurrentUser()
                .then((result) => {
                  ...
                  this.orginalData = filteredData;
                  console.log(
                    "filteredData--> " + JSON.stringify(this.records)
                  );
                  console.log("filteredData LENGTH--> " + this.records.length);
                  console.log(
                    "filteredData--> " + JSON.stringify(this.records2)
                  );
    ...
    const storedStaffId = localStorage.getItem('hrStaffRecordId');
  ...
  handleTogglestaff() {
    ...
    if (this.filterState === "Active") {
      ...
      this.filteredRecords = this.records2.filter(
  ...
  handleFaciltyChange(event) {
    /*   praveen changes for nursing start*/
      ...
      /*  praveen changes for nursing awards end*/
       this.fetchRolesForFacility(this.StaffFacility);
  }

 displayText = 'Select a Role'; 
// toggleDropdown(e) {
//         e.stopPropagation();
...
//     }
    ...
    // fetchRolesForFacility(facilityId) {
    //     if (!facilityId) {
    //         console.warn('No facilityId provided, skipping role fetch.');
    //         return;
    //     }

    //     console.log('Fetching active roles for Facility:', facilityId);

    //     getRoleOptionsByFacility({ facilityId })
    //         .then(result => {
    //             console.log('Roles received from Apex:', JSON.stringify(result));

    //             this.roleoptionsforFacility = result.map((roleName, index) => ({
    //                 id: index,
    //                 label: roleName,
    //                 buttonClass: 'option-button',
    ...
    //                 isDisabled: false
    //             }));
    //           const firstActive = this.roleoptionsforFacility.find(opt => opt.checked);
    //           this.displayText = firstActive ? firstActive.label : 'Select a Role';
    //             console.log('roleoptionsforFacility:', JSON.stringify(this.roleoptionsforFacility));
    //         })
    //         .catch(error => {
...
fetchRolesForFacility(facilityId) {
    if (!facilityId) {
        console.warn('⚠️ No facilityId provided, skipping role fetch.');
        return;
    }
    ...
    getRoleOptionsByFacility({ facilityId })
        .then(result => {
            console.log('✅ Roles received from Apex:', JSON.stringify(result, null, 2));
            ...
            if (result && result.length > 0) {
                this.roleoptionsforFacility = result.map((roleName, index) => ({
                    ...
                    buttonClass: this.getOptionButtonClass ? this.getOptionButtonClass(false) : 'option-button option-button-inactive',
                    badgeClass: this.getBadgeClass ? this.getBadgeClass(false) : 'status-badge1 status-badge-inactive1',
                    statusText: 'Inactive',
                    isDisabled: false
                }));

                const firstActive = this.roleoptionsforFacility.find(opt => opt.isActive);
                this.displayText = firstActive ? firstActive.label : 'Select a Role';

                console.log('🎯 roleoptionsforFacility after mapping:', JSON.stringify(this.roleoptionsforFacility, null, 2));
            } else {
                this.roleoptionsforFacility = [];
                this.displayText = 'No Roles Found';
            }
        })
        .catch(error => {
            console.error('❌ Error fetching roles:', error);
            this.showToast('Error', 'Failed to load roles for this Facility', 'error');
        });
}
    ...
    updateSelectedRoles() {
        console.log('🔹 [updateSelectedRoles] START');

        // 1️⃣ Show the current state of all role options
        console.log('🧩 roleoptionsforFacility (full list):', JSON.stringify(this.roleoptionsforFacility, null, 2));

        // 2️⃣ Filter out only checked (selected) roles
        const checkedRoles = this.roleoptionsforFacility.filter(option => option.checked);
        console.log('✅ Filtered checked roles:', JSON.stringify(checkedRoles, null, 2));

        // 3️⃣ Extract role names (label) from checked roles
        this.selectedRoleValues = checkedRoles.map(option => option.label);
        console.log('🎯 Selected Role Labels:', JSON.stringify(this.selectedRoleValues, null, 2));

        // 4️⃣ Prepare label-value pairs for UI or Apex
        this.selectedRoleEmploymentValues = this.selectedRoleValues.map(val => ({
            label: val,
            value: val
        }));
        console.log('🧾 Selected Role Employment Values:', JSON.stringify(this.selectedRoleEmploymentValues, null, 2));

        console.log('🔹 [updateSelectedRoles] END');
    }
    ...
    //                 badgeClass: isChecked ? 'status-badge active' : 'status-badge inactive'
    //             };
    //         }
    //         return opt;
    //     });
    //     console.log('🔄 Updated role list:', JSON.stringify(this.roleoptionsforFacility));
...
{ label: 'Occupational Health Nurse - level 3', value: 'Occupational Health Nurse - level 3' }
  ...
  handleSubmit(event) {
    ...
    fields.SelectedRole__c = this.selectedRole;//manendra
   ...
   // fields.Role__c = this.createdShiftRole;
    ...
    fields.Voluntary_Contribution_Fixed__c =
      this.voluntaryContributionCureencyVal;
    fields.Voluntary_Contribution_Percent__c =
      this.voluntaryContributionPercentVal;
  ...
  handleSuccess(event) {
    ...
    createStaffRoles({ 
           ...
           dropdownSelectedRole: this.selectedRole || '',
       ...
       .then(() => {
           console.log(' StaffRoles updated successfully in Apex');
            ...
            console.log(' sleepover',this.classificationPayType);
  ...
  employmentTypeChange(event) {
    ...
    if (
      ...
      /* getShadAwards({
        categoryType: this.categoryType,
        jobType: this.jobType,
        classificationType: this.classificationLevel,
        classificationPayType: this.classificationPaytype
      }).then((result) => {
  ...
  applyFilters() {
    ...
    if (this.activeFilterOn && !this.inactiveFilterOn) {
      result = result.filter(
        (staff) => staff.Status__c === true || staff.Status__c === "true"
      );
    } else if (this.inactiveFilterOn && !this.activeFilterOn) {
      result = result.filter(
        (staff) => staff.Status__c === false || staff.Status__c === "false"
      );
    }

    this.records = result;
    this.filteredRecords = [...result];
     console.log('ManendrRecord',JSON.stringify(this.recordsWithToggleStyle) );
  ...
  hierarchicalOptions = [
        ...
        children: [
            ...
            },
            {
                ...
                children: [
                    ...
                    { id: 'creditcard/debitcard', label: 'Credit Card/Debit Card (25pts)', value: 'Credit Card/Debit Card', parent: 'secondary-id' }
                ]
            }
        ]
    },
    {
        ...
        children: [
            ...
            { id: 'contract', label: 'Contract Agreement', value: 'Contract Agreement', parent: 'other-docs' },
