         ...
         if (key1) {
            data = data.filter(group =>
                ...
                group.Child_Staffs__r.some(doc =>
                    ...
                    (doc.Type__c || '').toLowerCase().includes(key1)
                )
            );
        }
        if (key2) {
...
/* applyFilters() {
    ...
    if (this.isStaffView) {
        ...
        if (this.selectedDocType !== 'All') {
            data = data.filter(staff =>
                staff.Child_Staffs__r?.some(doc =>
                    (doc.Type__c || '').toLowerCase() === this.selectedDocType.toLowerCase()
                )
            );
        }
    }
...
@track typeOfDocument;
   ...
   /*  groupByDocumentType() {
    ...
    this.records.forEach(staff => {
        staff.Child_Staffs__r?.forEach(doc => {
            if (!groups[doc.Type__c]) groups[doc.Type__c] = [];
            
            groups[doc.Type__c].push({
                ...doc,
    ...
    // Iterate original staff records and group their child docs by Type__c
