...
handleSearch(event) {
    ...
    if (searchKey) {
        result = result.filter(record => {
            ...
            return nameMatch || contactMatch || roleMatch;
        });
    }

    // ✅ Facility filter (apply only selected facilities)
    if (this.selectedFacilities && this.selectedFacilities.length > 0) {
        ...
        result = result.filter(record => selectedFacilityIds.includes(record.Facility__r?.Id));
    }
