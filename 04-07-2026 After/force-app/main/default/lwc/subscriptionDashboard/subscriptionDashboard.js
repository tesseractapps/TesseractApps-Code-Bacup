...
import deleteFilesFromAwsJson from '@salesforce/apex/AwsBatchController.deleteFilesFromAwsJson';
...
export default class SubscriptionDashboard extends LightningElement {
    ...
    try {
        ...
        const result = await organizationDetails();

        console.log('🏢 ORG DETAILS:', JSON.stringify(result));

        this.orgData = result;

        // Set storage limit from Organisation__c
        if (result?.listofPriceBook?.Storage_Limit__c) {
           this.storageLimitAWS =result?.listofPriceBook?.Storage_Limit__c ?? 0;
        }

        // After org data is loaded, load AWS usage
        this.loadStorageFromAWS(result.listofPriceBook.Id);
...
async loadAgreements() {
    ...
    try {
        ...
        this.agreements = result.map(ag => ({
            ...
            name: ag?.Name || '',
            fileUrl: ag?.Billing_Agreement_Attachments__r?.[0]?.AWS_URL__c || '',
            awsJson: ag?.Billing_Agreement_Attachments__r?.[0]?.AWS_JSON__c || ''
        }));
...
async deleteAgreementRecord(id) {
    console.log('🗑️ deleteAgreementRecord START');
    console.log('📌 Agreement Id:', id);

    try {
        console.log('📚 this.agreements:', JSON.stringify(this.agreements));

        const agreement = this.agreements.find(item => item.id === id);
        console.log('📄 Matched agreement:', JSON.stringify(agreement));

        const awsJson = agreement?.awsJson;
        console.log('🧾 AWS JSON value:', awsJson);

        if (awsJson) {
            console.log('📡 Calling Apex deleteFilesFromAwsJson...');
            const awsDeleteResult = await deleteFilesFromAwsJson({ awsJsonText: awsJson });
            console.log('✅ AWS files deleted successfully:', JSON.stringify(awsDeleteResult));
        } else {
            console.warn('⚠️ No AWS JSON found, skipping AWS delete');
        }

        console.log('🗑️ Deleting Salesforce agreement record...');
        await deleteAgreement({ agreementId: id });
        console.log('✅ Salesforce agreement deleted successfully');

        this.showToast('Success', 'Agreement deleted', 'success');

        console.log('🔄 Reloading agreements...');
        await this.loadAgreements();
        console.log('✅ Agreements reloaded');

    } catch (error) {
        console.error('❌ deleteAgreementRecord FAILED');
        console.error('❌ DELETE ERROR FULL:', JSON.stringify(error));
        console.error('❌ DELETE ERROR OBJECT:', error);
        console.error('❌ DELETE ERROR MESSAGE:', error.body?.message || error.message || 'Delete failed');
        ...
        console.log('🔚 deleteAgreementRecord END');
    }
}
...
handleBillingPageSize(event) {
  ...
  //==================== Storage Logic =====================
async loadStorageFromAWS(orgId) {
    try {
      ...
      //  this.storageLimitAWS = data.limit / (1024 * 1024 * 1024); // GB
        this.storageUsedAWS =(data.grandTotalBytes || 0) / (1024 * 1024 * 1024);

        // Optional: round to 2 decimals
        this.storageUsedAWS = Number(this.storageUsedAWS.toFixed(4));
        console.log('storageUsedAWS ==>'+this.storageUsedAWS);
...
async handleTestHiddenUpload() {
    console.log('🚀 Hidden upload test started');

    try {
        const uploader = this.template.querySelector('c-aws-batch-lwc');

        if (!uploader) {
            console.error('❌ Child uploader component not found');
            return;
        }

        const testPayload = {
            name: 'Test Subscription Upload',
            organisationId: this.organisationId,
            timestamp: new Date().toISOString(),
            source: 'SubscriptionDashboard'
        };

        console.log('📦 Test payload:', JSON.stringify(testPayload));

        const result = await uploader.uploadPayload(
            testPayload,
            'subscription-test.json',
            'application/json'
        );

        console.log('✅ Hidden upload success');
        console.log('📁 Upload result:', JSON.stringify(result));

    } catch (error) {
        console.error('❌ Hidden upload failed');
        console.error('❌ Error object:', error);
        console.error('❌ Error message:', error?.body?.message || error?.message || JSON.stringify(error));
    } finally {
        console.log('🔚 Hidden upload test finished');
    }
}
