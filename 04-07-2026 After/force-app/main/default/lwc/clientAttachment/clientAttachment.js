...
console.log('allRecords before map => ', this.allRecords);
                    this.records = [...this.allRecords]; //manendra added for global search
                    console.log('allRecords after map => ', JSON.stringify(this.allRecords));
console.log('records after copy => ', JSON.stringify(this.records));
                    this.totalRecords = this.records.length;
...
handleArchiveSearch(event) {
    ...
    this.archivedRecords = this.allArchivedRecords.filter(file => {
        ...
        console.log(
            `📄 File: ${file.Name} | Search: ${searchTerm} | Match: ${isMatch}`
        );
        return isMatch;
    });
    console.log(
