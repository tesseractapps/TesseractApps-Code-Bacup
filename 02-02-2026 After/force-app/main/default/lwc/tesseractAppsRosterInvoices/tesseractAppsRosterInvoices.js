...
/* -------------------------
   ...
   ------------------------- */
    ...
    async  handleCheckboxChange(event) {
        ...
        console.log('accList :', JSON.parse(JSON.stringify(this.accList)));
        ...
        //     const result = await getCompanyAndAccountData1({ participantIdList: allKeys, dashboardFacilityId: this.faclist });
        //     console.log('✅ getCompanyAndAccountData1 result:', JSON.stringify(result));
        //     if (!result?.isSuccess) {
        //         this.disableBool = true;
        //         this.dispatchEvent(
        //             new ShowToastEvent({
        ...
        //         .filter(key => companyResponse[key]?.startsWith('NoData '));
        ...
        //         //     if (missingClientIds.includes(clientId)) {
        //         //         return {
        //         //             ...acc,
        ...
        //                     acc.isChecked === true &&                 // 🔥 CHANGE
        //                     missingClientIds.includes(clientId),      // 🔥 CHANGE
...
//     // 1️⃣ Update bulkServices (UI source of truth)
//     this.bulkServices = this.bulkServices.map(inv =>
//         inv.Id === recordId ? { ...inv, isChecked } : inv
//     );

//     // 2️⃣ Validate entity AFTER update (same pattern as first table)
//     const isEntityValid = this.checkBulkEntityExist();
//     if (!isEntityValid) {
//         // revert DOM
//         event.target.checked = false;
...
//         return;
//     }
// this.updateBulkInvoiceButtonState();
//     // // 3️⃣ Enable / disable Bulk button
//     // this.disableBulkInvoice =
//     //     this.bulkServices.filter(i => i.isChecked).length === 0;

//     // 4️⃣ Update header checkbox (current page)
//     this.bulkSelectAllChecked =
//         this.bulkPageList.length > 0 &&
//         this.bulkPageList.every(inv =>
//             this.bulkServices.find(r => r.Id === inv.Id)?.isChecked
//         );
// }
...
//     const pageIds = new Set(this.bulkServices.map(inv => inv.Id));

//   /*  this.recordsBulk = this.recordsBulk.map(inv => {
//         if (!pageIds.has(inv.Id)) {
//             return inv; // other pages untouched
//         }
...
//         if (isChecked && !hasEntity) {
//             hasInvalidEntity = true;
//             return { ...inv, isChecked: false }; // 🔴 force uncheck
//         }

//         return { ...inv, isChecked };
//     }); */
...
//     this.disableBulkInvoice =
//         this.recordsBulk.filter(r => r.isChecked).length === 0;

//     // Header checkbox reflects CURRENT PAGE ONLY
//     this.bulkSelectAllChecked =
//         this.bulkServices.length > 0 &&
//         this.bulkServices.every(r => r.isChecked);
// }
...
//     // 2️⃣ Validate entity (auto-uncheck + toast)
//    const isValid = this.checkBulkEntityExist();
//      if (!isValid) {
//         // 🔴 FORCE UI uncheck (important)
//         event.target.checked = false;

//         // 🔴 FORCE state uncheck (extra safety)
//         this.bulkServices = this.bulkServices.map(inv =>
//             inv.Id === recordId ? { ...inv, isChecked: false } : inv
//         );
//     }
//     // 3️⃣ Update button state
//     this.updateBulkInvoiceButtonState();

//     // 4️⃣ Header checkbox (current page only)
//     this.bulkSelectAllChecked =
//         this.bulkServices.length > 0 &&
//         this.bulkServices.every(i => i.isChecked);
// }
...
//     // 1️⃣ Apply selection to current page
//     this.bulkServices = this.bulkServices.map(inv => ({
//         ...inv,
//         isChecked
//     }));

//     // 2️⃣ Validate entities (auto-uncheck invalid + toast)
//     this.checkBulkEntityExist();

//     // 3️⃣ Enable Bulk Invoice only if at least one valid row is checked
//     this.disableBulkInvoice =
//         !this.bulkServices.some(i => i.isChecked);

//     // 4️⃣ Header checkbox reflects final state
//     this.bulkSelectAllChecked =
//         this.bulkServices.length > 0 &&
//         this.bulkServices.every(i => i.isChecked);
// }
...
//         // 1️⃣ Collect selected invoice IDs
//         const selectedInvoiceIds = this.bulkServices
...
//             // 3️⃣ Reset UI selection (IMPORTANT)
//             this.bulkServices = this.bulkServices.map(row => ({
//                 ...row,
//                 isChecked: false
//             }));
