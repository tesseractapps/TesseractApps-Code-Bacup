...
export default class Repository extends NavigationMixin(LightningElement) {
    ...
    const dynamicTypes = [...new Set(this.records.map(r => r.Type__c))].map(t => ({
        label: t,
        value: t
    }));
  ...
  @track storedFacilityId;
                ...
                if (data) {
                    ...
                    this.tempConList = data.map((record) => {
                        ...
                        return {
                            ...
                            Type__c: (record?.Type__c || '').toLowerCase() === 'other'? (record?.Comments__c || 'Others'): record?.Type__c,
                            document:record?.Type__c || '',
    ...
    records.forEach(repo => {
        const type = repo.Type__c || 'Others';
...
/* applyFilters() {
    ...
    this.filteredRecords = this.records.filter(rec => {
        const matchesText =
            ...
            // rec.Type__c.toLowerCase().includes(text);

        const matchesType =
            type === 'All' ? true : rec.Type__c === type;
...
exportCSV() {
    ...
    source.forEach(parent => {
        // Parent Row
        csv += `"${parent.Name || ''}","${parent.Type__c || ''}","${parent.Comments__c || ''}","${parent.User_Name__c || ''}","${parent.Date_Format__c || ''}",,"${parent.totalChildSize || ''}"\n`;
