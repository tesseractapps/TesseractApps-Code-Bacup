...
export default class TesseractAppsPublishShift extends LightningElement {
  ...
  connectedCallback() {
    ...
    organizationDetails().then((response) => {
      ...
      this.RoleFilter = [AllFilter, ...this.RoleFilter]; */
       fetchBulkRoles({facilityIDList:this.facilityValue}).then(
              facRoles =>{
      ...
      .then((result) => {
        ...
        if (this.addShiftData.AddShiftRole) {
          ...
          filteredStaff = result.filter((staff) => {
            ...
            console.log(
              `👤 ${staff.Name} | Roles: [${roles.join(", ")}] | Match: ${hasRole}`
            );
            return hasRole;
          });
        ...
        console.log(
          `📋 Final staffData set with ${this.staffData.length} records`
        );
      })
      .catch((error) => {
    ...
    if (this.headingLabel === "Add Shift(s)") {
      ...
      this.staffData = this.staffData.map((staff) => ({
        ...
        isDisabled:
          ...
          originalSelectedIds.includes(staff.Id)
      }));
    }
  ...
  getShiftTimingsByFacility(shifNames) {
    console.log(
      "shift timings IN FACILITY timings  " + JSON.stringify(shifNames)
    );
