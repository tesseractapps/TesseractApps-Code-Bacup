    ...
    if (this.currentRecordPdf && Array.isArray(this.currentRecordPdf)) {
        this.currentRecordPdf = this.currentRecordPdf.map(record => {
            ...
            allocationData.forEach(item => {
                const shifts = [
                     { name: 'Morning', rate: item.General_Hourly_pay_rate__c, hours: item.General_Working_Hours__c },
                    ...
                    /* { name: 'Morning', rate: item.General_Hourly_pay_rate__c ?? 0, hours: item.General_Working_Hours__c ?? 0 },
                    { name: 'Long Morning', rate: item.Long_Morning_Hourly_Rate__c ?? 0, hours: item.Long_Morning_Shift_Duartion__c ?? 0 },
                    { name: 'Morning Two', rate: item.Long_Morning_Rate_Two__c ?? 0, hours: item.Long_Moring_Duration_Two__c ?? 0 },
                    { name: 'Afternoon', rate: item.Afternoon_shift_Hourly_Rate__c ?? 0, hours: item.Afternoon_Shift_hours__c ?? 0 },
                    { name: 'Long Afternoon', rate: item.Long_Afternoon_Hourly_Rate__c ?? 0, hours: item.Long_Afternoon_Shift_Duartion__c ?? 0 },
                    { name: 'Afternoon Two', rate: item.Long_Afternoon_Rate_Two__c ?? 0, hours: item.Long_Afternoon_Duration_Two__c ?? 0 },
                    { name: 'Night', rate: item.Night_shift_Hourly_Rate__c ?? 0, hours: item.Night_Shift_Hours__c ?? 0 },
                    { name: 'Long Night', rate: item.Long_Night_Hourly_Rate__c ?? 0, hours: item.Long_Night_Shift_Duartion__c ?? 0 },
                    { name: 'Sleepover', rate: item.Sleepover_Shift_Hourly_Rate__c ?? 0, hours: item.Sleepover_Shift_Hours__c ?? 0 },
                    { name: 'Sleepover Night', rate: item.Sleepover_Night_Shift_hourly_rate__c ?? 0, hours: item.Sleepover_Night_shift_hours__c ?? 0 },
                    { name: 'Saturday', rate: item.Saturday_Hourly_Rate__c ?? 0, hours: item.Saturday_Hours__c ?? 0 },
                    { name: 'Sunday', rate: item.Sunday_Hourly_Rate__c ?? 0, hours: item.Sunday_hours__c ?? 0 },
                    { name: 'Long Sleepover', rate: item.Long_Sleepover_Allowance__c ?? 0, hours: item.Long_Sleepover_Duartion__c ?? 0 },
                    { name: 'Extended Hours', rate: item.Extended_Hourly_Rate__c ?? 0, hours: item.Extended_Hours__c ?? 0 } */
                ];
                /*  console.log('Shifts',JSON.stringify(shifts));
               const validShifts = shifts.filter(shift => Number(shift.hours) > 0);
               console.log('validShifts',JSON.stringify(validShifts));
                allShifts.push(...validShifts); */
    ...
    renderedCallback() {
      ...
      /*   combineSubsequentShifts(shifts) {
        ...
        for (let i = 0; i < shifts.length; i++) {
            ...
            if (!current.hours || !current.rate) continue; // skip zero hours or zero rate
            let totalHours = current.hours;
            let shiftNames = [current.name];

            // Combine consecutive shifts with the same rate
            while (
                i + 1 < shifts.length &&
                shifts[i + 1].rate === current.rate &&
                shifts[i + 1].hours > 0
            ) {
                totalHours += shifts[i + 1].hours;
                shiftNames.push(shifts[i + 1].name);
                i++;
            }

            combined.push({
                shiftTypes: shiftNames.join(' + '),
                rate: current.rate,
                hours: totalHours
            });
        }
...
combineSubsequentShifts(shifts) {
    ...
    const shiftGroups = {
        Morning: ['Morning', 'Long Morning', 'Morning Two'],
    ...
    const normalizedShifts = shifts
        ...
        .filter(shift => shift.hours > 0 ); // remove invalid entries
