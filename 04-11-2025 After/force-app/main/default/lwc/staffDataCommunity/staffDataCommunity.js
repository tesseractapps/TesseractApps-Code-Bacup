...
@track DocumentColumns = [
   ...
   /*  fetchRoleOptions() {
        getRoleOptionsByFacility({ facilityId: this.StaffFacility })
            .then(result => {
                console.log('✅ Role options received:', JSON.stringify(result));
                this.roleoptionsforFacility = result.map(role => ({
                    label: role,
                    value: role
                }));
            })
...
//             console.error('❌ Error fetching role options:', error);
//         });
// }//manendra cahnge for custom combo
...
//     console.log('🔄 Active Roles:', JSON.stringify(this.selectedRoleValues));
// }
...
// -----------------------------------------
// Update selected roles (existing method)
// -----------------------------------------
/* updateSelectedRoles() {
    this.selectedRoleValues = this.roleoptionsforFacility
    ...
    console.log('📋 Updated selected roles:', this.selectedRoleValues);
} *///manendra
updateSelectedRoles() {
    this.selectedRoleValues = this.roleoptionsforFacility
        .filter(option => option.checked)
    ...
    // ✅ Reset selectedEmploymentRole when nothing is selected
...
handleResize() {
    ...
    handleCreateNewStaff() {
        ...
        this.selectedNationality = ''; // or whatever property is bound to lightning-input
...
{ label: 'Occupational Health Nurse - level 3', value: 'Occupational Health Nurse - level 3' }
    ...
    // Validate selected role
          ...
          console.log('manendracheck2', this.selectedRole);
   if (!this.selectedRoleValues || this.selectedRoleValues.length === 0 || !this.selectedRole) {
    this.dispatchEvent(
        new ShowToastEvent({
            title: 'Error',
    ...
    // Role is selected, proceed with saving the form
