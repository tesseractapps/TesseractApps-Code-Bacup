...
export default class OfferTemplateList extends LightningElement {
  ...
  // ✅ Fetch recipients first to determine the user's position (unchanged behavior)
  getAllRecipients({ recordId: this.recordId })
    .then((allRecipients) => {
      ...
      // Select data source (unchanged behavior)
      ...
      // ⬇️ Load libraries: main pdfJS + fonts ONLY (do not load the worker as a script)
      return Promise.all([
    ...
    .then(() => {
      // Point pdf.js at the worker **file URL** so it can spawn a real worker
      if (window.pdfjsLib && window.pdfjsLib.GlobalWorkerOptions) {
        window.pdfjsLib.GlobalWorkerOptions.workerSrc = pdfWorker; // separate SR URL
      }
...
//   const total = this.placeholders?.length || 0;
//   const onPage = (this.placeholdersByPage?.[cur] || []).length;
//   this._log('_updateAllPlaceholderStyles:begin', { cur, totalPlaceholders: total, onCurrentPage: onPage });

//   // recompute styles only for current page in the flat list
//   const t0 = performance.now();
//   let recomputed = 0;
...
//       const style = this.getPlaceholderStyle(ph, rect);
//       const s1 = performance.now();
//       if (this.DEBUG && (s1 - s0) > 5) {
//         this._log('style:slowItem', { id: ph.id, ms: Math.round(s1 - s0) });
//       }
//       return { ...ph, style };
//     }
//     return ph;
...
//   // and for the paged map
//   const list = (this.placeholdersByPage?.[cur] ?? []).map(ph => ({
//     ...ph,
//     style: this.getPlaceholderStyle(ph, rect)
//   }));
//   this.placeholdersByPage = { ...this.placeholdersByPage, [cur]: list };
...
//       const baseW = baseImage.naturalWidth || baseImage.width;
//       const baseH = baseImage.naturalHeight || baseImage.height;
...
//       // Uniform mapping CSS → pixels
//       const imgRect = imgEl.getBoundingClientRect();
//       const ratio   = baseW / imgRect.width;

//       const destX = Math.round((boundingRect.x - imgRect.x) * ratio);
//       const destY = Math.round((boundingRect.y - imgRect.y) * ratio);
//       const destW = Math.round(boundingRect.width  * ratio);
//       const destH = Math.round(boundingRect.height * ratio);

//       const overlay = await createImageBitmap(imageBlob);

//       // If overlay was produced by updatePlaceholderWithCanvas with pixelSpace=true,
//       // its intrinsic size should already be destW×destH → draw 1:1 (no blur)
...
//     // Fallback: render the page to the container width and do the same uniform mapping
//     const page = await this.pdfDoc.getPage(pageNumber);
...
//     const cssRect = container ? container.getBoundingClientRect() : { x: 0, y: 0, width: viewport.width, height: viewport.height };
//     const ratio   = canvas.width / cssRect.width;

//     const destX = Math.round((boundingRect.x - cssRect.x) * ratio);
//     const destY = Math.round((boundingRect.y - cssRect.y) * ratio);
//     const destW = Math.round(boundingRect.width  * ratio);
//     const destH = Math.round(boundingRect.height * ratio);
...
async embedImageWithAdjustedHeight(imageBlob, boundingRect, pageNumber) {
  try {
    ...
    // (You can set this._embeddingPlaceholder = the placeholder being embedded.)
    ...
    if (imgEl && this.imageSrc) {
      ...
      if (hasPt) {
        // NEW: compute from PDF page-points → image pixels
        destX = Math.round((ph.xPt / this._pagePt.w) * baseW);
        destY = Math.round((ph.yPt / this._pagePt.h) * baseH);
        // width/height: prefer wPt/hPt if present; otherwise fall back to DOM mapping
        ...
        // ORIGINAL path (unchanged): uniform mapping CSS → pixels
        const imgRect = imgEl.getBoundingClientRect();
        const ratio   = baseW / imgRect.width;
      ...
      // Clamp to avoid bottom/right clipping from rounding
    ...
    // keep a canonical page-point size around for next calls
    this._pagePt = { w: vp1.width, h: vp1.height };
    ...
    if (hasPt) {
      // NEW: page-point path against freshly rendered canvas
      destX = Math.round((ph.xPt / this._pagePt.w) * canvas.width);
      destY = Math.round((ph.yPt / this._pagePt.h) * canvas.height);
      if (ph.wPt != null && ph.hPt != null) {
        destW = Math.max(1, Math.round((ph.wPt / this._pagePt.w) * canvas.width));
        destH = Math.max(1, Math.round((ph.hPt / this._pagePt.h) * canvas.height));
      } else {
    ...
    destW = Math.max(1, Math.min(destW, canvas.width));
    destH = Math.max(1, Math.min(destH, canvas.height));
    destX = Math.max(0, Math.min(destX, canvas.width  - destW));
    destY = Math.max(0, Math.min(destY, canvas.height - destH));
...
handleInputChange(event, srcField, successMessage) {
  ...
  placeholder.value = newValue;
  await this.updatePlaceholderWithCanvas(placeholder, srcField, successMessage);
}, 2000);
...
async updatePlaceholderWithCanvas(placeholder, srcField, successMessage) {
  ...
  try {
    if (this.pdfDoc && this.currentPage && el && this.imageSrc) {
      ...
      if (imgEl) {
        ...
        // PDF bottom-left origin for the text probe
        ...
        if (best) {
          ...
          desiredFontPx = Math.max(1, Math.round(fontPt * pxPerPtY));
        }
      }
    }
  } catch (_) {
    // non-fatal; keep fallback size
  }

  // ---------- B) Persist size/position in PAGE POINTS (top-left convention for our own layout) ----------
  if (el && this._pagePt) {
    const imgEl = this.template.querySelector('.image-container img');
    if (imgEl) {
      const imgRect = imgEl.getBoundingClientRect();
      ...
      // Use top-left page-points for our own placement (consistent with getPlaceholderStyle)
      const scaleX = this._pagePt.w / imgRect.width;
      const scaleY = this._pagePt.h / imgRect.height;

      // Save width/height in points (so they can be reconstructed on any preview size)
      const wPt = cssRect.width  * scaleX;
      const hPt = cssRect.height * scaleY;

      // Save position in points (top-left)
      const xPt = (cssRect.left - imgRect.left) * scaleX;
      const yPt = (cssRect.top  - imgRect.top ) * scaleY;

      placeholder.wPt = wPt;
      placeholder.hPt = hPt;
      placeholder.xPt = (placeholder.xPt ?? xPt); // only set if not already stored
      placeholder.yPt = (placeholder.yPt ?? yPt);

      // (Optional) remember last CSS size too for fallback
      placeholder.cssW = cssRect.width;
      placeholder.cssH = cssRect.height;
    }
  }

  // ---------- C) Generate overlay bitmap in DESTINATION PIXELS (same as before) ----------
  let base64Data;
  if (el && this.imageSrc) {
    const imgEl = this.template.querySelector('.image-container img');
    const cssRect = el.getBoundingClientRect();
    ...
    const ratio = natW / imgEl.getBoundingClientRect().width; // single ratio (no stretch)

    const destW = Math.max(1, Math.round(cssRect.width  * ratio));
    const destH = Math.max(1, Math.round(cssRect.height * ratio));
    const destFontPx = Math.max(1, Math.round(desiredFontPx * ratio));

    placeholder._pixelW = destW;
    placeholder._pixelH = destH;

    base64Data = this.createCanvasImage(placeholder.value || '', {
      targetWidth:  destW,
      targetHeight: destH,
      fontFamily:   placeholder.fontFamily || 'Roboto',
      fontPx:       destFontPx,  // shrink-to-fit only
      paddingX:     0,
      paddingY:     0,
      align:        'center',
      vAlign:       'middle',
  ...
  // ---------- D) Update state and STYLE REACTIVELY (include width/height) ----------
  placeholder.hasImage = true;
  placeholder[srcField] = base64Data;

  // recompute style with width/height for the current preview (prevents enlargement on pagination)
  const img = this.template.querySelector('.image-container img');
  const imgRectForStyle = img?.getBoundingClientRect();
  if (imgRectForStyle && this.getPlaceholderStyle) {
    placeholder.style = this.getPlaceholderStyle(placeholder, imgRectForStyle);
  }

  // trigger reactivity
  this.placeholders = this.placeholders.map(p => p.id === placeholder.id ? { ...placeholder } : p);

  // Reactively update the paged data (no in-place mutation)
...
if (idx > -1) {
  ...
  // (defensive) if not present, append
  updatedPage = [...current, { ...placeholder }];
}

// replace the page array AND the container object immutably
...
// Old use: createCanvasImage(text, fontSize, fontFamily)
// New use: createCanvasImage(text, { targetWidth, targetHeight, fontPx, fontFamily, paddingX, paddingY, align, vAlign, pixelSpace })
createCanvasImage(text, fontSizeOrOptions = 15, fontFamilyMaybe = 'Roboto') {
  ...
  if (!shrinkOnly) {
    while (size + 1 <= availH && measureWidth(size + 1) <= availW) size += 1;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ctx.fillText(lines[i], x, y, availW);
    y += lineBox + lineGap;
  }
