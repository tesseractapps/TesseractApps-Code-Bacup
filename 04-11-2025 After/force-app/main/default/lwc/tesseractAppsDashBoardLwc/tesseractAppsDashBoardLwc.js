...
/* import isAdminChatUser from '@salesforce/apex/FirebaseUserInt.isAdminChatUser'; */
...
export default class TesseractAppsDashBoardLwc extends LightningElement {
  ...
  async bootstrap() {
  console.log('[Router] bootstrap(): starting AdminChat flag check…');

  try {
    this.isAdminChat = await isAdminChatUser();
    console.log('[Router] Apex returned isAdminChat =', this.isAdminChat);
  } catch (e) {
    // Fail safe → show client chat
    // eslint-disable-next-line no-console
    console.error('[Router] AdminChat flag check failed:', e);
    this.isAdminChat = false;
  } finally {
    // IMPORTANT: don't override the flag here
    this.loaded = true;
    console.log('[Router] bootstrap(): finished. Final isAdminChat =', this.isAdminChat);
  }
}
  ...
  handleKeyboardShortcuts(event) {
    ...
    console.log('alt key', event.key);
      ...
      if (tag === "input" || tag === "textarea") return;
          ...
          // 🧠 If user is ICT Staff/Admin → ensure Alt+T opens ICT Timesheet always
          if (
              this.currentUserType &&
              ["ict staff", "ict admin"].includes(this.currentUserType.toLowerCase())
          ) {
              if (event.altKey && event.key.toLowerCase() === "t") {
                  console.log("✅ ICT user shortcut triggered: Alt+T → ICT Timesheet");
                  this.triggerShortcut("ICT Timesheet", "ICT Timesheet");
                  return; // stop here so it doesn’t duplicate trigger below
              }
          }
        ...
        switch (true) {
            ...
            case event.altKey && event.key.toLowerCase() === "y":
                ...
                break;
            case event.altKey && event.key.toLowerCase() === "c":
                this.triggerShortcut("Chatter", "Chatter");
                break;
...
// 🪄 Trigger both menu and module logic
  ...
  triggerShortcut(field, content) {
      ...
      // ✅ Also always allow ICT users to access ICT Timesheet
      if (
          this.currentUserType &&
          ["ict staff", "ict admin"].includes(this.currentUserType.toLowerCase()) &&
          field.toLowerCase() === "ict timesheet"
      ) {
          console.log("✅ [ICT Override] Access granted for ICT Timesheet shortcut");
      } else {
          // Normal access check for everyone else
          if (!this.modules || this.modules.length === 0) {
              console.warn("⚠️ Modules not loaded or empty — skipping shortcut.");
              return;
          }
      ...
      // ✅ Move this section outside the else block (so it runs for all valid users)
      console.log(`⚡ Shortcut triggered for: ${field}`);
  ...
  setModuleFlags() {
    ...
    if(accountingServices == 'Xero'){
      this.XeroModule = this.modules.includes("Accounting");
      //this.AccountingModule = this.modules.includes("Accounting");
    } else{
      this.AccountingModule = this.modules.includes("Accounting");
    }
