...
// ⭐ NEW — build render matrix for LWC
if (isTableBlock && tableConfig) {
  const cols = tableConfig.columns || [];
  const rows = tableConfig.rowNames || [];
  const values = tableConfig.cellValues || {};

  tableConfig.renderRows = rows.map((row, rIndex) => {
    return {
      ...row,
      renderCells: cols.map((col, cIndex) => ({
        ...col,
        cellKey: `${rIndex}-${cIndex}`,
        cellValue: values[`${rIndex}-${cIndex}`] || ""
      }))
    };
  });
}
    ...
    formJson.forEach((row, rowIndex) => {
      ...
      row.cells.forEach((cell, colIndex) => {
        ...
        tableData.push({
          ...
          isRadioButton,
          isRegularField:
            !isUploadFile && !isCheckbox && !isRichText && !isRadioButton && !isSignature
        });
      });
    });
...
async drawPdfContent(doc, logoData) {
      ...
      rawValue === "action:approval"
        ...
        value = stripHtml(rawValue);
          ...
          this._pdfSignatureImages.push({
            rowIndex: rows.length, // where it will appear
            base64: rawValue
          });
        ...
        item.dataType === "Date Field" ||
        item.field?.dataType === "Date Field" ||
  ...
  // =====================================
// 🖼️ OPTIMIZED ORG LOGO INSIDE HEADER
// =====================================
if (logoData?.base64) {
  try {
...
//     // ✅ Increase row height BEFORE drawing
//     didParseCell: (data) => {
//       if (!this._pdfSignatureImages?.length) return;
//       if (data.section !== "body") return;
//       if (data.column.index !== 1) return;

//       const sig = this._pdfSignatureImages.find(
//         s => s.rowIndex === data.row.index
//       );

//       if (sig) {
//         // Give enough space for signature
//         data.cell.styles.minCellHeight = 40;
//       }
//     },

//     // ✅ Draw scaled signature image
//     didDrawCell: (data) => {
//       if (!this._pdfSignatureImages?.length) return;
//       if (data.section !== "body") return;
//       if (data.column.index !== 1) return;
...
//         const maxWidth = 65; // Increase width (professional size)
...
//         doc.addImage(sig.base64, "PNG", x, y, imgWidth, imgHeight);

//         console.log("🖼️ Signature drawn with proper scaling");
//       } catch (e) {
  ...
  this.tableRows.forEach(r => {
    r.cells.forEach(c => {
      if (String(c.id) === String(parentId)) {
        if (!c.tableConfig.cellValues) {
          c.tableConfig.cellValues = {};
        }
        c.tableConfig.cellValues[`${row}-${col}`] = value;
      }
    });
  });
  this.updateStepVisibleRows();
