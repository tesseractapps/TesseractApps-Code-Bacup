...
import ENV_LABEL from "@salesforce/label/c.Environment_Label";
...
export default class ChatApp extends LightningElement {
  ...
  connectedCallback() {
    ...
    if (!this.firebaseInitialized) {
      Promise.all([
        ...
        .then(() => {
          ...
          // ✅ Unified flow: fetch from cache/Firestore and sync with Apex
          this.syncUsersWithFirestore();
          this.fetchGroupChats();

          // ✅ Listen to real-time chat updates
          this.listenToUserChats();

          // ✅ Handle deferred user initialization
        ...
        .catch((error) => {
          console.error(
            "❌ Error loading Firebase SDKs from static resource:",
            error
          );
        });
    }
    ...
    this.filteredNewChats = [
      ...this.newChats.filter((user) => user.id !== this.currentUserId)
    ];

    console.log(
      "📌 Restored Existing Chats:",
      JSON.stringify(this.filteredExistingChats)
    );

    // // ✅ Refresh wire if available
  ...
  disconnectedCallback() {
    ...
    if (this.unsubscribeGroupChats) this.unsubscribeGroupChats();
    if (this.groupMessageListeners) {
      Object.values(this.groupMessageListeners).forEach((unsub) => unsub());
    }
  ...
  hasInitializedChats = false; // 🔒 Guard to prevent duplicate listeners

  listenToUserChats() {
    if (!this.currentUserId) {
      console.warn(
        "⚠️ Current user ID not set, unable to listen for messages."
      );
      return;
    }

    if (this.hasInitializedChats) {
      console.warn("🔁 Chat listener already initialized. Skipping setup.");
      return;
    }

    this.hasInitializedChats = true;

    if (this.unsubscribeUserChats) {
      console.warn("♻️ Existing listener detected. Unsubscribing first...");
      this.unsubscribeUserChats();
      this.unsubscribeUserChats = null;
    }

    console.log(
      "👂 Setting up real-time chat listener for:",
      this.currentUserId
    );

    this.unsubscribeUserChats = firebase
      .firestore()
      .collection("chats")
      ...
      .onSnapshot((snapshot) => {
        ...
        snapshot.docChanges().forEach((change) => {
          ...
          const lastMessageAt = chatData.lastMessageAt
            ? new Date(chatData.lastMessageAt.toDate())
            : null;
          const formattedTime = lastMessageAt
            ? this.formatLastMessageTime(lastMessageAt)
            : "";
          ...
          if (userIndex !== -1) {
            ...
            this.updateLastMessageUI(otherUserId, lastMessage);
          } else {
            console.warn(`⚠️ Chat update: No user found for ID ${otherUserId}`);
          }
        });

        // ✅ Sort and filter after updates
        updatedUsers.sort(
          (a, b) => (b.lastMessageAt || 0) - (a.lastMessageAt || 0)
        );

        this.filteredUsers = updatedUsers.filter(
          (user) => user.id !== this.currentUserId
        );

        console.log(
          "✅ Real-time updated filtered users:",
          this.filteredUsers.map((user) => user.name)
        );
      });
  }
  ...
  //     try {
  //       console.log(
  //         "✅ Users Fetched from Apex (wiredOrgUsers):",
  ...
  //       console.log(
  //         "✅ Formatted Org Users:",
  //         JSON.stringify(this.orgusers, null, 2)
  //       );
  ...
  //   try {
  //     console.log(
  //       "🔥 Initializing Users in Firestore:",
  //       JSON.stringify(this.orgusers, null, 2)
  //     );
  ...
  //       existingUsersSnapshot.docs.map((doc) => doc.data().userId)
  //     );
  ...
  //       await batch.commit();
  //       console.log(
  //         `✅ Successfully added/updated ${addedUsers} users in Firestore.`
  //       );
  //     } else {
  ...
  fetchCurrentUser() {
    ...
    const currentUser = this.orgusers.find(
    ...
    if (currentUser) {
      ...
      console.log(
        `✅ Current User Found: ${this.currentUserName}, Avatar: ${this.currentUserAvatar}`
      );
    } else {
      console.warn(
        "⚠️ No matching user found for current user ID in orgusers."
      );
  ...
  filterUsers() {
    ...
    console.log(
      "📌 Filtering out Current User ID:",
      `"${this.currentUserId.trim()}"`
    );

    // ✅ Filter out the current user
    this.filteredUsers = this.users.filter(
      (user) => user.id.trim() !== this.currentUserId.trim()
    );
  ...
  async fetchLastMessages() {
    ...
    try {
      const chatSnapshots = await firebase
        .firestore()
        .collection("chats")
        .where("participants", "array-contains", this.currentUserId)
        .get();
      ...
      this.newChats = usersWithoutMessages.filter(
        (user) => user.id !== this.currentUserId
      ); // ✅ Remove current user
      ...
      console.log(
        "✅ Existing Chats (with messages):",
        this.filteredExistingChats.map((user) => user.name)
      );
      console.log(
        "✅ New Chats (without messages):",
        this.filteredNewChats.map((user) => user.name)
      );
      //this.isLoading=false;
    } catch (error) {
  ...
  async ensureDefaultChatsExist() {
    ...
    const missingChats = this.users.filter(
      (user) => user.userId !== this.currentUserId
    );
  ...
  sortAndUpdateFilteredUsers(usersWithMessages, usersWithoutMessages) {
    ...
    // ✅ Merge both lists
    this.filteredUsers = [...usersWithMessages, ...usersWithoutMessages].filter(
      (user) => user.userId.trim() !== this.currentUserId.trim()
    );
  ...
  handleSearch(event) {
    ...
    if (this.searchQuery) {
      ...
      const existingContains = this.existingChats.filter(
        (user) =>
      ...
      const newContains = this.newChats.filter(
        (user) =>
          user.name.toLowerCase().includes(this.searchQuery) &&
          !user.name.toLowerCase().startsWith(this.searchQuery)
      );
  ...
  async selectNewUser(event) {
    ...
    try {
      ...
      this.newChats = this.newChats.filter(
  ...
  async selectUser(event) {
    ...
    let user =
      this.existingChats.find((u) => u.userId === userId) ||
      this.newChats.find((u) => u.userId === userId);

    if (!user) {
      console.warn(
        "❌ User not found in chat lists. Fetching from Firestore..."
      );
  ...
  async createOrGetChat(currentUserId, otherUserId) {
    try {
      ...
      // ✅ Check if chat already exists, filtered by orgId too
      const chatsSnapshot = await firebase
        .firestore()
        .collection("chats")
        .where("participants", "array-contains", currentUserId)
        .where("orgId", "==", this.orgid) // ✅ Added orgId filter
        .get();
      ...
      const newChatRef = await firebase
        .firestore()
        .collection("chats")
        .add({
          createdAt: timestamp,
          lastMessage: "",
          lastMessageAt: null,
          participants: [currentUserId, otherUserId], // ✅ Existing line untouched
          orgId: this.orgid // ✅ Added new orgId field
        });
  ...
  //   this.isSending = true;
  //   let messageText = ""; // ✅ Declare outside
  ...
  //     messageText = this.messageInput.trim(); // ✅ Assign here
  //     const message = {
  //       chatId: chatId,
  ...
  //     this.updateLastMessageUI(this.selectedUserId, messageText); // ✅ Safe to use
  //   }
  // }
  ...
  async sendMessage() {
    ...
    try {
      ...
      const messageRef = db.collection("messages").doc(); // Predefine message ID
      const chatRef = db.collection("chats").doc(chatId);

      const timestamp = firebase.firestore.FieldValue.serverTimestamp();
      messageText = this.messageInput.trim();
      ...
      await batch.commit(); // ✅ Single write to Firestore
      ...
      console.log("✅ Message sent via batch:", message);
    } catch (error) {
      console.error("❌ Error sending message (batched):", error);
      alert("❌ Failed to send message. Please try again.");
    } finally {
      this.isSending = false;
      this.updateLastMessageUI(this.selectedUserId, messageText);
    }
  }

  updateLastMessageUI(userId, messageText) {
    console.log(`📝 Attempting to update last message for userId: ${userId}`);
    ...
    if (user) {
      ...
      user.lastMessageTime = this.formatLastMessageTime(now);

      console.log(
        `✅ Updated last message for ${user.name || "Unknown User"}:`,
        {
      ...
      // Optionally update filtered lists if needed
      const updateInList = (list, label) => {
        const index = list.findIndex((u) => u.id === userId);
        if (index !== -1) {
          list[index].lastMessage = messageText;
          list[index].lastMessageAt = now;
          list[index].lastMessageTime = this.formatLastMessageTime(now);
          console.log(
            `🛠️ Synced last message in ${label} list for: ${user.name}`
          );
        }
      };

      updateInList(this.filteredExistingChats, "filteredExistingChats");
      updateInList(this.filteredNewChats, "filteredNewChats");
    } else {
      console.warn(`⚠️ No matching user found in this.users for ID: ${userId}`);
    }
  }

  handleKeyPress(event) {
    if (event.key === "Enter" && !event.shiftKey) {
      // Prevents new line on Shift + Enter
    ...
    try {
      this.unsubscribeMessages = firebase
        ...
        .onSnapshot((snapshot) => {
          ...
          // Reverse for chronological order since we fetched descending
          const sortedDocs = snapshot.docs.reverse();
          ...
          sortedDocs.forEach((doc) => {
            ...
            let isNewMessage =
              ...
              !this.messages.find((msg) => msg.id === doc.id);
            ...
            const now = new Date();
            const isEditable = isSender && now - messageDate <= 10 * 60 * 1000; // 10 mins in ms
            ...
            groupedMessages.push({
              ...
              createdAt: messageDate.toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit"
              }),
  ...
  getTimestampClass(msg) {
    return msg.senderClass === "message-sent"
      ? "timestamp timestamp-right"
      : "timestamp timestamp-left";
  }
  ...
  fetchAllUsers() {
    ...
    // ✅ Filter by orgId (already scoped users)
    const orgUsers = this.users.filter((user) => user.orgId === this.orgid);

    if (!orgUsers.length) {
      console.warn(`⚠️ No users found for orgId: ${this.orgid}`);
      return;
    }

    // ✅ Build the user list for group chat
    this.allUsersList = orgUsers.map((user) => {
      const isMandatory = (user.userId || user.id) === this.currentUserId;

      if (isMandatory) {
        console.log(
          `🔐 Logged-in user is mandatory in group: ${user.name} (ID: ${user.userId || user.id})`
        );
      }
    ...
    this.allUsersList.forEach((user) => {
      console.log(
        `🆔 ID: ${user.id}, 👤 Name: ${user.name}, ${user.isMandatory ? "⭐ MANDATORY" : ""}`
      );
    });
  }
  ...
  toggleGroupMembers() {
    ...
    if (this.showGroupMembers) {
      this.processedGroupParticipants = this.groupParticipants.map(
        (memberId) => {
          // Match using either userId or id
          let user = this.users.find(
            (u) => u.userId === memberId || u.id === memberId
          );

          const isAdmin = memberId === this.currentUserId;
          if (isAdmin) {
            console.log(`👑 Admin detected: ${user?.name || "Unknown"}`);
          }
  ...
  handleUserSelection(event) {
    ...
    if (event.target.checked) {
      ...
      this.selectedUsersForGroup = this.selectedUsersForGroup.filter(
        (id) => id !== userId
      );
    }
  ...
  handleSearchgc(event) {
    ...
    console.log(
      "🔍 Filtered Users:",
      this.filteredUsersForGroupgc.map((user) => user.name)
    );
  }
  ...
  async createGroupChat() {
    ...
    try {
      ...
      const participants = Array.from(
        new Set([...this.selectedUsersForGroup, this.currentUserId])
      );
  ...
  async fetchGroupChats() {
    try {
      if (!this.currentUserId || typeof this.currentUserId !== "string") {
        console.warn(
          "⚠️ fetchGroupChats: Current user ID is invalid or not set:",
          this.currentUserId
        );
        return;
      }
      ...
      if (!this.orgid) {
        console.warn("⚠️ fetchGroupChats: Org ID is missing.");
        return;
      }

      console.log(
        "🔄 Fetching existing group chats for user:",
        `"${this.currentUserId}" in Org: "${this.orgid}"`
      );

      const groupSnapshots = await firebase
        .firestore()
        .collection("groupChats")
        .where("orgId", "==", this.orgid)
        .where("participants", "array-contains", this.currentUserId.trim()) // ✅ Filter by current user
        .get();

      if (groupSnapshots.empty) {
        console.warn(
          "⚠️ No group chats found for this user in org:",
          `"${this.currentUserId}" / "${this.orgid}"`
        );
        this.groupChats = [];
        return;
      }
      ...
      this.groupChats = groupSnapshots.docs.map((doc) => {
        ...
        return {
          ...
          lastMessageTime: data.lastMessageAt
            ? this.formatLastMessageTime(new Date(data.lastMessageAt.toDate()))
            : "",
          admin: data.admin,
          orgId: data.orgId
        };
      ...
      this.groupChats.sort(
        (a, b) => (b.lastMessageAt || 0) - (a.lastMessageAt || 0)
      );

      console.log("✅ Group Chats Loaded:", this.groupChats.length);
      this.groupChats.forEach((chat, index) => {
        console.log(
          `#${index + 1} 🧵 Group: ${chat.name}, Participants: ${chat.participants.length}`
        );
      });
      ...
      .onSnapshot(
        (snapshot) => {
          ...
          snapshot.docs.forEach((doc) => {
            ...
            if (data.isPoll) {
              ...
              groupedMessages.push({
                ...
                pollOptions: data.pollOptions.map((opt) => ({
                  value: opt.value,
                  votes: opt.votes,
                  votersText: opt.voters
                    ? `Voted: ${opt.voters.join(", ")}`
                    : "No votes yet",
                  voters: opt.voters || [],
                  showVoters: false
                })),
                userVoted: data.votedUsers
                  ? data.votedUsers.includes(this.currentUserId)
                  : false,
                isSender: isSender,
                senderClass: senderClass,
                computedClass: computedClass,
                timestampClass: isSender
                  ? "timestamp timestamp-right"
                  : "timestamp timestamp-left",
            ...
            let messageText = data.isDeleted
            ...
            if (data.senderId === "SYSTEM") {
              ...
              groupedMessages.push({
                ...
                showOptions:
                  !data.isDeleted && data.senderId === this.currentUserId, // ✅ Hide edit/delete if deleted
          ...
          if (lastMessageAt) {
            firebase
              ...
              .update({
                ...
                lastMessageAt:
                  firebase.firestore.Timestamp.fromDate(lastMessageAt)
              })
              .then(() => {
                console.log(
                  `✅ Updated last message in group chat: "${lastMessage}"`
                );
              })
              .catch((error) => {
          ...
          requestAnimationFrame(() => {
            setTimeout(() => {
              const messageWrappers =
                this.template.querySelectorAll(".message-wrapper");
              console.log(
                "🟢 Found",
                messageWrappers.length,
  ...
  async votePollOption(event) {
    ...
    try {
      ...
      if (pollDoc.exists) {
        ...
        if (
          pollData.votedUsers &&
          pollData.votedUsers.includes(this.currentUserId)
        ) {
        ...
        const updatedOptions = pollData.pollOptions.map((opt) =>
          opt.value === selectedOption
            ? {
                ...opt,
                votes: opt.votes + 1,
                voters: [...(opt.voters || []), this.currentUserName]
              }
            : opt
        );
  ...
  //       this.currentUserName = currentUser.name || "User";
  //       this.currentUserAvatar = currentUser.avatar || "";
  //       console.log(`🔄 Refreshed senderName as: ${this.currentUserName}`);
  //     } else {
  //       console.warn("⚠️ Cannot resolve sender name before sending.");
  //       return;
  //     }
  //   }
  //   console.log("🧾 Sender Name to be saved:", this.currentUserName);
  ...
  //     }
  //     // this.listenToGroupMessages(this.selectedGroupId);
  //   } catch (error) {
  ...
  async sendGroupMessage() {
    ...
    const senderName = this.getCurrentUserName();
    ...
    try {
      ...
      const messageRef = db.collection("groupMessages").doc();
      ...
      console.log("✅ Group message sent and saved.");
    } catch (error) {
      console.error("❌ Error sending group message:", error);
    } finally {
      this.isSending = false;
  ...
  listenToGroupChats() {
    if (!this.currentUserId) {
      console.warn(
        "⚠️ Current user ID not set, unable to listen for group chats."
      );
      return;
    }
    ...
    // Cleanup previous groupChats listener
    if (this.unsubscribeGroupChats) {
      this.unsubscribeGroupChats();
    }

    // Track individual groupMessage listeners
    if (!this.groupMessageListeners) {
      this.groupMessageListeners = {};
    }

    this.unsubscribeGroupChats = firebase
      .firestore()
      .collection("groupChats")
      .where("participants", "array-contains", this.currentUserId)
      .onSnapshot(
        (snapshot) => {
          ...
          snapshot.docs.forEach((doc) => {
            ...
            let lastMessageAt = groupData.lastMessageAt
              ? new Date(groupData.lastMessageAt.toDate())
              : null;

            // ✅ Avoid duplicate listener per group
            if (this.groupMessageListeners[groupId]) {
              // Listener already active for this group
              const groupObj = {
                id: groupId,
                name: groupData.name,
              ...
              if (existingIndex !== -1) {
                updatedGroups[existingIndex] = groupObj;
              } else {
            ...
            // ✅ Listen to latest message for this group
            const unsubscribeMessage = firebase
              .firestore()
              .collection("groupMessages")
              .where("chatId", "==", groupId)
              ...
              .onSnapshot((messageSnapshot) => {
                if (!messageSnapshot.empty) {
                  messageSnapshot.forEach((msgDoc) => {
                    ...
                    lastMessage = messageData.isDeleted
                      ? "This message was deleted"
                      : messageData.text;
                    lastMessageAt = messageData.createdAt
                      ? new Date(messageData.createdAt.toDate())
                      : null;
                  });
                }
                ...
                console.log(
                  "✅ Real-time updated group chats:",
                  this.groupChats.map((g) => g.name)
                );
              });

            // ✅ Store listener for cleanup
            this.groupMessageListeners[groupId] = unsubscribeMessage;
          });
        },
        (error) => {
          console.error("❌ Firestore listener error:", error);
  ...
  handleClickOutside(event) {
    if (
      !this.template
        .querySelector(".options-menu-container")
  ...
  toggleEditPopup() {
    ...
    if (!this.groupParticipants.includes(this.groupAdminId)) {
      console.warn(
        "⚠️ Admin was missing from participants! Re-adding admin..."
      );
  ...
  processUsersList() {
    ...
    this.processedUsersList = this.users
      .map((user) => ({
        id: user.userId,
        name: user.name,
    ...
    console.log(
      "✅ Processed Users List (Admin First):",
      JSON.stringify(this.processedUsersList, null, 2)
    );
  }
  ...
  handleAdminUserSelection(event) {
    ...
    if (event.target.checked) {
      ...
      this.groupParticipants = this.groupParticipants.filter(
        (id) => id !== userId
      );
    }
  ...
  async saveGroupChanges() {
    ...
    try {
      ...
      // ✅ Find added users
      const addedUsers = this.groupParticipants.filter(
        (user) => !previousParticipants.includes(user)
      );

      // ✅ Find removed users
      const removedUsers = previousParticipants.filter(
        (user) => !this.groupParticipants.includes(user)
      );
      ...
      if (addedUsers.length > 0) {
        const addedNames = addedUsers
          .map((userId) => this.getUserNameById(userId))
          .join(", ");
      ...
      if (removedUsers.length > 0) {
        ...
        this.addSystemMessage(
          `${this.currentUserName} removed ${removedNames}`
        );
      }
    } catch (error) {
  ...
  async saveEditedMessage() {
    ...
    try {
      ...
      if (!lastMessageSnapshot.empty) {
        ...
        newLastMessage = lastMessageData.isDeleted
          ? "This message was deleted"
      ...
      this.messages = this.messages.map((msg) =>
        msg.id === this.editingMessageId
          ? { ...msg, text: this.editedText.trim(), isEdited: true }
          : msg
      );
  ...
  async saveEditedGroupMessage() {
    ...
    try {
      await firebase
        .firestore()
        .collection("groupMessages")
        .doc(this.editingGroupMessageId)
        .update({
      ...
      console.log(
        ...
        this.editingGroupMessageId
      );

      // ✅ Update UI
      this.messages = this.messages.map((msg) =>
        msg.id === this.editingGroupMessageId
          ? { ...msg, text: this.editedGroupText.trim(), isEdited: true }
          : msg
      );
  ...
  async confirmDeleteGroupMessage() {
    ...
    try {
      await firebase
        .firestore()
        .collection("groupMessages")
        .doc(this.deletingGroupMessageId)
        .update({
          text: "This message was deleted",
          isDeleted: true
        });
      ...
      console.log(
        "✅ Group message marked as deleted:",
        this.deletingGroupMessageId
      );
  ...
  async sendPollMessage() {
    ...
    console.log(
      "📌 [DEBUG] Original Poll Options:",
      JSON.stringify(this.pollOptions, null, 2)
    );
    ...
    if (emptyOptions.length > 0) {
      ...
      console.log(
        `❌ [DEBUG] Empty poll options detected:`,
        JSON.stringify(emptyOptions, null, 2)
      );
      return;
    }
    ...
    try {
      ...
      const pollData = {
        ...
        pollOptions: this.pollOptions.map((opt) => ({
          value: opt.value,
          votes: 0,
          voters: []
        })),
  ...
  async confirmDeleteGroupMessage() {
    ...
    try {
      ...
      if (pollData.isPoll) {
        ...
        console.log(
          "✅ Poll successfully marked as deleted in Firestore:",
          this.deletingGroupMessageId
        );
      } else {
        ...
        console.log(
          "✅ Group message marked as deleted in Firestore:",
          this.deletingGroupMessageId
        );
      }
      ...
      console.error(
        "❌ Error marking group message as deleted in Firestore:",
        error
      );
    }
  }
  ...
  async deleteUATUsersAndRelatedData() {
    try {
      console.log(
        "🔥 Deleting users, group chats, messages, and chats for env='R&D'..."
      );
      ...
      const groupChatsSnapshot = await firebase
        .firestore()
        .collection("groupChats")
        .where("env", "==", "R&D")
        .get();
      ...
      for (let i = 0; i < userIdsToDelete.length; i += chunkSize) {
        ...
        console.log(
          `✅ Deleted ${chatsSnapshot.size} chats for ${chunk.length} users.`
        );
      }
      ...
      for (let i = 0; i < userIdsToDelete.length; i += chunkSize) {
        ...
        const groupMessagesSnapshot = await firebase
          .firestore()
          .collection("groupMessages")
        ...
        console.log(
          `✅ Deleted ${groupMessagesSnapshot.size} group messages for ${chunk.length} users.`
        );
      }
  ...
  SESSION_KEY = "firebaseUsersWithTime";
  CACHE_TTL_MS = 10 * 60 * 1000; // 10 minutes

  // ✅ Get users from session cache
  getCachedFirebaseUsers() {
    const sessionData = sessionStorage.getItem(this.SESSION_KEY);

    if (!sessionData) {
      console.log("📦 No Firebase user cache found in session storage.");
      return null;
    }
    ...
    try {
      ...
      if (isValid) {
        console.log(
          `📦 Using cached Firebase users (valid for ${(this.CACHE_TTL_MS - age) / 1000}s)`
        );
        return parsed.users;
      } else {
        console.log("⏳ Firebase cache expired, ignoring session cache.");
        return null;
      }
    } catch (e) {
      console.warn("⚠️ Failed to parse cached Firebase users:", e);
      return null;
    }
  }

  // ✅ Store users in session cache
  cacheFirebaseUsers(users) {
  ...
  async pushDeltaUsersToFirebase(users) {
    if (!users.length || typeof firebase === "undefined") return;
    ...
    try {
      ...
      console.log(
        `🚀 Preparing to push ${users.length} delta user(s) to Firebase...`
      );

      users.forEach((user) => {
        const docRef = usersRef.doc(user.id.trim());
        console.log(
          `📤 Adding to batch - ID: ${user.id}, Name: ${user.name}, Email: ${user.email}`
        );
      ...
      await batch.commit();
      console.log("🔥 Delta users pushed to Firebase successfully.");
    } catch (e) {
      console.error("❌ Error pushing users to Firebase:", e);
    }
  }

  async syncUsersWithFirestore() {
    console.groupCollapsed("🔄 [SYNC] Firestore <-> Apex");

    try {
      const env = ENV_LABEL;
      const cacheKey = `firebaseUsers_${this.orgid}_${env}`;
      const cachedData = sessionStorage.getItem(cacheKey);

      // ✅ Use cached users if available
      if (cachedData) {
        const cachedUsers = JSON.parse(cachedData);
        console.log(`📦 Loaded ${cachedUsers.length} users from session cache`);

        cachedUsers.forEach((user, index) => {
          console.log(
            `🗃️ [CACHE ${index + 1}] ID: ${user.id}, Name: ${user.name}`
          );
        });

        this.users = cachedUsers;
        this.cacheFirebaseUsers(this.users); // Optional second-layer cache
        this.filterUsers();
        console.groupEnd();
        return;
      }

      // ✅ First load: go to Firestore + Apex
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        firestoreUsers.push({
          id: data.id?.trim() || doc.id,
          userId: data.userId?.trim() || data.id?.trim() || doc.id,
      ...
      console.log(`☁️ Fetched ${firestoreUsers.length} Firestore users`);

      // 2. Fetch Apex users
      const apexResult = await getAllUsers();
      const parsedApex = JSON.parse(apexResult);
      if (!parsedApex) {
        console.warn("⚠️ No Apex users returned");
        return;
      }

      const apexUsers = Object.values(parsedApex).flatMap((org) =>
        org.usersList.map((user) => ({
          id: user.userId || user.id || "",
      ...
      console.log("📦 Apex Users:", apexUsers.length);

      // 3. Compare Firestore and Apex by email
      const firestoreEmails = new Set(
        firestoreUsers.map((u) => u.email?.toLowerCase().trim())
      );

      const deltaUsers = apexUsers.filter(
        (u) =>
          u.email &&
          !firestoreEmails.has(u.email?.toLowerCase().trim()) &&
          u.orgId === this.orgid &&
          u.env === env
      );

      console.log(`🆕 New Users to Push: ${deltaUsers.length}`);
      console.table(deltaUsers.map((u) => ({ name: u.name, email: u.email })));

      // 4. Push delta users to Firestore
      if (deltaUsers.length > 0) {
        await this.pushDeltaUsersToFirebase(deltaUsers);
      } else {
        console.log("✅ Firestore already in sync with Apex users");
      }

      // 5. Merge lists
      this.users = [...firestoreUsers, ...deltaUsers].filter(
        (user) => user.orgId === this.orgid && user.env === env
      );

      console.log("✅ Final Synced Users:", this.users.length);

      // ✅ Cache merged users
      sessionStorage.setItem(cacheKey, JSON.stringify(this.users));
      this.cacheFirebaseUsers(this.users); // Optional secondary cache
      this.filterUsers();
    } catch (e) {
      console.error("❌ Error syncing Firestore with Apex:", e);
    }
