...
export default class TesseractAppsUserAccessLwc extends LightningElement {
    ...
    moduleMap = {
        ...
        'Facility Admin' : ['Roster Manager', 'Human Resources', 'Sign In', 'Participants','Accounting', 'T sign', 'Access Manager', 'Incident Register', 'Repository', 'My Profile', 'Forms', 'Reports'],
   ...
   @wire(getUserTypeValuesfromOrg, { selectedroleValue: '$selectedRole', userId: '$userId' })
    ...
    /* updateModules() {
        ...
        if (userType === 'NDIS Participant') {
            ...
            console.log('🔁 Normalized userType to:', userType);
        }

        if (userType === 'Facility Admin') {
            if (typeofUser === 'NDIS User') {
                this.availableModules = ['Admin', 'Roster Manager', 'Human Resources', 'Sign In', 'Participants', 'Accounting', 'T sign', 'Access Manager', 'Incident Register', 'Repository', 'My Profile', 'Reports'];
        ...
        // ✅ Replace 'Participants' with preferred name in display, but preserve original for matching
        this.modulesChecked = this.availableModules.map(module => {
            const isParticipant = module === 'Participants';
            const displayName = isParticipant && this.participantPreferredName
                ? this.participantPreferredName
                : module;

            return {
                name: displayName,                   // display name in UI
                internalName: module,               // original module name for matching
                checked: this.selectedModules.includes(module), // match against original
            };
        });

        console.log('this.modulesChecked >>', JSON.stringify(this.modulesChecked));
    } */
    ...
    updateModules() {
        ...
        this.modulesChecked = this.availableModules.map(module => {
            ...
            // Show Document instead of Repository
            ...
            return {
                name: displayName,      // UI label
                internalName: module,   // actual module value
    ...
    handleConfirmReset() {
        ...
        if (this.selectedModules.length > 0) {
            // ✅ Convert custom label back to 'Participants' before saving
            const modulesToSave = this.selectedModules.map(module =>
            ...
            updateUserModuleNames({ userIds: [this.userId], modules: modulesString })
                .then(() => {
                    ...
                    this.showToast('Success', 'Access to user modules granted successfully.', 'success');
                    this.refreshData(); // Trigger wire service refresh
                })
                .catch(error => {
