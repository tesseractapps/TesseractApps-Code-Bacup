...
@track sectionIcons = {
  ...
  Roster: { ...DEFAULT_ICON }
    ...
    //         // First click: Show the section
    //         this.sectionFlags[sectionId] = true;
    //         this.sectionIcons[sectionId] = '\u2B9F'; // Down arrow when expanded
    //     } else {
    //         // Subsequent clicks: Toggle the visibility
    ...
    //         } else {
    //             // If element doesn't exist, use the flag to determine icon
...
handleSectionToggle(event) {
  ...
  if (!this.sectionFlags[sectionId]) {
    ...
    async autoOpenBulkEntity() {
        ...
        // if (!this.isFromManageInvoice) {
        //     console.log('this.isFromManageInvoice : ', isFromManageInvoice);
        //     await this.checkCompanyExist();
        // } else {
        //     console.log('⏭ Skipping company check (already handled in Manage Invoice)');
        //     this.isCompanyExist = false;
        //     this.existingCompanyId = null;
        // }
...
// }
    ...
    //         // CRITICAL: Handle Type of Service and Documents
    //         console.log('📄 Setting up Type of Service and Documents...');
    ...
    //             // Step 3: Check for saved documents
    ...
    //             }
    //         } else {
    //             console.warn('⚠️ No Type of Service found. Documents dropdown will not be populated.');
    //             this.documentsList = [];
    //             this.documentsDisplayText = "Select Documents";
    //         }
...
@track disablePayrollSettingValue =false;
  ...
  handlestatuschange() {
    ...
    handleSubmit(event){
        ...
        // if(this.NdisFlag  && !this.BMSIDValue ){
        //      this.dispatchEvent(
        //         new ShowToastEvent({
        //             title: 'Error',
        //             message: 'Please complete the field BMS Identifier',
        //             variant: 'error'
        //         })
        //     );
        //     return; 
        // }
        //  if(this.NdisFlag  &&  !this.softwareIDValue){
        //      this.dispatchEvent(
        //         new ShowToastEvent({
        //             title: 'Error',
        //             message: 'Please complete the field Software ID',
        //             variant: 'error'
        //         })
        //     );
        //     return; 
  ...
  ConfirmClose (){
   ...
   handledetails(event){
    ...
    handleSelectAll(event) {
        ...
        console.log('Filtered Catalogs BEFORE Update:',
            JSON.parse(JSON.stringify(this.filteredCatalogs))
        );
        ...
        const updatedRecords = this.filteredCatalogs.map(item => {

            if (!item.isSelectedcheckBox) {

                console.log('Updating record:', item.Id);
            ...
            console.log('Skipping record (isSelectedcheckBox = true):', item.Id);

            return item;
        });

        console.log('Updated Records AFTER isSelected change:',
            JSON.parse(JSON.stringify(updatedRecords))
        );

        this.filteredCatalogs = updatedRecords;

        console.log('filteredCatalogs assigned');

        /* ---------------------------------------
        STEP 2: Update trackedRecords
        --------------------------------------- */

        console.log('trackedRecords BEFORE Update:',
            JSON.parse(JSON.stringify(this.trackedRecords))
        );

        if (checked) {

            console.log('Select All = TRUE → Adding all visible records');

            const map = new Map(
                this.trackedRecords.map(r => [r.Id, r])
            );

            console.log('Existing trackedRecords Map Size:', map.size);

            updatedRecords.forEach(rec => {
                console.log('Adding / Updating Record Id:', rec.Id);
                map.set(rec.Id, rec);
            });

            this.trackedRecords = Array.from(map.values());

            console.log('trackedRecords AFTER merge:',
                JSON.parse(JSON.stringify(this.trackedRecords))
            );

        } else {

            console.log('Select All = FALSE → Removing visible records');

            const visibleIds = new Set(
                updatedRecords.map(r => r.Id)
            );

            console.log('Visible Record Ids:', Array.from(visibleIds));

            this.trackedRecords = this.trackedRecords.filter(
                r => !visibleIds.has(r.Id)
            );

            console.log('trackedRecords AFTER removal:',
                JSON.parse(JSON.stringify(this.trackedRecords))
            );
        }
        ...
        console.log('Calling updateSelectAllCheckbox()');

        this.updateSelectAllCheckbox();

        console.log('isSelectAllChecked value now:', this.isSelectAllChecked);

        /* ---------------------------------------
        STEP 4: Refresh Pagination
        --------------------------------------- */

        console.log('Calling cataloguePaginate()');
        ...
        console.log('FINAL trackedRecords:',
            JSON.parse(JSON.stringify(this.trackedRecords))
        );

        console.log('FINAL filteredCatalogs:',
            JSON.parse(JSON.stringify(this.filteredCatalogs))
        );
    ...
    handleSaveforServiceCatalogue() {
        ...
        if (!this.trackedRecords || this.trackedRecords.length === 0) {
        ...
        const payload = JSON.stringify(this.trackedRecords);
    ...
    // ✅ ALWAYS call Apex
        /* processSelectedRecord({
            ...
            selectedRecordJson: JSON.stringify(updatedRecord)
        })
        .then(() => {
            console.log(
                checked
                    ? 'Record selected & activated'
                    : 'Record unselected & deactivated'
            );
        })
        .catch(error => {
    ...
    handleRowSelection(event) {
        ...
        // ✅ Store in track variable
        if (checked) {
            // add / replace
            const index = this.trackedRecords.findIndex(r => r.Id === recordId);

            if (index > -1) {
                this.trackedRecords[index] = updatedRecord;
                this.trackedRecords = [...this.trackedRecords];
            } else {
                this.trackedRecords = [...this.trackedRecords, updatedRecord];
            }
        } else {
...
get selectedTrainingsString() {
    ...
    //     try {
    //         console.log('Checking company for facility:', this.recordId);

    //        const companyId = await doesCompanyExist({
    //             facilityId: this.recordId
    //         });
    //         if (companyId) {
    //             this.isCompanyExist = true;
    //             this.existingCompanyId = companyId;
    ...
    //         console.error(error);
    //         this.showErrorToast(
    //             error.body?.message || 'Error checking company'
    //         );
    //     }
    // }
        ...
        try {
           ...
           // console.log('Fund Tracker Id:', this.createCompanyFundTrackerId);
           // let fundTrackerIdToSend = null;

            // if (this.isFromManageInvoice) {
            //     fundTrackerIdToSend = this.createCompanyFundTrackerId;
            // }
    ...
    // handleCreateCompanyBack(){
    //     this.facviewFlag=true;
    //     this.createCompanyFlag=false;
    //     this.isFromFacility = false;
    // }
    ...
    //     this.selectedServiceTypeLabel = event.target.value;

    //     console.log('Search Key:', searchKey);
    //     console.log('All Service Types:', JSON.parse(JSON.stringify(this.serviceTypeOptions)));

    //     // Always keep the first option
    //     const firstOption = this.serviceTypeOptions[0];

    //     // Filter remaining options
    //     const filtered = this.serviceTypeOptions.filter(service =>
    //         service.label.toLowerCase().includes(searchKey)
    //     );

    //     // Ensure first option is always first
    //     if (!filtered.some(s => s.value === firstOption.value)) {
    //         filtered.unshift(firstOption);
    //     }
    ...
    //     console.log('Filtered Services:', JSON.parse(JSON.stringify(this.filteredServiceTypes)));
    // }
    ...
    handleServiceTypeSearch(event) {
        ...
        if (!searchKey) {
            filtered = [...this.serviceTypeOptions];
        } else {
            filtered = this.serviceTypeOptions.filter(service =>
                service.label.toLowerCase().includes(searchKey)
            );
        }
