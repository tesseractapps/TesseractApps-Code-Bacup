...
@track  apiFieldMap = {
    ...
    renderedCallback() {
        ...
        .then(() => {
            ...
            console.log('🔌 jsPDF + autoTable initialized');
        })
        .catch(error => {
                ...
                getFacilityData()
                    .then(response => {
                        ...
                        if (this.facilityOptions.length > 0) {
                            ...
                            const selectedFacility = this.facilityOptions.find(opt => opt.value === this.facSelectedValue);
                            this.facSelectedLabel = selectedFacility ? selectedFacility.label : '';
    ...
    getFieldClass(fieldName) {
        return this.fieldErrorMap[fieldName] ? 'floating-label2' : 'floating-label';
    }
...
handleGetlocation(){
  ...
  getJSONdata( {shiftid: this.shiftID, shiftstatus: this.Status, sdate: this.Sdate} )
  .then(result => {
...
setLatitudeLongitudeData() {
  ...
  hideHandleLocation(){
        ...
        getMismatchedShifts({ staffId: staffId, weekStart: start, weekEnd: end })
            .then(shifts => {
                const filteredShifts = (shifts || []).filter(shift => {
                    ...
                    const signatureIconClass =  shift.Client_Signature__c ? 'material-icons signature-icon-blue' : 'material-icons signature-icon-red';
                    const isClickable = !isApproved;
                    const iconStyle = isClickable ? '' : 'pointer-events: none; opacity: 0.5;';
                    const signatureTitle = shift.Client_Signature__c ? 'Signature' : 'Signature';
                    ...
                    return {
                        ...
                        shiftType: shift.Type_of_shift__c === 'Sleepover Shift' ? 'Sleepover' : shift.Type_of_shift__c,
            ...
            .then(shifts => {
                ...
                const mismatchedShifts = (shifts || [])
                    ...
                    .map(shift => {
                        ...
                        const signatureIconClass =  shift.Client_Signature__c ? 'material-icons overview-icon signature-icon-blue' : 'material-icons overview-icon signature-icon-red';
                        const signatureTitle = shift.Client_Signature__c ? 'Signature' : 'Signature';
                        const isSignatureClickable = !!shift.Client_Signature__c;
                        ...
                        return {
                            ...
                            signatureTitle,
                            isSignatureClickable,
                            shiftType: (shift.Type_of_shift__c || '').trim().toLowerCase() === 'sleepover shift'
                                        ? 'Sleepover'
    ...
    handleEditClick(event) {
        ...
        getAddShiftDataById({ shiftId: this.shiftID }).then(result => {
            ...
            const buildRow = (label, startMs, endMs, rate, duration, index,sourceLabel) => {
            ...
            const tryBuildRow = (label, isActive, startTime, endTime, rate, duration, index,sourceLabel) => {
             if (isActive ) {
                     rows.push(buildRow(label, startTime, endTime, rate, duration, index,sourceLabel));
                }
            };

            tryBuildRow('Morning', shift.Is_long_Morning__c, shift.Long_Morning_Start_Time__c, shift.Long_Morning_End_Time__c, shift.Long_Morning_Hourly_Rate__c, shift.Long_Morning_Shift_Duartion__c, shift.Long_Morning_Index__c,'morningShift');
            tryBuildRow('Afternoon', shift.Is_Long_Afternoon__c, shift.Long_Afternoon_Start_Time__c, shift.Long_Afternoon_End_Time__c, shift.Long_Afternoon_Hourly_Rate__c, shift.Long_Afternoon_Shift_Duartion__c, shift.Long_Afternoon_Index__c,'afternoonShift');
            tryBuildRow('Night', shift.Is_Long_Night_Shift__c, shift.Long_Night_Start_Time__c, shift.Long_Night_End_Time__c, shift.Long_Night_Hourly_Rate__c, shift.Long_Night_Shift_Duartion__c, shift.Long_Night_Index__c,'nightShift');
            tryBuildRow('Sleepover Shift', shift.Is_Long_SleepOver__c, shift.Long_Sleepover_Start_Time__c, shift.Long_Sleepover_End_Time__c, shift.Long_Sleepover_Allowance__c, shift.Long_Sleepover_Duartion__c, shift.Long_Sleepover_index__c,'sleepOverShift');
            tryBuildRow('Morning', shift.Is_long_Morning_Shift_two__c, shift.Long_Morning_Start_Time_Two__c, shift.Long_Morning_End_Time_Two__c, shift.Long_Morning_Rate_Two__c, shift.Long_Morning_Duartion_Two__c, shift.Long_Morning_Index_Two__c,'morningShiftTwo');
            tryBuildRow('Afternoon', shift.Is_Long_afternoon_shift_two__c, shift.Long_Afternoon_Start_Time_two__c, shift.Long_afternoon_End_Time_Two__c, shift.Long_Afternoon_Rate_Two__c, shift.Long_Afternoon_Duration_Two__c, shift.Long_Afternoon_Index_Two__c,'afternoonshiftTwo');
       ...
       //     this.IsLongShift = this.rateRows.length >= 1;
            ...
            console.log('this.rateRows:  in   final edit ', JSON.stringify(this.rateRows)); 
 ...
 handleRateRowInputChange(event) {
    ...
    this.rateRows = [...this.rateRows];
        console.log('this.rateRows:  in   during onchange  edit ', JSON.stringify(this.rateRows)); 
    ...
    async handlesubmit(event) {
        ...
        if (totalHours == 0) {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message: 'Total hours cannot be zero. Please approve a shift with valid duration',
                    variant: 'error'
                })
            );
            return;
  ...
  convertTo24HourFormat(timeStr) {
    ...
    handlegeolocationClick(event){
        ...
        console.log('Activity clicked for shift ID:', shiftId+''+this.Sdate+''+this.Status );
        this.shiftID = shiftId;
    ...
    handleSignature(event){
         ...
         console.log('Activity clicked for shift ID:', shiftId+''+this.Sdate+''+ this.Sign );
        ...
        if(validShifts.length == 0){
             this.dispatchEvent(
                new ShowToastEvent({
                    ...
                    message: 'Please complete at least one shift before approving the timesheet.',
                    variant: 'Info'
                })
            );
        }
    ...
    get totalDuration() {
        return this.selectedStaffShifts.reduce((sum, shift) => {
    ...
    handleExport(){
        ...
        console.log('this.isModalOpenfortimesheetExport >>', this.isModalOpenfortimesheetExport);
        ...
        if(this.shiftInformation == true){
            if (this.frequency === 'Fortnightly' || this.frequency === 'Monthly' || this.frequency === 'Weekly') {
                console.log(`🗓 Frequency: ${this.frequency} → Fetching mismatched shifts`);
                console.log('this.staffId >>', this.StaffId);
                ...
                .then(shifts => {
                    console.log('✅ Raw shifts returned from Apex >>', JSON.parse(JSON.stringify(shifts)));
                    ...
                    const processedShifts = (shifts || []).map(shift => {
                        ...
                        console.log(`   ➡️ Mileage Totals for Shift [${shift.Id}] → Others: ${totalMileageOthers}, Amount: ${totalMileageAmount}`);
                        ...
                        return {
                            ...
                            formattedLogin: shift.Login_Time_Formula__c,
                            formattedLogout: shift.Logout_Time_Formula__c,
                            duration: isNaN(shift.Actual_Duration__c) ? 0 : shift.Actual_Duration__c,
                            shiftDate: this.formatDateToDDMMYYYY(shift.Date__c),
                            serviceTypeShort: shift.Services_and_Support_Plans__r?.[0]?.Service_Type_Name__c?.substring(0, 30) + '...' || '--',
                            serviceType: shift.Services_and_Support_Plans__r?.[0]?.Service_Type_Name__c || '--',
                            facilityName: shift.Facility__c || '--',
                    ...
                    console.log(`✅ Processed shifts count: ${processedShifts.length}`, JSON.parse(JSON.stringify(processedShifts)));

                    // Assign values back
                    this.selectedStaffShiftsforPDF = processedShifts;
                    this.selectedShiftIds = processedShifts.map(shift => shift.Id);

                    this.totalShiftDuration = processedShifts.reduce((sum, shift) => sum + (parseFloat(shift.duration) || 0), 0);
                    this.totalKms = processedShifts.reduce((sum, shift) => sum + (parseFloat(shift.mileageOthersTotal) || 0), 0);
                    this.totalexpenses = processedShifts.reduce((sum, shift) => sum + (parseFloat(shift.mileageAmountTotal) || 0), 0);
                    this.sleepoverDuration = processedShifts.reduce((sum, shift) => sum + (parseFloat(shift.Long_Sleepover_Duartion__c) || 0), 0);
                    this.totalApprovedLoggedDuration = processedShifts.reduce((sum, shift) => {
                        return shift.Approval_Status__c === 'Approved' ? sum + (parseFloat(shift.duration) || 0) : sum;
                    }, 0);
    ...
    handleDownloadPDF() {
        try {
            ...
            const serviceOrParticipantLabel = this.otherThanNdis ? 'Participant' : 'Service';

            const headers = [
                "Date", "Shift", serviceOrParticipantLabel, this.facilityPreferredName, "Start Time",
                "End Time", "Break(min)", "Duration (hr)", "Mileage (km)", "Expense", "Variance (hr)", "Signature"
            ];
            ...
            const totalDuration = this.selectedStaffShifts.reduce((sum, shift) => sum + parseFloat(shift.duration), 0);
            const totalExpenses = this.selectedStaffShifts.reduce((sum, shift) => sum + parseFloat(shift.mileageAmountTotal || 0), 0);
            ...
            doc.text(`Total Duration: ${totalDuration.toFixed(2)} hours`, 10, finalY + 16);
    ...
    handleDownloadCSV() {
        try {
            ...
            const serviceOrParticipantLabel = this.otherThanNdis ? 'Participant' : 'Service';
            const headers = [
                "Date", "Shift", serviceOrParticipantLabel, "Facility", "Start Time",
                "End Time", "Break(min)", "Duration (hr)", "Mileage (km)", 
                "Expense", "Variance (hr)", "Signature"
            ];

            // Data Rows
            const dataRows = (this.selectedStaffShifts || []).map(shift => [
                shift.shiftDate,
                shift.shiftType,
                this.otherThanNdis ? shift.clientName : shift.serviceType,
                shift.facilityName,
            ...
            // Build CSV content
            let csvContent = [headers, ...dataRows].map(row => row.join(",")).join("\n");

            // Add Summary Section
            const totalShifts = (this.selectedStaffShifts || []).length;
            const totalDuration = (this.selectedStaffShifts || []).reduce((sum, shift) => sum + parseFloat(shift.duration || 0), 0);
            const totalExpenses = (this.selectedStaffShifts || []).reduce((sum, shift) => sum + parseFloat(shift.mileageAmountTotal || 0), 0);
    ...
    get modalClass() {
        return this.isModalOpenfortimesheetExport ? 'slds-modal slds-fade-in-open' : 'slds-modal';
    }

    get backdropClass() {
        return this.isModalOpenfortimesheetExport ? 'slds-backdrop slds-backdrop_open' : 'slds-backdrop';
    }
    ...
    computeCellCss(hasVariance, isApproved) {
        return hasVariance ? 'variance-cell' : (isApproved ? 'approved-cell' : 'default-cell');
    }
    ...
    createMockShifts() {
        ...
        this.staffDirectory.forEach(staffRecord => {
            ...
            staffSummary.totalHours = staffSummary.allShifts.reduce((sum, s) => sum + (s.Duration__c || 0), 0);
            staffSummary.totalMileage = staffSummary.allShifts.reduce((sum, s) => sum + (s.Mileage__c || 0), 0);
            staffSummary.totalExpense = staffSummary.allShifts.reduce((sum, s) => sum + (s.Expense__c || 0), 0);
            staffSummary.sleepoverTotal = staffSummary.allShifts.reduce((sum, s) => sum + (s.Sleepover_Shift_Hours__c || 0), 0);
            staffSummary.varianceTotal = staffSummary.allShifts.reduce((sum, s) => sum + (s.Variance_Wage__c || 0), 0);
            ...
            staffSummary.extendedTotal = parseFloat(
                staffSummary.allShifts.reduce((sum, s) => sum + (parseFloat(s.Extended_Duration__c) || 0), 0).toFixed(2)
            );
            ...
            staffSummary.allShifts.forEach(s => {
                if (Array.isArray(s.Reimbursements__r)) {
                    s.Reimbursements__r.forEach(r => {
                        staffSummary.mileageAmountSum += parseFloat(r.Mileage_Amount__c || 0);
                        staffSummary.mileageOthersSum += parseFloat(r.Mileage_Others__c || 0);
                    });
                }
            });

            staffSummary.mileageAmountSum = parseFloat((staffSummary.mileageAmountSum || 0).toFixed(2));
            staffSummary.mileageOthersSum = parseFloat((staffSummary.mileageOthersSum || 0).toFixed(2));

            this.memberSummaries.push(staffSummary);
        });

        console.log('✅ Final memberSummaries:', JSON.stringify(this.memberSummaries, null, 2));
    ...
    get paginatedSummaries() {
        ...
        console.log('➡️ currentPage:', this.currentPage, 'pageSize:', this.pageSize);
        console.log('➡️ start:', start, 'end:', end);
        console.log('📄 paginatedSummaries:', JSON.parse(JSON.stringify(paginated)));
    ...
    get processedSummaries() {
        ...
        return this.paginatedSummaries.map((staff) => {
            ...
            console.log('✅ Processed Staff12345:', JSON.stringify(processedStaff, null, 2));
            return processedStaff;
        });
    }
    ...
    handleDownloadClickfortimesheet() {
        ...
        // Add totals
        header.push('Total (hr)', 'Mileage (km)', 'Expense', 'Sleepover (hr)', 'Variance (hr)');
    ...
    /* handleDownloadPDFFortimesheet() {
        ...
        this.processedSummaries.forEach(staff => {
            ...
            let totalHours = 0, totalMileage = 0, totalExpense = 0, totalSleepover = 0, totalVariance = 0;
        ...
        // 👉 Add header text
        doc.text(`Facility: ${this.facSelectedLabel || 'N/A'}`, 148, 15, { align: 'center' });

        // 👉 Build table with fixed column widths
        const colWidths = [30]; // First column "Staff"
        const totalCols = header.length - 1; // exclude Staff
        const dynamicWidth = (doc.internal.pageSize.getWidth() - 40) / totalCols; // distribute rest space
        for (let i = 0; i < totalCols; i++) {
            colWidths.push(dynamicWidth);
        }
        ...
        doc.autoTable({
            ...
            body: body,
            startY: 25,
            margin: 5, // equal margin on all sides
            theme: 'grid',
            styles: {
                fontSize: 7,
                halign: 'center',
        ...
        // 👉 Save file with dd-mm-yy format
        const today = new Date();
    ...
    handleDownloadPDFFortimesheet() {
        ...
        dateChunks.forEach((chunk, chunkIndex) => {
            // Add a new page for every table except the first
            if (chunkIndex > 0) {
                doc.addPage();
            }

            // Add facility header at the top of each page
            doc.text(`${this.facilityPreferredName || 'N/A'}: ${this.facSelectedLabel || 'N/A'}`, 148, 15, { align: 'center' });

            const header = ['Staff', ...chunk.map(d => formatDate(d)), 'Total (hr)', 'Mileage (km)', 'Expense', 'Sleepover (hr)', 'Variance (hr)'];
            ...
            const colWidths = [30];
            const dynamicWidth = (doc.internal.pageSize.getWidth() - 40) / (header.length - 1);
            for (let i = 0; i < header.length - 1; i++) colWidths.push(dynamicWidth);

            doc.autoTable({
                head: [header],
                body: body,
                startY: 25, // start below header
                margin: 5,
                theme: 'grid',
                styles: { 
                    fontSize: 7, 
                    halign: 'center', 
                    valign: 'middle',       // Increased padding for body rows only
                    minCellHeight: 15  
                },
                headStyles: { 
                    fillColor: [41, 128, 185], 
