...
handlesavebutton(event) {
    ...
    .then(result => {
        ...
        result.forEach(fund => {
            console.log(`🔄 Mapping fund: ID=${fund.Id}, Available=${fund.Available_Funds__c}, Name=${fund.Name}`);
            fundMap.set(fund.Id, {
        ...
        if (!fund) {
            console.error('❌ Fund not found in result for serviceTypevalue:', this.serviceTypevalue);
        ...
        console.log(`💰 Fund Check: Name="${this.fundsName}", Available=${fund.available}, Used (Single)=${totalAmount}`);

        // Step 3: Validate Single Shift Amount
        if (!this.AddShiftRecurringCheckboxValue && totalAmount > fund.available) {
            console.error(`❌ Fund exceeded for single use. Used=${totalAmount.toFixed(2)}, Available=${fund.available.toFixed(2)}`);
            this.showToast(
                'Error',
        ...
        console.log(`🔁 Recurring Enabled: ${this.AddShiftRecurringCheckboxValue}, Occurrences: ${this.numberOfOccurrences}`);
        console.log(`🧮 Recurring Total Amount: ${totalAmount} x ${this.numberOfOccurrences} = ${totalAmountinReccuring}`);

        if (this.AddShiftRecurringCheckboxValue && totalAmountinReccuring > fund.available) {
            console.error(`❌ Recurring total exceeded. Total=${totalAmountinReccuring.toFixed(2)}, Available=${fund.available.toFixed(2)}`);
            this.showToast(
                'Error',
        ...
        console.log('📤 Sending payload to Apex → saveShiftData:', JSON.stringify(payload));
