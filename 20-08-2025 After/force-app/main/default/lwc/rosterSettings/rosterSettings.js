...
export default class RosterSettings extends LightningElement {
    ...
    handleSaveAllShifts() {
        ...
        console.log('📤 Sending Custom shifts to Apex:', JSON.stringify(this.templates));

        // ✅ Filter out empty custom templates
        const cleanedTemplates = this.templates.filter(template => {
            // Keep if at least one child shift has start or end time filled
            return template.shifts.some(shift => {
                return (
            ...
            results.forEach((result, index) => {
                if (result && result.Id) {
                    if (index < totalDefault) {
                        ...
                        console.log('📤 Sending Custom shifts to Apex:', JSON.stringify(this.shifts));
                        ...
                        console.log('📤 Sending Custom shifts to Apex:', JSON.stringify(this.shiftsforDesign));
                    } else {
                        // Custom shift
    ...
    validateShift(shift) {
        const startTimeMs = shift.startTime24 ?? this.getMsFromParts(shift.startHour, shift.startMinute, shift.startAMPM);
        const endTimeMs   = shift.endTime24 ?? this.getMsFromParts(shift.endHour, shift.endMinute, shift.endAMPM);
    ...
    handleStartTimeChangeforCustom(event) {
        ...
        // ✅ After updating, validate sequence rule
        const updatedTemplate = this.templates.find(temp => temp.id === templateId);

        if (updatedTemplate && updatedTemplate.shifts.length > 1) {
            let hasMismatch = false;

            // Loop through each row except the last one, compare End Time with next row's Start Time
            for (let i = 0; i < updatedTemplate.shifts.length - 1; i++) {
                const currentEnd = updatedTemplate.shifts[i].endTime24 || updatedTemplate.shifts[i].end_time;
                const nextStart  = updatedTemplate.shifts[i + 1].startTime24 || updatedTemplate.shifts[i + 1].start_time;

                if (currentEnd && nextStart && currentEnd !== nextStart) {
                    hasMismatch = true;
                    console.error(
                        `⛔ Mismatch found: Row ${i + 1} End (${currentEnd}) ≠ Row ${i + 2} Start (${nextStart})`
                    );
            ...
            if (hasMismatch) {
                this.showErrorMessage('Start Time of a shift must match End Time of the Last shift.');
            }
        }
    }
            ...
            if (template.id === templateId) {
                ...
                const updatedShifts = template.shifts.map(shift => {
                    ...
                    if (shift.id === shiftId) {
                        ...
                        console.log(`   ❌ Shift ID ${shift.id} does not match`);
                    }
                    return shift;
                });
