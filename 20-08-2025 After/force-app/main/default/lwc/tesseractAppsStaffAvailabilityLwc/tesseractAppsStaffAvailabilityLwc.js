       ...
       /*  this.DocumentTableData = []; */
        ...
        for (let rec of childRec) {
           /*  if (rec.Type__c) {
            ...
            if (
                rec.Compliance__c === true && new Date(rec.Expiry_Date__c) < today) {
                this.documentExpired = true;
                console.log('Expired doc',this.documentExpired);
                break; // ⬅️ Stop as soon as expired compliance found
            }
        }
    });
}
...
confirmDelete(){
    ...
    get calendarDays() {
        ...
        const availabilityMap = new Map();
        this.availability.forEach(slot => {
            const normalizedDate = new Date(slot.Start_Date__c).toISOString().split('T')[0];
        ...
        console.log('todayStr1 >>',todayStr1);
        ...
        for (let i = 1; i <= totalDays; i++) {
            ...
            // Log for debugging 31st
            if (i === 31) {
                console.log('📅 dateStr (31st):', dateStr);
                console.log('📅 availabilityMap has:', availabilityMap.has(dateStr));
           ...
           /*  const displayLimit = 3;
            ...
            const hasMore = availabilitySlots.length > displayLimit;
            const visibleSlots = availabilitySlots.slice(0, displayLimit).map(slot => ({
                id: slot.Id,
                startDate: slot.Start_Date__c,
                isAssigned: slot.Assigned_Service_OR_Shift__c,
                startTime: this.convertMillisTo12Hour(slot.Start_Time__c),
...
fetchAvailability() {
    ...
    getStaffAvailability({ datePicker: formattedDate })
        .then(response => {
            // Normalize Start_Date__c to YYYY-MM-DD
            this.availability = response.map(item => {
                return {
                    ...item,
                    Start_Date__c: new Date(item.Start_Date__c).toISOString().split('T')[0],
                    startTimeFormatted: this.convertMillisTo12Hour(item.Start_Time__c),
                    endTimeFormatted: this.convertMillisTo12Hour(item.End_Time__c)
                };
            });

            console.log('Normalized availability:', JSON.stringify(this.availability));
        })
        .catch(error => {
            console.error('Error fetching services:', error);
        });
        ...
        .then(result => {
            ...
            const userType = result.Usertype;
                if(userType == 'NDIS Org Admin'){
                    ...
                    this.facilityId = result.staff.Facility__c;
...
getOrganisationTimingsRosterOrFacilityAdmin (){
      getfacilityById({ facId: this.facilityId })
        ...
        switch (this.shiftTypeValue) {
            case 'Morning':
                if (!this.validateShiftFields(facilityresult.Morning_Shift_Start_Time__c, facilityresult.Morning_Shift_End_Time__c)) return;
                this.AddShiftStartTimeAMPM = facilityresult.Morning_Shift_Start_Time__c?.toUpperCase();
                this.AddShiftEndTimeAMPM = facilityresult.Morning_Shift_End_Time__c?.toUpperCase();
                this.AddShiftStartTime = this.convertTo24HourFormat(facilityresult.Morning_Shift_Start_Time__c?.toLowerCase());
                this.AddShiftEndTime = this.convertTo24HourFormat(facilityresult.Morning_Shift_End_Time__c?.toLowerCase());
                this.disableTimeButton=true;
            ...
            case 'Afternoon':
                if (!this.validateShiftFields(facilityresult.Afternoon_Shift_Start_Time__c, facilityresult.Afternoon_Shift_End_Time__c)) return;
                this.AddShiftStartTimeAMPM =  facilityresult.Afternoon_Shift_Start_Time__c?.toUpperCase();
                this.AddShiftEndTimeAMPM = facilityresult.Afternoon_Shift_End_Time__c?.toUpperCase();
                this.AddShiftStartTime = this.convertTo24HourFormat(facilityresult.Afternoon_Shift_Start_Time__c?.toLowerCase());
                this.AddShiftEndTime = this.convertTo24HourFormat(facilityresult.Afternoon_Shift_End_Time__c?.toLowerCase());
                this.disableTimeButton=true;
                //this.checkFatigue();
            ...
            case 'Night':
                if (!this.validateShiftFields(facilityresult.Night_Shift_Start_Time__c, facilityresult.Night_Shift_End_Time__c)) return;
                this.AddShiftStartTimeAMPM = facilityresult.Night_Shift_Start_Time__c?.toUpperCase();
                this.AddShiftEndTimeAMPM = facilityresult.Night_Shift_End_Time__c?.toUpperCase();
                this.AddShiftStartTime = this.convertTo24HourFormat(facilityresult.Night_Shift_Start_Time__c?.toLowerCase());
                this.AddShiftEndTime = this.convertTo24HourFormat(facilityresult.Night_Shift_End_Time__c?.toLowerCase());
                this.disableTimeButton=true;
                //this.checkFatigue();
            ...
            case 'General':
                if (!this.validateShiftFields(facilityresult.General_Shift_Start_Time__c, facilityresult.General_Shift_End_Time__c)) return;
                this.AddShiftStartTimeAMPM = facilityresult.General_Shift_Start_Time__c?.toUpperCase();
                this.AddShiftEndTimeAMPM = facilityresult.General_Shift_End_Time__c?.toUpperCase();
                this.AddShiftStartTime = this.convertTo24HourFormat(facilityresult.General_Shift_Start_Time__c?.toLowerCase());
                this.AddShiftEndTime = this.convertTo24HourFormat(facilityresult.General_Shift_End_Time__c?.toLowerCase());
                this.disableTimeButton=true;
                //this.checkFatigue();
            ...
            case 'Custom':
                if (!this.validateShiftFields(facilityresult.Custom_Shift_Start_Time__c, facilityresult.Custom_Shift_End_Time__c)) return;
                this.IsLongShift=true;
                this.AddShiftStartTimeAMPM = facilityresult.Custom_Shift_Start_Time__c?.toUpperCase();
                this.AddShiftEndTimeAMPM = facilityresult.Custom_Shift_End_Time__c?.toUpperCase();
                this.AddShiftStartTime = this.convertTo24HourFormat(facilityresult.Custom_Shift_Start_Time__c?.toLowerCase());
                this.AddShiftEndTime = this.convertTo24HourFormat(facilityresult.Custom_Shift_End_Time__c?.toLowerCase());
                this.disableTimeButton=false;
                this.cutsomShiftTemplate=true;
                //this.checkFatigue();
                ...
                break;
            case 'Sleepover Shift':
                if (!this.validateShiftFields(facilityresult.Sleepover_Shift_Strat_Time__c, facilityresult.Sleepover_Shift_End_Time__c)) return;
                this.AddShiftStartTimeAMPM = facilityresult.Sleepover_Shift_Strat_Time__c?.toUpperCase();
                this.AddShiftEndTimeAMPM = facilityresult.Sleepover_Shift_End_Time__c?.toUpperCase();
                this.AddShiftStartTime = this.convertTo24HourFormat(facilityresult.Sleepover_Shift_Strat_Time__c?.toLowerCase());
                this.AddShiftEndTime = this.convertTo24HourFormat(facilityresult.Sleepover_Shift_End_Time__c?.toLowerCase());
                this.disableTimeButton=false;
                //this.checkFatigue();
...
handlesavebutton(event) {
    ...
    if (!this.AddShiftRecurringCheckboxValue && this.shiftTypeValue != 'Sleepover Shift') {
        const shiftDate = this.todayDate;
        const startTime = this.AddShiftStartTimeAMPM;
        const endTime = this.AddShiftEndTimeAMPM;
        const staffId = this.StaffId;
        const avaliableId = this.AvaliableId;

        processSingleShift({ avaliableId, shiftDate, startTime, endTime, staffId })
            .then((result) => {
        ...
        // ✅ If recurring, proceed without check
        this.executeSaveStaffData();
    }
} 
