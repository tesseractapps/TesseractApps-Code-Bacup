...
export default class TesseractAppsBankingRecieveMoneyLwc extends LightningElement {
   ...
   // @track urlforEmail;
    ...
    connectedCallback(){
         ...
         console.log("TesseractAppsBankingRecieveMoneyLwc");
         ...
         if (storedCompanyId && storedCompanyName) {
             ...
             console.log('Company from localStorage connectedCallback bank receive money:', this.selectedCompany, this.companyname);
         } else {
             console.warn('No company info found in localStorage connectedCallback bank receive money');
         }        
        console.log("this.companyname in receive money"+this.companyname);
        console.log("this.selectedCompany in receive money"+this.selectedCompany);
    ...
    handleReceiveMoneyChange(event) {
    ...
    updateMasterCheckboxState() {
        ...
        if (allSelected) {
            ...
            if(this.totalPaid && this.outOfBalance==0){
                this.disabledSubmit = false;
    ...
    //                 ...invoice,
    //                 isEditing: true,
    //                 toPayValue: parseFloat(invoice.toPay?.replace('$', '')) || 0
    //             };
    //         }
    //         return invoice;
    //     });
        ...
        let updatedTable = this.invoiceRecordsTable.map(invoice => {
            if (invoice.Id === invoiceId) {
                ...
                if (newPaid > amount  || newPaid > balance) {
                    this.showToast('Error', 'Received value cannot exceed row amount or balance .', 'error');

                    return {
                        ...invoice,
                        toPayValue: 0,      // clear input
    ...
    handleDropdownPosition(event) {
        ...
        // const top = rect.top + scrollY + rect.height - (window.innerHeight * 0.04)-27; // subtract 2% from Y
        // const left = rect.left + scrollX - (window.innerWidth * 0.165)+34; // subtract 10% from X
    ...
    //         console.log(' this.entryType  in submit:', this.entryType );
    //         return {
    //             Id: row.Id,
    ...
    //     .then(result => {
    //         console.log('✅ Invoices successfully updated:', result);
    //         this.dispatchEvent(
    //             new ShowToastEvent({
    //                 title: 'Success',
    //                 message: `${selectedRows.length} invoice(s) submitted successfully.`,
    //                 variant: 'success'
    //             })
    //         );
    ...
    async handleRemittanceSubmit() {
        try { 
            ...
            console.log('✅ Receipts submitted successfully:', paymentId);
            this.showToast('Success', ' Receipts submitted successfully', 'success');
    ...
    updatePayFromAccountLedgerId() {
        updateLedgerItemId({ ledgerItemId: this.selectedLedgerItemId ,amount:this.amount, category: this.selectedOptionCategory , entryType:this.entryType  })
            .then(result => {
    ...
    console.log('RESULT'+JSON.stringify(result));
        ...
        doc.autoTable({
            ...
            head: [["Invoice No", "Date", "Total", "Received to Date", "Recieved", "Balance"]],
            body: result,
            theme: "plain",
            /* styles: { halign: "left" }, */
            margin: { left: 10 },
        ...
        didParseCell: function (data) {
            ...
            // Highlight Overpayment row
            if (overRowIndex !== null && row === overRowIndex) {
        ...
        didDrawCell: function (data) {
            ...
            if (overRowIndex !== null &&
                data.section === "body" &&
                data.row.index === overRowIndex) {
                return;
            }
