...
from "@salesforce/apex/LayoutController.doesLayoutTitleExist";

import doesPublishedFormExist
from "@salesforce/apex/FormController.doesPublishedFormExist";
...
//   @track dataTypes = [
//   { id: '1', label: 'Text Field',     icon: 'short_text' },
//   { id: '2', label: 'Number Field',   icon: 'calculate' },
//   { id: '3', label: 'Date Field',     icon: 'calendar_today' },
//   { id: '4', label: 'Time Field',     icon: 'schedule' },
//   { id: '5', label: 'Checkbox Field', icon: 'check_box' },
//   { id: '6', label: 'Dropdown Field', icon: 'arrow_drop_down_circle' },
//   { id: '7', label: 'Blank',          icon: 'check_box_outline_blank' },
//   { id: '8', label: 'Upload File',    icon: 'upload_file' },
//   { id: '9', label: 'Radio Button',   icon: 'radio_button_checked' },
//   { id: '10', label: 'Header',         icon: 'title' }
// ];
...
//   @track dataTypes = [
//   { id: '1', label: 'Text',     icon: 'short_text' },
//   { id: '2', label: 'Number',   icon: 'calculate' },
//   { id: '3', label: 'Date',     icon: 'calendar_today' },
//   { id: '4', label: 'Time',     icon: 'schedule' },
//   { id: '5', label: 'Checkbox', icon: 'check_box' },
//   { id: '6', label: 'Dropdown', icon: 'arrow_drop_down_circle' },
//   { id: '7', label: 'Blank',          icon: 'check_box_outline_blank' },
//   { id: '8', label: 'Upload',    icon: 'upload_file' },
//   { id: '9', label: 'Radio Button',   icon: 'radio_button_checked' },
//   { id: '10', label: 'Header',         icon: 'title' }
// ];
...
async handlePublish() {
  if (this.isLoading) {
    console.warn("⏳ Publish already in progress — blocked duplicate click");
    return;
  }
  ...
  try {
    const isFormTitleMissing = !this.formTitle;
    const isFormForMissing = !this.audience;
    const isFormTypeMissing = !this.selectedFormType;

    if (isFormTitleMissing || isFormForMissing || isFormTypeMissing) {
      ...
      if (isFormForMissing) {
        errorMessage += 'Form For, ';
      }

      if (isFormTypeMissing) {
        errorMessage += 'Form Type, ';
      }
      ...
      this.showToast(
        ...
        `${errorMessage} ${errorMessage.includes(',') ? 'are' : 'is'} required.`,
        'error'
      );
    ...
    this.showToast(
      "Error",
      error?.body?.message || error?.message || "Failed to publish form.",
      "error"
    );

  } finally {
    this.isLoading = false;
    console.log("⏹️ [Publish] Finished — spinner OFF");
  }
}
  ...
  loadClonedLayout(message) {
    try {
      ...
      this.selectedModuleType =
        message.formModule || this.layoutData.selectedModuleType || "";
      ...
      console.log("📌 Form Type set to:", this.selectedFormType);
       ...
       this.audience =
      message.audience ||
      this.layoutData?.audience ||
      "";

    console.log("👥 Audience prefilled:", this.audience);
...
async loadLayoutById(layoutId) {
  try {
    ...
    this.selectedFormType = layout.displayName || ""; // or whatever you use as form type
     ...
     this.audience = layout.audience || layout.Audience__c || "";

    console.log("Loaded audience:", this.audience);
 ...
 handleUndo() {
  ...
  // Last snapshot is the current state, second-to-last is the previous
  ...
  if (removedFields.length > 0) {
    console.log(
      "🧹 Undo removed the following field(s):",
      removedFields.join(", ")
    );
  } else {
  ...
  // 🔹 Keep column count in sync with restored table
...
handleAddRow() {
  // 1) Work out how many columns the table currently has
  let cols = 0;

  if (this.tableRows && this.tableRows.length) {
    cols = this.tableRows.reduce(
      (max, row) => Math.max(max, (row.cells ? row.cells.length : 0)),
      0
    );
  }

  // Fallback to currentCols or at least 1
  if (!cols || cols < 1) {
    cols = this.currentCols && this.currentCols > 0 ? this.currentCols : 1;
  }
  ...
  const newRow = { id: `row-${rowIndex}`, cells: [] };
...
//   if (this.currentCell && this.currentCell.field && this.currentCell.field.isDraft) {
//     this.currentCell.field = null;
//     this.tableRows = [...this.tableRows];
//   }
...
handleCancel() {
  // Reset the popup fields (keep existing behavior)
  this.fieldLabel = "";
  this.isMandatory = false;
  this.comments = "";
  this.showPopup = false; // Hide the field settings popup

  // 🔹 keep: if a draft placeholder was shown, remove it
  if (this.currentCell && this.currentCell.field && this.currentCell.field.isDraft) {
    this.currentCell.field = null;
    this.tableRows = [...this.tableRows];
  }

  // Cancel should not persist the draft change (keep existing behavior)
  this.hasPendingFieldChange = false;

  // ✅ NEW: reset popup context / selection (prevents stale references)
  this.selectedField = null;
  this.currentFieldType = null;

  // ✅ NEW: reset drag state so the next new drag-drop works
  this.draggingFieldType = null;
  this.draggingFieldId = null;

  // ✅ NEW: optionally clear currentCell reference after rollback
  // (safe because popup is closed and we already removed draft if needed)
  ...
  if (this.draggingFieldId) {
    ...
    const columns = this.tableColumnWidths.map(c => ({
      ...
      // 🔥 IMPORTANT — immediately sync preview
...
//   if (this.currentCell) {
//     const fieldData = {
//       // keep placeholder id if present, otherwise generate a new one
...
//       isDraft: false // 🔹 NEW: finalize (was a draft placeholder)
//     };

//     // (unchanged) Save specific options based on the field type
//     if (this.isTextField) {
...
//     // (unchanged) Set field data to the cell
//     this.currentCell.field = fieldData;

//     // (unchanged) Reset popup fields and hide the popup
...
//   });
//   // ✅ Special case: Header saves its own text + styles and skips Field Label validation
//   if (this.isHeader && this.currentCell) {
//     const id = this.currentCell.field
//       ? this.currentCell.field.id
//       : `field-${Math.random().toString(36).substr(2, 9)}`;

//     // Use the selected header if available; otherwise defaults
...
//     const headerData = {
//       id,
//       // keep "label" for grid/tooling; map it to the header text
//       label: '\u200B',
//       dataType: 'Header',
//       type: 'header',
//       isHeader: true,
...
//       // keep your booleans consistent with other field types
//       ...(this.getTypeFlags
//         ? this.getTypeFlags('Header')
//         : { isHeader: true })
//     };
...
//     // trigger rerender for the grid
//     this.tableRows = [...this.tableRows];

//     // reset + close popup (same flow as below)
//     this.resetPopupFields();
//     this.showPopup = false;
//     console.log('🟢 AFTER (header path) | showPopup =', this.showPopup);
...
//       // eslint-disable-next-line no-console
//       console.log('🔁 microtask check (header) | showPopup =', this.showPopup);
//     });
//     return; // <-- important: skip the generic Field Label path
//   }
...
//     this.resetPopupFields();
//     this.showPopup = false;
//      console.log('🟢 AFTER (generic path) | showPopup =', this.showPopup);
//   }
...
//   this.isFormSaved = false;
//    // Log once more on microtask and next macrotask (in case some other code reopens it)
//   Promise.resolve().then(() => {
//     // eslint-disable-next-line no-console
//     console.log('🔁 microtask check (generic) | showPopup =', this.showPopup);
//   });
//   setTimeout(() => {
//     // eslint-disable-next-line no-console
//     console.log('⏱️ next tick check | showPopup =', this.showPopup);
//   }, 0);
// }

// Add once at the top of the class:
_closingPopup = false;
  ...
  fieldData.value = Array.isArray(this.currentCell?.field?.value)
    ...
    : [];
      ...
      const rowNames = Array.from(
        ...
        (_, i) => ({
          key: `row-${i}`,
          value: prevCfg.rowNames?.[i]?.value || ''
        })
      );
    ...
    try {
      ...
      // Even if write fails, try to close
    }
...
//   // ❌ Block publish if popup is open
//   if (this.showPopup) {

//     console.log(
//       '⚠️ Please save the field first, then publish.'
//     );

//     // Optional toast message (recommended UX)
//     this.dispatchEvent(
//       new ShowToastEvent({
//         title: 'Save Required',
...
async handleConfirmPublish() {
  ...
  // 🔥 CHECK DUPLICATE IN DYNAMIC FORM
  const exists = await doesPublishedFormExist({
      formName: this.formTitle,
      orgId: this.orgid, audience: this.audience
  });
...
//     console.log(
//       '⚠️ Please save the field first, then continue.'
//     );
...
async handleConfirmSaveLayout() {
  ...
  // 🔥 CHECK DUPLICATE IN CUSTOM FORM
  const exists = await doesLayoutTitleExist({
      title: this.formTitle,
      orgId: this.orgid, audience: this.audience
  });
...
openDefaultLayoutByQuery(query) {
 ...
 @track activeLandingTab = 'describe';     // 'describe' | 'new' | 'pdf'
...
handleHeaderChange(event) {
  ...
  try { console.groupCollapsed('🧩 handleHeaderChange'); } catch(e) {}
  ...
  console.log('📨 event:', {
    type: event?.type,
    keyFromDataset: key,
    valueFromDetailOrTarget: val,
    detail: event?.detail,
    targetValue: event?.target?.value
  });

  if (!key) {
    console.warn('⚠️ Missing data-field on the control. Aborting.');
    try { console.groupEnd(); } catch(e) {}
    return;
  }

  if (!this.isHeader) {
    console.warn('⚠️ Not in header mode (isHeader=false). Aborting.');
    try { console.groupEnd(); } catch(e) {}
    return;
  }

  if (!this.selectedField) {
    console.warn('⚠️ selectedField is undefined. Aborting.');
    try { console.groupEnd(); } catch(e) {}
    return;
  }

  const before = { ...this.selectedField };
  console.log('🧱 Selected field (before):', before);
  ...
  console.log('✅ Selected field (after):', after);

  // --- persist into current cell if it’s the same field ---
  const currentId = this.currentCell?.field?.id;
  console.log('📌 currentCell.field.id:', currentId, '  |  selectedField.id:', after.id);

  if (currentId === after.id) {
    this.currentCell.field = { ...after };
    console.log('📝 currentCell.field updated.');
  } else {
    console.warn('↪️ Skipped writing to currentCell.field (IDs did not match).');
  }

  // --- trigger rerender ---
  this.tableRows = [...this.tableRows];
  console.log('🔄 tableRows reassigned -> rerender scheduled.');

  // Helpful diff log (what changed)
  try {
    const changed = Object.keys(after).reduce((acc, k) => {
      if (before[k] !== after[k]) acc[k] = { from: before[k], to: after[k] };
      return acc;
    }, {});
...
handleHeaderToggle(e) {
  this.hasTableHeader = e.target.checked;
  this.buildHeaderInputs();
  this.updateLiveTablePreview(); 
}

handleRowLabelToggle(e) {
  this.hasRowLabels = e.target.checked;
  this.buildRowInputs();
  this.updateLiveTablePreview(); 
}
...
updateLiveTablePreview() {

  if (!this.isTableBlock || !this.currentCell) return;

  const cell =
    this.tableRows[this.currentCell.row]
      .cells[this.currentCell.col];

  if (!cell?.field?.isTableBlock) return;
  ...
  widths.map((c, i) => {
    ...
    return {
      ...
      // 🔵 row-label header is separate
      header:
        prevCfg.columns?.[i]?.header || '',
  ...
  cell.field.label = this.fieldLabel;
...
handleInlineTableEdit(e) {
  ...
  // ──────────────────────────────
  // 🔵 COLUMN HEADERS (including row-label column)
  // ──────────────────────────────
  if (type === 'header' && colIndex !== null) {
    cfg.columns[colIndex].header = e.target.value;
  }
