...
import getAllTemplates from "@salesforce/apex/tSignDocsController.getAllTemplates";
   ...
   connectedCallback() {  
        ...
        switch (activeTab) {
            case 'hrMyTemplates':
                ...
                console.log('this.templatesFlag in hr templates connectedCallback:' ,this.templatesFlag);
                break;
            case 'hrCreatedTemplates':
                this.createtemplatesFlag = true;
                console.log('this.createtemplatesFlag in hr templates connectedCallback:' ,this.createtemplatesFlag);
                break;
           
            default:
                this.templatesFlag = true;
                console.log('this.templatesFlag in hr templates default  connectedCallback:' ,this.templatesFlag);
...
stopCardActionClick = (e) => {
 ...
 openZoom = (event) => {
  ...
  @track selectedPlaceholders;
    ...
    this.selectedUrl = event.currentTarget.dataset.url;
    this.selectedPlaceholders = event.currentTarget.dataset.placeholders;

    this.isCreateOptionsModalOpen = true; // open same child
...
handleViewTemplateFromChild(){
  ...
  this.selectedPlaceholders=null;
...
if (lowerUrl.endsWith(".docx")) {
  ...
  wiredTemplatesMain(result) {
    ...
    if (data) {
     console.log('this.createdTemplateList',JSON.stringify(data));
   this.templateList = data.map((t) => {
    let awsjson = [];

    if (t.Aws_Json__c) {
        try {
            // If it's a string like "[{\"key\":\"value\"}]" → parse it
            awsjson = typeof t.Aws_Json__c === 'string' 
                ? JSON.parse(t.Aws_Json__c)
                : t.Aws_Json__c;
        } catch (error) {
            console.error('Error parsing Aws_Json__c:', error);
            awsjson = [];
        }
    }
  ...
  awsKey: Array.isArray(awsjson)
    ...
    if (data) {
        this.createdTemplateList = data;
        console.log('this.createdTemplateList',JSON.stringify(this.createdTemplateList));
  ...
  updateCreatedTemplatePagination() {
    ...
    const filtered = this.createdTemplateList.filter((tpl) =>
      tpl.File_Name__c?.toLowerCase().includes(search)
    );
