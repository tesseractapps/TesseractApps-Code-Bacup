...
combineSubsequentShifts(shifts) {
    ...
    const normalizedShifts = shifts
        ...
        .filter(shift => (shift.hours > 0 )||( shift.rawName === 'Cancelled') ||( shift.rawName === 'Broken_Allowance')); // remove invalid entries
...
Object.keys(shiftsByName).forEach(shiftName => {
    ...
    rows.forEach((row, index) => {
       ...
       // const total = row.name=='Broken_Allowance' ||row.name=='Cancelled'?row.rate : rate * hours;
        if (row.name === 'Broken_Allowance' || row.name === 'Cancelled') {
...
//Leave Summary
if (
    this.currentRecordPdf[0].staffData.Type_of_Employe__c &&
    (
        this.currentRecordPdf[0].staffData.Type_of_Employe__c === 'Full Time Permanent' ||
        this.currentRecordPdf[0].staffData.Type_of_Employe__c === 'Part Time Permanent'
    )
) {
    startY = checkPageSpace(doc, startY + 6, 6);
