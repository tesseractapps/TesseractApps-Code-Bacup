...
removeOutsideListenerIfNeeded() {
 ...
 handleFacilityToggleActive(event) {
    ...
    /* buildHierarchicalFromFacility(result) {
        ...
        identitySections.push({
            ...
            children: groupedIdentity.Primary.length
                ? groupedIdentity.Primary.map(doc => {
                    ...
                    console.log('PRIMARY DOC:', JSON.stringify(doc));

                    // ✅ UNIQUE KEY (fix)
                    const uniqueKey = `${doc.name}__${doc.facilityId || doc.organisationId || (doc.isDefault ? 'DEFAULT' : 'ORG')}`;
                    ...
                    if (doc.isDefault) {
                        ...
                        displayName = doc.facilityName
                            ? `${doc.name} - ${doc.facilityName}`
                            : `${doc.name} - Facility`;

                    } else {
                        displayName = `${doc.name} - Organisation`;
                    }

                    return {
                        id: `id-${uniqueKey.toLowerCase().replace(/[^a-z0-9]/g, '-')}`, // ✅ FIXED
                        label: withMandatoryMark(doc, displayName) + 
                            (doc.points ? ` (${doc.points} points)` : ''),
                        value: displayName, // ✅ FIXED (CRITICAL)
                        key: uniqueKey, // ✅ FIXED (CRITICAL)
                        originalName: doc.name, // optional
                        points: doc.points || 0,
                        children: []
                    };
                })
        ...
        identitySections.push({
            ...
            children: groupedIdentity.Secondary.length
                ? groupedIdentity.Secondary.map(doc => {
                    ...
                    console.log('SECONDARY DOC:', JSON.stringify(doc));
                    ...
                    return {
                        id: `id-${uniqueKey.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
                        label: withMandatoryMark(doc, displayName) + 
                            (doc.points ? ` (${doc.points} points)` : ''),
                        value: displayName, // ✅ FIXED (CRITICAL)
                        key: uniqueKey,
        ...
        identitySections.push({
            ...
            children: groupedIdentity.Other.length
                ? groupedIdentity.Other.map(doc => {
                    ...
                    console.log('OTHER DOC:', JSON.stringify(doc));
        ...
        console.log('==== DOCUMENT DOCS RAW ====');
        console.log(JSON.stringify(documentDocs, null, 2));
        ...
        hierarchy[1].children = documentDocs.length
            ? documentDocs.map(doc => {
                ...
                // ✅ Create UNIQUE key
                const uniqueKey = `${doc.name}__${doc.facilityId || doc.organisationId || 'ORG'}`;

                // ✅ Store maps using unique key
                this.documentPointsMap[uniqueKey] = Number(doc.points) || 0;
                this.documentMetaMap[uniqueKey] = doc;

                // ✅ Build display name (NO CHANGE)
                let displayName;
                ...
                console.log('FINAL LABEL:', displayName);
                console.log('UNIQUE KEY:', uniqueKey);

                return {
                    id: `doc-${uniqueKey.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,

                    // ✅ UI DISPLAY
                    label: withMandatoryMark(doc, displayName) + 
                        (doc.points ? ` (${doc.points} points)` : ''),

                    // ✅ ORIGINAL VALUE (IMPORTANT FIX)
                    value: doc.name,
        ...
        hierarchy[2].children = trainingDocs.length
            ? trainingDocs.map(doc => {
                ...
                console.log('TRAINING DOC:', JSON.stringify(doc));

                // ✅ UNIQUE KEY (consistent with identity/docs)
                const uniqueKey = `${doc.name}__${doc.facilityId || doc.organisationId || (doc.isDefault ? 'DEFAULT' : 'ORG')}`;

                // ✅ Fix map overwrite issue
                this.documentPointsMap[uniqueKey] = Number(doc.points) || 0;
                this.documentMetaMap[uniqueKey] = doc;

                // ✅ SAME LOGIC AS IDENTITY (important consistency)
                let displayName;
                ...
                console.log('TRAINING UNIQUE KEY:', uniqueKey);
                console.log('TRAINING LABEL:', displayName);

                return {
                    id: `training-${uniqueKey.toLowerCase().replace(/[^a-z0-9]/g, '-')}`, // ✅ FIXED
                    label: withMandatoryMark(doc, displayName) + 
                        (doc.points ? ` (${doc.points} points)` : ''),
                    value: displayName, // ✅ FIXED (CRITICAL)
