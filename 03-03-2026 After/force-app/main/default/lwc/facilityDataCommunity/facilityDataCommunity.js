  ...
  ConfirmClose (){
   ...
   handledetails(event){
    ...
    handleSelectAll(event) {
        ...
        // 2️⃣ Update trackedRecords also
        if (checked) {

            // merge all current page records into trackedRecords
            const map = new Map(
                this.trackedRecords.map(r => [r.Id, r])
            );

            updatedRecords.forEach(rec => {
                map.set(rec.Id, rec);   // add / replace
            });
            ...
            // remove only records that are currently visible
        ...
        console.log('trackedRecords => ', JSON.stringify(this.trackedRecords));
