...
export default class SupportCoordinatorReports extends LightningElement {
    ...
    async initializePdfLibraries() {
        ...
        if (!this.jsPdfLoadingPromise) {
            this.jsPdfLoadingPromise = Promise.all([
                // If static resource is a single JS file:
                loadScript(this, jsPDFLib),
                loadScript(this, autoTableLib)

                // If they are ZIPs, use exact internal paths instead:
    ...
    async handleExportPdfReconciliation() {
        try {
            ...
            console.error('Reconciliation PDF Export Error:', error);
        }
    }
    ...
    async handleExportPdfCaseNotes() {
        try {
            ...
            doc.save(
                `Case_Notes_Report_${this.caseNotesParticipantName || 'Participant'}.pdf`
            );
        } catch (error) {
