...
async convertWordToPdf(file, fileName, batchIndex) {
  ...
  try {
    ...
    if (sizeMB > 10) {
      ...
      pdfBlob = await this.compressPdf(this.pageImages, sizeMB);
  ...
  wiredTemplates(result) {
    ...
    if (data) {
     ...
     /*  this.templateList = data.map((t) => ({
       ...
       awsjson: t.Aws_Json__c ? t.Aws_Json__c: []
      })); */
   this.templateList = data.map((t) => {
    let awsjson = [];
    ...
    return {
        ...
        AWS_Document__c: t.AWS_Document__c,
        awsjson,
        // Optional helper field to directly access the key
        awsKey: awsjson && awsjson.length > 0 ? awsjson[0].key : null
    };
});
