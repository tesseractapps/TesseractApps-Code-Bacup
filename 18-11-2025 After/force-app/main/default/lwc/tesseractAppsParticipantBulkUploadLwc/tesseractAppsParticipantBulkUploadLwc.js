...
export default class TesseractAppsParticipantBulkUploadLwc extends LightningElement {
   ...
   // @track isClientOrIndividual = false;
      allFields = [
        ...
        { key: 'PrimaryContactRelationship', label: 'Primary Contact Relationship' },
    ...
    requiredClientHeaders = [
        ...
        'PrimaryContactRelationship', 'PrimaryContactNumber',
   ...
   get isCompany() {
        ...
        if(this.facilityService === 'NDIS'){
           // this.templateType === 'Client'
             // this.nonNdisFlag= false;
              const fileUrl = '/resource/Participant_Template';  // Update the file URL if necessary
              window.open(fileUrl, '_blank');
        }  else {
    ...
    reader.onload = () => {
        const fileContent = reader.result;
        console.log('File content loaded:', fileContent.substring(0, 200)); // log first 200 chars
        const parsedData = this.parseCSV(fileContent);
    ...
    paginateData() {
        if (!this.tableData || this.tableData.length === 0) {
            ...
            this.totalRecords = 0;
            this.totalPages = 0;
            this.updatePaginationButtons(); // ensure buttons reset
            return;
        }
        ...
        this.paginatedData = this.tableData.slice(start, end).map(user => ({
            ...user,
        ...
        // ✅ refresh pagination buttons every time data is recalculated
        this.updatePaginationButtons();
    }
    ...
    handleFinalSubmit() {
        ...
        switch (this.templateType) {
            case 'Client':
                templateHeaders = this.requiredClientHeaders;
                break;
            case 'Company':
                templateHeaders = this.requiredCompanyHeaders;
                break;
            case 'Individual':
                templateHeaders = this.requiredIndividualHeaders;
                break;
        }
        ...
        if (invalidRecords.length) {
            ...
            this.showToast('Error', 'Some records have missing required fields. Check console.', 'error');
            return;
        }
        console.log('unprocessedRecords:', JSON.stringify(unprocessedRecords));
        this.uploadDataToApex(unprocessedRecords);
    }
    
    uploadDataToApex(unprocessedRecords) {
        createClientsOnBulk({ templateType: this.templateType, records: unprocessedRecords,facilityId:this.facilityId })
            .then(result => {
                const { successRecords, failedRecords } = result;
                console.log('result:', JSON.stringify(result));

                this.tableData = this.tableData.map(row => {
                    const success = successRecords.find(r => this.matchRecord(r, row));
                    const fail = failedRecords.find(r => this.matchRecord(r, row));

                   // if (success) return { ...row, ProcessedStatus: 'Success ✅' };
                    //if (fail) return { ...row, ProcessedStatus: `Error ❌: ${fail.ErrorMessage}` };
                    if (success){
						// row.isSelected = false;
                       
					    return { ...row, ProcessedStatus: 'Success ✅' };
				    } else if (fail){
						return { ...row, ProcessedStatus: `Error ❌: ${fail.ErrorMessage}` };
					}
                    return row;
                });
            ...
            .catch(error => {
                ...
                this.tableData = this.tableData.map(row => ({
                    ...row,
                    ProcessedStatus: row.isSelected ? 'Error ❌: Unexpected Error' : row.ProcessedStatus
                }));
                this.isProcessed = true;
                this.forceTableUpdate();
            });
    ...
    matchRecord(recordA, recordB) {
        if (this.templateType === 'Company') {
            return recordA.CompanyName?.trim().toLowerCase() === recordB.CompanyName?.trim().toLowerCase();
        }
        return recordA.EmailID?.trim().toLowerCase() === recordB.EmailID?.trim().toLowerCase();
    }
