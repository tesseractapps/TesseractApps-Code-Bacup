...
import getChildRows from "@salesforce/apex/SubmissionsController.getChildRows";
import getUnreadReimbursementIds from '@salesforce/apex/NotificationServices.getUnreadReimbursementIds';//manendra added for notification
import getLoggedInStaffId from '@salesforce/apex/NotificationServices.getLoggedInStaffId';//manendra added for notification
import markNotificationRead from '@salesforce/apex/NotificationServices.markNotificationRead';//manendra added for notification
export default class SubmissionJune extends LightningElement {
  ...
  @track pageSizeOptions = ['Auto', 10, 20, 50];
  ...
  @track pageSizeOptionssub = ['Auto', 10, 20, 50];
  @track pagedSigninList = [];
  @track pageSizeOptionssub1 = ['Auto', 10, 20, 50];
  @track pageSizeSub1 = 5;
  @track pageNumberSub1 = 1;
  @track totalPagesSub1 = 1;
  @track totalRecordsSub1 = 0;
  @track signinList1 = [];
  @track selectedPageSize = 'Auto';
  @track selectedPageSizeSub = 'Auto';
  @track selectedPageSizeSub1 = 'Auto';
...
get isTransportExpense() {
    return this.transportType === 'Other Expenses';
}
get  isActivityVehicleType(){
    return this.transportMode === 'Vehicle';
}

get  isActivityPublicVehicle(){
...
get isImage() {
    return this.currentUrl &&
        (this.currentUrl.endsWith('.png') ||
         this.currentUrl.endsWith('.jpg') ||
         this.currentUrl.endsWith('.jpeg') ||
         this.currentUrl.endsWith('.gif') ||
         this.currentUrl.endsWith('.webp'));
}

get isPdf() {
    return this.currentUrl && this.currentUrl.endsWith('.pdf');
}
...
expenseTypeOptions = [
  ...
  currentUserInfo({ error, data }) {
    if (data) {
      ...
      if (!this.isAdmin && this.activeTab === 'approvals') {
        this.activeTab = 'submissions';
      }      

      // Default activeTab only if not already set by openByUID
      if (!this.activeTab) {
        const savedTab = localStorage.getItem('activeReimbursementTab');
      if (!this.isAdmin) {
      ...
      // Dispatch subrouteupdate to make it canonical, but only if not suppressed
      if (!this._suppressEmit) {
        this.dispatchEvent(new CustomEvent('subrouteupdate', {
            detail: { subView: this.activeTab, replace: true },
            bubbles: true,
            composed: true
        }));
      }
      ...
      if (data) {
          ...
          getCurrentLoggedUserInfo()
              .then((userInfo) => {
                ...
                //manendra added notifactions badges
...
.then((staffId) => {
    ...
    return getUnreadReimbursementIds({
        staffId: staffId
    });
})
...
.then((unreadIds) => {
    ...
    this.reimbursementList =
        this.reimbursementList.map(item => {
        console.log(
            'COMPARE >>>',
            item.Id,
...
this.paginationHelper();
    console.log(
        'Updated reimbursementList >>>',
        JSON.stringify(this.reimbursementList)
    );
})
...
.catch((error) => {
    ...
    console.error(
        'Error fetching unread reimbursement ids',
        error
    );
});

//mnendra added notifactions badges end
  ...
  initResizeObserver() {
    ...
    this._resizeObserver = new ResizeObserver((entries) => {
      window.requestAnimationFrame(() => {
        ...
        for (let entry of entries) {
          ...
          if (target.classList.contains('adminTable')) {
            const headerEl = target.querySelector('.header-wrapper');
            const headerHeight = headerEl ? headerEl.getBoundingClientRect().height : 45;
            const rowEl = target.querySelector('tbody tr');
            const rowHeight = rowEl ? rowEl.getBoundingClientRect().height : 40;
            
            const availableHeight = containerHeight - headerHeight - 15;
            const calcSize = Math.max(1, Math.floor(availableHeight / rowHeight));
            
            if (this.selectedPageSize === 'Auto' && this.pageSize !== calcSize) {
              this.pageSize = calcSize;
              this.pageNumber = 1;
              needsUpdate = true;
            }
          } else if (target.classList.contains('staffSubmissionTableApproval')) {
            const headerEl = target.querySelector('.header-wrapper');
            const headerHeight = headerEl ? headerEl.getBoundingClientRect().height : 45;
            const rowEl = target.querySelector('tbody tr');
            const rowHeight = rowEl ? rowEl.getBoundingClientRect().height : 40;
            ...
            if (this.selectedPageSizeSub === 'Auto' && this.pageSizeSub !== calcSize) {
              this.pageSizeSub = calcSize;
              this.pageNumberSub = 1;
              needsUpdate = true;
            }
          } else if (target.classList.contains('signinTable')) {
            const headerEl = target.querySelector('.header-wrapper');
            const headerHeight = headerEl ? headerEl.getBoundingClientRect().height : 45;
            const rowEl = target.querySelector('tbody tr');
            const rowHeight = rowEl ? rowEl.getBoundingClientRect().height : 40;
            ...
            if (this.selectedPageSizeSub1 === 'Auto' && this.pageSizeSub1 !== calcSize) {
              this.pageSizeSub1 = calcSize;
        ...
        if (needsUpdate) {
          if (this.selectedPageSize === 'Auto') {
            this.paginationHelper();
          }
          if (this.selectedPageSizeSub === 'Auto') {
            this.paginationHelperSub();
          }
          if (this.selectedPageSizeSub1 === 'Auto') {
    ...
    const adminTable = this.template.querySelector('.adminTable');
    if (adminTable) {
      this._resizeObserver.observe(adminTable);
    }

    const subTable = this.template.querySelector('.staffSubmissionTableApproval');
    if (subTable) {
      this._resizeObserver.observe(subTable);
    }

    const signinTable = this.template.querySelector('.signinTable');
    if (signinTable) {
      this._resizeObserver.observe(signinTable);
    }
  }
 ...
 processReimbursementData(data) {
    ...
    const dataToProcess =
      ...
      this.userTypevalueforcurrentlogin == "Roster Manager"
        ? data.filter(
            (user) =>
             user.ShiftwithStaff__r.Shift_Facility__c ?user.ShiftwithStaff__r.Shift_Facility__c : user.ShiftwithStaff__r.Add_Shift__r.Facility__c === this.facilityValuefromcatch
             // user.ShiftwithStaff__r?.Add_Shift__r?.Facility__c ===   this.facilityValuefromcatch
             //user.Shift_Facility__c === this.facilityValuefromcatch
          )
        : data;
...
@track activityTransportMaxKms;
  ...
  handleTabChange(event) {
    ...
    if (!this._suppressEmit) {
      this.dispatchEvent(new CustomEvent('subrouteupdate', {
          detail: { subView: this.activeTab, replace: false },
          bubbles: true,
          composed: true
      }));
    }    
    ...
    .then((data) => {
      ...
      this.submissionList = data.map((item, index) => {
        ...
        return {
          ...
          statusStyle,
          vehicleStyle,
          isRejected: item.Approval_Status__c === 'Rejected',
          mileage:item.Mileage_Others__c || '-',
          staffName: item.ShiftwithStaff__r?.Staff__r?.NameToDisplay__c || '',
          Typeofvechile:item.Type_of_Vehicle__c || '-'
        };
      });
  ...
  handleSignInRowSelection(event) {
    ...
    let logoutTime = event.currentTarget.dataset.logout;
     this.disabletransportType=false;
    this.resetFormData();
  ...
  @api openByUID(uid, tab, isEdit) {
    console.log('📌 [staffSubmissionsLwc] openByUID called with:', uid);
    this._suppressEmit = true;
    try {
        const lowerUid = String(uid || '').toLowerCase();
        if (lowerUid === 'approvals' || lowerUid === 'submissions') {
            // Block non-admin from entering approvals
            if (lowerUid === 'approvals' && !this.isAdmin && this.currentUserRole) {
                this.activeTab = 'submissions';
            } else {
                this.activeTab = lowerUid;
            }
        }
    } finally {
  ...
  handleCommentsChange(event) {
    this.approverComments = event.target.value;
  }
...
if (name === "odoStart" || name === "odoEnd") {
  ...
  onSubmitForApproval() {
    if (!this.transportType) {
        this.showError('Please select Reimbursement Type');
        return;
    }
    if (this.transportType !== 'Other Expenses') {
    ...
    createReimbursementRecord() {
      console.log('this.odoStart',this.odoStart);
       ...
       console.log('this.isUpdate',this.isUpdate);
        console.log('this.recordIdforUpdate',this.recordIdforUpdate);
        console.log('this.reimbursementId',this.reimbursementId);
  ...
  await this.markReimbursementAsRead(recordId);//manendra added
...
console.log('this.reimbursementList', JSON.stringify(record));
 ...
 if (isApproved && serviceIds.length > 0) {

        console.log('✅ Approved → overriding rows');
        this.isViewMode=true;

        const childRows = await getChildRows({ serviceIds: serviceIds, reimbursementId: this.reimbursementId });
        console.log('childRows',JSON.stringify(childRows));

        this.reimbursementRows = childRows.map(item => ({
            id: item.Id,
              participantId: item.Services_and_Support_Plan__r?.Client__c,
             selectedshiftServiceItem: item.Services_and_Support_Plan__c,
    services: this.ServicesInShift.map(s => ({
        label: s.Support_Item_Name__c,
        value: s.Id
    })),

    // ✅ Transport (IMPORTANT)
    selectedTransportItem: item.Reimbrusment_line_item__c,
    transports: [{
        label: item.Reimbrusment_Support_item__c,
        value: item.Reimbrusment_line_item__c
    }],

            transportserviceName: item.Reimbrusment_Support_item__c,
            transportItemNumber: item.Reimbrusment_line_item__c,
            transportQty: item.Reimbrusment_Quantity__c,
            transportRate: item.Reimbrusment_Rate__c,
            transportTotal: item.Reimbrusement_Amount__c
        }));
    }
        this.selectedRecord = this.normalizeRecord(record);
       // this.reimbursementId = recordId;
        this.recordType = record.Transport_Type__c;
        this.approverComments = record.Approver_Comments__c || '';
 
        console.log('selectedRecord', JSON.stringify(this.selectedRecord));

        // JSON parse only when needed
        if (this.recordType === 'Other Expenses') {
            try {
  ...
  get isTransportType() {
    return this.selectedRecord?.Transport_Type__c === 'Other Expenses';
}
get isProviderType() {
...
get isActivityPublic() {
    return this.selectedRecord?.transportMethod === 'Public';
}
...
/* get isViewMode() {
    return this.selectedRecord?.status === 'Approved';
} */
...
handleInputChange(event) {
  ...
  normalizeRecord(record) {
    return {
        ...
        Cost_per_KM__c: record?.Fuel_Electric_Price__c || '0',
...
formatDateToDDMMYYYY(dateStr) {
 ...
 async  handleReject(event) {
    ...
    console.log('recordName',recordName);
       await this.markReimbursementAsRead(recordId);//manendre added
    this.reimbursementNameforApprove =  recordName;
    this.reimbursementAmountforApprove = new Intl.NumberFormat('en-US', { 
        minimumFractionDigits: 2, 
        maximumFractionDigits: 2 
...
nextPage() {
  ...
  recalculateAutoPageSize() {
    const adminTable = this.template.querySelector('.adminTable');
    if (adminTable) {
      const containerHeight = adminTable.getBoundingClientRect().height;
      if (containerHeight > 0) {
        const headerEl = adminTable.querySelector('.header-wrapper');
        const headerHeight = headerEl ? headerEl.getBoundingClientRect().height : 45;
        const rowEl = adminTable.querySelector('tbody tr');
        const rowHeight = rowEl ? rowEl.getBoundingClientRect().height : 40;
        const availableHeight = containerHeight - headerHeight - 15;
        const calcSize = Math.max(1, Math.floor(availableHeight / rowHeight));
        if (this.pageSize !== calcSize) {
            this.pageSize = calcSize;
            this.paginationHelper();
        }
      }
  ...
  recalculateAutoPageSizeSub() {
    const subTable = this.template.querySelector('.staffSubmissionTableApproval');
    if (subTable) {
      const containerHeight = subTable.getBoundingClientRect().height;
      if (containerHeight > 0) {
        const headerEl = subTable.querySelector('.header-wrapper');
        const headerHeight = headerEl ? headerEl.getBoundingClientRect().height : 45;
        const rowEl = subTable.querySelector('tbody tr');
        const rowHeight = rowEl ? rowEl.getBoundingClientRect().height : 40;
        const availableHeight = containerHeight - headerHeight - 15;
        const calcSize = Math.max(1, Math.floor(availableHeight / rowHeight));
        if (this.pageSizeSub !== calcSize) {
            this.pageSizeSub = calcSize;
            this.paginationHelperSub();
        }
      }
  ...
  recalculateAutoPageSizeSub1() {
    const signinTable = this.template.querySelector('.signinTable');
    if (signinTable) {
      const containerHeight = signinTable.getBoundingClientRect().height;
      if (containerHeight > 0) {
        const headerEl = signinTable.querySelector('.header-wrapper');
        const headerHeight = headerEl ? headerEl.getBoundingClientRect().height : 45;
        const rowEl = signinTable.querySelector('tbody tr');
        const rowHeight = rowEl ? rowEl.getBoundingClientRect().height : 40;
        const availableHeight = containerHeight - headerHeight - 15;
        const calcSize = Math.max(1, Math.floor(availableHeight / rowHeight));
        if (this.pageSizeSub1 !== calcSize) {
            this.pageSizeSub1 = calcSize;
            this.totalPagesSub1 = Math.ceil(this.totalRecordsSub1 / this.pageSizeSub1);
            this.updatePagedData();
        }
...
handleRecordsPerPageSub(event) {
    const val = event.target.value;
    this.selectedPageSizeSub = val;
    if (val !== 'Auto') {
        this.pageSizeSub = parseInt(val, 10);
    } else {
        this.recalculateAutoPageSizeSub();
    }
...
// Update paged data for current page
    updatePagedData() {
       /*  const start = (this.pageNumberSub1 - 1) * this.pageSizeSub1;
        ...
        if (this.selectedPageSizeSub1 === 'Auto') {
            setTimeout(() => {
                if (this.isConnected) {
                    this.recalculateAutoPageSizeSub1();
                }
            }, 50);
        } */
        this.totalRecordsSub1 = this.signinList1.length;
    ...
    if (this.pageNumberSub1 <= 1) {
        this.pageNumberSub1 = 1;
    } else if (this.pageNumberSub1 >= this.totalPagesSub1) {
        this.pageNumberSub1 = this.totalPagesSub1;
    }

    let start = (this.pageNumberSub1 - 1) * this.pageSizeSub1;
    let end = this.pageNumberSub1 * this.pageSizeSub1;
    this.pagedSigninList = this.signinList1.slice(start, end);

    this.bDisableFirstSub1 = (this.pageNumberSub1 === 1);
    this.bDisableLastSub1 = (this.pageNumberSub1 === this.totalPagesSub1);
    ...
    handlePageSizeChange(event) {
        const val = event.target.value;
        this.selectedPageSizeSub1 = val;
        if (val !== 'Auto') {
            this.pageSizeSub1 = parseInt(val, 10);
        } else {
            this.recalculateAutoPageSizeSub1();
        }
        this.pageNumberSub1 = 1;
...
@track currentUploadRowId = '';
    ...
    this.recordIdforUpdate = event.currentTarget.dataset.shiftid;
    this.disabletransportType=true;


    console.log('✏️ Editing reimbursement:', recId);
    console.log('this.recordIdforUpdate', this.recordIdforUpdate);

   // this.isEditMode = true;
    //this.editRecordId = recId;

    // open same form
    this.shiftselected = true;
    this.isUpdate = true;

    this.loadReimbursementForEdit(recId);
}



loadReimbursementForEdit(recId) {

    //this.resetFormData();
  this.reimbursementId = recId;
    const record = this.submissionList.find(r => r.Id === recId);
    if (!record) return;
     this.isEditMode = true;

    console.log('📦 Edit Data:', JSON.stringify(record));

    // ✅ STEP 1: Set base values
    this.transportType = record.Transport_Type__c;
    this.transportMode = record.Transport_Method__c;

    // 🔥 STEP 2: TRIGGER UI LOGIC (VERY IMPORTANT)
    this.handleChange1({
        target: {
            name: 'transportType',
            value: this.transportType
        }
    });
    ...
    // =========================
    this.vehicleValue = record.Type_of_Vehicle__c;
    this.others = record.Mileage_Others__c;
    this.comments = record.Comments__c;
    this.approverComments = record.Approver_Comments__c;
    this.activityVehicleType =  record.Activity_Transport_Vehicle__c;

    this.odoStart = record.Odo_Meter_Start__c;
    this.odoEnd = record.Odo_Meter_End__c;

    this.activityVehicleType = record.Activity_Transport_Vehicle__c;
    this.amount = record.Amount__c;

    // =========================
    // STEP 4: TIME (Provider Travel)
    // =========================
 /* this.uistarttime = this.formatMillisecondsToTime(record.Travel_Start_Time__c);

if (this.uistarttime) {
    const [time, period] = this.uistarttime.split(" ");   // "12:01 AM"
    const [hour, minute] = time.split(":");

    this.startTimeSelectedHour = hour;     // "12"
    this.startTimeSelectedMinute = minute; // "01"
    this.startTimeAMPM = period;           // "AM"
}

// END TIME
this.uiendtime = this.formatMillisecondsToTime(record.Travel_End_Time__c);

if (this.uiendtime) {
    const [time, period] = this.uiendtime.split(" ");
    const [hour, minute] = time.split(":");
    ...
    this.uistarttime = this.formatMillisecondsToTime(record.Travel_Start_Time__c);
    console.log(' this.uistarttime', this.uistarttime);

if (this.uistarttime) {
    const [time, period] = this.uistarttime.split(" ");
    const [hour, minute] = time.split(":");
    ...
    // 🔥 IMPORTANT
    this.startTime = this.convertTo24Hour(hour, minute, period);
}

// END
this.uiendtime = this.formatMillisecondsToTime(record.Travel_End_Time__c);
 console.log(' this.uiendtime', this.uiendtime);
...
if (this.uiendtime) {
    ...
    // 🔥 IMPORTANT
    this.endTime = this.convertTo24Hour(hour, minute, period);
}

    // =========================
    // STEP 5: FILE
    // =========================
   /*  if (record.Amazon_file_URL__c) {
        this.fileName = 'Existing File';
    } */
    ...
    if (record.Multiple_Expenses__c) {

        try {
            const parsed = JSON.parse(record.Multiple_Expenses__c);

            console.log('🔥 Parsed Expenses:', parsed);

            this.expenseRows = parsed.map(row => ({
                id: row.id,
        ...
        console.warn('⚠️ No Multiple_Expenses__c found');
    }
}
...
formatMillisecondsToTime(ms) {
    ...
    return `${hh}:${mm} ${ampm}`;  // ✅ IMPORTANT FORMAT
}
...
//manendra added 
async markReimbursementAsRead(recordId) {
    ...
    try {
        ...
        console.log(
            '✅ Reimbursement notification marked read:',
            recordId
        );
        ...
        console.error(
            '❌ Error marking reimbursement read',
            error
        );
    }
}
  ...
  handleSort(event) {
      ...
      const state = this.tableSortState[tableName];
      ...
      switch (tableName) {
          ...
          case 'reimbursement':
              this.reimbursementList = this.sortData(
                  this.reimbursementList,
                  state.field,
                  state.direction
              );
          ...
          case 'submission':
              this.submissionList = this.sortData(
                  this.submissionList,
                  state.field,
                  state.direction
              );
          ...
          case 'signin':
              this.signinList1 = this.sortData(
                  this.signinList1,
                  state.field,
                  state.direction
              );
      ...
      this.refreshSortedTable(tableName);
  }
...
//manendra end
  ...
  get approvalStatusClass() {
    return this.getStatusClass(this.selectedRecord?.Approval_Status__c);
}
