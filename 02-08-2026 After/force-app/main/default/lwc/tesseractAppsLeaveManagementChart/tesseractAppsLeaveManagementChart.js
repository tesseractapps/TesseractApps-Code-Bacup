...
export default class LeaveStatusChart extends LightningElement {
    ...
    @api refresh1(facilityIdFromParent) {
        console.log('facilityId in Refresh Method LeaveStatusChart>>', facilityIdFromParent);
        this.facilityId = facilityIdFromParent;
        console.log('facilityId in Refresh Method LeaveStatusChart >>', this.facilityId);
        this.fetchTaskData();
    }
    ...
    fetchTaskData() {
        try {
            console.log('Calling Apex with facilityId:', this.facilityId);
            ...
            .then((data) => {
                console.log('Apex Data LeaveStatusChart :', JSON.stringify(data));
                if (data) {
                    this.taskData = data;
                    this.initializeChart();
                }
            ...
            console.error('Unexpected JS error in fetchTaskData:', err);
        }
    }
    ...
    handleStatusClick(label) {
        publish(this.messageContext, DASHBOARD_REDIRECT_CHANNEL, {
            ...
            data: {
                statusKey: label,       // 'Requested' | 'Approved' | 'Rejected'
                statusLabel: label,
                facilityId: this.facilityId
            }
        });
