...
export default class FeedbackLwc extends LightningElement {
    ...
    connectedCallback(){
     ...
     let  Orgid = response.Id;
      ...
      getStaffList({ orgId: Orgid, facilityId: storedFacilityId })
            .then((data) => {
    ...
    handleCreate(){
        this.successmessge = 'Feedback created successfully.';
        ...
        if (data) {
           // console.log('Fetch Feedback >' + JSON.stringify(data));
            ...
            data.forEach(record => {
                ...
                tempRec.FeedbackUID = tempRec.Feedback_UID__c;
                tempRec.staffName = tempRec.Staff__r.Display_Nickname__c;
                tempRec.staffId = tempRec.Staff__r.Id;
    ...
    handleSubmit(event){  
        event.preventDefault(); 
         const mandatory = this.template.querySelector('[data-mandatory="mandatory"]');
          if (!this.selectedStaff) {
        mandatory.reportValidity(); // shows “Complete this field”
        return;
    }
    if (!this.rating) {
    ...
    handleEdit(event) {      
         this.successmessge = 'Feedback updated successfully.';        
        this.recordId = event.currentTarget.dataset.id;
        this.selectedStaff = event.currentTarget.dataset.staffid;
    ...
    _syncRoute(replace = false) {
        let subView = '';
        if (this.isOpenModal) {
            subView = this.recordId ? `${this._getFeedbackUid(this.recordId)}/edit` : 'add-feedback';
        }
        
        console.log('[Routing] feedbackLwc dispatching subrouteupdate:', subView);
    ...
    processSubroute(subroute) {
        ...
        if (subroute === 'add-feedback') {
            ...
            if (uid.startsWith('fdb-') || /^[a-z0-9]{15}$|^[a-z0-9]{18}$/i.test(uid)) {
                if (this.records && this.records.length > 0) {
                    const record = this.records.find(r => r.FeedbackUID === uid || r.Id === uid);
                    if (record) {
                        this.recordId = record.Id;
                        this.selectedStaff = record.staffId;
