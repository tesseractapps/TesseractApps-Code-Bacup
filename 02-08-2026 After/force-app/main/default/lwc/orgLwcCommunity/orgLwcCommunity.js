...
import syncXero from '@salesforce/apex/XeroIntegrationController.syncXero';
import createOZEDIClient from '@salesforce/apex/OZEDIController.createClient';
...
export default class OrgLwcCommunity extends NavigationMixin(LightningElement) {
    ...
    userId = Id;
    userProfileName;
    // ===== HASH ROUTER bridge: set whenever a record opens (URL is authoritative) =====
    currentUid = '';
    // Stable public slug straight from the Organisation_UID__c formula field.
    orgUid = '';
    // True while the shell is driving us (restore) — we must NOT emit during that.
    _suppressEmit = false;    
    ...
    @track creatingOZEDIClient = false;
    @track fieldErrorMap = {};
...
@track sectionIcons = {
  Organisation: { ...DEFAULT_ICON },
  Addressdetails: { ...DEFAULT_ICON },
  Bank: { ...DEFAULT_ICON },
  Roster: { ...DEFAULT_ICON },
  payroll: { ...DEFAULT_ICON },
  User: { ...DEFAULT_ICON },
  Activity: { ...DEFAULT_ICON },
  AdminSettings: { ...DEFAULT_ICON },
  prefferedName: { ...DEFAULT_ICON },
  leave: { ...DEFAULT_ICON },
  Reimbrusement: { ...DEFAULT_ICON },

  Organisation1: { ...DEFAULT_ICON },
  Addressdetails1: { ...DEFAULT_ICON },
  Bank1: { ...DEFAULT_ICON },
  Roster1: { ...DEFAULT_ICON },
  payroll1: { ...DEFAULT_ICON },
  User1: { ...DEFAULT_ICON },
  Activity1: { ...DEFAULT_ICON },
  AdminSettings1: { ...DEFAULT_ICON },
  OZEDISettings1: { ...DEFAULT_ICON },
  leave1: { ...DEFAULT_ICON },
  Reimbrusement1: { ...DEFAULT_ICON },
  Wyzed:{ ...DEFAULT_ICON },
 Wyzed1:{ ...DEFAULT_ICON }
};
  ...
  @track removeRadius = false;
  
    

     /* handleErrorCss(event) {
        const field = event.target.fieldName;
        const helpTextElements = this.template.querySelectorAll('.slds-form-element__help');
        cons
        if(helpTextElements.length === 0){
         this.fieldErrorMap[field] = false;
        }else{
            this.fieldErrorMap[field] = true;
        }
 ...
 /*   handleErrorCss(event) {
    ...
    // Use a delay to let Salesforce finish rendering the DOM
    setTimeout(() => {
        let hasError = false;

        // Query all error messages visible in the DOM
        const allHelpTexts = this.template.querySelectorAll('.slds-form-element__help');

        allHelpTexts.forEach(el => {
            const nameAttr = el.getAttribute('data-name') || '';
            const helpMsg = el.textContent?.trim();

            // ✅ If the help message is for this field and not empty, treat it as an error
            if (nameAttr === field && helpMsg) {
                hasError = true;
            }
        });

        // Update field error map
        this.fieldErrorMap[field] = hasError;

        // Debug output
        console.log(`${field} onchange → hasError:`, hasError);
        console.log('Updated fieldErrorMap:', JSON.stringify(this.fieldErrorMap));
    }, 2000);
} */
...
/* handleErrorCss(event) {
    ...
    // Wait for DOM update (error messages render after onchange)
    setTimeout(() => {
        let hasError = false;

        // Get all help text elements
        const helpTextElements = this.template.querySelectorAll('.slds-form-element__help');
        console.log('helpTextElements:', JSON.stringify(helpTextElements))

        helpTextElements.forEach((el) => {
            const name = el.getAttribute('data-name'); // Important: Salesforce adds this
        ...
        console.log(`${field} onchange → hasError: ${hasError}`);
        console.log('fieldErrorMap:', JSON.stringify(this.fieldErrorMap));
    }, 0);
} */
   ...
   /*  handleErrorCss(event) {
    ...
    // Get the wrapper/form element for the current field only
    const formElement = event.target.closest('.slds-form-element');
    
    // Check if this specific field has an error help message
    const helpText = formElement ? formElement.querySelector('.slds-form-element__help') : null;

    console.log(`Help text for ${field}:`, helpText);

    this.fieldErrorMap[field] = !!helpText;
} */
 handleErrorCss(event) {
    ...
    @track usertype;
    @track NdisFlag;
    @track oZEDISuccessMessage = '';
    @track oZEDIErrorMessage = '';
    ...
    wiredClient(result) {
      this.wiredClientResult = result;
        ...
        if (data) {
            ...
            this.accountingServices = this.clientData.Accounting_Services__c;
            ...
            if(this.accountingServices == 'Quick Books'){
                this.QuickBooksflag = true;
            }

            const syncStatus = this.clientData.Xero_MYOB_Sync_Status__c || '';

            if (syncStatus.includes('Xero synced successfully!')) {
                this.xeroSuccessMessage = syncStatus;
            } else if (syncStatus.includes('Xero sync failed')) {
                this.xeroErrorMessage = syncStatus;
            }

            if (this.clientData?.OZEDI_Client_ID__c) {
                this.oZEDISuccessMessage = 'Your organisation is registered and ready to submit superannuation payments.';
                this.oZEDIErrorMessage = '';
            } else {
                this.oZEDISuccessMessage = '';
                this.oZEDIErrorMessage = 'Your organisation is not registered to submit superannuation payments.';
            }
            console.log('IMAGE:', this.image); 
            ...
            console.log('this.clientData?.OZEDI_Client_Id__c '+ this.clientData.OZEDI_Client_ID__c);
    ...
    get ozediClientId() {
        // Support the API response casing used by existing cached organisation data.
        return this.clientData?.OZEDI_Client_ID__c || this.clientData?.OZEDI_Client_Id__c || '';
    }

    get isCreateOZEDIClientDisabled() {
        console.log('===== isCreateOZEDIClientDisabled =====');
        console.log('clientData:', this.clientData);
        console.log('OZEDI_Client_ID__c:', this.clientData.OZEDI_Client_ID__c);

        const isDisabled = Boolean(this.clientData.OZEDI_Client_ID__c);
    ...
    get showOZEDISection() {
        return this.NdisFlag && !this.Xeroflag && !this.MYOBflag;
    }

    async handleCreateOZEDIClient() {
        this.creatingOZEDIClient = true;
        try {
            await createOZEDIClient();
            await refreshApex(this.wiredClientResult);
            this.dispatchEvent(new ShowToastEvent({
                title: 'Success',
                message: 'Superannuation client was created successfully.',
                variant: 'success'
            }));
        } catch (error) {
            this.dispatchEvent(new ShowToastEvent({
                title: 'Error creating Superannuation client',
                message: error?.body?.message || error.message || 'An unexpected error occurred.',
                variant: 'error'
            }));
        } finally {
            this.creatingOZEDIClient = false;
        }
    }
...
//         orgDetails().then(response => {
   ...
   async connectedCallback() {
    // Await so isSupportCoordinator is known before the URL tab restore runs —
    // otherwise a Support Coordinator deep-linking to a restricted tab could slip
    // past the access guard on first load.
    await this.loadCurrentUserInfo();
        orgDetails().then(response => {    
            this.orgRecord = response;
            this.Picklist_Value = response.Id;
            // Stable public slug from the Organisation_UID__c formula field (URL boundary).
            this.orgUid = response.Organisation_UID__c || '';
            // Put #/admin/<uid>/<tab> in the URL as soon as Admin opens (in-place).
            this._seedRecordUrl();
            console.log('🏢 Organisation Id:', this.Picklist_Value);
        ...
        if(isStart){
            this.plantypestart=true;
        }
        
        const activeTab = localStorage.getItem('activeAdminOrgTab');
        console.log('activeTab in  Org connectedCallback')
        this.orgflag=false;
        this.bankflag=false;
        ...
        // this.serviceCatalogueFlag = false;
        // switch (activeTab) {
        //     case 'adminOrg':
        //         this.orgflag = true;
        //          this.showEditButton = true;
        //         console.log('this.orgflag in Org connectedCallback:' ,this.orgflag);
        //         break;
        //     case 'adminOrgSettings':
        //         this.settingsFlag = true;
        //          this.showEditButton = true;
        //          this.loadOrganisationSettings();
        //         console.log('this.settingsFlag in Org connectedCallback:' ,this.settingsFlag);
        //         break;
        //     case 'adminOrgOnboard':
        //         this.onBoardFlag = true;
        //         this.showEditButton = false;
        //          this.loadOrganisationSettings();
        //         console.log('this.onBoardFlag in Org connectedCallback:' ,this.onBoardFlag);
        //         break;
        //     case 'adminDocuments':
        ...
        //         this.showEditButton = false;
        //         break;
        //     case 'adminServiceCatalogue':
        //         this.serviceCatalogueFlag = true;
        //         break;
        //     default:
        //         this.orgflag = true;
        //         this.showEditButton = true;
        //         console.log('this.orgflag in Org default  connectedCallback:' ,this.orgflag);
        ...
        this.facilitydocument = false;
        this.serviceCatalogueFlag = false;
        // 🔗 URL is the source of truth: restore the tab named in the hash (no flash);
        // otherwise fall back to the default Organisation tab.
        if (!this._restoreTabFromUrl()) {
            this.orgflag = true;
            this.showEditButton = true;
        }
        document.addEventListener('click', this.handleOutsideClick.bind(this));
    ...
    disconnectedCallback() {
        ...
        //localStorage.removeItem('activeAdminOrgTab');
    ...
    handleeditClose() {
        ...
        //this.handleflag();
        this.userRole='';
        this.xeroErrorMessage = '';
      //  this.ticketSupportEmail='';
        this.bmsIdButtonDisable=false;
        this.softwareButtonDisbale=false;
        this.loadOrganisationSettings();
        // Leaving edit → drop /edit from the URL (#/admin/<uid>/<tab>). Use a normal
        // (push) write — it's the reliable path in this community; replaceState can
        // silently no-op in Locker/LWS.
   ...
   handleError(event) {
    ...
    /* handlesave(event){
        ...
        if (!isValid) {
            // stop submit if any required field is missing
            return;
        }
        ...
        // alert(JSON.stringify(fields));
          ...
          fields.Xero_MYOB_Sync_Status__c = this.xeroSuccessMessage && this.xeroSuccessMessage.trim() ? this.xeroSuccessMessage : this.xeroErrorMessage;
    ...
    async handlesave(event) {
        ...
        if (this.Xeroflag === true) {
            ...
            fields.forEach(field => {
                if (field.fieldName === 'Client_id__c') {
                    clientId = field.value;
                }

                if (field.fieldName === 'Client_secret__c') {
                    clientSecret = field.value;
                }
            });

            // Only check Xero if both fields have values
            if (clientId && clientSecret) {

                await this.handleXeroSync();

                if (this.xeroErrorMessage?.trim()) {
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Error',
                            message:
                                'Xero Client ID and Client Secret are not synced. Please sync successfully before saving.',
                            variant: 'error'
                        })
                    );
                    return;
                ...
                if (
                    !this.xeroErrorMessage?.trim() &&
                    !this.xeroSuccessMessage?.trim()
                ) {
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Error',
                            message:
                                'Please click "Sync Now" and complete the Xero sync before saving.',
                            variant: 'error'
                        })
                    );
                    return;
        ...
        if (
            ...
            fields.Xero_MYOB_Sync_Status__c =
                this.xeroSuccessMessage?.trim()
                    ? this.xeroSuccessMessage
                    : this.xeroErrorMessage;
    ...
    get majorContainerClass() {
        return (this.facilitydocument || this.onBoardFlag || this.subscriptionFlag || this.serviceCatalogueFlag)
            ? 'major-cont custom-scroll overflow-hidden-cont'
            : 'major-cont custom-scroll';
    }
    ...
    return this.subscriptionFlag ? 'menu-item1' : 'menu-item';
    }

    get serviceCatalogueclass() {
     return this.serviceCatalogueFlag ? 'menu-item1' : 'menu-item';
    }

    // =====================================================================
    // HASH ROUTER bridge (additive) — reuses EXISTING tab handlers, no rename.
    // URL model handled by the shell: #/admin/<uid>/<tab>
    // =====================================================================

    // Derive the shell's verbatim `x-####-####-####` uid from the org Id in JS
    // (same math as the CASESAFEID formula — no Apex/field/schema change needed).
    // Verbatim `x-####-####-####` uid — sourced from the Organisation_UID__c
    // formula field (returned by orgDetails() and stored on load). No slicing.
    get _recordUid() {
        return this.orgUid || '';
    }

    // tab slug -> EXISTING handler name (labels mirror the sidebar tabs)
    get _tabHandlerBySlug() {
        return {
            'organisation': 'handledeorg',
            'settings': 'handleSettings',
            'onboarding': 'handleOnboard',
            'documents': 'handledocument',
            'subscription': 'handleSubscription',
            'service-catalogue': 'handleServiceCatalogue'
        };
    }

    // Tell the shell about every view change (bubbling + composed).
    // `replace` = update the URL in place (used for the default view on module open).
    notifySubRoute(subView, replace) {
        ...
        // Skip until the org record id is known, so we never emit a uid-less path.
        if (!this._recordUid) return;
        this.dispatchEvent(new CustomEvent('subrouteupdate', {
            detail: { subView, recordId: this.Picklist_Value, replace: !!replace },
            bubbles: true,
            composed: true
        }));
    }

    // Which tab is currently showing → slug (used to seed the URL on module open).
    _currentTabSlug() {
        if (this.settingsFlag) return 'settings';
        if (this.onBoardFlag) return 'onboarding';
        if (this.facilitydocument) return 'documents';
        if (this.subscriptionFlag) return 'subscription';
        if (this.serviceCatalogueFlag) return 'service-catalogue';
        return 'organisation';
    }

    // URL is the source of truth for the active tab. If the hash names a tab (and
    // optionally /edit), open it here in connectedCallback so there is no flash of
    // the default Organisation tab before the shell's openByUID() runs. Emits are
    // suppressed (the shell owns the URL). Returns true if it handled the tab.
    _restoreTabFromUrl() {
        const parts = (window.location.hash || '')
            .replace(/^#\/?/, '')
            .split('/')
            .filter(Boolean);
        if (parts[0] !== 'admin') return false; // this component only lives under Admin
        const uidIdx = parts.findIndex((p) => /^(org|x|fac)-/i.test(p));
        const tab = uidIdx >= 0 ? parts[uidIdx + 1] : parts[1];
        const isEdit = (uidIdx >= 0 ? parts[uidIdx + 2] : parts[2]) === 'edit';
        if (!tab || !this._tabHandlerBySlug[tab]) return false;
        this._suppressEmit = true;
        try {
            this._selectTab(tab);
            if (isEdit) this.handleEditOrg();
        } finally {
    ...
    // On Admin open, put the record uid in the URL (#/admin/<uid>/<tab>) WITHOUT a
    // new history entry — unless the URL already carries a deep link (shell drives that).
    _seedRecordUrl() {
        if (!this._recordUid) return;
        const parts = (window.location.hash || '')
            .replace(/^#\/?/, '')
            .split('/')
            .filter(Boolean);
        const hasDeepLink = parts.length > 1; // e.g. admin/<uid>/... already present
        if (hasDeepLink) return;
        this.notifySubRoute(`${this._recordUid}/${this._currentTabSlug()}`, true);
    }

    // // Reverse action: select a tab by slug via its EXISTING handler (no duplicated logic).
    // _selectTab(slug) {
    //     const fn = this._tabHandlerBySlug[slug] || 'handledeorg';
    //     if (typeof this[fn] === 'function') this[fn]();
    // }

    // A Support Coordinator only gets the Organisation tab — the other tab headers
    // are hidden in the markup (`if:false={isSupportCoordinator}`). Mirror that here
    // so a hand-edited/deep-linked URL cannot open a tab they have no access to.
    _isTabAllowed(slug) {
        if (this.isSupportCoordinator && slug && slug !== 'organisation') {
            return false;
        }
        return true;
    }
    ...
    // Reverse action: select a tab by slug via its EXISTING handler (no duplicated logic).
    _selectTab(slug) {
        // Access guard: fall back to the Organisation tab for restricted users.
        if (!this._isTabAllowed(slug)) {
            slug = 'organisation';
        }
        const fn = this._tabHandlerBySlug[slug] || 'handledeorg';
        if (typeof this[fn] === 'function') this[fn]();
    ...
    // Shell -> child: open this record at a tab, optionally in edit mode.
    // Emits are suppressed here — the shell already owns the URL during restore.
    @api openByUID(uid, tab, isEdit) {
        this._suppressEmit = true;
        try {
            this.currentUid = uid || this._recordUid;
            this._selectTab(tab || 'organisation');
            if (isEdit) this.handleEditOrg(); // re-open the edit popup for the active tab
        } finally {
            this._suppressEmit = false;
        }
    }
    // Org is a single-record view (no create flow) — safe no-op for router symmetry.
    @api openCreate() {}
    ...
    handledeorg(event){
        ...
        localStorage.setItem('activeAdminOrgTab', 'adminOrg');
        this.notifySubRoute(`${this._recordUid}/organisation`);
    }
    handleaddress(event){
        this.orgflag=false;
        this.bankflag=false;
    ...
    handleEditOrg() {
        ...
        if (this.orgflag) {
            ...
            this.orgeditflag = true;
            this.fieldErrorMap = {};
            this.successMessage = 'Organisation updated successfully.';
        ...
        // Reflect edit mode in the URL: #/admin/<uid>/<tab>/edit (new history entry).
        this.notifySubRoute(`${this._recordUid}/${this._currentTabSlug()}/edit`);
    }
    handleRole(){
        this.orgflag=false;
        this.bankflag=false;
    ...
    handleSettings(event) {
        ...
        this.serviceCatalogueFlag = false;
        // this.loadOrganisationSettings();
        localStorage.setItem('activeAdminOrgTab', 'adminOrgSettings');
        this.loadOrganisationSettings();
        this.notifySubRoute(`${this._recordUid}/settings`);
    }

    async handleOnboard(event) {
          console.log('👉 Onboarding clicked');

    try {
        const isStart = await isStartPlan();

        console.log('🔥 Apex result:', isStart);

        // 🔴 BLOCK ONBOARDING
        if (isStart) {
            console.log('🚫 Blocking Onboarding');
        ...
        console.log('🚀 handleOnboard CLICKED');

        // ⭐ SAVE TAB FIRST
        localStorage.setItem('activeAdminOrgTab', 'adminOrgOnboard');
        console.log('📦 localStorage activeAdminOrgTab:',
            localStorage.getItem('activeAdminOrgTab')
        );

        // 🔎 BEFORE RESET — show current state
        console.log('🔍 BEFORE RESET:', {
            orgflag: this.orgflag,
            settingsFlag: this.settingsFlag,
            onBoardFlag: this.onBoardFlag,
            facilitydocument: this.facilitydocument
        });
        ...
        // 🔎 AFTER SETTING FLAG
        console.log('✅ AFTER SETTING onBoardFlag:', this.onBoardFlag);

        this.showEditButton = false;

        console.log('🎯 handleOnboard FINAL STATE:', {
            onBoardFlag: this.onBoardFlag,
            facilitydocument: this.facilitydocument,
        ...
        console.log('📞 Calling loadOrganisationSettings()...');
        this.loadOrganisationSettings();

        // ⭐ CHECK STATE AFTER METHOD CALL (async safe)
        setTimeout(() => {
            console.log('⏱️ FINAL STATE AFTER 500ms:', {
                onBoardFlag: this.onBoardFlag,
                facilitydocument: this.facilitydocument,
                orgflag: this.orgflag,
                settingsFlag: this.settingsFlag
    ...
    handledocument(event) {
        ...
        this.subscriptionFlag = false;
        this.serviceCatalogueFlag = false;
        console.log('handledocument', this.facilitydocument);
        localStorage.setItem('activeAdminOrgTab', 'adminDocuments');
        this.notifySubRoute(`${this._recordUid}/documents`);
    }

    handleServiceCatalogue(event) {
        this.orgflag = false;
        this.bankflag = false;
        this.rosterflag = false;
        this.addressflag = false;
        ...
        this.subscriptionFlag = false;
        this.serviceCatalogueFlag = true;
        console.log('handledocument', this.serviceCatalogueFlag);
        localStorage.setItem('activeAdminOrgTab', 'adminServiceCatalogue');
        this.notifySubRoute(`${this._recordUid}/service-catalogue`);
    }
    ...
    showEditButton = true;
       ...
       // this.subscriptionFlag = false;
        ...
        localStorage.setItem('activeAdminOrgTab', 'adminSubscription');
        this.notifySubRoute(`${this._recordUid}/subscription`);
    }
    ...
    const dual = this.template.querySelector('lightning-dual-listbox');

    if (selected.length > 12) {
        dual.setCustomValidity('You can select a maximum of 12 users.');
        dual.reportValidity();
        // optionally: don't update tracked value so model stays at previous valid state
        return;
    }

    dual.setCustomValidity('');
    dual.reportValidity();
    this.ticketSelectedUserIds = selected;
...
async handleXeroSync() {
    ...
    try {
        ...
        if (
            result?.includes('Xero sync failed') ||
            result?.includes('Xero auth failed') ||
            result?.includes('Unexpected error') ||
            result?.includes('Connected to Xero but no tenant connections found') ||
            result?.includes('Token valid but connections check failed')
        ) {
            this.xeroErrorMessage = result;
        } else {
            this.xeroSuccessMessage = result;
        }
    } catch (error) {
        this.xeroErrorMessage =
            error?.body?.message ||
