...
//   @track ParticpantRecordForm = false;
  ...
  @api openEditWizard(stepName) {
      console.log('[Routing] clientDetail openEditWizard called with step:', stepName);
      this._ParticpantRecordForm = true;
      if (stepName) {
          this._createCurrentStep = stepName.replace('step', 'createstep');
      } else {
  ...
  get participantUid() {
      return this.wiredClientResult?.data?.[0]?.Participant_UID__c || '';
  }
  ...
  _syncRoute() {
      ...
      let subView = `${uid}/details`;
      if (this._ParticpantRecordForm) {
          const stepNum = this._createCurrentStep ? this._createCurrentStep.replace('createstep', '') : '1';
          subView = `${uid}/details/edit/step${stepNum}`;
      }
      
      console.log('[Routing] clientDetail dispatching subrouteupdate:', subView);
  ...
  @track clientWithUser;
    ...
    wiredClient(result) {
        ...
        if (data) {
            ...
            this.clientWithUser=data[0].Client_with_User__c;
