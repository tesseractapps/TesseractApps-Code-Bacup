...
export default class CreateEditClientLwc extends NavigationMixin(LightningElement) {
  ...
  @track _showContactTypeModal = false;
  @api get showContactTypeModal() { return this._showContactTypeModal; }
  set showContactTypeModal(val) {
      this._showContactTypeModal = val;
      this.dispatchEvent(new CustomEvent('subpopup', {
          detail: { slug: val ? 'contact-type' : '' },
          bubbles: true,
          composed: true
      }));
  }
...
@track _showContactAttachmentModal = false;
@api get showContactAttachmentModal() { return this._showContactAttachmentModal; }
set showContactAttachmentModal(val) {
    this._showContactAttachmentModal = val;
    this.dispatchEvent(new CustomEvent('subpopup', {
        detail: { slug: val ? 'contact-attachment' : '' },
        bubbles: true,
        composed: true
    }));
}
...
@track _showAttachmentListModal = false;
@api get showAttachmentListModal() { return this._showAttachmentListModal; }
set showAttachmentListModal(val) {
    this._showAttachmentListModal = val;
    this.dispatchEvent(new CustomEvent('subpopup', {
        detail: { slug: val ? 'attachment-list' : '' },
        bubbles: true,
        composed: true
    }));
}
...
@api get isModalOpen() { return this._isModalOpen; }
set isModalOpen(val) {
    this._isModalOpen = val;
    this.dispatchEvent(new CustomEvent('subpopup', {
        detail: { slug: val ? 'view-attachment' : '' },
        bubbles: true,
        composed: true
    }));
}
           ...
           const contactField = stepEl?.querySelector('[data-id="contact"]');
            ...
            if (contactField) {
                ...
                if (!contact) {
                    ...
                    contactField.closest('.floating-label, .ContactNumberClass')?.classList.remove('has-error');
                }
            }
...
wiredClient(result) {
    ...
    if (data) {
        ...
        this.clientWithUser=client.Client_with_User__c;
...
handleEditAttachment(event) {
    ...
    @api get isEdit() {
        return !!(this.detailseditflag || this.addresseditflag || this.identificationeditflag || this.insuranceeditflag || this.primaryeditflag || this.secondaryeditflag || this.participanteditflag);
    }

    @api get currentStep() {
        const step = (this.createCurrentStep || 'createstep1').replace('create', '');
        console.log('Child LWC: currentStep getter evaluated. Value:', step);
        return step;
    }
    ...
    @api currentTabSlug() {
        if (this.detailsflag) return 'personal';
        if (this.addressflag) return 'address';
        if (this.identificationflag) return 'identification';
        if (this.insuranceflag) return 'insurance';
        if (this.primaryflag) return 'primary';
        if (this.participantflag) return 'preferred-staff';
        if (this.secondaryflag) return 'secondary';
        return 'personal';
    }

    @api selectTab(slug) {
        const s = String(slug || '').toLowerCase();
        if (s === 'details' || s === 'clientdetails' || s === 'personal') {
            this.handledetails();
        } else if (s === 'address' || s === 'clientaddress') {
            this.handleaddress();
        } else if (s === 'identification' || s === 'clientidentification') {
            this.handleidentification();
        } else if (s === 'insurance' || s === 'clientinsurance') {
            this.handleinsurance();
        } else if (s === 'primary' || s === 'clientprimary') {
            this.handleprimary();
        } else if (s === 'participant' || s === 'clientparticipant' || s === 'preferred-staff' || s === 'preferredstaff') {
            this.handleParticipant();
        } else if (s === 'secondary' || s === 'clientsecondary') {
            this.handlesecondary();
        }
    }

    @api openPopup(slug) {
        if (slug === 'contact-type') {
            this.showContactTypeModal = true;
    ...
    notifyStateChange() {
        console.log('Child LWC: notifyStateChange() executing. createCurrentStep:', this.createCurrentStep);
