...
export default class TessaAiChat extends LightningElement {
    ...
    async loadTypeaheadIndex() {
        try {
            const res = await fetch(TYPEAHEAD_INDEX);
            if (res.ok) this._suggestIndex = await res.json();
        } catch (e) {
            // Typeahead is a nicety; if the index fails to load, the chat
    ...
    updateSuggestions(raw) {
        ...
        for (const e of this._suggestIndex) {
            const ql = e.q.toLowerCase();
            const kw = e.kw || '';
            // Every typed word must appear somewhere (AND match) for precision.
            if (!tokens.every(t => ql.includes(t) || kw.includes(t))) continue;
            let score = 0;
            if (ql.startsWith(q)) score += 100;
            if (ql.includes(q))   score += 40;
            for (const t of tokens) {
                if (ql.includes(t)) score += 6;      // word in the question itself
                else if (kw.includes(t)) score += 2; // word in rephrasings/feature
            }
            if (score > 0) scored.push({ e, score });
        }
        scored.sort((a, b) => b.score - a.score || a.e.q.length - b.e.q.length);
        this._activeSuggestion = -1;
        this.suggestions = scored.slice(0, 6).map((s, i) => ({
