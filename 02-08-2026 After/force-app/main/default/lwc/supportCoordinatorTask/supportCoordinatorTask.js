...
chargeLevelRates = {
    ...
    connectedCallback() {
        ...
        const savedFrequency = localStorage.getItem(this.FREQUENCY_STORAGE_KEY);
    ...
    async loadTasks() {
        ...
        try {
            ...
            this.allTasks = result.map((task, index) => {
                ...
                const rawDuration = Number(task.Duration_Hours__c || 0);
                
                const durationHours   = Math.floor(rawDuration);

                const durationMinutes = Math.round((rawDuration - durationHours) * 60);

                const startTime =
                        task.Start_Time__c !== null && task.Start_Time__c !== undefined
                ...
                const normalizedTask = {
                    ...
                    billable:
                        task.Billable__c === true,

                    priority:
                        task.Priority__c || '',

                    status:
                        task.Status__c || 'Non Billable',
                    ...
                    participantId:
                        task.SupportClient_To_Notes__c,
                    ...
                    // =====================================
                    serviceType:
                        task.Service_Type__r?.Name || '',

                    serviceTypeId:
                        task.Support_CoordinatorCatalogueGroup__c  || '',
                    ...
                    serviceItem:
                        task
                            .SupportNotes_To_SupportCatalogue__r
                            ?.Support_Item_Name__c || '',

                    serviceItemId:
                        task.Support_CoordinatorCatalogue__c  || '',

                    // =====================================
                    // Billing
                    // =====================================
                    rate:
                        Number(task.Charge_Level__c || 0),

                    formattedRate:
                        this.formatAmount(
                            task.Charge_Level__c
                        ),

                    totalCost:
                        Number(task.Total_Cost__c || 0),
                    ...
                    // =====================================
                    createdFrom:
                        task.Created_From__c || '',
                    
                    street:     task.Address__Street__s    || '',
                    city:       task.Address__City__s      || '',
                    province:   task.Address__StateCode__s  || '',
                    postalCode: task.Address__PostalCode__s || '',
                    country:    task.Address__CountryCode__s || '',
    ...
    async handleFieldChange(event) {
        ...
        if (field === 'duration') {
            if (Number(value) >= 24) {
                this.taskForm = { ...this.taskForm, duration: 24, durationMinutes: 0 };
            }            
            this._refreshBudget();
        }
        ...
        if (field === 'ServiceType') {
            ...
            if (selectedGroup) {
                ...
                this.fundingInfo = {
                    ...
                    currentSpending: this.formatAmount(totalSpent),
                ...
                console.log(
                    'fundingInfo:',
                    JSON.stringify(this.fundingInfo)
                );
                console.log('==========================');
                ...
                this.serviceItemOptions =
                    (selectedGroup.Support_Catalogues__r || []).map(item => ({
                        label:
                            `${item.Support_Item_Name__c} ($${this.formatAmount(item.Amount__c)})`,
                        value: item.Id
                    }));
            }
        }
    ...
    resetTaskForm() {
        this.taskForm = {
            recordId: '', title: '', participant: '', date: '', startTime: '',
            priority: '', duration: 1, durationMinutes: 0,
            description: '', billable: false,
            ServiceType: '', ServiceItem: '', chargeLevel: 0, entryCost: 0,
            street: '', city: '', province: '', postalCode: '', country: ''  // ← ADD
        };
        this.isBillable               = false;
        this.isEditingQty             = false;
        this.taskcreationflag = 'Create Task';
        this.editableRate             = 0;
        this.serviceTypeOptions       = [];
        this.serviceItemOptions       = [];
        this.selectedParticipantGroups = [];
    ...
    _refreshBudget() {
        ...
        console.log('==============================');
        console.log('🔄 _refreshBudget START');
        ...
        const selectedGroup =
            (this.selectedParticipantGroups || []).find(
                group => group.Id === this.taskForm.ServiceType
            );
        ...
        if (!selectedGroup) {
            console.log('❌ Selected Service Type not found');
            this._resetBudget();
            return;
        }
        ...
        console.log('📌 Billable:', this.isBillable);
        ...
        console.log('🧾 Current Task Cost:', currentTaskCost);

        const updatedSpent =
            alreadySpent + currentTaskCost;

        const remainingBudget =
            totalBudget - updatedSpent;

        const percentUsed =
            totalBudget > 0
                ? Math.round(
                    (updatedSpent / totalBudget) * 100
                )
                : 0;

        console.log('📈 Updated Spent:', updatedSpent);
        ...
        console.log(
            '✅ fundingInfo Updated:',
            JSON.stringify(this.fundingInfo, null, 2)
        );

        console.log('🔄 _refreshBudget END');
        console.log('==============================');
    }
    ...
    async saveTask() {
        ...
        const missing = required.filter(f => {
            const value = this.taskForm[f.key];
        ...
        if (this.isBillable && this.taskForm.ServiceType) {

            console.log('====================================');
            console.log('Starting Service Type Budget Validation');
            ...
            if (!selectedGroup) {
                ...
                this.showToast(
                    'Error',
                    'Unable to find selected Service Type.',
                    'error'
                );
            ...
            if (entryCost > availableFunds) {
                ...
                this.showToast(
                    'Insufficient Funds',
                    `Available budget is $${availableFunds.toFixed(2)}, but this entry requires $${entryCost.toFixed(2)}.`,
                    'error'
                );
            ...
            console.log(
                'Budget validation skipped (not billable or Service Type not selected)'
            );
        }
        ...
        try {
            ...
            const payload = {
                ...this.taskForm,
                billable: this.isBillable,
                entryCost: Number(String(this.entryCost).replace(/,/g, '')),
                address: [
                    this.taskForm.street,
                    this.taskForm.city,
                    this.taskForm.province,
                    this.taskForm.postalCode,
                    this.taskForm.country
                ].filter(Boolean).join(', ')   // ← concatenate into single string
            };
    ...
    get dailyTasks() {
        ...
        return tasks.map((task, idx) => {
           /*  const colWidthPct = 100 / numCols;
            ...
            const topPx = task._startMin * PX_PER_MIN;


            // Correct height calculation
            const heightPx = Math.max(
                (task._endMin - task._startMin) * PX_PER_MIN,
                36
            );
    ...
    get isDurationMinutesDisabled() {
        return Number(this.taskForm?.duration) >= 24;
    }

    async handleTaskCardClick(event) {
        console.log('======================================');
        console.log('🚀 handleTaskCardClick START');
        this.taskcreationflag = 'Update Task';

        const taskId = event.currentTarget.dataset.taskid;

        console.log('📌 Clicked Task Id:', taskId);
        console.log('📋 Total Tasks Available:', this.allTasks?.length);

        const selectedTask = this.allTasks.find(task => task.id === taskId);

        console.log('🔍 Selected Task:', JSON.stringify(selectedTask, null, 2));
        ...
        console.log('📝 Populating taskForm...');

        this.taskForm = {
            recordId:        selectedTask.id,
            title:           selectedTask.title,
            participant:     selectedTask.participantId,
            date:            selectedTask.date,
            startTime:       this.convertTo24Hour(selectedTask.time),
            priority:        selectedTask.priority,
            duration:        selectedTask.duration,
            durationMinutes: selectedTask.durationMinutes,
            description:     selectedTask.description,
            ServiceType:     selectedTask.serviceTypeId,
            ServiceItem:     selectedTask.serviceItemId,
            street:          selectedTask.street     || '',
            city:            selectedTask.city       || '',
            province:        selectedTask.province   || '',
            postalCode:      selectedTask.postalCode || '',
            country:         selectedTask.country    || ''
        };

        this.isBillable   = selectedTask.billable;
        this.editableRate = selectedTask.rate;

        // ── Reload service type + item options ──
        if (selectedTask.participantId) {
            try {
                const groups = await getServiceTypesByParticipant({
                    participantId: selectedTask.participantId
                });

                this.selectedParticipantGroups = groups || [];

                this.serviceTypeOptions = this.selectedParticipantGroups.map(group => ({
                    label: group.Name,
                    value: group.Id
                }));
                ...
                if (selectedTask.serviceTypeId) {
                    const selectedGroup = this.selectedParticipantGroups.find(
                        g => g.Id === selectedTask.serviceTypeId
                    );

                    if (selectedGroup) {
                        this.serviceItemOptions = (selectedGroup.Support_Catalogues__r || []).map(item => ({
                            label: `${item.Support_Item_Name__c} ($${this.formatAmount(item.Amount__c)})`,
                            value: item.Id
                        }));

                        const totalBudget = Number(selectedGroup.TotalBudget) || 0;
                        const totalSpent  = Number(selectedGroup.TotalSpent)  || 0;

                        this.fundingInfo = {
                            percentUsed:     totalBudget > 0 ? Math.round((totalSpent / totalBudget) * 100) : 0,
                            totalBudget:     this.formatAmount(totalBudget),
                            currentSpending: this.formatAmount(totalSpent),
                            remaining:       this.formatAmount(totalBudget - totalSpent)
                        };
                    }
                }

            } catch (error) {
                this.showToast('Error', 'Failed to load service types', 'error');
            }
        }

        this.showTaskPopup = true;

        console.log('✅ Task popup opened successfully');
        console.log('======================================');
        console.log('🚀 handleTaskCardClick END');
    }
    ...
    handleTranscript(event) {
      ...
      // this.taskForm.description =
        ...
        this.taskForm = {
            ...this.taskForm,
            description:
                (this.taskForm.description || '') + event.detail.text
        };
        console.log('this.taskForm.description  : ',this.taskForm.description);
    }
