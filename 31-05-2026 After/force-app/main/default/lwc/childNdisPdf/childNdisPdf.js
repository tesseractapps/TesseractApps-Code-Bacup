    ...
    connectedCallback(){
        ...
        if (this.currentRecordPdf && Array.isArray(this.currentRecordPdf)) {
            this.currentRecordPdf = this.currentRecordPdf.map(record => {
                ...
                allocationData.forEach(item => {
                    const shifts = [
                      ...
                      //  { name: 'Saturday', rate: item.Saturday_Hourly_Rate__c, hours: item.Saturday_Hours__c },
                        ...
                        { name: 'Extended Hours', rate: item.Extended_Hourly_Rate__c, hours: Number(parseFloat(item.Extended_Hours__c || 0).toFixed(2)) },
                        { name: 'Public Holiday', rate: item.Public_Holiday_Hour_Rate__c, hours: item.Public_Holiday_Hours__c },
                        { name: 'Extended Broken Hours', rate: item.Broken_Extended_Rate__c, hours: Number(parseFloat(item.Broken_Extended_Duration__c || 0).toFixed(2)) },
                        { name: 'Over Time Tier One Hours', rate: item.Over_Time_Tier_One_Rate__c, hours: item.Over_Time_Tier_One__c },
                        { name: 'Over Time Tier Two Hours', rate: item.Over_Time_Tier_Two_Rate__c, hours: item.Over_Time_Tier_Two__c }
                    ];

                     if (item.Long_Sleepover_Allowance__c > 0) {
                        shifts.push({
                            name: 'Sleepover',
                            rate: item.Long_Sleepover_Allowance__c,
                            hours: 0,
                            qty: 1,
                            isDirectRate: true
                        });
                     ...
                     if (item.Sleepover_Shift_Hourly_Rate__c > 0) {
                        shifts.push({
                            name: 'Sleepover',
                            rate: item.Sleepover_Shift_Hourly_Rate__c,
                            hours: 0,
                            qty: 1,
                            isDirectRate: true
                        });
                    ...
                    if (item.Broken_Allowance__c > 0) {
                        shifts.push({
                            name: 'Broken_Allowance',
                            rate: item.Broken_Allowance__c,
                            hours: 0,
                            qty: 1
                        });
                    }
                ...
                /* 🔥 START: PAID & UNPAID LEAVE */

                if (record && record.staffpayrollData) {

                    const payroll = record.staffpayrollData;

                    console.log('🔥 Payroll Data:', JSON.parse(JSON.stringify(payroll)));

                    const leaveTypes = [
                        { name: 'Annual Leave', field: 'Annual_Paid_Leave_Hours__c' },
                        { name: 'Personal Leave', field: 'Personal_Paid_Leave_Hours__c' },
                        { name: 'Compassionate Leave', field: 'Compassionate_Paid_Leave_Hours__c' },
                        { name: 'TOIL', field: 'Toil_Paid_Leave_Hours__c' },
                        { name: 'RDO/ADO', field: 'Rdo_Paid_Leave_Hours__c' },
                        { name: 'Bereavement Leave', field: 'Bereavement_Paid_Leave_Hours__c' },
                        { name: 'Birthday Leave', field: 'Birthday_Paid_Leave_Hours__c' },
                        { name: 'Wellness Leave', field: 'Wellness_Paid_Leave_Hours__c' },
                        { name: 'Volunteer Leave', field: 'Volunteer_Paid_Leave_Hours__c' },
                    ];

                    leaveTypes.forEach(type => {
                        const hours = payroll[type.field];

                        console.log(`👉 Processing ${type.name} | Hours: ${hours}`);

                        if (hours && hours > 0) {
                            allShifts.push({
                                name: type.name,
                                rate: payroll.Paid_Leave_Rate__c || 0,
                                hours: hours,
                                qty: 1
                            });
                        }
                    ...
                    if (payroll.Parental_Unpaid_Leave_Hours__c && payroll.Parental_Unpaid_Leave_Hours__c > 0) {
                        allShifts.push({
                            name: 'Parental Leave',
                            rate: 0,
                            hours: payroll.Parental_Unpaid_Leave_Hours__c,
                            qty: 1
                        });
                    }

                    if (payroll.Unpaid_Carers_Unpaid_Leave_Hours__c && payroll.Unpaid_Carers_Unpaid_Leave_Hours__c > 0) {
                        allShifts.push({
                            name: 'Unpaid Carers Leave',
                            rate: 0,
                            hours: payroll.Unpaid_Carers_Unpaid_Leave_Hours__c,
                            qty: 1
                        });
                    }

                    if (payroll.Extended_Unpaid_Leave_Hours__c && payroll.Extended_Unpaid_Leave_Hours__c > 0) {
                        allShifts.push({
                            name: 'Extended Leave',
                            rate: 0,
                            hours: payroll.Extended_Unpaid_Leave_Hours__c,
                            qty: 1
                        });
                    }
                }
...
combineSubsequentShifts(shifts) {
    ...
    const shiftGroups = {
        ...
        Sleepover: ['Sleepover', 'Sleepover Night'],
        Saturday: ['Saturday'],
        Sunday: ['Sunday'],
        ...
        Extended_Broken_Hours: ['Extended Broken Hours'],
        ...
        Unpaid_Carers_Leave: ['Unpaid Carers Leave'],
        Extended_Leave: ['Extended Leave'],
        Over_Time_Tier_One_Hours: ['Over Time Tier One Hours'],
        Over_Time_Tier_Two_Hours: ['Over Time Tier Two Hours'],
    ...
    const normalizedShifts = shifts
        .map(shift => ({
            ...
            hours: parseFloat(shift.hours) || 0,
             qty: parseInt(shift.qty) || 0,
             isDirectRate: shift.isDirectRate || false

        }))
        .filter(shift => (shift.hours > 0 )||( shift.rawName === 'Cancelled') ||  shift.isDirectRate  || ( shift.rawName === 'Broken_Allowance')); // remove invalid entries
    ...
    normalizedShifts.forEach(shift => {
        ...
        if (combinedMap.has(key)) {
            ...
            combinedMap.get(key).qty += shift.qty;
        } else {
...
const shiftYtdMap = {
    ...
    Broken_Allowance: this.currentRecordPdf[0].staffpayrollData.Current_Broken_Allowance_YTD__c || 0,
    Extended_Broken_Hours:  this.currentRecordPdf[0].staffpayrollData.Current_Broken_YTD__c || 0,
    Cancelled : this.currentRecordPdf[0].staffpayrollData.Current_Cancel_YTD__c || 0,
    Paid_Leave : this.currentRecordPdf[0].staffpayrollData.Current_Paid_Leave_Ytd__c || 0,
    // ===== PAID LEAVE YTD (NEW) =====
    Annual_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Annual_Leave_YTD__c || 0,
    Personal_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Personal_Leave_Ytd__c || 0,
    Compassionate_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Compassionate_Leave_Ytd__c || 0,
    TOIL: this.currentRecordPdf[0].staffpayrollData.Current_Toil_Leave_Ytd__c || 0,
    RDO_ADO: this.currentRecordPdf[0].staffpayrollData.Current_Rdo_Leave_Ytd__c || 0,
    Bereavement_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Bereavement_Leave_Ytd__c || 0,
    Birthday_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Birthday_Leave_Ytd__c || 0,
    Wellness_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Wellness_Leave_Ytd__c || 0,
    Volunteer_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Volunteer_Leave_Ytd__c || 0,

    // ===== UNPAID =====
    Unpaid_Leave: this.currentRecordPdf[0].staffpayrollData.Current_Unpaid_Leave_YTD__c || 0,
    Over_Time_Tier_Two_Hours: this.currentRecordPdf[0].staffpayrollData.Current_Over_Time_Tier_One_YTD__c || 0,
    Over_Time_Tier_One_Hours: this.currentRecordPdf[0].staffpayrollData.Current_Over_Time_Tier_Two_YTD__c || 0
};

console.log('shiftYtdMap =>', JSON.stringify(shiftYtdMap, null, 2));
...
Object.keys(shiftsByName).forEach(shiftName => {
    ...
    rows.forEach((row, index) => {
       // const hours = row.hours || 0;
        ...
        let displayQty = row.hours || 0; // default for normal shifts
        let total = rate * displayQty;
       // const total = row.name=='Broken_Allowance' ||row.name=='Cancelled'?row.rate : rate * hours;
        if (row.name === 'Broken_Allowance' || row.name === 'Cancelled' || row.isDirectRate) {
                displayQty = row.qty || 0;
                total = rate * displayQty;
        }

        startY = checkPageSpace(doc, startY, 6);

       // doc.text(`${shiftName} `, 10, startY);/* @ ${rate} */
       const displayShiftName = shiftName.replace(/_/g, ' ');
       doc.text(displayShiftName, 10, startY);
       // doc.text(hours.toString(), 97, startY);
        doc.text(displayQty.toString(), 97, startY);
...
doc.text(dollarUSLocale.format(reimbValue.toFixed(2)), 144, startY);
 ...
 const roles = this.currentRecordPdf?.[0]?.staffData?.StaffRoles__r || [];

const primaryRole = roles.find(role => role.Primary__c === true) || roles[0];


 //Leave Summary
if (
   primaryRole?.Type_of_Job__c &&
    (
        primaryRole.Type_of_Job__c === 'Full-time' ||
        primaryRole.Type_of_Job__c === 'Part-time'
    )
) {
    startY = checkPageSpace(doc, startY + 6, 6);
    ...
    // Column headers
    doc.text("ACCRUED", 122, startY);
    doc.text("USED", 144, startY);
   /*  doc.text("Used(Current)", 144, startY); */
    ...
    Object.keys(leaveSummary).forEach((leaveType) => {
        ...
        const totalAccrued   = leaveData["Total Accrued"] ?? 0;
        const totalUsed      = leaveData["Total Used"] ?? 0;
       /*  const usedThisPeriod = leaveData["Used This Period"] ?? 0; */
        const balance        = leaveData["Balance"] ?? 0;
        ...
        // Values
        doc.text(totalAccrued.toFixed(2), 122, startY);
        doc.text(totalUsed.toFixed(2), 144, startY);
       /*  doc.text(usedThisPeriod.toFixed(2), 144, startY); */
        doc.text(balance.toFixed(2), 179, startY);
    });
}
