            ...
            console.log('storedFacilityLabel'+storedFacilityLabel);
               ...
               getCurrentLoggedUserInfo().then(userData=>{
                    ...
                    if( userType =='NDIS Org Admin' || userType == 'ICT Admin'){
                        ...
                        const allParticipants = this.refreshTable.data || [];
                        const normalizedUserEmail = (userEmail || '').toLowerCase();
                        const safeParticipantType = participantType || 'Parent';

                        // 🔎 Logged-in participant
                        const loggedInParticipant = allParticipants.find(
                            p => (p.Email__c || '').toLowerCase() === normalizedUserEmail
                        );

                        let filteredParticipants = [];

                        // 🔴 PARENT → only logged-in participant
                        if (safeParticipantType === 'Parent') {
                            filteredParticipants = [loggedInParticipant];
                        }

                        // 🟡 CHILD → logged-in email must appear in someone else's contacts
                        else if (safeParticipantType === 'Child') {
                            filteredParticipants = allParticipants.filter(p =>
                                (p.Participant_Contacts__r || []).some(
                                    c => (c.Email__c || '').toLowerCase() === normalizedUserEmail
                                )
                            );
                        }

                        // 🟢 BOTH → logged-in + related child/parent participants
                        else if (safeParticipantType === 'Both') {
                            filteredParticipants = allParticipants.filter(p =>
                                (p.Email__c || '').toLowerCase() === normalizedUserEmail || // logged-in
                                (p.Participant_Contacts__r || []).some(
                                    c => (c.Email__c || '').toLowerCase() === normalizedUserEmail
                                ) || // parent of logged-in
                                (loggedInParticipant.Participant_Contacts__r || []).some(
                                    c => (c.Email__c || '').toLowerCase() === (p.Email__c || '').toLowerCase()
                                ) // children of logged-in
                            );
                        }

                        // 🛑 Safety fallback
                        else {
                            console.warn('⚠️ Unknown participant type:', safeParticipantType);
                            filteredParticipants = [];
                        }

                        // 🎨 Decorate facility names
                        const finalParticipants = filteredParticipants.map(p => {
                            const activeFaciltyDisplay = (p.Participant_Facilities__r || [])
                                .filter(f => f.Active__c)
                                .map(f => f.Facility__r?.Name)
                                .filter(Boolean)
                                .join(', ');
                        ...
                        console.log('✅ Final Participants:', JSON.stringify(finalParticipants));
